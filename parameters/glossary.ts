...
export const GLOSSARY: Glossary = {
  ...
  _calibrateSoundSimulateLoudspeaker: {
    ...
    default: "",
    explanation:
      '_calibrateSoundSimulateLoudspeaker (default empty) allows you to provide the filename of a CSV or XLSX file specifying a loudspeaker-simulation filter for sound calibration (1000 Hz and All Hz). The filter file itself must be uploaded by the EasyEyes compiler. Either include the filter file with the experiment spreadsheet in a zip file, or feed the filter file to the compiler before feeding the experiment spreadsheet to the compiler. When uploaded in a zip the filter file is retained only for this experiment. When uploaded separately it is retained forever in the "sound files" folder in your Pavlovia account\'s EasyEyesResources repository. The filter file can be an impulse response function (i.e. gain vs. time) OR a frequency response (i.e. gain vs. frequency). NOTE. THE IMPULSE RESPONSE IS CURRENTLY SUPPORTED. THE FREQ RESPONSE IS NOT YET SUPPORTED. The end of the filename (before the .csv or .xlsx extension) must be ".gainVTime" (indicating impuse response) or ".gainVFreq" (indicating frequency response). The compiler will reject any other filename. The gainVFreq file format is the industry standard that we already use to save gain profiles. The gainVTime file format is new. The first row is headings, which must be time and amplitude. The first column is time (sec) and the second column is amplitude. Currently we require that the sampling frequency equal _calibrateSoundSamplingDesiredHz, but later we may allow any sampling frequency and automatically resample at frequency _calibrateSoundSamplingDesiredHz. The stimulated output sound is the input sound convolved with both the loudspeaker and microphone impulse responses. You can only simulate both or neither. Either or both can be the identity (an impulse response with just 1 at time zero). Analyses that isolate the loudspeaker or microphone will convolve with the inverse impulse response (IIR) of the other transducer. COMPILER REQUIRES: Impulse response files must be provided for both loudspeaker and microphone or neither. Filename (before extension) must end in ".gainVTime" or ".gainVFreq".',
  },
  ...
  needTargetSizeDownToDeg: {
    ...
    type: "obsolete",
    default: "",
    explanation: "Use needTargetAsSmallAsDeg instead.",
  },
  notes: {
    name: "notes",
    availability: "now",
