...
export const GLOSSARY: Glossary = {
  ...
  _calibrateSoundBurstScalar_dB: {
    ...
    default: "6.6",
    explanation:
      "_calibrateSoundBurstScalar_dB (default 6.6), if _calibrateSoundBurstUses1000HzGainBool==FALSE, then add this dB offset to the gain at every frequency of the gain profile. Using intuitive names, reported gain level at each frequency is \ngain_dB = scalar_dB + output+dB - input_dB\nUsing actual input parameter names, this is\ngain_dB =_calibrateSoundBurstScalar_dB + output_dB - _calibrateSoundBurstDb",
  },
  ...
  _pavloviaNewExperimentBool: {
    ...
    default: "TRUE",
    explanation:
      "🕑 _pavloviaNewExperimentBool (default TRUE) can be set to FALSE to accommodate users without an institutional Pavlovia license. Without that license, Pavlovia requires assigning credit (money) to each experiment before it can run in RUNNING mode. (PILOT mode is free, but only accessible manually within Pavlovia.) \n     If _pavloviaNewExperimentBool=TRUE (the default), then, when EasyEyes compiles your experiment, EasyEyes appends the smallest possible integer (at least 1) to the spreadsheet filename (without extension) to create a unique (unused) experiment name. That keeps versions apart, and keeps the data from each version in its own repository. \n     However, for users without a site license, Pavlovia requires that credits be assigned in advance to the specific experiment (repo). For them, every time EasyEyes creates a new repo, they must visit Pavlovia to assign credit to it, which is a nuisance. Credit users (i.e. without a site license) can minimize the credit-assignment nuisance by setting _pavloviaNewExperimentBool=FALSE to reuse the old repo, instead of creating a new repo every time they compile. The downside is that if you collect data, edit the experiment, and collect more data, the data files will all be together in the same repo, distinguished only by date.\n     When _pavloviaNewExperimentBool=FALSE, scientists only need to assign credit the first time they compile (when it's a new repo). Once it has credits, they can keep testing, through countless compiles, without visiting Pavlovia, until the experiment runs out of credits. \n     This flag doesn't affect PILOTING mode, which is always free and can only be used from within Pavlovia. \n     Also, any users concerned over the huge proliferation of repos might like to set _pavloviaNewExperimentBool=FALSE to minimize the number of repos created by EasyEyes.\n\nSee _pavloviaPreferRUNNINGModeBool for a more advice on working without an institutional Pavlovia site license.",
  },
