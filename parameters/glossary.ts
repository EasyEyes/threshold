...
export const GLOSSARY: Glossary = {
  ...
  needTargetSizeDownToDeg: {
    ...
    default: "1",
    explanation:
      "🕑 needTargetSizeDownToDeg (default 1 deg) allows you to state the smallest target size that this condition of the experiment will need. This is used solely in the Requirements page, to reject screens that don't have enough pixels. The screen must satisfy each block's needed: needTargetSizeDownToDeg, targetMinPhysicalPx, needScreenHeightDeg, and needScreenWidthDeg. The parameters are combined in each block to compute the screen resolution needed by that block,\nneededScreenHeightPx = targetMinPhysicalPx * needScreenHeightDeg / needTargetSizeDownToDeg\nneededScreenWidthPx = targetMinPhysicalPx * needScreenWidthDeg / needTargetSizeDownToDeg\nThe max of each across all blocks is enforced in the Requirements page. \n\nNote that needTargetSizeDownToDeg is used solely for the resolution requirement (screen width and height in pixels), so you can eliminate the resolution requirement by setting needTargetSizeDownToDeg to a large value, e.g. 10.\n\nFor more details see: needScreenHeightDeg.",
  },
  notes: {
    name: "notes",
    availability: "now",
