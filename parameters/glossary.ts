...
export const GLOSSARY: Glossary = {
  ...
  fontPadding: {
    ...
    default: "0.5",
    explanation:
      "⭑ fontPadding (default 0.5; :warning: exceeding 2 may lose trials to time out) is a positive number specifying how much padding PIXI.js should add around each string to avoid clipping, at the cost of increased risk of timing out when rendering large lacy fonts on slow computers.\n\n1. CLIPPING. Find the smallest value of fontPadding that renders all your font’s characters without clipping. More lacy fonts with long flourishes need more fontPadding to avoid clipping. \nfontPadding=0 is enough to render Roboto Mono (from Google) without clipping.\nfontPadding=0.5 is enough for  Catalina, Burgues, and Arabic.\nfontPadding=?? is enough for Edwardian (from Google).\n\n2. TIMEOUTS. fontPadding below 2 may run without timeouts. For values of 2 or more, follow the assessment advice below. You may have to reduce fontPadding, accepting some clipping, to minimize the loss of trials (on slower computers). If possible, switch to a faster font, i.e. one that times out less. \nUsing a fast computer to draw a large lacy font with:\nfontPadding ≤ 2 no timeouts.\nfontPadding = 3, ⚠ most trials time out due to lateness or wrong duration. Roughly half the stimuli are unacceptably late and roughly half have unacceptably short or long duration. \nSlower computer will tolerate less fontPadding of large lacy fonts. \n\nASSESSMENT. For your chosen font and fontPadding, at the largest size you need, assess timeouts by plotting histograms of targetMeasuredLatenessSec and targetMeasuredDurationSec from the CSV results file. To analyze crashes use _logFontBool, which saves the details of every font rendering, so you can examine the last one before the crash. The maximum safe values will be lower for slower computers, which are common online. Use thresholdAllowedDurationRatio and thresholdAllowedLatenessSec to specify what's acceptable.",
  },
  fontRenderMaxPx: {
    ...
    default: "1.00E+06",
    explanation:
      "⚠ fontRenderMaxPx (default 1e6) uses faster coarser rendering when nominal font size (in px) exceeds fontRenderMaxPx. Normally, EasyEyes uses PIXI.font, but when the size exceeds fontRenderMaxPx, then EasyEyes instead uses PIXI.bitmapfont, which we thought would be faster and coarser. In fact, we found no advantage to useing PIXI.bitmapfont, so we don't recommend using it. We keep this here solely for future testing of text rendering. \n     fontRenderMaxPx does not affect the displayed size, only speed and resolution are affected. The complementary fontMaxPx imposes an upper limit on the font size. When QUEST requests a size bigger than fontMaxPx, EasyEyes uses size fontMaxPx. ",
  },
  fontSource: {
    name: "fontSource",
    availability: "now",
