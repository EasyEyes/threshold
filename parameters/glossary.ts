...
export const GLOSSARY: Glossary = {
  ...
  showTargetSpecsBool: {
    ...
    default: "FALSE",
    explanation:
      "showTargetSpecsBool (default FALSE). For debugging. If TRUE, showTargetSpecsBool (default FALSE) displays various target parameters, including size and spacing, in lower left corner, similar to the trial/block counter. We have several text messages that stack up in the lower left corner. If all four are present, then showText on top, above showConditionNameBool, above showExperimentNameBool, above showTargetSpecsBool.",
  },
  showText: {
    name: "showText",
    availability: "now",
  ...
  targetMinPhysicalPx: {
    ...
    default: "8",
    explanation:
      "targetMinPhysicalPx (default 8) specifies the minimum target size, measured in (small) physical pixels, in the direction specified by targetSizeIsHeightBool. Set this big enough to guarantee enough resolution for decent rendering of this condition’s target. \n\ntargetMinPhysicalPx is measured in (small) physical px, whereas fontMaxPx is measured in (big) CSS px. As a measure of length, window.devicePixelRatio physical px is one CSS px.\n\nIn the modern era of high-DPI displays, object size and screen resolution are usually specified in CSS px. A CSS pixel is software concept. A physical pixel is a uniform glowing square in the screen. As a measure of length, each CSS px is a number of physical px, given by window.devicePixelRatio, which, among current displays, may be 1, 1.5, 2, 3, or 4. \n\nIt seems likely that the Sloan font would be rendered well with targetMinPhysicalPx=5, but I haven't tested it.\n\nAlso see fontMaxPx, which is measured in (big) CSS pixels.",
  },
  targetN: {
    name: "targetN",
    availability: "now",
