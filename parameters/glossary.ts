...
export const GLOSSARY: Glossary = {
  ...
  _needTimingToleranceSec: {
    ...
    default: "0.05",
    explanation:
      "🕑 _needTimingToleranceSec (default 0.05) is the largest acceptable RMS error in generating a 0.15-second interval. We suspect that this depends on both the CPU speed and the number of processes being timeshared, and thus can be reduced by closing other browser windows, and quitting other apps. In practice, we discovered that requiring 6 cpu cores (_needProcessorCoresMinimum) has eliminated bad timing. (Currently our experiments reject about as many participants as they accept. I suspect this is due to the cores requirement. We currently have no data on participants rejected by the Needs page. We're changing that to get data on the rejected computers, so we can figure out why they're rejected.) We prefer that solution because the participant can understand in a study description that we need a certain number of cores (and can check themselves), whereas they can't check timing themselves. We try to minimize the number of participants that we turn away at the Nees page (to not waste their time, and to avoid creating a situation where participants try to work around the Needs page limits), so it's better to require things that allow participants to assess elibility themselves, before reaching the Needs page.",
  },
  _needWeb: {
    name: "_needWeb",
    availability: "now",
  ...
  _pavlovia_Database_ResultsFormatBool: {
    ...
    default: "TRUE",
    explanation:
      "🕑 _pavlovia_Database_ResultsFormatBool (default TRUE) allows the scientist to select which results format Pavlovia will use for this experiment. After the participants run the experiment, Pavlovia's compact \"Database\" results format provides one merged CSV file with all participant results. The alternative is the \"CSV\" results format and returns one CSV file for each participant. \n\nWORK AROUND PAVLOVIA'S BROKEN CSV RESULTS FORMAT: This parameter is being implemented April 4, 2024. At this time, Pavlovia’s CSV results format is broken, so that in columns after the 37th, the headers are one row down and one column over. Pavlovia’s Database results format is fine. So we are setting the default to TRUE, to use the working results format. The EasyEyes “Download results” button has been enhanced to download either kind of CSV file, as appropriate, and split any “Download”-style merged CSV into individual CSV files, practically equivalent to the CSV files that were returned by the “CSV”-mode when it worked properly.\n\nMANUAL OVERRIDE. We can't think of a reason to do it, but the scientist could use the manual control in the Pavlovia dashboard to change the experiment's results format, overriding whatever was selected in the experiment spreadsheet.\n\nIMPLEMENTATION. As the experiment is compiled (into a newly created repository in Pavlovia bearing the experiment's name), the compiler will use the current value (default or assigned) of _pavlovia_Database_ResultsFormatBool to set the experiment’s Results Format in the experiment's Pavlovia dashboard to either “CSV” or “Database”.",
  },
