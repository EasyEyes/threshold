...
export const GLOSSARY: GlossaryFullItem[] = [
  ...
  },
  {
    ...
    example: "A",
    explanation:
      '⏳ blockShuffleSubsubgroups (default is empty cell) allows the scientist to collect contiguous blocks (and their conditions) by giving them the same "subsubgroup name" (an alphanumeric string), and requests shuffling the order of those subsubgroups within whatever subgroup they\'re in. The containing subgroups are defined by blockShuffleSubgroups. The subsubgroups will have various numbers of conditions and blocks. Order follows the left-to-right positions of the columns. Within each containing subgroup, EasyEyes counts the subsubgroups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th subsubgroup by the subsubgroup whose index is i-th in the shuffled list. \n\nA "subsubgroup" is defined by assigning the same subsubgroup name to all the conditions in several contiguous blocks.  One containing subgroup (defined by blockShuffleSubgroup) can contain many subsubgroups, each with a different name, and not all blocks in the subgroup need be in subsubgroups. Each cell in the blockShuffleSubsubgroup row must be empty or have a subsubgroup name, all the cells in a block must be the same, and all the blocks in a subsubgroup must be contiguous and have the same subsubgroup name. \n\nUse of blockShuffleSubsubgroups requires use of blockShuffleSubgroups as well. Subsubgroup  shuffling is performed before subgroup shuffling.\n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "text",
    default: "",
    categories: "",
  },
