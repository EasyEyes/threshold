...
export const GLOSSARY: GlossaryFullItem[] = [
  ...
  },
  {
    ...
    example: "",
    explanation:
      "fontMaxPxShrinkage (default 0.9) reduces the condition's fontMaxPx after a bad text stimulus (i.e. with disallowed duration, lateness, or a fully-black bounding box), so that future trials of this condition will eventually reach a safe text size. All three issues—disallowed duration or lateness, or an all-black bounding box — occur only when rendering very large letters and are cured by reducing fontMaxPx, but the largest safe size depends on the font, the set of characters, the computer's rendering speed, and the amount of heap space provided by the browser running the web app. Specifically, EasyEyes sets fontMaxPx to the product of fontMaxPxShrinkage and the nominal font size (in px) of the failed text stimulus. Since the current font size is always less than or equal to fontMaxPx, the reductions are cumulative — reducing fontMaxPx further and further. The Glossary's large default value for fontMaxPx is chosen to be typically safe for most fonts on most computers. When EasyEyes detects that a bad text stimulus was presented, it does not pass the trial to Quest, but that's not enough because, without input, Quest will suggest the same stimulus strength on the next trial, and since this trial failed, very likely the next will too, which could result in wasting the rest of the trials in the block, presenting every one at the same overly large size. fontMaxPxShrinkage fixs the problem, over several failed trials, by successively reducing fontMaxPx until we reach a safe value.\nAlso see fontMaxPx, thresholdAllowedLatenessSec and thresholdAllowedDurationRatio. Detection of an all-black bounding box will be implemented soon.",
    type: "numerical",
    default: "0.9",
    categories: "",
  },
