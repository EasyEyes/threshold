...
export const GLOSSARY: GlossaryFullItem[] = [
  ...
  },
  {
    ...
    example: "A",
    explanation:
      'blockShuffleGroups1 (default is empty) allows the scientist to join one or more contiguous blocks by giving all the included conditions the same "group name" (an alphanumeric string), and requests shuffling the order of those groups within the experiment.\n\nFor any N in the range 1 to 4, a group is defined by putting the same group name in all the conditions in several contiguous whole blocks in a blockShuffleGroups(N) row. blockShuffleGroups1 can be used alone. For any N>1, each group defined in the blockShuffleGroups(N) row must be a subset of a containing group defined in a blockShuffleGroups(N-1) row.\n\nThe groups in blockShuffleGroups(N) will have various numbers of conditions and blocks. Order follows the left-to-right positions of the columns.  Not all blocks need be in groups. Within each containing group, EasyEyes counts the groups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th group by the group whose index is i-th in the shuffled list.\n\nblockShuffleGroups(N) are shuffled before blockShuffleGroups(N-1). Shuffling preserves the total number of blocks and conditions. To ease analysis, despite shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.\n\nThus, each cell in any blockShuffleGroups(N) row must be empty or have a group name. All the cells in a block must be the same. All the blocks in a group must be contiguous, and have the same group name. ',
    type: "text",
    default: "",
    categories: "",
  },
