...
export const GLOSSARY: GlossaryFullItem[] = [
  ...
  },
  {
    ...
    example: "A",
    explanation:
      '❌block_shuffleBlocksWithinGroups (default is empty cell) allows the scientist to group blocks (and super blocks, see block_glue) by assigning them the same group name (an alphanumeric string), and requests shuffling the order of blocks (and super blocks) within each group. \n\n❌A "group" is defined by assigning the same group name to all the conditions in several contiguous blocks.  There can be many groups, each with a different name. Each cell in this row must be empty or have a group name, all the cells in a block must be the same (either empty or same group name), and all the blocks in a group must be contiguous and have the same group name. Each super block must be entirely inside or outside the group. These group names are used solely by block_shuffleBlocksWithinGroups. \n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite block and group shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants',
    type: "text",
    default: "",
    categories: "",
  },
  {
    ...
    example: "A",
    explanation:
      '❌block_shuffleGroups (default is empty cell) allows the scientist to group blocks (and their conditions) by giving them the same group name (an alphanumeric string), and requests shuffling the order of those groups in the experiment. The groups will have various numbers of conditions and blocks. Order is expressed by the left-to-right position of the columns. EasyEyes counts the groups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th group by the group whose index is i-th in the shuffled list. \n\nA "group" is defined by assigning the same group name to all the conditions in several contiguous blocks.  There can be many groups, each with a different name. Each cell in this row must be empty or have a group name, all the cells in a block must be the same, and all the blocks in a group must be contiguous and have the same group name. \n\nWhen both block_shuffleGroups and block_shuffleSubgroups parameters are present, subgroup  shuffling is performed before group shuffling.\n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite block and group shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "text",
    default: "",
    categories: "",
  },
  {
    ...
    example: "A",
    explanation:
      '⏳ blockShuffleGroups (default is empty cell) allows the scientist to collect contiguous blocks (and their conditions) by giving them the same "group name" (an alphanumeric string), and requests shuffling the order of those groups in the experiment. The groups will have various numbers of conditions and blocks. Order follows the left-to-right positions of the columns. EasyEyes counts the groups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th group by the group whose index is i-th in the shuffled list. \n\nA "group" is defined by assigning the same group name to all the conditions in several contiguous blocks. There can be many groups, each with a different name. Each cell in the blockShuffleGroups row must be empty or have a group name, all the cells in a block must be the same, and all the blocks in a group must be contiguous and have the same group name. \n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "text",
    default: "",
    categories: "",
  },
  {
    ...
    example: "A",
    explanation:
      '⏳ blockShuffleSubgroups (default is empty cell) allows the scientist to collect contiguous blocks (and their conditions) by giving them the same "subgroup name" (an alphanumeric string), and requests shuffling the order of those subgroups within whatever group they\'re in. The containing groups are defined by blockShuffleGroups. The subgroups will have various numbers of conditions and blocks. Order follows the left-to-right positions of the columns.  Within each containing group, EasyEyes counts the subgroups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th subgroup by the subgroup whose index is i-th in the shuffled list.\n\nA "subgroup" is defined by assigning the same subgroup name to all the conditions in several contiguous blocks.  One containing group (defined by blockShuffleGroup) can contain many subgroups, each with a different name, and not all blocks in the group need be in subgroups.\nEach cell in the blockShuffleSubgroups row must be empty or have a subgroup name, all the cells in a block must be the same, and all the blocks in a subgroup must be contiguous and have the same subgroup name. \n\nUse of blockShuffleSubgroups requires use of blockShuffleGroups as well. Subgroup  shuffling is performed before group shuffling.\n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "text",
    default: "",
    categories: "",
  },
  {
    ...
    example: "A",
    explanation:
      '⏳ blockShuffleSub2groups (default is empty cell) allows the scientist to collect contiguous blocks (and their conditions) by giving them the same "sub2group name" (an alphanumeric string), and requests shuffling the order of those sub2groups within whatever subgroup they\'re in. The containing subgroups are defined by blockShuffleSubgroups. The sub2groups will have various numbers of conditions and blocks. Order follows the left-to-right positions of the columns. Within each containing subgroup, EasyEyes counts the sub2groups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th sub2group by the sub2group whose index is i-th in the shuffled list. \n\nA "sub2group" is defined by assigning the same sub2group name to all the conditions in several contiguous blocks.  One containing subgroup (defined by blockShuffleSubgroup) can contain many sub2groups, each with a different name, and not all blocks in the subgroup need be in sub2groups. Each cell in the blockShuffleSub2group row must be empty or have a sub2group name, all the cells in a block must be the same, and all the blocks in a sub2group must be contiguous. \n\nUse of blockShuffleSub2groups requires use of blockShuffleSubgroups as well. Sub2group  shuffling is performed before subgroup shuffling.\n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "text",
    default: "",
    categories: "",
  },
  {
    ...
    example: "A",
    explanation:
      '⏳ blockShuffleSub3groups (default is empty cell) allows the scientist to collect contiguous blocks (and their conditions) by giving them the same "sub3group name" (an alphanumeric string), and requests shuffling the order of those sub3groups within whatever sub2group they\'re in. The containing sub2groups are defined by blockShuffleSub2groups. The sub2groups will have various numbers of conditions and blocks. Order follows the left-to-right positions of the columns. Within each containing sub2group, EasyEyes counts the sub3groups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th sub3group by the sub3group whose index is i-th in the shuffled list. \n\nA "sub3group" is defined by assigning the same sub3group name to all the conditions in several contiguous blocks.  One containing sub2group (defined by blockShuffleSub2group) can contain many sub3groups, each with a different name, and not all blocks in the sub2group need be in sub3groups. Each cell in the blockShuffleSub3group row must be empty or have a sub3group name, all the cells in a block must be the same, and all the blocks in a sub3group must be contiguous. \n\nUse of blockShuffleSub3groups requires use of blockShuffleSub2groups as well. Sub3group  shuffling is performed before sub2group shuffling.\n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "text",
    default: "",
    categories: "",
  },
