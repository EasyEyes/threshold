...
export const GLOSSARY: GlossaryFullItem[] = [
  ...
  },
  {
    ...
    example: "",
    explanation:
      'markFlies, until the target appears, display a swarm of moving "flies" (each like a crosshair) that make it hard to track the real moving crosshair with the cursor unless your eye is on the crosshair. The flies are confined to a circular area with radius radiusDeg centered on either the actual crosshair (which might move) or the nominal crosshair (which is static). Each fly moves a fixed radial distance on each frame, set by the speed degPerSec. The direction of each step is random. Any fly whose center is more than radiusDeg from the circle\'s center disappears and is replaced by a new fly at a random location in the circle.  markFlies accepts several arguments as comma separated values:\nnumber, radiusDeg, degPerSec, centeredOnNominalFixationBool, thicknessDeg, lengthDeg, colorRGBA\nnumber (default 10) is the number of flies.\nradiusDeg (default 2) is the radius of the circular area that the flies are confined to.\ncenteredOnNominalFixationBool (default TRUE) centered on, if TRUE, the nominal fixation location (that the actual crosshair circles around), otherwise centered on the actual moving crosshair.\nthicknessDeg (default 0.03) is the line thickness.\nlengthDeg (default 1) is the length of each of the two lines that make one "fly".\ncolorRGBA has same rules as targetColorRGBA. "0,0,0,1" is black, "1,1,1,1" is white; "1,0,0,1" to red. Last number is alpha, the weight assigned to this color (as opposed to what\'s behind it).',
    type: "text",
