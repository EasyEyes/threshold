...
export const GLOSSARY: GlossaryFullItem[] = [
  ...
  },
  {
    ...
    example: "",
    explanation:
      'block_shuffleSubgroups (default is empty cell) allows the scientist to glue blocks (and their conditions) by giving them the same subgroup name (an alphanumeric string), and requests shuffling the order of those subgroups within whatever group they\'re in. The groups are defined by block_shuffleGroups. The subgroups will have various numbers of conditions and blocks. Order is expressed by the left-to-right position of the columns. Within each group, EasyEyes counts the subgroups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th subgroup by the subgroup whose index is i-th in the shuffled list. \n\nA "subgroup" is defined by assigning the same subgroup name to all the conditions in several contiguous blocks.  One group (defined by block_shuffleGroup) can contain many subgroups, each with a different name. Each cell in this row must be empty or have a subgroup name, all the cells in a block must be the same, and all the blocks in a subgroup must be contiguous and have the same subgroup name. \n\nUse of block_shuffleSubgroups, requires use of block_shuffleGroups as well. When both a present, subgroup  shuffling is performed before group shuffling.\n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite block and group shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "",
    default: "",
    categories: "",
  },
  {
    ...
    example: "",
    explanation:
      'block_shuffleSubsubgroups (default is empty cell) allows the scientist to glue blocks (and their conditions) by giving them the same subsubgroup name (an alphanumeric string), and requests shuffling the order of those subsubgroups within whatever subgroup they\'re in. The subgroups are defined by block_shuffleSubgroups. The subsubgroups will have various numbers of conditions and blocks. Order is expressed by the left-to-right position of the columns. Within each subgroup, EasyEyes counts the subsubgroups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th subsubgroup by the subsubgroup whose index is i-th in the shuffled list. \n\nA "subsubgroup" is defined by assigning the same subsubgroup name to all the conditions in several contiguous blocks.  One subgroup (defined by block_shuffleSubgroup) can contain many subsubgroups, each with a different name. Each cell in this row must be empty or have a subsubgroup name, all the cells in a block must be the same, and all the blocks in a subsubgroup must be contiguous and have the same subsubgroup name. \n\nUse of block_shuffleSubsubgroups, requires use of block_shuffleSubgroups as well. When both a present, subsubgroup  shuffling is performed before subgroup shuffling.\n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite block and group shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "",
    default: "",
    categories: "",
  },
