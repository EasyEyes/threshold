...
export const GLOSSARY: GlossaryFullItem[] = [
  ...
  },
  {
    ...
    example: "",
    explanation:
      "If _languageSelectionByParticipantBool is TRUE (default TRUE), then there will be at least one opportunity for the participant to use a pull-down menu to select any language for the rest of the experiment. (If FALSE then the language is set by _language for the whole experiment.) The participant will select among the native names of the languages, e.g. English, Deutsch, عربي. We currently offer 28 languages, and it would be very easy to add more. If there's demand, we could add another parameter to specify a list of languages to offer to the participant.",
    type: "boolean",
    default: "TRUE",
    categories: "",
  },
  ...
  },
  {
    ...
    example: "3",
    explanation:
      "targetRepeatsMaxLines (default 3) can be 1, 2, 3, … . This is relevant only when targetKind=repeatedLetters. targetRepeatsMaxLines specifies the desired number of lines, but fewer lines may be displayed if limited by screen height. We recommend 3. Small children are alarmed by the repeatedLetters display if there are many lines, and this alarm is minimized by using only 1 or 3 lines. If there is more than one line, then the line spacing (baseline to baseline) is the product of spacingOverSizeRatio and the height of the bounding box of fontCharacterSet.",
    type: "numerical",
    default: "3",
    categories: "",
  },
