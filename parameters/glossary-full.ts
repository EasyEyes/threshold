...
export const GLOSSARY: GlossaryFullItem[] = [
  ...
  },
  {
    ...
    example: "A",
    explanation:
      '⏳ blockShuffleGroups2 (default is empty cell) allows the scientist to collect contiguous blocks (and their conditions) by giving them the same "subgroup name" (an alphanumeric string), and requests shuffling the order of those subgroups within whatever group they\'re in. The containing groups are defined by blockShuffleGroups1. The subgroups will have various numbers of conditions and blocks. Order follows the left-to-right positions of the columns.  Within each containing group, EasyEyes counts the subgroups (from left to right), creates a shuffled list of the numbers from 1 to n, where n is the total count, and then replaces the i-th subgroup by the subgroup whose index is i-th in the shuffled list.\n\nA "subgroup" is defined by assigning the same subgroup name to all the conditions in several contiguous blocks.  One containing group (defined by blockShuffleGroups1) can contain many subgroups, each with a different name, and not all blocks in the group need be in subgroups. Each cell in the blockShuffleGroups 2row must be empty or have a subgroup name, all the cells in a block must be the same, and all the blocks in a subgroup must be contiguous and have the same subgroup name. \n\nUse of blockShuffleGroups2 requires use of blockShuffleGroups1 as well. Subgroup  shuffling is performed before group shuffling.\n\nShuffling preserves the total number of blocks and conditions. To ease analysis, despite shuffling, each block retains its original block number in the CSV results file. Blocks are performed and reported in the shuffled column order, left to right, so in the CSV results, the block number sequence will be nonmonotonic and will vary across participants.',
    type: "text",
    default: "",
    categories: "",
  },
