...
(() => {
    ...
    while (true) {
      ...
      try {
        ...
        if (_RETRYABLE_STATUSES.has(status)) {
          ...
          const delay = retryAfterHeader !== null ? parseFloat(retryAfterHeader) * 1e3 : getRetryDelayMs(attempt);
          attempt++;
          console.warn(
  ...
  // tests/mitmproxy/fixture/harness.ts
  window._retryablePavloviaPost = _retryablePavloviaPost;
  var url = window.FIXTURE_URL ?? new URLSearchParams(location.search).get("url") ?? "";
  if (url) {
    const statusEl = document.getElementById("status");
    const resultEl = document.getElementById("result");
    statusEl.textContent = "Saving your results, please wait\u2026";
    _retryablePavloviaPost(url, { key: "data", value: "test" }).then((resp) => {
