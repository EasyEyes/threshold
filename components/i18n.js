...
export const phrases = {
  ...
  RC_backToExperimentWindow: {
    ...
    ur: 'مرکزی تجربہ ونڈو میں پیشرفت ظاہر ہوتی ہے۔ براہ کرم اب وہاں جائیں ، اوپر "ایزیئس تجربہ" ٹیب پر کلک کرکے (یا آرک براؤزر میں بائیں طرف)۔',
  ...
  RC_viewingDistanceIntroLiMethod: {
    ...
    he: "הֶסבֵּר. לשלב זה, התקרב, במידת הצורך, כך שאתה לא רחוק יותר מרוחב המסך. לכל עין יש כתב עיוור. נמצא את שלך כדי לגלות כמה רחוק העין שלך מהמסך. תתבקש לסגור או לכסות עין אחת כדי לבודד את השנייה. בזמן שאתה מתמקד בשערות הצלב, צפה בנקודה המהבהבת מזווית העין שלך, ופגע בחזרה כשהוא נעלם לתוך העיוור שלך. תלוי במרחק הצפייה ובגודל המסך, הנקודה המהבהבת בדרך כלל תגיע לספוט העיוור שלך לפני שהוא יגיע לאמצע המסך. תעשה זאת עם כל עין, לסירוגין, עד ששתי המדידות האחרונות יהיו עקביות. אנא שמור על מרחק הצפייה שלך כשאתה בודק את שתי העיניים. הערה. אם הנקודה לעולם אינה נעלמת, זה אומר שהעין שלך עוקבת אחר הנקודה, במקום להישאר על שערות הצלב. מבחן זה צריך שתפקח עין על שערות הצלב תוך כדי פיקוח על הנקודה המהבהבת בפינת העין שלך. זה קשה בהתחלה, אבל הופך להיות קל יותר עם תרגול.",
    ...
    ko: "설명. 이 단계에서는 필요한 경우 더 가까워 지므로 화면 너비의 두 배 이상이 아닙니다. 모든 눈에는 사각 지대가 있습니다. 우리는 당신의 눈이 화면에서 얼마나 멀리 떨어져 있는지 알게 될 것입니다. 다른 눈을 닫거나 덮으려면 다른 눈을 분리해야합니다. 십자 머리카락에 집중하는 동안 눈 모퉁이에서 깜박 거리는 점을보고, 사각 지대로 사라질 때 귀환하십시오. 볼거리와 화면 크기에 따라 깜박 거리는 점은 일반적으로 화면 중앙에 도달하기 전에 블라인드 스팟에 도달합니다. 마지막 두 측정이 일관되게 될 때까지 번갈아 가며 각 눈 으로이 작업을 수행합니다. 두 눈을 테스트 할 때 시청 거리를 유지하십시오. 메모. 점이 사라지지 않으면, 십자 머리카락에 머무르는 대신 눈이 점을 따르고 있음을 의미합니다. 이 테스트는 눈의 모퉁이에 깜박 거리는 점을 모니터링하면서 십자 머리카락을 주시해야합니다. 처음에는 어렵지만 연습으로 더 쉬워집니다.",
  },
  ...
  T_rsvpReadingTaskBrief: {
    ...
    ta: "இப்போது நீங்கள் 111 சொற்களை விரைவாகப் படிப்பீர்கள். வார்த்தைகள் ஒன்றன்பின் ஒன்றாக, அதே இடத்தில் தோன்றும்.",
