...
export const phrases = {
  ...
  EE_ID: {
    ...
    ja: "セッション全体でデータをリンクするには、一意のIDが必要です。このコンピューターは、以前に** ddd **のセッションに参加しました。それがあなたである場合、OKを押してください。それ以外の場合は、EasyEyes IDファイルがある場合は、下のファイル選択ボタンを押して開きます。 （ファイル名は「easyeyes」で始まり、「.txt」で終わり、通常はダウンロードフォルダーにあります。）ファイルを持っていないが、easyeyes IDを知っている場合は、入力します。それ以外の場合は、独自のIDを構成するだけです。これは、少なくとも5文字の英数字の文字列になります。必要に応じて、それはあなたの名前かもしれません。このコンピューターでは覚えていますが、別のコンピューターにアクセスした場合は、EasyEyes IDファイル（ダウンロードフォルダーの最新のファイル）を使用するか、少なくともEasyEyes IDを書き留めてください。セッションからセッションにデータをリンクするために毎回使用してください。",
  ...
  T_middleLetterDemo: {
    ...
    ur: "ہر ٹیسٹ پر ، جب آپ کراس کے بالوں کو دیکھیں تو ، آپ کی آنکھ کے کونے میں تین بے ترتیب حرفوں کا ایک ٹرپل چمک اٹھے گا۔ صرف ایک جھلک کام کو جاننے کے لئے ، مستحکم کارکردگی کے ساتھ پہلے اسے آزمائیں۔ جب آپ نیچے کراس بال کو دیکھیں تو ، آنکھیں ہلائے بغیر ، درمیانی خط کی شناخت کرنے کی کوشش کریں۔",
  ...
  T_thresholdRsvpReadingBeginBlock: {
    ...
    pt: "Você está prestes a iniciar um bloco de testes xxx. Cada julgamento mostrará rapidamente palavras, uma após a outra, e pede que você relate o que lê",
