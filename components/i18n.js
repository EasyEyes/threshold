/*
  Do not modify this file! Run npm `npm run phrases` at ROOT of this project to fetch from the Google Sheets.
  https://docs.google.com/spreadsheets/d/1UFfNikfLuo8bSromE34uWDuJrMPFiJG3VpoQKdCGkII/edit#gid=0
*/

export const phrases = {
  EE_languageDirection: {
    "en-US": "LTR",
    de: "LTR",
    fr: "LTR",
    es: "LTR",
    pt: "LTR",
    it: "LTR",
    ro: "LTR",
    pl: "LTR",
    ru: "LTR",
    hy: "LTR",
    fi: "LTR",
    ka: "LTR",
    he: "RTL",
    ar: "RTL",
    ur: "RTL",
    hi: "LTR",
    ta: "LTR",
    ml: "LTR",
    te: "LTR",
    kn: "LTR",
    bn: "LTR",
    id: "LTR",
    "zh-CN": "LTR",
    "zh-HK": "LTR",
    ja: "LTR",
    ko: "LTR",
  },
  EE_languageUseSpace: {
    "en-US": "1",
    de: "1",
    fr: "1",
    es: "1",
    pt: "1",
    it: "1",
    ro: "1",
    pl: "1",
    ru: "1",
    hy: "1",
    fi: "1",
    ka: "1",
    he: "1",
    ar: "1",
    ur: "1",
    hi: "1",
    ta: "1",
    ml: "1",
    te: "1",
    kn: "1",
    bn: "1",
    id: "1",
    "zh-CN": "0",
    "zh-HK": "0",
    ja: "0",
    ko: "0",
  },
  EE_languageFont: {
    "en-US": "Noto Serif",
    de: "Noto Serif",
    fr: "Noto Serif",
    es: "Noto Serif",
    pt: "Noto Serif",
    it: "Noto Serif",
    ro: "Noto Serif",
    pl: "Noto Serif",
    ru: "Noto Serif",
    hy: "Noto Serif Armenian",
    fi: "Noto Serif",
    ka: "Noto Serif",
    he: "Noto Serif Hebrew",
    ar: "Noto Kufi Arabic",
    ur: "Noto Nastaliq Urdu",
    hi: "Noto Serif Devanagari ",
    ta: "Noto Serif Tamil",
    ml: "Noto Serif Malayalam",
    te: "Noto Serif Telugu",
    kn: "Noto Serif Kannada",
    bn: "Noto Serif Bengali",
    id: "Noto Serif",
    "zh-CN": "Noto Serif SC",
    "zh-HK": "Noto Serif TC",
    ja: "Noto Serif JP",
    ko: "Noto Serif KR",
  },
  EE_languageNameEnglish: {
    "en-US": "English",
    de: "German",
    fr: "French",
    es: "Spanish",
    pt: "Portuguese",
    it: "Italian",
    ro: "Romanian",
    pl: "Polish",
    ru: "Russian",
    hy: "Armenian",
    fi: "Finnish",
    ka: "Georgian",
    he: "Hebrew",
    ar: "Arabic",
    ur: "Urdu",
    hi: "Hindi",
    ta: "Tamil",
    ml: "Malayalam",
    te: "Telugu",
    kn: "Kannada",
    bn: "Bangla",
    id: "Indonesian",
    "zh-CN": "Simplified Chinese",
    "zh-HK": "Traditional Chinese",
    ja: "Japanese",
    ko: "Korean",
  },
  EE_languageNameNative: {
    "en-US": "English",
    de: "Deutsch",
    fr: "Français",
    es: "Español",
    pt: "Português",
    it: "Italiano",
    ro: "Română",
    pl: "Polski",
    ru: "Русский",
    hy: "հայերեն",
    fi: "Suomalainen",
    ka: "ქართული",
    he: "עִברִית",
    ar: "عربي",
    ur: "اردو",
    hi: "हिंदी",
    ta: "தமிழ்",
    ml: "മലയാളം",
    te: "తెలుగు",
    kn: "ಕನ್ನಡ",
    bn: "বাংলা",
    id: "bahasa Indonesia",
    "zh-CN": "简体中文",
    "zh-HK": "繁體中文",
    ja: "日本",
    ko: "한국인",
  },
  EE_languageChoose: {
    "en-US": "Choose a language for this study.",
    de: "Wählen Sie eine Sprache für diese Studie.",
    fr: "Choisissez une langue pour cette étude.",
    es: "Elija un idioma para este estudio.",
    pt: "Escolha um idioma para este estudo.",
    it: "Scegli una lingua per questo studio.",
    ro: "Alegeți o limbă pentru acest studiu.",
    pl: "Wybierz język do tego badania.",
    ru: "Выберите язык для этого исследования.",
    hy: "Ընտրեք լեզու այս ուսումնասիրության համար:",
    fi: "Valitse kieli tähän tutkimukseen.",
    ka: "შეარჩიეთ ენა ამ კვლევისთვის.",
    he: "בחר שפה למחקר זה.",
    ar: "اختر لغة لهذه الدراسة.",
    ur: "اس مطالعے کے لئے ایک زبان کا انتخاب کریں۔",
    hi: "इस अध्ययन के लिए एक भाषा चुनें।",
    ta: "இந்த ஆய்வுக்கு ஒரு மொழியைத் தேர்வுசெய்க.",
    ml: "ഈ പഠനത്തിനായി ഒരു ഭാഷ തിരഞ്ഞെടുക്കുക.",
    te: "ఈ అధ్యయనం కోసం ఒక భాషను ఎంచుకోండి.",
    kn: "ಈ ಅಧ್ಯಯನಕ್ಕಾಗಿ ಭಾಷೆಯನ್ನು ಆರಿಸಿ.",
    bn: "এই অধ্যয়নের জন্য একটি ভাষা চয়ন করুন।",
    id: "Pilih bahasa untuk penelitian ini.",
    "zh-CN": "为这项研究选择一种语言。",
    "zh-HK": "為這項研究選擇一種語言。",
    ja: "この調査の言語を選択してください。",
    ko: "이 연구에 대한 언어를 선택하십시오.",
  },
  EE_phraseSource: {
    "en-US":
      "Denis Pelli & Peiling Jiang 2021.10.10; 2021.10.14; 2021.10.19; Marialuisa Martelli 2021.10.21",
    de: "Google Translate from English; Denis Pelli 2021.10.19 improved translation of trial and beep.",
    fr: "Google Translate from Spanish, Italian, English, and German; Denis Pelli 2021.10.19",
    es: "Google Translate from English; Denis Pelli 2021.10.15; Maria Pombo 2021.10.24",
    pt: "Google Translate from Spanish",
    it: "Google Translate from English; Marialuisa Martelli 2021.10.21",
    ro: "Google Translate from Spanish",
    pl: "Google Translate from English; Jan Kurzawski 2021.10.20",
    ru: "Google Translate from English; Elena Sizikova 2021.12.14",
    hy: "Google Translate from English",
    fi: "Google Translate from English",
    ka: "Google Translate from English",
    he: "Google Translate from English",
    ar: "Google Translate from English; Marianne Azar 2021.10.7",
    ur: "Google Translate from Hindi",
    hi: "Google Translate from English; Mrugank Dake and Ajay Subramanian 2021.10.14",
    ta: "Google Translate from English",
    ml: "Google Translate from English",
    te: "Google Translate from English",
    kn: "Google Translate from English",
    bn: "Google Translate from English",
    id: "Google Translate from English",
    "zh-CN": "Google Translate from English; Elizabeth Zhou 2022.1.6",
    "zh-HK": "Google Translate from Simplified Chinese",
    ja: "Google Translate from English",
    ko: "Google Translate from English",
  },
  EE_compatibilityTitle: {
    "en-US": "EasyEyes Compatibility",
    de: "Easyeyes Kompatibilität",
    fr: "Compatibilité facile",
    es: "Compatibilidad de EasyEyes",
    pt: "Compatibilidade easyeyes",
    it: "Compatibilità EasyEyes",
    ro: "Compatibilitate ușoară",
    pl: "Kompatybilność Easyeyes",
    ru: "Easyeyes совместимость",
    hy: "EasyEyes համատեղելիությունը",
    fi: "Easyeyes -yhteensopivuus",
    ka: "Easyeyes თავსებადობა",
    he: "תאימות EasyEees",
    ar: "التوافق Easyeyes",
    ur: "ایزیئز مطابقت",
    hi: "Easyeyes संगतता",
    ta: "எளிதான பொருந்தக்கூடிய தன்மை",
    ml: "എളുപ്പമുള്ള അനുയോജ്യത അനുയോജ്യത",
    te: "ఈజీయెస్ అనుకూలత",
    kn: "ಸುಲಭವಾದ ಹೊಂದಾಣಿಕೆ",
    bn: "ইজিওয়াইস সামঞ্জস্যতা",
    id: "Kompatibilitas Eyeseyes",
    "zh-CN": "EasyEyes兼容性",
    "zh-HK": "EasyEyes兼容性",
    ja: "EasyEyes互換性",
    ko: "EasyEyes 호환성",
  },
  EE_compatible: {
    "en-US": "✅ Excellent! This study can proceed. ",
    de: "✅ Ausgezeichnet! Diese Studie kann fortgesetzt werden.",
    fr: "✅ Excellent! Cette étude peut se dérouler.",
    es: "✅ ¡Excelente! Este estudio puede proceder.",
    pt: "✅ Excelente! Este estudo pode prosseguir.",
    it: "✅ Eccellente! Questo studio può procedere.",
    ro: "✅ Excelent! Acest studiu poate continua.",
    pl: "✅ Doskonałe! To badanie może kontynuować.",
    ru: "✅ Отлично! Это исследование может продолжаться.",
    hy: "✅ Գերազանց: Այս ուսումնասիրությունը կարող է ընթանալ:",
    fi: "✅ Erinomainen! Tämä tutkimus voi edetä.",
    ka: "✅ შესანიშნავი! ამ კვლევას შეუძლია გაგრძელდეს.",
    he: "✅ מעולה! מחקר זה יכול להמשיך.",
    ar: "✅ ممتاز! يمكن أن تستمر هذه الدراسة.",
    ur: "✅ عمدہ! یہ مطالعہ آگے بڑھ سکتا ہے۔",
    hi: "✅ बहुत बढ़िया! यह अध्ययन आगे बढ़ सकता है।",
    ta: "✅ சிறந்தது! இந்த ஆய்வு தொடரலாம்.",
    ml: "മികച്ചത്! ഈ പഠനത്തിന് തുടരാം.",
    te: "✅ అద్భుతమైనది! ఈ అధ్యయనం కొనసాగవచ్చు.",
    kn: "✅ ಅತ್ಯುತ್ತಮ! ಈ ಅಧ್ಯಯನವು ಮುಂದುವರಿಯಬಹುದು.",
    bn: "✅ দুর্দান্ত! এই অধ্যয়ন এগিয়ে যেতে পারে।",
    id: "✅ Luar biasa! Studi ini dapat dilanjutkan.",
    "zh-CN": "✅好！这项研究可以进行。",
    "zh-HK": "✅好！這項研究可以進行。",
    ja: "✅素晴らしい！この研究は進むことができます。",
    ko: "✅ 우수! 이 연구는 진행될 수 있습니다.",
  },
  EE_compatibleBrowserDeviceCores: {
    "en-US":
      "As stated in its description, this study is only compatible with the BBB browser running on a DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie nur mit dem BBB -Browser kompatibel, der auf einem DDD mit mindestens 222 CPU -Kernen ausgeführt wird",
    fr: "Comme indiqué dans sa description, cette étude n'est compatible qu'avec le navigateur BBB fonctionnant sur un DDD avec au moins 222 cœurs de CPU",
    es: "Como se indica en su descripción, este estudio solo es compatible con el navegador BBB que se ejecuta en un DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é compatível apenas com o navegador BBB em execução em um DDD com pelo menos 222 núcleos da CPU",
    it: "Come indicato nella sua descrizione, questo studio è compatibile solo con il browser BBB in esecuzione su un DDD con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este compatibil doar cu browserul BBB care rulează pe un DDD cu cel puțin 222 de nuclee de procesor",
    pl: "Jak stwierdzono w opisie, badanie to jest kompatybilne tylko z przeglądarką BBB działającą na DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование совместимо только с браузером BBB, работающим на DDD с не менее 222 ядрами ЦП",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը համատեղելի է միայն BBB զննարկչի հետ, որը գործում է DDD- ով առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus on yhteensopiva vain DDD: llä käytävän BBB -selaimen kanssa vähintään 222 CPU -ytimellä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა მხოლოდ BBB ბრაუზერის მიერ DDD– ზე მუშაობს, მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה תואם רק לדפדפן ה- BBB הפועל על DDD עם לפחות 222 ליבות מעבד",
    ar: "كما ذكر في وصفها ، فإن هذه الدراسة متوافقة فقط مع متصفح BBB الذي يعمل على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ صرف بی بی بی براؤزر کے ساتھ مطابقت رکھتا ہے جس میں کم از کم 222 سی پی یو کور کے ساتھ ڈی ڈی ڈی پر چل رہا ہے۔",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन केवल कम से कम 222 सीपीयू कोर के साथ डीडीडी पर चलने वाले बीबीबी ब्राउज़र के साथ संगत है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு பிபிபி உலாவியுடன் டி.டி.டி.யில் குறைந்தது 222 சிபியு கோர்களுடன் மட்டுமே பொருந்தும்",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം കുറഞ്ഞത് 222 സി.പി.യു കോറുകളുള്ള ഒരു ഡിഡിഡിയിൽ പ്രവർത്തിക്കുന്ന ബിബിബി ബ്ര browser സറുമായി മാത്രമേ പൊരുത്തപ്പെടുകയുള്ളൂ",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం కనీసం 222 సిపియు కోర్లతో డిడిడిలో నడుస్తున్న బిబిబి బ్రౌజర్‌తో మాత్రమే అనుకూలంగా ఉంటుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು ಡಿಡಿಡಿಯಲ್ಲಿ ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಚಾಲನೆಯಲ್ಲಿರುವ ಬಿಬಿಬಿ ಬ್ರೌಸರ್‌ಗೆ ಮಾತ್ರ ಹೊಂದಿಕೊಳ್ಳುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই গবেষণাটি কেবলমাত্র বিবিবি ব্রাউজারের সাথে কমপক্ষে 222 সিপিইউ কোরের সাথে একটি ডিডিডি -তে চলার সাথে সামঞ্জস্যপূর্ণ",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini hanya kompatibel dengan browser BBB yang berjalan pada DDD dengan setidaknya 222 inti CPU",
    "zh-CN":
      "如其描述中所述，这项研究仅与在DDD上运行至少222个CPU内核的BBB浏览器兼容",
    "zh-HK":
      "如其描述中所述，這項研究僅與在DDD上運行至少222個CPU內核的BBB瀏覽器兼容",
    ja: "その説明で述べたように、この研究は、少なくとも222 CPUコアを持つDDDで実行されているBBBブラウザとのみ互換性があります",
    ko: "설명에 언급 된 바와 같이,이 연구는 최소 222 CPU 코어가있는 DDD에서 실행되는 BBB 브라우저와 만 호환됩니다.",
  },
  EE_compatibleBrowserNotOSDeviceCores: {
    "en-US":
      "As stated in its description, this study is incompatible with ooo, and requires the BBB browser running on a DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie mit OOO unvereinbar und erfordert den BBB -Browser, der auf einem DDD mit mindestens 222 CPU -Kernen ausgeführt wird",
    fr: "Comme indiqué dans sa description, cette étude est incompatible avec OOO et nécessite le navigateur BBB fonctionnant sur un DDD avec au moins 222 cœurs CPU",
    es: "Como se indica en su descripción, este estudio es incompatible con OOO y requiere el navegador BBB que se ejecuta en un DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é incompatível com o OOO e requer o navegador BBB em um DDD com pelo menos 222 núcleos da CPU",
    it: "Come affermato nella sua descrizione, questo studio è incompatibile con OOO e richiede il browser BBB in esecuzione su un DDD con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este incompatibil cu OOO și necesită ca browserul BBB să funcționeze pe un DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w opisie, badanie to jest niezgodne z OOO i wymaga przeglądarki BBB działającej na DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование несовместимо с OOO и требует, чтобы браузер BBB работал на DDD с не менее 222 ядрами процессора",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը անհամատեղելի է OOO- ի հետ եւ պահանջում է BBB զննարկիչը, որն աշխատում է DDD- ով `առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus ei ole yhteensopiva OOO: n kanssa ja vaatii BBB",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა შეუთავსებელია OOO- სთან და მოითხოვს BBB ბრაუზერს, რომელიც მუშაობს DDD– ზე, მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה אינו תואם את OOO, ומחייב את דפדפן ה- BBB הפועל על DDD עם לפחות 222 ליבות מעבד",
    ar: "كما هو مذكور في وصفها ، لا تتوافق هذه الدراسة مع OOO ، وتتطلب متصفح BBB الذي يعمل على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ او او او کے ساتھ مطابقت نہیں رکھتا ہے ، اور اس کی ضرورت ہے کہ بی بی بی براؤزر کم از کم 222 سی پی یو کور کے ساتھ ڈی ڈی ڈی پر چل رہا ہو۔",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन OOO के साथ असंगत है, और कम से कम 222 CPU कोर के साथ DDD पर चलने वाले BBB ब्राउज़र की आवश्यकता है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு OOO உடன் பொருந்தாது, மேலும் குறைந்தது 222 CPU கோர்களுடன் ஒரு DDD இல் பிபிபி உலாவி தேவைப்படுகிறது",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം ooo- യുമായി പൊരുത്തപ്പെടുന്നില്ല, കൂടാതെ ബിബിബി ബ്ര browser സർ കുറഞ്ഞത് 222 സി.പി.യു കോറുകളും",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం OOO కి విరుద్ధంగా ఉంది మరియు కనీసం 222 CPU కోర్లతో DDD లో BBB బ్రౌజర్ నడుస్తుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು OOO ಗೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ, ಮತ್ತು ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಡಿಡಿಡಿಯಲ್ಲಿ ಚಾಲನೆಯಲ್ಲಿರುವ ಬಿಬಿಬಿ ಬ್ರೌಸರ್ ಅಗತ್ಯವಿದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই অধ্যয়নটি ওওর সাথে বেমানান, এবং বিবিবি ব্রাউজারটি কমপক্ষে 222 সিপিইউ কোর সহ একটি ডিডিডি -তে চলমান প্রয়োজন",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini tidak sesuai dengan OOO, dan membutuhkan browser BBB yang berjalan pada DDD dengan setidaknya 222 core CPU",
    "zh-CN":
      "如其描述中所述，这项研究与OOO不相容，并且需要在DDD上运行至少222个CPU内核的BBB浏览器",
    "zh-HK":
      "如其描述中所述，這項研究與OOO不相容，並且需要在DDD上運行至少222個CPU內核的BBB瀏覽器",
    ja: "その説明に記載されているように、この研究はOOOと互換性があり、少なくとも222 CPUコアのDDDで実行されるBBBブラウザが必要です",
    ko: "설명에 언급 된 바와 같이,이 연구는 OOO와 호환되지 않으며 최소 222 CPU 코어가있는 DDD에서 BBB 브라우저를 실행해야합니다.",
  },
  EE_compatibleBrowserOSDeviceCores: {
    "en-US":
      "As stated in its description, this study is only compatible with the BBB browser running on a OOO DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie nur mit dem BBB -Browser kompatibel",
    fr: "Comme indiqué dans sa description, cette étude n'est compatible qu'avec le navigateur BBB fonctionnant sur un DDD OOO avec au moins 222 cœurs de CPU",
    es: "Como se indica en su descripción, este estudio solo es compatible con el navegador BBB que se ejecuta en un OOO DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é compatível apenas com o navegador BBB em execução em um DDD OOO com pelo menos 222 núcleos da CPU",
    it: "Come indicato nella sua descrizione, questo studio è compatibile solo con il browser BBB in esecuzione su un DDD OOO con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este compatibil doar cu browserul BBB care rulează pe un OOO DDD cu cel puțin 222 de nuclee CPU",
    pl: "Jak stwierdzono w opisie, badanie to jest kompatybilne tylko z przeglądarką BBB działającą na OOO DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование совместимо только с браузером BBB, работающим на OOO DDD с не менее 222 ядрами ЦП",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը համատեղելի է միայն BBB զննարկչի հետ, որն աշխատում է OOO DDD- ով, առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus on yhteensopiva vain OOO -DDD: llä käyvän BBB -selaimen kanssa, jossa on vähintään 222 CPU -ytimiä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა მხოლოდ BBB ბრაუზერის შეესაბამება OOO DDD– ზე, მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה תואם רק לדפדפן ה- BBB הפועל ב- OOO DDD עם לפחות 222 ליבות מעבד",
    ar: "كما ذكر في وصفها ، فإن هذه الدراسة متوافقة فقط مع متصفح BBB الذي يعمل على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ صرف بی بی بی براؤزر کے ساتھ مطابقت رکھتا ہے جو کم از کم 222 سی پی یو کور کے ساتھ او او ڈی ڈی ڈی پر چل رہا ہے۔",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन केवल कम से कम 222 सीपीयू कोर के साथ एक OOO DDD पर चलने वाले BBB ब्राउज़र के साथ संगत है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு குறைந்தது 222 CPU கோர்களுடன் OOO DDD இல் இயங்கும் பிபிபி உலாவியுடன் மட்டுமே இணக்கமானது",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം കുറഞ്ഞത് 222 സിപിയു കോറുകളുള്ള ഒരു ooo ​​ddd- ൽ പ്രവർത്തിക്കുന്ന BBB ബ്ര browser സറുമായി മാത്രമേ പൊരുത്തപ്പെടുകയുള്ളൂ",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం కనీసం 222 CPU కోర్లతో OOO DDD లో నడుస్తున్న BBB బ్రౌజర్‌తో మాత్రమే అనుకూలంగా ఉంటుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು OOO ಡಿಡಿಡಿಯಲ್ಲಿ ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಚಾಲನೆಯಲ್ಲಿರುವ ಬಿಬಿಬಿ ಬ್ರೌಸರ್‌ಗೆ ಮಾತ್ರ ಹೊಂದಿಕೊಳ್ಳುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই গবেষণাটি কেবলমাত্র বিবিবি ব্রাউজারের সাথে কমপক্ষে 222 সিপিইউ কোরের সাথে একটি ওও ডিডিডিতে চলার সাথে সামঞ্জস্যপূর্ণ",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini hanya kompatibel dengan browser BBB yang berjalan pada OOO DDD dengan setidaknya 222 inti CPU",
    "zh-CN":
      "如其描述中所述，这项研究仅与在OOO DDD上运行至少222个CPU核心的BBB浏览器兼容",
    "zh-HK":
      "如其描述中所述，這項研究僅與在OOO DDD上運行至少222個CPU核心的BBB瀏覽器兼容",
    ja: "その説明で述べたように、この研究は、少なくとも222 CPUコアを使用してOOO DDDで実行されているBBBブラウザとのみ互換性があります",
    ko: "설명에 언급 된 바와 같이,이 연구는 최소 222 CPU 코어가있는 OOO DDD에서 실행되는 BBB 브라우저와 만 호환됩니다.",
  },
  EE_compatibleBrowserVersionDeviceCores: {
    "en-US":
      "As stated in its description, this study is only compatible with the BBB browser (version at least 111) running on a DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie nur mit dem BBB -Browser (Version 111) kompatibel",
    fr: "Comme indiqué dans sa description, cette étude n'est compatible qu'avec le navigateur BBB (version au moins 111) fonctionnant sur un DDD avec au moins 222 cœurs de CPU",
    es: "Como se indica en su descripción, este estudio solo es compatible con el navegador BBB (versión al menos 111) que se ejecuta en un DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é compatível apenas com o navegador BBB (versão pelo menos 111) em execução em um DDD com pelo menos 222 núcleos de CPU",
    it: "Come indicato nella sua descrizione, questo studio è compatibile solo con il browser BBB (versione almeno 111) in esecuzione su un DDD con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este compatibil doar cu browserul BBB (versiunea cel puțin 111) care rulează pe un DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w opisie, badanie to jest kompatybilne tylko z przeglądarką BBB (wersja co najmniej 111) działająca na DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование совместимо только с браузером BBB (версия не менее 111), работающим на DDD с не менее 222 ядрами ЦП",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը համատեղելի է միայն BBB զննարկչի հետ (տարբերակ առնվազն 111), որը գործում է DDD- ով `առնվազն 222 պրոցեսորի միջուկով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus on yhteensopiva vain BBB -selaimen (versio vähintään 111) kanssa, joka toimii DDD: llä, jossa on vähintään 222 CPU -ytimiä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა მხოლოდ BBB ბრაუზერის (ვერსიით მინიმუმ 111) მუშაობს DDD– ზე, მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה תואם רק לדפדפן BBB (גרסה לפחות 111) הפועל על DDD עם לפחות 222 ליבות מעבד",
    ar: "كما هو مذكور في وصفها ، فإن هذه الدراسة متوافقة فقط مع متصفح BBB (الإصدار 111 على الأقل) الذي يعمل على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ صرف بی بی بی براؤزر (کم از کم 111 ورژن) کے ساتھ مطابقت رکھتا ہے جس میں کم از کم 222 سی پی یو کورز کے ساتھ ڈی ڈی ڈی پر چل رہا ہے۔",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन केवल बीबीबी ब्राउज़र (कम से कम 111 संस्करण) के साथ कम से कम 222 सीपीयू कोर के साथ डीडीडी पर चल रहा है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு பிபிபி உலாவி (பதிப்பு குறைந்தது 111) ஒரு டி.டி.டி.யில் குறைந்தது 222 சிபியு கோர்களுடன் இயங்குகிறது",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം ബിബിബി ബ്ര browser സറുമായി (കുറഞ്ഞത് 111 പതിപ്പ്) കുറഞ്ഞത് 222 സി.പി.യു കോറുകളോടെ പ്രവർത്തിക്കുന്നു",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం కనీసం 222 సిపియు కోర్లతో డిడిడిలో నడుస్తున్న బిబిబి బ్రౌజర్ (వెర్షన్ కనీసం 111) తో మాత్రమే అనుకూలంగా ఉంటుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು ಬಿಬಿಬಿ ಬ್ರೌಸರ್ (ಆವೃತ್ತಿ ಕನಿಷ್ಠ 111) ಗೆ ಮಾತ್ರ ಹೊಂದಿಕೊಳ್ಳುತ್ತದೆ ಮತ್ತು ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಡಿಡಿಡಿಯಲ್ಲಿ ಚಾಲನೆಯಲ್ಲಿದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই অধ্যয়নটি কেবলমাত্র বিবিবি ব্রাউজারের সাথে সামঞ্জস্যপূর্ণ (কমপক্ষে ১১১ সংস্করণ) কমপক্ষে ২২২ সিপিইউ কোরের সাথে ডিডিডিতে চলছে",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini hanya kompatibel dengan browser BBB (versi setidaknya 111) yang berjalan pada DDD dengan setidaknya 222 core CPU",
    "zh-CN":
      "如其描述中所述，这项研究仅与在DDD上运行至少222个CPU内核的BBB浏览器（版本至少为111）",
    "zh-HK":
      "如其描述中所述，這項研究僅與在DDD上運行至少222個CPU內核的BBB瀏覽器（版本至少為111）",
    ja: "その説明で述べたように、この研究は、少なくとも222 CPUコアのDDDで実行されているBBBブラウザー（少なくとも111版）とのみ互換性があります。",
    ko: "설명에 언급 된 바와 같이,이 연구는 최소 222 CPU 코어가있는 DDD에서 실행되는 BBB 브라우저 (버전 111) 와만 호환됩니다.",
  },
  EE_compatibleBrowserVersionNotOSDeviceCores: {
    "en-US":
      "As stated in its description, this study is incompatible with ooo, and requires the BBB browser (version at least 111) running on a DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie nicht mit OOO kompatibel und erfordert den BBB -Browser (Version mindestens 111), der auf einem DDD mit mindestens 222 CPU -Kernen ausgeführt wird",
    fr: "Comme indiqué dans sa description, cette étude est incompatible avec OOO et nécessite le navigateur BBB (version au moins 111) fonctionnant sur un DDD avec au moins 222 cœurs CPU",
    es: "Como se indica en su descripción, este estudio es incompatible con OOO y requiere el navegador BBB (versión al menos 111) que se ejecuta en un DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é incompatível com o OOO e requer o navegador BBB (versão pelo menos 111) em execução em um DDD com pelo menos 222 núcleos da CPU",
    it: "Come affermato nella sua descrizione, questo studio è incompatibile con OOO e richiede il browser BBB (versione almeno 111) in esecuzione su un DDD con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este incompatibil cu OOO și necesită browserul BBB (versiunea cel puțin 111) care rulează pe un DDD cu cel puțin 222 de nuclee CPU",
    pl: "Jak stwierdzono w opisie, badanie to jest niezgodne z OOO i wymaga przeglądarki BBB (wersja co najmniej 111) działająca na DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование несовместимо с OOO и требует, чтобы браузер BBB (версия не менее 111) работал на DDD с не менее 222 ядрами ЦП",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը անհամատեղելի է OOO- ի հետ եւ պահանջում է BBB զննարկիչ (տարբերակ առնվազն 111) վազում է DDD- ով `առնվազն 222 պրոցեսորի միջուկով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus ei ole yhteensopiva OOO: n kanssa, ja se vaatii BBB -selaimen (versio vähintään 111), joka toimii DDD: llä vähintään 222 CPU -ytimellä,",
    ka: "როგორც აღწერა აღწერილი, ეს კვლევა შეუთავსებელია OOO- სთან და მოითხოვს BBB ბრაუზერს (ვერსია მინიმუმ 111), რომელიც მუშაობს DDD– ზე მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה אינו תואם את OOO, ומחייב את דפדפן ה- BBB (גרסה לפחות 111) הפועל על DDD עם לפחות 222 ליבות מעבד",
    ar: "كما هو مذكور في وصفها ، لا تتوافق هذه الدراسة مع OOO ، وتتطلب متصفح BBB (الإصدار 111 على الأقل) الذي يعمل على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ او او او کے ساتھ مطابقت نہیں رکھتا ہے ، اور اس کے لئے بی بی بی براؤزر (کم از کم 111 ورژن) کی ضرورت ہے جو کم از کم 222 سی پی یو کور کے ساتھ ڈی ڈی ڈی پر چل رہا ہے۔",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन OOO के साथ असंगत है, और कम से कम 222 CPU कोर के साथ DDD पर चल रहे BBB ब्राउज़र (कम से कम 111 संस्करण) की आवश्यकता है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு OOO உடன் பொருந்தாது, மேலும் பிபிபி உலாவி (பதிப்பு குறைந்தது 111) ஒரு டி.டி.டி.யில் குறைந்தது 222 சிபியு கோர்களுடன் இயங்குகிறது",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം ooo- യുമായി പൊരുത്തപ്പെടുന്നില്ല, കൂടാതെ കുറഞ്ഞത് 222 സി.പി.യു കോറുകളുള്ള ഒരു ഡിഡിഡിയിൽ പ്രവർത്തിക്കുന്ന ബിബിബി ബ്ര browser സർ (പതിപ്പ് കുറഞ്ഞത് 111) ആവശ്യമാണ്",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం OOO కి విరుద్ధంగా ఉంది మరియు కనీసం 222 CPU కోర్లతో DDD లో BBB బ్రౌజర్ (కనీసం 111) నడుస్తుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು OOO ಗೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ, ಮತ್ತು ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಡಿಡಿಡಿಯಲ್ಲಿ ಚಾಲನೆಯಲ್ಲಿರುವ ಬಿಬಿಬಿ ಬ್ರೌಸರ್ (ಆವೃತ್ತಿ ಕನಿಷ್ಠ 111) ಅಗತ್ಯವಿದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই অধ্যয়নটি ওওর সাথে বেমানান, এবং বিবিবি ব্রাউজার (কমপক্ষে 111 সংস্করণ) প্রয়োজন একটি ডিডিডি -তে কমপক্ষে 222 সিপিইউ কোর সহ চলমান",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini tidak sesuai dengan OOO, dan membutuhkan browser BBB (versi setidaknya 111) yang berjalan pada DDD dengan setidaknya 222 core CPU",
    "zh-CN":
      "如其描述中所述，这项研究与OOO不相容，并且需要在DDD上运行至少222个CPU内核的BBB浏览器（版本至少为111）",
    "zh-HK":
      "如其描述中所述，這項研究與OOO不相容，並且需要在DDD上運行至少222個CPU內核的BBB瀏覽器（版本至少為111）",
    ja: "その説明に記載されているように、この研究はOOOと互換性があり、少なくとも222 CPUコアでDDDで実行されているBBBブラウザー（少なくとも111バージョン）が必要です",
    ko: "설명에 언급 된 바와 같이,이 연구는 OOO와 호환되지 않으며 최소 222 CPU 코어가있는 DDD에서 BBB 브라우저 (버전 111)가 필요합니다.",
  },
  EE_compatibleBrowserVersionOSDeviceCores: {
    "en-US":
      "As stated in its description, this study is only compatible with the BBB browser (version at least 111) running on a OOO DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie nur mit dem BBB -Browser (Version 111) kompatibel",
    fr: "Comme indiqué dans sa description, cette étude n'est compatible qu'avec le navigateur BBB (version au moins 111) fonctionnant sur un DDD OOO avec au moins 222 cœurs CPU",
    es: "Como se indica en su descripción, este estudio solo es compatible con el navegador BBB (versión al menos 111) que se ejecuta en un OOO DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é compatível apenas com o navegador BBB (versão pelo menos 111) em execução em um DDD OOO com pelo menos 222 núcleos da CPU",
    it: "Come indicato nella sua descrizione, questo studio è compatibile solo con il browser BBB (versione almeno 111) in esecuzione su un DDD OOO con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este compatibil doar cu browserul BBB (versiunea cel puțin 111) care rulează pe un OOO DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w opisie, badanie to jest kompatybilne tylko z przeglądarką BBB (wersja co najmniej 111) działająca na OOO DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование совместимо только с браузером BBB (версия не менее 111), работающего на OOO DDD с не менее 222 ядрами ЦП",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը համատեղելի է միայն BBB զննարկչի հետ (տարբերակ առնվազն 111) վազում է OOO DDD- ով `առնվազն 222 պրոցեսորի միջուկով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus on yhteensopiva vain BBB -selaimen (versio vähintään 111) kanssa, joka toimii OOO -DDD: llä vähintään 222 CPU -ytimellä,",
    ka: "როგორც აღწერილია მისი აღწერით, ეს კვლევა მხოლოდ BBB ბრაუზერთან (ვერსია მინიმუმ 111) მუშაობს OOO DDD– ზე მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה תואם רק לדפדפן BBB (גרסה לפחות 111) הפועל על OOO DDD עם לפחות 222 ליבות מעבד",
    ar: "كما هو مذكور في وصفها ، فإن هذه الدراسة متوافقة فقط مع متصفح BBB (الإصدار 111 على الأقل) الذي يعمل على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ صرف بی بی بی براؤزر (کم از کم 111 ورژن) کے ساتھ مطابقت رکھتا ہے جو کم از کم 222 سی پی یو کور کے ساتھ او او ڈی ڈی ڈی پر چل رہا ہے۔",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन केवल BBB ब्राउज़र (कम से कम 111 संस्करण) के साथ संगत है, जो कम से कम 222 CPU कोर के साथ OOO DDD पर चल रहा है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு பிபிபி உலாவி (பதிப்பு குறைந்தது 111) ஒரு OOO டி.டி.டி யில் குறைந்தது 222 CPU கோர்களுடன் மட்டுமே பொருந்தும்",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം ബിബിബി ബ്ര browser സറുമായി (കുറഞ്ഞത് 111 പതിപ്പ്) ഒരു ooo ​​ddd- ൽ പ്രവർത്തിക്കുന്നതാണ്",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం కనీసం 222 సిపియు కోర్లతో OOO DDD లో నడుస్తున్న BBB బ్రౌజర్ (వెర్షన్ కనీసం 111) తో మాత్రమే అనుకూలంగా ఉంటుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು ಬಿಬಿಬಿ ಬ್ರೌಸರ್ (ಆವೃತ್ತಿ ಕನಿಷ್ಠ 111) ಗೆ ಒಯುಒ ಡಿಡಿಡಿಯಲ್ಲಿ ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಮಾತ್ರ ಹೊಂದಿಕೊಳ್ಳುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই অধ্যয়নটি কেবল বিবিবি ব্রাউজারের সাথে সামঞ্জস্যপূর্ণ (কমপক্ষে ১১১ সংস্করণ) কমপক্ষে ২২২ সিপিইউ কোরের সাথে একটি ওও ডিডিডি -তে চলছে",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini hanya kompatibel dengan browser BBB (versi setidaknya 111) yang berjalan pada OOO DDD dengan setidaknya 222 core CPU",
    "zh-CN":
      "如其描述中所述，这项研究仅与BBB浏览器（版本至少111）兼容，在OOO DDD上至少222个CPU内核兼容",
    "zh-HK":
      "如其描述中所述，這項研究僅與BBB瀏覽器（版本至少111）兼容，在OOO DDD上至少222個CPU內核兼容",
    ja: "その説明で述べたように、この研究は、少なくとも222 CPUコアでOOO DDDで実行されているBBBブラウザー（少なくとも111バージョン）とのみ互換性があります",
    ko: "설명에 언급 된 바와 같이,이 연구는 최소 222 CPU 코어가있는 OOO DDD에서 실행되는 BBB 브라우저 (버전 111) 와만 호환됩니다.",
  },
  EE_compatibleDeviceCores: {
    "en-US":
      "As stated in its description, this study runs only on a DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, wird diese Studie nur auf einem DDD mit mindestens 222 CPU -Kernen betrieben",
    fr: "Comme indiqué dans sa description, cette étude ne fonctionne que sur un DDD avec au moins 222 cœurs CPU",
    es: "Como se indica en su descripción, este estudio se ejecuta solo en un DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é executado apenas em um DDD com pelo menos 222 núcleos da CPU",
    it: "Come affermato nella sua descrizione, questo studio funziona solo su un DDD con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu se desfășoară doar pe un DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w opisie, badanie to działa tylko na DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование работает только на DDD с не менее 222 ядрами процессора",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը գործում է միայն DDD- ով `առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus toimii vain DDD: llä, jossa on vähintään 222 CPU -ytimiä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა მხოლოდ DDD- ზე გადის მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה פועל רק ב- DDD עם לפחות 222 ליבות מעבד",
    ar: "كما ذكر في وصفها ، تعمل هذه الدراسة فقط على DDD مع 222 نوى على الأقل من وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ صرف ڈی ڈی ڈی پر چلتا ہے جس میں کم از کم 222 سی پی یو کور ہیں",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन केवल कम से कम 222 सीपीयू कोर के साथ डीडीडी पर चलता है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு குறைந்தது 222 சிபியு கோர்களுடன் டி.டி.டி.யில் மட்டுமே இயங்குகிறது",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം കുറഞ്ഞത് 222 സിപിയു കോറുകളുള്ള ഒരു ഡിഡിഡിയിൽ മാത്രം പ്രവർത്തിക്കുന്നു",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం కనీసం 222 సిపియు కోర్లతో డిడిడిలో మాత్రమే నడుస్తుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಡಿಡಿಡಿಯಲ್ಲಿ ಮಾತ್ರ ನಡೆಯುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই গবেষণাটি কেবলমাত্র কমপক্ষে 222 সিপিইউ কোর সহ একটি ডিডিডিতে চলে",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini hanya berjalan pada DDD dengan setidaknya 222 inti CPU",
    "zh-CN": "如其描述中所述，本研究仅在DDD上运行至少222个CPU核心",
    "zh-HK": "如其描述中所述，本研究僅在DDD上運行至少222個CPU核心",
    ja: "その説明で述べたように、この研究は少なくとも222のCPUコアを持つDDDでのみ実行されます",
    ko: "설명에 언급 된 바와 같이,이 연구는 최소 222 CPU 코어가있는 DDD에서만 실행됩니다.",
  },
  EE_compatibleExceptForScreenResolution: {
    "en-US":
      "This computer will become compatible if you increase the screen resolution to at least 333⨉444 pixels.",
    de: "Dieser Computer wird kompatibel, wenn Sie die Bildschirmauflösung auf mindestens 333⨉444 Pixel erhöhen.",
    fr: "Cet ordinateur deviendra compatible si vous augmentez la résolution d'écran à au moins 333⨉444 pixels.",
    es: "Esta computadora se volverá compatible si aumenta la resolución de la pantalla a al menos 333⨉444 píxeles.",
    pt: "Este computador se tornará compatível se você aumentar a resolução da tela para pelo menos 333⨉444 pixels.",
    it: "Questo computer diventerà compatibile se aumenti la risoluzione dello schermo ad almeno 333⨉444 pixel.",
    ro: "Acest computer va deveni compatibil dacă creșteți rezoluția ecranului la cel puțin 333⨉444 pixeli.",
    pl: "Ten komputer stanie się kompatybilny, jeśli zwiększysz rozdzielczość ekranu do co najmniej 333⨉444 pikseli.",
    ru: "Этот компьютер станет совместимым, если вы увеличите разрешение экрана как минимум до 333⨉444 пикселей.",
    hy: "Այս համակարգիչը համատեղելի կդառնա, եթե էկրանի լուծումը մեծացնեք առնվազն 333⨉444 պիքսել:",
    fi: "Tästä tietokoneesta tulee yhteensopiva, jos nostat näytön tarkkuuden vähintään 333⨉444 pikseliin.",
    ka: "ეს კომპიუტერი თავსებადი გახდება, თუ ეკრანის რეზოლუციას გაზრდის მინიმუმ 333⨉444 პიქსელამდე.",
    he: "מחשב זה יהפוך לתואם אם תגדיל את רזולוציית המסך לפחות 333⨉444 פיקסלים.",
    ar: "سيصبح هذا الكمبيوتر متوافقًا إذا قمت بزيادة دقة الشاشة إلى ما لا يقل عن 333⨉444 بكسل.",
    ur: "اگر آپ اسکرین ریزولوشن کو کم سے کم 333⨉444 پکسلز میں بڑھا دیتے ہیں تو یہ کمپیوٹر مطابقت پذیر ہوجائے گا۔",
    hi: "यदि आप स्क्रीन रिज़ॉल्यूशन को कम से कम 333⨉444 पिक्सेल तक बढ़ाते हैं तो यह कंप्यूटर संगत हो जाएगा।",
    ta: "திரை தெளிவுத்திறனை குறைந்தது 333⨉444 பிக்சல்களாக அதிகரித்தால் இந்த கணினி இணக்கமாகிவிடும்.",
    ml: "നിങ്ങൾ സ്ക്രീൻ റെസല്യൂഷൻ കുറഞ്ഞത് 333⨉444 പിക്സലുകളായി വർദ്ധിപ്പിച്ചാൽ ഈ കമ്പ്യൂട്ടർ പൊരുത്തപ്പെടും.",
    te: "మీరు స్క్రీన్ రిజల్యూషన్‌ను కనీసం 333⨉444 పిక్సెల్‌లకు పెంచినట్లయితే ఈ కంప్యూటర్ అనుకూలంగా మారుతుంది.",
    kn: "ನೀವು ಪರದೆಯ ರೆಸಲ್ಯೂಶನ್ ಅನ್ನು ಕನಿಷ್ಠ 333⨉444 ಪಿಕ್ಸೆಲ್‌ಗಳಿಗೆ ಹೆಚ್ಚಿಸಿದರೆ ಈ ಕಂಪ್ಯೂಟರ್ ಹೊಂದಿಕೊಳ್ಳುತ್ತದೆ.",
    bn: "আপনি যদি স্ক্রিন রেজোলিউশনটি কমপক্ষে 333⨉444 পিক্সেলে বাড়ান তবে এই কম্পিউটারটি সামঞ্জস্যপূর্ণ হয়ে উঠবে।",
    id: "Komputer ini akan menjadi kompatibel jika Anda meningkatkan resolusi layar ke setidaknya 333⨉444 piksel.",
    "zh-CN": "如果将屏幕分辨率提高到至少333⨉444像素，则该计算机将变得兼容。",
    "zh-HK": "如果將屏幕分辨率提高到至少333⨉444像素，則該計算機將變得兼容。",
    ja: "このコンピューターは、画面の解像度を少なくとも333⨉444ピクセルに増やすと互換性があります。",
    ko: "스크린 해상도를 최소 333⨉444 픽셀로 늘리면이 컴퓨터가 호환됩니다.",
  },
  EE_compatibleNotBrowserDeviceCores: {
    "en-US":
      "As stated in its description, this study is incompatible with the bbb browser(s), and runs only on a DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie nicht mit dem BBB -Browser (s) kompatibel und läuft nur auf einem DDD mit mindestens 222 CPU -Kernen",
    fr: "Comme indiqué dans sa description, cette étude est incompatible avec le (s) navigateur (s) BBB et ne fonctionne que sur un DDD avec au moins 222 cœurs de processeur",
    es: "Como se indica en su descripción, este estudio es incompatible con los navegadores BBB, y se ejecuta solo en un DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é incompatível com o (s) navegador (s) BBB (s) e é executado apenas em um DDD com pelo menos 222 núcleos de CPU",
    it: "Come affermato nella sua descrizione, questo studio è incompatibile con il browser BBB e funziona solo su un DDD con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este incompatibil cu browserul (browserul) BBB și se desfășoară doar pe un DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w opisie, badanie to jest niezgodne z przeglądarką (-ów) BBB i działa tylko na DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование несовместимо с браузером BBB и работает только на DDD с не менее 222 ядрами процессора",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը անհամատեղելի է BBB զննարկչի (ներ) ի հետ եւ աշխատում է միայն DDD- ով `առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus ei ole yhteensopiva BBB -selaimen (S) kanssa, ja se toimii vain DDD: llä, jossa on vähintään 222 CPU -ydintä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა შეუთავსებელია BBB ბრაუზერთან (ებ) ს და გადის მხოლოდ DDD– ზე, მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה אינו תואם את דפדפן ה- BBB, ופועל רק ב- DDD עם לפחות 222 ליבות מעבד",
    ar: "كما ذُكر في وصفها ، لا تتوافق هذه الدراسة مع متصفح (متصفح) BBB ، وتدير فقط على DDD مع 222 نوى على الأقل من وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ بی بی بی براؤزر (زبانیں) سے مطابقت نہیں رکھتا ہے ، اور صرف ڈی ڈی ڈی پر چلتا ہے جس میں کم از کم 222 سی پی یو کور ہیں۔",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन बीबीबी ब्राउज़र (एस) के साथ असंगत है, और केवल कम से कम 222 सीपीयू कोर के साथ डीडीडी पर चलता है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு பிபிபி உலாவி (கள்) உடன் பொருந்தாது, மேலும் குறைந்தது 222 சிபியு கோர்களுடன் டி.டி.டி.யில் மட்டுமே இயங்குகிறது",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം ബിബിബി ബ്രൗസറുമായി പൊരുത്തപ്പെടുന്നില്ല, കൂടാതെ കുറഞ്ഞത് 222 സിപിയു കോറുകളുള്ള ഒരു ഡിഡിഡിയിൽ മാത്രം പ്രവർത്തിക്കുന്നു",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం BBB బ్రౌజర్ (ల) కు విరుద్ధంగా ఉంది మరియు కనీసం 222 CPU కోర్లతో DDD లో మాత్రమే నడుస్తుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು ಬಿಬಿಬಿ ಬ್ರೌಸರ್ (ಗಳ) ಗೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ ಮತ್ತು ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಡಿಡಿಡಿಯಲ್ಲಿ ಮಾತ್ರ ಚಲಿಸುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই গবেষণাটি বিবিবি ব্রাউজার (গুলি) এর সাথে বেমানান, এবং কমপক্ষে 222 সিপিইউ কোরের সাথে কেবল একটি ডিডিডিতে চালিত হয়",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini tidak sesuai dengan browser BBB, dan hanya berjalan pada DDD dengan setidaknya 222 core CPU",
    "zh-CN":
      "如其描述中所述，这项研究与BBB浏览器不相容，并且仅在DDD上运行至少222个CPU核心",
    "zh-HK":
      "如其描述中所述，這項研究與BBB瀏覽器不相容，並且僅在DDD上運行至少222個CPU核心",
    ja: "その説明に記載されているように、この研究はBBBブラウザーと互換性がなく、少なくとも222 CPUコアのDDDでのみ実行されます",
    ko: "설명에 언급 된 바와 같이,이 연구는 BBB 브라우저와 호환되지 않으며 최소 222 CPU 코어가있는 DDD에서만 실행됩니다.",
  },
  EE_compatibleNotBrowserNotOSDeviceCores: {
    "en-US":
      "As stated in its description, this study is incompatible with the bbb browser(s) and also incompatible with ooo. It runs only on a DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie nicht mit dem BBB -Browser und auch mit OOO kompatibel. Es läuft nur auf einem DDD mit mindestens 222 CPU -Kernen",
    fr: "Comme indiqué dans sa description, cette étude est incompatible avec le ou les navigateurs BBB et également incompatible avec OOO. Il fonctionne uniquement sur un DDD avec au moins 222 cœurs de processeur",
    es: "Como se indica en su descripción, este estudio es incompatible con los navegadores BBB y también incompatible con OOO. Se ejecuta solo en un DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é incompatível com o (s) navegador (s) BBB (s) e também incompatível com o OOO. Ele é executado apenas em um DDD com pelo menos 222 núcleos da CPU",
    it: "Come affermato nella sua descrizione, questo studio è incompatibile con il browser BBB e anche incompatibile con OOO. Funziona solo su un DDD con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este incompatibil cu browserul (browserul) BBB și, de asemenea, incompatibil cu OOO. Se rulează doar pe un DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w swoim opisie, badanie to jest niezgodne z przeglądarką BBB, a także niekompatybilną z OOO. Działa tylko na DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование несовместимо с браузером BBB, а также несовместимо с OOO. Он работает только на DDD с не менее 222 ядрами процессора",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը անհամատեղելի է BBB զննարկչի (ներ) ի հետ եւ նաեւ անհամատեղելի OOO- ի հետ: Այն գործում է միայն DDD- ով `առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus ei ole yhteensopiva BBB -selaimen (t) kanssa ja myös yhteensopimattomana OOO: n kanssa. Se toimii vain DDD: llä, jossa on vähintään 222 CPU -ydintä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა შეუთავსებელია BBB ბრაუზერთან (ებ) ს და ასევე შეუთავსებელია OOO- სთან. ის მხოლოდ DDD- ზე გადის მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה אינו תואם את דפדפן ה- BBB וגם אינו תואם את OOO. זה פועל רק ב- DDD עם לפחות 222 ליבות מעבד",
    ar: "كما هو مذكور في وصفها ، لا تتوافق هذه الدراسة مع متصفح (متصفح) BBB وأيضًا غير متوافق مع OOO. يعمل فقط على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ بی بی بی براؤزر (زبانیں) سے مطابقت نہیں رکھتا ہے اور او او او سے بھی مطابقت نہیں رکھتا ہے۔ یہ صرف ڈی ڈی ڈی پر کم از کم 222 سی پی یو کور کے ساتھ چلتا ہے",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन बीबीबी ब्राउज़र (एस) के साथ असंगत है और ओओओ के साथ भी असंगत है। यह केवल एक डीडीडी पर कम से कम 222 सीपीयू कोर के साथ चलता है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு பிபிபி உலாவி (கள்) உடன் பொருந்தாது, மேலும் OOO உடன் பொருந்தாது. இது குறைந்தது 222 CPU கோர்களுடன் ஒரு டி.டி.டி.",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം ബിബിബി ബ്ര browser സറിനുമായി പൊരുത്തപ്പെടുന്നില്ല, മാത്രമല്ല ഇത് ooo- യുമായി പൊരുത്തപ്പെടുന്നില്ല. കുറഞ്ഞത് 222 സിപിയു കോറുകളുള്ള ഒരു ഡിഡിഡിയിൽ മാത്രം പ്രവർത്തിക്കുന്നു",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం BBB బ్రౌజర్ (ల) కు విరుద్ధంగా లేదు మరియు OOO కి కూడా విరుద్ధంగా ఉంటుంది. ఇది కనీసం 222 సిపియు కోర్లతో డిడిడిలో మాత్రమే నడుస్తుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು ಬಿಬಿಬಿ ಬ್ರೌಸರ್ (ಗಳು) ಗೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ ಮತ್ತು OOO ಗೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ. ಇದು ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಡಿಡಿಡಿಯಲ್ಲಿ ಮಾತ್ರ ಚಲಿಸುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই অধ্যয়নটি বিবিবি ব্রাউজার (গুলি) এর সাথে বেমানান এবং ওওর সাথেও বেমানান। এটি কমপক্ষে 222 সিপিইউ কোর সহ কেবল একটি ডিডিডিতে চলে",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini tidak sesuai dengan browser BBB dan juga tidak sesuai dengan OOO. Ini hanya berjalan pada DDD dengan setidaknya 222 core CPU",
    "zh-CN":
      "如其描述中所述，这项研究与BBB浏览器不相容，并且与OOO也不兼容。它仅在具有至少222个CPU内核的DDD上运行",
    "zh-HK":
      "如其描述中所述，這項研究與BBB瀏覽器不相容，並且與OOO也不兼容。它僅在具有至少222個CPU內核的DDD上運行",
    ja: "その説明で述べたように、この研究はBBBブラウザーと互換性があり、OOOとも互換性がありません。少なくとも222のCPUコアを持つDDDでのみ実行されます",
    ko: "설명에 언급 된 바와 같이,이 연구는 BBB 브라우저와 호환되지 않으며 OOO와 호환되지 않습니다. 최소 222 CPU 코어가있는 DDD에서만 실행됩니다.",
  },
  EE_compatibleNotBrowserOSDeviceCores: {
    "en-US":
      "As stated in its description, this study is incompatible with the bbb browser(s) and runs only on a OOO DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie nicht mit dem BBB -Browser (s) kompatibel und läuft nur auf einem OOO DDD mit mindestens 222 CPU -Kernen",
    fr: "Comme indiqué dans sa description, cette étude est incompatible avec le (s) navigateur (s) BBB et ne fonctionne que sur un DDD OOO avec au moins 222 cœurs CPU",
    es: "Como se indica en su descripción, este estudio es incompatible con los navegadores BBB y se ejecuta solo en un OOO DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é incompatível com o (s) navegador (s) BBB (s) e é executado apenas em um DDD OOO com pelo menos 222 núcleos da CPU",
    it: "Come affermato nella sua descrizione, questo studio è incompatibile con il browser BBB e funziona solo su un DDD OOO con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este incompatibil cu browserul (browserul) BBB și rulează doar pe un OOO DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w opisie, badanie to jest niezgodne z przeglądarką (-ów) BBB i działa tylko na OOO DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование несовместимо с браузером BBB (S) и работает только на OOO DDD с не менее 222 ядрами ЦП",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը անհամատեղելի է BBB զննարկչի (ներ) ի հետ եւ գործում է միայն OOO DDD- ի վրա առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus ei ole yhteensopiva BBB -selaimen (S) kanssa ja toimii vain OOO -DDD: llä, jossa on vähintään 222 CPU -ytimiä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა შეუთავსებელია BBB ბრაუზერთან (ებ) ს და გადის მხოლოდ OOO DDD– ზე მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה אינו תואם את דפדפן ה- BBB (ים) ופועל רק ב- OOO DDD עם לפחות 222 ליבות מעבד",
    ar: "كما ذكر في وصفها ، لا تتوافق هذه الدراسة مع متصفح (متصفح) BBB ويعمل فقط على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ بی بی بی براؤزر (زبانیں) سے مطابقت نہیں رکھتا ہے اور صرف او او ڈی ڈی ڈی پر چلتا ہے جس میں کم از کم 222 سی پی یو کور ہیں۔",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन बीबीबी ब्राउज़र (एस) के साथ असंगत है और केवल कम से कम 222 सीपीयू कोर के साथ एक ओओओ डीडीडी पर चलता है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு பிபிபி உலாவி (கள்) உடன் பொருந்தாது மற்றும் குறைந்தது 222 CPU கோர்களுடன் OOO DDD இல் மட்டுமே இயங்குகிறது",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം ബിബിബി ബ്ര browser സർ (കൾ) ഉപയോഗിച്ച് പൊരുത്തപ്പെടുന്നില്ല, കൂടാതെ കുറഞ്ഞത് 222 സിപിയു കോറുകളും ഉപയോഗിച്ച് ഒരു ooo ​​ഡിഡിഡിയിൽ മാത്രം പ്രവർത്തിക്കുന്നു",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం BBB బ్రౌజర్ (ల) కు విరుద్ధంగా ఉంది మరియు కనీసం 222 CPU కోర్లతో OOO DDD లో మాత్రమే నడుస్తుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು ಬಿಬಿಬಿ ಬ್ರೌಸರ್ (ಗಳು) ಗೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ ಮತ್ತು ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ OOO ಡಿಡಿಡಿಯಲ್ಲಿ ಮಾತ್ರ ಚಲಿಸುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই অধ্যয়নটি বিবিবি ব্রাউজারের সাথে বেমানান এবং কেবলমাত্র একটি ওও ডিডিডি -তে কমপক্ষে 222 সিপিইউ কোরের সাথে চালিত হয়",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini tidak kompatibel dengan browser BBB dan hanya berjalan pada ooo DDD dengan setidaknya 222 inti CPU",
    "zh-CN":
      "如其描述中所述，这项研究与BBB浏览器不相容，并且仅在具有至少222个CPU内核的OOO DDD上运行",
    "zh-HK":
      "如其描述中所述，這項研究與BBB瀏覽器不相容，並且僅在具有至少222個CPU內核的OOO DDD上運行",
    ja: "その説明に記載されているように、この研究はBBBブラウザーと互換性がなく、少なくとも222のCPUコアを持つOOO DDDでのみ実行されます",
    ko: "설명에 언급 된 바와 같이,이 연구는 BBB 브라우저와 호환되지 않으며 최소 222 CPU 코어가있는 OOO DDD에서만 실행됩니다.",
  },
  EE_compatibleNotOSDeviceCores: {
    "en-US":
      "As stated in its description, this study is incompatible with ooo. It runs only on a DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, ist diese Studie mit OOO unvereinbar. Es läuft nur auf einem DDD mit mindestens 222 CPU -Kernen",
    fr: "Comme indiqué dans sa description, cette étude est incompatible avec OOO. Il fonctionne uniquement sur un DDD avec au moins 222 cœurs de processeur",
    es: "Como se indica en su descripción, este estudio es incompatible con OOO. Se ejecuta solo en un DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é incompatível com o OOO. Ele é executado apenas em um DDD com pelo menos 222 núcleos da CPU",
    it: "Come affermato nella sua descrizione, questo studio è incompatibile con OOO. Funziona solo su un DDD con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu este incompatibil cu OOO. Se rulează doar pe un DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w opisie, badanie to jest niezgodne z OOO. Działa tylko na DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование несовместимо с OOO. Он работает только на DDD с не менее 222 ядрами процессора",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը անհամատեղելի է OOO- ի հետ: Այն գործում է միայն DDD- ով `առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus on yhteensopimattomana OOO: n kanssa. Se toimii vain DDD: llä, jossa on vähintään 222 CPU -ydintä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა შეუთავსებელია OOO- სთან. ის მხოლოდ DDD- ზე გადის მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה אינו תואם את OOO. זה פועל רק ב- DDD עם לפחות 222 ליבות מעבד",
    ar: "كما هو مذكور في وصفها ، لا تتوافق هذه الدراسة مع OOO. يعمل فقط على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ OOO سے مطابقت نہیں رکھتا ہے۔ یہ صرف ڈی ڈی ڈی پر کم از کم 222 سی پی یو کور کے ساتھ چلتا ہے",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन OOO के साथ असंगत है। यह केवल एक डीडीडी पर कम से कम 222 सीपीयू कोर के साथ चलता है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு OOO உடன் பொருந்தாது. இது குறைந்தது 222 CPU கோர்களுடன் ஒரு டி.டி.டி.",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം ooo- യുമായി പൊരുത്തപ്പെടുന്നില്ല. കുറഞ്ഞത് 222 സിപിയു കോറുകളുള്ള ഒരു ഡിഡിഡിയിൽ മാത്രം പ്രവർത്തിക്കുന്നു",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం OOO కి విరుద్ధంగా లేదు. ఇది కనీసం 222 సిపియు కోర్లతో డిడిడిలో మాత్రమే నడుస్తుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು OOO ಗೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ. ಇದು ಕನಿಷ್ಠ 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ ಡಿಡಿಡಿಯಲ್ಲಿ ಮಾತ್ರ ಚಲಿಸುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই গবেষণাটি ওওর সাথে বেমানান। এটি কমপক্ষে 222 সিপিইউ কোর সহ কেবল একটি ডিডিডিতে চলে",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini tidak sesuai dengan OOO. Ini hanya berjalan pada DDD dengan setidaknya 222 core CPU",
    "zh-CN":
      "如其描述中所述，这项研究与OOO不相容。它仅在具有至少222个CPU内核的DDD上运行",
    "zh-HK":
      "如其描述中所述，這項研究與OOO不相容。它僅在具有至少222個CPU內核的DDD上運行",
    ja: "その説明で述べたように、この研究はOOOと互換性がありません。少なくとも222のCPUコアを持つDDDでのみ実行されます",
    ko: "설명에 언급 된 바와 같이,이 연구는 OOO와 호환되지 않습니다. 최소 222 CPU 코어가있는 DDD에서만 실행됩니다.",
  },
  EE_compatibleOSDeviceCores: {
    "en-US":
      "As stated in its description, this study runs only on a OOO DDD with at least 222 CPU cores",
    de: "Wie in ihrer Beschreibung angegeben, wird diese Studie nur auf einem OOO DDD mit mindestens 222 CPU -Kernen ausgeführt",
    fr: "Comme indiqué dans sa description, cette étude ne fonctionne que sur un DDD OOO avec au moins 222 cœurs CPU",
    es: "Como se indica en su descripción, este estudio se ejecuta solo en un OOO DDD con al menos 222 núcleos de CPU",
    pt: "Conforme declarado em sua descrição, este estudo é executado apenas em um DDD OOO com pelo menos 222 núcleos da CPU",
    it: "Come affermato nella sua descrizione, questo studio funziona solo su un DDD OOO con almeno 222 core CPU",
    ro: "După cum s -a menționat în descrierea sa, acest studiu se desfășoară doar pe un OOO DDD cu cel puțin 222 de nuclee de procesare",
    pl: "Jak stwierdzono w opisie, badanie to działa tylko na OOO DDD z co najmniej 222 rdzeniami procesora",
    ru: "Как указано в его описании, это исследование работает только на OOO DDD, по крайней мере, с 222 ядрами процессора",
    hy: "Ինչպես նշված է իր նկարագրության մեջ, այս ուսումնասիրությունը գործում է միայն OOO DDD- ի վրա `առնվազն 222 պրոցեսորի միջուկներով",
    fi: "Kuten kuvauksessaan todetaan, tämä tutkimus kulkee vain OOO -DDD: llä, jossa on vähintään 222 CPU -ytimiä",
    ka: "როგორც აღწერა აღწერა, ეს კვლევა გადის მხოლოდ OOO DDD– ზე, მინიმუმ 222 CPU ბირთვით",
    he: "כאמור בתיאורו, מחקר זה פועל רק ב- OOO DDD עם לפחות 222 ליבות מעבד",
    ar: "كما هو مذكور في وصفها ، تعمل هذه الدراسة فقط على DDD مع ما لا يقل عن 222 نوى وحدة المعالجة المركزية",
    ur: "جیسا کہ اس کی تفصیل میں کہا گیا ہے ، یہ مطالعہ صرف OOO DDD پر چلتا ہے جس میں کم از کم 222 CPU کور ہیں",
    hi: "जैसा कि इसके विवरण में कहा गया है, यह अध्ययन केवल एक OOO DDD पर कम से कम 222 CPU कोर के साथ चलता है",
    ta: "அதன் விளக்கத்தில் கூறப்பட்டுள்ளபடி, இந்த ஆய்வு குறைந்தது 222 CPU கோர்களுடன் OOO DDD இல் மட்டுமே இயங்குகிறது",
    ml: "അതിന്റെ വിവരണത്തിൽ പറഞ്ഞിരിക്കുന്നതുപോലെ, ഈ പഠനം കുറഞ്ഞത് 222 സിപിയു കോറുകളുള്ള ഒരു ooo ​​ddd ൽ മാത്രമേ പ്രവർത്തിക്കൂ",
    te: "దాని వర్ణనలో చెప్పినట్లుగా, ఈ అధ్యయనం కనీసం 222 సిపియు కోర్లతో OOO DDD లో మాత్రమే నడుస్తుంది",
    kn: "ಅದರ ವಿವರಣೆಯಲ್ಲಿ ಹೇಳಿರುವಂತೆ, ಈ ಅಧ್ಯಯನವು OOO DDD ಯಲ್ಲಿ ಕನಿಷ್ಠ 222 CPU ಕೋರ್ಗಳೊಂದಿಗೆ ಮಾತ್ರ ನಡೆಯುತ್ತದೆ",
    bn: "এর বর্ণনায় যেমন বলা হয়েছে, এই গবেষণাটি কেবলমাত্র কমপক্ষে 222 সিপিইউ কোর সহ একটি ওও ডিডিডি -তে চলে",
    id: "Seperti yang dinyatakan dalam deskripsinya, penelitian ini hanya berjalan pada ooo ddd dengan setidaknya 222 core cpu",
    "zh-CN": "如其描述中所述，本研究仅在具有至少222个CPU内核的OOO DDD上运行",
    "zh-HK": "如其描述中所述，本研究僅在具有至少222個CPU內核的OOO DDD上運行",
    ja: "その説明で述べたように、この研究は少なくとも222のCPUコアを持つOOO DDDでのみ実行されます",
    ko: "설명에 언급 된 바와 같이,이 연구는 최소 222 CPU 코어가있는 OOO DDD에서만 실행됩니다.",
  },
  EE_compatibleScreenHeight: {
    "en-US": " and a screen at least 222 pixels high",
    de: " und ein Bildschirm mindestens 222 Pixel hoch",
    fr: " et un écran d'au moins 222 pixels de haut",
    es: " y una pantalla de al menos 222 píxeles de altura",
    pt: " e uma tela pelo menos 222 pixels de altura",
    it: " e uno schermo alto almeno 222 pixel",
    ro: " și un ecran de cel puțin 222 de pixeli înălțime",
    pl: " i ekran co najmniej 222 pikseli wysokości",
    ru: " и экран не менее 222 пикселей высотой",
    hy: " եւ էկրանը առնվազն 222 պիքսել բարձր է",
    fi: " ja näyttö vähintään 222 pikseliä korkea",
    ka: " და ეკრანი მინიმუმ 222 პიქსელის სიმაღლეზე",
    he: " ומסך לפחות 222 פיקסלים",
    ar: " وشاشة على الأقل 222 بكسل",
    ur: " اور کم از کم 222 پکسلز اونچی اسکرین",
    hi: " और एक स्क्रीन कम से कम 222 पिक्सेल ऊँची",
    ta: " மற்றும் ஒரு திரை குறைந்தது 222 பிக்சல்கள் உயரமானது",
    ml: " കൂടാതെ കുറഞ്ഞത് 222 പിക്സലെങ്കിലും ഉയർന്നതാണ്",
    te: " మరియు స్క్రీన్ కనీసం 222 పిక్సెల్స్ ఎత్తు",
    kn: " ಮತ್ತು ಕನಿಷ್ಠ 222 ಪಿಕ್ಸೆಲ್‌ಗಳಷ್ಟು ಪರದೆಯು",
    bn: " এবং একটি স্ক্রিন কমপক্ষে 222 পিক্সেল উচ্চ",
    id: " dan layar setidaknya 222 piksel",
    "zh-CN": " 屏幕至少高222像素",
    "zh-HK": " 屏幕至少高222像素",
    ja: " 少なくとも222ピクセルの画面",
    ko: " 그리고 최소 222 픽셀의 스크린",
  },
  EE_compatibleScreenSize: {
    "en-US": " and a screen resolution of at least 111⨉222 pixels",
    de: " und eine Bildschirmauflösung von mindestens 111⨉222 Pixel",
    fr: " et une résolution d'écran d'au moins 111⨉222 pixels",
    es: " y una resolución de pantalla de al menos 111⨉222 píxeles",
    pt: " e uma resolução de tela de pelo menos 111⨉222 pixels",
    it: " e una risoluzione dello schermo di almeno 111⨉222 pixel",
    ro: " și o rezoluție de ecran de cel puțin 111⨉222 pixeli",
    pl: " oraz rozdzielczość ekranu co najmniej 111⨉222 pikseli",
    ru: " и разрешение экрана не менее 111⨉222 пикселей",
    hy: " եւ էկրանի լուծում առնվազն 111222 պիքսել",
    fi: " ja näytön resoluutio vähintään 111⨉222 pikseliä",
    ka: " და ეკრანის რეზოლუცია მინიმუმ 111⨉222 პიქსელი",
    he: " ורזולוציית מסך של לפחות 111⨉222 פיקסלים",
    ar: " ودقة شاشة لا تقل عن 111 درجة 222 بكسل",
    ur: " اور کم از کم 111⨉222 پکسلز کی اسکرین ریزولوشن",
    hi: " और कम से कम 111⨉222 पिक्सल का एक स्क्रीन रिज़ॉल्यूशन",
    ta: " மற்றும் குறைந்தது 111⨉222 பிக்சல்களின் திரை தெளிவுத்திறன்",
    ml: " കൂടാതെ കുറഞ്ഞത് 111⨉222 പിക്സലുകളുടെ ഒരു സ്ക്രീൻ മിഴിവ്",
    te: " మరియు కనీసం 111⨉222 పిక్సెల్స్ యొక్క స్క్రీన్ రిజల్యూషన్",
    kn: " ಮತ್ತು ಕನಿಷ್ಠ 111⨉222 ಪಿಕ್ಸೆಲ್‌ಗಳ ಪರದೆಯ ರೆಸಲ್ಯೂಶನ್",
    bn: " এবং কমপক্ষে 111⨉222 পিক্সেলের একটি স্ক্রিন রেজোলিউশন",
    id: " dan resolusi layar setidaknya 111⨉222 piksel",
    "zh-CN": " 和至少111⨉222像素的屏幕分辨率",
    "zh-HK": " 和至少111⨉222像素的屏幕分辨率",
    ja: " 少なくとも111⨉222ピクセルの画面解像度",
    ko: " 및 최소 111 ℃ 픽셀의 스크린 해상도",
  },
  EE_compatibleScreenWidth: {
    "en-US": " and a screen at least 111 pixels wide",
    de: " und ein Bildschirm mindestens 111 Pixel breit",
    fr: " et un écran d'au moins 111 pixels de large",
    es: " y una pantalla de al menos 111 píxeles de ancho",
    pt: " e uma tela de pelo menos 111 pixels de largura",
    it: " e uno schermo largo almeno 111 pixel",
    ro: " și un ecran de cel puțin 111 pixeli lățime",
    pl: " i ekran o szerokości co najmniej 111 pikseli",
    ru: " и экран не менее 111 пикселей в ширину",
    hy: " եւ էկրանը առնվազն 111 պիքսել լայն",
    fi: " ja näyttö vähintään 111 pikseliä",
    ka: " და ეკრანი მინიმუმ 111 პიქსელის სიგანე",
    he: " ומסך ברוחב של לפחות 111 פיקסלים",
    ar: " وشاشة لا تقل عن 111 بكسل",
    ur: " اور کم از کم 111 پکسلز چوڑا اسکرین",
    hi: " और एक स्क्रीन कम से कम 111 पिक्सेल चौड़ी",
    ta: " மற்றும் ஒரு திரை குறைந்தது 111 பிக்சல்கள் அகலம்",
    ml: " കൂടാതെ കുറഞ്ഞത് 111 പിക്സൽ വീതിയും",
    te: " మరియు కనీసం 111 పిక్సెల్స్ వెడల్పు స్క్రీన్",
    kn: " ಮತ್ತು ಕನಿಷ್ಠ 111 ಪಿಕ್ಸೆಲ್‌ಗಳ ಅಗಲದ ಪರದೆ",
    bn: " এবং একটি স্ক্রিন কমপক্ষে 111 পিক্সেল প্রশস্ত",
    id: " dan layar setidaknya 111 piksel lebar",
    "zh-CN": " 和屏幕至少111像素宽",
    "zh-HK": " 和屏幕至少111像素寬",
    ja: " 少なくとも111ピクセルの画面",
    ko: " 그리고 최소 111 픽셀의 스크린",
  },
  EE_ok: {
    "en-US": "Ok",
    de: "OK",
    fr: "D'accord",
    es: "De acuerdo",
    pt: "OK",
    it: "OK",
    ro: "Bine",
    pl: "OK",
    ru: "Хорошо",
    hy: "Լավ",
    fi: "Hyvä",
    ka: "Კარგი",
    he: "בסדר",
    ar: "نعم",
    ur: "ٹھیک ہے",
    hi: "ठीक",
    ta: "சரி",
    ml: "ശരി",
    te: "అలాగే",
    kn: "ಸರಿ",
    bn: "ঠিক আছে",
    id: "Oke",
    "zh-CN": "好的",
    "zh-HK": "好的",
    ja: "Ok",
    ko: "좋아요",
  },
  EE_or: {
    "en-US": " or",
    de: " oder",
    fr: " ou",
    es: " o",
    pt: " ou",
    it: " O",
    ro: " sau",
    pl: " Lub",
    ru: " или",
    hy: " կամ",
    fi: " tai",
    ka: " ან",
    he: " אוֹ",
    ar: " أو",
    ur: " یا",
    hi: " या",
    ta: " அல்லது",
    ml: " അഥവാ",
    te: " లేదా",
    kn: " ಅಥವಾ",
    bn: " বা",
    id: " atau",
    "zh-CN": " 或者",
    "zh-HK": " 或者",
    ja: " また",
    ko: " 또는",
  },
  EE_refresh: {
    "en-US": "Refresh",
    de: "Aktualisierung",
    fr: "Rafraîchir",
    es: "Actualizar",
    pt: "Atualizar",
    it: "ricaricare",
    ro: "Reîmprospăta",
    pl: "Odświeżać",
    ru: "Обновлять",
    hy: "Թարմացնել",
    fi: "virkistää",
    ka: "განახლება",
    he: "לְרַעֲנֵן",
    ar: "ينعش",
    ur: "ریفریش",
    hi: "ताज़ा करना",
    ta: "புதுப்பிப்பு",
    ml: "ഉന്മേഷം വീണ്ടെടുക്കുക",
    te: "రిఫ్రెష్",
    kn: "ಬಿಚ್ಚು",
    bn: "রিফ্রেশ",
    id: "Menyegarkan",
    "zh-CN": "刷新",
    "zh-HK": "刷新",
    ja: "リフレッシュします",
    ko: "새로 고치다",
  },
  EE_describeDevice: {
    "en-US":
      "It appears that the current browser is BBB (version 111), running on a OOO DDD with 222 CPU cores.",
    de: "Es scheint, dass der aktuelle Browser BBB (Version 111) ist und auf einem OOO DDD mit 222 CPU -Kernen ausgeführt wird.",
    fr: "Il semble que le navigateur actuel soit BBB (version 111), fonctionnant sur un DDD OOO avec 222 cœurs de processeur.",
    es: "Parece que el navegador actual es BBB (versión 111), que se ejecuta en un OOO DDD con 222 núcleos de CPU.",
    pt: "Parece que o navegador atual é BBB (versão 111), executando em um DDD OOO com 222 núcleos da CPU.",
    it: "Sembra che il browser corrente sia BBB (versione 111), in esecuzione su un DDD OOO con 222 core CPU.",
    ro: "Se pare că browserul curent este BBB (versiunea 111), care rulează pe un OOO DDD cu 222 nuclee de procesor.",
    pl: "Wygląda na to, że obecna przeglądarka to BBB (wersja 111), działająca na OOO DDD z 222 rdzeniami procesora.",
    ru: "Похоже, что текущий браузер - BBB (версия 111), работающий на Ooo DDD с 222 ядрами процессора.",
    hy: "Պարզվում է, որ ներկայիս զննարկիչը BBB է (111 տարբերակ), վազելով OOO DDD- ով, 222 պրոցեսորի միջուկով:",
    fi: "Vaikuttaa siltä, ​​että nykyinen selain on BBB (versio 111), joka toimii OOO -DDD: llä, jossa on 222 CPU -ytimen.",
    ka: "როგორც ჩანს, მიმდინარე ბრაუზერი არის BBB (ვერსია 111), რომელიც მუშაობს OOO DDD– ზე 222 CPU ბირთვით.",
    he: "נראה כי הדפדפן הנוכחי הוא BBB (גרסה 111), הפועל על OOO DDD עם 222 ליבות מעבד.",
    ar: "يبدو أن المتصفح الحالي هو BBB (الإصدار 111) ، ويعمل على DDD مع 222 وحدة المعالجة المركزية.",
    ur: "ایسا معلوم ہوتا ہے کہ موجودہ براؤزر بی بی بی (ورژن 111) ہے ، جو 222 سی پی یو کور کے ساتھ او او ڈی ڈی ڈی پر چل رہا ہے۔",
    hi: "ऐसा प्रतीत होता है कि वर्तमान ब्राउज़र BBB (संस्करण 111) है, जो 222 CPU कोर के साथ OOO DDD पर चल रहा है।",
    ta: "தற்போதைய உலாவி BBB (பதிப்பு 111) என்று தோன்றுகிறது, இது 222 CPU கோர்களுடன் OOO DDD இல் இயங்குகிறது.",
    ml: "222 സിപിയു കോറുകളുള്ള ഒരു ooo ​​ഡിഡിഡിയിൽ പ്രവർത്തിക്കുന്ന ബിബിബി (പതിപ്പ് 111) ആണ് നിലവിലെ ബ്ര browser സർ (പതിപ്പ് 111).",
    te: "ప్రస్తుత బ్రౌజర్ BBB (వెర్షన్ 111), 222 CPU కోర్లతో OOO DDD లో నడుస్తుంది.",
    kn: "ಪ್ರಸ್ತುತ ಬ್ರೌಸರ್ ಬಿಬಿಬಿ (ಆವೃತ್ತಿ 111), 222 ಸಿಪಿಯು ಕೋರ್ಗಳೊಂದಿಗೆ OOO ಡಿಡಿಡಿಯಲ್ಲಿ ಚಾಲನೆಯಲ್ಲಿದೆ ಎಂದು ತೋರುತ್ತದೆ.",
    bn: "এটি প্রদর্শিত হয় যে বর্তমান ব্রাউজারটি বিবিবি (সংস্করণ 111), 222 সিপিইউ কোর সহ একটি ওও ডিডিডি -তে চলছে।",
    id: "Tampaknya browser saat ini adalah BBB (versi 111), berjalan pada OOO DDD dengan 222 core CPU.",
    "zh-CN":
      "看来当前的浏览器是BBB（版本111），在带有222个CPU内核的OOO DDD上运行。",
    "zh-HK":
      "看來當前的瀏覽器是BBB（版本111），在帶有222個CPU內核的OOO DDD上運行。",
    ja: "現在のブラウザはBBB（バージョン111）であり、222 CPUコアを備えたOOO DDDで実行されているようです。",
    ko: "현재 브라우저는 BBB (버전 111)이며 222 CPU 코어가있는 OOO DDD에서 실행됩니다.",
  },
  EE_describeScreenSize: {
    "en-US": " And a 111⨉222 pixel screen.",
    de: " Und ein 111⨉222 Pixel -Bildschirm.",
    fr: " Et un écran de 111⨉222 pixels.",
    es: " Y una pantalla de 111⨉222 píxeles.",
    pt: " E uma tela de pixels de 111⨉222.",
    it: " E uno schermo pixel 111⨉222.",
    ro: " Și un ecran de 111⨉222 pixeli.",
    pl: " Oraz ekran piksela 111⨉222.",
    ru: " И экран 111⨉222 пикселей.",
    hy: " Եւ 111⨉222 պիքսելային էկրան:",
    fi: " Ja 111⨉222 pikselin näyttö.",
    ka: " და 111⨉222 პიქსელის ეკრანი.",
    he: " ומסך 111⨉222 פיקסלים.",
    ar: " وشاشة 111⨉222 بكسل.",
    ur: " اور ایک 111⨉222 پکسل اسکرین۔",
    hi: " और 1112222 पिक्सेल स्क्रीन।",
    ta: " மற்றும் 111⨉222 பிக்சல் திரை.",
    ml: " 111222 പിക്സൽ സ്ക്രീൻ.",
    te: " మరియు 111⨉222 పిక్సెల్ స్క్రీన్.",
    kn: " ಮತ್ತು 111⨉222 ಪಿಕ್ಸೆಲ್ ಪರದೆ.",
    bn: " এবং একটি 111⨉222 পিক্সেল স্ক্রিন।",
    id: " Dan layar piksel 111⨉222.",
    "zh-CN": " 和111⨉222像素屏幕。",
    "zh-HK": " 和111⨉222像素屏幕。",
    ja: " および111⨉222ピクセル画面。",
    ko: " 및 1112222 픽셀 화면.",
  },
  EE_IDRequested: {
    "en-US": "EasyEyes ID Requested",
    de: "Easyeyes -ID angefordert",
    fr: "EasyEyes ID demandé",
    es: "EasyEyes ID solicitada",
    pt: "Easyeyes ID solicitada",
    it: "EasyEyes ID richiesto",
    ro: "ID -ul EasyEyes solicitat",
    pl: "Żądany identyfikator Easyeyes",
    ru: "Easyeyes id запросил",
    hy: "EasyEyes ID- ն պահանջեց",
    fi: "Easyeyes -tunnus pyydetty",
    ka: "Easyeyes ID მოითხოვა",
    he: "מזהה EasyEyes מבוקש",
    ar: "easyeyes معرف طلب",
    ur: "ایزیئس آئی ڈی نے درخواست کی",
    hi: "Easyeyes आईडी अनुरोध किया",
    ta: "எளிதான ஐடி கோரப்பட்டது",
    ml: "എളുപ്പമുള്ള ഐഡി അഭ്യർത്ഥിച്ചു",
    te: "ఈజీయెస్ ఐడి అభ్యర్థించబడింది",
    kn: "ಈಸಿ ಐಡಿ ವಿನಂತಿಸಲಾಗಿದೆ",
    bn: "ইজিয়েয়েস আইডি অনুরোধ করা হয়েছে",
    id: "ID EYEYEYES diminta",
    "zh-CN": "请求EasyEyes ID",
    "zh-HK": "請求EasyEyes ID",
    ja: "Easyeyes IDが要求されました",
    ko: "EasyEyes ID가 요청되었습니다",
  },
  EE_noFileSelected: {
    "en-US": "No File Selected",
    de: "Keine Datei ausgewählt",
    fr: "Aucun fichier sélectionné",
    es: "Ningún archivo seleccionado",
    pt: "Nenhum arquivo selecionado",
    it: "Nessun file selezionato",
    ro: "Niciun fisier selectat",
    pl: "Nie wybrano pliku",
    ru: "Файл не выбран",
    hy: "Ոչ մի ֆայլ ընտրված չէ",
    fi: "Ei tiedostoa valittuna",
    ka: "Ფაილი არ არის არჩეული",
    he: "אף קובץ לא נבחר",
    ar: "لم يتم اختيار اي ملف",
    ur: "کوئی فائل منتخب نہیں کی گئی",
    hi: "कोई फ़ाइल चयनित नहीं",
    ta: "எந்த கோப்பும் தேர்வு செய்ய படவில்லை",
    ml: "ഫയലുകളൊന്നും തിരഞ്ഞെടുത്തിട്ടില്ല",
    te: "ఫైల్ ఎంచుకోబడలేదు",
    kn: "ಯಾವುದೇ ಫೈಲ್ ಅನ್ನು ಆಯ್ಕೆ ಮಾಡಲಾಗಿಲ್ಲ",
    bn: "নির্বাচিত কোন ফাইল",
    id: "Tidak ada file yang dipilih",
    "zh-CN": "未选择文件",
    "zh-HK": "未選擇文件",
    ja: "ファイルが選択されていません",
    ko: "파일이 선택되지 않았습니다",
  },
  EE_browseForID: {
    "en-US": "Select EasyEyes ID file",
    de: "Wählen Sie Easyeyes -ID -Datei aus",
    fr: "Sélectionnez le fichier d'identification EasyEyes",
    es: "Seleccione el archivo de ID de EasyEyes",
    pt: "Selecione o arquivo de identificação easeyes",
    it: "Seleziona il file ID EasyEyes",
    ro: "Selectați fișierul ID EAYEYES",
    pl: "Wybierz plik ID Easyeyes",
    ru: "Выберите идентификационный файл easyeyes",
    hy: "Ընտրեք EasyEyes ID ֆայլը",
    fi: "Valitse Easyeyes ID -tiedosto",
    ka: "აირჩიეთ Easyeyes ID ფაილი",
    he: "בחר קובץ מזהה EasyEyes",
    ar: "حدد ملف ID Easyeyes",
    ur: "ایزی ای ایس آئی ڈی فائل کو منتخب کریں",
    hi: "Easyeyes id फ़ाइल का चयन करें",
    ta: "எளிதான ஐடி கோப்பைத் தேர்ந்தெடுக்கவும்",
    ml: "ഐഡിഇസ് ഐഡി ഫയൽ തിരഞ്ഞെടുക്കുക",
    te: "ఈజీయెస్ ఐడి ఫైల్‌ను ఎంచుకోండి",
    kn: "ಈಸಿ ಐಇಎಸ್ ಐಡಿ ಫೈಲ್ ಆಯ್ಕೆಮಾಡಿ",
    bn: "ইজিওয়াইস আইডি ফাইল নির্বাচন করুন",
    id: "Pilih File ID EyyEyes",
    "zh-CN": "选择EasyEyes ID文件",
    "zh-HK": "選擇EasyEyes ID文件",
    ja: "EasyEyes IDファイルを選択します",
    ko: "Easyeyes ID 파일을 선택하십시오",
  },
  EE_ID_noLocalStorage: {
    "en-US":
      "The researcher requested you to provide your EasyEyes ID from the previous session, please type it here, or upload the file downloaded when the last session ends.",
    de: "Der Forscher forderte Sie auf, Ihre Easyeyes -ID aus der vorherigen Sitzung anzugeben. Bitte geben Sie sie hier ein oder laden Sie die Datei hoch, die beim Ende der letzten Sitzung heruntergeladen wurde.",
    fr: "Le chercheur vous a demandé de fournir votre identifiant EasyEyes à partir de la session précédente, veuillez le taper ici ou télécharger le fichier téléchargé à la fin de la dernière session.",
    es: "El investigador le solicitó que proporcione su ID de EasyEyes de la sesión anterior, escriba aquí o cargue el archivo descargado cuando finalice la última sesión.",
    pt: "O pesquisador solicitou que você fornecesse seu ID easia da sessão anterior, digite -o aqui ou faça o upload do arquivo baixado quando a última sessão terminar.",
    it: "Il ricercatore ti ha chiesto di fornire il tuo ID EasyEyes dalla sessione precedente, digitalo qui o caricare il file scaricato al termine dell'ultima sessione.",
    ro: "Cercetătorul v -a solicitat să furnizați ID -ul dvs. EasyEyes din sesiunea precedentă, vă rugăm să -l tastați aici sau să încărcați fișierul descărcat când se încheie ultima sesiune.",
    pl: "Badacz poprosił o podanie identyfikatora Easyeyes z poprzedniej sesji, wpisz go tutaj lub przesłać plik pobrany po zakończeniu ostatniej sesji.",
    ru: "Исследователь попросил вас предоставить ваш идентификатор easyeyes из предыдущего сеанса, введите его здесь или загрузите файл, загруженный при окончании последнего сеанса.",
    hy: "Հետազոտողը խնդրել է ձեզ տրամադրել ձեր EasyEyes ID- ն նախորդ նիստից, մուտքագրեք այն այստեղ, կամ վերբեռնեք ներբեռնված ֆայլը, երբ ավարտվում է վերջին նիստը:",
    fi: "Tutkija pyysi sinua antamaan Easyeyes -tunnuksen edellisestä istunnosta, kirjoita se tähän tai ladata tiedosto, joka ladataan viimeisen istunnon päättyessä.",
    ka: "მკვლევარმა მოგთხოვა წინა სესიიდან მიაწოდოთ თქვენი EasyEyes ID, გთხოვთ, აკრიფოთ აქ, ან ატვირთოთ ჩამოტვირთვის ფაილი, როდესაც ბოლო სესია დასრულდება.",
    he: "החוקר ביקש ממך לספק את מזהה ה- EasyEyes שלך מהפגישה הקודמת, אנא הקלד אותו כאן, או העלה את הקובץ שהורד עם סיום ההפעלה האחרונה.",
    ar: "طلب منك الباحث تقديم معرف Easyeyes الخاص بك من الجلسة السابقة ، يرجى كتابةه هنا ، أو تحميل الملف الذي تم تنزيله عندما تنتهي الجلسة الأخيرة.",
    ur: "محقق نے آپ سے درخواست کی کہ آپ پچھلے سیشن سے اپنی ایزی ای ایس آئی ڈی فراہم کریں ، براہ کرم اسے یہاں ٹائپ کریں ، یا آخری سیشن ختم ہونے پر ڈاؤن لوڈ والی فائل کو اپ لوڈ کریں۔",
    hi: "शोधकर्ता ने आपसे अनुरोध किया कि आप पिछले सत्र से अपनी Easyeyes ID प्रदान करें, कृपया इसे यहां टाइप करें, या अंतिम सत्र समाप्त होने पर डाउनलोड की गई फ़ाइल को अपलोड करें।",
    ta: "முந்தைய அமர்விலிருந்து உங்கள் ஈஸி ஐடியை வழங்குமாறு ஆராய்ச்சியாளர் உங்களிடம் கேட்டுக்கொண்டார், தயவுசெய்து அதை இங்கே தட்டச்சு செய்க அல்லது கடைசி அமர்வு முடிவடையும் போது பதிவிறக்கம் செய்யப்பட்ட கோப்பை பதிவேற்றவும்.",
    ml: "മുമ്പത്തെ സെഷനിൽ നിന്ന് നിങ്ങളുടെ എളുപ്പത്തിലുള്ള ഐഡി നൽകാൻ ഗവേഷകൻ നിങ്ങളോട് അഭ്യർത്ഥിച്ചു, ദയവായി ഇത് ഇവിടെ ടൈപ്പ് ചെയ്യുക, അല്ലെങ്കിൽ ഡൗൺലോഡുചെയ്ത ഫയൽ അവസാന സെഷൻ അവസാനിക്കുമ്പോൾ അപ്ലോഡ് ചെയ്യുക.",
    te: "మునుపటి సెషన్ నుండి మీ ఈజీయెస్ ఐడిని అందించమని పరిశోధకుడు మిమ్మల్ని అభ్యర్థించాడు, దయచేసి దీన్ని ఇక్కడ టైప్ చేయండి లేదా చివరి సెషన్ ముగిసినప్పుడు డౌన్‌లోడ్ చేసిన ఫైల్‌ను అప్‌లోడ్ చేయండి.",
    kn: "ಹಿಂದಿನ ಅಧಿವೇಶನದಿಂದ ನಿಮ್ಮ ಸುಲಭವಾದ ID ಯನ್ನು ಒದಗಿಸಲು ಸಂಶೋಧಕರು ನಿಮ್ಮನ್ನು ವಿನಂತಿಸಿದ್ದಾರೆ, ದಯವಿಟ್ಟು ಅದನ್ನು ಇಲ್ಲಿ ಟೈಪ್ ಮಾಡಿ, ಅಥವಾ ಕೊನೆಯ ಅಧಿವೇಶನ ಮುಗಿದಾಗ ಡೌನ್‌ಲೋಡ್ ಮಾಡಿದ ಫೈಲ್ ಅನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.",
    bn: "গবেষক আপনাকে পূর্ববর্তী সেশন থেকে আপনার ইজিয়েয়েস আইডি সরবরাহ করার জন্য অনুরোধ করেছিলেন, দয়া করে এটি এখানে টাইপ করুন বা শেষ সেশনটি শেষ হলে ডাউনলোড করা ফাইলটি আপলোড করুন।",
    id: "Peneliti meminta Anda untuk memberikan ID EyyEyes Anda dari sesi sebelumnya, silakan ketik di sini, atau unggah file yang diunduh saat sesi terakhir berakhir.",
    "zh-CN":
      "研究人员要求您提供上一个会话中的EasyEyes ID，请在此处输入，或在上次会话结束时上传下载的文件。",
    "zh-HK":
      "研究人員要求您提供上一個會話中的EasyEyes ID，請在此處輸入，或在上次會話結束時上傳下載的文件。",
    ja: "研究者は、前のセッションからEasyEyes IDを提供するように要求しました。ここに入力するか、最後のセッションが終了したらダウンロードしたファイルをアップロードしてください。",
    ko: "연구원은 이전 세션에서 EasyEyes ID를 제공하거나 여기에 입력하거나 마지막 세션이 종료되면 다운로드 한 파일을 업로드하도록 요청했습니다.",
  },
  EE_ID: {
    "en-US":
      'A unique ID is needed to link data across sessions. This computer previously participated in a session on **ddd** with EasyEyes ID “**sss**”. Press OK if that\'s you. Otherwise, if you have an EasyEyes ID file, press the file selection button below to open it. (The filename begins with "EasyEyes", ends with ".txt", and is typically in the Downloads folder.) If you don\'t have a file, but know your EasyEyes ID, then type it. Otherwise, just make up your own ID. It can be any alphanumeric string at least 5 characters long. We\'ll remember it for you on this computer, but if you go to another computer, please take your EasyEyes ID file (the most recent file in your Downloads folder), or at least write down your EasyEyes ID. Use it every time to link your data from session to session.',
    de: 'Eine eindeutige ID ist erforderlich, um Daten über Sitzungen hinweg zu verknüpfen. Dieser Computer nahm zuvor an einer Sitzung auf ** ddd ** mit Easyeyes ID "** SSS **" teil. Drücken Sie OK, wenn Sie das sind. Wenn Sie eine Easyeyes -ID -Datei haben, drücken Sie die Dateiauswahltaste unten, um sie zu öffnen. (Der Dateiname beginnt mit "Easyeyes", endet mit ".txt" und befindet sich normalerweise im Ordner Downloads.) Wenn Sie keine Datei haben, aber Ihre Easyeyes -ID kennen, geben Sie sie dann ein. Ansonsten machen Sie einfach Ihre eigene ID. Es kann jede alphanumerische Zeichenfolge mindestens 5 Zeichen lang sein. Wir werden es für Sie auf diesem Computer erinnern, aber wenn Sie zu einem anderen Computer gehen, nehmen Sie bitte Ihre Easyeyes -ID -Datei (die neueste Datei in Ihrem Download -Ordner) oder schreiben Sie zumindest Ihre Easyeyes -ID auf. Verwenden Sie es jedes Mal, um Ihre Daten von Sitzung mit der Sitzung zu verknüpfen.',
    fr: "Un ID unique est nécessaire pour relier les données entre les sessions. Cet ordinateur avait précédemment participé à une session sur ** ddd ** avec EasyEyes ID «** SSS **». Appuyez sur OK si c'est vous. Sinon, si vous avez un fichier ID EasyEyes, appuyez sur le bouton de sélection de fichiers ci-dessous pour l'ouvrir. (Le nom de fichier commence par \"EasyEyes\", se termine par \".txt\" et est généralement dans le dossier des téléchargements.) Si vous n'avez pas de fichier, mais connaissez votre identifiant EasyEyes, alors tapez-le. Sinon, inventez simplement votre propre pièce d'identité. Il peut s'agir de n'importe quelle chaîne alphanumérique au moins 5 caractères. Nous nous en souviendrons pour vous sur cet ordinateur, mais si vous allez sur un autre ordinateur, veuillez prendre votre fichier ID EasyEyes (le fichier le plus récent de votre dossier de téléchargements), ou au moins noter votre identifiant EasyEyes. Utilisez-le à chaque fois pour relier vos données de la session à la session.",
    es: 'Se necesita una identificación única para vincular los datos entre las sesiones. Esta computadora participó previamente en una sesión en ** DDD ** con EasyEyes ID "** SSS **". Presione OK si ese eres tú. De lo contrario, si tiene un archivo de ID de EasyEyes, presione el botón de selección de archivos a continuación para abrirlo. (El nombre de archivo comienza con "EasyEyes", termina con ".txt", y generalmente está en la carpeta de descargas). Si no tiene un archivo, pero conozca su ID de EasyEyes, luego escribalo. De lo contrario, simplemente cree su propia identificación. Puede ser cualquier cadena alfanumérica de al menos 5 caracteres de largo. Lo recordaremos por usted en esta computadora, pero si va a otra computadora, tome su archivo de ID de EasyEyes (el archivo más reciente en su carpeta de descargas) o al menos escriba su ID de EasyEyes. Úselo cada vez para vincular sus datos de sesión a sesión.',
    pt: 'É necessário um ID exclusivo para vincular dados nas sessões. Este computador participou anteriormente de uma sessão em ** ddd ** com o Easyeyes ID “** SSS **”. Pressione OK se for você. Caso contrário, se você tiver um arquivo de identificação easyeyes, pressione o botão Seleção de arquivo abaixo para abri -lo. (O nome do arquivo começa com "easyeyes", termina com ".txt" e normalmente está na pasta de downloads.) Se você não tiver um arquivo, mas conheça o seu ID easyeyes, digite -o. Caso contrário, basta inventar sua própria identificação. Pode ter qualquer corda alfanumérica com pelo menos 5 caracteres. Lembraremos disso para você neste computador, mas se você for a outro computador, pegue seu arquivo de identificação Easyeyes (o arquivo mais recente da sua pasta de downloads) ou pelo menos anote o seu ID easyeyes. Use -o sempre para vincular seus dados de sessão à sessão.',
    it: 'È necessario un ID univoco per collegare i dati tra le sessioni. Questo computer aveva precedentemente partecipato a una sessione su ** ddd ** con ID EasyEyes “** sss **”. Premi OK se sei tu. Altrimenti, se si dispone di un file ID EasyEyes, premere il pulsante di selezione del file in basso per aprirlo. (Il nome file inizia con "EasyEyes", termina con ".txt" ed è in genere nella cartella dei download.) Se non hai un file, ma conosci il tuo ID EasyEyes, quindi digitalo. Altrimenti, basta creare il tuo documento d\'identità. Può essere qualsiasi stringa alfanumerica lunga almeno 5 caratteri. Lo ricorderemo per te su questo computer, ma se vai a un altro computer, prendi il tuo file ID EasyEyes (il file più recente nella cartella dei download) o almeno scrivi il tuo ID EasyEyes. Usalo ogni volta per collegare i tuoi dati da sessione alla sessione.',
    ro: "Un ID unic este necesar pentru a conecta datele în sesiuni. Acest computer a participat anterior la o sesiune pe ** DDD ** cu ID -ul EasyEyes „** SSS **”. Apăsați OK dacă sunteți tu. În caz contrar, dacă aveți un fișier ID EAYEYES, apăsați butonul de selecție a fișierului de mai jos pentru a -l deschide. . În caz contrar, doar alcătuiește -ți propriul ID. Poate fi orice șir alfanumeric de cel puțin 5 caractere. Ne vom aminti pentru dvs. pe acest computer, dar dacă mergeți pe un alt computer, vă rugăm să luați fișierul dvs. de id EasyEs (cel mai recent fișier din folderul dvs. de descărcări) sau cel puțin să scrieți ID -ul dvs. EasyEs. Utilizați -l de fiecare dată pentru a conecta datele dvs. de la sesiune la sesiune.",
    pl: "Potrzebny jest unikalny identyfikator, aby połączyć dane między sesjami. Ten komputer wcześniej uczestniczył w sesji na ** DDD ** z ID Easyeyes „** SSS **”. Naciśnij OK, jeśli to ty. W przeciwnym razie, jeśli masz plik ID Easyeyes, naciśnij poniższy przycisk Plik, aby go otworzyć. (Nazwa pliku zaczyna się od „Easyeyes”, kończy „.txt” i zazwyczaj znajduje się w folderze pobierania.) Jeśli nie masz pliku, ale znasz identyfikator Easyeyes, wpisz go. W przeciwnym razie po prostu zrób swój własny identyfikator. Może to być dowolny ciąg alfanumeryczny o długości co najmniej 5 znaków. Zapamiętamy to dla Ciebie na tym komputerze, ale jeśli przejdziesz do innego komputera, weź plik ID Easyeyes (najnowszy plik w folderze do pobrania) lub przynajmniej zapisz swój identyfikator Easyeyes. Użyj go za każdym razem, aby połączyć dane z sesji do sesji.",
    ru: "Уникальный идентификатор необходим для связи данных в разных сеансах. Этот компьютер ранее участвовал в сеансе на ** ddd ** с easyeyes id «** sss **». Нажмите ОК, если это ты. В противном случае, если у вас есть файл идентификатора easyeyes, нажмите кнопку выбора файла ниже, чтобы открыть его. (Имя файла начинается с «Easyeyes», заканчивается «.txt» и обычно находится в папке загрузки.) Если у вас нет файла, но вы знаете свой идентификатор easyeyes, то введите его. В противном случае, просто придумайте свой собственный идентификатор. Это может быть любая буквенно -цифровая строка не менее 5 символов. Мы запомним это для вас на этом компьютере, но если вы зайдете на другой компьютер, пожалуйста, возьмите свой файл идентификатора easteyes (самый последний файл в папке загрузки) или, по крайней мере, запишите свой идентификатор easyeyes. Используйте его каждый раз, чтобы связать ваши данные из сеанса с сеансом.",
    hy: "Նիստերը կապելու համար անհրաժեշտ է եզակի ID: Այս համակարգիչը նախկինում մասնակցել է «DDD ** - ին,« ** SSS ** »- ին ** DDD ** նիստին: Սեղմեք OK, եթե դա ձեզ է: Հակառակ դեպքում, եթե ունեք EasyEyes ID ֆայլ, սեղմեք ներքեւի ֆայլի ընտրության կոճակը, այն բացելու համար: (Անունը սկսվում է «EasyEyes» - ի հետ, ավարտվում է «.txt» - ով եւ սովորաբար ներբեռնման պանակում է :) Եթե ֆայլ չունեք, բայց մուտքագրեք այն: Հակառակ դեպքում, պարզապես կազմեք ձեր սեփական ID- ն: Դա կարող է լինել ցանկացած այբբենական տող առնվազն 5 նիշ: Մենք դա կհիշենք ձեզ համար այս համակարգչում, բայց եթե մեկ այլ համակարգիչ եք գնում, խնդրում ենք վերցրեք ձեր EasyEyes ID ֆայլը (ձեր ներբեռնման պանակում ամենավերջին ֆայլը), կամ գոնե գրեք ձեր EasyEyes ID- ն: Օգտագործեք այն ամեն անգամ `ձեր տվյալները նիստից նիստին կապելու համար:",
    fi: 'Tietojen linkittämiseen tarvitaan ainutlaatuinen tunnus. Tämä tietokone osallistui aiemmin ** DDD **: n istuntoon, jossa on Easyeyes ID “** SSS **”. Paina OK, jos se olet sinä. Muuten, jos sinulla on Easyeyes ID -tiedosto, avata se alla olevaa tiedostovalintapainiketta. (Tiedostonimi alkaa "Easyeyes", päättyy ".txt", ja se on tyypillisesti latauskansiossa.) Jos sinulla ei ole tiedostoa, mutta tiedä Easyeyes -tunnuksesi, kirjoita se sitten. Muuten muodosta vain oma henkilöllisyystodistus. Se voi olla mikä tahansa aakkosnumeerinen merkkijono vähintään 5 merkkiä pitkä. Muistamme sen sinulle tällä tietokoneella, mutta jos siirryt toiseen tietokoneeseen, ota Easyeyes ID -tiedosto (viimeisin tiedosto latauskansioon) tai ainakin kirjoita Easyeyes -tunnus. Käytä sitä joka kerta linkittääksesi tietosi istunnosta toiseen.',
    ka: 'უნიკალური პირადობის მოწმობაა საჭირო მონაცემების სესიების დასაკავშირებლად. ეს კომპიუტერი ადრე მონაწილეობდა სხდომაზე ** ddd ** easyeyes id "** sss **". დააჭირეთ OK- ს, თუ ეს შენ ხარ. წინააღმდეგ შემთხვევაში, თუ თქვენ გაქვთ EasyEyes ID ფაილი, დააჭირეთ ქვემოთ მოცემულ ფაილის შერჩევის ღილაკს, რომ გახსნათ იგი. (ფაილის სახელი იწყება "Easyeyes", მთავრდება ".txt" - ით და, როგორც წესი, გადმოტვირთვის საქაღალდეშია.) თუ ფაილი არ გაქვთ, მაგრამ იცოდეთ თქვენი EasyEyes ID, შემდეგ ჩაწერეთ იგი. წინააღმდეგ შემთხვევაში, უბრალოდ შეადგინეთ საკუთარი პირადობის მოწმობა. ეს შეიძლება იყოს ნებისმიერი ალფანუმერული სიმებიანი მინიმუმ 5 სიმბოლო. ჩვენ ეს მახსოვს თქვენთვის ამ კომპიუტერში, მაგრამ თუ სხვა კომპიუტერში მიდიხართ, გთხოვთ, აიღოთ თქვენი EasyEyes ID ფაილი (უახლესი ფაილი თქვენს ჩამოტვირთვის საქაღალდეში), ან მინიმუმ ჩამოწერეთ თქვენი EasyEyes ID. გამოიყენეთ იგი ყოველ ჯერზე, რომ დააკავშიროთ თქვენი მონაცემები სესიიდან სესიამდე.',
    he: 'יש צורך במזהה ייחודי לקישור נתונים על פני הפעלות. מחשב זה השתתף בעבר בסשן ב- ** DDD ** עם IDEEIES ID "** SSS **". לחץ על אישור אם זה אתה. אחרת, אם יש לך קובץ מזהה EasyEyes, לחץ על כפתור בחירת הקבצים למטה כדי לפתוח אותו. (שם הקובץ מתחיל ב"קלח ", מסתיים ב-" .txt ", והוא בדרך כלל בתיקיית ההורדות.) אם אין לך קובץ, אך דע את מזהה ה- EasyEyes שלך, אז הקלד אותו. אחרת, פשוט הכינו תעודת זהות משלכם. זה יכול להיות כל מחרוזת אלפא -נומרית באורך של לפחות 5 תווים. אנו נזכור זאת עבורך במחשב זה, אך אם אתה הולך למחשב אחר, אנא קח את קובץ המזהה של EasyEyes שלך (הקובץ האחרון בתיקיית ההורדות שלך), או לפחות לרשום את מזהה ה- EasyEyes שלך. השתמש בו בכל פעם כדי לקשר את הנתונים שלך מהפעלה לפגישה.',
    ar: 'هناك حاجة إلى معرف فريد لربط البيانات عبر الجلسات. شارك هذا الكمبيوتر سابقًا في جلسة على ** DDD ** مع معرف Easyeyes "** SSS **". اضغط موافق إذا كان هذا أنت. خلاف ذلك ، إذا كان لديك ملف معرف Easyeyes ، فاضغط على زر تحديد الملف أدناه لفتحه. (يبدأ اسم الملف بـ "Easyeyes" ، ينتهي بـ ".txt" ، وعادة ما يكون في مجلد التنزيلات.) إذا لم يكن لديك ملف ، ولكن تعرف على معرف Easyeyes الخاص بك ، ثم اكتبه. خلاف ذلك ، فقط قم بتكوين هويتك الخاصة. يمكن أن يكون أي سلسلة أبجدية رقمية لا تقل عن 5 أحرف. سنتذكر ذلك لك على هذا الكمبيوتر ، ولكن إذا ذهبت إلى جهاز كمبيوتر آخر ، فيرجى أخذ ملف معرف Easyeyes الخاص بك (أحدث ملف في مجلد التنزيلات) ، أو على الأقل اكتب معرف Easyeyes الخاص بك. استخدمه في كل مرة لربط بياناتك من جلسة إلى جلسة.',
    ur: 'سیشنوں میں ڈیٹا کو جوڑنے کے لئے ایک انوکھا ID کی ضرورت ہے۔ اس کمپیوٹر نے اس سے قبل ** ڈی ڈی ڈی ** پر سیشن میں Easyeyes ID "** SSS **" کے ساتھ حصہ لیا تھا۔ ٹھیک ہے دبائیں اگر آپ ہیں۔ بصورت دیگر ، اگر آپ کے پاس ایزی ای ایس آئی ڈی فائل ہے تو ، اسے کھولنے کے لئے نیچے فائل سلیکشن بٹن دبائیں۔ ۔ بصورت دیگر ، صرف اپنی شناخت بنائیں۔ یہ کم از کم 5 حروف لمبا کوئی بھی حرفی عمر کا تار ہوسکتا ہے۔ ہم اسے اس کمپیوٹر پر آپ کے لئے یاد رکھیں گے ، لیکن اگر آپ کسی دوسرے کمپیوٹر پر جاتے ہیں تو ، براہ کرم اپنی ایزیئس آئی ڈی فائل (اپنے ڈاؤن لوڈ فولڈر میں حالیہ فائل) لیں ، یا کم از کم اپنے ایزی ای ایس آئی ڈی کو لکھیں۔ اپنے ڈیٹا کو سیشن سے سیشن میں لنک کرنے کے لئے ہر بار اس کا استعمال کریں۔',
    hi: 'सत्रों में डेटा को लिंक करने के लिए एक अद्वितीय आईडी की आवश्यकता होती है। इस कंप्यूटर ने पहले Easyeyes ID "** SSS **" के साथ ** ddd ** पर एक सत्र में भाग लिया था। यदि आप हो तो ठीक दबाएं। अन्यथा, यदि आपके पास एक EasyEyes ID फ़ाइल है, तो इसे खोलने के लिए नीचे दिए गए फ़ाइल चयन बटन दबाएँ। (फ़ाइल नाम "easyeyes" के साथ शुरू होता है, ".txt" के साथ समाप्त होता है, और आमतौर पर डाउनलोड फ़ोल्डर में होता है।) यदि आपके पास फ़ाइल नहीं है, लेकिन अपनी Easyeyes ID को जानें, तो इसे टाइप करें। अन्यथा, बस अपनी खुद की आईडी बनाएं। यह कम से कम 5 वर्णों के किसी भी अल्फ़ान्यूमेरिक स्ट्रिंग हो सकता है। हम इसे इस कंप्यूटर पर आपके लिए याद रखेंगे, लेकिन यदि आप किसी अन्य कंप्यूटर पर जाते हैं, तो कृपया अपनी Easyeyes ID फ़ाइल (अपने डाउनलोड फ़ोल्डर में सबसे हालिया फ़ाइल) लें, या कम से कम अपनी Easyeyes ID लिखें। अपने डेटा को सत्र से सत्र तक लिंक करने के लिए हर बार इसका उपयोग करें।',
    ta: "அமர்வுகள் முழுவதும் தரவை இணைக்க ஒரு தனித்துவமான ஐடி தேவை. இந்த கணினி முன்பு ** ddd ** இல் ஒரு அமர்வில் பங்கேற்றது “** sss **”. அது நீங்கள் என்றால் சரி என்பதை அழுத்தவும். இல்லையெனில், உங்களிடம் எளிதான ஐடி கோப்பு இருந்தால், அதைத் திறக்க கீழே உள்ள கோப்பு தேர்வு பொத்தானை அழுத்தவும். . இல்லையெனில், உங்கள் சொந்த ஐடியை உருவாக்குங்கள். இது குறைந்தது 5 எழுத்துக்கள் நீளமாக எந்த எண்ணெழுத்து சரமாக இருக்கலாம். இந்த கணினியில் நாங்கள் அதை நினைவில் கொள்வோம், ஆனால் நீங்கள் வேறொரு கணினிக்குச் சென்றால், தயவுசெய்து உங்கள் ஈஸி ஐடி கோப்பை (உங்கள் பதிவிறக்கக் கோப்புறையில் மிகச் சமீபத்திய கோப்பு) எடுத்துக் கொள்ளுங்கள், அல்லது குறைந்தபட்சம் உங்கள் ஈஸியீஸ் ஐடியை எழுதுங்கள். உங்கள் தரவை அமர்விலிருந்து அமர்வுக்கு இணைக்க ஒவ்வொரு முறையும் இதைப் பயன்படுத்தவும்.",
    ml: 'സെഷനുകളിലുടനീളം ഡാറ്റ ലിങ്കുചെയ്യുന്നതിന് ഒരു അദ്വിതീയ ഐഡി ആവശ്യമാണ്. ഈ കമ്പ്യൂട്ടർ മുമ്പ് ** ddd- ൽ ഒരു സെഷനിൽ പങ്കെടുത്തു ** anieseyes id "** SSS **". അത് നിങ്ങളാണെങ്കിൽ ശരി അമർത്തുക. അല്ലെങ്കിൽ, നിങ്ങൾക്ക് ഒരു എളുപ്പമുള്ള ഐഡി ഫയൽ ഉണ്ടെങ്കിൽ, അത് തുറക്കുന്നതിന് ചുവടെയുള്ള ഫയൽ തിരഞ്ഞെടുക്കൽ ബട്ടൺ അമർത്തുക. . അല്ലെങ്കിൽ, നിങ്ങളുടെ സ്വന്തം ഐഡി നിർമ്മിക്കുക. കുറഞ്ഞത് 5 പ്രതീകങ്ങളെങ്കിലും ദൈർഘ്യമേറിയ ഏതെങ്കിലും ആൽഫാന്യൂമെറിക് സ്ട്രിംഗും ഇത് ആകാം. ഈ കമ്പ്യൂട്ടറിൽ ഞങ്ങൾ ഇത് ഓർക്കും, പക്ഷേ നിങ്ങൾ മറ്റൊരു കമ്പ്യൂട്ടറിലേക്ക് പോയാൽ, ദയവായി നിങ്ങളുടെ ഐഡി ഫയൽ (നിങ്ങളുടെ ഡ download ൺലോഡുകൾ ഫോൾഡറിലെ ഏറ്റവും പുതിയ ഫയൽ എടുക്കുക), അല്ലെങ്കിൽ നിങ്ങളുടെ എളുപ്പത്തിലുള്ള ഐഡി എഴുതുക. നിങ്ങളുടെ ഡാറ്റ സെഷനിൽ നിന്ന് സെഷനിൽ നിന്ന് ലിങ്കുചെയ്യുന്നതിന് ഓരോ തവണയും ഉപയോഗിക്കുക.',
    te: "సెషన్లలో డేటాను లింక్ చేయడానికి ఒక ప్రత్యేకమైన ID అవసరం. ఈ కంప్యూటర్ గతంలో ఈజీయెస్ ఐడి “** SSS **” తో ** DDD ** లో ఒక సెషన్‌లో పాల్గొంది. అది మీరే అయితే సరే నొక్కండి. లేకపోతే, మీకు ఈజీయేస్ ఐడి ఫైల్ ఉంటే, దాన్ని తెరవడానికి దిగువ ఫైల్ ఎంపిక బటన్‌ను నొక్కండి. . లేకపోతే, మీ స్వంత ఐడిని తయారు చేయండి. ఇది కనీసం 5 అక్షరాల పొడవు ఏదైనా ఆల్ఫాన్యూమరిక్ స్ట్రింగ్ కావచ్చు. మేము ఈ కంప్యూటర్‌లో మీ కోసం దీన్ని గుర్తుంచుకుంటాము, కానీ మీరు మరొక కంప్యూటర్‌కు వెళితే, దయచేసి మీ ఈజీయెస్ ఐడి ఫైల్‌ను (మీ డౌన్‌లోడ్ ఫోల్డర్‌లో ఇటీవలి ఫైల్) తీసుకోండి లేదా కనీసం మీ ఈజీయెస్ ఐడిని రాయండి. మీ డేటాను సెషన్ నుండి సెషన్‌కు లింక్ చేయడానికి ప్రతిసారీ దీన్ని ఉపయోగించండి.",
    kn: "ಸೆಷನ್‌ಗಳಲ್ಲಿ ಡೇಟಾವನ್ನು ಲಿಂಕ್ ಮಾಡಲು ಅನನ್ಯ ID ಅಗತ್ಯವಿದೆ. ಈ ಕಂಪ್ಯೂಟರ್ ಈ ಹಿಂದೆ ** ಡಿಡಿಡಿ ** ನಲ್ಲಿನ ಅಧಿವೇಶನದಲ್ಲಿ ಈಸಿ ಐಸ್ ಐಡಿ “** ಎಸ್‌ಎಸ್‌ಎಸ್ **” ನೊಂದಿಗೆ ಭಾಗವಹಿಸಿದೆ. ಅದು ನೀವೇ ಆಗಿದ್ದರೆ ಸರಿ ಒತ್ತಿರಿ. ಇಲ್ಲದಿದ್ದರೆ, ನೀವು ಸುಲಭವಾದ ಐಡಿ ಫೈಲ್ ಹೊಂದಿದ್ದರೆ, ಅದನ್ನು ತೆರೆಯಲು ಕೆಳಗಿನ ಫೈಲ್ ಆಯ್ಕೆ ಬಟನ್ ಒತ್ತಿರಿ. . ಇಲ್ಲದಿದ್ದರೆ, ನಿಮ್ಮ ಸ್ವಂತ ID ಯನ್ನು ಮಾಡಿ. ಇದು ಕನಿಷ್ಠ 5 ಅಕ್ಷರಗಳಷ್ಟು ಉದ್ದವಾದ ಯಾವುದೇ ಆಲ್ಫಾನ್ಯೂಮರಿಕ್ ಸ್ಟ್ರಿಂಗ್ ಆಗಿರಬಹುದು. ಈ ಕಂಪ್ಯೂಟರ್‌ನಲ್ಲಿ ನಾವು ಅದನ್ನು ನಿಮಗಾಗಿ ನೆನಪಿಸಿಕೊಳ್ಳುತ್ತೇವೆ, ಆದರೆ ನೀವು ಇನ್ನೊಂದು ಕಂಪ್ಯೂಟರ್‌ಗೆ ಹೋದರೆ, ದಯವಿಟ್ಟು ನಿಮ್ಮ ಈಸಿಐಇಎಸ್ ಐಡಿ ಫೈಲ್ ಅನ್ನು ತೆಗೆದುಕೊಳ್ಳಿ (ನಿಮ್ಮ ಡೌನ್‌ಲೋಡ್‌ಗಳ ಫೋಲ್ಡರ್‌ನಲ್ಲಿರುವ ಇತ್ತೀಚಿನ ಫೈಲ್), ಅಥವಾ ಕನಿಷ್ಠ ನಿಮ್ಮ ಸುಲಭ ಐಡಿಯನ್ನು ಬರೆಯಿರಿ. ನಿಮ್ಮ ಡೇಟಾವನ್ನು ಅಧಿವೇಶನದಿಂದ ಅಧಿವೇಶನಕ್ಕೆ ಲಿಂಕ್ ಮಾಡಲು ಪ್ರತಿ ಬಾರಿಯೂ ಇದನ್ನು ಬಳಸಿ.",
    bn: 'সেশনগুলি জুড়ে ডেটা লিঙ্ক করার জন্য একটি অনন্য আইডি প্রয়োজন। এই কম্পিউটারটি পূর্বে ** ডিডিডি ** ইজিয়েয়েস আইডি "** এসএসএস **" এর সাথে একটি সেশনে অংশ নিয়েছিল। ঠিক আছে টিপুন যদি আপনি হয়। অন্যথায়, আপনার যদি কোনও ইজিয়েইস আইডি ফাইল থাকে তবে এটি খুলতে নীচে ফাইল নির্বাচন বোতাম টিপুন। (ফাইলের নামটি "ইজিয়েয়েস" দিয়ে শুরু হয়, ".txt" দিয়ে শেষ হয় এবং সাধারণত ডাউনলোড ফোল্ডারে থাকে)) আপনার যদি ফাইল না থাকে তবে আপনার ইজিয়েস আইডিটি জানুন, তবে এটি টাইপ করুন। অন্যথায়, কেবল আপনার নিজের আইডি তৈরি করুন। এটি কমপক্ষে 5 টি অক্ষর দীর্ঘ কোনও আলফানিউমেরিক স্ট্রিং হতে পারে। আমরা এই কম্পিউটারে এটি আপনার জন্য মনে রাখব, তবে আপনি যদি অন্য কোনও কম্পিউটারে যান তবে দয়া করে আপনার ইজিয়েয়েস আইডি ফাইলটি নিন (আপনার ডাউনলোড ফোল্ডারে সর্বাধিক সাম্প্রতিক ফাইল), বা কমপক্ষে আপনার ইজিয়েস আইডি লিখুন। আপনার ডেটা সেশন থেকে সেশনে লিঙ্ক করতে প্রতিবার এটি ব্যবহার করুন।',
    id: 'ID unik diperlukan untuk menghubungkan data lintas sesi. Komputer ini sebelumnya berpartisipasi dalam sesi di ** DDD ** dengan ID EyyEyes “** SSS **”. Tekan OK jika itu Anda. Kalau tidak, jika Anda memiliki file ID EXYEYES, tekan tombol pemilihan file di bawah ini untuk membukanya. (Nama file dimulai dengan "EyyEyes", diakhiri dengan ".txt", dan biasanya di folder unduhan.) Jika Anda tidak memiliki file, tetapi ketahui ID easyEyes Anda, lalu ketik. Kalau tidak, buat saja ID Anda sendiri. Ini bisa berupa string alfanumerik setidaknya 5 karakter. Kami akan mengingatnya untuk Anda di komputer ini, tetapi jika Anda pergi ke komputer lain, silakan ambil file ID EyyEyes Anda (file terbaru di folder unduhan Anda), atau setidaknya tuliskan ID EyyEyes Anda. Gunakan setiap kali untuk menautkan data Anda dari sesi ke sesi.',
    "zh-CN":
      "需要一个唯一的ID来链接跨会话的数据。这台计算机以前参加了有关easyeyes id“ ** sss **”的** ddd **的会话。如果是你，请按确定。否则，如果您有EasyEyes ID文件，请按下面的文件选择按钮打开它。 （该文件名以“ EasyEyes”开头，以“ .txt”结尾，通常在下载文件夹中。）如果您没有文件，但是知道您的EasyEyes ID，则输入。否则，只需弥补自己的身份证即可。它可以是至少5个字符的任何字母数字字符串。我们会在这台计算机上为您记住它，但是如果您转到另一台计算机，请获取您的EasyEyes ID文件（您的下载文件夹中的最新文件），或者至少写下您的EasyEyes ID。每次使用它将您的数据从会话链接到会话。",
    "zh-HK":
      "需要一個唯一的ID來鏈接跨會話的數據。這台計算機以前參加了有關easyeyes id“ ** sss **”的** ddd **的會話。如果是你，請按確定。否則，如果您有EasyEyes ID文件，請按下面的文件選擇按鈕打開它。 （該文件名以“ EasyEyes”開頭，以“ .txt”結尾，通常在下載文件夾中。）如果您沒有文件，但是知道您的EasyEyes ID，則輸入。否則，只需彌補自己的身份證即可。它可以是至少5個字符的任何字母數字字符串。我們會在這台計算機上為您記住它，但是如果您轉到另一台計算機，請獲取您的EasyEyes ID文件（您的下載文件夾中的最新文件），或者至少寫下您的EasyEyes ID。每次使用它將您的數據從會話鏈接到會話。",
    ja: "セッション全体でデータをリンクするには、一意のIDが必要です。このコンピューターは、以前に** ddd **のセッションに参加しました。それがあなたである場合、OKを押してください。それ以外の場合は、EasyEyes IDファイルがある場合は、下のファイル選択ボタンを押して開きます。 （ファイル名は「easyeyes」で始まり、「.txt」で終わり、通常はダウンロードフォルダーにあります。）ファイルを持っていないが、easeyeyes IDを知っている場合は、入力します。それ以外の場合は、独自のIDを作成します。少なくとも5文字の長さの英数字の文字列にすることができます。このコンピューターでは覚えていますが、別のコンピューターにアクセスした場合は、EasyEyes IDファイル（ダウンロードフォルダーの最新のファイル）を使用するか、少なくともEasyEyes IDを書き留めてください。セッションからセッションにデータをリンクするために毎回使用してください。",
    ko: '세션에서 데이터를 연결하려면 고유 한 ID가 필요합니다. 이 컴퓨터는 이전에 Easyeyes id“** sss **”와 함께 ** ddd ** 세션에 참여했습니다. 당신이라면 확인을 누릅니다. 그렇지 않으면 EasyEeyes ID 파일이있는 경우 아래 파일 선택 버튼을 눌러 열 수 있습니다. (파일 이름은 "easyeyes"로 시작하여 ".txt"로 끝나고 일반적으로 다운로드 폴더에 있습니다.) 파일이 없지만 Easyeyes ID를 알고 있다면 입력하십시오. 그렇지 않으면 자신의 ID를 구성하십시오. 최소 5 자 이상의 영숫자 문자열 일 수 있습니다. 이 컴퓨터에서는 기억하지만 다른 컴퓨터로 이동하면 Easyeyes ID 파일 (다운로드 폴더의 최신 파일)을 가져 가거나 최소한 Easyeyes ID를 적어 두십시오. 데이터를 세션에서 세션으로 연결하기 위해 매번 사용하십시오.',
  },
  EE_ID_uploadOrValidID: {
    "en-US": "Please either upload a file or enter a valid EasyEyes ID.",
    de: "Bitte laden Sie entweder eine Datei hoch oder geben Sie eine gültige Easyeyes -ID ein.",
    fr: "Veuillez télécharger un fichier ou entrer un ID EasyEyes valide.",
    es: "Sube un archivo o ingrese una ID de EasyEyes válida.",
    pt: "Faça o upload de um arquivo ou digite um ID do Easyeyes válido.",
    it: "Carica un file o inserisci un ID EasyEyes valido.",
    ro: "Vă rugăm să încărcați un fișier sau să introduceți un ID Valid EasyEyes.",
    pl: "Prześlij plik lub wprowadź prawidłowy identyfikator Easyeyes.",
    ru: "Пожалуйста, загрузите файл, либо введите действительный идентификатор easyeyes.",
    hy: "Խնդրում ենք կամ ֆայլ վերբեռնել կամ մուտքագրել վավեր EasyEyes ID:",
    fi: "Ole hyvä ja lataa tiedosto tai kirjoita kelvollinen Easyeyes -tunnus.",
    ka: "გთხოვთ, ატვირთოთ ფაილი ან შეიყვანოთ მოქმედი EasyEyes ID.",
    he: "אנא העלה קובץ או הזן מזהה EasyEyes תקף.",
    ar: "يرجى إما تحميل ملف أو إدخال معرف Easyeyes صالح.",
    ur: "براہ کرم یا تو فائل اپ لوڈ کریں یا ایک درست ایزی ای ایس آئی ڈی درج کریں۔",
    hi: "कृपया या तो एक फ़ाइल अपलोड करें या एक मान्य easyeyes आईडी दर्ज करें।",
    ta: "தயவுசெய்து ஒரு கோப்பைப் பதிவேற்றவும் அல்லது செல்லுபடியாகும் எளிதான ஐடியை உள்ளிடவும்.",
    ml: "ഒന്നുകിൽ ഒരു ഫയൽ അപ്ലോഡ് ചെയ്യുക അല്ലെങ്കിൽ സാധുവായ ഈസിയേൽ ഐഡി നൽകുക.",
    te: "దయచేసి ఫైల్‌ను అప్‌లోడ్ చేయండి లేదా చెల్లుబాటు అయ్యే ఈజీస్ ఐడిని నమోదు చేయండి.",
    kn: "ದಯವಿಟ್ಟು ಫೈಲ್ ಅನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ ಅಥವಾ ಮಾನ್ಯ ಸುಲಭವಾದ ಐಡಿಯನ್ನು ನಮೂದಿಸಿ.",
    bn: "হয় হয় একটি ফাইল আপলোড করুন বা একটি বৈধ ইজিয়েয়েস আইডি লিখুন।",
    id: "Harap unggah file atau masukkan ID EyyEyes yang valid.",
    "zh-CN": "请上传文件或输入有效的EasyEyes ID。",
    "zh-HK": "請上傳文件或輸入有效的EasyEyes ID。",
    ja: "ファイルをアップロードするか、有効なEasyEyes IDを入力してください。",
    ko: "파일을 업로드하거나 유효한 Easyeyes ID를 입력하십시오.",
  },
  EE_ID_invalidID: {
    "en-US":
      "The EasyEyes ID contains invalid characters. Only letters and numbers are allowed",
    de: "Die Easyeyes -ID enthält ungültige Zeichen. Es sind nur Buchstaben und Zahlen erlaubt",
    fr: "L'ID EasyEyes contient des caractères invalides. Seules les lettres et les chiffres sont autorisés",
    es: "La identificación de EasyEyes contiene caracteres no válidos. Solo se permiten letras y números",
    pt: "O ID do Easyeyes contém caracteres inválidos. Somente letras e números são permitidos",
    it: "L'ID EasyEyes contiene caratteri non validi. Sono consentiti solo lettere e numeri",
    ro: "ID -ul EasyEyes conține caractere nevalide. Sunt permise doar litere și numere",
    pl: "ID Easyeyes zawiera nieprawidłowe znaki. Dozwolone są tylko litery i liczby",
    ru: "Идентификатор easyeyes содержит недействительные символы. Разрешены только буквы и цифры",
    hy: "EasyEyes ID- ն պարունակում է անվավեր նիշ: Թույլատրվում են միայն տառերն ու համարները",
    fi: "Easyeyes -tunnus sisältää virheelliset merkit. Vain kirjaimet ja numerot ovat sallittuja",
    ka: "EasyEyes ID შეიცავს არასწორი სიმბოლოებს. დასაშვებია მხოლოდ ასოები და ციფრები",
    he: "מזהה EasyEyes מכיל תווים לא חוקיים. רק אותיות ומספרים מותרות",
    ar: "يحتوي معرف Easyeyes على أحرف غير صالحة. لا يُسمح سوى الحروف والأرقام",
    ur: "ایزیس آئی ڈی میں غلط حروف شامل ہیں۔ صرف خطوط اور نمبروں کی اجازت ہے",
    hi: "Easyeyes ID में अमान्य वर्ण होते हैं। केवल अक्षरों और संख्याओं की अनुमति है",
    ta: "எளிதான ஐடியில் தவறான எழுத்துக்கள் உள்ளன. கடிதங்கள் மற்றும் எண்கள் மட்டுமே அனுமதிக்கப்படுகின்றன",
    ml: "എളുപ്പമുള്ള ഐഡിയിൽ അസാധുവായ പ്രതീകങ്ങൾ അടങ്ങിയിരിക്കുന്നു. അക്ഷരങ്ങളും അക്കങ്ങളും മാത്രമേ അനുവദിക്കൂ",
    te: "ఈజీయెస్ ఐడిలో చెల్లని అక్షరాలు ఉన్నాయి. అక్షరాలు మరియు సంఖ్యలు మాత్రమే అనుమతించబడతాయి",
    kn: "ಈಸಿ ಐಸ್ ಐಡಿ ಅಮಾನ್ಯ ಅಕ್ಷರಗಳನ್ನು ಒಳಗೊಂಡಿದೆ. ಅಕ್ಷರಗಳು ಮತ್ತು ಸಂಖ್ಯೆಗಳನ್ನು ಮಾತ್ರ ಅನುಮತಿಸಲಾಗಿದೆ",
    bn: "ইজিয়েয়েস আইডিতে অবৈধ অক্ষর রয়েছে। শুধুমাত্র চিঠি এবং সংখ্যা অনুমোদিত",
    id: "ID EYEYEYES berisi karakter yang tidak valid. Hanya huruf dan angka yang diizinkan",
    "zh-CN": "EasyEyes ID包含无效的字符。只允许字母和数字",
    "zh-HK": "EasyEyes ID包含無效的字符。只允許字母和數字",
    ja: "EasyEyes IDには無効な文字が含まれています。文字と番号のみが許可されています",
    ko: "EasyEyes ID에는 잘못된 문자가 포함되어 있습니다. 글자와 숫자 만 허용됩니다",
  },
  EE_ID_invalidFile: {
    "en-US":
      "Sorry. Wrong format for an EasyEyes ID file. Try another file, or type something.",
    de: "Verzeihung. Falsches Format für eine Easyeyes -ID -Datei. Probieren Sie eine andere Datei aus oder geben Sie etwas ein.",
    fr: "Désolé. Mauvais format pour un fichier ID EasyEyes. Essayez un autre fichier ou tapez quelque chose.",
    es: "Lo siento. Formato incorrecto para un archivo de ID de EasyEyes. Pruebe con otro archivo o escriba algo.",
    pt: "Desculpe. Formato errado para um arquivo de identificação easyeyes. Experimente outro arquivo ou digite algo.",
    it: "Scusa. Formato sbagliato per un file ID EasyEyes. Prova un altro file o digita qualcosa.",
    ro: "Îmi pare rău. Format greșit pentru un fișier ID EAYEYES. Încercați un alt fișier sau tastați ceva.",
    pl: "Przepraszam. Niewłaściwy format dla pliku ID Easyeyes. Wypróbuj inny plik lub wpisz coś.",
    ru: "Извини. Неправильный формат для файла идентификации easyeyes. Попробуйте другой файл или что -нибудь введите.",
    hy: "Ներողություն. EasyEyes ID ֆայլի սխալ ձեւաչափը: Փորձեք մեկ այլ ֆայլ կամ ինչ-որ բան մուտքագրեք:",
    fi: "Anteeksi. Väärä muoto Easyeyes -tunnustiedostolle. Kokeile toista tiedostoa tai kirjoita jotain.",
    ka: "Ბოდიში. არასწორი ფორმატი EasyEyes ID ფაილისთვის. სცადეთ სხვა ფაილი, ან ჩაწერეთ რამე.",
    he: "מצטער. פורמט שגוי עבור קובץ מזהה EasyEyes. נסה קובץ אחר, או הקלד משהו.",
    ar: "آسف. تنسيق خاطئ لملف معرف Easyeyes. جرب ملفًا آخر ، أو اكتب شيئًا.",
    ur: "معذرت ایزیئس آئی ڈی فائل کے لئے غلط شکل۔ دوسری فائل آزمائیں ، یا کچھ ٹائپ کریں۔",
    hi: "क्षमा मांगना। एक easyeyes आईडी फ़ाइल के लिए गलत प्रारूप। एक और फ़ाइल का प्रयास करें, या कुछ टाइप करें।",
    ta: "மன்னிக்கவும். எளிதான ஐடி கோப்பிற்கான தவறான வடிவம். மற்றொரு கோப்பை முயற்சிக்கவும் அல்லது ஏதாவது தட்டச்சு செய்யவும்.",
    ml: "ക്ഷമിക്കണം. ഒരു ആശയമയമുള്ള ഐഡി ഫയലിനായി തെറ്റായ ഫോർമാറ്റ്. മറ്റൊരു ഫയൽ പരീക്ഷിക്കുക, അല്ലെങ്കിൽ എന്തെങ്കിലും ടൈപ്പ് ചെയ്യുക.",
    te: "క్షమించండి. ఈజీయెస్ ఐడి ఫైల్ కోసం తప్పు ఫార్మాట్. మరొక ఫైల్‌ను ప్రయత్నించండి లేదా ఏదైనా టైప్ చేయండి.",
    kn: "ಕ್ಷಮಿಸಿ. ಸುಲಭವಾದ ಐಡಿ ಫೈಲ್‌ಗಾಗಿ ತಪ್ಪು ಸ್ವರೂಪ. ಇನ್ನೊಂದು ಫೈಲ್ ಅನ್ನು ಪ್ರಯತ್ನಿಸಿ, ಅಥವಾ ಏನನ್ನಾದರೂ ಟೈಪ್ ಮಾಡಿ.",
    bn: "দুঃখিত ইজিয়েয়েস আইডি ফাইলের জন্য ভুল ফর্ম্যাট। অন্য ফাইল চেষ্টা করুন, বা কিছু টাইপ করুন।",
    id: "Maaf. Format yang salah untuk file ID EXYEYES. Coba file lain, atau ketik sesuatu.",
    "zh-CN":
      "对不起。 easyeyes ID文件的错误格式。尝试另一个文件，或键入某些内容。",
    "zh-HK":
      "對不起。 easyeyes ID文件的錯誤格式。嘗試另一個文件，或鍵入某些內容。",
    ja: "ごめん。 EasyEyes IDファイルの間違った形式。別のファイルを試すか、何かを入力してください。",
    ko: "죄송합니다. EasyEeyes ID 파일에 대한 잘못된 형식. 다른 파일을 시도하거나 무언가를 입력하십시오.",
  },
  EE_incompatible: {
    "en-US": "❌ INCOMPATIBLE. Sorry. Please return this study to Prolific.co.",
    de: "❌ inkompatibel. Verzeihung. Bitte geben Sie diese Studie an produktiv.co zurück.",
    fr: "❌ incompatible. Désolé. Veuillez retourner cette étude sur prolific.co.",
    es: "❌ Incompatible. Lo siento. Devuelva este estudio a Prolific.co.",
    pt: "❌ Incompatível. Desculpe. Por favor, devolva este estudo ao prolific.co.",
    it: "❌ incompatibile. Scusa. Si prega di restituire questo studio a prolific.co.",
    ro: "❌ incompatibil. Îmi pare rău. Vă rugăm să returnați acest studiu la Prolific.co.",
    pl: "❌ Niezgodne. Przepraszam. Zwróć to badanie do Flific.co.",
    ru: "❌ Несовместимый. Извини. Пожалуйста, верните это исследование в Plenific.co.",
    hy: "❌ Անհամատեղելի: Ներողություն. Խնդրում ենք վերադարձնել այս ուսումնասիրությունը Pringific.co- ին:",
    fi: "❌ yhteensopimattomia. Anteeksi. Palauta tämä tutkimus tuotteelle Lensific.co.",
    ka: "❌ შეუთავსებელი. Ბოდიში. გთხოვთ, დააბრუნოთ ეს კვლევა Prolific.co- ზე.",
    he: "❌ לא תואם. מצטער. אנא החזירו מחקר זה ל- ProLific.co.",
    ar: "❌ غير متوافق. آسف. يرجى إعادة هذه الدراسة إلى prolific.co.",
    ur: "❌ متضاد۔ معذرت براہ کرم اس مطالعہ کو prolific.co پر واپس کریں۔",
    hi: "❌ असंगत। क्षमा मांगना। कृपया इस अध्ययन को prolific.co पर लौटाएं।",
    ta: "பொருந்தாது. மன்னிக்கவும். தயவுசெய்து இந்த ஆய்வை plemific.co க்கு திருப்பி விடுங்கள்.",
    ml: "❌ പൊരുത്തപ്പെടുന്നില്ല. ക്ഷമിക്കണം. ഈ പഠനം volliciff.co ലേക്ക് മടങ്ങുക.",
    te: "❌ అననుకూలమైనది. క్షమించండి. దయచేసి ఈ అధ్యయనాన్ని ఫలవంతమైన.కోకు తిరిగి ఇవ్వండి.",
    kn: "ಹೊಂದಾಣಿಕೆಯಾಗುವುದಿಲ್ಲ. ಕ್ಷಮಿಸಿ. ದಯವಿಟ್ಟು ಈ ಅಧ್ಯಯನವನ್ನು verific.co ಗೆ ಹಿಂತಿರುಗಿ.",
    bn: "❌ বেমানান। দুঃখিত দয়া করে এই অধ্যয়নটি PLOLIFIC.CO এ ফিরিয়ে দিন।",
    id: "❌ Tidak kompatibel. Maaf. Harap kembalikan studi ini ke Prolific.co.",
    "zh-CN": "❌不相容。对不起。请将这项研究退还给Prolific.co。",
    "zh-HK": "❌不相容。對不起。請將這項研究退還給Prolific.co。",
    ja: "complatabatible。ごめん。この調査をProlific.coに返送してください。",
    ko: "❌ 호환되지 않습니다. 죄송합니다. 이 연구를 Prolific.co에 반환하십시오.",
  },
  EE_incompatibleReturnToProlific: {
    "en-US":
      "This study is incompatible with your computer or browser, so please return it to Prolific.co. Submission for payment will be rejected (which damages your reputation), so please return it.",
    de: "Diese Studie ist mit Ihrem Computer oder Browser nicht kompatibel. Bitte geben Sie sie bitte an protious.co zurück. Die Zahlungseintritt wird abgelehnt (was Ihren Ruf schadet). Bitte geben Sie sie zurück.",
    fr: "Cette étude est incompatible avec votre ordinateur ou votre navigateur, veuillez donc le retourner sur prolific.co. La soumission pour le paiement sera rejetée (qui endommage votre réputation), veuillez donc la retourner.",
    es: "Este estudio es incompatible con su computadora o navegador, así que devuélvalo a Prolific.co. El envío para el pago será rechazado (lo que daña su reputación), así que devuélvalo.",
    pt: "Este estudo é incompatível com o seu computador ou navegador, então devolva -o ao prolific.co. O envio para pagamento será rejeitado (que danifica sua reputação); portanto, devolva -o.",
    it: "Questo studio è incompatibile con il computer o il browser, quindi ti preghiamo di restituirlo a prolific.co. L'invio per il pagamento verrà respinto (che danneggia la tua reputazione), quindi ti preghiamo di restituirla.",
    ro: "Acest studiu este incompatibil cu computerul sau browserul, așa că vă rugăm să -l returnați la Prolific.co. Trimiterea pentru plată va fi respinsă (ceea ce vă dăunează reputației), așa că vă rugăm să o returnați.",
    pl: "To badanie jest niezgodne z komputerem lub przeglądarką, więc zwróć je do Prolific.co. Zgłoszenie płatności zostanie odrzucone (co uszkadza Twoją reputację), więc zwróć ją.",
    ru: "Это исследование несовместимо с вашим компьютером или браузером, поэтому, пожалуйста, верните его на Plustific.co. Отправка для оплаты будет отклонена (которая повреждает вашу репутацию), поэтому, пожалуйста, верните его.",
    hy: "Այս ուսումնասիրությունը անհամատեղելի է ձեր համակարգչի կամ զննարկչի հետ, այնպես որ խնդրում ենք այն վերադարձնել Prolific.co- ին: Վճարման ներկայացումը մերժվելու է (ինչը վնասում է ձեր հեղինակությանը), այնպես որ խնդրում ենք վերադարձնել այն:",
    fi: "Tämä tutkimus on ristiriidassa tietokoneesi tai selaimen kanssa, joten palauta se tuotteeseen Prenfic.co. Maksujen lähettäminen hylätään (mikä vahingoittaa mainetta), joten palauta se.",
    ka: "ეს კვლევა არ შეესაბამება თქვენს კომპიუტერს ან ბრაუზერს, ამიტომ გთხოვთ, დააბრუნოთ იგი Prolific.co. გადახდის წარდგენა უარყოფილი იქნება (რაც ზიანს აყენებს თქვენს რეპუტაციას), ამიტომ გთხოვთ, დააბრუნოთ იგი.",
    he: "מחקר זה אינו תואם את המחשב או הדפדפן שלך, אז אנא החזיר אותו ל- prolific.co. הגשה לתשלום תידחה (מה שפוגע במוניטין שלך), אז אנא החזיר אותו.",
    ar: "هذه الدراسة غير متوافقة مع جهاز الكمبيوتر الخاص بك أو متصفحك ، لذا يرجى إعادتها إلى Prolific.co. سيتم رفض التقديم للدفع (الذي يضر بسمعتك) ، لذا يرجى إعادته.",
    ur: "یہ مطالعہ آپ کے کمپیوٹر یا براؤزر سے مطابقت نہیں رکھتا ہے ، لہذا براہ کرم اسے profific.co پر واپس کریں۔ ادائیگی کے لئے جمع کرانے کو مسترد کردیا جائے گا (جو آپ کی ساکھ کو نقصان پہنچاتا ہے) ، لہذا براہ کرم اسے واپس کردیں۔",
    hi: "यह अध्ययन आपके कंप्यूटर या ब्राउज़र के साथ असंगत है, इसलिए कृपया इसे prolific.co पर लौटाएं। भुगतान के लिए सबमिशन को अस्वीकार कर दिया जाएगा (जो आपकी प्रतिष्ठा को नुकसान पहुंचाता है), इसलिए कृपया इसे वापस करें।",
    ta: "இந்த ஆய்வு உங்கள் கணினி அல்லது உலாவியுடன் பொருந்தாது, எனவே தயவுசெய்து அதை plemific.co க்கு திருப்பி அனுப்புங்கள். கட்டணத்திற்கான சமர்ப்பிப்பு நிராகரிக்கப்படும் (இது உங்கள் நற்பெயரை சேதப்படுத்தும்), எனவே தயவுசெய்து அதைத் திருப்பித் தரவும்.",
    ml: "ഈ പഠനം നിങ്ങളുടെ കമ്പ്യൂട്ടറോ ബ്രൗസറോടോ പൊരുത്തപ്പെടുന്നില്ല, അതിനാൽ അത് സമൃദ്ധമായ.cCO ലേക്ക് തിരികെ നൽകുക. പേയ്മെന്റിനായുള്ള സമർപ്പണം നിരസിക്കും (ഇത് നിങ്ങളുടെ പ്രശസ്തി നശിപ്പിക്കുന്നു), അതിനാൽ ദയവായി അത് തിരികെ നൽകുക.",
    te: "ఈ అధ్యయనం మీ కంప్యూటర్ లేదా బ్రౌజర్‌తో విరుద్ధంగా ఉంది, కాబట్టి దయచేసి దాన్ని ఫలవంతమైన.కోకు తిరిగి ఇవ్వండి. చెల్లింపు కోసం సమర్పణ తిరస్కరించబడుతుంది (ఇది మీ ప్రతిష్టను దెబ్బతీస్తుంది), కాబట్టి దయచేసి దాన్ని తిరిగి ఇవ్వండి.",
    kn: "ಈ ಅಧ್ಯಯನವು ನಿಮ್ಮ ಕಂಪ್ಯೂಟರ್ ಅಥವಾ ಬ್ರೌಸರ್‌ಗೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ, ಆದ್ದರಿಂದ ದಯವಿಟ್ಟು ಅದನ್ನು verific.co ಗೆ ಹಿಂತಿರುಗಿ. ಪಾವತಿಗಾಗಿ ಸಲ್ಲಿಕೆಯನ್ನು ತಿರಸ್ಕರಿಸಲಾಗುತ್ತದೆ (ಇದು ನಿಮ್ಮ ಖ್ಯಾತಿಗೆ ಹಾನಿ ಮಾಡುತ್ತದೆ), ಆದ್ದರಿಂದ ದಯವಿಟ್ಟು ಅದನ್ನು ಹಿಂತಿರುಗಿ.",
    bn: "এই অধ্যয়নটি আপনার কম্পিউটার বা ব্রাউজারের সাথে বেমানান, সুতরাং দয়া করে এটি Plovific.co এ ফিরিয়ে দিন। অর্থ প্রদানের জন্য জমা দেওয়া প্রত্যাখ্যান করা হবে (যা আপনার খ্যাতির ক্ষতি করে), তাই দয়া করে এটি ফিরিয়ে দিন।",
    id: "Studi ini tidak kompatibel dengan komputer atau browser Anda, jadi silakan kembalikan ke Prolific.co. Pengajuan pembayaran akan ditolak (yang merusak reputasi Anda), jadi silakan kembalikan.",
    "zh-CN":
      "这项研究与您的计算机或浏览器不兼容，因此请将其退还给Prolific.co。提交的付款将被拒绝（这会损害您的声誉），因此请退还。",
    "zh-HK":
      "這項研究與您的計算機或瀏覽器不兼容，因此請將其退還給Prolific.co。提交的付款將被拒絕（這會損害您的聲譽），因此請退還。",
    ja: "この調査はコンピューターまたはブラウザと互換性がないため、Prolific.coに返送してください。支払いの提出は拒否されます（あなたの評判は損傷します）ので、返品してください。",
    ko: "이 연구는 컴퓨터 나 브라우저와 호환되지 않으므로 Prolific.co로 반환하십시오. 지불 제출은 거부됩니다 (평판이 손상됨).",
  },
  EE_studyURL: {
    "en-US": "Study URL: xxx",
    de: "",
    fr: "",
    es: "",
    pt: "",
    it: "",
    ro: "",
    pl: "URL studiów: xxx",
    ru: "URL -адрес исследования: xxx",
    hy: "Ուսումնական URL. xxx",
    fi: "Opinto -URL: xxx",
    ka: "შესწავლილი URL: xxx",
    he: "לימוד כתובת אתר: xxx",
    ar: "URL url url: xxx",
    ur: "مطالعہ یو آر ایل: ایکس ایکس ایکس",
    hi: "अध्ययन URL: xxx",
    ta: "படிக்கவும் URL: xxx",
    ml: "URL പഠിക്കുക: xxx",
    te: "అధ్యయనం URL: xxx",
    kn: "ಅಧ್ಯಯನ URL: xxx",
    bn: "অধ্যয়ন ইউআরএল: xxx",
    id: "URL belajar: xxx",
    "zh-CN": "研究URL：xxx",
    "zh-HK": "研究URL：xxx",
    ja: "Study URL：xxx",
    ko: "연구 URL : xxx",
  },
  RC_angleDistanceIntro: {
    "en-US":
      "Just turn around, and we will find your viewing distance! When asked, please grant access to your camera.",
    de: "Drehen Sie sich einfach um und wir werden Ihre Betrachtungsentfernung finden! Wenn Sie gefragt werden, gewähren Sie bitte Zugriff auf Ihre Kamera.",
    fr: "Retournez-vous et nous trouverons votre distance de vision! Lorsqu'on vous a demandé, veuillez accorder l'accès à votre appareil photo.",
    es: "¡Date la vuelta y encontraremos tu distancia de visión! Cuando se le pregunte, otorgue el acceso a su cámara.",
    pt: "Basta virar e encontraremos sua distância de visualização! Quando solicitado, conceda acesso à sua câmera.",
    it: "Girati e troveremo la tua distanza di visione! Alla domanda, si prega di concedere l'accesso alla fotocamera.",
    ro: "Pur și simplu întoarceți -vă și vă vom găsi distanța de vizionare! Când ați solicitat, vă rugăm să acordați acces la camera dvs.",
    pl: "Po prostu odwróć się, a znajdziemy twoją odległość oglądania! Zapytany, daj dostęp do aparatu.",
    ru: "Просто развернитесь, и мы найдем ваше расстояние просмотра! Когда его спросили, дайте доступ к вашей камере.",
    hy: "Պարզապես շրջվեք, եւ մենք կգտնենք ձեր դիտման հեռավորությունը: Հարցին, խնդրում ենք մուտք ունենալ ձեր տեսախցիկ:",
    fi: "Käännä vain ympäri, ja löydämme katseluetäisyyden! Kun kysytään, anna pääsy kamerallesi.",
    ka: "უბრალოდ მოტრიალდით და ჩვენ ვიპოვით თქვენს სანახავი მანძილს! კითხვაზე, გთხოვთ, მიეცით წვდომა თქვენს კამერაზე.",
    he: "פשוט הסתובב, ונמצא את מרחק הצפייה שלך! כשנשאל, אנא הענק גישה למצלמה שלך.",
    ar: "فقط استدر ، وسوف نجد مسافة المشاهدة الخاصة بك! عندما سئل ، يرجى منح الوصول إلى الكاميرا.",
    ur: "بس مڑیں ، اور ہم آپ کے دیکھنے کا فاصلہ تلاش کریں گے! جب ان سے پوچھا گیا تو ، براہ کرم اپنے کیمرے تک رسائی دیں۔",
    hi: "बस चारों ओर मुड़ें, और हम आपकी देखने की दूरी पाएंगे! जब पूछा गया, तो कृपया अपने कैमरे तक पहुंच प्रदान करें।",
    ta: "திரும்பிச் செல்லுங்கள், உங்கள் பார்வை தூரத்தை நாங்கள் காண்போம்! கேட்டபோது, ​​தயவுசெய்து உங்கள் கேமராவுக்கு அணுகலை வழங்கவும்.",
    ml: "തിരിഞ്ഞുനോക്കുക, നിങ്ങളുടെ കാഴ്ചപ്പാട് ഞങ്ങൾ കണ്ടെത്തും! ചോദിച്ചപ്പോൾ, നിങ്ങളുടെ ക്യാമറയിലേക്ക് പ്രവേശനം നൽകുക.",
    te: "చుట్టూ తిరగండి, మరియు మేము మీ వీక్షణ దూరాన్ని కనుగొంటాము! అడిగినప్పుడు, దయచేసి మీ కెమెరాకు ప్రాప్యత ఇవ్వండి.",
    kn: "ತಿರುಗಿ, ಮತ್ತು ನಿಮ್ಮ ವೀಕ್ಷಣೆಯ ದೂರವನ್ನು ನಾವು ಕಾಣುತ್ತೇವೆ! ಕೇಳಿದಾಗ, ದಯವಿಟ್ಟು ನಿಮ್ಮ ಕ್ಯಾಮೆರಾಗೆ ಪ್ರವೇಶವನ್ನು ನೀಡಿ.",
    bn: "কেবল ঘুরে দেখুন, এবং আমরা আপনার দেখার দূরত্বটি খুঁজে পাব! জিজ্ঞাসা করা হলে, দয়া করে আপনার ক্যামেরায় অ্যাক্সেস দিন।",
    id: "Balikkan saja, dan kami akan menemukan jarak menonton Anda! Ketika ditanya, harap berikan akses ke kamera Anda.",
    "zh-CN":
      "只需转身，我们就会找到您的观看距离！当被问到时，请授予您相机的访问权限。",
    "zh-HK":
      "只需轉身，我們就會找到您的觀看距離！當被問到時，請授予您相機的訪問權限。",
    ja: "振り向くだけで、視聴距離が見つかります！尋ねられたら、カメラへのアクセスを許可してください。",
    ko: "그냥 돌아 서면 시청 거리를 찾을 수 있습니다! 요청하면 카메라에 액세스 할 수 있습니다.",
  },
  RC_angleDistanceTitle: {
    "en-US": "Measure Viewing Distance",
    de: "Messen Sie den Betrachtungsabstand",
    fr: "Mesurer la distance d'observation",
    es: "Medir la distancia de visualización",
    pt: "Medir a distância de visualização",
    it: "Misurare la distanza di visualizzazione",
    ro: "Măsurați distanța de vizualizare",
    pl: "Zmierz odległość oglądania",
    ru: "Измерение расстояния просмотра",
    hy: "Չափել դիտման հեռավորությունը",
    fi: "Mittaa katseluetäisyys",
    ka: "გაზომეთ სანახავი მანძილი",
    he: "למדוד את מרחק הצפייה",
    ar: "قياس مسافة المشاهدة",
    ur: "دیکھنے کے فاصلے کی پیمائش کریں",
    hi: "देखने की दूरी को मापें",
    ta: "பார்க்கும் தூரத்தை அளவிடவும்",
    ml: "കാണുന്ന ദൂരം അളക്കുക",
    te: "వీక్షణ దూరాన్ని కొలవండి",
    kn: "ವೀಕ್ಷಣೆ ದೂರವನ್ನು ಅಳೆಯಿರಿ",
    bn: "দেখার দূরত্ব পরিমাপ করুন",
    id: "Ukur jarak tampilan",
    "zh-CN": "测量观看距离",
    "zh-HK": "測量觀看距離",
    ja: "表示距離を測定します",
    ko: "시청 거리를 측정하십시오",
  },
  RC_cancel: {
    "en-US": "Restart this calibration",
    de: "Starten Sie diese Kalibrierung neu",
    fr: "Redémarrer cet étalonnage",
    es: "Reiniciar esta calibración",
    pt: "Reinicie esta calibração",
    it: "Riavvia questa calibrazione",
    ro: "Reporniți această calibrare",
    pl: "Anuluj",
    ru: "Перезапустите эту калибровку",
    hy: "Վերականգնել այս տրամաչափումը",
    fi: "Käynnistä tämä kalibrointi uudelleen",
    ka: "გადატვირთეთ ეს კალიბრაცია",
    he: "הפעל מחדש את הכיול הזה",
    ar: "الغاء",
    ur: "بند کریں",
    hi: "बंद करे",
    ta: "இந்த அளவுத்திருத்தத்தை மறுதொடக்கம் செய்யுங்கள்",
    ml: "ഈ കാലിബ്രേഷൻ പുനരാരംഭിക്കുക",
    te: "ఈ క్రమాంకనాన్ని పున art ప్రారంభించండి",
    kn: "ಈ ಮಾಪನಾಂಕ ನಿರ್ಣಯವನ್ನು ಮರುಪ್ರಾರಂಭಿಸಿ",
    bn: "এই ক্রমাঙ্কন পুনরায় চালু করুন",
    id: "Mulai ulang kalibrasi ini",
    "zh-CN": "重做此校准",
    "zh-HK": "重做此校準",
    ja: "キャンセル",
    ko: "이 교정을 다시 시작하십시오",
  },
  RC_distanceTracking: {
    "en-US": "Distance Tracking",
    de: "Entfernungsverfolgung",
    fr: "Suivi de distance",
    es: "Seguimiento de distancia",
    pt: "Rastreamento de distância",
    it: "Monitoraggio a distanza",
    ro: "Urmărirea distanței",
    pl: "Śledzenie odległości od ekranu",
    ru: "Отслеживание расстояний",
    hy: "Հետեւելու հեռավորությունները",
    fi: "Etäisyyden seuranta",
    ka: "მანძილის თვალყურის დევნება",
    he: "מעקב מרחק",
    ar: "تتبع الرأس",
    ur: "فاصلے سے باخبر رہنا",
    hi: "दूरी ट्रैकिंग",
    ta: "தூர கண்காணிப்பு",
    ml: "ദൂര ട്രാക്കിംഗ്",
    te: "దూర ట్రాకింగ్",
    kn: "ದೂರ ಟ್ರ್ಯಾಕಿಂಗ್",
    bn: "দূরত্ব ট্র্যাকিং",
    id: "Pelacakan jarak",
    "zh-CN": "视距追踪",
    "zh-HK": "視距追踪",
    ja: "距離追跡",
    ko: "거리 추적",
  },
  RC_distanceTrackingCloseL: {
    "en-US":
      "1. Close your LEFT eye. (This is the left side of the screen.)\n2. Use ▶ arrow key to slide the flickering dot. (Or just drag it, if it's not responding to the arrow key.) While looking at the cross hairs, monitor the flickering dot in the corner of your eye, and slide it left and right until it disappears into your blindspot.\n3. Hit RETURN.\nNOTE: Keep your eye on the cross hairs. The flickering dot won't disappear while your eye follows it. ",
    de: "1. Schließen Sie Ihr linkes Auge. (Dies ist die linke Seite des Bildschirms.)\n2. Verwenden Sie ▶ Pfeiltaste, um den flackernden Punkt zu schieben. (Oder ziehen Sie es einfach, wenn es nicht auf die Pfeiltaste reagiert.) Beim Betrachten der Kreuzhaare den flackernden Punkt in Ihrem Augenwinkel und schieben Sie ihn nach links und rechts, bis er in Ihren Blindpot verschwindet.\n3. Return to Return.\nHinweis: Behalten Sie die Kreuzhaare im Auge. Der flackernde Punkt wird nicht verschwinden, während Ihr Auge ihm folgt.",
    fr: "1. Fermez votre œil gauche. (C'est le côté gauche de l'écran.)\n2. Utilisez ▶ Clé flèche pour faire glisser le point scintillant. (Ou faites-le simplement glisser, s'il ne répond pas à la clé de flèche.) En regardant les poils de la transmission, surveillez le point vacillant dans le coin de votre œil et faites-le glisser à gauche et à droite jusqu'à ce qu'il disparaisse dans votre point mort.\n3. Appuyez sur Retour.\nRemarque: Gardez l'œil sur les poils. Le point vacillant ne disparaîtra pas pendant que votre œil le suit.",
    es: "1. Cierre el ojo izquierdo. (Este es el lado izquierdo de la pantalla).\n2. Use la tecla de flecha ▶ para deslizar el punto parpadeante. (O simplemente arrástrelo, si no está respondiendo a la llave de flecha). Mientras mira los pelos transversales, monitoree el punto parpadeante en la esquina de su ojo y deslicela a la izquierda y a la derecha hasta que desaparezca en su punto ciego.\n3. Vuelva a retocar.\nNota: Mantén tu ojo en la cruz. El punto parpadeante no desaparecerá mientras su ojo lo sigue.",
    pt: "1. Feche o olho esquerdo. (Este é o lado esquerdo da tela.)\n2. Use ▶ Chave de seta para deslizar o ponto de tremeling. (Ou simplesmente arraste -o, se não estiver respondendo à tecla de seta.) Enquanto olha os cabelos cruzados, monitore o ponto tremeluzente no canto do olho e deslize -o para a esquerda e para a direita até desaparecer no seu ponto cego.\n3. Bata de retorno.\nNota: Fique de olho nos cabelos cruzados. O ponto tremeluzente não desaparece enquanto seu olho o segue.",
    it: "1. Chiudi l'occhio sinistro. (Questo è il lato sinistro dello schermo.)\n2. Utilizzare ▶ Tasto freccia per far scorrere il punto tremolante. (O semplicemente trascinalo, se non risponde alla chiave freccia.) Mentre guarda i peli incrociati, monitora il punto tremolo nell'angolo dell'occhio e fai scorrere a sinistra e a destra fino a quando non scompare nel tuo cieco.\n3. Colpire il ritorno.\nNota: tieni d'occhio i capelli incrociati. Il punto tremolante non scomparirà mentre il tuo occhio lo segue.",
    ro: "1. Închideți ochiul stâng. (Aceasta este partea stângă a ecranului.)\n2. Utilizați ▶ Tasta săgeată pentru a glisa punctul pâlpâit. (Sau pur și simplu trageți -l, dacă nu răspunde la cheia săgeată.) În timp ce priviți părul încrucișat, monitorizați punctul pâlpâitor din colțul ochiului și glisați -l la stânga și la dreapta până când va dispărea în blabspot.\n3. Apăsați revenirea.\nNotă: Fii cu ochii pe firele de păr. Punctul pâlpâit nu va dispărea în timp ce ochiul tău îl urmărește.",
    pl: "1. Zamknij lewe oko. (To jest lewa strona ekranu.)\n2. Użyj ▶ Klucz strzałki, aby przesunąć migoczącą kropkę. (Lub po prostu przeciągnij go, jeśli nie reaguje na klawisz strzałki.) Patrząc na włosy krzyżowe, monitoruj migoczącą kropkę w rogu oka i przesuń go w lewo i w prawo, aż zniknie w twojej ślepej.\n3. Hit Return.\nUwaga: miej oko na włosy krzyżowe. Migocząca kropka nie zniknie, gdy podąża za nim oko.",
    ru: "1. Закройте левый глаз. (Это левая сторона экрана.)\n2. Используйте ▶ Ключ стрелка, чтобы скользить мерцающую точку. (Или просто перетащите его, если он не реагирует на ключ стрелки.), Глядя на кросс -волоски, следите за мерцающей точкой в ​​углу глаз и скользите влево и вправо, пока не исчезнет в вашем слепой точке.\n3. Нажмите на возврат.\nПримечание: следите за кросс -волосами. Мерцающая точка не исчезнет, ​​пока ваш глаз следует за ней.",
    hy: "1. Փակեք ձեր ձախ աչքը: (Սա էկրանի ձախ կողմն է):\n2. Օգտագործեք ▶ սլաքի ստեղնը `շողոքորթ կետը սահելու համար: (Կամ պարզապես քաշեք այն, եթե դա չի արձագանքում սլաքի բանալին :) Խաչի մազերը նայելով, վերահսկեք ձեր աչքի անկյունում գտնվող շողոքորթ կետը եւ սահեցրեք այն եւ աջը:\n3. Հիթ վերադարձ:\nՆշում. Ձեր աչքը պահեք խաչի մազերի վրա: Հղումային կետը չի վերանա, մինչդեռ ձեր աչքը հետեւում է դրան:",
    fi: "1. Sulje vasen silmä. (Tämä on näytön vasen puoli.)\n2. Liu'uta vilkkuva pisteen nuolinäppäintä. (Tai vedä sitä vain, jos se ei vastaa nuolinäppäimeen.) Tarkastele silmän nurkassa olevaa vilkkuvaa pistettä katsellessasi ja liu'uta sitä vasemmalle ja oikealle, kunnes se katoaa blindspot -laitteeseesi.\n3. Lyö paluu.\nHUOMAUTUS: Pidä silmällä ristikarvoja. Vilkkuva piste ei katoa, kun silmäsi seuraa sitä.",
    ka: "1. დახურეთ მარცხენა თვალი. (ეს არის ეკრანის მარცხენა მხარე.)\n2. გამოიყენეთ ▶ ისრის ღილაკი, რომ გააფართოვოთ მბჟუტავი წერტილი. (ან უბრალოდ გადაიტანეთ იგი, თუ ის არ რეაგირებს ისრის ღილაკზე.) ჯვრის თმას გადახედვისას, თვალის კუთხეში მბზინავი წერტილი დააკვირდით და გადაიტანეთ მარცხნივ და მარჯვნივ, სანამ ის არ გაქრება თქვენს ჟალუზაში.\n3. მოხვდი დაბრუნებას.\nშენიშვნა: მიადევნეთ თვალი ჯვრის თმას. მბჟუტავი წერტილი არ გაქრება, სანამ შენი თვალი მისდევს.",
    he: "1. סגור את העין השמאלית שלך. (זהו הצד השמאלי של המסך.)\n2. השתמש במפתח חץ כדי להחליק את הנקודה המהבהבת. (או סתם לגרור אותו, אם זה לא מגיב למפתח החץ.) תוך כדי התבוננות בשערות הצלב, עקוב אחר הנקודה המהבהבת בפינת העין שלך, והחליק אותה שמאלה וימינה עד שהוא נעלם לתוך העיוור שלך.\n3. פגע בחזרה.\nהערה: השגיח על שערות הצלב. הנקודה המהבהבת לא תיעלם בזמן שהעין שלך עוקבת אחריה.",
    ar: "1. أغلق عينيك اليسرى. (هذا هو الجانب الأيسر من الشاشة.)\n2. استخدم ▶ مفتاح السهم لتزويد النقطة الخفقان. (أو فقط اسحبه ، إذا لم يستجب لمفتاح السهم.) أثناء النظر إلى الشعر المتقاطع ، راقب النقطة الوهمية في زاوية عينيك ، وحركها إلى اليسار واليمين حتى تختفي في النقطة العمياء.\n3. ضرب العودة.\nملاحظة: راقب الشعر عبر الشعر. لن تختفي النقطة الخفقان بينما تتبعها عينك.",
    ur: "1. اپنی بائیں آنکھ کو بند کریں۔ (یہ اسکرین کا بائیں طرف ہے۔)\n2. ٹمٹماہٹ ڈاٹ سلائیڈ کرنے کے لئے ▶ یرو کلید کا استعمال کریں۔ (یا صرف اسے گھسیٹیں ، اگر یہ تیر کی چابی کا جواب نہیں دے رہا ہے۔) کراس بالوں کو دیکھتے وقت ، اپنی آنکھ کے کونے میں ٹمٹماہٹ ڈاٹ کی نگرانی کریں ، اور اسے بائیں اور دائیں سلائیڈ کریں جب تک کہ یہ آپ کے اندھے مقام میں غائب نہ ہوجائے۔\n3. واپسی کو ہٹ.\nنوٹ: کراس بالوں پر نگاہ رکھیں۔ جب آپ کی آنکھ اس کی پیروی کرتی ہے تو چمکتا ہوا ڈاٹ غائب نہیں ہوگا۔",
    hi: "1. अपनी बाईं आंख बंद करें। (यह स्क्रीन का बायाँ हिस्सा है।)\n2. टिमटिमाती डॉट को स्लाइड करने के लिए ▶ तीर कुंजी का उपयोग करें। (या बस इसे खींचें, अगर यह तीर कुंजी का जवाब नहीं दे रहा है।) क्रॉस हेयर को देखते समय, अपनी आंख के कोने में टिमटिमाती डॉट की निगरानी करें, और इसे बाएं और दाएं स्लाइड करें जब तक कि यह आपके अंधा में गायब नहीं हो जाता।\n3. हिट रिटर्न।\nनोट: अपनी नजर क्रॉस हेयर पर रखें। झिलमिलाहट डॉट गायब नहीं होगा जबकि आपकी आंख इसका अनुसरण करती है।",
    ta: "1. உங்கள் இடது கண்ணை மூடு. (இது திரையின் இடது புறம்.)\n2. ஒளிரும் புள்ளியை சறுக்குவதற்கு ▶ அம்பு விசையைப் பயன்படுத்தவும். .\n3. வருவாய்.\nகுறிப்பு: குறுக்கு முடிகள் மீது உங்கள் கண் வைத்திருங்கள். உங்கள் கண் அதைப் பின்தொடரும் போது ஒளிரும் புள்ளி மறைந்துவிடாது.",
    ml: "1. നിങ്ങളുടെ ഇടത് കണ്ണ് അടയ്ക്കുക. (ഇത് സ്ക്രീനിന്റെ ഇടതുവശമാണ്.)\n2. ഫ്ലിക്കറിംഗ് ഡോട്ട് സ്ലൈഡുചെയ്യാൻ ▶ അമ്പടയാളം ഉപയോഗിക്കുക. (അല്ലെങ്കിൽ അമ്പടയാള കീയോട് പ്രതികരിക്കുന്നില്ലെങ്കിൽ, അത് വലിച്ചിടുക.)\n3. റിട്ടേൺ അമർത്തുക.\nകുറിപ്പ്: ക്രോസ് രോമങ്ങളിൽ നിങ്ങളുടെ കണ്ണു സൂക്ഷിക്കുക. നിങ്ങളുടെ കണ്ണ് അത് പിന്തുടരുമ്പോൾ മിന്നുന്ന ഡോട്ട് അപ്രത്യക്ഷമാകില്ല.",
    te: "1. మీ ఎడమ కన్ను మూసివేయండి. (ఇది స్క్రీన్ యొక్క ఎడమ వైపు.)\n2. మినుకుమినుకుమనే చుక్కను స్లైడ్ చేయడానికి ▶ బాణం కీని ఉపయోగించండి. .\n3. రిటర్న్ హిట్.\nగమనిక: క్రాస్ వెంట్రుకలపై మీ కన్ను ఉంచండి. మీ కన్ను అనుసరించేటప్పుడు మినుకుమినుకుమనే చుక్క కనిపించదు.",
    kn: "1. ನಿಮ್ಮ ಎಡಗಣ್ಣನ್ನು ಮುಚ್ಚಿ. (ಇದು ಪರದೆಯ ಎಡಭಾಗವಾಗಿದೆ.)\n2. ಮಿನುಗುವ ಚುಕ್ಕೆ ಸ್ಲೈಡ್ ಮಾಡಲು ▶ ಬಾಣದ ಕೀಲಿಯನ್ನು ಬಳಸಿ. .\n3. ರಿಟರ್ನ್ ಹಿಟ್.\nಗಮನಿಸಿ: ಅಡ್ಡ ಕೂದಲಿನ ಮೇಲೆ ಕಣ್ಣಿಡಿ. ನಿಮ್ಮ ಕಣ್ಣು ಅದನ್ನು ಅನುಸರಿಸುವಾಗ ಮಿನುಗುವ ಚುಕ್ಕೆ ಕಣ್ಮರೆಯಾಗುವುದಿಲ್ಲ.",
    bn: "1. আপনার বাম চোখ বন্ধ করুন। (এটি পর্দার বাম দিক))\n2. ফ্লিকারিং ডট স্লাইড করতে ▶ তীর কী ব্যবহার করুন। (বা কেবল এটি টেনে আনুন, যদি এটি তীর কীতে সাড়া না দেয়)) ক্রস চুলের দিকে তাকানোর সময়, আপনার চোখের কোণে ঝলকানি বিন্দুটি পর্যবেক্ষণ করুন এবং এটি আপনার ব্লাইন্ডস্পটে অদৃশ্য হওয়া অবধি বাম এবং ডানদিকে স্লাইড করুন।\n3. হিট রিটার্ন।\nদ্রষ্টব্য: ক্রস চুলের দিকে নজর রাখুন। আপনার চোখ এটি অনুসরণ করার সময় ঝলকানি বিন্দু অদৃশ্য হয়ে যাবে না।",
    id: "1. Tutup mata kiri Anda. (Ini adalah sisi kiri layar.)\n2. Gunakan ▶ tombol panah untuk menggeser titik yang berkedip -kedip. ;\n3. Tekan kembali.\nCatatan: Perhatikan rambut salib. Dot yang berkedip tidak akan hilang saat mata Anda mengikutinya.",
    "zh-CN":
      "1.闭上左眼。 （这是屏幕的左侧。）\n2.使用▶箭头键滑动闪烁的点。 （或者只是拖动它，如果它没有响应箭头钥匙。）看着十字头发时，请监视眼角的闪烁点，然后向左和向右滑动，直到它消失在盲点中​​。\n3.命中返回。\n注意：请密切关注十字头发。当您的眼睛跟随它时，闪烁的点不会消失。",
    "zh-HK":
      "1.閉上左眼。 （這是屏幕的左側。）\n2.使用▶箭頭鍵滑動閃爍的點。 （或者只是拖動它，如果它沒有響應箭頭鑰匙。）看著十字頭髮時，請監視眼角的閃爍點，然後向左和向右滑動，直到它消失在盲點中。\n3.命中返回。\n注意：請密切關註十字頭髮。當您的眼睛跟隨它時，閃爍的點不會消失。",
    ja: "1.左目を閉じます。 （これは画面の左側です。）\n2.▶矢印キーを使用して、ちらつきのドットをスライドさせます。 （または、矢印キーに応答しない場合は、ドラッグしてください。）交差毛を見ながら、目の隅にあるちらつきのドットを監視し、盲点に消えるまで左右にスライドさせます。\n3.返信をヒットします。\n注：十字毛に目を向けてください。目が続く間、ちらつきのドットは消えません。",
    ko: "1. 왼쪽 눈을 닫습니다. (이것은 화면의 왼쪽입니다.)\n2. ▶ 화살표 키를 사용하여 깜박 거리는 점을 밀어 넣습니다. (또는 화살표 키에 응답하지 않으면 드래그하십시오.) 십자 머리카락을 보면서 눈의 모서리에 깜박 거리는 점을 모니터링하고 사각 지대로 사라질 때까지 왼쪽과 오른쪽으로 밀어 넣으십시오.\n3. 반환을 누르십시오.\n참고 : 크로스 털을 주시하십시오. 눈이 뒤 따르는 동안 깜박 거리는 점은 사라지지 않습니다.",
  },
  RC_distanceTrackingCloseR: {
    "en-US":
      "1. Close your RIGHT eye. (This is the right side of the screen.)\n2. Use ◀ arrow key to slide the flickering dot. While looking at the cross hairs, monitor the flickering dot in the corner of your eye, and slide it left and right until it disappears into your blindspot.\n3. Hit RETURN.\nNOTE: Keep your eye on the cross hairs. The flickering dot won't disappear while your eye follows it. ",
    de: "1. Schließen Sie Ihr rechtes Auge. (Dies ist die rechte Seite des Bildschirms.)\n2. Verwenden Sie ◀ Pfeilschlüssel, um den flackernden Punkt zu schieben. Überwachen Sie beim Betrachten der Kreuzhaare den flackernden Punkt in Ihrem Augenwinkel und schieben Sie ihn nach links und rechts, bis er in Ihren Blindspot verschwindet.\n3. Return to Return.\nHinweis: Behalten Sie die Kreuzhaare im Auge. Der flackernde Punkt wird nicht verschwinden, während Ihr Auge ihm folgt.",
    fr: "1. Fermez votre œil droit. (C'est le côté droit de l'écran.)\n2. Utilisez la touche flèche ◀ pour faire glisser le point scintillant. Tout en regardant les poils, surveillez le point scintillant dans le coin de votre œil et faites-le glisser à gauche et à droite jusqu'à ce qu'il disparaisse dans votre point mort.\n3. Appuyez sur Retour.\nRemarque: Gardez l'œil sur les poils. Le point vacillant ne disparaîtra pas pendant que votre œil le suit.",
    es: "1. Cierre el ojo derecho. (Este es el lado derecho de la pantalla).\n2. Use la tecla de flecha ◀ para deslizar el punto parpadeante. Mientras mira los pelos transversales, monitoree el punto parpadeante en la esquina de su ojo y deslicelo a la izquierda y a la derecha hasta que desaparezca en su punto ciego.\n3. Vuelva a retocar.\nNota: Mantén tu ojo en la cruz. El punto parpadeante no desaparecerá mientras su ojo lo sigue.",
    pt: "1. Feche o olho direito. (Este é o lado direito da tela.)\n2. Use ◀ Tecla de seta para deslizar o ponto de tremer. Enquanto olha para os cabelos cruzados, monitore o ponto tremeluzente no canto do olho e deslize -o para a esquerda e para a direita até desaparecer no seu ponto cego.\n3. Bata de retorno.\nNota: Fique de olho nos cabelos cruzados. O ponto tremeluzente não desaparece enquanto seu olho o segue.",
    it: "1. Chiudi l'occhio destro. (Questo è il lato destro dello schermo.)\n2. Utilizzare ◀ Tasto freccia per far scorrere il punto tremolante. Mentre guardi i peli incrociati, monitora il punto tremolo nell'angolo dell'occhio e fai scorrere a sinistra e a destra fino a quando non scompare nel tuo cieco.\n3. Colpire il ritorno.\nNota: tieni d'occhio i capelli incrociati. Il punto tremolante non scomparirà mentre il tuo occhio lo segue.",
    ro: "1. Închideți ochiul drept. (Aceasta este partea dreaptă a ecranului.)\n2. Utilizați ◀ Tasta săgeată pentru a glisa punctul pâlpâit. În timp ce te uiți la părul încrucișat, monitorizează punctul pâlpâitor din colțul ochiului și glisează -l la stânga și la dreapta până când dispare în Blindspot.\n3. Apăsați revenirea.\nNotă: Fii cu ochii pe firele de păr. Punctul pâlpâit nu va dispărea în timp ce ochiul tău îl urmărește.",
    pl: "1. Zamknij prawe oko. (To jest prawa strona ekranu.)\n2. Użyj klawisza strzałki, aby przesunąć migoczącą kropkę. Patrząc na włosy krzyżowe, monitoruj migoczącą kropkę w rogu oka i przesuń je w lewo i w prawo, aż zniknie w twojej ślepej.\n3. Hit Return.\nUwaga: miej oko na włosy krzyżowe. Migocząca kropka nie zniknie, gdy podąża za nim oko.",
    ru: "1. Закройте правый глаз. (Это правая сторона экрана.)\n2. Используйте ◀ Ключ стрелки, чтобы скользить мерцающую точку. Посмотрев на кросс -волоски, следите за мерцающей точкой в ​​углу глаза, и сдвиньте ее влево и направо, пока она не исчезнет в вашей слепой точке.\n3. Нажмите на возврат.\nПримечание: следите за кросс -волосами. Мерцающая точка не исчезнет, ​​пока ваш глаз следует за ней.",
    hy: "1. Փակեք ձեր աջ աչքը: (Սա էկրանի աջ կողմն է):\n2. Օգտագործեք ◀ սլաքի ստեղնը `շողոքորթ կետը սահելու համար: Խաչի մազերը նայելիս վերահսկեք ձեր աչքի անկյունում գտնվող թիթեղի կետը եւ սահեցրեք այն ձախ եւ աջ, մինչեւ այն վերանա ձեր կույր:\n3. Հիթ վերադարձ:\nՆշում. Ձեր աչքը պահեք խաչի մազերի վրա: Հղումային կետը չի վերանա, մինչդեռ ձեր աչքը հետեւում է դրան:",
    fi: "1. Sulje oikea silmä. (Tämä on näytön oikea puoli.)\n2. Liu'uta viehättävä piste ◀ nuolinäppäimellä. Kun katsot ristiinkarvoja, tarkkaile vilkkuvaa pistettä silmäsi nurkassa ja liu'uta se vasemmalle ja oikealle, kunnes se katoaa blindspotisi.\n3. Lyö paluu.\nHUOMAUTUS: Pidä silmällä ristikarvoja. Vilkkuva piste ei katoa, kun silmäsi seuraa sitä.",
    ka: "1. დახურეთ თქვენი მარჯვენა თვალი. (ეს არის ეკრანის მარჯვენა მხარე.)\n2. გამოიყენეთ ◀ ისრის ღილაკი, რომ გააფართოვოთ მბჟუტავი წერტილი. ჯვრის თმას დათვალიერებისას, თვალის კუთხეში მბზინავი წერტილი დააკვირდით და გადაიტანეთ მარცხნივ და მარჯვნივ, სანამ ის არ ქრება თქვენს ბრალოსში.\n3. მოხვდი დაბრუნებას.\nშენიშვნა: მიადევნეთ თვალი ჯვრის თმას. მბჟუტავი წერტილი არ გაქრება, სანამ შენი თვალი მისდევს.",
    he: "1. סגור את העין הימנית שלך. (זהו הצד הימני של המסך.)\n2. השתמש במפתח חץ כדי להחליק את הנקודה המהבהבת. תוך כדי התבוננות בשערות הצלב, עקוב אחר הנקודה המהבהבת בפינת העין שלך, והחליק אותה שמאלה וימינה עד שהוא נעלם לתוך העיוור שלך.\n3. פגע בחזרה.\nהערה: השגיח על שערות הצלב. הנקודה המהבהבת לא תיעלם בזמן שהעין שלך עוקבת אחריה.",
    ar: "1. أغلق عينك اليمنى. (هذا هو الجانب الأيمن من الشاشة.)\n2. استخدم ◀ مفتاح السهم لتزويد النقطة الخفقان. أثناء النظر إلى الشعر المتقاطع ، راقب النقطة الوهمية في زاوية عينيك ، وحركها إلى اليسار واليمين حتى تختفي في لعبة Blindspot.\n3. ضرب العودة.\nملاحظة: راقب الشعر عبر الشعر. لن تختفي النقطة الخفقان بينما تتبعها عينك.",
    ur: "1. اپنی دائیں آنکھ کو بند کریں۔ (یہ اسکرین کا دائیں طرف ہے۔)\n2. ٹمٹماہٹ ڈاٹ سلائیڈ کرنے کے لئے ◀ یرو کلید کا استعمال کریں۔ کراس بالوں کو دیکھتے ہوئے ، اپنی آنکھ کے کونے میں ٹمٹماہٹ ڈاٹ کی نگرانی کریں ، اور اسے بائیں اور دائیں سلائیڈ کریں جب تک کہ یہ آپ کے اندھے مقام میں غائب نہ ہوجائے۔\n3. واپسی کو ہٹ.\nنوٹ: کراس بالوں پر نگاہ رکھیں۔ جب آپ کی آنکھ اس کی پیروی کرتی ہے تو چمکتا ہوا ڈاٹ غائب نہیں ہوگا۔",
    hi: "1. अपनी दाहिनी आंख बंद करें। (यह स्क्रीन का दाहिना हिस्सा है।)\n2. टिमटिमाते हुए डॉट को स्लाइड करने के लिए to तीर कुंजी का उपयोग करें। क्रॉस हेयर को देखते हुए, अपनी आंख के कोने में टिमटिमाते हुए डॉट की निगरानी करें, और इसे बाएं और दाएं स्लाइड करें जब तक कि यह आपके अंधा में गायब न हो जाए।\n3. हिट रिटर्न।\nनोट: अपनी नजर क्रॉस हेयर पर रखें। झिलमिलाहट डॉट गायब नहीं होगा जबकि आपकी आंख इसका अनुसरण करती है।",
    ta: "1. உங்கள் வலது கண்ணை மூடு. (இது திரையின் வலது பக்கம்.)\n2. ஒளிரும் புள்ளியை சறுக்குவதற்கு ◀ அம்பு விசையைப் பயன்படுத்தவும். குறுக்கு முடிகளைப் பார்க்கும்போது, ​​உங்கள் கண்ணின் மூலையில் உள்ள ஒளிரும் புள்ளியைக் கண்காணித்து, அது உங்கள் குருட்டுக்குள் மறைந்து போகும் வரை இடது மற்றும் வலதுபுறமாக சறுக்கவும்.\n3. வருவாய்.\nகுறிப்பு: குறுக்கு முடிகள் மீது உங்கள் கண் வைத்திருங்கள். உங்கள் கண் அதைப் பின்தொடரும் போது ஒளிரும் புள்ளி மறைந்துவிடாது.",
    ml: "1. നിങ്ങളുടെ വലത് കണ്ണ് അടയ്ക്കുക. (ഇത് സ്ക്രീനിന്റെ വലതുവശമാണ്.)\n2. ഫ്ലിക്കറിംഗ് ഡോട്ട് സ്ലൈഡുചെയ്യാൻ ◀ അമ്പടയാളം ഉപയോഗിക്കുക. ക്രോസ് ഹെയറുകളിലേക്ക് നോക്കുമ്പോൾ, നിങ്ങളുടെ കണ്ണിന്റെ മൂലയിൽ മിന്നുന്ന ഡോട്ട് നിരീക്ഷിക്കുക, അത് നിങ്ങളുടെ ബ്ലൈൻഡ്സ്പോട്ടിലേക്ക് അപ്രത്യക്ഷമാകുന്നതുവരെ അവശേഷിക്കുകയും വലത്തോട്ടും സ്ലൈഡുചെയ്യുകയും ചെയ്യുക.\n3. റിട്ടേൺ അമർത്തുക.\nകുറിപ്പ്: ക്രോസ് രോമങ്ങളിൽ നിങ്ങളുടെ കണ്ണു സൂക്ഷിക്കുക. നിങ്ങളുടെ കണ്ണ് അത് പിന്തുടരുമ്പോൾ മിന്നുന്ന ഡോട്ട് അപ്രത്യക്ഷമാകില്ല.",
    te: "1. మీ కుడి కన్ను మూసివేయండి. (ఇది స్క్రీన్ యొక్క కుడి వైపు.)\n2. మినుకుమినుకుమనే చుక్కను స్లైడ్ చేయడానికి ◀ బాణం కీని ఉపయోగించండి. క్రాస్ హెయిర్స్ వైపు చూస్తున్నప్పుడు, మీ కంటి మూలలో ఉన్న మినుకుమినుకుమనే చుక్కను పర్యవేక్షించండి మరియు మీ బ్లైండ్ స్పాట్‌లోకి అదృశ్యమయ్యే వరకు దాన్ని ఎడమ మరియు కుడి వైపుకు జారండి.\n3. రిటర్న్ హిట్.\nగమనిక: క్రాస్ వెంట్రుకలపై మీ కన్ను ఉంచండి. మీ కన్ను అనుసరించేటప్పుడు మినుకుమినుకుమనే చుక్క కనిపించదు.",
    kn: "1. ನಿಮ್ಮ ಬಲಗಣ್ಣನ್ನು ಮುಚ್ಚಿ. (ಇದು ಪರದೆಯ ಬಲಭಾಗವಾಗಿದೆ.)\n2. ಮಿನುಗುವ ಚುಕ್ಕೆ ಸ್ಲೈಡ್ ಮಾಡಲು ◀ ಬಾಣದ ಕೀಲಿಯನ್ನು ಬಳಸಿ. ಅಡ್ಡ ಕೂದಲನ್ನು ನೋಡುವಾಗ, ನಿಮ್ಮ ಕಣ್ಣಿನ ಮೂಲೆಯಲ್ಲಿರುವ ಮಿನುಗುವ ಚುಕ್ಕೆಯನ್ನು ಮೇಲ್ವಿಚಾರಣೆ ಮಾಡಿ ಮತ್ತು ನಿಮ್ಮ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್‌ಗೆ ಕಣ್ಮರೆಯಾಗುವವರೆಗೆ ಅದನ್ನು ಎಡ ಮತ್ತು ಬಲಕ್ಕೆ ಸ್ಲೈಡ್ ಮಾಡಿ.\n3. ರಿಟರ್ನ್ ಹಿಟ್.\nಗಮನಿಸಿ: ಅಡ್ಡ ಕೂದಲಿನ ಮೇಲೆ ಕಣ್ಣಿಡಿ. ನಿಮ್ಮ ಕಣ್ಣು ಅದನ್ನು ಅನುಸರಿಸುವಾಗ ಮಿನುಗುವ ಚುಕ್ಕೆ ಕಣ್ಮರೆಯಾಗುವುದಿಲ್ಲ.",
    bn: "1. আপনার ডান চোখ বন্ধ করুন। (এটি পর্দার ডান দিক))\n2. ফ্লিকারিং ডট স্লাইড করতে ◀ তীর কী ব্যবহার করুন। ক্রস কেশগুলি দেখার সময়, আপনার চোখের কোণে ফ্লিকারিং ডটটি পর্যবেক্ষণ করুন এবং এটি আপনার ব্লাইন্ডস্পটে অদৃশ্য না হওয়া পর্যন্ত বাম এবং ডানদিকে স্লাইড করুন।\n3. হিট রিটার্ন।\nদ্রষ্টব্য: ক্রস চুলের দিকে নজর রাখুন। আপনার চোখ এটি অনুসরণ করার সময় ঝলকানি বিন্দু অদৃশ্য হয়ে যাবে না।",
    id: "1. Tutup mata kanan Anda. (Ini adalah sisi kanan layar.)\n2. Gunakan ◀ tombol panah untuk menggeser titik yang berkedip -kedip. Sambil melihat rambut salib, pantau titik -titik yang berkedip -kedip di sudut mata Anda, dan geser ke kiri dan kanan sampai menghilang ke dalam buta Anda.\n3. Tekan kembali.\nCatatan: Perhatikan rambut salib. Dot yang berkedip tidak akan hilang saat mata Anda mengikutinya.",
    "zh-CN":
      "1.闭上右眼。 （这是屏幕的右侧。）\n2.使用◀箭头键滑动闪烁的点。看着十字头发时，请监视眼角的闪烁点，然后向左和向右滑动，直到它消失在盲点中​​。\n3.命中返回。\n注意：请密切关注十字头发。当您的眼睛跟随它时，闪烁的点不会消失。",
    "zh-HK":
      "1.閉上右眼。 （這是屏幕的右側。）\n2.使用◀箭頭鍵滑動閃爍的點。看著十字頭髮時，請監視眼角的閃爍點，然後向左和向右滑動，直到它消失在盲點中。\n3.命中返回。\n注意：請密切關註十字頭髮。當您的眼睛跟隨它時，閃爍的點不會消失。",
    ja: "1.右目を閉じます。 （これは画面の右側です。）\n2.◀矢印キーを使用して、ちらつきのドットをスライドさせます。交差毛を見ながら、目の隅にあるちらつきのドットを監視し、盲点に消えるまで左右にスライドさせます。\n3.返信をヒットします。\n注：十字毛に目を向けてください。目が続く間、ちらつきのドットは消えません。",
    ko: "1. 오른쪽 눈을 감습니다. (이것은 화면의 오른쪽입니다.)\n2. ◀ 화살표 키를 사용하여 깜박 거리는 점을 밀어냅니다. 십자 머리카락을 보면서 눈의 모서리에 깜박 거리는 점을 모니터링하고 사각 지대로 사라질 때까지 왼쪽과 오른쪽으로 밀어 넣으십시오.\n3. 반환을 누르십시오.\n참고 : 크로스 털을 주시하십시오. 눈이 뒤 따르는 동안 깜박 거리는 점은 사라지지 않습니다.",
  },
  RC_distanceTrackingGuide: {
    "en-US":
      "Your eyes are xx1 cm from the screen, but the experiment needs approximately xx2 cm. The experiment will resume when you reach the correct distance.",
    de: "Ihre Augen sind xx1 cm vom Bildschirm entfernt, aber das Experiment benötigt ungefähr xx2 cm. Das Experiment wird wieder aufgenommen, wenn Sie den richtigen Abstand erreichen.",
    fr: "Vos yeux sont à xx1 cm de l'écran, mais l'expérience a besoin d'environ xx2 cm. L'expérience reprendra lorsque vous atteignez la distance correcte.",
    es: "Tus ojos están a xx1 cm de la pantalla, pero el experimento necesita aproximadamente xx2 cm. El experimento se reanudará cuando alcances la distancia correcta.",
    pt: "Seus olhos estão xx1 cm da tela, mas o experimento precisa de aproximadamente xx2 cm. O experimento será retomado quando você atingir a distância correta.",
    it: "I tuoi occhi sono xx1 cm dallo schermo, ma l'esperimento necessita di circa xx2 cm. L'esperimento riprenderà quando raggiungi la distanza corretta.",
    ro: "Ochii tăi sunt xx1 cm de pe ecran, dar experimentul are nevoie de aproximativ xx2 cm. Experimentul se va relua când ajungeți la distanța corectă.",
    pl: "Twoje oczy są xx1 cm od ekranu, ale eksperyment wymaga około xx2 cm. Eksperyment zostanie wznowiony po osiągnięciu prawidłowej odległości.",
    ru: "Ваши глаза находятся в xx1 см от экрана, но эксперимент нуждается в приблизительно xx2 см. Эксперимент возобновится, когда вы достигнете правильного расстояния.",
    hy: "Ձեր աչքերը էկրանից xx1 սմ-ում են, բայց փորձը պետք է մոտավորապես xx2 սմ: Փորձը կվերսկսվի ճիշտ հեռավորության վրա:",
    fi: "Silmäsi ovat xx1 cm näytöltä, mutta koe tarvitsee noin xx2 cm. Koe jatkuu, kun saavutat oikean etäisyyden.",
    ka: "თქვენი თვალები ეკრანიდან xx1 სმ არის, მაგრამ ექსპერიმენტს სჭირდება დაახლოებით xx2 სმ. ექსპერიმენტი განახლდება, როდესაც სწორ მანძილზე მიაღწევთ.",
    he: 'העיניים שלך הן xx1 ס"מ מהמסך, אך הניסוי זקוק לכ- xx2 ס"מ. הניסוי יחודש כשתגיע למרחק הנכון.',
    ar: "عيناك هي xx1 سم من الشاشة ، لكن التجربة تحتاج إلى حوالي xx2 سم. ستستأنف التجربة عند الوصول إلى المسافة الصحيحة.",
    ur: "آپ کی آنکھیں اسکرین سے xx1 سینٹی میٹر ہیں ، لیکن اس تجربے کو تقریبا x xx2 سینٹی میٹر کی ضرورت ہے۔ جب آپ صحیح فاصلے پر پہنچیں گے تو تجربہ دوبارہ شروع ہوگا۔",
    hi: "आपकी आँखें स्क्रीन से xx1 सेमी हैं, लेकिन प्रयोग को लगभग xx2 सेमी की आवश्यकता होती है। जब आप सही दूरी तक पहुंचेंगे तो प्रयोग फिर से शुरू हो जाएगा।",
    ta: "உங்கள் கண்கள் திரையில் இருந்து xx1 செ.மீ., ஆனால் எக்ஸ் xx2 செமீ பற்றி தேவை. நீங்கள் விரும்பிய தூரம் செல்லும்போது பரிசோதனை மீண்டும் தொடங்கும்.",
    ml: "സ്ക്രീനിൽ നിന്നുള്ള xx1 സെന്റിമീറ്റർ നിങ്ങളുടെ കണ്ണുകൾ ഉണ്ട്, പക്ഷേ പരീക്ഷണത്തിന് ഏകദേശം xx2 സെ. നിങ്ങൾ ശരിയായ ദൂരത്തിലെത്തുമ്പോൾ പരീക്ഷണം പുനരാരംഭിക്കും.",
    te: "మీ కళ్ళు స్క్రీన్ నుండి xx1 సెం.మీ., కానీ ప్రయోగానికి సుమారు xx2 సెం.మీ. మీరు సరైన దూరానికి చేరుకున్నప్పుడు ప్రయోగం తిరిగి ప్రారంభమవుతుంది.",
    kn: "ನಿಮ್ಮ ಕಣ್ಣುಗಳು ಪರದೆಯಿಂದ xx1 ಸೆಂ.ಮೀ., ಆದರೆ ಪ್ರಯೋಗಕ್ಕೆ ಸುಮಾರು xx2 ಸೆಂ ಅಗತ್ಯವಿದೆ. ನೀವು ಸರಿಯಾದ ದೂರವನ್ನು ತಲುಪಿದಾಗ ಪ್ರಯೋಗವು ಪುನರಾರಂಭಗೊಳ್ಳುತ್ತದೆ.",
    bn: "আপনার চোখ স্ক্রিন থেকে xx1 সেমি, তবে পরীক্ষায় প্রায় xx2 সেমি প্রয়োজন। আপনি যখন সঠিক দূরত্বে পৌঁছেছেন তখন পরীক্ষাটি আবার শুরু হবে।",
    id: "Mata Anda adalah xx1 cm dari layar, tetapi percobaan membutuhkan sekitar xx2 cm. Eksperimen akan dilanjutkan saat Anda mencapai jarak yang benar.",
    "zh-CN":
      "您的眼睛离屏幕约 xx1 厘米，但我们需要大约 xx2 厘米。 当您移动到接近的距离后实验将会继续进行。",
    "zh-HK":
      "您的眼睛離屏幕約 xx1 厘米，但我們需要大約 xx2 厘米。 當您移動到接近的距離後實驗將會繼續進行。",
    ja: "あなたの目は画面からxx1 cmですが、実験ではおよそxx2 cmを必要とします。適切な距離になると実験が再開されます。",
    ko: "눈은 화면에서 xx1 cm이지만 실험은 대략 xx2 cm가 필요합니다. 올바른 거리에 도달하면 실험이 재개됩니다.",
  },
  RC_distanceTrackingIntroEnd: {
    "en-US": "Once you're done, distance tracking will begin.",
    de: "Sobald Sie fertig sind, beginnt die Distanzverfolgung.",
    fr: "Une fois que vous aurez terminé, le suivi à distance commencera.",
    es: "Una vez que haya terminado, comenzará el seguimiento de distancia.",
    pt: "Quando terminar, o rastreamento da distância começará.",
    it: "Una volta che hai finito, inizierà il monitoraggio della distanza.",
    ro: "După ce ați terminat, va începe urmărirea distanței.",
    pl: "Po zakończeniu rozpocznie się śledzenie odległości.",
    ru: "Как только вы закончите, начнется отслеживание расстояний.",
    hy: "Ավարտելուց հետո սկսվելու է հեռավորության հետեւում:",
    fi: "Kun olet valmis, etäisyysseuranta alkaa.",
    ka: "დასრულების შემდეგ, დისტანციური თვალყურის დევნება დაიწყება.",
    he: "לאחר שתסיים, מעקב מרחק יתחיל.",
    ar: "بمجرد الانتهاء من ذلك ، سيبدأ تتبع المسافة.",
    ur: "ایک بار جب آپ کام کرلیں تو ، فاصلے سے باخبر رہنا شروع ہوجائے گا۔",
    hi: "एक बार जब आप कर लेते हैं, तो दूरी ट्रैकिंग शुरू हो जाएगी।",
    ta: "நீங்கள் முடித்ததும், தூர கண்காணிப்பு தொடங்கும்.",
    ml: "നിങ്ങൾ പൂർത്തിയാക്കിക്കഴിഞ്ഞാൽ, വിദൂര ട്രാക്കിംഗ് ആരംഭിക്കും.",
    te: "మీరు పూర్తి చేసిన తర్వాత, దూర ట్రాకింగ్ ప్రారంభమవుతుంది.",
    kn: "ನೀವು ಮುಗಿದ ನಂತರ, ದೂರ ಟ್ರ್ಯಾಕಿಂಗ್ ಪ್ರಾರಂಭವಾಗುತ್ತದೆ.",
    bn: "আপনার কাজ শেষ হয়ে গেলে, দূরত্বের ট্র্যাকিং শুরু হবে।",
    id: "Setelah selesai, pelacakan jarak akan dimulai.",
    "zh-CN": "在您完成后，视距追踪将开始。",
    "zh-HK": "在您完成後，視距追踪將開始。",
    ja: "完了したら、距離追跡が開始されます。",
    ko: "완료되면 거리 추적이 시작됩니다.",
  },
  RC_distanceTrackingIntroStart: {
    "en-US": "",
    de: "",
    fr: "",
    es: "",
    pt: "",
    it: "",
    ro: "",
    pl: "",
    ru: "",
    hy: "",
    fi: "",
    ka: "",
    he: "",
    ar: "",
    ur: "",
    hi: "",
    ta: "",
    ml: "",
    te: "",
    kn: "",
    bn: "",
    id: "",
    "zh-CN": "",
    "zh-HK": "",
    ja: "",
    ko: "",
  },
  RC_distanceTrackingMoveCloser: {
    "en-US": "Move closer",
    de: "Komm näher",
    fr: "Approchez-vous",
    es: "Acércate a la pantalla",
    pt: "Aproveite -se",
    it: "Avvicinarsi",
    ro: "Vino mai aproape",
    pl: "Podejdź bliżej",
    ru: "Двигайтесь ближе",
    hy: "Շարժվել ավելի մոտ",
    fi: "Liikkua lähemmäs",
    ka: "უფრო ახლოს გადაადგილება",
    he: "תתקרב",
    ar: "تقترب",
    ur: "قریب جائیں",
    hi: "निकट आएं",
    ta: "நெருங்கி நகர்த்தவும்",
    ml: "അടുത്ത് നീങ്ങുക",
    te: "దగ్గరగా కదలండి",
    kn: "ಹತ್ತಿರ ಸರಿಸಿ",
    bn: "আরো কাছে আস",
    id: "Merapat",
    "zh-CN": "靠近一点。",
    "zh-HK": "靠近一點。",
    ja: "もっと近づいてください。",
    ko: "더 가까이 움직입니다",
  },
  RC_distanceTrackingMoveFurther: {
    "en-US": "Move farther",
    de: "Sich weiter bewegen",
    fr: "Se déplacer plus loin",
    es: "Aléjate de la plantalla",
    pt: "Mover mais longe",
    it: "Muoversi più lontano",
    ro: "Mutați -vă mai departe",
    pl: "Poruszaj się dalej",
    ru: "Двигаться дальше",
    hy: "Առաջ շարժվել",
    fi: "Liikkua kauemmas",
    ka: "უფრო შორს გადაადგილება",
    he: "לנוע רחוק יותר",
    ar: "تحرك أبعد",
    ur: "آگے بڑھیں",
    hi: "आगे बढ़ना",
    ta: "தூரம் செல்லுங்கள்",
    ml: "കൂടുതൽ നീങ്ങുക",
    te: "దూరంగా కదలండి",
    kn: "ದೂರ ಸರಿಸಿ",
    bn: "আরও দূরে সরানো",
    id: "Bergerak lebih jauh",
    "zh-CN": "离远一点。",
    "zh-HK": "離遠一點。",
    ja: "もっと離れてください。",
    ko: "더 멀리 움직입니다",
  },
  RC_distanceTrackingRedo: {
    "en-US":
      "PROBLEM? If the distance seems terribly wrong, click here to recalibrate it, and restart this block.",
    de: "PROBLEM? Wenn die Entfernung furchtbar falsch erscheint, klicken Sie hier, um sie neu zu kalibrieren und diesen Block neu zu starten.",
    fr: "PROBLÈME? Si la distance semble terriblement erronée, cliquez ici pour le recalibrer et redémarrez ce bloc.",
    es: "¿PROBLEMA? Si la distancia parece terriblemente incorrecta, haga clic aquí para recalibrarla y reiniciar este bloque.",
    pt: "PROBLEMA? Se a distância parecer terrivelmente errada, clique aqui para recalibrá -la e reinicie esse bloco.",
    it: "PROBLEMA? Se la distanza sembra terribilmente sbagliata, fai clic qui per ricalibrarla e riavviare questo blocco.",
    ro: "PROBLEMĂ? Dacă distanța pare teribil de greșită, faceți clic aici pentru a -l recalibra și reporniți acest bloc.",
    pl: "PROBLEM? Jeśli odległość wydaje się strasznie błędna, kliknij tutaj, aby ją ponownie kalibrować i uruchom ponownie ten blok.",
    ru: "ПРОБЛЕМА? Если расстояние кажется ужасно неправильным, нажмите здесь, чтобы перекалибровать его и перезапустить этот блок.",
    hy: "Խնդիր? Եթե ​​հեռավորությունը սարսափելի սխալ է թվում, կտտացրեք այստեղ `այն վերահաշվարկելու եւ այս բլոկը վերագործարկելու համար:",
    fi: "ONGELMA? Jos etäisyys näyttää kauhean väärin, napsauta tätä uudelleen kalibroidaksesi uudelleen ja käynnistä tämä lohko uudelleen.",
    ka: "პრობლემა? თუ მანძილი საშინლად არასწორია, დააჭირეთ აქ, რომ გადააკეთოთ იგი და გადატვირთოთ ეს ბლოკი.",
    he: "בְּעָיָה? אם המרחק נראה שגוי להחריד, לחץ כאן כדי לכייל אותו מחדש ולהפעיל מחדש את החסימה הזו.",
    ar: "مشكلة؟ إذا كانت المسافة تبدو خاطئة بشكل رهيب ، فانقر هنا لإعادة معايرةها ، وأعد تشغيل هذه الكتلة.",
    ur: "مسئلہ؟ اگر فاصلہ بہت غلط معلوم ہوتا ہے تو ، اس کی بحالی کے لئے یہاں کلک کریں ، اور اس بلاک کو دوبارہ شروع کریں۔",
    hi: "संकट? यदि दूरी बहुत गलत लगती है, तो इसे पुन: व्यवस्थित करने के लिए यहां क्लिक करें, और इस ब्लॉक को पुनरारंभ करें।",
    ta: "பிரச்சனை? தூரம் மிகவும் தவறானது என்று தோன்றினால், அதை மறுபரிசீலனை செய்ய இங்கே கிளிக் செய்து, இந்த தொகுதியை மறுதொடக்கம் செய்யுங்கள்.",
    ml: "പ്രശ്നം? ദൂരം ഭയങ്കരമായി തെറ്റാണെന്ന് തോന്നുകയാണെങ്കിൽ, അത് വീണ്ടും വാങ്ങാനായി ഇവിടെ ക്ലിക്കുചെയ്യുക, ഈ ബ്ലോക്ക് പുനരാരംഭിക്കുക.",
    te: "సమస్య? దూరం చాలా తప్పుగా అనిపిస్తే, దాన్ని రీకాలిబ్రేట్ చేయడానికి ఇక్కడ క్లిక్ చేయండి మరియు ఈ బ్లాక్‌ను పున art ప్రారంభించండి.",
    kn: "ಸಮಸ್ಯೆ? ದೂರವು ಭಯಾನಕ ತಪ್ಪು ಎಂದು ತೋರುತ್ತಿದ್ದರೆ, ಅದನ್ನು ಮರುಸಂಗ್ರಹಿಸಲು ಇಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡಿ ಮತ್ತು ಈ ಬ್ಲಾಕ್ ಅನ್ನು ಮರುಪ್ರಾರಂಭಿಸಿ.",
    bn: "সমস্যা? যদি দূরত্বটি ভয়াবহভাবে ভুল বলে মনে হয় তবে এটি পুনরুদ্ধার করতে এখানে ক্লিক করুন এবং এই ব্লকটি পুনরায় চালু করুন।",
    id: "MASALAH? Jika jaraknya tampaknya sangat salah, klik di sini untuk mengkalibrasi ulang, dan memulai kembali blok ini.",
    "zh-CN":
      "问题？如果距离似乎非常错误，请单击此处重新校准，然后重新启动此块。",
    "zh-HK":
      "問題？如果距離似乎非常錯誤，請單擊此處重新校準，然後重新啟動此塊。",
    ja: "問題？距離がひどく間違っているように見える場合は、ここをクリックして再調整し、このブロックを再起動してください。",
    ko: "문제? 거리가 끔찍한 것처럼 보이면 여기를 클릭하여 다시 교정 하고이 블록을 다시 시작하십시오.",
  },
  RC_distanceTrackingTitle: {
    "en-US": "Find Your Blindspot",
    de: "Finden Sie Ihren Blindspot",
    fr: "Trouvez votre blindspot",
    es: "Configurar para seguimiento de distancia",
    pt: "Configurar para rastreamento de distância",
    it: "Trova il tuo cieco",
    ro: "Configurați pentru urmărirea distanței",
    pl: "Skonfiguruj śledzenie odległości ",
    ru: "Найдите свою слепую точку",
    hy: "Գտեք ձեր կուր կետը",
    fi: "Löydä blindspot",
    ka: "იპოვნეთ თქვენი ჟალუზის წერტილი",
    he: "מצא את העיוור שלך",
    ar: "الإعداد لتتبع الرأس",
    ur: "سر سے باخبر رہنے کے لئے تیار ہوجائیں\n",
    hi: "हेड ट्रैकिंग के लिए तयार हो जाए\n",
    ta: "உங்கள் பிளைண்ட்ஸ்பாட்டைக் கண்டறியவும்",
    ml: "നിങ്ങളുടെ ബ്ലൈൻഡ്സ്പോട്ട് കണ്ടെത്തുക",
    te: "మీ బ్లైండ్‌స్పాట్‌ను కనుగొనండి",
    kn: "ನಿಮ್ಮ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್ ಅನ್ನು ಹುಡುಕಿ",
    bn: "আপনার ব্লাইন্ডস্পটটি সন্ধান করুন",
    id: "Temukan blindspot Anda",
    "zh-CN": "视距追踪初始化",
    "zh-HK": "視距追踪初始化",
    ja: "距離追跡を設定します。",
    ko: "블라인드 스팟을 찾으십시오",
  },
  RC_environment: {
    "en-US": "System Information",
    de: "System Information",
    fr: "Informations sur le système",
    es: "Información del sistema",
    pt: "Informação do sistema",
    it: "Informazioni sul sistema",
    ro: "Informatii despre sistem",
    pl: "Informacje o systemie",
    ru: "Системная информация",
    hy: "Համակարգի տեղեկատվություն",
    fi: "Järjestelmätiedot",
    ka: "Სისტემის ინფორმაცია",
    he: "מידע מערכת",
    ar: "معلومات النظام",
    ur: "سسٹم کی معلومات",
    hi: "सिस्टम की जानकारी",
    ta: "கணினி தகவல்",
    ml: "സിസ്റ്റം വിവരങ്ങൾ",
    te: "సిస్టమ్ సమాచారం",
    kn: "ಯಂತ್ರದ ಮಾಹಿತಿ",
    bn: "পদ্ধতিগত তথ্য",
    id: "Sistem Informasi",
    "zh-CN": "系统信息",
    "zh-HK": "系統信息",
    ja: "システム情報",
    ko: "시스템 정보",
  },
  RC_errorCameraUseDenied: {
    "en-US":
      "This study requires using the webcam. Granting access to your webcam is optional, but we can't continue without it. If you said No by mistake and now wish to give us access to your camera, please find the camera icon in the address bar, click it, allow camera access, and refresh the page.",
    de: "Diese Studie erfordert die Verwendung der Webcam. Es ist optional, Zugriff auf Ihre Webcam zu gewähren, aber wir können ohne sie nicht weitermachen. Wenn Sie versehentlich nein gesagt haben und uns nun Zugriff auf Ihre Kamera ermöglichen möchten, finden Sie das Kamerasymbol in der Adressleiste, klicken Sie darauf, ermöglichen Sie die Kamerazugriff und aktualisieren Sie die Seite.",
    fr: "Cette étude nécessite l'utilisation de la webcam. Accorder l'accès à votre webcam est facultatif, mais nous ne pouvons pas continuer sans lui. Si vous avez dit non par erreur et que vous souhaitez maintenant nous donner accès à votre appareil photo, veuillez trouver l'icône de la caméra dans la barre d'adresse, cliquez dessus, permettez à l'accès de la caméra et actualisez la page.",
    es: "Este estudio requiere usar la cámara web. Otorgar acceso a su cámara web es opcional, pero no podemos continuar sin ella. Si dijo que no por error y ahora desea darnos acceso a su cámara, busque el icono de la cámara en la barra de direcciones, haga clic en, permita el acceso de la cámara y actualice la página.",
    pt: "Este estudo requer o uso da webcam. A concessão de acesso à sua webcam é opcional, mas não podemos continuar sem ela. Se você disse não por engano e agora deseja nos dar acesso à sua câmera, encontre o ícone da câmera na barra de endereços, clique nele, permita o acesso à câmera e atualize a página.",
    it: "Questo studio richiede l'uso della webcam. Concedere l'accesso alla tua webcam è facoltativo, ma non possiamo continuare senza di essa. Se hai detto di no per errore e ora desideri darci l'accesso alla fotocamera, ti preghiamo di trovare l'icona della fotocamera nella barra degli indirizzi, fare clic su di esso, consentire l'accesso alla fotocamera e aggiornare la pagina.",
    ro: "Acest studiu necesită utilizarea camerei web. Acordarea accesului la camera web este opțională, dar nu putem continua fără el. Dacă nu ați spus nu din greșeală și acum doriți să ne oferiți acces la camera dvs., vă rugăm să găsiți pictograma camerei în bara de adrese, faceți clic pe ea, permiteți accesul camerei și reîmprospătați pagina.",
    pl: "To badanie wymaga użycia kamery internetowej. Przyznanie dostępu do kamery internetowej jest opcjonalne, ale nie możemy bez niego kontynuować. Jeśli powiedziałeś nie przez pomyłkę i teraz chcesz dać nam dostęp do aparatu, znajdź ikonę aparatu na pasku adresu, kliknij ją, zezwól na dostęp do aparatu i odśwież stronę.",
    ru: "Это исследование требует использования веб -камеры. Предоставление доступа к вашей веб -камере не является обязательным, но мы не можем продолжать без нее. Если вы сказали «нет по ошибке», и теперь хотите предоставить нам доступ к вашей камере, найдите значок камеры в адресной панели, нажмите на нее, разрешить доступ к камере и обновить страницу.",
    hy: "Այս ուսումնասիրությունը պահանջում է օգտագործել վեբ-տեսախցիկը: Ձեր վեբ-տեսախցիկ մուտք ունենալը պարտադիր չէ, բայց մենք չենք կարող շարունակել առանց դրա: Եթե ​​սխալմամբ ասեք, եւ հիմա ցանկանում եք մեզ մուտք գործել ձեր տեսախցիկ, խնդրում ենք գտնել տեսախցիկի պատկերակը Հասցեի բարում, կտտացրեք այն եւ թարմացրեք էջը:",
    fi: "Tämä tutkimus vaatii verkkokameran käyttöä. Pääsy verkkokamerallesi on valinnaista, mutta emme voi jatkaa ilman sitä. Jos sanoit vahingossa ja haluat nyt antaa meille pääsyn kameraan, etsi kamerakuvake osoitepalkista, napsauta sitä, salli kameran käyttöoikeus ja päivitä sivu.",
    ka: "ეს კვლევა მოითხოვს ვებკამერის გამოყენებას. თქვენს ვებკამერაზე წვდომის მინიჭება არჩევითია, მაგრამ ამის გარეშე ჩვენ ვერ გავაგრძელებთ. თუ შეცდომით უარი თქვით და ახლა გსურთ მოგვმართოთ თქვენი კამერა, გთხოვთ, იპოვოთ კამერის ხატი მისამართის ზოლში, დააჭირეთ მას, დაუშვით კამერის წვდომა და განაახლეთ გვერდი.",
    he: "מחקר זה דורש שימוש במצלמת הרשת. מתן גישה למצלמת הרשת שלך הוא אופציונלי, אך איננו יכולים להמשיך בלעדיו. אם אמרת לא בטעות וכעת ברצונך לתת לנו גישה למצלמה שלך, אנא מצא את סמל המצלמה בסרגל הכתובות, לחץ עליו, אפשר גישה למצלמה ורענן את הדף.",
    ar: "تتطلب هذه الدراسة استخدام كاميرا الويب. يعد منح الوصول إلى كاميرا الويب الخاصة بك اختياريًا ، لكن لا يمكننا المتابعة بدونها. إذا قلت لا عن طريق الخطأ وترغب الآن في منحنا الوصول إلى الكاميرا ، فيرجى العثور على أيقونة الكاميرا في شريط العناوين ، وانقر فوقه ، والسماح للوصول إلى الكاميرا ، وتحديث الصفحة.",
    ur: "اس مطالعے کے لئے ویب کیم استعمال کرنے کی ضرورت ہے۔ آپ کے ویب کیم تک رسائی دینا اختیاری ہے ، لیکن ہم اس کے بغیر جاری نہیں رکھ سکتے۔ اگر آپ نے غلطی سے نہیں کہا ہے اور اب ہمیں اپنے کیمرہ تک رسائی دینا چاہتے ہیں تو ، براہ کرم ایڈریس بار میں کیمرہ آئیکن تلاش کریں ، اس پر کلک کریں ، کیمرہ تک رسائی کی اجازت دیں ، اور صفحہ کو تازہ کریں۔",
    hi: "इस अध्ययन के लिए वेबकैम का उपयोग करने की आवश्यकता है। अपने वेबकैम तक पहुंच प्रदान करना वैकल्पिक है, लेकिन हम इसके बिना जारी नहीं रख सकते। यदि आपने गलती से नहीं कहा है और अब हमें अपने कैमरे तक पहुंच देना चाहते हैं, तो कृपया एड्रेस बार में कैमरा आइकन ढूंढें, इसे क्लिक करें, कैमरा एक्सेस की अनुमति दें, और पेज को रिफ्रेश करें।",
    ta: "இந்த ஆய்வுக்கு வெப்கேம் பயன்படுத்த வேண்டும். உங்கள் வெப்கேமுக்கு அணுகலை வழங்குவது விருப்பமானது, ஆனால் அது இல்லாமல் எங்களால் தொடர முடியாது. நீங்கள் தவறில்லை என்று சொன்னால், இப்போது உங்கள் கேமராவை எங்களுக்கு அணுக விரும்பினால், தயவுசெய்து முகவரி பட்டியில் கேமரா ஐகானைக் கண்டுபிடித்து, அதைக் கிளிக் செய்து, கேமரா அணுகலை அனுமதிக்கவும், பக்கத்தைப் புதுப்பிக்கவும்.",
    ml: "ഈ പഠനത്തിന് വെബ്ക്യാം ഉപയോഗിക്കേണ്ടതുണ്ട്. നിങ്ങളുടെ വെബ്ക്യാമിലേക്കുള്ള ആക്സസ് അനുവദിക്കുന്നത് ഓപ്ഷണലാണ്, പക്ഷേ ഞങ്ങൾക്ക് ഇത് കൂടാതെ തുടരാനാവില്ല. നിങ്ങൾ അബദ്ധത്തിൽ പറഞ്ഞിട്ടും നിങ്ങളുടെ ക്യാമറയിലേക്ക് ഞങ്ങൾക്ക് ആക്സസ് നൽകാൻ ആഗ്രഹിക്കുന്നുവെങ്കിൽ, ദയവായി വിലാസ ബാറിലെ ക്യാമറ ഐക്കൺ കണ്ടെത്തുക, അതിൽ ക്ലിക്കുചെയ്യുക, ക്യാമറ ആക്സസ് അനുവദിക്കുക, പേജ് പുതുക്കുക.",
    te: "ఈ అధ్యయనానికి వెబ్‌క్యామ్‌ను ఉపయోగించడం అవసరం. మీ వెబ్‌క్యామ్‌కు ప్రాప్యతను ఇవ్వడం ఐచ్ఛికం, కానీ మేము అది లేకుండా కొనసాగలేము. మీరు పొరపాటున నో చెప్పి, ఇప్పుడు మీ కెమెరాకు మాకు ప్రాప్యత ఇవ్వాలనుకుంటే, దయచేసి చిరునామా పట్టీలో కెమెరా చిహ్నాన్ని కనుగొనండి, దాన్ని క్లిక్ చేయండి, కెమెరా యాక్సెస్‌ను అనుమతించండి మరియు పేజీని రిఫ్రెష్ చేయండి.",
    kn: "ಈ ಅಧ್ಯಯನಕ್ಕೆ ವೆಬ್‌ಕ್ಯಾಮ್ ಬಳಸುವ ಅಗತ್ಯವಿದೆ. ನಿಮ್ಮ ವೆಬ್‌ಕ್ಯಾಮ್‌ಗೆ ಪ್ರವೇಶವನ್ನು ನೀಡುವುದು ಐಚ್ al ಿಕವಾಗಿದೆ, ಆದರೆ ಅದು ಇಲ್ಲದೆ ನಾವು ಮುಂದುವರಿಯಲು ಸಾಧ್ಯವಿಲ್ಲ. ನೀವು ತಪ್ಪಾಗಿ ಇಲ್ಲ ಎಂದು ಹೇಳಿದ್ದರೆ ಮತ್ತು ಈಗ ನಿಮ್ಮ ಕ್ಯಾಮೆರಾಗೆ ನಮಗೆ ಪ್ರವೇಶವನ್ನು ನೀಡಲು ಬಯಸಿದರೆ, ದಯವಿಟ್ಟು ವಿಳಾಸ ಪಟ್ಟಿಯಲ್ಲಿರುವ ಕ್ಯಾಮೆರಾ ಐಕಾನ್ ಅನ್ನು ಹುಡುಕಿ, ಅದನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ, ಕ್ಯಾಮೆರಾ ಪ್ರವೇಶವನ್ನು ಅನುಮತಿಸಿ ಮತ್ತು ಪುಟವನ್ನು ರಿಫ್ರೆಶ್ ಮಾಡಿ.",
    bn: "এই অধ্যয়নের জন্য ওয়েবক্যাম ব্যবহার করা দরকার। আপনার ওয়েবক্যামে অ্যাক্সেস দেওয়া al চ্ছিক, তবে আমরা এটি ছাড়া চালিয়ে যেতে পারি না। যদি আপনি ভুল করে না বলেন এবং এখন আমাদের আপনার ক্যামেরায় অ্যাক্সেস দিতে চান তবে দয়া করে ঠিকানা বারে ক্যামেরা আইকনটি সন্ধান করুন, এটি ক্লিক করুন, ক্যামেরা অ্যাক্সেসের অনুমতি দিন এবং পৃষ্ঠাটি রিফ্রেশ করুন।",
    id: "Studi ini membutuhkan penggunaan webcam. Memberikan akses ke webcam Anda adalah opsional, tetapi kami tidak dapat melanjutkan tanpanya. Jika Anda mengatakan tidak secara tidak sengaja dan sekarang ingin memberi kami akses ke kamera Anda, silakan temukan ikon kamera di bilah alamat, klik, izinkan akses kamera, dan menyegarkan halamannya.",
    "zh-CN":
      "这项研究需要使用网络摄像头。授予您网络摄像头的访问是可选的，但是没有它，我们将无法继续。如果您错误地说，现在希望让我们访问您的相机，请在地址栏中找到相机图标，单击它，允许访问摄像头并刷新页面。",
    "zh-HK":
      "這項研究需要使用網絡攝像頭。授予您網絡攝像頭的訪問是可選的，但是沒有它，我們將無法繼續。如果您錯誤地說，現在希望讓我們訪問您的相機，請在地址欄中找到相機圖標，單擊它，允許訪問攝像頭並刷新頁面。",
    ja: "この調査では、ウェブカメラを使用する必要があります。ウェブカメラへのアクセスを許可することはオプションですが、それなしでは継続することはできません。誤ってノーと言って、カメラにアクセスできるようにしたい場合は、アドレスバーにカメラアイコンを見つけて、クリックしてカメラアクセスを許可し、ページを更新してください。",
    ko: "이 연구는 웹캠을 사용해야합니다. 웹캠에 대한 액세스 권한을 부여하는 것은 선택 사항이지만, 우리는 그 없이는 계속할 수 없습니다. 실수로 거절하지 말고 카메라에 액세스 할 수 있으시면 주소 표시 줄에서 카메라 아이콘을 찾아 클릭하고 카메라에 액세스 할 수 있으며 페이지를 새로 고치십시오.",
  },
  RC_errorNoCamera: {
    "en-US":
      "This study requires a webcam, but we can't find one. Please check if your webcam is connected correctly and supported by the browser.",
    de: "Diese Studie erfordert eine Webcam, aber wir können keine finden. Bitte überprüfen Sie, ob Ihre Webcam korrekt angeschlossen und vom Browser unterstützt wird.",
    fr: "Cette étude nécessite une webcam, mais nous ne pouvons pas en trouver une. Veuillez vérifier si votre webcam est connectée correctement et prise en charge par le navigateur.",
    es: "Este estudio requiere una cámara web, pero no podemos encontrar una. Compruebe si su cámara web está conectada correctamente y admite el navegador.",
    pt: "Este estudo requer uma webcam, mas não podemos encontrar um. Verifique se sua webcam está conectada corretamente e suportada pelo navegador.",
    it: "Questo studio richiede una webcam, ma non possiamo trovarne una. Controlla se la tua webcam è connessa correttamente e supportata dal browser.",
    ro: "Acest studiu necesită o cameră web, dar nu putem găsi unul. Vă rugăm să verificați dacă camera web este conectată corect și acceptată de browser.",
    pl: "To badanie wymaga kamery internetowej, ale nie możemy jej znaleźć. Sprawdź, czy kamera internetowa jest poprawnie podłączona i obsługiwana przez przeglądarkę.",
    ru: "Это исследование требует веб -камеры, но мы не можем ее найти. Пожалуйста, проверьте, правильно ли подключена ваша веб -камера и поддерживается браузером.",
    hy: "Այս ուսումնասիրությունը պահանջում է վեբ-տեսախցիկ, բայց մենք չենք կարող գտնել մեկը: Խնդրում ենք ստուգել, ​​արդյոք ձեր վեբ-տեսախցիկը միացված է ճիշտ եւ աջակցվում է զննարկչի կողմից:",
    fi: "Tämä tutkimus vaatii verkkokameran, mutta emme löydä sitä. Tarkista, onko verkkokamerasi kytketty oikein ja tukee se selain.",
    ka: "ამ კვლევას ვებკამერა სჭირდება, მაგრამ ვერ ვპოულობთ. გთხოვთ, შეამოწმოთ, თქვენი ვებკამერა სწორად არის დაკავშირებული და ბრაუზერის მხარდაჭერით.",
    he: "מחקר זה דורש מצלמת רשת, אך איננו יכולים למצוא אחת כזו. אנא בדוק אם מצלמת הרשת שלך מחוברת כראוי ותומכת על ידי הדפדפן.",
    ar: "تتطلب هذه الدراسة كاميرا ويب ، لكن لا يمكننا العثور على واحدة. يرجى التحقق مما إذا كانت كاميرا الويب الخاصة بك متصلة بشكل صحيح ودعمها المتصفح.",
    ur: "اس مطالعے کے لئے ایک ویب کیم کی ضرورت ہے ، لیکن ہم کوئی نہیں ڈھونڈ سکتے ہیں۔ براہ کرم چیک کریں کہ آیا آپ کا ویب کیم صحیح طور پر منسلک ہے اور براؤزر کے ذریعہ اس کی تائید کی گئی ہے۔",
    hi: "इस अध्ययन के लिए एक वेबकैम की आवश्यकता होती है, लेकिन हम एक नहीं पा सकते हैं। कृपया जांचें कि क्या आपका वेबकैम सही तरीके से जुड़ा हुआ है और ब्राउज़र द्वारा समर्थित है।",
    ta: "இந்த ஆய்வுக்கு ஒரு வெப்கேம் தேவைப்படுகிறது, ஆனால் ஒன்றைக் கண்டுபிடிக்க முடியவில்லை. உங்கள் வெப்கேம் சரியாக இணைக்கப்பட்டு உலாவியால் ஆதரிக்கப்படுகிறதா என்பதை சரிபார்க்கவும்.",
    ml: "ഈ പഠനത്തിന് ഒരു വെബ്ക്യാം ആവശ്യമാണ്, പക്ഷേ ഞങ്ങൾക്ക് ഒന്ന് കണ്ടെത്താൻ കഴിയില്ല. നിങ്ങളുടെ വെബ്ക്യാം ശരിയായി ബന്ധിപ്പിച്ച് ബ്ര browser സർ പിന്തുണയ്ക്കുന്നുണ്ടോയെന്ന് പരിശോധിക്കുക.",
    te: "ఈ అధ్యయనానికి వెబ్‌క్యామ్ అవసరం, కానీ మేము ఒకదాన్ని కనుగొనలేము. దయచేసి మీ వెబ్‌క్యామ్ సరిగ్గా కనెక్ట్ అయి బ్రౌజర్ మద్దతు ఇస్తుందో లేదో తనిఖీ చేయండి.",
    kn: "ಈ ಅಧ್ಯಯನಕ್ಕೆ ವೆಬ್‌ಕ್ಯಾಮ್ ಅಗತ್ಯವಿದೆ, ಆದರೆ ನಮಗೆ ಒಂದನ್ನು ಕಂಡುಹಿಡಿಯಲಾಗುವುದಿಲ್ಲ. ನಿಮ್ಮ ವೆಬ್‌ಕ್ಯಾಮ್ ಅನ್ನು ಸರಿಯಾಗಿ ಸಂಪರ್ಕಿಸಲಾಗಿದೆಯೇ ಮತ್ತು ಬ್ರೌಸರ್‌ನಿಂದ ಬೆಂಬಲಿಸಲಾಗಿದೆಯೇ ಎಂದು ದಯವಿಟ್ಟು ಪರಿಶೀಲಿಸಿ.",
    bn: "এই অধ্যয়নের জন্য একটি ওয়েবক্যামের প্রয়োজন, তবে আমরা এটি খুঁজে পাচ্ছি না। আপনার ওয়েবক্যামটি সঠিকভাবে সংযুক্ত এবং ব্রাউজার দ্বারা সমর্থিত কিনা তা পরীক্ষা করে দেখুন।",
    id: "Studi ini membutuhkan webcam, tetapi kami tidak dapat menemukannya. Silakan periksa apakah webcam Anda terhubung dengan benar dan didukung oleh browser.",
    "zh-CN": "我们找不到您的摄像头。",
    "zh-HK": "我們找不到您的攝像頭。",
    ja: "この調査ではウェブカメラが必要ですが、見つけることができません。ウェブカメラが正しく接続され、ブラウザによってサポートされているかどうかを確認してください。",
    ko: "이 연구에는 웹캠이 필요하지만 우리는 찾을 수 없습니다. 웹캠이 올바르게 연결되어 브라우저에서 지원되는지 확인하십시오.",
  },
  RC_gazeTracking: {
    "en-US": "Gaze Tracking",
    de: "Blickverfolgung",
    fr: "Suivi du regard",
    es: "Seguimiento de la mirada",
    pt: "Olhando",
    it: "Monitoraggio dello sguardo",
    ro: "In cautarea",
    pl: "Śledzenie wzroku.",
    ru: "Отслеживание взгляда",
    hy: "Նայելով հայացքը",
    fi: "Katseiden seuranta",
    ka: "მზერა თვალყურის დევნება",
    he: "מעקב אחר מבט",
    ar: "تتبع النظر",
    ur: "آنکھ سے باخبر رہنا",
    hi: "आँखों का ट्रैकिंग",
    ta: "பார்வை கண்காணிப்பு",
    ml: "ലോട്ടർ ട്രാക്കിംഗ്",
    te: "చూపుల ట్రాకింగ్",
    kn: "ನೋಟ",
    bn: "গেজ ট্র্যাকিং",
    id: "Tatapan pelacakan",
    "zh-CN": "眼动追踪",
    "zh-HK": "眼動追踪",
    ja: "視線追跡",
    ko: "시선 추적",
  },
  RC_gazeTrackingIntro: {
    "en-US":
      'PLAY "CATCH THE CROSS!" Now we’ll set up gaze tracking, to monitor eye position. Keep your face in the video feed (see below). Click on the center of the cross hairs, wherever it is. Make sure that your eyes are on it when you click it. When you click it, the cross hairs will move to a new location. Click it in each new location, until the calibration is done. It\'s a game. Catch the cross!',
    de: 'Spielen Sie "Catch the Cross!" Jetzt werden wir eine Blickverfolgung einrichten, um die Augenposition zu überwachen. Behalten Sie Ihr Gesicht im Video -Feed (siehe unten). Klicken Sie auf die Mitte der Kreuzhaare, wo immer es ist. Stellen Sie sicher, dass Ihre Augen darauf sind, wenn Sie darauf klicken. Wenn Sie darauf klicken, bewegen sich die Cross -Haare an einen neuen Ort. Klicken Sie an jedem neuen Standort, bis die Kalibrierung abgeschlossen ist. Es ist ein Spiel. Fang das Kreuz!',
    fr: "Jouez \"Catch the Cross!\" Nous allons maintenant configurer le suivi du regard pour surveiller la position des yeux. Gardez votre visage dans le flux vidéo (voir ci-dessous). Cliquez sur le centre des poils, où qu'il se trouve. Assurez-vous que vos yeux sont dessus lorsque vous cliquez dessus. Lorsque vous cliquez sur celui-ci, les poils se déplaceront vers un nouvel emplacement. Cliquez dessus dans chaque nouvel emplacement jusqu'à ce que l'étalonnage soit terminé. C'est un jeu. Attrapez la croix!",
    es: 'Juega "¡Atrapa la cruz!" Ahora configuraremos el seguimiento de la mirada para monitorear la posición del ojo. Mantenga su cara en la alimentación del video (ver más abajo). Haga clic en el centro de la cruz, donde sea que esté. Asegúrese de que sus ojos estén en él cuando haga clic en él. Cuando haga clic en él, los pelos cruzados se moverán a una nueva ubicación. Haga clic en cada nueva ubicación, hasta que se realice la calibración. Es un juego. ¡Atrapa la cruz!',
    pt: 'Jogue "Catch the Cross!" Agora, configuraremos o rastreamento do olhar, para monitorar a posição dos olhos. Mantenha seu rosto no feed de vídeo (veja abaixo). Clique no centro dos cabelos cruzados, onde quer que esteja. Certifique -se de que seus olhos estejam nele quando clicar nele. Ao clicar nele, os cabelos cruzados se movem para um novo local. Clique nele em cada novo local, até que a calibração esteja concluída. É um jogo. Pegue a cruz!',
    it: 'Gioca "Catch the Cross!" Ora imposteremo il monitoraggio dello sguardo, per monitorare la posizione degli occhi. Tieni la faccia nel feed video (vedi sotto). Fai clic sul centro dei peli incrociati, ovunque si trovi. Assicurati che i tuoi occhi siano sopra quando fai clic. Quando fai clic su di esso, i capelli incrociati si sposteranno in una nuova posizione. Fare clic su ogni nuova posizione, fino a quando la calibrazione è terminata. È un gioco. Cattura la croce!',
    ro: "Joacă „Prinde Crucea!” Acum vom stabili urmărirea privind privirea, pentru a monitoriza poziția ochilor. Păstrați -vă fața în fluxul video (vezi mai jos). Faceți clic pe centrul părului încrucișat, oriunde s -ar afla. Asigurați -vă că ochii dvs. sunt pe el când faceți clic pe el. Când faceți clic pe el, părul încrucișat se va muta într -o nouă locație. Faceți clic pe ea în fiecare locație nouă, până când se termină calibrarea. Este un joc. Crucea!",
    pl: "Zagraj w „Catch the Cross!” Teraz skonfigurujemy śledzenie spojrzenia, aby monitorować pozycję oczu. Trzymaj twarz w kanale wideo (patrz poniżej). Kliknij środek włosów krzyżowych, gdziekolwiek się znajdują. Upewnij się, że twoje oczy są na nim po kliknięciu. Po kliknięciu włosy krzyżowe przeniosą się do nowej lokalizacji. Kliknij go w każdej nowej lokalizacji, aż do zakończenia kalibracji. To jest gra. Złap krzyż!",
    ru: 'Играть "Поймай крест!" Теперь мы настроим отслеживание взгляда, чтобы контролировать положение глаз. Держите лицо в видеопроводе (см. Ниже). Нажмите на центр кросс -волосков, где бы это ни было. Убедитесь, что ваши глаза на нем, когда вы нажимаете на него. Когда вы нажимаете на него, кросс -волоска перейдет в новое место. Нажмите на него в каждом новом месте, пока калибровка не будет сделана. Это игра. Поймай крест!',
    hy: "Խաղացեք «Բռնել խաչը»: Այժմ մենք կստեղծենք հայացք նետելու, հետեւելու աչքի դիրքը: Ձեր դեմքը պահեք տեսանյութի մեջ (տես ստորեւ): Կտտացրեք խաչի մազերի կենտրոնին, որտեղ էլ որ լինի: Համոզվեք, որ ձեր աչքերը դրա վրա են, երբ կտտացրեք այն: Երբ կտտացնում եք այն, խաչաձեւ մազերը կտեղափոխվեն նոր վայր: Կտտացրեք այն յուրաքանչյուր նոր վայրում, մինչեւ ստուգումը կատարվի: Դա խաղ է: Բռնել խաչը:",
    fi: 'Pelaa "kiinni risti!" Nyt asetamme katseen seurannan tarkkaillaksesi silmäasennoa. Pidä kasvosi videosyötteessä (katso alla). Napsauta ristikästen keskustaa missä tahansa. Varmista, että silmäsi ovat siinä, kun napsautat sitä. Kun napsautat sitä, ristikarvat siirtyvät uuteen sijaintiin. Napsauta sitä jokaisessa uudessa paikassa, kunnes kalibrointi on valmis. Se on peli. Pudota risti!',
    ka: 'ითამაშეთ "დაჭერა ჯვარი!" ახლა ჩვენ დავაყენებთ მზერა თვალყურის დევნას, თვალის პოზიციის მონიტორინგს. შეინახეთ სახე ვიდეო საკვებად (იხ. ქვემოთ). დააჭირეთ ჯვრის თმას ცენტრს, სადაც არ უნდა იყოს. დარწმუნდით, რომ თქვენი თვალები მასზეა, როდესაც დააჭირეთ მას. როდესაც დააჭირეთ მას, ჯვრის თმები გადავა ახალ ადგილას. დააჭირეთ მას თითოეულ ახალ ადგილას, სანამ კალიბრაცია არ მოხდება. ეს თამაშია. დაიჭირე ჯვარი!',
    he: 'שחק "תפוס את הצלב!" כעת נקבע מעקב אחר מבט, כדי לפקח על מיקום העיניים. שמור את הפנים שלך בעדכון הווידיאו (ראה להלן). לחץ על מרכז שערות הצלב, בכל מקום שהוא. וודא שהעיניים שלך עומדות על זה כשאתה לוחץ עליו. כשאתה לוחץ עליו, שערות הצלב יעברו למיקום חדש. לחץ עליו בכל מיקום חדש, עד לסיום הכיול. זה משחק. לתפוס את הצלב!',
    ar: 'العب "اصطاد الصليب!" الآن سنقوم بإعداد تتبع النظرة ، لمراقبة وضع العين. حافظ على وجهك في تغذية الفيديو (انظر أدناه). انقر على مركز الشعر المتقاطع ، أينما كان. تأكد من أن عينيك على ذلك عند النقر فوقه. عند النقر فوقه ، سينتقل الشعر المتقاطع إلى موقع جديد. انقر فوقه في كل موقع جديد ، حتى يتم المعايرة. انها لعبة. اصطاد الصليب!',
    ur: 'کھیلو "کراس پکڑو!" اب ہم آنکھوں کی پوزیشن کی نگرانی کے لئے ، نگاہوں سے باخبر رہنے کے لئے ترتیب دیں گے۔ اپنا چہرہ ویڈیو فیڈ میں رکھیں (نیچے ملاحظہ کریں) کراس بالوں کے مرکز پر کلک کریں ، جہاں کہیں بھی ہے۔ اس بات کو یقینی بنائیں کہ جب آپ اس پر کلک کریں تو آپ کی آنکھیں اس پر ہیں۔ جب آپ اس پر کلک کرتے ہیں تو ، کراس بال ایک نئے مقام پر چلے جائیں گے۔ ہر نئے مقام پر اس پر کلک کریں ، جب تک کہ انشانکن نہ ہوجائے۔ یہ ایک کھیل ہے۔ کراس پکڑو!',
    hi: '"क्रॉस पकड़ो!" अब हम आंखों की स्थिति की निगरानी करने के लिए, गेज़ ट्रैकिंग सेट करेंगे। अपना चेहरा वीडियो फ़ीड में रखें (नीचे देखें)। जहां भी हो, क्रॉस हेयर के केंद्र पर क्लिक करें। सुनिश्चित करें कि जब आप इसे क्लिक करते हैं तो आपकी आंखें उस पर होती हैं। जब आप इसे क्लिक करते हैं, तो क्रॉस हेयर एक नए स्थान पर चले जाएंगे। कैलिब्रेशन होने तक इसे प्रत्येक नए स्थान पर क्लिक करें। यह एक खेल है। क्रॉस को पकड़ो!',
    ta: '"சிலுவையைப் பிடிக்கவும்!" கண் நிலையை கண்காணிக்க இப்போது நாம் பார்வை கண்காணிப்பை அமைப்போம். வீடியோ ஊட்டத்தில் உங்கள் முகத்தை வைத்திருங்கள் (கீழே காண்க). குறுக்கு முடிகளின் மையத்தில், அது எங்கிருந்தாலும் கிளிக் செய்க. நீங்கள் அதைக் கிளிக் செய்யும் போது உங்கள் கண்கள் அதில் இருப்பதை உறுதிப்படுத்திக் கொள்ளுங்கள். நீங்கள் அதைக் கிளிக் செய்தால், குறுக்கு முடிகள் புதிய இடத்திற்கு நகரும். அளவுத்திருத்தம் முடியும் வரை ஒவ்வொரு புதிய இடத்திலும் அதைக் கிளிக் செய்க. இது ஒரு விளையாட்டு. சிலுவையைப் பிடிக்கவும்!',
    ml: '"ക്രോസ് പിടിക്കുക!" ഇപ്പോൾ ഞങ്ങൾ കണ്ണ് സ്ഥാനം നിരീക്ഷിക്കാൻ ഞങ്ങൾ ലജ്ഗ് ട്രാക്കിംഗ് സജ്ജമാക്കും. വീഡിയോ ഫീഡിൽ മുഖം സൂക്ഷിക്കുക (ചുവടെ കാണുക). അത് എവിടെയായിരുന്നാലും ക്രോസ് ഹെയർ മധ്യഭാഗത്ത് ക്ലിക്കുചെയ്യുക. നിങ്ങൾ ക്ലിക്കുചെയ്യുമ്പോൾ നിങ്ങളുടെ കണ്ണുകൾ അതിലുണ്ടെന്ന് ഉറപ്പാക്കുക. നിങ്ങൾ അതിൽ ക്ലിക്കുചെയ്യുമ്പോൾ, ക്രോസ് ഹെയർ ഒരു പുതിയ സ്ഥലത്തേക്ക് നീങ്ങും. കാലിബ്രേഷൻ നടക്കുന്നതുവരെ ഓരോ പുതിയ സ്ഥലത്തും ക്ലിക്കുചെയ്യുക. ഇതൊരു ഗെയിമാണ്. കുരിശ് പിടിക്കൂ!',
    te: '"క్యాచ్ ది క్రాస్!" కంటి స్థానాన్ని పర్యవేక్షించడానికి ఇప్పుడు మేము చూపుల ట్రాకింగ్‌ను ఏర్పాటు చేస్తాము. మీ ముఖాన్ని వీడియో ఫీడ్‌లో ఉంచండి (క్రింద చూడండి). క్రాస్ హెయిర్స్ మధ్యలో క్లిక్ చేయండి, అది ఎక్కడ ఉన్నా. మీరు క్లిక్ చేసినప్పుడు మీ కళ్ళు దానిపై ఉన్నాయని నిర్ధారించుకోండి. మీరు దీన్ని క్లిక్ చేసినప్పుడు, క్రాస్ హెయిర్స్ క్రొత్త ప్రదేశానికి వెళతాయి. క్రమాంకనం పూర్తయ్యే వరకు ప్రతి క్రొత్త ప్రదేశంలో క్లిక్ చేయండి. ఇది ఒక ఆట. సిలువను పట్టుకోండి!',
    kn: '"ಶಿಲುಬೆಯನ್ನು ಹಿಡಿಯಿರಿ!" ಕಣ್ಣಿನ ಸ್ಥಾನವನ್ನು ಮೇಲ್ವಿಚಾರಣೆ ಮಾಡಲು ನಾವು ಈಗ ನೋಟದ ಟ್ರ್ಯಾಕಿಂಗ್ ಅನ್ನು ಹೊಂದಿಸುತ್ತೇವೆ. ನಿಮ್ಮ ಮುಖವನ್ನು ವೀಡಿಯೊ ಫೀಡ್‌ನಲ್ಲಿ ಇರಿಸಿ (ಕೆಳಗೆ ನೋಡಿ). ಅದು ಎಲ್ಲಿದ್ದರೂ ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗದಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡಿ. ನೀವು ಅದನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿದಾಗ ನಿಮ್ಮ ಕಣ್ಣುಗಳು ಅದರ ಮೇಲೆ ಇದ್ದವು ಎಂದು ಖಚಿತಪಡಿಸಿಕೊಳ್ಳಿ. ನೀವು ಅದನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿದಾಗ, ಅಡ್ಡ ಕೂದಲುಗಳು ಹೊಸ ಸ್ಥಳಕ್ಕೆ ಚಲಿಸುತ್ತವೆ. ಮಾಪನಾಂಕ ನಿರ್ಣಯವನ್ನು ಮಾಡುವವರೆಗೆ ಪ್ರತಿ ಹೊಸ ಸ್ಥಳದಲ್ಲಿ ಅದನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ. ಇದು ಒಂದು ಆಟ. ಶಿಲುಬೆಯನ್ನು ಹಿಡಿಯಿರಿ!',
    bn: '"ক্রস ধরুন!" চোখের অবস্থান নিরীক্ষণের জন্য এখন আমরা গেজ ট্র্যাকিং সেট আপ করব। ভিডিও ফিডে আপনার মুখ রাখুন (নীচে দেখুন)। ক্রস চুলের কেন্দ্রে ক্লিক করুন, এটি যেখানেই হোক। আপনি যখন এটি ক্লিক করেন তখন আপনার চোখ এটির দিকে রয়েছে তা নিশ্চিত করুন। আপনি যখন এটি ক্লিক করেন, ক্রস চুলগুলি একটি নতুন স্থানে চলে যাবে। ক্রমাঙ্কন শেষ না হওয়া পর্যন্ত প্রতিটি নতুন স্থানে এটি ক্লিক করুন। এটি একটি খেলা. ক্রস ধরুন!',
    id: 'Mainkan "Tangkap Salib!" Sekarang kami akan mengatur pelacakan tatapan, untuk memantau posisi mata. Simpan wajah Anda di umpan video (lihat di bawah). Klik di tengah rambut silang, di mana pun itu. Pastikan mata Anda tertuju saat Anda mengkliknya. Saat Anda mengkliknya, Cross Hairs akan pindah ke lokasi baru. Klik di setiap lokasi baru, sampai kalibrasi selesai. Ini permainan. Tangkap salib!',
    "zh-CN":
      "播放“赶十字架！”现在，我们将设置凝视跟踪，以监视眼睛位置。将您的脸放在视频供稿中（见下文）。无论何处，单击十字头发的中心。单击它时，请确保您的眼睛在上面。当您单击它时，十字头发将移至新位置。在每个新位置单击它，直到完成校准。游戏而已。抓住十字架！",
    "zh-HK":
      "播放“趕十字架！”現在，我們將設置凝視跟踪，以監視眼睛位置。將您的臉放在視頻供稿中（見下文）。無論何處，單擊十字頭髮的中心。單擊它時，請確保您的眼睛在上面。當您單擊它時，十字頭髮將移至新位置。在每個新位置單擊它，直到完成校準。遊戲而已。抓住十字架！",
    ja: "「Catch the Cross！」を再生しますこれで、目の位置を監視するために、視線追跡を設定します。ビデオフィードに顔を入れてください（以下を参照）。どこにいても、交差毛の中心をクリックします。クリックしたときに目がそれにあることを確認してください。クリックすると、クロスヘアが新しい場所に移動します。キャリブレーションが完了するまで、新しい場所ごとにクリックします。それはゲームです。十字架をキャッチ！",
    ko: '"크로스를 잡아라!" 이제 눈 위치를 모니터링하기 위해 시선 추적을 설정합니다. 비디오 피드에 얼굴을 유지하십시오 (아래 참조). 크로스 털의 중앙을 클릭하십시오. 클릭하면 눈이 켜져 있는지 확인하십시오. 클릭하면 크로스 헤어가 새로운 위치로 이동합니다. 교정이 완료 될 때까지 각 새 위치에서 클릭하십시오. 그것은 게임이다. 십자가를 잡아라!',
  },
  RC_gazeTrackingNudge: {
    "en-US":
      "CATCH THE CROSS! We need your help. The eye tracker needs more training. As shown by the arrow, when you clicked, the tracker thought your gaze was far from the cross hairs. Please click the cross hairs at each new position. It's a game. Catch the cross!",
    de: "Fang das Kreuz! Wir brauchen deine Hilfe. Der Eye Tracker braucht mehr Training. Wie der Pfeil gezeigt, dachte der Tracker beim Klicken, dass Ihr Blick weit von den Kreuzhaaren entfernt sei. Bitte klicken Sie auf die Cross -Haare an jeder neuen Position. Es ist ein Spiel. Fang das Kreuz!",
    fr: "Attrapez la croix! Nous avons besoin de votre aide. Le tracker des yeux a besoin de plus de formation. Comme le montre la flèche, lorsque vous avez cliqué, le tracker pensait que votre regard était loin des poils. Veuillez cliquer sur les poils à chaque nouvelle position. C'est un jeu. Attrapez la croix!",
    es: "¡Atrapa la cruz! Necesitamos tu ayuda. El rastreador ocular necesita más entrenamiento. Como se muestra en la flecha, cuando hizo clic, el rastreador pensó que su mirada estaba lejos de la cruz. Haga clic en la cruz en cada nueva posición. Es un juego. ¡Atrapa la cruz!",
    pt: "Pegue a cruz! Nós precisamos da sua ajuda. O rastreador ocular precisa de mais treinamento. Como mostrado pela seta, quando você clicou, o rastreador pensou que seu olhar estava longe dos cabelos cruzados. Clique nos cabelos cruzados em cada nova posição. É um jogo. Pegue a cruz!",
    it: "Cattura la croce! Abbiamo bisogno del tuo aiuto. Il tracker eye ha bisogno di più allenamento. Come mostrato dalla freccia, quando hai fatto clic, il tracker pensava che il tuo sguardo fosse lontano dai capelli incrociati. Fare clic sui peli incrociati in ogni nuova posizione. È un gioco. Cattura la croce!",
    ro: "Crucea! Noi avem nevoie de ajutorul tau. Trackerul pentru ochi are nevoie de mai mult antrenament. După cum arată săgeata, când ați dat clic, tracker -ul a crezut că privirea ta este departe de părul încrucișat. Vă rugăm să faceți clic pe părul încrucișat la fiecare poziție nouă. Este un joc. Crucea!",
    pl: "Złap krzyż! Potrzebujemy Twojej pomocy. Śledzenie wzroku wymaga więcej treningu. Jak pokazuje strzałka, po kliknięciu, tracker pomyślał, że twoje spojrzenie jest dalekie od włosów krzyżowych. Kliknij włosy krzyżowe w każdej nowej pozycji. To jest gra. Złap krzyż!",
    ru: "Поймай крест! Мы нуждаемся в твоей помощи. Трекер глаз нуждается в большем обучении. Как показано стрелкой, когда вы нажимаете, трекер думал, что ваш взгляд был далеко от кросс -волос. Пожалуйста, нажмите на кросс -волоски в каждой новой позиции. Это игра. Поймай крест!",
    hy: "Բռնել խաչը: Մենք քո օգնության կարիքը ունենք. Աչքի հետեւորդին ավելի շատ մարզումներ են պետք: Ինչպես ցույց է տրված սլաքը, երբ սեղմել եք, հետեւորդը կարծում էր, որ ձեր հայացքը հեռու է խաչի մազերից: Խնդրում ենք կտտացնել խաչի մազերը յուրաքանչյուր նոր դիրքում: Դա խաղ է: Բռնել խաչը:",
    fi: "Pudota risti! Tarvitsemme apuasi. Silmäseuranta tarvitsee enemmän koulutusta. Kuten nuoli näkyy, kun napsautit, seurannan mielestä katseesi oli kaukana ristikarvoista. Napsauta ristikarvoja jokaisessa uudessa asennossa. Se on peli. Pudota risti!",
    ka: "დაიჭირე ჯვარი! Ჩვენ გვჭირდება თქვენი დახმარება. თვალის ტრეკერს მეტი ტრენინგი სჭირდება. როგორც ისარი აჩვენეთ, როდესაც დააჭირეთ, ტრეკერი ფიქრობდა, რომ თქვენი მზერა ჯვრის თმებისგან შორს იყო. გთხოვთ, დააჭირეთ ჯვრის თმას თითოეულ ახალ პოზიციაზე. ეს თამაშია. დაიჭირე ჯვარი!",
    he: "לתפוס את הצלב! אנחנו צריכים את עזרתך. גשש העיניים זקוק לאימונים נוספים. כפי שמוצג על ידי החץ, כשלחצתם, הגשש חשב שהמבט שלכם היה רחוק משערות הצלב. אנא לחץ על שערות הצלב בכל עמדה חדשה. זה משחק. לתפוס את הצלב!",
    ar: "اصطاد الصليب! نحن نحتاج مساعدتك. يحتاج متتبع العين إلى مزيد من التدريب. كما هو موضح في السهم ، عندما نقرت ، اعتقد المتتبع أن نظرتك كانت بعيدة عن الشعر المتقاطع. يرجى النقر على الشعر المتقاطع في كل موقف جديد. انها لعبة. اصطاد الصليب!",
    ur: "کراس پکڑو! ہمیں آپ کی مدد کی ضرورت ہے۔ آئی ٹریکر کو مزید تربیت کی ضرورت ہے۔ جیسا کہ تیر کے ذریعہ دکھایا گیا ہے ، جب آپ نے کلک کیا تو ، ٹریکر نے سوچا کہ آپ کی نگاہیں کراس بالوں سے بہت دور ہیں۔ براہ کرم ہر نئی پوزیشن پر کراس بالوں پر کلک کریں۔ یہ ایک کھیل ہے۔ کراس پکڑو!",
    hi: "क्रॉस को पकड़ो! हमे आपकी मदद की जरूरत है। आई ट्रैकर को अधिक प्रशिक्षण की आवश्यकता है। जैसा कि तीर द्वारा दिखाया गया है, जब आपने क्लिक किया, तो ट्रैकर ने सोचा कि आपका टकटकी क्रॉस हेयर से दूर है। कृपया प्रत्येक नई स्थिति में क्रॉस हेयर पर क्लिक करें। यह एक खेल है। क्रॉस को पकड़ो!",
    ta: "சிலுவையைப் பிடிக்கவும்! உங்களின் உதவி எங்களுக்கு தேவை. கண் டிராக்கருக்கு கூடுதல் பயிற்சி தேவை. அம்பு காட்டியபடி, நீங்கள் கிளிக் செய்தபோது, ​​உங்கள் பார்வை குறுக்கு முடிகளிலிருந்து வெகு தொலைவில் இருப்பதாக டிராக்கர் நினைத்தார். ஒவ்வொரு புதிய நிலையிலும் குறுக்கு முடிகளைக் கிளிக் செய்க. இது ஒரு விளையாட்டு. சிலுவையைப் பிடிக்கவும்!",
    ml: "കുരിശ് പിടിക്കൂ! ഞങ്ങൾക്ക് നിങ്ങളുടെ സഹായം ആവശ്യമാണ്. ഐ ട്രാക്കർ കൂടുതൽ പരിശീലനം ആവശ്യമാണ്. അമ്പടയാളം കാണിച്ചിരിക്കുന്നതുപോലെ, നിങ്ങൾ ക്ലിക്കുചെയ്യുമ്പോൾ, നിങ്ങളുടെ നോട്ടം ക്രോസ് ഹെയർസിൽ നിന്ന് വളരെ അകലെയാണെന്ന് ട്രാക്കർ കരുതി. ഓരോ പുതിയ സ്ഥാനത്തും ക്രോസ് ഹെറിൽ ക്ലിക്കുചെയ്യുക. ഇതൊരു ഗെയിമാണ്. കുരിശ് പിടിക്കൂ!",
    te: "సిలువను పట్టుకోండి! మాకు మీ సహాయం కావాలి. కంటి ట్రాకర్‌కు మరింత శిక్షణ అవసరం. బాణం చూపినట్లుగా, మీరు క్లిక్ చేసినప్పుడు, మీ చూపులు క్రాస్ హెయిర్‌లకు దూరంగా ఉన్నాయని ట్రాకర్ భావించారు. దయచేసి ప్రతి కొత్త స్థానంలో క్రాస్ హెయిర్స్ క్లిక్ చేయండి. ఇది ఒక ఆట. సిలువను పట్టుకోండి!",
    kn: "ಶಿಲುಬೆಯನ್ನು ಹಿಡಿಯಿರಿ! ನಿಮ್ಮ ಸಹಾಯ ನಮಗೆ ಬೇಕು. ಕಣ್ಣಿನ ಟ್ರ್ಯಾಕರ್‌ಗೆ ಹೆಚ್ಚಿನ ತರಬೇತಿಯ ಅಗತ್ಯವಿದೆ. ಬಾಣವು ತೋರಿಸಿದಂತೆ, ನೀವು ಕ್ಲಿಕ್ ಮಾಡಿದಾಗ, ನಿಮ್ಮ ನೋಟವು ಅಡ್ಡ ಕೂದಲಿನಿಂದ ದೂರವಿದೆ ಎಂದು ಟ್ರ್ಯಾಕರ್ ಭಾವಿಸಿದ್ದಾನೆ. ದಯವಿಟ್ಟು ಪ್ರತಿ ಹೊಸ ಸ್ಥಾನದಲ್ಲಿ ಅಡ್ಡ ಕೂದಲನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ. ಇದು ಒಂದು ಆಟ. ಶಿಲುಬೆಯನ್ನು ಹಿಡಿಯಿರಿ!",
    bn: "ক্রস ধরুন! আমাদের আপনার সাহায্য দরকার. চোখের ট্র্যাকারটির আরও প্রশিক্ষণ প্রয়োজন। তীর দ্বারা দেখানো হিসাবে, আপনি যখন ক্লিক করেছেন, ট্র্যাকারটি ভেবেছিল আপনার দৃষ্টিশক্তি ক্রস কেশ থেকে অনেক দূরে। প্রতিটি নতুন পজিশনে ক্রস চুল ক্লিক করুন। এটি একটি খেলা. ক্রস ধরুন!",
    id: "Tangkap salib! Kami membutuhkan bantuan Anda. Pelacak mata membutuhkan lebih banyak pelatihan. Seperti yang ditunjukkan oleh panah, ketika Anda mengklik, pelacak berpikir tatapan Anda jauh dari rambut silang. Silakan klik Cross Hairs di setiap posisi baru. Ini permainan. Tangkap salib!",
    "zh-CN":
      "抓住十字架！我们需要你的帮助。眼睛跟踪器需要更多的培训。如箭头所示，当您单击时，跟踪器认为您的目光远非十字。请点击每个新位置的十字头发。游戏而已。抓住十字架！",
    "zh-HK":
      "抓住十字架！我們需要你的幫助。眼睛跟踪器需要更多的培訓。如箭頭所示，當您單擊時，跟踪器認為您的目光遠非十字。請點擊每個新位置的十字頭髮。遊戲而已。抓住十字架！",
    ja: "十字架をキャッチ！君の力が必要なんだ。アイトラッカーにはさらにトレーニングが必要です。矢印で示されているように、クリックしたとき、トラッカーはあなたの視線が十字毛からはほど遠いと思った。新しい位置ごとにクロスヘアをクリックしてください。それはゲームです。十字架をキャッチ！",
    ko: "십자가를 잡아라! 우리는 너의 도움이 필요해. 눈 추적기에는 더 많은 훈련이 필요합니다. 화살표에서 볼 수 있듯이, 클릭했을 때 트래커는 시선이 십자 머리카락과는 거리가 멀다고 생각했습니다. 각각의 새로운 위치에서 십자 머리카락을 클릭하십시오. 그것은 게임이다. 십자가를 잡아라!",
  },
  RC_gazeTrackingTitle: {
    "en-US": "Set Up Gaze Tracking",
    de: "Bauen Sie eine Blickverfolgung ein",
    fr: "Configuration pour le suivi du regard",
    es: "Configuración para seguimiento de mirada",
    pt: "Configuração para visualização",
    it: "Impostare il monitoraggio dello sguardo",
    ro: "Configurare pentru vizualizare",
    pl: "Ustaw śledzenie wzroku",
    ru: "Настройка отслеживания взгляда",
    hy: "Աչքի հետեւում տեղադրելը",
    fi: "Aseta katseen seuranta",
    ka: "მოაწყეთ მზერა თვალყურის დევნება",
    he: "הגדר מעקב אחר מבט",
    ar: "الإعداد لتتبع النظر",
    ur: "ٹریکنگ کے لئے آنکھیں طے کرنے دیں",
    hi: "आँखों का ट्रैकिंग के लिए तयार हो जाए",
    ta: "பார்வை கண்காணிப்பை அமைக்கவும்",
    ml: "കണക്കെടുപ്പ് ആരംഭിക്കുക",
    te: "చూపుల ట్రాకింగ్‌ను సెటప్ చేయండి",
    kn: "ನೋಟದ ಟ್ರ್ಯಾಕಿಂಗ್ ಅನ್ನು ಹೊಂದಿಸಿ",
    bn: "গেজ ট্র্যাকিং সেট আপ করুন",
    id: "Mengatur pelacakan tatapan",
    "zh-CN": "眼动追踪初始化",
    "zh-HK": "眼動追踪初始化",
    ja: "視線追跡を設定します。",
    ko: "시선 추적을 설정하십시오",
  },
  RC_ifStuckPleaseReturn: {
    "en-US":
      "It's rare, but if your computer gets stuck in calibration, please return your study to Prolific.co. ",
    de: "Es ist selten, aber wenn Ihr Computer in der Kalibrierung stecken bleibt, geben Sie Ihre Studie an produktiv.co zurück.",
    fr: "C'est rare, mais si votre ordinateur est coincé dans l'étalonnage, veuillez retourner votre étude sur prolific.co.",
    es: "Es raro, pero si su computadora se atasca en la calibración, devuelva su estudio a Prolific.co.",
    pt: "É raro, mas se o seu computador ficar preso na calibração, devolva seu estudo ao prolific.co.",
    it: "È raro, ma se il tuo computer rimane bloccato nella calibrazione, ti preghiamo di restituire il tuo studio a prolific.co.",
    ro: "Este rar, dar dacă computerul dvs. se blochează în calibrare, vă rugăm să returnați studiul la Prolific.co.",
    pl: "Jest to rzadkie, ale jeśli komputer utknie w kalibracji, zwróć swoje badanie na Prolific.co.",
    ru: "Это редко, но если ваш компьютер застрял в калибровке, верните свое исследование в Pellific.co.",
    hy: "Հազվադեպ է, բայց եթե ձեր համակարգիչը խրված է տրամաչափման մեջ, խնդրում ենք վերադարձնել ձեր ուսումնասիրությունը Pringific.co- ին:",
    fi: "Se on harvinaista, mutta jos tietokoneesi takertuu kalibrointiin, palauta opintosi tuotteelle.",
    ka: "ეს იშვიათია, მაგრამ თუ თქვენი კომპიუტერი ჩერდება კალიბრაციაში, გთხოვთ, დააბრუნოთ თქვენი შესწავლა Prolific.co- ზე.",
    he: "זה נדיר, אבל אם המחשב שלך נתקע בכיול, אנא החזיר את המחקר שלך ל- prolific.co.",
    ar: "إنه أمر نادر الحدوث ، ولكن إذا كان جهاز الكمبيوتر الخاص بك عالقًا في المعايرة ، فيرجى إعادة دراستك إلى Prolific.co.",
    ur: "یہ شاذ و نادر ہی ہے ، لیکن اگر آپ کا کمپیوٹر انشانکن میں پھنس جاتا ہے تو ، براہ کرم اپنا مطالعہ prolific.co پر واپس کریں۔",
    hi: "यह दुर्लभ है, लेकिन यदि आपका कंप्यूटर अंशांकन में फंस जाता है, तो कृपया अपना अध्ययन prolific.co पर लौटाएं।",
    ta: "இது அரிதானது, ஆனால் உங்கள் கணினி அளவுத்திருத்தத்தில் சிக்கிக்கொண்டால், தயவுசெய்து உங்கள் ஆய்வை plemific.co க்கு திருப்பி விடுங்கள்.",
    ml: "ഇത് അപൂർവമാണ്, പക്ഷേ നിങ്ങളുടെ കമ്പ്യൂട്ടർ കാലിബ്രേഷനിൽ കുടുങ്ങുകയാണെങ്കിൽ, ദയവായി നിങ്ങളുടെ പഠനം സമൃദ്ധമായ.cCO- ലേക്ക് തിരികെ നൽകുക.",
    te: "ఇది చాలా అరుదు, కానీ మీ కంప్యూటర్ క్రమాంకనం చేస్తే, దయచేసి మీ అధ్యయనాన్ని ఫలవంతమైన.కోకు తిరిగి ఇవ్వండి.",
    kn: "ಇದು ಅಪರೂಪ, ಆದರೆ ನಿಮ್ಮ ಕಂಪ್ಯೂಟರ್ ಮಾಪನಾಂಕ ನಿರ್ಣಯದಲ್ಲಿ ಸಿಲುಕಿಕೊಂಡರೆ, ದಯವಿಟ್ಟು ನಿಮ್ಮ ಅಧ್ಯಯನವನ್ನು verific.co ಗೆ ಹಿಂತಿರುಗಿ.",
    bn: "এটি বিরল, তবে যদি আপনার কম্পিউটারটি ক্রমাঙ্কনটিতে আটকে যায় তবে দয়া করে আপনার অধ্যয়নটি Plovific.co এ ফিরিয়ে দিন।",
    id: "Ini jarang, tetapi jika komputer Anda macet dalam kalibrasi, silakan kembalikan studi Anda ke Prolific.co.",
    "zh-CN":
      "这很少见，但是如果您的计算机被卡在校准中，请将您的书房归还Pollific.co。",
    "zh-HK":
      "這很少見，但是如果您的計算機被卡在校準中，請將您的書房歸還Pollific.co。",
    ja: "まれですが、コンピューターがキャリブレーションで詰まっている場合は、勉強をProlific.coに返送してください。",
    ko: "드물지만 컴퓨터가 교정에 갇히면 학습을 Prolific.co로 반환하십시오.",
  },
  RC_nearPointIntro: {
    "en-US":
      "We'll measure the distance between your eyes. Use the video image to watch your eyes. Hold a pen horizontally against your face in front of your eyes. Align your right and left thumb nails with the centers of your right and left pupils. Then bring the pen to the on-screen ruler to measure the separation between your  thumbs. Allign your left thumb with zero and notice where your right thumb is. Put down the pen and click the ruler at that point. Finally, click OK to continue.",
    de: "Wir werden den Abstand zwischen Ihren Augen messen. Verwenden Sie das Videobild, um Ihre Augen zu sehen. Halten Sie einen Stift horizontal gegen Ihr Gesicht vor Ihren Augen. Richten Sie Ihre rechten und linken Daumennägel auf die Zentren Ihrer rechten und linken Schüler aus. Bringen Sie dann den Stift zum Lineal auf dem Bildschirm, um die Trennung zwischen Ihren Daumen zu messen. Allign Ihren linken Daumen mit Null und bemerken Sie, wo sich Ihr rechter Daumen befindet. Legen Sie den Stift nieder und klicken Sie zu diesem Zeitpunkt auf das Lineal. Klicken Sie schließlich auf OK, um fortzufahren.",
    fr: "Nous mesurerons la distance entre vos yeux. Utilisez l'image vidéo pour regarder vos yeux. Tenez un stylo horizontalement contre votre visage devant vos yeux. Alignez vos ongles de pouce droit et gauche avec les centres de vos élèves droits et gauche. Ensuite, apportez le stylo à la règle à l'écran pour mesurer la séparation entre vos pouces. Allignez votre pouce gauche avec zéro et remarquez où se trouve votre pouce droit. Posez le stylo et cliquez sur la règle à ce moment-là. Enfin, cliquez sur OK pour continuer.",
    es: "Mediremos la distancia entre sus ojos. Usa la imagen de video para ver tus ojos. Mantenga un bolígrafo horizontalmente contra su cara frente a sus ojos. Alinee las uñas de pulgar derecha e izquierda con los centros de sus alumnos derecho e izquierdo. Luego traiga la pluma a la regla en pantalla para medir la separación entre los pulgares. Allinee su pulgar izquierdo con cero y observe dónde está su pulgar derecho. Coloque la pluma y haga clic en la regla en ese punto. Finalmente, haga clic en Aceptar para continuar.",
    pt: "Mediremos a distância entre seus olhos. Use a imagem de vídeo para assistir seus olhos. Segure uma caneta horizontalmente contra o seu rosto na frente dos seus olhos. Alinhe as unhas do polegar direito e esquerdo com os centros das pupilas direita e esquerda. Em seguida, traga a caneta para a régua na tela para medir a separação entre os polegares. Alinhe o polegar esquerdo com zero e observe onde está o polegar direito. Abaixe a caneta e clique na régua nesse ponto. Por fim, clique em OK para continuar.",
    it: "Misurare la distanza tra i tuoi occhi. Usa l'immagine video per guardare i tuoi occhi. Tieni una penna in orizzontale contro il viso davanti ai tuoi occhi. Allinea le unghie del pollice destro e sinistro con i centri delle alunni destra e sinistra. Quindi porta la penna sul sovrano sullo schermo per misurare la separazione tra i pollici. Alllinea il pollice sinistro con zero e nota dove si trova il pollice destro. Metti giù la penna e fai clic sul righello a quel punto. Infine, fai clic su OK per continuare.",
    ro: "Vom măsura distanța dintre ochi. Folosiți imaginea video pentru a vă urmări ochii. Țineți un stilou orizontal pe față în fața ochilor. Aliniați -vă unghiile de degetul mare din dreapta și stânga cu centrele elevilor din dreapta și stânga. Apoi aduceți stiloul la conducătorul de pe ecran pentru a măsura separarea dintre degetele mari. Alujați degetul stâng cu zero și observați unde este degetul mare. Puneți stiloul și faceți clic pe riglă în acel moment. În cele din urmă, faceți clic pe OK pentru a continua.",
    pl: "Zmienimy odległość między twoimi oczami. Użyj obrazu wideo, aby oglądać oczy. Trzymaj pióro poziomo na twarzy przed oczami. Wyrównaj swoje prawe i lewe paznokcie kciuka z centrami prawej i lewej źrenicy. Następnie przynieś pióro do linijki na ekranie, aby zmierzyć rozdział między kciukami. All lewy kciuk zero i zauważ, gdzie jest twój prawy kciuk. Odłóż pióro i kliknij władcę w tym momencie. Na koniec kliknij OK, aby kontynuować.",
    ru: "Мы измеряем расстояние между вашими глазами. Используйте видео изображение, чтобы посмотреть свои глаза. Держите ручку горизонтально к лицу перед вашими глазами. Совместите свои правые и левые пальцы с помощью центров ваших правых и левых учеников. Затем принесите ручку к линейке на экране, чтобы измерить разделение между большими пальцами. Выравнивайте левый большой палец с нулем и обратите внимание, где находится правый большой палец. Положите ручку и щелкните правителя в этот момент. Наконец, нажмите ОК, чтобы продолжить.",
    hy: "Մենք չափելու ենք ձեր աչքերի միջեւ եղած հեռավորությունը: Օգտագործեք տեսանյութի պատկերը `ձեր աչքերը դիտելու համար: Ձեր աչքերի առջեւ ձեր դեմքի դեմ հորիզոնական գրիչ պահեք: Հավասարեցրեք ձեր աջ եւ ձախ մատի եղունգները ձեր աջ եւ ձախ աշակերտների կենտրոններով: Այնուհետեւ գրիչը հասցրեք էկրանի տիրակալին `ձեր մատների միջեւ տարանջատումը չափելու համար: Ձեռք բերեք ձեր ձախ բութը զրոյով եւ նկատեք, թե որտեղ է ձեր աջ բութը: Ներդրեք գրիչը եւ այդ պահին սեղմեք կառավարիչը: Վերջապես, կտտացրեք OK, շարունակելու համար:",
    fi: "Mittaamme silmäsi välisen etäisyyden. Katso videokuvasi silmäsi. Pidä kynää vaakasuoraan kasvojasi vasten silmäsi edessä. Kohdista oikeat ja vasen peukalokynnet oikean ja vasemman oppilaasi keskuksiin. Tuo sitten kynä näytön hallitsijalle mitata peukaloidesi välinen ero. Allign vasen peukalo nollalla ja huomaa missä oikea peukalo on. Laita kynä alas ja napsauta viivainta siinä vaiheessa. Lopuksi napsauta OK jatkaaksesi.",
    ka: "ჩვენ გავზომავთ მანძილს თქვენს თვალებს შორის. გამოიყენეთ ვიდეო სურათი, რომ ნახოთ თქვენი თვალები. თვალების წინ შეაჩერეთ კალამი ჰორიზონტალურად. თქვენი მარჯვენა და მარცხენა თითის ფრჩხილების გასწორება თქვენი მარჯვენა და მარცხენა მოსწავლეების ცენტრებთან. შემდეგ კალამი მიიტანეთ ეკრანზე მმართველთან, რათა გაზომოთ თქვენს თითებს შორის განცალკევება. მარცხენა ცერა თითი ნულოვანი და შეამჩნიეთ სად არის თქვენი მარჯვენა თითი. ჩაიცვი კალამი და დააჭირეთ მმართველს იმ ეტაპზე. დაბოლოს, დააჭირეთ ღილაკს OK, რომ გააგრძელოთ.",
    he: "נמדוד את המרחק בין העיניים שלך. השתמש בתמונת הווידיאו כדי לצפות בעיניים שלך. החזיקו עט אופקית על פניכם מול העיניים. יישר את ציפורני האגודל הימנית והשמאלית שלך עם מרכזי התלמידים הימניים והשמאליים שלך. ואז הביאו את העט לשליט על המסך כדי למדוד את ההפרדה בין האגודלים. יש ליישר את האגודל השמאלי שלך עם אפס ושים לב איפה האגודל הימני שלך. הניחו את העט ולחצו על השליט באותה נקודה. לבסוף, לחץ על אישור כדי להמשיך.",
    ar: 'سنقوم بقياس المسافة بين عينيك. استخدم صورة الفيديو لمشاهدة عينيك. امسك قلمًا أفقيًا على وجهك أمام عينيك. قم بمحاذاة أظافر الإبهام اليمنى واليسرى مع مراكز التلاميذ الأيمن واليسرى. ثم أحضر القلم إلى المسطرة على الشاشة لقياس الفصل بين إبهامك. قم بتوليف إبهامك الأيسر بدون صفر ولاحظ أين يوجد إبهامك الأيمن. ضع القلم وانقر فوق الحاكم في تلك المرحلة. أخيرًا ، انقر فوق "موافق" للمتابعة.',
    ur: "ہم آپ کی آنکھوں کے درمیان فاصلہ طے کریں گے۔ آنکھیں دیکھنے کے لئے ویڈیو امیج کا استعمال کریں۔ اپنی آنکھوں کے سامنے اپنے چہرے کے خلاف افقی طور پر قلم رکھیں۔ اپنے دائیں اور بائیں انگوٹھے کے ناخن کو اپنے دائیں اور بائیں شاگردوں کے مراکز کے ساتھ سیدھ کریں۔ اس کے بعد اپنے انگوٹھوں کے مابین علیحدگی کی پیمائش کرنے کے لئے قلم آن اسکرین حکمران کو لائیں۔ اپنے بائیں انگوٹھے کو صفر سے الگ کریں اور نوٹس کریں کہ آپ کا دائیں انگوٹھا کہاں ہے۔ قلم کو نیچے رکھیں اور اس مقام پر حکمران پر کلک کریں۔ آخر میں ، جاری رکھنے کے لئے ٹھیک ہے پر کلک کریں۔",
    hi: "हम आपकी आंखों के बीच की दूरी को मापेंगे। अपनी आँखें देखने के लिए वीडियो छवि का उपयोग करें। अपनी आंखों के सामने अपने चेहरे के खिलाफ क्षैतिज रूप से एक पेन पकड़ो। अपने दाहिने और बाएं अंगूठे के नाखूनों को अपने दाएं और बाएं विद्यार्थियों के केंद्रों के साथ संरेखित करें। फिर अपने अंगूठे के बीच अलगाव को मापने के लिए पेन को ऑन-स्क्रीन शासक के पास लाएं। अपने बाएं अंगूठे को शून्य के साथ एलगेट करें और नोटिस करें कि आपका दाहिना अंगूठा कहां है। कलम को नीचे रखें और उस बिंदु पर शासक पर क्लिक करें। अंत में, जारी रखने के लिए ओके पर क्लिक करें।",
    ta: "உங்கள் கண்களுக்கு இடையிலான தூரத்தை நாங்கள் அளவிடுவோம். உங்கள் கண்களைக் காண வீடியோ படத்தைப் பயன்படுத்தவும். உங்கள் கண்களுக்கு முன்னால் உங்கள் முகத்திற்கு எதிராக கிடைமட்டமாக ஒரு பேனாவைப் பிடித்துக் கொள்ளுங்கள். உங்கள் வலது மற்றும் இடது கட்டைவிரல் நகங்களை உங்கள் வலது மற்றும் இடது மாணவர்களின் மையங்களுடன் சீரமைக்கவும். உங்கள் கட்டைவிரல்களுக்கு இடையில் பிரிவினை அளவிட பேனாவை திரையில் உள்ள ஆட்சியாளரிடம் கொண்டு வாருங்கள். உங்கள் இடது கட்டைவிரலை பூஜ்ஜியத்துடன் ஒதுக்கி, உங்கள் வலது கட்டைவிரல் எங்கே என்பதைக் கவனியுங்கள். பேனாவை கீழே வைத்து, அந்த நேரத்தில் ஆட்சியாளரைக் கிளிக் செய்க. இறுதியாக, தொடர சரி என்பதைக் கிளிக் செய்க.",
    ml: "നിങ്ങളുടെ കണ്ണുകൾ തമ്മിലുള്ള ദൂരം ഞങ്ങൾ അളക്കും. നിങ്ങളുടെ കണ്ണുകൾ കാണാൻ വീഡിയോ ചിത്രം ഉപയോഗിക്കുക. നിങ്ങളുടെ മുഖത്ത് നിങ്ങളുടെ മുഖത്തിന് മുന്നിൽ ഒരു പേന അമർത്തിപ്പിടിക്കുക. നിങ്ങളുടെ വലതുവശത്ത് നിങ്ങളുടെ വലതുവശത്തും ഉപേക്ഷിച്ച വിദ്യാർത്ഥികളോടും ഒപ്പം വിന്യസിക്കുക. നിങ്ങളുടെ തള്ളവിരൽ തമ്മിലുള്ള വേർപിരിയൽ അളക്കാൻ പേന ഓൺ-സ്ക്രീൻ ഭരണാധികാരിയിലേക്ക് കൊണ്ടുവരിക. നിങ്ങളുടെ ഇടത് തള്ളവിരൽ പൂജ്യവും അറിയിപ്പും ഉപയോഗിച്ച് നിങ്ങളുടെ വലത് തലം. പേന താഴെയിടുക, ആ സമയത്ത് ഭരണാധികാരി ക്ലിക്കുചെയ്യുക. അവസാനമായി, തുടരാൻ ശരി ക്ലിക്കുചെയ്യുക.",
    te: "మేము మీ కళ్ళ మధ్య దూరాన్ని కొలుస్తాము. మీ కళ్ళు చూడటానికి వీడియో చిత్రాన్ని ఉపయోగించండి. మీ కళ్ళ ముందు మీ ముఖానికి వ్యతిరేకంగా పెన్ను పెన్ను పట్టుకోండి. మీ కుడి మరియు ఎడమ బొటనవేలు గోళ్లను మీ కుడి మరియు ఎడమ విద్యార్థుల కేంద్రాలతో సమలేఖనం చేయండి. అప్పుడు మీ బ్రొటనవేళ్లు మధ్య విభజనను కొలవడానికి తెరపై పెన్నును తెరపైకి తీసుకురండి. మీ ఎడమ బొటనవేలును సున్నాతో మరియు మీ కుడి బొటనవేలు ఎక్కడ ఉందో గమనించండి. పెన్ను అణిచివేసి, ఆ సమయంలో పాలకుడిని క్లిక్ చేయండి. చివరగా, కొనసాగించడానికి సరే క్లిక్ చేయండి.",
    kn: "ನಿಮ್ಮ ಕಣ್ಣುಗಳ ನಡುವಿನ ಅಂತರವನ್ನು ನಾವು ಅಳೆಯುತ್ತೇವೆ. ನಿಮ್ಮ ಕಣ್ಣುಗಳನ್ನು ವೀಕ್ಷಿಸಲು ವೀಡಿಯೊ ಚಿತ್ರವನ್ನು ಬಳಸಿ. ನಿಮ್ಮ ಕಣ್ಣುಗಳ ಮುಂದೆ ನಿಮ್ಮ ಮುಖದ ವಿರುದ್ಧ ಅಡ್ಡಲಾಗಿ ಪೆನ್ನು ಹಿಡಿದುಕೊಳ್ಳಿ. ನಿಮ್ಮ ಬಲ ಮತ್ತು ಎಡ ಹೆಬ್ಬೆರಳು ಉಗುರುಗಳನ್ನು ನಿಮ್ಮ ಬಲ ಮತ್ತು ಎಡ ವಿದ್ಯಾರ್ಥಿಗಳ ಕೇಂದ್ರಗಳೊಂದಿಗೆ ಜೋಡಿಸಿ. ನಂತರ ನಿಮ್ಮ ಹೆಬ್ಬೆರಳುಗಳ ನಡುವಿನ ಪ್ರತ್ಯೇಕತೆಯನ್ನು ಅಳೆಯಲು ಆನ್-ಸ್ಕ್ರೀನ್ ಆಡಳಿತಗಾರನಿಗೆ ಪೆನ್ನು ತಂದುಕೊಡಿ. ನಿಮ್ಮ ಎಡ ಹೆಬ್ಬೆರಳನ್ನು ಶೂನ್ಯದೊಂದಿಗೆ ಅಲೈನ್ ಮಾಡಿ ಮತ್ತು ನಿಮ್ಮ ಬಲ ಹೆಬ್ಬೆರಳು ಎಲ್ಲಿದೆ ಎಂಬುದನ್ನು ಗಮನಿಸಿ. ಪೆನ್ನು ಕೆಳಗೆ ಇರಿಸಿ ಮತ್ತು ಆ ಸಮಯದಲ್ಲಿ ಆಡಳಿತಗಾರನನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ. ಅಂತಿಮವಾಗಿ, ಮುಂದುವರಿಸಲು ಸರಿ ಕ್ಲಿಕ್ ಮಾಡಿ.",
    bn: "আমরা আপনার চোখের মধ্যে দূরত্ব পরিমাপ করব। আপনার চোখ দেখতে ভিডিও চিত্রটি ব্যবহার করুন। আপনার চোখের সামনে আপনার মুখের বিরুদ্ধে অনুভূমিকভাবে একটি কলম ধরে রাখুন। আপনার ডান এবং বাম ছাত্রদের কেন্দ্রগুলির সাথে আপনার ডান এবং বাম থাম্ব নখগুলি সারিবদ্ধ করুন। তারপরে আপনার থাম্বগুলির মধ্যে বিচ্ছেদ পরিমাপ করতে অন-স্ক্রিন শাসকের কাছে কলমটি আনুন। আপনার বাম থাম্বকে শূন্য দিয়ে অলিগ করুন এবং আপনার ডান থাম্বটি কোথায় তা লক্ষ্য করুন। কলমটি নামিয়ে দিন এবং সেই মুহুর্তে শাসককে ক্লিক করুন। অবশেষে, চালিয়ে যেতে ওকে ক্লিক করুন।",
    id: "Kami akan mengukur jarak di antara mata Anda. Gunakan gambar video untuk menonton mata Anda. Pegang pena secara horizontal di wajah Anda di depan mata Anda. Sejajarkan kuku ibu jari kanan dan kiri Anda dengan pusat -pusat murid kanan dan kiri Anda. Kemudian bawa pena ke penguasa di layar untuk mengukur pemisahan antara ibu jari Anda. Lepaskan ibu jari kiri Anda dengan nol dan perhatikan di mana ibu jari kanan Anda berada. Letakkan pena dan klik penguasa pada saat itu. Akhirnya, klik OK untuk melanjutkan.",
    "zh-CN":
      "我们将测量您的眼睛之间的距离。使用视频图像观看您的眼睛。在眼前，水平握在脸上。将您的左右拇指指甲与左右学生的中心对齐。然后将笔带到屏幕上的统治者，以测量拇指之间的分离。零拇指的左拇指为零，并注意右拇指在哪里。放下笔，然后单击标尺。最后，单击确定继续。",
    "zh-HK":
      "我們將測量您的眼睛之間的距離。使用視頻圖像觀看您的眼睛。在眼前，水平握在臉上。將您的左右拇指指甲與左右學生的中心對齊。然後將筆帶到屏幕上的統治者，以測量拇指之間的分離。零拇指的左拇指為零，並註意右拇指在哪裡。放下筆，然後單擊標尺。最後，單擊確定繼續。",
    ja: "目の間の距離を測定します。ビデオ画像を使用して目を見る。目の前で顔に水平にペンを保持します。右と左の親指の爪を右と左の生徒の中心に合わせます。次に、画面上の定規にペンを持ってきて、親指の分離を測定します。左手の親指をゼロで割り当て、右親指がどこにあるかに注意してください。ペンを置いて、その時点で定規をクリックします。最後に、[OK]をクリックして続行します。",
    ko: "우리는 당신의 눈 사이의 거리를 측정 할 것입니다. 비디오 이미지를 사용하여 눈을보십시오. 눈 앞에서 얼굴에 수평으로 펜을 잡으십시오. 오른쪽과 왼쪽 엄지 손톱을 오른쪽과 왼쪽 학생의 중심과 정렬하십시오. 그런 다음 펜을 화면 통치자에게 가져와 엄지 손가락 사이의 분리를 측정하십시오. 왼쪽 엄지 손가락을 0으로 동조하고 오른쪽 엄지가 어디에 있는지 확인하십시오. 펜을 내려 놓고 그 시점에서 통치자를 클릭하십시오. 마지막으로 확인을 클릭하여 계속하십시오.",
  },
  RC_nearPointTitle: {
    "en-US": "Measure Pupillary Distance",
    de: "Messung der Pupillarabstand",
    fr: "Mesurer la distance pupillaire",
    es: "Medir la distancia pupilar",
    pt: "Medir a distância pupilar",
    it: "Misurare la distanza pupillare",
    ro: "Măsurați distanța pupilară",
    pl: "Zmierz odległość źrenicy",
    ru: "Измерьте расстояние зрачка",
    hy: "Չափել աշակերտական ​​հեռավորությունը",
    fi: "Mittaa pupilli -etäisyys",
    ka: "გაზომეთ მოსწავლეთა მანძილი",
    he: "למדוד מרחק גלגול",
    ar: "قياس المسافة الواحدات",
    ur: "pupillary فاصلہ کی پیمائش",
    hi: "प्यूपिलरी दूरी को मापें",
    ta: "மாணவர் தூரத்தை அளவிடவும்",
    ml: "പ്യൂപ്പിലറി ദൂരം അളക്കുക",
    te: "పపిల్లరీ దూరాన్ని కొలవండి",
    kn: "ಪಪಿಲರಿ ದೂರವನ್ನು ಅಳೆಯಿರಿ",
    bn: "পিউপিলারি দূরত্ব পরিমাপ করুন",
    id: "Ukur jarak pupil",
    "zh-CN": "测量瞳孔距离",
    "zh-HK": "測量瞳孔距離",
    ja: "瞳孔距離を測定します",
    ko: "동공 거리를 측정하십시오",
  },
  RC_ok: {
    "en-US": "OK",
    de: "OK",
    fr: "OK",
    es: "DE ACUERDO",
    pt: "OK",
    it: "DE ACUERDO",
    ro: "OK",
    pl: "OK",
    ru: "OK",
    hy: "լավ",
    fi: "Hyvä",
    ka: "კარგი",
    he: "בסדר",
    ar: "نعم",
    ur: "درست",
    hi: "ठीक",
    ta: "சரி",
    ml: "ശരി",
    te: "అలాగే",
    kn: "ಸರಿ",
    bn: "ঠিক আছে",
    id: "OKE",
    "zh-CN": "确认",
    "zh-HK": "確認",
    ja: "確認",
    ko: "좋아요",
  },
  RC_panelButton: {
    "en-US": "Done",
    de: "Erledigt",
    fr: "Fait",
    es: "Hecho",
    pt: "Feito",
    it: "Fatto",
    ro: "Terminat",
    pl: "Zrobione",
    ru: "Сделанный",
    hy: "Կատարած",
    fi: "Tehty",
    ka: "შესრულებულია",
    he: "בוצע",
    ar: "منتهي",
    ur: "کیا",
    hi: "पूर्ण",
    ta: "முடிந்தது",
    ml: "നിറവേറ്റി",
    te: "పూర్తి",
    kn: "ಮುಗಿದ",
    bn: "সম্পন্ন",
    id: "Selesai",
    "zh-CN": "完毕",
    "zh-HK": "完畢",
    ja: "終わり",
    ko: "완료",
  },
  RC_panelIntro: {
    "en-US": "",
    de: "",
    fr: "",
    es: "",
    pt: "",
    it: "",
    ro: "",
    pl: "",
    ru: "",
    hy: "",
    fi: "",
    ka: "",
    he: "",
    ar: "",
    ur: "",
    hi: "",
    ta: "",
    ml: "",
    te: "",
    kn: "",
    bn: "",
    id: "",
    "zh-CN": "",
    "zh-HK": "",
    ja: "",
    ko: "",
  },
  RC_panelIntroNext: {
    "en-US": "",
    de: "",
    fr: "",
    es: "",
    pt: "",
    it: "",
    ro: "",
    pl: "",
    ru: "",
    hy: "",
    fi: "",
    ka: "",
    he: "",
    ar: "",
    ur: "",
    hi: "",
    ta: "",
    ml: "",
    te: "",
    kn: "",
    bn: "",
    id: "",
    "zh-CN": "",
    "zh-HK": "",
    ja: "",
    ko: "",
  },
  RC_panelTitle: {
    "en-US": "Please press a button to calibrate",
    de: "Bitte drücken Sie eine Taste, um zu kalibrieren",
    fr: "Veuillez appuyer sur un bouton pour calibrer",
    es: "Presione un botón para calibrar",
    pt: "Pressione um botão para calibrar",
    it: "Si prega di premere un pulsante per calibrare",
    ro: "Vă rugăm să apăsați un buton pentru a calibra",
    pl: "Naciśnij przycisk, aby kalibrować",
    ru: "Пожалуйста, нажмите кнопку для калибровки",
    hy: "Խնդրում ենք սեղմել կոճակը `տրամաչափելու համար",
    fi: "Paina painiketta kalibroidaksesi",
    ka: "გთხოვთ, დააჭირეთ ღილაკს კალიბრისთვის",
    he: "אנא לחץ על כפתור לכיול",
    ar: "يرجى الضغط على زر للمعايرة",
    ur: "براہ کرم کیلیبریٹ کے لئے ایک بٹن دبائیں",
    hi: "कृपया कैलिब्रेट करने के लिए एक बटन दबाएं",
    ta: "அளவீடு செய்ய ஒரு பொத்தானை அழுத்தவும்",
    ml: "കാലിബ്രേറ്റ് ചെയ്യുന്നതിന് ദയവായി ഒരു ബട്ടൺ അമർത്തുക",
    te: "క్రమాంకనం చేయడానికి దయచేసి ఒక బటన్‌ను నొక్కండి",
    kn: "ಮಾಪನಾಂಕ ನಿರ್ಣಯಿಸಲು ದಯವಿಟ್ಟು ಬಟನ್ ಒತ್ತಿರಿ",
    bn: "ক্যালিব্রেট করতে দয়া করে একটি বোতাম টিপুন",
    id: "Harap tekan tombol untuk mengkalibrasi",
    "zh-CN": "请按按钮校准",
    "zh-HK": "請按按鈕校準",
    ja: "ボタンを押して調整してください",
    ko: "버튼을 눌러 교정하십시오",
  },
  RC_panelTitleNext: {
    "en-US": "Thanks for calibrating. Hit the button to continue.",
    de: "Danke fürs Kalibrieren. Klicken Sie auf die Taste, um fortzufahren.",
    fr: "Merci d'avoir calibré. Appuyez sur le bouton pour continuer.",
    es: "Gracias por calibrar. Presione el botón para continuar.",
    pt: "Obrigado por calibrar. Aperte o botão para continuar.",
    it: "Grazie per la calibrazione. Premi il pulsante per continuare.",
    ro: "Mulțumesc pentru calibrare. Apăsați butonul pentru a continua.",
    pl: "Dzięki za kalibrację. Naciśnij przycisk, aby kontynuować.",
    ru: "Спасибо за калибровку. Нажмите кнопку, чтобы продолжить.",
    hy: "Շնորհակալություն տրամաչափի համար: Հարվածեք կոճակին շարունակելու համար:",
    fi: "Kiitos kalibroinnista. Napsauta painiketta jatkaaksesi.",
    ka: "მადლობა კალიბრაციისთვის. დააჭირეთ ღილაკს გასაგრძელებლად.",
    he: "תודה שכיול. לחץ על הכפתור כדי להמשיך.",
    ar: "شكرا لمعايرة. اضغط على الزر للمتابعة.",
    ur: "کیلیبریٹنگ کا شکریہ۔ جاری رکھنے کے لئے بٹن کو دبائیں۔",
    hi: "कैलिब्रेट करने के लिए धन्यवाद। जारी रखने के लिए बटन दबाएं।",
    ta: "அளவீடு செய்ததற்கு நன்றி. தொடர பொத்தானை அழுத்தவும்.",
    ml: "കാലിബ്രേറ്റിംഗിന് നന്ദി. തുടരാൻ ബട്ടൺ അമർത്തുക.",
    te: "క్రమాంకనం చేసినందుకు ధన్యవాదాలు. కొనసాగించడానికి బటన్‌ను నొక్కండి.",
    kn: "ಮಾಪನಾಂಕ ನಿರ್ಣಯಿಸಿದ್ದಕ್ಕಾಗಿ ಧನ್ಯವಾದಗಳು. ಮುಂದುವರಿಸಲು ಗುಂಡಿಯನ್ನು ಒತ್ತಿರಿ.",
    bn: "ক্যালিব্রেট করার জন্য ধন্যবাদ। চালিয়ে যেতে বোতামটি আঘাত করুন।",
    id: "Terima kasih telah mengkalibrasi. Tekan tombol untuk melanjutkan.",
    "zh-CN": "感谢您的校准。点击按钮继续。",
    "zh-HK": "感謝您的校準。點擊按鈕繼續。",
    ja: "キャリブレーションしてくれてありがとう。ボタンを押して続行します。",
    ko: "교정 해주셔서 감사합니다. 계속하려면 버튼을 누르십시오.",
  },
  RC_panelUsesWebcam: {
    "en-US": "uses webcam",
    de: "Verwendet Webcam",
    fr: "utilise la webcam",
    es: "Utiliza la cámara web",
    pt: "usa webcam",
    it: "Utilizza la webcam",
    ro: "utilizează webcam",
    pl: "używa kamery internetowej",
    ru: "использует веб -камеру",
    hy: "Օգտագործում է վեբ-տեսախցիկը",
    fi: "käyttää verkkokameraa",
    ka: "იყენებს ვებკამერას",
    he: "משתמש במצלמת רשת",
    ar: "يستخدم كاميرا الويب",
    ur: "ویب کیم استعمال کرتا ہے",
    hi: "वेबकैम का उपयोग करता है",
    ta: "வெப்கேம் பயன்படுத்துகிறது",
    ml: "വെബ്ക്യാം ഉപയോഗിക്കുന്നു",
    te: "వెబ్‌క్యామ్‌ను ఉపయోగిస్తుంది",
    kn: "ವೆಬ್‌ಕ್ಯಾಮ್ ಅನ್ನು ಬಳಸುತ್ತದೆ",
    bn: "ওয়েবক্যাম ব্যবহার করে",
    id: "menggunakan webcam",
    "zh-CN": "使用网络摄像头",
    "zh-HK": "使用網絡攝像頭",
    ja: "ウェブカメラを使用します",
    ko: "웹캠을 사용합니다",
  },
  RC_panelUsesWebcamPhone: {
    "en-US": "uses webcam and phone",
    de: "Verwendet Webcam und Telefon",
    fr: "Utilise la webcam et le téléphone",
    es: "Utiliza cámara web y teléfono",
    pt: "Usa webcam e telefone",
    it: "Utilizza webcam e telefono",
    ro: "Utilizează webcam și telefon",
    pl: "używa kamery internetowej i telefonu",
    ru: "использует веб -камеру и телефон",
    hy: "Օգտագործում է վեբ-տեսախցիկ եւ հեռախոս",
    fi: "käyttää verkkokameraa ja puhelinta",
    ka: "იყენებს ვებკამერასა და ტელეფონს",
    he: "משתמש במצלמת רשת ובטלפון",
    ar: "يستخدم كاميرا الويب والهاتف",
    ur: "ویب کیم اور فون استعمال کرتا ہے",
    hi: "वेबकैम और फोन का उपयोग करता है",
    ta: "வெப்கேம் மற்றும் தொலைபேசியைப் பயன்படுத்துகிறது",
    ml: "വെബ്ക്യാമും ഫോണും ഉപയോഗിക്കുന്നു",
    te: "వెబ్‌క్యామ్ మరియు ఫోన్‌ను ఉపయోగిస్తుంది",
    kn: "ವೆಬ್‌ಕ್ಯಾಮ್ ಮತ್ತು ಫೋನ್ ಬಳಸುತ್ತದೆ",
    bn: "ওয়েবক্যাম এবং ফোন ব্যবহার করে",
    id: "menggunakan webcam dan telepon",
    "zh-CN": "使用网络摄像头和手机",
    "zh-HK": "使用網絡攝像頭和電話",
    ja: "ウェブカメラと電話を使用します",
    ko: "웹캠과 전화를 사용합니다",
  },
  RC_performance: {
    "en-US": "Graphics Performance",
    de: "Grafikleistung",
    fr: "Performance graphique",
    es: "Rendimiento gráfico",
    pt: "Desempenho gráfico",
    it: "Prestazioni grafiche",
    ro: "Performanță grafică",
    pl: "Wydajność grafiki",
    ru: "Графическая производительность",
    hy: "Գրաֆիկայի կատարումը",
    fi: "Grafiikan suorituskyky",
    ka: "გრაფიკული შესრულება",
    he: "ביצועי גרפיקה",
    ar: "أداء الرسومات",
    ur: "گرافکس کی کارکردگی",
    hi: "ग्राफिक्स प्रदर्शन",
    ta: "கிராபிக்ஸ் செயல்திறன்",
    ml: "ഗ്രാഫിക്സ് പ്രകടനം",
    te: "గ్రాఫిక్స్ పనితీరు",
    kn: "ಗ್ರಾಫಿಕ್ಸ್ ಕಾರ್ಯಕ್ಷಮತೆ",
    bn: "গ্রাফিক্স পারফরম্যান্স",
    id: "Kinerja grafis",
    "zh-CN": "图形性能",
    "zh-HK": "圖形性能",
    ja: "グラフィックパフォーマンス",
    ko: "그래픽 성능",
  },
  RC_performanceIntro: {
    "en-US":
      "Please do not disturb. Drawing graphics to assess web browser performance ...",
    de: "Bitte nicht stören. Grafik zeichnen, um die Leistung des Webbrowsers zu bewerten ...",
    fr: "Prière de ne pas déranger. Dessin graphiques pour évaluer les performances du navigateur Web ...",
    es: "Por favor no molestar. Dibujo de gráficos para evaluar el rendimiento del navegador web ...",
    pt: "Por favor não incomode. Desenho de gráficos para avaliar o desempenho do navegador da web ...",
    it: "Per favore non disturbare. Disegnare grafica per valutare le prestazioni del browser Web ...",
    ro: "Va rog nu deranjati. Desenarea graficelor pentru a evalua performanța browserului web ...",
    pl: "Proszę nie przeszkadzać. Rysowanie grafiki w celu oceny wydajności przeglądarki internetowej ...",
    ru: "Пожалуйста, не беспокойте. Рисование графики для оценки производительности веб -браузера ...",
    hy: "Խնդրում ենք մի խանգարել: Գրաֆիկայի գծագրումը `վեբ զննարկչի աշխատանքը գնահատելու համար ...",
    fi: "Ole kiltti äläkä häiritse. Grafiikan piirtäminen verkkoselaimen suorituskyvyn arvioimiseksi ...",
    ka: "Გთხოვთ არ შემაწუხოთ. გრაფიკის შედგენა ვებ - ბრაუზერის შესრულების შესაფასებლად ...",
    he: "נא לא להפריע. ציור גרפיקה כדי להעריך את ביצועי דפדפן האינטרנט ...",
    ar: "الرجاء عدم الإزعاج. رسم الرسومات لتقييم أداء متصفح الويب ...",
    ur: "برائے مہربانی تنگ نہ کریں. ویب براؤزر کی کارکردگی کا اندازہ کرنے کے لئے گرافکس ڈرائنگ ...",
    hi: "कृपया परेशान न करें। वेब ब्राउज़र प्रदर्शन का आकलन करने के लिए ग्राफिक्स ड्राइंग ...",
    ta: "தயவுசெய்து தொந்தரவு செய்ய வேண்டாம். வலை உலாவி செயல்திறனை மதிப்பிடுவதற்கு கிராபிக்ஸ் வரைதல் ...",
    ml: "ശല്യപ്പെടുത്തരുത്. വെബ് ബ്ര browser സർ പ്രകടനം വിലയിരുത്താൻ ഗ്രാഫിക്സ് വരയ്ക്കുന്നു ...",
    te: "దయచేసి భంగం కలిగించవద్దు. వెబ్ బ్రౌజర్ పనితీరును అంచనా వేయడానికి గ్రాఫిక్స్ గీయడం ...",
    kn: "ದಯವಿಟ್ಟು ತೊಂದರೆ ನೀಡಬೇಡಿ. ವೆಬ್ ಬ್ರೌಸರ್ ಕಾರ್ಯಕ್ಷಮತೆಯನ್ನು ನಿರ್ಣಯಿಸಲು ಗ್ರಾಫಿಕ್ಸ್ ಅನ್ನು ಚಿತ್ರಿಸುವುದು ...",
    bn: "অনুগ্রহ করে বিরক্ত করবেন না. ওয়েব ব্রাউজারের পারফরম্যান্স মূল্যায়ন করতে গ্রাফিক্স অঙ্কন ...",
    id: "Tolong jangan ganggu. Menggambar grafik untuk menilai kinerja browser web ...",
    "zh-CN": "请不要打扰。绘制图形以评估Web浏览器性能...",
    "zh-HK": "請不要打擾。繪製圖形以評估Web瀏覽器性能...",
    ja: "邪魔しないでください。 Webブラウザのパフォーマンスを評価するためのグラフィックを描く...",
    ko: "방해하지 마십시오. 웹 브라우저 성능을 평가하기위한 그래픽 그리기 ...",
  },
  RC_performanceTitle: {
    "en-US": "Graphics Performance",
    de: "Grafikleistung",
    fr: "Performance graphique",
    es: "Rendimiento gráfico",
    pt: "Desempenho gráfico",
    it: "Prestazioni grafiche",
    ro: "Performanță grafică",
    pl: "Wydajność grafiki",
    ru: "Графическая производительность",
    hy: "Գրաֆիկայի կատարումը",
    fi: "Grafiikan suorituskyky",
    ka: "გრაფიკული შესრულება",
    he: "ביצועי גרפיקה",
    ar: "أداء الرسومات",
    ur: "گرافکس کی کارکردگی",
    hi: "ग्राफिक्स प्रदर्शन",
    ta: "கிராபிக்ஸ் செயல்திறன்",
    ml: "ഗ്രാഫിക്സ് പ്രകടനം",
    te: "గ్రాఫిక్స్ పనితీరు",
    kn: "ಗ್ರಾಫಿಕ್ಸ್ ಕಾರ್ಯಕ್ಷಮತೆ",
    bn: "গ্রাফিক্স পারফরম্যান্স",
    id: "Kinerja grafis",
    "zh-CN": "图形性能",
    "zh-HK": "圖形性能",
    ja: "グラフィックパフォーマンス",
    ko: "그래픽 성능",
  },
  RC_privacyCamera: {
    "en-US":
      "ASSURANCE OF CAMERA PRIVACY. No photos or video are recorded. Each webcam frame is used only inside this computer, solely to estimate eye and gaze positions, and then erased, within 10 seconds. Only the eye and gaze positions are retained.",
    de: "Gewissheit der Privatsphäre der Kamera. Es werden keine Fotos oder Videos aufgezeichnet. Jeder Webcam -Rahmen wird nur in diesem Computer verwendet, um nur die Augen- und Blickpositionen zu schätzen, und dann innerhalb von 10 Sekunden gelöscht. Nur die Auge- und Blickpositionen bleiben erhalten.",
    fr: "Assurance de la confidentialité de la caméra. Aucune photo ou vidéo n'est enregistrée. Chaque cadre de webcam est utilisé uniquement à l'intérieur de cet ordinateur, uniquement pour estimer les positions des yeux et du regard, puis effacée, dans les 10 secondes. Seules les positions des yeux et du regard sont conservées.",
    es: "Aseguramiento de la privacidad de la cámara. No se graban fotos o video. Cada marco de cámara web se usa solo dentro de esta computadora, únicamente para estimar las posiciones de los ojos y la mirada, y luego se borra, en 10 segundos. Solo se conservan las posiciones de ojo y mirada.",
    pt: "Garantia da privacidade da câmera. Nenhuma foto ou vídeo é gravado. Cada quadro da webcam é usado apenas dentro deste computador, apenas para estimar as posições dos olhos e do olhar, e depois apagado, em 10 segundos. Somente as posições de olho e olhar são retidas.",
    it: "Assicurazione della privacy della telecamera. Non vengono registrate foto o video. Ogni cornice di webcam viene utilizzato solo all'interno di questo computer, esclusivamente per stimare le posizioni degli occhi e dello sguardo, quindi cancellato, entro 10 secondi. Vengono mantenute solo le posizioni degli occhi e dello sguardo.",
    ro: "Asigurarea confidențialității camerei. Nu sunt înregistrate fotografii sau videoclipuri. Fiecare cadru webcam este utilizat doar în interiorul acestui computer, exclusiv pentru a estima pozițiile ochilor și privirea, apoi șterse, în 10 secunde. Sunt păstrate doar pozițiile ochiului și privirii.",
    pl: "Zapewnienie prywatności kamery. Żadne zdjęcia ani wideo nie są nagrywane. Każda ramka kamery internetowej jest używana tylko wewnątrz tego komputera, wyłącznie do oszacowania pozycji oka i spojrzenia, a następnie wymazana w ciągu 10 sekund. Zatrzymane są tylko pozycje oka i spojrzenia.",
    ru: "Уверенность в конфиденциальности камеры. Фотографии или видео не записаны. Каждая рама веб -камеры используется только внутри этого компьютера, исключительно для оценки положений глаз и взгляда, а затем стерт в течение 10 секунд. Только положения глаз и взгляда сохраняются.",
    hy: "Խցիկի գաղտնիության ապահովում: Ոչ մի լուսանկար կամ տեսանյութ չի արձանագրվում: Տեսախցիկի յուրաքանչյուր շրջանակ է օգտագործվում միայն այս համակարգչի ներսում, բացառապես աչքերի եւ հայացքի դիրքերը գնահատելու համար, այնուհետեւ ջնջվել, 10 վայրկյանում: Պահպանվում են միայն աչքի եւ հայացքի դիրքերը:",
    fi: "Kameran yksityisyyden varmuus. Valokuvia tai videoita ei ole tallennettu. Jokaista verkkokamerakehystä käytetään vain tämän tietokoneen sisällä, pelkästään silmä- ja katselukohteiden arvioimiseksi ja poistetaan sitten 10 sekunnissa. Vain silmä- ja katseasennot säilytetään.",
    ka: "კამერის კონფიდენციალურობის უზრუნველყოფა. არ არის ჩაწერილი ფოტოები ან ვიდეო. თითოეული ვებკამერის ჩარჩო გამოიყენება მხოლოდ ამ კომპიუტერის შიგნით, მხოლოდ თვალის და მზერა პოზიციების შესაფასებლად, შემდეგ კი წაშლილია, 10 წამის განმავლობაში. შენარჩუნებულია მხოლოდ თვალისა და მზერა.",
    he: "הבטחה לפרטיות המצלמה. לא מוקלטים תמונות או וידאו. כל מסגרת מצלמת רשת משמשת רק בתוך מחשב זה, אך ורק כדי להעריך עמדות עיניים ומבט ואז נמחקות, תוך 10 שניות. רק עמדות העין והמבט נשמרות.",
    ar: "ضمان خصوصية الكاميرا. لا يتم تسجيل أي صور أو فيديو. يتم استخدام كل إطار كاميرا ويب فقط داخل هذا الكمبيوتر ، فقط لتقدير مواقف العين والنظرة ، ثم تمحى ، في غضون 10 ثوان. يتم الاحتفاظ فقط بمواقف العين والنظرة.",
    ur: "کیمرے کی رازداری کی یقین دہانی۔ کوئی فوٹو یا ویڈیو ریکارڈ نہیں کی گئی ہے۔ ہر ویب کیم فریم صرف اس کمپیوٹر کے اندر استعمال ہوتا ہے ، مکمل طور پر آنکھوں اور نگاہوں کی پوزیشنوں کا تخمینہ لگانے کے لئے ، اور پھر 10 سیکنڈ کے اندر مٹ جاتا ہے۔ صرف آنکھ اور نگاہوں کی پوزیشنیں برقرار ہیں۔",
    hi: "कैमरा गोपनीयता का आश्वासन। कोई फ़ोटो या वीडियो रिकॉर्ड नहीं किया गया है। प्रत्येक वेबकैम फ्रेम का उपयोग केवल इस कंप्यूटर के अंदर किया जाता है, पूरी तरह से आंख और टकटकी पदों का अनुमान लगाने के लिए, और फिर 10 सेकंड के भीतर मिटा दिया जाता है। केवल आंख और टकटकी पदों को बरकरार रखा जाता है।",
    ta: "கேமரா தனியுரிமையின் உறுதி. புகைப்படங்கள் அல்லது வீடியோ எதுவும் பதிவு செய்யப்படவில்லை. ஒவ்வொரு வெப்கேம் சட்டமும் இந்த கணினிக்குள் மட்டுமே பயன்படுத்தப்படுகிறது, இது கண் மற்றும் பார்வை நிலைகளை மதிப்பிடுவதற்கு மட்டுமே, பின்னர் 10 வினாடிகளுக்குள் அழிக்கப்படுகிறது. கண் மற்றும் பார்வை நிலைகள் மட்டுமே தக்கவைக்கப்படுகின்றன.",
    ml: "ക്യാമറ സ്വകാര്യതയുടെ ഉറപ്പ്. ഫോട്ടോകളോ വീഡിയോയോ രേഖപ്പെടുത്തിയിട്ടില്ല. ഓരോ വെബ്ക്യാം ഫ്രെയിം ഈ കമ്പ്യൂട്ടറിനുള്ളിൽ മാത്രമേ ഉപയോഗിക്കുന്നുള്ളൂ, കണ്ണ്, നോട്ടങ്ങൾ സ്ഥാനങ്ങൾ കണക്കാക്കാൻ മാത്രം, തുടർന്ന് 10 സെക്കൻഡിനുള്ളിൽ മായ്ച്ചുകളഞ്ഞു. കണ്ണ്, നോട്ടം സ്ഥാനങ്ങൾ മാത്രം നിലനിർത്തുന്നു.",
    te: "కెమెరా గోప్యత యొక్క హామీ. ఫోటోలు లేదా వీడియో రికార్డ్ చేయబడలేదు. ప్రతి వెబ్‌క్యామ్ ఫ్రేమ్ ఈ కంప్యూటర్ లోపల మాత్రమే ఉపయోగించబడుతుంది, ఇది కంటి మరియు చూపు స్థానాలను అంచనా వేయడానికి మాత్రమే, ఆపై 10 సెకన్లలోపు తొలగించబడుతుంది. కంటి మరియు చూపుల స్థానాలు మాత్రమే అలాగే ఉంచబడతాయి.",
    kn: "ಕ್ಯಾಮೆರಾ ಗೌಪ್ಯತೆಯ ಭರವಸೆ. ಯಾವುದೇ ಫೋಟೋಗಳು ಅಥವಾ ವೀಡಿಯೊವನ್ನು ದಾಖಲಿಸಲಾಗುವುದಿಲ್ಲ. ಪ್ರತಿಯೊಂದು ವೆಬ್‌ಕ್ಯಾಮ್ ಫ್ರೇಮ್ ಅನ್ನು ಈ ಕಂಪ್ಯೂಟರ್ ಒಳಗೆ ಮಾತ್ರ ಬಳಸಲಾಗುತ್ತದೆ, ಕೇವಲ ಕಣ್ಣು ಮತ್ತು ನೋಟದ ಸ್ಥಾನಗಳನ್ನು ಅಂದಾಜು ಮಾಡಲು, ಮತ್ತು ನಂತರ 10 ಸೆಕೆಂಡುಗಳಲ್ಲಿ ಅಳಿಸಿಹಾಕಲಾಗುತ್ತದೆ. ಕಣ್ಣು ಮತ್ತು ನೋಟದ ಸ್ಥಾನಗಳನ್ನು ಮಾತ್ರ ಉಳಿಸಿಕೊಳ್ಳಲಾಗುತ್ತದೆ.",
    bn: "ক্যামেরার গোপনীয়তার আশ্বাস। কোনও ফটো বা ভিডিও রেকর্ড করা হয় না। প্রতিটি ওয়েবক্যাম ফ্রেম কেবলমাত্র এই কম্পিউটারের ভিতরে ব্যবহৃত হয়, কেবলমাত্র চোখ এবং দৃষ্টিতে অবস্থানগুলি অনুমান করার জন্য এবং তারপরে 10 সেকেন্ডের মধ্যে মুছে ফেলা হয়। কেবল চোখ এবং দৃষ্টিতে অবস্থানগুলি ধরে রাখা হয়।",
    id: "Jaminan privasi kamera. Tidak ada foto atau video yang direkam. Setiap bingkai webcam hanya digunakan di dalam komputer ini, semata -mata untuk memperkirakan posisi mata dan pandangan, dan kemudian dihapus, dalam 10 detik. Hanya posisi mata dan tatapan yang dipertahankan.",
    "zh-CN":
      "摄像机隐私的保证。没有录制照片或视频。每个网络摄像头框架仅在这台计算机内使用，仅用于估计眼睛和凝视位置，然后在10秒内擦除。只保留了眼睛和凝视位置。",
    "zh-HK":
      "攝像機隱私的保證。沒有錄製照片或視頻。每個網絡攝像頭框架僅在這台計算機內使用，僅用於估計眼睛和凝視位置，然後在10秒內擦除。只保留了眼睛和凝視位置。",
    ja: "カメラのプライバシーの保証。写真やビデオは記録されていません。各ウェブカメラフレームは、このコンピューター内でのみ使用され、目と視線の位置を推定するためだけに使用され、10秒以内に消去されます。目と視線の位置のみが保持されます。",
    ko: "카메라 프라이버시의 보증. 사진이나 비디오가 녹음되지 않습니다. 각 웹캠 프레임은이 컴퓨터 내부에서만 사용되며 눈과 시선 위치를 추정 한 다음 10 초 이내에 지워집니다. 눈과 시선 위치 만 유지됩니다.",
  },
  RC_privacyIphoneMicrophone: {
    "en-US":
      'ASSURANCE OF MICROPHONE PRIVACY. EasyEyes uses your iPhone/iPad microphone, rather than your desktop\'s built-in microphone, because the iPhone/iPad microphone is manufacturer-calibrated. The microphone records only while a loud sound plays through the desktop computer loudspeaker, AND both the desktop computer prominently displays "QUIET PLEASE! NOW RECORDING." The recording is used solely to estimate loudspeaker characteristics and then erased, within 30 seconds. Only the loudspeaker characteristics are retained.',
    de: 'Gewissheit der Privatsphäre von Mikrofon. Easyeyes verwendet Ihr iPhone/iPad-Mikrofon anstelle des integrierten Mikrofons Ihres Desktops, da das iPhone/iPad-Mikrofon Hersteller kalibriert ist. Das Mikrofon zeichnet nur auf, während ein lauter Sound den Desktop -Computer -Lautsprecher durchläuft, und sowohl im Desktop -Computer zeigen prominent "Quiet bitte! Jetzt aufnehmen". Die Aufzeichnung wird ausschließlich zur Schätzung von Lautsprechereigenschaften verwendet und dann innerhalb von 30 Sekunden gelöscht. Nur die Lautsprechermerkmale werden beibehalten.',
    fr: "Assurance de la confidentialité des microphones. EasyEyes utilise votre microphone iPhone / iPad, plutôt que le microphone intégré de votre bureau, car le microphone iPhone / iPad est calibré du fabricant. Le microphone n'enregistre que pendant qu'un son fort se joue dans le haut-parleur de l'ordinateur de bureau, et l'ordinateur de bureau affiche bien en vue \"silencieux s'il vous plaît! Maintenant enregistrant\". L'enregistrement est utilisé uniquement pour estimer les caractéristiques des haut-parleurs, puis effacée, dans les 30 secondes. Seules les caractéristiques des haut-parleurs sont conservées.",
    es: 'Aseguramiento de la privacidad del micrófono. EasyEyes utiliza su micrófono iPhone/iPad, en lugar del micrófono incorporado de su escritorio, porque el micrófono iPhone/iPad está calibrado por el fabricante. El micrófono registra solo mientras un sonido fuerte se reproduce a través del altavoz de la computadora de escritorio, y tanto la computadora de escritorio muestra prominentemente "Quiet Please! Ahora grabando". La grabación se utiliza únicamente para estimar las características del altavoz y luego se borra, en 30 segundos. Solo se conservan las características del altavoz.',
    pt: 'Garantia da privacidade do microfone. O Easyeyes usa seu microfone para iPhone/iPad, em vez do microfone embutido da sua área de trabalho, porque o microfone para iPhone/iPad é calibrado ao fabricante. O microfone registra apenas enquanto um som alto é exibido pelo alto -falante de computadores da área de trabalho, e o computador da área de trabalho exibe com destaque "Quiet, por favor! Agora, gravação". A gravação é usada apenas para estimar as características do alto -falante e depois apagada, em 30 segundos. Somente as características do alto -falante são mantidas.',
    it: 'Assicurazione della privacy del microfono. EasyEyes utilizza il tuo microfono iPhone/iPad, piuttosto che il microfono integrato del tuo desktop, perché il microfono iPhone/iPad è calibrato dal produttore. Il microfono registra solo mentre un suono forte suona attraverso l\'altoparlante del computer desktop, ed entrambi il computer desktop visualizza in modo prominente "Quiet Please! Ora Registrazione". La registrazione viene utilizzata esclusivamente per stimare le caratteristiche degli altoparlanti e quindi cancellata, entro 30 secondi. Vengono mantenute solo le caratteristiche degli altoparlanti.',
    ro: "Asigurarea confidențialității microfonului. EasyEyes folosește microfonul dvs. iPhone/iPad, mai degrabă decât microfonul încorporat al desktopului, deoarece microfonul iPhone/iPad este calibrat de producător. Microfonul se înregistrează doar în timp ce un sunet puternic se joacă prin difuzorul computerului desktop, iar atât computerul desktop afișează în mod evident „liniștit te rog! Acum înregistrare”. Înregistrarea este utilizată exclusiv pentru a estima caracteristicile difuzoarelor și apoi șterse, în 30 de secunde. Doar caracteristicile difuzoarelor sunt păstrate.",
    pl: "Zapewnienie prywatności mikrofonu. Easyeyes używa mikrofonu iPhone'a/iPada, a nie wbudowanego mikrofonu komputera stacjonarnego, ponieważ mikrofon iPhone/iPad jest kalibrowany producentem. Mikrofon rejestruje tylko podczas głośnego dźwięku przez głośnik komputerowy komputerowy, a oba komputer stacjonarny wyświetla „Cicho proszę! Now nagrywanie”. Nagrywanie służy wyłącznie do oszacowania właściwości głośników, a następnie usunięcia, w ciągu 30 sekund. Zatrzymywane są tylko charakterystyka głośnika.",
    ru: "Уверенность в конфиденциальности микрофона. Easyeyes использует ваш микрофон iPhone/iPad, а не встроенный микрофон на рабочем столе, потому что микрофон iPhone/iPad откалиброван производителем. Микрофон записывается только в то время как громкий звук играет через настольный компьютерный мангутинг, и оба настольного компьютера заметно отображают «тихо, пожалуйста! Теперь запись». Запись используется исключительно для оценки характеристик громкоговорителей, а затем стирается в течение 30 секунд. Только характеристики громкоговорителя сохраняются.",
    hy: "Խոսափողի գաղտնիության ապահովում: EasyEyes- ը օգտագործում է ձեր iPhone / iPad խոսափողը, այլ ոչ թե ձեր աշխատասեղանի ներկառուցված խոսափողը, քանի որ iPhone / iPad խոսափողը արտադրող-տրամաչափված է: Խոսափողի գրառումները միայն այն ժամանակ, երբ բարձրաձայն ձայնը խաղում է աշխատասեղանի համակարգչային բարձրախոսի միջոցով, եւ երկու աշխատասեղանի համակարգիչը ակնհայտորեն ցուցադրում է «հանգիստ»: Ձայնագրությունը օգտագործվում է բացառապես բարձրախոսի բնութագրերը գնահատելու համար, այնուհետեւ ջնջվում է, 30 վայրկյանում: Պահպանվում են միայն բարձրախոսի բնութագրերը:",
    fi: 'Mikrofonin yksityisyyden varmuus. Easyeyes käyttää iPhone/iPad-mikrofoniasi työpöydän sisäänrakennetun mikrofonin sijasta, koska iPhone/iPad-mikrofoni on valmistajakalibroitu. Mikrofoni tallentaa vain, kun äänekäs ääni soi työpöytätietokoneen kaiuttimen kautta, ja molemmat työpöytätietokone näyttää näkyvästi "hiljainen kiitos! Nyt nauhoittaminen". Tallennusta käytetään yksinomaan kaiutinominaisuuksien arvioimiseksi ja poistetaan sitten 30 sekunnissa. Vain kaiutinominaisuudet säilyvät.',
    ka: 'მიკროფონის კონფიდენციალურობის უზრუნველყოფა. EasyEyes იყენებს თქვენს iPhone/iPad მიკროფონს, ვიდრე თქვენი დესკტოპის ჩაშენებული მიკროფონი, რადგან iPhone/iPad მიკროფონი მწარმოებელ კალიბრირებულია. მიკროფონი ჩაწერს მხოლოდ მაშინ, როდესაც ხმამაღალი ხმა თამაშობს დესკტოპის კომპიუტერის დინამიკით, და ორივე დესკტოპის კომპიუტერი აშკარად აჩვენებს "მშვიდი გთხოვთ! ახლა ჩაწერეთ". ჩანაწერი გამოიყენება მხოლოდ დინამიკის მახასიათებლების შესაფასებლად და შემდეგ წაშლილია, 30 წამში. შენარჩუნებულია მხოლოდ დინამიკის მახასიათებლები.',
    he: 'הבטחה לפרטיות המיקרופון. EasyEyes משתמש במיקרופון ה- iPhone/iPad שלך, ולא במיקרופון המובנה של שולחן העבודה שלך, מכיוון שמיקרופון ה- iPhone/iPad מכויל יצרן. המיקרופון מתעדכן רק בזמן שצליל חזק משחק ברמקול המחשב השולחני, ושני המחשב השולחני מציג באופן בולט "שקט בבקשה! עכשיו הקלטת". ההקלטה משמשת אך ורק כדי להעריך את מאפייני הרמקול ואז למחוק, תוך 30 שניות. רק מאפייני הרמקול נשמרים.',
    ar: 'ضمان خصوصية الميكروفون. يستخدم Easyeyes ميكروفون iPhone/iPad الخاص بك ، بدلاً من الميكروفون المدمج في سطح المكتب ، لأن ميكروفون iPhone/iPad يتم معايرته. يسجل الميكروفون فقط بينما يتم تشغيل الصوت الصاخب من خلال مكبر الصوت كمبيوتر سطح المكتب ، ويعرض كلا الكمبيوتر المكتبي بشكل بارز "هادئ! التسجيل الآن". يتم استخدام التسجيل فقط لتقدير خصائص مكبر الصوت ثم تمحى ، في غضون 30 ثانية. يتم الاحتفاظ فقط بخصائص مكبر الصوت.',
    ur: 'مائکروفون پرائیویسی کی یقین دہانی۔ ایزیئس آپ کے ڈیسک ٹاپ کے بلٹ ان مائکروفون کے بجائے آپ کے آئی فون/آئی پیڈ مائکروفون کا استعمال کرتا ہے ، کیونکہ آئی فون/آئی پیڈ مائکروفون کارخانہ دار ہے۔ مائکروفون صرف اس وقت ریکارڈ کرتا ہے جب ایک تیز آواز ڈیسک ٹاپ کمپیوٹر لاؤڈ اسپیکر کے ذریعے چلتی ہے ، اور دونوں ڈیسک ٹاپ کمپیوٹر نمایاں طور پر "خاموش پلیز! اب ریکارڈنگ" دکھاتا ہے۔ ریکارڈنگ کا استعمال مکمل طور پر لاؤڈ اسپیکر کی خصوصیات کا اندازہ لگانے اور پھر مٹ جانے کے لئے کیا جاتا ہے ، 30 سیکنڈ کے اندر۔ صرف لاؤڈ اسپیکر خصوصیات کو برقرار رکھا گیا ہے۔',
    hi: 'माइक्रोफोन गोपनीयता का आश्वासन। Easyeyes आपके डेस्कटॉप के अंतर्निहित माइक्रोफोन के बजाय आपके iPhone/iPad माइक्रोफोन का उपयोग करता है, क्योंकि iPhone/iPad माइक्रोफोन निर्माता-कैलिब्रेटेड है। माइक्रोफोन केवल रिकॉर्ड करता है जबकि एक तेज ध्वनि डेस्कटॉप कंप्यूटर लाउडस्पीकर के माध्यम से खेलती है, और दोनों डेस्कटॉप कंप्यूटर प्रमुख रूप से "शांत कृपया! अब रिकॉर्डिंग" प्रदर्शित करता है। रिकॉर्डिंग का उपयोग केवल लाउडस्पीकर विशेषताओं का अनुमान लगाने के लिए किया जाता है और फिर 30 सेकंड के भीतर मिटा दिया जाता है। केवल लाउडस्पीकर विशेषताओं को बनाए रखा जाता है।',
    ta: 'மைக்ரோஃபோன் தனியுரிமையின் உறுதி. உங்கள் டெஸ்க்டாப்பின் உள்ளமைக்கப்பட்ட மைக்ரோஃபோனைக் காட்டிலும் ஈஸிஇஸ் உங்கள் ஐபோன்/ஐபாட் மைக்ரோஃபோனைப் பயன்படுத்துகிறது, ஏனெனில் ஐபோன்/ஐபாட் மைக்ரோஃபோன் உற்பத்தியாளர்-அளவீடு செய்யப்படுகிறது. டெஸ்க்டாப் கம்ப்யூட்டர் ஒலிபெருக்கி மூலம் உரத்த ஒலி இயங்கும்போது மட்டுமே மைக்ரோஃபோன் பதிவு செய்கிறது, மேலும் டெஸ்க்டாப் கணினி இரண்டும் முக்கியமாக "அமைதியாக தயவுசெய்து! இப்போது பதிவுசெய்கின்றன" என்று காட்டுகிறது. ஒலிபெருக்கி பண்புகளை மதிப்பிடுவதற்கு மட்டுமே பதிவு பயன்படுத்தப்படுகிறது, பின்னர் 30 வினாடிகளுக்குள் அழிக்கப்படுகிறது. ஒலிபெருக்கி பண்புகள் மட்டுமே தக்கவைக்கப்படுகின்றன.',
    ml: 'മൈക്രോഫോൺ സ്വകാര്യതയുടെ ഉറപ്പ്. നിങ്ങളുടെ ഡെസ്ക്ടോപ്പിന്റെ അന്തർനിർമ്മിത മൈക്രോഫോണിനേക്കാൾ നിങ്ങളുടെ ഐഫോൺ / ഐപാഡ് മൈക്രോഫോൺ ഉപയോഗിക്കുന്നു, കാരണം ഐഫോൺ / ഐപാഡ് മൈക്രോഫോൺ നിർമ്മാതാവ് കാലിബ്രേറ്റ എന്നാണ്. മൈക്രോഫോൺ റെക്കോർഡുകൾ ഡെസ്ക്ടോപ്പ് കമ്പ്യൂട്ടർ ഉച്ചഭാഷിക്കുന്നതിലൂടെ ഉച്ചത്തിലുള്ള ശബ്ദം, ഡെസ്ക്ടോപ്പ് കമ്പ്യൂട്ടറിലും "ശാന്തമായ ദയവായി! ഇപ്പോൾ റെക്കോർഡിംഗ്" ഉച്ചത്തിലുള്ള പ്രക്ഷോക്കർ സ്വഭാവസവിശേഷതകൾ കണക്കാക്കാനും 30 സെക്കൻഡിനുള്ളിൽ മായ്ക്കാനും റെക്കോർഡിംഗ് ഉപയോഗിക്കുന്നു. ഉച്ചഭാഷിണിയുടെ സവിശേഷതകൾ മാത്രമേ നിലനിർത്തുകയുള്ളൂ.',
    te: 'మైక్రోఫోన్ గోప్యత యొక్క హామీ. మీ డెస్క్‌టాప్ యొక్క అంతర్నిర్మిత మైక్రోఫోన్ కాకుండా ఈజీయెస్ మీ ఐఫోన్/ఐప్యాడ్ మైక్రోఫోన్‌ను ఉపయోగిస్తుంది, ఎందుకంటే ఐఫోన్/ఐప్యాడ్ మైక్రోఫోన్ తయారీదారు-క్రమాంకనం చేయబడింది. మైక్రోఫోన్ డెస్క్‌టాప్ కంప్యూటర్ లౌడ్‌స్పీకర్ ద్వారా బిగ్గరగా ధ్వని ఆడుతున్నప్పుడు మాత్రమే రికార్డ్ చేస్తుంది మరియు డెస్క్‌టాప్ కంప్యూటర్ రెండూ "నిశ్శబ్దమైన దయచేసి! ఇప్పుడు రికార్డింగ్" అని ప్రముఖంగా ప్రదర్శిస్తాయి. రికార్డింగ్ లౌడ్‌స్పీకర్ లక్షణాలను అంచనా వేయడానికి మాత్రమే ఉపయోగించబడుతుంది మరియు తరువాత 30 సెకన్లలో తొలగించబడుతుంది. లౌడ్‌స్పీకర్ లక్షణాలు మాత్రమే అలాగే ఉంచబడతాయి.',
    kn: 'ಮೈಕ್ರೊಫೋನ್ ಗೌಪ್ಯತೆಯ ಭರವಸೆ. ನಿಮ್ಮ ಡೆಸ್ಕ್‌ಟಾಪ್‌ನ ಅಂತರ್ನಿರ್ಮಿತ ಮೈಕ್ರೊಫೋನ್‌ಗಿಂತ ಈಸಿ ಐಗಳು ನಿಮ್ಮ ಐಫೋನ್/ಐಪ್ಯಾಡ್ ಮೈಕ್ರೊಫೋನ್ ಅನ್ನು ಬಳಸುತ್ತವೆ, ಏಕೆಂದರೆ ಐಫೋನ್/ಐಪ್ಯಾಡ್ ಮೈಕ್ರೊಫೋನ್ ತಯಾರಕ-ಮಾಪನಾಂಕಿತವಾಗಿದೆ. ಮೈಕ್ರೊಫೋನ್ ಡೆಸ್ಕ್‌ಟಾಪ್ ಕಂಪ್ಯೂಟರ್ ಧ್ವನಿವರ್ಧಕ ಮೂಲಕ ಜೋರಾಗಿ ಶಬ್ದವನ್ನು ಆಡುವಾಗ ಮಾತ್ರ ದಾಖಲಿಸುತ್ತದೆ, ಮತ್ತು ಡೆಸ್ಕ್‌ಟಾಪ್ ಕಂಪ್ಯೂಟರ್ ಎರಡೂ "ಸ್ತಬ್ಧ ದಯವಿಟ್ಟು! ಈಗ ರೆಕಾರ್ಡಿಂಗ್" ಅನ್ನು ಪ್ರಮುಖವಾಗಿ ಪ್ರದರ್ಶಿಸುತ್ತದೆ. ಧ್ವನಿವರ್ಧಕ ಗುಣಲಕ್ಷಣಗಳನ್ನು ಅಂದಾಜು ಮಾಡಲು ಮತ್ತು ನಂತರ 30 ಸೆಕೆಂಡುಗಳಲ್ಲಿ ಅಳಿಸಿಹಾಕಲು ರೆಕಾರ್ಡಿಂಗ್ ಅನ್ನು ಮಾತ್ರ ಬಳಸಲಾಗುತ್ತದೆ. ಧ್ವನಿವರ್ಧಕ ಗುಣಲಕ್ಷಣಗಳನ್ನು ಮಾತ್ರ ಉಳಿಸಿಕೊಳ್ಳಲಾಗುತ್ತದೆ.',
    bn: 'মাইক্রোফোন গোপনীয়তার নিশ্চয়তা। ইজিয়েয়েস আপনার ডেস্কটপের অন্তর্নির্মিত মাইক্রোফোনের পরিবর্তে আপনার আইফোন/আইপ্যাড মাইক্রোফোন ব্যবহার করে, কারণ আইফোন/আইপ্যাড মাইক্রোফোনটি প্রস্তুতকারক-ক্যালিব্রেটেড। মাইক্রোফোনটি কেবলমাত্র ডেস্কটপ কম্পিউটার লাউডস্পিকারের মাধ্যমে একটি জোরে শব্দ বাজানোর সময় রেকর্ড করে এবং ডেস্কটপ কম্পিউটার উভয়ই স্পষ্টতই "শান্ত দয়া করে! এখন রেকর্ডিং" প্রদর্শন করে। রেকর্ডিংটি সম্পূর্ণরূপে লাউডস্পিকারের বৈশিষ্ট্যগুলি অনুমান করার জন্য ব্যবহৃত হয় এবং তারপরে 30 সেকেন্ডের মধ্যে মুছে ফেলা হয়। শুধুমাত্র লাউডস্পিকারের বৈশিষ্ট্যগুলি ধরে রাখা হয়।',
    id: 'Jaminan privasi mikrofon. EyyEyes menggunakan mikrofon iPhone/iPad Anda, daripada mikrofon built-in desktop Anda, karena mikrofon iPhone/iPad dikalibrasi. Mikrofon hanya merekam saat suara keras diputar melalui pengeras suara komputer desktop, dan kedua komputer desktop menampilkan "sunyi! Sunyi! Sekarang merekam." Rekaman ini digunakan semata -mata untuk memperkirakan karakteristik pengeras suara dan kemudian dihapus, dalam waktu 30 detik. Hanya karakteristik pengeras suara yang dipertahankan.',
    "zh-CN":
      "保证麦克风隐私。 EasyEyes使用您的iPhone/iPad麦克风，而不是台式机的内置麦克风，因为iPhone/iPad麦克风是制造商校准的。麦克风仅记录在台式计算机扬声器中播放的声音时，台式计算机都显着地显示“请安静！现在录制”。该录音仅用于估计扬声器特征，然后在30秒内删除。仅保留扬声器特性。",
    "zh-HK":
      "保證麥克風隱私。 EasyEyes使用您的iPhone/iPad麥克風，而不是台式機的內置麥克風，因為iPhone/iPad麥克風是製造商校準的。麥克風僅記錄在台式計算機揚聲器中播放的聲音時，台式計算機都顯著地顯示“請安靜！現在錄製”。該錄音僅用於估計揚聲器特徵，然後在30秒內刪除。僅保留揚聲器特性。",
    ja: "マイクのプライバシーの保証。 Easyeyesは、iPhone/iPadマイクがメーカーキャリブレーションされているため、デスクトップの内蔵マイクではなく、iPhone/iPadマイクを使用します。マイクは、デスクトップコンピュータースピーカーを通じて大きな音が再生され、デスクトップコンピューターが「静かにお願いします！今録音」を顕著に表示している間にのみ記録されます。録音は、スピーカーの特性を推定するためだけに使用され、30秒以内に消去されます。スピーカーの特性のみが保持されます。",
    ko: '마이크 프라이버시의 보증. EasyEyes는 iPhone/iPad 마이크가 제조업체 캘리브레이션되기 때문에 데스크탑의 내장 마이크 대신 iPhone/iPad 마이크를 사용합니다. 마이크는 데스크탑 컴퓨터 라우드 스피커를 통해 큰 소리가 재생되는 동안에 만 기록되며, 데스크탑 컴퓨터는 "조용히 해주세요! 지금 녹음"을 표시합니다. 기록은 라우드 스피커 특성을 추정하는 데만 사용 된 다음 30 초 이내에 지워집니다. 라우드 스피커 특성 만 유지됩니다.',
  },
  RC_requestCamera: {
    "en-US": "To proceed, this study needs your permission to use the camera. ",
    de: "Um fortzufahren, erfordert diese Studie Ihre Erlaubnis, die Kamera zu verwenden.",
    fr: "Pour continuer, cette étude a besoin de votre autorisation pour utiliser la caméra.",
    es: "Para continuar, este estudio necesita su permiso para usar la cámara.",
    pt: "Para prosseguir, este estudo precisa de sua permissão para usar a câmera.",
    it: "Per procedere, questo studio richiede il permesso di utilizzare la fotocamera.",
    ro: "Pentru a continua, acest studiu are nevoie de permisiunea dvs. de a utiliza camera.",
    pl: "Aby kontynuować, to badanie wymaga zgody na korzystanie z aparatu.",
    ru: "Чтобы продолжить, это исследование требует вашего разрешения на использование камеры.",
    hy: "Շարունակելու համար այս ուսումնասիրությունը պետք է ձեր թույլտվությունը օգտագործելու տեսախցիկը:",
    fi: "Jatkamaan tämä tutkimus tarvitsee lupaasi käyttää kameraa.",
    ka: "გასაგრძელებლად, ამ კვლევას სჭირდება თქვენი ნებართვა კამერის გამოყენების შესახებ.",
    he: "כדי להמשיך, מחקר זה זקוק לרשותך להשתמש במצלמה.",
    ar: "للمتابعة ، تحتاج هذه الدراسة إلى إذنك لاستخدام الكاميرا.",
    ur: "آگے بڑھنے کے ل this ، اس مطالعے کو کیمرہ استعمال کرنے کے ل your آپ کی اجازت کی ضرورت ہے۔",
    hi: "आगे बढ़ने के लिए, इस अध्ययन को कैमरे का उपयोग करने के लिए आपकी अनुमति की आवश्यकता है।",
    ta: "தொடர, இந்த ஆய்வுக்கு கேமராவைப் பயன்படுத்த உங்கள் அனுமதி தேவை.",
    ml: "തുടരാൻ, ഈ പഠനത്തിന് ക്യാമറ ഉപയോഗിക്കാൻ നിങ്ങളുടെ അനുമതി ആവശ്യമാണ്.",
    te: "కొనసాగడానికి, ఈ అధ్యయనానికి కెమెరాను ఉపయోగించడానికి మీ అనుమతి అవసరం.",
    kn: "ಮುಂದುವರಿಯಲು, ಈ ಅಧ್ಯಯನಕ್ಕೆ ಕ್ಯಾಮೆರಾವನ್ನು ಬಳಸಲು ನಿಮ್ಮ ಅನುಮತಿ ಅಗತ್ಯವಿದೆ.",
    bn: "এগিয়ে যাওয়ার জন্য, এই অধ্যয়নের জন্য ক্যামেরাটি ব্যবহারের জন্য আপনার অনুমতি প্রয়োজন।",
    id: "Untuk melanjutkan, studi ini membutuhkan izin Anda untuk menggunakan kamera.",
    "zh-CN": "为了进行，本研究需要您使用相机的许可。",
    "zh-HK": "為了進行，本研究需要您使用相機的許可。",
    ja: "続行するには、この調査にはカメラを使用する許可が必要です。",
    ko: "진행하려면이 연구는 카메라 사용 권한이 필요합니다.",
  },
  RC_screenSize: {
    "en-US": "Screen Size",
    de: "Bildschirmgröße",
    fr: "Taille de l'écran",
    es: "Tamaño de pantalla",
    pt: "Tamanho da tela",
    it: "Dimensione dello schermo",
    ro: "Marimea ecranului",
    pl: "Rozmiar ekranu",
    ru: "Размер экрана",
    hy: "Էկրանի չափը",
    fi: "Näytön koko",
    ka: "Ეკრანის ზომა",
    he: "גודל מסך",
    ar: "حجم الشاشة",
    ur: "اسکرین سائز",
    hi: "स्क्रीन का साईज़",
    ta: "திரை அளவு",
    ml: "സ്ക്രീനിന്റെ വലിപ്പം",
    te: "తెర పరిమాణము",
    kn: "ತೆರೆಯಳತೆ",
    bn: "পর্দার আকার",
    id: "Ukuran layar",
    "zh-CN": "屏幕尺寸",
    "zh-HK": "屏幕尺寸",
    ja: "画面サイズ",
    ko: "화면 크기",
  },
  RC_screenSizeCredit: {
    "en-US":
      'Credit card suggested by the Li et al. (2020) "Virtual Chinrest" paper.',
    de: 'Kreditkarte von Li et al. (2020) "Virtual Chinentrest" -Papier.',
    fr: 'Carte de crédit suggérée par Li et al. (2020) Papier "Chinrest virtuel".',
    es: 'Tarjeta de crédito sugerida por Li et al. (2020) Papel "Chinrest virtual".',
    pt: 'Cartão de crédito sugerido por Li et al. (2020) Artigo "Virtual ChinRest".',
    it: 'Carta di credito suggerita da Li et al. (2020) carta "Virtual Chinrest".',
    ro: "Card de credit sugerat de Li și colab. (2020) Hârtie „virtuală Chinrest”.",
    pl: "Karta kredytowa sugerowana przez Li i in. (2020) Papier „Virtual Chinrest”.",
    ru: "Кредитная карта, предложенная Li et al. (2020) «Виртуальный подбородок».",
    hy: "Վարկային քարտ, որն առաջարկվում է Li et al- ի կողմից: (2020) «Վիրտուալ chinrest» թուղթ:",
    fi: 'Li et al. Ehdotti luottokortti. (2020) "Virtual Chinrest" -paperi.',
    ka: 'საკრედიტო ბარათი, რომელიც შემოთავაზებულია Li et al. (2020) "ვირტუალური Chinrest" ქაღალდი.',
    he: 'כרטיס אשראי שהוצע על ידי ה- Li et al. (2020) נייר "צ\'ינרסט וירטואלי".',
    ar: 'بطاقة الائتمان التي اقترحها Li et al. (2020) ورقة "chinrest الافتراضية".',
    ur: 'لی ایٹ ال کے ذریعہ کریڈٹ کارڈ تجویز کردہ۔ (2020) "ورچوئل چنیسٹ" کاغذ۔',
    hi: 'ली एट अल द्वारा सुझाए गए क्रेडिट कार्ड। (2020) "वर्चुअल चिनरेस्ट" पेपर।',
    ta: 'லி மற்றும் பலர் பரிந்துரைத்த கிரெடிட் கார்டு. (2020) "மெய்நிகர் சின்ரெஸ்ட்" காகிதம்.',
    ml: 'Li et at al നിർദ്ദേശിച്ച ക്രെഡിറ്റ് കാർഡ്. (2020) "വെർച്വൽ ചിൻറെസ്റ്റ്" പേപ്പർ.',
    te: 'క్రెడిట్ కార్డ్ లి మరియు ఇతరులు సూచించింది. (2020) "వర్చువల్ చిన్‌రెస్ట్" పేపర్.',
    kn: 'ಲಿ ಮತ್ತು ಇತರರು ಸೂಚಿಸಿದ ಕ್ರೆಡಿಟ್ ಕಾರ್ಡ್. (2020) "ವರ್ಚುವಲ್ ಚಿನ್‌ರೆಸ್ಟ್" ಕಾಗದ.',
    bn: 'লি এট আল দ্বারা প্রস্তাবিত ক্রেডিট কার্ড। (2020) "ভার্চুয়াল চিনেস্ট" কাগজ।',
    id: 'Kartu kredit yang disarankan oleh Li et al. (2020) kertas "virtual chinrest".',
    "zh-CN": "Li等人建议的信用卡。 （2020）“虚拟Chinrest”纸。",
    "zh-HK": "Li等人建議的信用卡。 （2020）“虛擬Chinrest”紙。",
    ja: "Li et alが提案したクレジットカード。 （2020）「仮想チンレスト」論文。",
    ko: 'Li et al. (2020) "Virtual Chinrest"용지.',
  },
  RC_screenSizeCreditCard: {
    "en-US": "a credit card",
    de: "eine Kreditkarte",
    fr: "Une carte de crédit",
    es: "una tarjeta de crédito",
    pt: "um cartão de crédito",
    it: "una carta di credito",
    ro: "un card de credit",
    pl: "karta kredytowa",
    ru: "кредитная карта",
    hy: "Վարկային քարտ",
    fi: "luottokortti",
    ka: "საკრედიტო ბარათი",
    he: "כרטיס אשראי",
    ar: "بطاقة ائتمان",
    ur: "ادھار کا کارڈ",
    hi: "एक क्रेडिट कार्ड",
    ta: "கடன் அட்டை",
    ml: "ഒരു ക്രെഡിറ്റ് കാർഡ്",
    te: "ఒక క్రెడిట్ కార్డు",
    kn: "ಕ್ರೆಡಿಟ್ ಕಾರ್ಡ್",
    bn: "একটি ক্রেডিট কার্ড",
    id: "sebuah kartu kredit",
    "zh-CN": "一张信用卡",
    "zh-HK": "一張信用卡",
    ja: "クレジットカード",
    ko: "신용 카드",
  },
  RC_screenSizeHave: {
    "en-US": "I have xxx with me.",
    de: "Ich habe xxx bei mir.",
    fr: "J'ai xxx avec moi.",
    es: "Tengo xxx conmigo.",
    pt: "Eu tenho xxx comigo.",
    it: "Ho xxx con me.",
    ro: "Am xxx cu mine.",
    pl: "Mam ze mną xxx.",
    ru: "У меня со мной ххх.",
    hy: "Ես ինձ հետ xxx ունեմ:",
    fi: "Minulla on xxx kanssani.",
    ka: "მე მაქვს xxx ჩემთან.",
    he: "יש לי xxx איתי.",
    ar: "لدي xxx معي.",
    ur: "میرے ساتھ xxx ہے۔",
    hi: "मेरे पास xxx है।",
    ta: "என்னிடம் xxx உள்ளது.",
    ml: "എനിക്ക് എന്നോടൊപ്പം xxx ഉണ്ട്.",
    te: "నాతో xxx ఉంది.",
    kn: "ನನ್ನೊಂದಿಗೆ xxx ಇದೆ.",
    bn: "আমার সাথে xxx আছে।",
    id: "Saya memiliki xxx dengan saya.",
    "zh-CN": "我和我在一起。",
    "zh-HK": "我和我在一起。",
    ja: "私はxxxを持っています。",
    ko: "나와 함께 xxx가 있습니다.",
  },
  RC_screenSizeIntro: {
    "en-US":
      "We will measure the size of this screen. Use the pull-down menu to choose something handy. You can use any common USB connector (including the one providing power to your computer) or a credit card (or any card of the same size). Place your object on the screen and move the slider to adjust the image to match your object's size. When they match, press RETURN (or click OK) to proceed.",
    de: "Wir werden die Größe dieses Bildschirms messen. Verwenden Sie das Pulldown-Menü, um etwas Handiges auszuwählen. Sie können jeden gängigen USB -Anschluss (einschließlich derjenigen verwenden, die Ihren Computer anbietet) oder einer Kreditkarte (oder einer beliebigen Karte derselben Größe). Platzieren Sie Ihr Objekt auf dem Bildschirm und bewegen Sie den Schieberegler, um das Bild an die Größe Ihres Objekts anzupassen. Wenn sie übereinstimmen, drücken Sie die Rückgabe (oder klicken Sie auf OK), um fortzufahren.",
    fr: "Nous mesurerons la taille de cet écran. Utilisez le menu déroulant pour choisir quelque chose de pratique. Vous pouvez utiliser n'importe quel connecteur USB commun (y compris celui qui fournit de l'énergie à votre ordinateur) ou une carte de crédit (ou toute carte de la même taille). Placez votre objet sur l'écran et déplacez le curseur pour ajuster l'image pour correspondre à la taille de votre objet. Lorsqu'ils correspondent, appuyez sur retour (ou cliquez sur OK) pour continuer.",
    es: "Mediremos el tamaño de esta pantalla. Use el menú desplegable para elegir algo útil. Puede usar cualquier conector USB común (incluido el que proporciona energía a su computadora) o una tarjeta de crédito (o cualquier tarjeta del mismo tamaño). Coloque su objeto en la pantalla y mueva el control deslizante para ajustar la imagen para que coincida con el tamaño de su objeto. Cuando coincidan, presione Return (o haga clic en Aceptar) para continuar.",
    pt: "Mediremos o tamanho desta tela. Use o menu suspenso para escolher algo útil. Você pode usar qualquer conector USB comum (incluindo o que fornece energia ao seu computador) ou um cartão de crédito (ou qualquer cartão do mesmo tamanho). Coloque seu objeto na tela e mova o controle deslizante para ajustar a imagem para corresponder ao tamanho do seu objeto. Quando eles corresponderem, pressione Return (ou clique em OK) para prosseguir.",
    it: "Misurare le dimensioni di questa schermata. Usa il menu a discesa per scegliere qualcosa di utile. È possibile utilizzare qualsiasi connettore USB comune (incluso quello che fornisce energia al tuo computer) o una carta di credito (o qualsiasi carta della stessa dimensione). Posiziona il tuo oggetto sullo schermo e sposta il cursore per regolare l'immagine in modo che corrisponda alle dimensioni del tuo oggetto. Quando corrispondono, premere return (o fare clic su OK) per procedere.",
    ro: "Vom măsura dimensiunea acestui ecran. Utilizați meniul derulant pentru a alege ceva la îndemână. Puteți utiliza orice conector USB comun (inclusiv cel care furnizează energie computerului dvs.) sau un card de credit (sau orice card de aceeași dimensiune). Plasați obiectul pe ecran și mutați glisorul pentru a regla imaginea pentru a se potrivi cu dimensiunea obiectului. Când se potrivesc, apăsați Return (sau faceți clic pe OK) pentru a continua.",
    pl: "Zmienimy rozmiar tego ekranu. Użyj menu rozwiniętego, aby wybrać coś przydatnego. Możesz użyć dowolnego wspólnego złącza USB (w tym jednego dostarczającego zasilania komputera) lub karty kredytowej (lub dowolnej karty tego samego rozmiaru). Umieść obiekt na ekranie i przesuń suwak, aby dostosować obraz, aby pasował do rozmiaru obiektu. Po dopasowaniu naciśnij Return (lub kliknij OK), aby kontynuować.",
    ru: "Мы измеряем размер этого экрана. Используйте выпадающее меню, чтобы выбрать что-то удобное. Вы можете использовать любой общий USB -разъем (включая тот, который обеспечивает питание для вашего компьютера) или кредитную карту (или любую карту того же размера). Поместите свой объект на экран и переместите слайдер, чтобы настроить изображение, чтобы соответствовать размеру вашего объекта. Когда они соответствуют, нажмите «Возврат» (или нажмите ОК), чтобы продолжить.",
    hy: "Մենք չափելու ենք այս էկրանի չափը: Օգտագործեք ներքեւի ընտրացանկը `ինչ-որ բան ընտրելու համար: Կարող եք օգտագործել ցանկացած ընդհանուր USB միակցիչ (ներառյալ ձեր համակարգչին ուժ ունեցող մեկ ապահովող) կամ վարկային քարտ (կամ նույն չափի ցանկացած քարտ): Տեղադրեք ձեր օբյեկտը էկրանին եւ տեղափոխեք սահնակը `պատկերը կարգավորելու համար` ձեր օբյեկտի չափը համապատասխանելու համար: Երբ նրանք համընկնում են, սեղմեք RETURT (կամ կտտացրեք OK), շարունակելու համար:",
    fi: "Mittaamme tämän näytön koon. Valitse jotain kätevää. Voit käyttää mitä tahansa yleistä USB -liitintä (mukaan lukien tietokoneellesi virran) tai luottokortti (tai mikä tahansa samankokoinen kortti). Aseta objekti näytölle ja siirrä liukusäädintä kuvan säätämiseksi objektin kokoa. Kun ne vastaavat, jatka paina (tai napsauta OK).",
    ka: "ჩვენ გავზომავთ ამ ეკრანის ზომას. გამოიყენეთ მენიუ, რომ აირჩიოთ რაიმე მოსახერხებელი. თქვენ შეგიძლიათ გამოიყენოთ ნებისმიერი საერთო USB კონექტორი (მათ შორის, რომელიც თქვენს კომპიუტერში ენერგიას უზრუნველყოფს) ან საკრედიტო ბარათით (ან იმავე ზომის ნებისმიერი ბარათი). განათავსეთ თქვენი ობიექტი ეკრანზე და გადაიტანეთ სლაიდერი, რომ შეცვალოთ სურათი, რომ შეესაბამებოდეს თქვენი ობიექტის ზომას. როდესაც ისინი ემთხვევა, დააჭირეთ დაბრუნებას (ან დააჭირეთ ღილაკს OK).",
    he: "נמדוד את גודל המסך הזה. השתמש בתפריט הנפתח כדי לבחור משהו שימושי. אתה יכול להשתמש בכל מחבר USB נפוץ (כולל זה המספק כוח למחשב שלך) או בכרטיס אשראי (או בכל כרטיס באותו גודל). הנח את האובייקט שלך על המסך והזז את המחוון כדי להתאים את התמונה כך שתתאים לגודל האובייקט שלך. כאשר הם תואמים, לחץ על החזרה (או לחץ על אישור) כדי להמשיך.",
    ar: "سنقوم بقياس حجم هذه الشاشة. استخدم القائمة المنسدلة لاختيار شيء مفيد. يمكنك استخدام أي موصل USB شائع (بما في ذلك الموصل الذي يوفر الطاقة لجهاز الكمبيوتر الخاص بك) أو بطاقة ائتمان (أو أي بطاقة بنفس الحجم). ضع كائنك على الشاشة وحرك شريط التمرير لضبط الصورة لمطابقة حجم كائنك. عند مطابقة ، اضغط على Return (أو انقر فوق موافق) للمتابعة.",
    ur: "ہم اس اسکرین کے سائز کی پیمائش کریں گے۔ آسان چیز کا انتخاب کرنے کے لئے پل ڈاؤن مینو کا استعمال کریں۔ آپ کسی بھی عام USB کنیکٹر (بشمول اپنے کمپیوٹر کو بجلی فراہم کرنے والی) یا کریڈٹ کارڈ (یا ایک ہی سائز کا کوئی کارڈ) استعمال کرسکتے ہیں۔ اپنے آبجیکٹ کو اسکرین پر رکھیں اور اپنے آبجیکٹ کے سائز سے ملنے کے لئے تصویر کو ایڈجسٹ کرنے کے لئے سلائیڈر کو منتقل کریں۔ جب وہ میچ کرتے ہیں تو ، آگے بڑھنے کے لئے ریٹرن دبائیں (یا ٹھیک ہے پر کلک کریں)۔",
    hi: "हम इस स्क्रीन के आकार को मापेंगे। कुछ आसान चुनने के लिए पुल-डाउन मेनू का उपयोग करें। आप किसी भी सामान्य USB कनेक्टर (आपके कंप्यूटर को पावर प्रदान करने वाले) या क्रेडिट कार्ड (या उसी आकार के किसी भी कार्ड) का उपयोग कर सकते हैं। अपनी ऑब्जेक्ट को स्क्रीन पर रखें और अपनी ऑब्जेक्ट के आकार से मेल खाने के लिए छवि को समायोजित करने के लिए स्लाइडर को स्थानांतरित करें। जब वे मैच करते हैं, तो आगे बढ़ने के लिए रिटर्न (या ओके पर क्लिक करें) दबाएं।",
    ta: "இந்த திரையின் அளவை அளவிடுவோம். எளிமையான ஒன்றைத் தேர்வுசெய்ய புல்-டவுன் மெனுவைப் பயன்படுத்தவும். நீங்கள் பொதுவான யூ.எஸ்.பி இணைப்பான் (உங்கள் கணினிக்கு மின்சாரம் வழங்கும் ஒன்று உட்பட) அல்லது கிரெடிட் கார்டு (அல்லது அதே அளவிலான எந்த அட்டையும்) பயன்படுத்தலாம். உங்கள் பொருளை திரையில் வைக்கவும், உங்கள் பொருளின் அளவைப் பொருத்த படத்தை சரிசெய்ய ஸ்லைடரை நகர்த்தவும். அவை பொருந்தும்போது, ​​தொடர திரும்பவும் (சரி என்பதைக் கிளிக் செய்யவும்) அழுத்தவும்.",
    ml: "ഈ സ്ക്രീനിന്റെ വലുപ്പം ഞങ്ങൾ അളക്കും. ഹാൻഡി തിരഞ്ഞെടുക്കാൻ പുൾ-ഡ menu ൺ മെനു ഉപയോഗിക്കുക. നിങ്ങൾക്ക് ഏതെങ്കിലും സാധാരണ യുഎസ്ബി കണക്റ്റർ (നിങ്ങളുടെ കമ്പ്യൂട്ടറിന് നൽകുന്ന ഒന്ന് ഉൾപ്പെടെ) അല്ലെങ്കിൽ ഒരു ക്രെഡിറ്റ് കാർഡ് (അല്ലെങ്കിൽ ഒരേ വലുപ്പത്തിലുള്ള ഏതെങ്കിലും കാർഡ്) ഉപയോഗിക്കാം. നിങ്ങളുടെ ഒബ്ജക്റ്റ് സ്ക്രീനിൽ വയ്ക്കുക നിങ്ങളുടെ ഒബ്ജക്റ്റിന്റെ വലുപ്പവുമായി പൊരുത്തപ്പെടുന്നതിന് ഇമേജ് ക്രമീകരിക്കുന്നതിന് സ്ലൈഡർ നീക്കുക. തുടരുമ്പോൾ, തുടരുന്നതിന് റിട്ടേൺ അമർത്തുക (അല്ലെങ്കിൽ ശരി ക്ലിക്കുചെയ്യുക).",
    te: "మేము ఈ స్క్రీన్ పరిమాణాన్ని కొలుస్తాము. సులభమైనదాన్ని ఎంచుకోవడానికి పుల్-డౌన్ మెనుని ఉపయోగించండి. మీరు ఏదైనా సాధారణ USB కనెక్టర్‌ను (మీ కంప్యూటర్‌కు శక్తితో అందించే వాటితో సహా) లేదా క్రెడిట్ కార్డ్ (లేదా ఒకే పరిమాణంలోని ఏదైనా కార్డు) ఉపయోగించవచ్చు. మీ వస్తువు యొక్క పరిమాణానికి సరిపోయేలా మీ వస్తువును తెరపై ఉంచండి మరియు చిత్రాన్ని సర్దుబాటు చేయడానికి స్లైడర్‌ను తరలించండి. అవి సరిపోలినప్పుడు, కొనసాగడానికి రిటర్న్ నొక్కండి (లేదా సరే క్లిక్ చేయండి).",
    kn: "ನಾವು ಈ ಪರದೆಯ ಗಾತ್ರವನ್ನು ಅಳೆಯುತ್ತೇವೆ. ಸೂಕ್ತವಾದದ್ದನ್ನು ಆಯ್ಕೆ ಮಾಡಲು ಪುಲ್-ಡೌನ್ ಮೆನು ಬಳಸಿ. ನೀವು ಯಾವುದೇ ಸಾಮಾನ್ಯ ಯುಎಸ್‌ಬಿ ಕನೆಕ್ಟರ್ (ನಿಮ್ಮ ಕಂಪ್ಯೂಟರ್‌ಗೆ ಶಕ್ತಿಯನ್ನು ಒದಗಿಸುವ ಒಂದು ಸೇರಿದಂತೆ) ಅಥವಾ ಕ್ರೆಡಿಟ್ ಕಾರ್ಡ್ (ಅಥವಾ ಒಂದೇ ಗಾತ್ರದ ಯಾವುದೇ ಕಾರ್ಡ್) ಅನ್ನು ಬಳಸಬಹುದು. ನಿಮ್ಮ ವಸ್ತುವಿನ ಗಾತ್ರವನ್ನು ಹೊಂದಿಸಲು ಚಿತ್ರವನ್ನು ಹೊಂದಿಸಲು ನಿಮ್ಮ ವಸ್ತುವನ್ನು ಪರದೆಯ ಮೇಲೆ ಇರಿಸಿ ಮತ್ತು ಸ್ಲೈಡರ್ ಅನ್ನು ಸರಿಸಿ. ಅವರು ಹೊಂದಿಕೆಯಾದಾಗ, ಮುಂದುವರಿಯಲು ರಿಟರ್ನ್ ಒತ್ತಿರಿ (ಅಥವಾ ಸರಿ ಕ್ಲಿಕ್ ಮಾಡಿ).",
    bn: "আমরা এই পর্দার আকার পরিমাপ করব। সহজ কিছু চয়ন করতে পুল-ডাউন মেনু ব্যবহার করুন। আপনি যে কোনও সাধারণ ইউএসবি সংযোগকারী (আপনার কম্পিউটারে পাওয়ার সরবরাহকারী একটি সহ) বা ক্রেডিট কার্ড (বা একই আকারের কোনও কার্ড) ব্যবহার করতে পারেন। আপনার অবজেক্টটি স্ক্রিনে রাখুন এবং আপনার বস্তুর আকারের সাথে মেলে চিত্রটি সামঞ্জস্য করতে স্লাইডারটি সরান। যখন তারা মেলে, এগিয়ে যেতে রিটার্ন (বা ওকে ক্লিক করুন) টিপুন।",
    id: "Kami akan mengukur ukuran layar ini. Gunakan menu pull-down untuk memilih sesuatu yang berguna. Anda dapat menggunakan konektor USB umum apa pun (termasuk yang memberikan daya ke komputer Anda) atau kartu kredit (atau kartu apa pun dengan ukuran yang sama). Tempatkan objek Anda di layar dan pindahkan slider untuk menyesuaikan gambar agar sesuai dengan ukuran objek Anda. Saat mereka cocok, tekan kembali (atau klik OK) untuk melanjutkan.",
    "zh-CN":
      "我们将测量此屏幕的大小。使用下拉菜单选择方便的东西。您可以使用任何通用的USB连接器（包括为计算机提供电源的连接器）或信用卡（或任何相同尺寸的卡）。将对象放在屏幕上，然后移动滑块以调整图像以匹配对象的大小。匹配时，按返回（或单击“确定”）继续进行。",
    "zh-HK":
      "我們將測量此屏幕的大小。使用下拉菜單選擇方便的東西。您可以使用任何通用的USB連接器（包括為計算機提供電源的連接器）或信用卡（或任何相同尺寸的卡）。將對象放在屏幕上，然後移動滑塊以調整圖像以匹配對象的大小。匹配時，按返回（或單擊“確定”）繼續進行。",
    ja: "この画面のサイズを測定します。プルダウンメニューを使用して、便利なものを選択します。一般的なUSBコネクタ（コンピューターに電力を提供するものを含む）またはクレジットカード（または同じサイズのカード）を使用できます。オブジェクトを画面に配置し、スライダーを移動して、オブジェクトのサイズに合わせて画像を調整します。一致したら、return（または[OK]をクリックして）を押して続行します。",
    ko: "이 화면의 크기를 측정합니다. 풀다운 메뉴를 사용하여 편리한 것을 선택하십시오. 공통 USB 커넥터 (컴퓨터에 전원을 공급하는 것 포함) 또는 신용 카드 (또는 동일한 크기의 카드)를 사용할 수 있습니다. 물체를 화면에 놓고 슬라이더를 이동하여 객체의 크기에 맞게 이미지를 조정하십시오. 그들이 일치하면 반환 (또는 확인을 클릭)을 눌러 진행하십시오.",
  },
  RC_screenSizeTitle: {
    "en-US": "Screen Size Calibration",
    de: "Bildschirmgröße Kalibrierung",
    fr: "Étalonnage de la taille de l'écran",
    es: "Calibración del tamaño de la pantalla",
    pt: "Calibração do tamanho da tela",
    it: "Calibrazione delle dimensioni dello schermo",
    ro: "Calibrarea dimensiunii ecranului",
    pl: "Kalibracja rozmiaru ekranu",
    ru: "Калибровка размера экрана",
    hy: "Էկրանի չափի տրամաչափում",
    fi: "Näytön koon kalibrointi",
    ka: "ეკრანის ზომის კალიბრაცია",
    he: "כיול בגודל המסך",
    ar: "معايرة حجم الشاشة",
    ur: "اسکرین سائز انشانکن",
    hi: "स्क्रीन आकार अंशांकन",
    ta: "திரை அளவு அளவுத்திருத்தம்",
    ml: "സ്ക്രീൻ വലുപ്പം കാലിബ്രേഷൻ",
    te: "స్క్రీన్ సైజు క్రమాంకనం",
    kn: "ಪರದೆಯ ಗಾತ್ರ ಮಾಪನಾಂಕ ನಿರ್ಣಯ",
    bn: "স্ক্রিন আকারের ক্রমাঙ্কন",
    id: "Kalibrasi ukuran layar",
    "zh-CN": "屏幕尺寸校准",
    "zh-HK": "屏幕尺寸校準",
    ja: "画面サイズのキャリブレーション",
    ko: "화면 크기 교정",
  },
  RC_screenSizeUSBA: {
    "en-US": "a USB Type A connector",
    de: "Ein USB -Typ A Connector",
    fr: "Un connecteur USB Type A",
    es: "un conector USB tipo A",
    pt: "Um conector USB tipo A",
    it: "Un connettore di tipo A USB",
    ro: "Un conector USB de tip A",
    pl: "Złącze USB typu A",
    ru: "Разъем типа USB A",
    hy: "USB միակցիչ",
    fi: "USB -tyypin A -liitin",
    ka: "USB ტიპის A კონექტორი",
    he: "מחבר מסוג USB מסוג A",
    ar: "موصل USB من النوع A",
    ur: "USB ٹائپ ایک کنیکٹر",
    hi: "यूएसबी टाइप ए कनेक्टर",
    ta: "ஒரு யூ.எஸ்.பி வகை ஒரு இணைப்பு",
    ml: "ഒരു യുഎസ്ബി തരം ഒരു കണക്റ്റർ",
    te: "USB టైప్ ఎ కనెక్టర్",
    kn: "ಯುಎಸ್ಬಿ ಟೈಪ್ ಎ ಕನೆಕ್ಟರ್",
    bn: "একটি ইউএসবি টাইপ এ সংযোগকারী",
    id: "Konektor Tipe A USB",
    "zh-CN": "USB A 型接口",
    "zh-HK": "USB A 型接口",
    ja: "USB コネクタ　タイプA",
    ko: "USB 유형 A 커넥터",
  },
  RC_screenSizeUSBC: {
    "en-US": "a USB Type C connector",
    de: "Ein USB -Typ -C -Anschluss",
    fr: "Un connecteur USB de type C",
    es: "un conector USB tipo C",
    pt: "Um conector USB tipo C",
    it: "Un connettore di tipo C USB",
    ro: "Un conector USB de tip C",
    pl: "Złącze USB typu C",
    ru: "Разъем типа USB C",
    hy: "USB տիպի միակցիչ գ",
    fi: "USB -tyypin C -liitin",
    ka: "USB ტიპის C კონექტორი",
    he: "מחבר USB מסוג C",
    ar: "موصل USB من النوع C",
    ur: "USB ٹائپ سی کنیکٹر",
    hi: "यूएसबी टाइप सी कनेक्टर",
    ta: "ஒரு யூ.எஸ்.பி வகை சி இணைப்பு",
    ml: "ഒരു യുഎസ്ബി തരം സി കണക്റ്റർ",
    te: "USB రకం సి కనెక్టర్",
    kn: "ಯುಎಸ್ಬಿ ಟೈಪ್ ಸಿ ಕನೆಕ್ಟರ್",
    bn: "একটি ইউএসবি টাইপ সি সংযোগকারী",
    id: "Konektor USB Tipe C",
    "zh-CN": "USB C 型接口",
    "zh-HK": "USB C 型接口",
    ja: "USB コネクタ　タイプC",
    ko: "USB 유형 C 커넥터",
  },
  RC_soundCalibration: {
    "en-US": "Sound Calibration",
    de: "Schallkalibrierung",
    fr: "Étalonnage",
    es: "Calibración de sonido",
    pt: "Calibração do som",
    it: "Calibrazione del suono",
    ro: "Calibrare sunet",
    pl: "Kalibracja dźwięku",
    ru: "Калибровка звука",
    hy: "Ձայնային տրամաչափում",
    fi: "Äänen kalibrointi",
    ka: "ხმის კალიბრაცია",
    he: "כיול קול",
    ar: "معايرة الصوت",
    ur: "صوتی انشانکن",
    hi: "ध्वनि अंशांकन",
    ta: "ஒலி அளவுத்திருத்தம்",
    ml: "ശബ്ദ കാലിബ്രേഷൻ",
    te: "ధ్వని క్రమాంకనం",
    kn: "ಶಬ್ದ ಮಾಪನಾಂಕ ನಿರ್ಣಯ",
    bn: "শব্দ ক্রমাঙ্কন",
    id: "Kalibrasi suara",
    "zh-CN": "声音校准",
    "zh-HK": "聲音校準",
    ja: "サウンドキャリブレーション",
    ko: "음향 교정",
  },
  RC_soundCalibrationClickToStart: {
    "en-US": "Click Ok on your iPhone/iPad, and follow the instructions there.",
    de: "Klicken Sie auf OK auf Ihrem iPhone/iPad und befolgen Sie die Anweisungen dort.",
    fr: "Cliquez sur OK sur votre iPhone / iPad et suivez les instructions là-bas.",
    es: "Haga clic en Aceptar en su iPhone/iPad y siga las instrucciones allí.",
    pt: "Clique em OK no seu iPhone/iPad e siga as instruções lá.",
    it: "Fai clic su OK sul tuo iPhone/iPad e segui le istruzioni lì.",
    ro: "Faceți clic pe OK pe iPhone/iPad și urmați instrucțiunile de acolo.",
    pl: "Kliknij OK na iPhonie/iPadzie i postępuj zgodnie z instrukcjami.",
    ru: "Нажмите ОК на своем iPhone/iPad и следуйте инструкциям там.",
    hy: "Կտտացրեք OK ձեր iPhone / iPad- ում եւ հետեւեք այնտեղ հրահանգներին:",
    fi: "Napsauta OK iPhonessa/iPadissa ja noudata siellä olevia ohjeita.",
    ka: "დააჭირეთ OK თქვენს iPhone/iPad- ზე და მიჰყევით იქ მითითებებს.",
    he: "לחץ על אישור ב- iPhone/iPad שלך, ועקוב אחר ההוראות שם.",
    ar: 'انقر فوق "موافق" على جهاز iPhone/iPad ، واتبع التعليمات هناك.',
    ur: "اپنے آئی فون/آئی پیڈ پر اوکے پر کلک کریں ، اور وہاں کی ہدایات پر عمل کریں۔",
    hi: "अपने iPhone/iPad पर ठीक क्लिक करें, और वहां के निर्देशों का पालन करें।",
    ta: "உங்கள் ஐபோன்/ஐபாடில் சரி என்பதைக் கிளிக் செய்து, அங்குள்ள வழிமுறைகளைப் பின்பற்றவும்.",
    ml: "നിങ്ങളുടെ iPhone / iPad- ൽ ശരി ക്ലിക്കുചെയ്യുക, അവിടെയുള്ള നിർദ്ദേശങ്ങൾ പാലിക്കുക.",
    te: "మీ ఐఫోన్/ఐప్యాడ్‌లో సరే క్లిక్ చేసి, అక్కడి సూచనలను అనుసరించండి.",
    kn: "ನಿಮ್ಮ ಐಫೋನ್/ಐಪ್ಯಾಡ್ ಮೇಲೆ ಸರಿ ಕ್ಲಿಕ್ ಮಾಡಿ ಮತ್ತು ಅಲ್ಲಿನ ಸೂಚನೆಗಳನ್ನು ಅನುಸರಿಸಿ.",
    bn: "আপনার আইফোন/আইপ্যাডে ওকে ক্লিক করুন এবং সেখানে নির্দেশাবলী অনুসরণ করুন।",
    id: "Klik OK di iPhone/iPad Anda, dan ikuti instruksi di sana.",
    "zh-CN": "单击iPhone/iPad上的确定，然后按照那里的说明进行操作。",
    "zh-HK": "單擊iPhone/iPad上的確定，然後按照那裡的說明進行操作。",
    ja: "iPhone/iPadの[OK]をクリックし、そこの指示に従ってください。",
    ko: "iPhone/iPad에서 확인을 클릭하고 지침을 따르십시오.",
  },
  RC_soundCalibrationDone: {
    "en-US":
      "Thanks. The calibration is done, and you can put away the iPhone/iPad. IMPORTANT: DO NOT TOUCH THE VOLUME CONTROL until the experiment is done. Changing the volume will defeat the calibration and invalidate all the results. ",
    de: "Danke. Die Kalibrierung erfolgt und Sie können das iPhone/iPad weglegen. WICHTIG: Berühren Sie die Volumenkontrolle erst, wenn das Experiment durchgeführt wird. Durch das Ändern des Volumens wird die Kalibrierung besiegt und alle Ergebnisse ungültig.",
    fr: "Merci. L'étalonnage est fait et vous pouvez ranger l'iPhone / iPad. IMPORTANT: Ne touchez pas le contrôle du volume tant que l'expérience est terminée. La modification du volume vaincra l'étalonnage et invalidera tous les résultats.",
    es: "Gracias. La calibración está lista y puede guardar el iPhone/iPad. IMPORTANTE: No toque el control de volumen hasta que se realice el experimento. Cambiar el volumen derrotará la calibración e invalidará todos los resultados.",
    pt: "Obrigado. A calibração está concluída e você pode guardar o iPhone/iPad. IMPORTANTE: Não toque no controle de volume até que o experimento seja feito. Alterar o volume derrotará a calibração e invalidará todos os resultados.",
    it: "Grazie. La calibrazione è eseguita e puoi mettere via l'iPhone/iPad. IMPORTANTE: non toccare il controllo del volume fino a quando l'esperimento non è terminato. La modifica del volume sconfiggerà la calibrazione e invaliderà tutti i risultati.",
    ro: "Mulțumiri. Calibrarea se face și puteți elimina iPhone/iPad. IMPORTANT: Nu atingeți controlul volumului până la finalizarea experimentului. Modificarea volumului va învinge calibrarea și va invalida toate rezultatele.",
    pl: "Dzięki. Kalibracja jest zakończona i możesz odłożyć iPhone'a/iPada. Ważne: nie dotykaj kontroli głośności, dopóki eksperyment nie zostanie zakończony. Zmiana objętości pokonuje kalibrację i unieważni wszystkie wyniki.",
    ru: "Спасибо. Калибровка сделана, и вы можете убрать iPhone/iPad. Важно: не касайтесь регулятора громкости, пока эксперимент не будет проведен. Изменение тома победит калибровку и аннулирует все результаты.",
    hy: "Շնորհակալություն Կալիբրացումը կատարվում է, եւ դուք կարող եք տեղադրել iPhone / iPad- ը: ԿԱՐԵՎՈՐ. Մի դիպչեք ձայնի վերահսկմանը, մինչեւ փորձը կատարվի: Ծավալը փոխելը կխթանի տրամաչափումը եւ անվավեր ճանաչի բոլոր արդյունքները:",
    fi: "Kiitos. Kalibrointi on tehty, ja voit laittaa iPhonen/iPadin pois. TÄRKEÄÄ: Älä koske äänenvoimakkuuden hallintaa ennen kuin koe on tehty. Tängomentin muuttaminen voittaa kalibroinnin ja mitätöidä kaikki tulokset.",
    ka: "მადლობა. კალიბრაცია კეთდება და შეგიძლიათ iPhone/iPad ჩამოაგდოთ. მნიშვნელოვანია: არ შეეხოთ მოცულობის კონტროლს, სანამ ექსპერიმენტი არ მოხდება. მოცულობის შეცვლა დაამარცხებს კალიბრაციას და გააუქმებს ყველა შედეგს.",
    he: "תודה. הכיול נעשה, ותוכלו להרחיק את האייפון/אייפד. חשוב: אל תיגע בבקרת עוצמת הקול עד לסיום הניסוי. שינוי הנפח יביס את הכיול ויבטל את כל התוצאות.",
    ar: "شكرًا. تتم المعايرة ، ويمكنك وضع iPhone/iPad بعيدًا. هام: لا تلمس التحكم في مستوى الصوت حتى تتم التجربة. سيؤدي تغيير مستوى الصوت إلى هزيمة المعايرة وإبطال جميع النتائج.",
    ur: "شکریہ. انشانکن ہو گیا ہے ، اور آپ آئی فون/آئی پیڈ کو دور کرسکتے ہیں۔ اہم: جب تک تجربہ نہ ہو اس وقت تک حجم کے کنٹرول کو مت لگائیں۔ حجم کو تبدیل کرنے سے انشانکن کو شکست دی جائے گی اور تمام نتائج کو باطل کردیا جائے گا۔",
    hi: "धन्यवाद। अंशांकन किया जाता है, और आप iPhone/iPad को दूर कर सकते हैं। महत्वपूर्ण: प्रयोग होने तक वॉल्यूम नियंत्रण को न छूएं। वॉल्यूम बदलने से अंशांकन को हरा देगा और सभी परिणामों को अमान्य कर देगा।",
    ta: "நன்றி. அளவுத்திருத்தம் செய்யப்படுகிறது, மேலும் நீங்கள் ஐபோன்/ஐபாட் ஒதுக்கி வைக்கலாம். முக்கியமானது: சோதனை செய்யப்படும் வரை தொகுதி கட்டுப்பாட்டைத் தொட வேண்டாம். அளவை மாற்றுவது அளவுத்திருத்தத்தைத் தோற்கடித்து அனைத்து முடிவுகளையும் செல்லாது.",
    ml: "നന്ദി. കാലിബ്രേഷൻ ചെയ്തു, നിങ്ങൾക്ക് ഐഫോൺ / ഐപാഡ് മാറ്റിസ്ഥാപിക്കാം. പ്രധാനം: പരീക്ഷണം നടക്കുന്നതുവരെ വോളിയം നിയന്ത്രണത്തിൽ തൊടരുത്. വോളിയം മാറ്റുന്നത് കാലിബ്രേഷനെ പരാജയപ്പെടുത്തുകയും എല്ലാ ഫലങ്ങളും അസാധുവാക്കുകയും ചെയ്യും.",
    te: "ధన్యవాదాలు. క్రమాంకనం పూర్తయింది మరియు మీరు ఐఫోన్/ఐప్యాడ్‌ను దూరంగా ఉంచవచ్చు. ముఖ్యమైనది: ప్రయోగం పూర్తయ్యే వరకు వాల్యూమ్ నియంత్రణను తాకవద్దు. వాల్యూమ్‌ను మార్చడం క్రమాంకనాన్ని ఓడిస్తుంది మరియు అన్ని ఫలితాలను చెల్లదు.",
    kn: "ಧನ್ಯವಾದಗಳು. ಮಾಪನಾಂಕ ನಿರ್ಣಯವನ್ನು ಮಾಡಲಾಗುತ್ತದೆ, ಮತ್ತು ನೀವು ಐಫೋನ್/ಐಪ್ಯಾಡ್ ಅನ್ನು ದೂರವಿಡಬಹುದು. ಪ್ರಮುಖ: ಪ್ರಯೋಗವನ್ನು ಮಾಡುವವರೆಗೆ ಪರಿಮಾಣ ನಿಯಂತ್ರಣವನ್ನು ಮುಟ್ಟಬೇಡಿ. ಪರಿಮಾಣವನ್ನು ಬದಲಾಯಿಸುವುದರಿಂದ ಮಾಪನಾಂಕ ನಿರ್ಣಯವನ್ನು ಸೋಲಿಸುತ್ತದೆ ಮತ್ತು ಎಲ್ಲಾ ಫಲಿತಾಂಶಗಳನ್ನು ಅಮಾನ್ಯಗೊಳಿಸುತ್ತದೆ.",
    bn: "ধন্যবাদ। ক্রমাঙ্কনটি সম্পন্ন হয়েছে এবং আপনি আইফোন/আইপ্যাডটি ফেলে দিতে পারেন। গুরুত্বপূর্ণ: পরীক্ষাটি না হওয়া পর্যন্ত ভলিউম নিয়ন্ত্রণটি স্পর্শ করবেন না। ভলিউম পরিবর্তন করা ক্রমাঙ্কনকে পরাস্ত করবে এবং সমস্ত ফলাফলকে অকার্যকর করবে।",
    id: "Terima kasih. Kalibrasi selesai, dan Anda dapat menyingkirkan iPhone/iPad. Penting: Jangan menyentuh kontrol volume sampai percobaan selesai. Mengubah volume akan mengalahkan kalibrasi dan membatalkan semua hasil.",
    "zh-CN":
      "谢谢。校准已经完成，您可以收起iPhone/iPad。重要的是：在实验完成之前，请勿触摸音量控制。更改音量将破坏校准并使所有结果无效。",
    "zh-HK":
      "謝謝。校準已經完成，您可以收起iPhone/iPad。重要的是：在實驗完成之前，請勿觸摸音量控制。更改音量將破壞校準並使所有結果無效。",
    ja: "ありがとう。キャリブレーションが行われ、iPhone/iPadを片付けることができます。重要：実験が完了するまで、ボリュームコントロールに触れないでください。ボリュームを変更すると、キャリブレーションが倒され、すべての結果が無効になります。",
    ko: "감사해요. 교정이 완료되고 iPhone/iPad를 제거 할 수 있습니다. 중요 : 실험이 완료 될 때까지 볼륨 컨트롤을 건드리지 마십시오. 볼륨을 변경하면 교정이 무효화되고 모든 결과가 무효화됩니다.",
  },
  RC_soundCalibrationHoldIPhoneOk: {
    "en-US": "Press Ok to continue. \n\n",
    de: "Drücken Sie OK, um fortzufahren.\n\n",
    fr: "Appuyez sur OK pour continuer.\n\n",
    es: "Presione OK para continuar.\n\n",
    pt: "Pressione OK para continuar.\n\n",
    it: "Premi OK per continuare.\n\n",
    ro: "Apăsați OK pentru a continua.\n\n",
    pl: "Naciśnij OK, aby kontynuować.\n\n",
    ru: "Нажмите OK, чтобы продолжить.\n\n",
    hy: "Շարունակելու համար սեղմեք OK:\n\n",
    fi: "Jatka painamalla OK.\n\n",
    ka: "დააჭირეთ ღილაკს, რომ გააგრძელოთ.\n\n",
    he: "לחץ על אישור כדי להמשיך.\n\n",
    ar: "اضغط موافق للمتابعة.\n\n",
    ur: "جاری رکھنے کے لئے ٹھیک دبائیں۔\n\n",
    hi: "जारी रखने के लिए ठीक दबाएं।\n\n",
    ta: "தொடர சரி என்பதை அழுத்தவும்.\n\n",
    ml: "തുടരാൻ ശരി അമർത്തുക.\n\n",
    te: "కొనసాగించడానికి సరే నొక్కండి.\n\n",
    kn: "ಮುಂದುವರಿಸಲು ಸರಿ ಒತ್ತಿರಿ.\n\n",
    bn: "চালিয়ে যেতে ওকে টিপুন।\n\n",
    id: "Tekan OK untuk melanjutkan.\n\n",
    "zh-CN": "按确定继续。\n\n",
    "zh-HK": "按確定繼續。\n\n",
    ja: "OKを押して続行します。\n\n",
    ko: "계속하려면 확인을 누릅니다.\n\n",
  },
  RC_soundCalibrationNeedsIPhone: {
    "en-US":
      "To produce accurate sound, the experiment requires calibration of this computer's loudspeakers. This requires an iPhone or iPad that's connected to the internet. Do you have one handy? \n",
    de: "Um einen genauen Klang zu erzeugen, erfordert das Experiment die Kalibrierung der Lautsprecher dieses Computers. Dies erfordert ein iPhone oder iPad, das mit dem Internet verbunden ist. Hast du eine Handy?\n",
    fr: "Pour produire un son précis, l'expérience nécessite l'étalonnage des haut-parleurs de cet ordinateur. Cela nécessite un iPhone ou un iPad connecté à Internet. Avez-vous un à portée de main?\n",
    es: "Para producir un sonido preciso, el experimento requiere la calibración de los altavoces de esta computadora. Esto requiere un iPhone o iPad que esté conectado a Internet. ¿Tienes uno a mano?\n",
    pt: "Para produzir som preciso, o experimento requer a calibração dos alto -falantes deste computador. Isso requer um iPhone ou iPad conectado à Internet. Você tem um útil?\n",
    it: "Per produrre un suono accurato, l'esperimento richiede la calibrazione degli altoparlanti di questo computer. Ciò richiede un iPhone o un iPad connesso a Internet. Hai uno a portata di mano?\n",
    ro: "Pentru a produce un sunet precis, experimentul necesită calibrarea difuzoarelor acestui computer. Acest lucru necesită un iPhone sau un iPad conectat la internet. Ai unul la îndemână?\n",
    pl: "Aby uzyskać dokładny dźwięk, eksperyment wymaga kalibracji głośników tego komputera. Wymaga to iPhone'a lub iPada, które są podłączone do Internetu. Czy masz jeden przydatny?\n",
    ru: "Чтобы получить точный звук, эксперимент требует калибровки громкоговорителей этого компьютера. Это требует iPhone или iPad, который подключен к Интернету. У тебя есть одна удобная?\n",
    hy: "Acc շգրիտ ձայն տալու համար փորձը պահանջում է այս համակարգչի բարձրախոսների տրամաչափումը: Սա պահանջում է iPhone կամ iPad, որը միացված է ինտերնետին: Դուք ունեք մեկ հարմար:\n",
    fi: "Tarkan äänen tuottamiseksi koe vaatii tämän tietokoneen kaiuttimien kalibroinnin. Tämä vaatii Internetiin kytketyn iPhonen tai iPadin. Onko sinulla yksi kätevä?\n",
    ka: "ზუსტი ხმის შესაქმნელად, ექსპერიმენტი მოითხოვს ამ კომპიუტერის დინამიკის კალიბრაციას. ეს მოითხოვს iPhone- ს ან iPad- ს, რომელიც ინტერნეტთან არის დაკავშირებული. ერთი მოსახერხებელი გაქვს?\n",
    he: "כדי לייצר צליל מדויק, הניסוי דורש כיול של רמקולים של מחשב זה. זה דורש אייפון או אייפד המחוברים לאינטרנט. יש לך שימושי אחד?\n",
    ar: "لإنتاج صوت دقيق ، تتطلب التجربة معايرة مكبرات الصوت لهذا الكمبيوتر. يتطلب ذلك جهاز iPhone أو iPad متصل بالإنترنت. هل لديك واحد في متناول يدي؟\n",
    ur: "درست آواز پیدا کرنے کے ل the ، اس تجربے کے لئے اس کمپیوٹر کے لاؤڈ اسپیکر کے انشانکن کی ضرورت ہے۔ اس کے لئے آئی فون یا آئی پیڈ کی ضرورت ہے جو انٹرنیٹ سے منسلک ہو۔ کیا آپ کے پاس ایک آسان ہے؟\n",
    hi: "सटीक ध्वनि का उत्पादन करने के लिए, प्रयोग के लिए इस कंप्यूटर के लाउडस्पीकरों के अंशांकन की आवश्यकता होती है। इसके लिए एक iPhone या iPad की आवश्यकता होती है जो इंटरनेट से जुड़ा हो। क्या आपके पास एक काम है?\n",
    ta: "துல்லியமான ஒலியை உருவாக்க, சோதனைக்கு இந்த கணினியின் ஒலிபெருக்கிகளின் அளவுத்திருத்தம் தேவைப்படுகிறது. இதற்கு இணையத்துடன் இணைக்கப்பட்ட ஐபோன் அல்லது ஐபாட் தேவைப்படுகிறது. உங்களிடம் ஒரு எளிது இருக்கிறதா?\n",
    ml: "കൃത്യമായ ശബ്ദം പുറപ്പെടുവിക്കാൻ, പരീക്ഷണത്തിന് ഈ കമ്പ്യൂട്ടറിന്റെ ഉച്ചഭാഷിണികളുടെ കാലിബ്രേഷൻ ആവശ്യമാണ്. ഇതിന് ഇന്റർനെറ്റുമായി ബന്ധിപ്പിച്ച ഒരു ഐഫോൺ അല്ലെങ്കിൽ ഐപാഡ് ആവശ്യമാണ്. നിങ്ങൾക്ക് ഒരു ഹാൻഡി ഉണ്ടോ?\n",
    te: "ఖచ్చితమైన ధ్వనిని ఉత్పత్తి చేయడానికి, ప్రయోగానికి ఈ కంప్యూటర్ యొక్క లౌడ్ స్పీకర్ల క్రమాంకనం అవసరం. దీనికి ఇంటర్నెట్‌కు అనుసంధానించబడిన ఐఫోన్ లేదా ఐప్యాడ్ అవసరం. మీకు ఒక సులభ ఉందా?\n",
    kn: "ನಿಖರವಾದ ಧ್ವನಿಯನ್ನು ಉತ್ಪಾದಿಸಲು, ಪ್ರಯೋಗಕ್ಕೆ ಈ ಕಂಪ್ಯೂಟರ್‌ನ ಧ್ವನಿವರ್ಧಕಗಳ ಮಾಪನಾಂಕ ನಿರ್ಣಯದ ಅಗತ್ಯವಿದೆ. ಇದಕ್ಕೆ ಇಂಟರ್ನೆಟ್‌ಗೆ ಸಂಪರ್ಕ ಹೊಂದಿದ ಐಫೋನ್ ಅಥವಾ ಐಪ್ಯಾಡ್ ಅಗತ್ಯವಿದೆ. ನೀವು ಒಂದು ಸೂಕ್ತವಾಗಿದ್ದೀರಾ?\n",
    bn: "সঠিক শব্দ উত্পাদন করতে, পরীক্ষার জন্য এই কম্পিউটারের লাউডস্পিকারগুলির ক্রমাঙ্কন প্রয়োজন। এটির জন্য একটি আইফোন বা আইপ্যাড প্রয়োজন যা ইন্টারনেটের সাথে সংযুক্ত। তোমার কি এক হাত আছে?\n",
    id: "Untuk menghasilkan suara yang akurat, percobaan membutuhkan kalibrasi pengeras suara komputer ini. Ini membutuhkan iPhone atau iPad yang terhubung ke Internet. Apakah Anda punya satu berguna?\n",
    "zh-CN":
      "为了产生准确的声音，实验需要校准该计算机的扬声器。这需要连接到Internet的iPhone或iPad。你有方便吗？\n",
    "zh-HK":
      "為了產生準確的聲音，實驗需要校準該計算機的揚聲器。這需要連接到Internet的iPhone或iPad。你有方便嗎？\n",
    ja: "正確なサウンドを生成するには、この実験では、このコンピューターのスピーカーのキャリブレーションが必要です。これには、インターネットに接続されているiPhoneまたはiPadが必要です。便利なものはありますか？\n",
    ko: "정확한 사운드를 생성하려면 실험을 위해서는이 컴퓨터의 라우드 스피커를 교정해야합니다. 인터넷에 연결된 iPhone 또는 iPad가 필요합니다. 편리한 사람이 있습니까?\n",
  },
  RC_soundCalibrationNo: {
    "en-US": "No, end experiment now.",
    de: "Nein, Ende Experiment jetzt.",
    fr: "Non, Expérience de fin maintenant.",
    es: "No, finalice el experimento ahora.",
    pt: "Não, experimento final agora.",
    it: "No, End Experiment ora.",
    ro: "Nu, End Experiment acum.",
    pl: "Nie, końcowe eksperyment teraz.",
    ru: "Нет, теперь конечный эксперимент.",
    hy: "Ոչ, հիմա ավարտեք փորձը:",
    fi: "Ei, loppukoe nyt.",
    ka: "არა, დაასრულეთ ექსპერიმენტი ახლა.",
    he: "לא, סיום ניסוי עכשיו.",
    ar: "لا ، تجربة نهاية الآن.",
    ur: "نہیں ، اب آخری تجربہ۔",
    hi: "नहीं, अब अंत प्रयोग।",
    ta: "இல்லை, இப்போது சோதனை.",
    ml: "ഇല്ല, ഇപ്പോൾ അവസാന പരീക്ഷണം.",
    te: "లేదు, ఇప్పుడు ప్రయోగం ముగింపు.",
    kn: "ಇಲ್ಲ, ಈಗ ಪ್ರಯೋಗವನ್ನು ಕೊನೆಗೊಳಿಸಿ.",
    bn: "না, এখন শেষ পরীক্ষা।",
    id: "Tidak, akhiri percobaan sekarang.",
    "zh-CN": "不，现在结束实验。",
    "zh-HK": "不，現在結束實驗。",
    ja: "いいえ、実験を終了します。",
    ko: "아니요, 이제 실험을 끝내십시오.",
  },
  RC_soundCalibrationQR: {
    "en-US":
      "When the QR code appears below, point your iPhone or iPad camera at it, and click the link that appears on your iPhone/iPad screen. Then follow the instuctions on the iPhone/iPad screen.\n——————————————————————————————————————————\nQUIET PLEASE! NOW RECORDING. DO NOT TOUCH THE VOLUME CONTROL.\n——————————————————————————————————————————\nPROBLEM?                                                                                                                                 \n1. This needs WiFi. If your phone and computer are not both on the same WiFi network then the iPhone won't get as far as asking your permission to use the microphone, and you'll wait forever. So connect the computer and iPhone to the same WiFi or quit.\n                                                                                                                                 \n2. If the iPhone reports an error in the connection, try quitting and restarting the iPhone browser, or even restarting the iPhone.\n                                                                                                                             \n3. It's rare, but, if the sound doesn't play, or, after it plays, no Proceed button appears, it means the calibration failed. You can try again by refreshing this browser page. That usually does the trick.",
    de: "Wenn der QR -Code unten angezeigt wird, zeigen Sie Ihre iPhone oder iPad -Kamera darauf und klicken Sie auf den Link, der auf Ihrem iPhone/iPad -Bildschirm angezeigt wird. Folgen Sie dann den Instuktionen auf dem iPhone/iPad -Bildschirm.\n————————————————————————————————————————————————————————————————————————————————————————————————————————————————— zugezogene\nRUHIG BITTE! Jetzt aufnehmen. Berühren Sie die Lautstärkeregelung nicht.\n————————————————————————————————————————————————————————————————————————————————————————————————————————————————— zugezogene\nPROBLEM?\n1. Dies braucht WLAN. Wenn Ihr Telefon und Ihr Computer nicht beide im selben WLAN -Netzwerk sind, wird das iPhone nicht so weit angeht, dass Sie Ihre Erlaubnis zur Verwendung des Mikrofons bitten, und Sie werden ewig warten. Schließen Sie also den Computer und das iPhone mit demselben WLAN an oder beenden Sie.\n                                                                                                                                 \n2. Wenn das iPhone einen Fehler in der Verbindung meldet, beenden Sie den iPhone -Browser an und starten Sie das iPhone sogar neu.\n                                                                                                                             \n3. Es ist selten, aber wenn der Sound nicht spielt oder nach dem Spielen keine Taste erscheint, bedeutet dies, dass die Kalibrierung fehlgeschlagen ist. Sie können es erneut versuchen, indem Sie diese Browser -Seite aktualisieren. Das macht normalerweise den Trick.",
    fr: "Lorsque le code QR apparaît ci-dessous, indiquez votre caméra iPhone ou iPad, puis cliquez sur le lien qui apparaît sur votre écran iPhone / iPad. Suivez ensuite les instructions sur l'écran iPhone / iPad.\n————————————————————————————————————————————\nSILENCE S.V.P! Maintenant enregistrer. Ne touchez pas le contrôle du volume.\n————————————————————————————————————————————\nPROBLÈME?\n1. Cela nécessite le wifi. Si votre téléphone et votre ordinateur ne sont pas tous les deux sur le même réseau WiFi, l'iPhone n'obtiendra pas jusqu'à demander votre autorisation d'utiliser le microphone, et vous attendez éternellement. Connectez donc l'ordinateur et l'iPhone au même WiFi ou à quitter.\n                                                                                                                                 \n2. Si l'iPhone signale une erreur dans la connexion, essayez de quitter et de redémarrer le navigateur iPhone ou même de redémarrer l'iPhone.\n                                                                                                                             \n3. C'est rare, mais, si le son ne joue pas, ou, après son jeu, aucun bouton de procédure n'apparaît, cela signifie que l'étalonnage a échoué. Vous pouvez réessayer en rafraîchissant cette page de navigateur. Cela fait généralement l'affaire.",
    es: "Cuando el código QR aparezca a continuación, apunte su cámara para iPhone o iPad y haga clic en el enlace que aparece en la pantalla de su iPhone/iPad. Luego siga las instricciones en la pantalla de iPhone/iPad.\n——————————————————————————————————————————————\n¡SILENCIO POR FAVOR! Ahora grabando. No toque el control de volumen.\n——————————————————————————————————————————————\n¿PROBLEMA?\n1. Esto necesita wifi. Si su teléfono y computadora no están en la misma red WiFi, el iPhone no llegará tan lejos como pedir su permiso para usar el micrófono, y esperará para siempre. Por lo tanto, conecte la computadora y el iPhone al mismo wifi o renunciar.\n                                                                                                                                 \n2. Si el iPhone informa un error en la conexión, intente dejar de fumar y reiniciar el navegador de iPhone, o incluso reiniciar el iPhone.\n                                                                                                                             \n3. Es raro, pero, si el sonido no se reproduce o, después de que se reproduce, no aparece el botón proceder, significa que la calibración falló. Puede volver a intentarlo actualizando esta página del navegador. Eso generalmente hace el truco.",
    pt: "Quando o código QR aparecer abaixo, aponte sua câmera para iPhone ou iPad e clique no link que aparece na tela do iPhone/iPad. Em seguida, siga as instituições na tela do iPhone/iPad.\n———————————————————————————————————————————\nQUIETO POR FAVOR! Agora gravação. Não toque no controle de volume.\n———————————————————————————————————————————\nPROBLEMA?\n1. Isso precisa de wifi. Se o telefone e o computador não estiverem na mesma rede WiFi, o iPhone não chegará a pedir sua permissão para usar o microfone e você esperará para sempre. Portanto, conecte o computador e o iPhone ao mesmo wifi ou desista.\n                                                                                                                                 \n2. Se o iPhone relatar um erro na conexão, tente desistir e reiniciar o navegador do iPhone ou até mesmo reiniciar o iPhone.\n                                                                                                                             \n3. É raro, mas, se o som não reproduzir, ou, depois de tocar, nenhum botão de processo aparece, significa que a calibração falhou. Você pode tentar novamente atualizando esta página do navegador. Isso geralmente faz o truque.",
    it: "Quando il codice QR viene visualizzato di seguito, punta il tuo iPhone o la fotocamera iPad e fai clic sul collegamento che viene visualizzato nella schermata iPhone/iPad. Quindi seguire le istruzioni sullo schermo iPhone/iPad.\n——————————————————————————————————————————————\nSILENZIO PREGO! Ora registrazione. Non toccare il controllo del volume.\n——————————————————————————————————————————————\nPROBLEMA?\n1. Questo ha bisogno di wifi. Se il tuo telefono e computer non si trovano entrambi sulla stessa rete WiFi, l'iPhone non arriverà fino a chiedere il permesso di usare il microfono e aspetterai per sempre. Quindi collega il computer e l'iPhone allo stesso wifi o smettila.\n                                                                                                                                 \n2. Se l'iPhone segnala un errore nella connessione, prova ad smettere e riavviare il browser iPhone o addirittura riavviare l'iPhone.\n                                                                                                                             \n3. È raro, ma, se il suono non suona o, dopo che riproduce, non appare il pulsante di procedimento, significa che la calibrazione non è riuscita. Puoi riprovare aggiornando questa pagina del browser. Questo di solito fa il trucco.",
    ro: "Când codul QR apare mai jos, îndreptați -vă camera iPhone sau iPad -ul și faceți clic pe linkul care apare pe ecranul iPhone/iPad. Apoi, urmați instincțiile de pe ecranul iPhone/iPad.\n———————————————————————————————————————————-\nVĂ ROG LINIȘTE! Acum înregistrare. Nu atingeți controlul volumului.\n———————————————————————————————————————————-\nPROBLEMĂ?\n1. Aceasta are nevoie de wifi. Dacă telefonul și computerul dvs. nu sunt ambele pe aceeași rețea WiFi, atunci iPhone -ul nu va ajunge până la cererea permisiunii dvs. de a utiliza microfonul și veți aștepta pentru totdeauna. Deci, conectați computerul și iPhone -ul la același wifi sau renunțați.\n                                                                                                                                 \n2. Dacă iPhone -ul raportează o eroare în conexiune, încercați să renunțați și să reporniți browserul iPhone sau chiar să reporniți iPhone -ul.\n                                                                                                                             \n3. Este rar, dar, dacă sunetul nu se joacă, sau, după ce se joacă, nu apare butonul de procedură, înseamnă că calibrarea a eșuat. Puteți încerca din nou reîmprospătarea acestei pagini de browser. Asta de obicei face trucul.",
    pl: "Gdy kod QR pojawi się poniżej, skieruj na niego kamerę iPhone'a lub iPada i kliknij link, który pojawi się na ekranie iPhone/iPad. Następnie postępuj zgodnie z instukcjami na ekranie iPhone/iPad.\n——————————————————————————————————————————\nCISZA PROSZĘ! Teraz nagrywanie. Nie dotykaj kontroli głośności.\n——————————————————————————————————————————\nPROBLEM?\n1. Wymaga to Wi -Fi. Jeśli Twój telefon i komputer nie znajdują się w tej samej sieci Wi -Fi, iPhone nie osiągnie prośby o zgodę na użycie mikrofonu, a będziesz czekać wiecznie. Podłącz komputer i iPhone do tego samego Wi -Fi lub zrezygnuj.\n                                                                                                                                 \n2. Jeśli iPhone zgłosi błąd w połączeniu, spróbuj rzucić palenie i ponowne uruchomienie przeglądarki iPhone'a, a nawet ponownie uruchomić iPhone'a.\n                                                                                                                             \n3. Jest to rzadkie, ale jeśli dźwięk nie odtwarza lub, po odtworzeniu, nie pojawia się przycisk Przejdź, oznacza to, że kalibracja nie powiodła się. Możesz spróbować ponownie, odświeżając tę ​​stronę przeglądarki. To zwykle robi sprawę.",
    ru: "Когда QR -код появляется ниже, укажите на него камеру iPhone или iPad и нажмите на ссылку, которая появляется на вашем экране iPhone/iPad. Затем следуйте инстакциям на экране iPhone/iPad.\n—————————————————————————————————————\nТИХО, ПОЖАЛУЙСТА! Сейчас запись. Не касайтесь регулятора громкости.\n—————————————————————————————————————\nПРОБЛЕМА?\n1. Это нуждается в Wi -Fi. Если ваш телефон и компьютер не находятся в одной и той же сети Wi -Fi, то iPhone не дойдет до того, чтобы попросить ваше разрешение использовать микрофон, и вы будете ждать вечно. Так что подключите компьютер и iPhone к тому же Wi -Fi или уходите.\n                                                                                                                                 \n2. Если iPhone сообщает об ошибке в соединении, попробуйте покинуть и перезапустить браузер iPhone или даже перезагрузить iPhone.\n                                                                                                                             \n3. Это редко, но, если звук не играет, или, после его воспроизведения, кнопка прохождения не появляется, это означает, что калибровка не удалась. Вы можете попробовать еще раз, освежив эту страницу браузера. Это обычно делает.",
    hy: "Երբ QR ծածկագիրը հայտնվում է ներքեւում, մատնանշեք ձեր iPhone կամ iPad տեսախցիկը դրա վրա եւ կտտացրեք հղումը, որը հայտնվում է ձեր iPhone / iPad էկրանին: Այնուհետեւ հետեւեք IPhone / iPad էկրանին ներկայացված ներարկումներին:\n-------------------------------\nԼՌՈՒԹՅՈՒՆ ԽՆԴՐՈՒՄ ԵՄ! Հիմա ձայնագրվում է: Մի դիպչեք ձայնի վերահսկմանը:\n-------------------------------\nԽնդիր?\n1. Այս կարիքն ունի wifi: Եթե ​​ձեր հեռախոսը եւ համակարգիչը երկուսն էլ նույն WiFi ցանցում չեն, ապա iPhone- ը չի ստանա այնքանով, որքանով հարցնեք խոսափողը օգտագործելու ձեր թույլտվությունը, եւ դուք կսպասեք հավերժ: Այսպիսով, միացրեք համակարգիչը եւ iPhone- ը նույն wifi- ին կամ թողեք:\n                                                                                                                                 \n2. Եթե iPhone- ը սխալ է հաղորդում կապի մեջ, փորձեք թողնել եւ վերագործարկել iPhone- ի զննարկիչը կամ նույնիսկ վերագործարկել iPhone- ը:\n                                                                                                                             \n3. Հազվադեպ է, բայց եթե ձայնը չի խաղում, կամ դրանում ներկայացվում է, որեւէ ընթացքի կոճակը չի երեւում: Կարող եք կրկին փորձել `թարմացնելով այս զննարկչի էջը: Դա սովորաբար անում է հնարքը:",
    fi: "Kun QR -koodi näkyy alla, osoita siihen iPhone tai iPad -kamera ja napsauta linkkiä, joka näkyy iPhone/iPad -näytöllä. Seuraa sitten iPhonen/iPad -näytön instituutioita.\n——————————————————————————————————————————————————————————\nHILJAA KIITOS! Nyt nauhoittaminen. Älä kosketa äänenvoimakkuuden säätöä.\n——————————————————————————————————————————————————————————\nONGELMA?\n1. Tämä tarvitsee wifi. Jos puhelimesi ja tietokoneesi eivät ole molemmat samassa WiFi -verkossa, iPhone ei pääse niin pitkälle, että kysyisi lupaa käyttää mikrofonia, ja odotat ikuisesti. Joten kytke tietokone ja iPhone samaan wifi- tai lopeta.\n                                                                                                                                 \n2. Jos iPhone ilmoittaa virheen yhteydessä, yritä lopettaa ja käynnistää iPhone -selaimen uudelleen tai jopa käynnistää iPhone uudelleen.\n                                                                                                                             \n3. Se on harvinaista, mutta jos ääni ei soita tai sen pelaamisen jälkeen ei tule painiketta, se tarkoittaa, että kalibrointi epäonnistui. Voit yrittää uudelleen virkistämällä tätä selain -sivua. Se yleensä tekee tempun.",
    ka: "როდესაც QR კოდი ქვემოთ გამოჩნდება, მიუთითეთ თქვენი iPhone ან iPad კამერა მასზე და დააჭირეთ ბმულს, რომელიც ჩანს თქვენს iPhone/iPad ეკრანზე. შემდეგ მიჰყევით ინსტიტუტებს iPhone/iPad ეკრანზე.\n——————————————————————————————————————————\nᲒᲗᲮᲝᲕᲗ ᲩᲣᲛᲐᲓ! ახლა ჩაწერა. არ შეეხოთ მოცულობის კონტროლს.\n——————————————————————————————————————————\nპრობლემა?\n1. ამას სჭირდება wifi. თუ თქვენი ტელეფონი და კომპიუტერი ორივე არ არის ერთსა და იმავე WiFi ქსელში, მაშინ iPhone ვერ მიიღებს რამდენადაც ითხოვს მიკროფონის გამოყენების ნებართვას და სამუდამოდ დაელოდებით. ასე რომ, დააკავშირეთ კომპიუტერი და iPhone იმავე WiFi- სთან ან დატოვეთ.\n                                                                                                                                 \n2. თუ iPhone აფიქსირებს შეცდომას კავშირში, შეეცადეთ დატოვოთ და გადატვირთოთ iPhone ბრაუზერი, ან თუნდაც iPhone– ის გადატვირთვა.\n                                                                                                                             \n3. ეს იშვიათია, მაგრამ, თუ ხმა არ თამაშობს, ან, მას შემდეგ, რაც ის თამაშობს, ღილაკი არ გამოჩნდება, ეს ნიშნავს, რომ კალიბრაცია ვერ მოხერხდა. შეგიძლიათ კვლავ სცადოთ ამ ბრაუზერის გვერდის განახლებით. ეს ჩვეულებრივ ხრიკს აკეთებს.",
    he: "כאשר קוד ה- QR מופיע למטה, כוון את המצלמה של ה- iPhone או ה- iPad שלך ולחץ על הקישור שמופיע במסך ה- iPhone/iPad שלך. לאחר מכן עקוב אחר ההסכמים במסך האייפון/iPad.\n————————————————————————————————————————————— -\nשקט בבקשה! עכשיו הקלטה. אל תיגע בבקרת עוצמת הקול.\n————————————————————————————————————————————— -\nבְּעָיָה?\n1. זה זקוק ל- wifi. אם הטלפון והמחשב שלך לא נמצאים שניהם באותה רשת WiFi, האייפון לא יגיע עד לבקש את רשותך להשתמש במיקרופון ותמתין לנצח. אז חבר את המחשב והאייפון לאותו WiFi או צא.\n                                                                                                                                 \n2. אם ה- iPhone מדווח על שגיאה בחיבור, נסה להפסיק ולהפעיל מחדש את דפדפן האייפון, או אפילו להפעיל מחדש את ה- iPhone.\n                                                                                                                             \n3. זה נדיר, אבל אם הצליל לא מנגן, או אחרי שהוא משחק, שום כפתור המשך לא מופיע, זה אומר שהכיול נכשל. אתה יכול לנסות שוב על ידי רענון דף הדפדפן הזה. זה בדרך כלל עושה את העבודה.",
    ar: "عندما يظهر رمز الاستجابة السريعة أدناه ، قم بإشارة كاميرا iPhone أو iPad عليه ، وانقر فوق الرابط الذي يظهر على شاشة iPhone/iPad. ثم اتبع المؤسسات على شاشة iPhone/iPad.\n———————————————————————————————————————————————————————————————————————————————————————————————————————————–\nيرجى الهدوء! التسجيل الآن. لا تلمس التحكم في مستوى الصوت.\n———————————————————————————————————————————————————————————————————————————————————————————————————————————–\nمشكلة؟\n1. هذا يحتاج إلى wifi. إذا لم يكن هاتفك والكمبيوتر على حد سواء على نفس شبكة WiFi ، فلن يحصل iPhone إلى أبعد من طلب إذنك لاستخدام الميكروفون ، وسوف تنتظر إلى الأبد. لذا قم بتوصيل الكمبيوتر و iPhone بنفس WiFi أو الإقلاع عن التدخين.\n                                                                                                                                 \n2. إذا أبلغ iPhone عن خطأ في الاتصال ، فحاول الإقلاع عن متصفح iPhone وإعادة تشغيله ، أو حتى إعادة تشغيل iPhone.\n                                                                                                                             \n3. إنه أمر نادر الحدوث ، ولكن إذا لم يتم تشغيل الصوت ، أو بعد أن يلعب ، لا يظهر زر المتابعة ، فهذا يعني أن المعايرة فشلت. يمكنك المحاولة مرة أخرى عن طريق تحديث صفحة المتصفح هذه. التي عادة ما تفعل الخدعة.",
    ur: "جب کیو آر کوڈ نیچے ظاہر ہوتا ہے تو ، اپنے آئی فون یا آئی پیڈ کیمرا کی طرف اس کی نشاندہی کریں ، اور اس لنک پر کلک کریں جو آپ کے آئی فون/آئی پیڈ اسکرین پر ظاہر ہوتا ہے۔ اس کے بعد آئی فون/آئی پیڈ اسکرین پر شامل ہونے کی پیروی کریں۔\n—————————————————————————————————————————————————————-\nخاموش! اب ریکارڈنگ۔ حجم کنٹرول کو مت چھوئے۔\n—————————————————————————————————————————————————————-\nمسئلہ؟\n1. اس کو وائی فائی کی ضرورت ہے۔ اگر آپ کا فون اور کمپیوٹر دونوں ایک ہی وائی فائی نیٹ ورک پر نہیں ہیں تو آئی فون کو مائیکروفون استعمال کرنے کی اجازت سے پوچھ نہیں سکے گا ، اور آپ ہمیشہ کے لئے انتظار کریں گے۔ لہذا کمپیوٹر اور آئی فون کو اسی وائی فائی سے مربوط کریں یا چھوڑ دیں۔\n                                                                                                                                 \nIf. اگر آئی فون کنکشن میں کسی غلطی کی اطلاع دیتا ہے تو ، آئی فون براؤزر کو چھوڑنے اور دوبارہ شروع کرنے کی کوشش کریں ، یا آئی فون کو دوبارہ شروع کریں۔\n                                                                                                                             \nIt. یہ نایاب ہے ، لیکن ، اگر آواز نہیں چلتی ہے ، یا ، اس کے کھیلنے کے بعد ، کوئی آگے بڑھنے کا بٹن ظاہر نہیں ہوتا ہے ، اس کا مطلب ہے کہ انشانکن ناکام ہوگیا۔ آپ اس براؤزر کے صفحے کو تازہ دم کرکے دوبارہ کوشش کر سکتے ہیں۔ یہ عام طور پر چال چلاتا ہے۔",
    hi: "जब QR कोड नीचे दिखाई देता है, तो उस पर अपने iPhone या iPad कैमरे को इंगित करें, और आपके iPhone/iPad स्क्रीन पर दिखाई देने वाले लिंक पर क्लिक करें। फिर iPhone/iPad स्क्रीन पर इंस्टॉक्शन का पालन करें।\n————————————————————————————————————————————————————————————\nकृपया शांत! अब रिकॉर्डिंग। वॉल्यूम कंट्रोल को न स्पर्श करें।\n————————————————————————————————————————————————————————————\nसंकट?\n1. इसके लिए वाईफाई की जरूरत है। यदि आपका फ़ोन और कंप्यूटर दोनों एक ही वाईफाई नेटवर्क पर नहीं हैं, तो iPhone को माइक्रोफोन का उपयोग करने के लिए आपकी अनुमति पूछने के लिए नहीं मिलेगा, और आप हमेशा के लिए इंतजार करेंगे। इसलिए कंप्यूटर और iPhone को उसी वाईफाई से कनेक्ट करें या छोड़ दें।\n                                                                                                                                 \n2. यदि iPhone कनेक्शन में एक त्रुटि की रिपोर्ट करता है, तो iPhone ब्राउज़र को छोड़ने और पुनरारंभ करने का प्रयास करें, या यहां तक ​​कि iPhone को पुनरारंभ करने का प्रयास करें।\n                                                                                                                             \n3. यह दुर्लभ है, लेकिन, अगर ध्वनि नहीं खेलती है, या, खेलने के बाद, कोई आगे बढ़ने वाला बटन प्रकट नहीं होता है, तो इसका मतलब है कि अंशांकन विफल हो गया। आप इस ब्राउज़र पेज को रिफ्रेश करके फिर से प्रयास कर सकते हैं। यह आमतौर पर चाल करता है।",
    ta: "QR குறியீடு கீழே தோன்றும்போது, ​​உங்கள் ஐபோன் அல்லது ஐபாட் கேமராவை சுட்டிக்காட்டவும், உங்கள் ஐபோன்/ஐபாட் திரையில் தோன்றும் இணைப்பைக் கிளிக் செய்யவும். ஐபோன்/ஐபாட் திரையில் உள்ள நிறுவனங்களைப் பின்பற்றவும்.\n——————————————————————————————————————————\nதயவுசெய்து அமைதியாக இருக்கவும்! இப்போது பதிவு. தொகுதி கட்டுப்பாட்டைத் தொட வேண்டாம்.\n——————————————————————————————————————————\nபிரச்சனை?\n1. இதற்கு வைஃபை தேவை. உங்கள் தொலைபேசி மற்றும் கணினி இரண்டும் ஒரே வைஃபை நெட்வொர்க்கில் இல்லை என்றால், மைக்ரோஃபோனைப் பயன்படுத்த உங்கள் அனுமதியைக் கேட்பது வரை ஐபோன் வராது, நீங்கள் எப்போதும் காத்திருப்பீர்கள். எனவே கணினி மற்றும் ஐபோனை அதே வைஃபை உடன் இணைக்கவும் அல்லது வெளியேறவும்.\n                                                                                                                                 \n2. ஐபோன் இணைப்பில் பிழையைப் புகாரளித்தால், ஐபோன் உலாவியை விட்டு வெளியேறவும், மறுதொடக்கம் செய்யவும் அல்லது ஐபோனை மறுதொடக்கம் செய்யவும் முயற்சிக்கவும்.\n                                                                                                                             \n3. இது அரிதானது, ஆனால், ஒலி இயங்கவில்லை என்றால், அல்லது, அது விளையாடிய பிறகு, எந்த தொடர பொத்தானும் தோன்றவில்லை என்றால், அளவுத்திருத்தம் தோல்வியடைந்தது என்று அர்த்தம். இந்த உலாவி பக்கத்தை புதுப்பிப்பதன் மூலம் மீண்டும் முயற்சி செய்யலாம். அது பொதுவாக தந்திரத்தை செய்கிறது.",
    ml: "QR കോഡ് ചുവടെ ദൃശ്യമാകുമ്പോൾ, അതിൽ നിങ്ങളുടെ iPhone അല്ലെങ്കിൽ IPAD ക്യാമറ ചൂണ്ടിക്കാണിക്കുക, കൂടാതെ നിങ്ങളുടെ iPhone / iPad സ്ക്രീനിൽ ദൃശ്യമാകുന്ന ലിങ്കിൽ ക്ലിക്കുചെയ്യുക. ഐഫോൺ / ഐപാഡ് സ്ക്രീനിൽ പ്രോത്സാഹനം പിന്തുടരുക.\n----------------------------------------------------------------------------------------------------------------------\nദയവായി നിശബ്ദമാകൂ! ഇപ്പോൾ റെക്കോർഡിംഗ്. വോളിയം നിയന്ത്രണത്തിൽ തൊടരുത്.\n----------------------------------------------------------------------------------------------------------------------\nപ്രശ്നം?\n1. ഇതിന് വൈഫൈ ആവശ്യമാണ്. നിങ്ങളുടെ ഫോണും കമ്പ്യൂട്ടറും ഒരേ വൈഫൈ നെറ്റ്വർക്കിൽ ഇല്ലെങ്കിൽ, മൈക്രോഫോൺ ഉപയോഗിക്കാൻ നിങ്ങളുടെ അനുമതി ആവശ്യപ്പെടുന്നതിനായി ഐഫോൺ ലഭിക്കില്ല, നിങ്ങൾ എന്നെന്നേക്കുമായി കാത്തിരിക്കും. അതിനാൽ കമ്പ്യൂട്ടറിനെയും ഐഫോണിനെയും ഒരേ വൈഫൈയിലേക്ക് ബന്ധിപ്പിക്കുക അല്ലെങ്കിൽ ഉപേക്ഷിക്കുക.\n                                                                                                                                 \n2. കണക്ഷനിൽ ഐഫോൺ ഒരു പിശക് റിപ്പോർട്ടുചെയ്യുന്നുവെങ്കിൽ, ഐഫോൺ ബ്ര browser സർ ഉപേക്ഷിച്ച് പുനരാരംഭിക്കാൻ ശ്രമിക്കുക, അല്ലെങ്കിൽ ഐഫോൺ പുനരാരംഭിക്കുക.\n                                                                                                                             \n3. ഇത് അപൂർവമാണ്, പക്ഷേ, ശബ്ദം കളിക്കുന്നില്ലെങ്കിൽ, അല്ലെങ്കിൽ, അത് പ്ലേ ചെയ്യുമ്പോൾ, മുന്നോട്ട് പോയതിനുശേഷം, മുന്നോട്ട് ബട്ടൺ ദൃശ്യമാകുന്നില്ല, അതിനർത്ഥം കാലിബ്രേഷൻ പരാജയപ്പെട്ടു എന്നാണ്. ഈ ബ്ര browser സർ പേജ് പുതുക്കുന്നതിലൂടെ നിങ്ങൾക്ക് വീണ്ടും ശ്രമിക്കാം. അത് സാധാരണയായി തന്ത്രം ചെയ്യുന്നു.",
    te: "QR కోడ్ క్రింద కనిపించినప్పుడు, మీ ఐఫోన్ లేదా ఐప్యాడ్ కెమెరాను దానిపై సూచించండి మరియు మీ ఐఫోన్/ఐప్యాడ్ స్క్రీన్‌లో కనిపించే లింక్‌ను క్లిక్ చేయండి. అప్పుడు ఐఫోన్/ఐప్యాడ్ స్క్రీన్‌లో ఇన్‌స్టక్షన్లను అనుసరించండి.\n——————————————————————————————————————————————\nదయచేసి నిశ్శబ్ధంగా ఉండండి! ఇప్పుడు రికార్డింగ్. వాల్యూమ్ నియంత్రణను తాకవద్దు.\n——————————————————————————————————————————————\nసమస్య?\n1. దీనికి వైఫై అవసరం. మీ ఫోన్ మరియు కంప్యూటర్ రెండూ ఒకే వైఫై నెట్‌వర్క్‌లో లేకపోతే, మైక్రోఫోన్‌ను ఉపయోగించమని మీ అనుమతి అడిగినంతవరకు ఐఫోన్ లభించదు మరియు మీరు ఎప్పటికీ వేచి ఉంటారు. కాబట్టి కంప్యూటర్ మరియు ఐఫోన్‌ను అదే వైఫైకి కనెక్ట్ చేయండి లేదా నిష్క్రమించండి.\n                                                                                                                                 \n2. ఐఫోన్ కనెక్షన్‌లో లోపం నివేదిస్తే, ఐఫోన్ బ్రౌజర్‌ను విడిచిపెట్టడానికి మరియు పున art ప్రారంభించడానికి లేదా ఐఫోన్‌ను పున art ప్రారంభించడానికి ప్రయత్నించండి.\n                                                                                                                             \n3. ఇది చాలా అరుదు, కానీ, ధ్వని ఆడకపోతే, లేదా, అది ఆడిన తర్వాత, కొనసాగిన తర్వాత, కొనసాగింపు బటన్ కనిపించదు, అంటే క్రమాంకనం విఫలమైందని అర్థం. ఈ బ్రౌజర్ పేజీని రిఫ్రెష్ చేయడం ద్వారా మీరు మళ్ళీ ప్రయత్నించవచ్చు. అది సాధారణంగా ట్రిక్ చేస్తుంది.",
    kn: "ಕ್ಯೂಆರ್ ಕೋಡ್ ಕೆಳಗೆ ಕಾಣಿಸಿಕೊಂಡಾಗ, ನಿಮ್ಮ ಐಫೋನ್ ಅಥವಾ ಐಪ್ಯಾಡ್ ಕ್ಯಾಮೆರಾವನ್ನು ಅದರಲ್ಲಿ ತೋರಿಸಿ, ಮತ್ತು ನಿಮ್ಮ ಐಫೋನ್/ಐಪ್ಯಾಡ್ ಪರದೆಯಲ್ಲಿ ಗೋಚರಿಸುವ ಲಿಂಕ್ ಅನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ. ನಂತರ ಐಫೋನ್/ಐಪ್ಯಾಡ್ ಪರದೆಯಲ್ಲಿ ಸ್ಥಾಪನೆಗಳನ್ನು ಅನುಸರಿಸಿ.\n————————————————————————————————————————————\nದಯವಿಟ್ಟು ನಿಷ್ಯಬ್ಧವಾಗಿರಿ! ಈಗ ರೆಕಾರ್ಡಿಂಗ್. ಪರಿಮಾಣ ನಿಯಂತ್ರಣವನ್ನು ಮುಟ್ಟಬೇಡಿ.\n————————————————————————————————————————————\nಸಮಸ್ಯೆ?\n1. ಇದಕ್ಕೆ ವೈಫೈ ಅಗತ್ಯವಿದೆ. ನಿಮ್ಮ ಫೋನ್ ಮತ್ತು ಕಂಪ್ಯೂಟರ್ ಒಂದೇ ವೈಫೈ ನೆಟ್‌ವರ್ಕ್‌ನಲ್ಲಿ ಇಲ್ಲದಿದ್ದರೆ, ಮೈಕ್ರೊಫೋನ್ ಬಳಸಲು ನಿಮ್ಮ ಅನುಮತಿಯನ್ನು ಕೇಳುವಷ್ಟು ಐಫೋನ್ ಸಿಗುವುದಿಲ್ಲ, ಮತ್ತು ನೀವು ಶಾಶ್ವತವಾಗಿ ಕಾಯುತ್ತೀರಿ. ಆದ್ದರಿಂದ ಕಂಪ್ಯೂಟರ್ ಮತ್ತು ಐಫೋನ್ ಅನ್ನು ಒಂದೇ ವೈಫೈಗೆ ಸಂಪರ್ಕಪಡಿಸಿ ಅಥವಾ ತ್ಯಜಿಸಿ.\n                                                                                                                                 \n2. ಸಂಪರ್ಕದಲ್ಲಿ ಐಫೋನ್ ದೋಷವನ್ನು ವರದಿ ಮಾಡಿದರೆ, ಐಫೋನ್ ಬ್ರೌಸರ್ ಅನ್ನು ತ್ಯಜಿಸಲು ಮತ್ತು ಮರುಪ್ರಾರಂಭಿಸಲು ಪ್ರಯತ್ನಿಸಿ, ಅಥವಾ ಐಫೋನ್ ಅನ್ನು ಮರುಪ್ರಾರಂಭಿಸಿ.\n                                                                                                                             \n3. ಇದು ಅಪರೂಪ, ಆದರೆ, ಧ್ವನಿ ಆಡದಿದ್ದರೆ, ಅಥವಾ, ಅದು ಆಡಿದ ನಂತರ, ಯಾವುದೇ ಮುಂದುವರಿಕೆ ಬಟನ್ ಕಾಣಿಸುವುದಿಲ್ಲ, ಇದರರ್ಥ ಮಾಪನಾಂಕ ನಿರ್ಣಯ ವಿಫಲವಾಗಿದೆ. ಈ ಬ್ರೌಸರ್ ಪುಟವನ್ನು ರಿಫ್ರೆಶ್ ಮಾಡುವ ಮೂಲಕ ನೀವು ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಬಹುದು. ಅದು ಸಾಮಾನ್ಯವಾಗಿ ಟ್ರಿಕ್ ಮಾಡುತ್ತದೆ.",
    bn: "যখন কিউআর কোডটি নীচে প্রদর্শিত হবে, আপনার আইফোন বা আইপ্যাড ক্যামেরা এটিতে নির্দেশ করুন এবং আপনার আইফোন/আইপ্যাড স্ক্রিনে প্রদর্শিত লিঙ্কটি ক্লিক করুন। তারপরে আইফোন/আইপ্যাড স্ক্রিনে ইনস্টরশনগুলি অনুসরণ করুন।\n——————————————————————————————————————————\nশান্ত দয়া করে! এখন রেকর্ডিং। ভলিউম নিয়ন্ত্রণ স্পর্শ করবেন না।\n——————————————————————————————————————————\nসমস্যা?\n1. এটি ওয়াইফাই প্রয়োজন। যদি আপনার ফোন এবং কম্পিউটার উভয়ই একই ওয়াইফাই নেটওয়ার্কে না থাকে তবে আইফোনটি মাইক্রোফোনটি ব্যবহার করার জন্য আপনার অনুমতি চেয়ে যতটা পাবে না এবং আপনি চিরকাল অপেক্ষা করবেন। সুতরাং কম্পিউটার এবং আইফোনটিকে একই ওয়াইফাইয়ের সাথে সংযুক্ত করুন বা প্রস্থান করুন।\n                                                                                                                                 \n২. যদি আইফোন সংযোগে কোনও ত্রুটির প্রতিবেদন করে তবে আইফোন ব্রাউজারটি ছাড়তে এবং পুনরায় চালু করার চেষ্টা করুন, বা এমনকি আইফোনটি পুনরায় চালু করার চেষ্টা করুন।\n                                                                                                                             \n৩. এটি বিরল, তবে, যদি শব্দটি বাজায় না, বা এটি বাজানোর পরে, কোনও প্রসেস বোতাম উপস্থিত না হয়, এর অর্থ ক্রমাঙ্কন ব্যর্থ হয়েছে। আপনি এই ব্রাউজার পৃষ্ঠাটি সতেজ করে আবার চেষ্টা করতে পারেন। যে সাধারণত কৌতুক আছে।",
    id: "Ketika kode QR muncul di bawah, arahkan kamera iPhone atau iPad Anda ke sana, dan klik tautan yang muncul di layar iPhone/iPad Anda. Kemudian ikuti instuksi di layar iPhone/iPad.\n——————————————————————————————————————————-\nHARAP TENANG! Sekarang merekam. Jangan menyentuh kontrol volume.\n——————————————————————————————————————————-\nMASALAH?\n1. Ini membutuhkan wifi. Jika ponsel dan komputer Anda tidak keduanya berada di jaringan WiFi yang sama maka iPhone tidak akan mendapatkan izin Anda untuk menggunakan mikrofon, dan Anda akan menunggu selamanya. Jadi hubungkan komputer dan iPhone ke wifi yang sama atau berhenti.\n                                                                                                                                 \n2. Jika iPhone melaporkan kesalahan dalam koneksi, cobalah berhenti dan mulai ulang browser iPhone, atau bahkan memulai kembali iPhone.\n                                                                                                                             \n3. Ini jarang, tetapi, jika suara tidak diputar, atau, setelah diputar, tidak ada tombol proses yang muncul, itu berarti kalibrasi gagal. Anda dapat mencoba lagi dengan menyegarkan halaman browser ini. Itu biasanya berhasil.",
    "zh-CN":
      "当QR码出现在下面的QR码时，将iPhone或iPad摄像头指向其，然后单击iPhone/iPad屏幕上出现的链接。然后在iPhone/iPad屏幕上关注启动。\n———————————————————————————————————————————————————————————————————————————\n请安静！现在录制。不要触摸音量控制。\n———————————————————————————————————————————————————————————————————————————\n问题？\n1.这需要wifi。如果您的手机和计算机都不在同一WiFi网络上，那么iPhone将不会征得您使用麦克风的许可，您将永远等待。因此，将计算机和iPhone连接到相同的WiFi或退出。\n                                                                                                                                 \n2.如果iPhone报告了连接中的错误，请尝试退出和重新启动iPhone浏览器，甚至重新启动iPhone。\n                                                                                                                             \n3.很少见，但是，如果声音不播放，或者在播放后，没有出现“继续”按钮，则意味着校准失败。您可以通过刷新此浏览器页面来重试。通常可以解决问题。",
    "zh-HK":
      "當QR碼出現在下面的QR碼時，將iPhone或iPad攝像頭指向其，然後單擊iPhone/iPad屏幕上出現的鏈接。然後在iPhone/iPad屏幕上關注啟動。\n———————————————————————————————————————————————————————————————————————————\n請安靜！現在錄製。不要觸摸音量控制。\n———————————————————————————————————————————————————————————————————————————\n問題？\n1.這需要wifi。如果您的手機和計算機都不在同一WiFi網絡上，那麼iPhone將不會徵得您使用麥克風的許可，您將永遠等待。因此，將計算機和iPhone連接到相同的WiFi或退出。\n                                                                                                                                 \n2.如果iPhone報告了連接中的錯誤，請嘗試退出和重新啟動iPhone瀏覽器，甚至重新啟動iPhone。\n                                                                                                                             \n3.很少見，但是，如果聲音不播放，或者在播放後，沒有出現“繼續”按鈕，則意味著校準失敗。您可以通過刷新此瀏覽器頁面來重試。通常可以解決問題。",
    ja: "QRコードが以下に表示されたら、iPhoneまたはiPadカメラをポイントし、iPhone/iPad画面に表示されるリンクをクリックします。次に、iPhone/iPad画面のインスタンスに従ってください。\n—————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————\nしずかにしてください！今録音。ボリュームコントロールに触れないでください。\n—————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————\n問題？\n1.これにはwifiが必要です。携帯電話とコンピューターが同じWiFiネットワーク上にない場合、iPhoneはマイクを使用する許可を求める限り得られず、永遠に待ちます。したがって、コンピューターとiPhoneを同じWiFiに接続するか、終了します。\n                                                                                                                                 \n2. iPhoneが接続にエラーを報告した場合は、iPhoneブラウザを終了して再起動したり、iPhoneを再起動したりしてみてください。\n                                                                                                                             \n3.まれですが、サウンドが再生されない場合、または再生ボタンが表示されない場合、キャリブレーションが失敗したことを意味します。このブラウザページを更新して、再試行できます。それは通常、トリックを行います。",
    ko: "QR 코드가 아래에 나타나면 iPhone 또는 iPad 카메라를 가리키고 iPhone/iPad 화면에 나타나는 링크를 클릭하십시오. 그런 다음 iPhone/iPad 화면의 인scations를 따르십시오.\n——————————————————————————————————————————\n조용 해주세요! 이제 녹음. 볼륨 컨트롤에 닿지 마십시오.\n——————————————————————————————————————————\n문제?\n1. 이것은 Wi -Fi가 필요합니다. 휴대 전화와 컴퓨터가 동일한 WiFi 네트워크에 있지 않으면 iPhone은 마이크 사용 권한을 요구하는 것만 큼 멀지 않으며 영원히 기다릴 것입니다. 따라서 컴퓨터와 iPhone을 동일한 WiFi 또는 종료에 연결하십시오.\n                                                                                                                                 \n2. iPhone이 연결에 오류가보고되면 iPhone 브라우저를 종료하고 다시 시작하거나 iPhone을 다시 시작하십시오.\n                                                                                                                             \n3. 드물지만 사운드가 재생되지 않거나 재생 후 진행 버튼이 나타나지 않으면 교정이 실패 함을 의미합니다. 이 브라우저 페이지를 새로 고쳐서 다시 시도 할 수 있습니다. 그것은 일반적으로 트릭을 수행합니다.",
  },
  RC_soundCalibrationTitle: {
    "en-US": "Measure Sound Level at 1000 Hz",
    de: "Messen Sie den Schallpegel bei 1000 Hz",
    fr: "Mesurer le niveau sonore à 1000 Hz",
    es: "Medir el nivel de sonido a 1000 Hz",
    pt: "Meça o nível de som a 1000 Hz",
    it: "Misura il livello del suono a 1000 Hz",
    ro: "Măsurați nivelul sunetului la 1000 Hz",
    pl: "Zmierz poziom dźwięku przy 1000 Hz",
    ru: "Измерьте уровень звука при 1000 Гц",
    hy: "Չափել ձայնի մակարդակը 1000 Հց",
    fi: "Mittaa äänitaso 1000 Hz: llä",
    ka: "გაზომეთ ხმის დონე 1000 ჰც -ზე",
    he: "למדוד רמת צליל ב 1000 הרץ",
    ar: "قياس مستوى الصوت عند 1000 هرتز",
    ur: "1000 ہرٹج پر صوتی سطح کی پیمائش کریں",
    hi: "1000 हर्ट्ज पर ध्वनि स्तर को मापें",
    ta: "ஒலி அளவை 1000 ஹெர்ட்ஸில் அளவிடவும்",
    ml: "1000 HZ- ൽ ശബ്ദ നില അളക്കുക",
    te: "1000 Hz వద్ద ధ్వని స్థాయిని కొలవండి",
    kn: "1000 Hz ನಲ್ಲಿ ಧ್ವನಿ ಮಟ್ಟವನ್ನು ಅಳೆಯಿರಿ",
    bn: "1000 হার্জেডে শব্দ স্তর পরিমাপ করুন",
    id: "Ukur level suara pada 1000 Hz",
    "zh-CN": "测量1000 Hz的声音水平",
    "zh-HK": "測量1000 Hz的聲音水平",
    ja: "1000 Hzで音レベルを測定します",
    ko: "1000Hz에서 사운드 레벨을 측정하십시오",
  },
  RC_soundCalibrationTitle1000Hz: {
    "en-US": "Measure Sound Level at 1000 Hz",
    de: "Messen Sie den Schallpegel bei 1000 Hz",
    fr: "Mesurer le niveau sonore à 1000 Hz",
    es: "Medir el nivel de sonido a 1000 Hz",
    pt: "Meça o nível de som a 1000 Hz",
    it: "Misura il livello del suono a 1000 Hz",
    ro: "Măsurați nivelul sunetului la 1000 Hz",
    pl: "Zmierz poziom dźwięku przy 1000 Hz",
    ru: "Измерьте уровень звука при 1000 Гц",
    hy: "Չափել ձայնի մակարդակը 1000 Հց",
    fi: "Mittaa äänitaso 1000 Hz: llä",
    ka: "გაზომეთ ხმის დონე 1000 ჰც -ზე",
    he: "למדוד רמת צליל ב 1000 הרץ",
    ar: "قياس مستوى الصوت عند 1000 هرتز",
    ur: "1000 ہرٹج پر صوتی سطح کی پیمائش کریں",
    hi: "1000 हर्ट्ज पर ध्वनि स्तर को मापें",
    ta: "ஒலி அளவை 1000 ஹெர்ட்ஸில் அளவிடவும்",
    ml: "1000 HZ- ൽ ശബ്ദ നില അളക്കുക",
    te: "1000 Hz వద్ద ధ్వని స్థాయిని కొలవండి",
    kn: "1000 Hz ನಲ್ಲಿ ಧ್ವನಿ ಮಟ್ಟವನ್ನು ಅಳೆಯಿರಿ",
    bn: "1000 হার্জেডে শব্দ স্তর পরিমাপ করুন",
    id: "Ukur level suara pada 1000 Hz",
    "zh-CN": "测量1000 Hz的声音水平",
    "zh-HK": "測量1000 Hz的聲音水平",
    ja: "1000 Hzで音レベルを測定します",
    ko: "1000Hz에서 사운드 레벨을 측정하십시오",
  },
  RC_soundCalibrationTitleAllHz: {
    "en-US": "Measure Sound Level at All Frequencies",
    de: "Messen Sie die Schallpegel bei allen Frequenzen",
    fr: "Mesurer le niveau sonore à toutes les fréquences",
    es: "Medir el nivel de sonido en todas las frecuencias",
    pt: "Meça o nível de som em todas as frequências",
    it: "Misura il livello del suono a tutte le frequenze",
    ro: "Măsurați nivelul sunetului la toate frecvențele",
    pl: "Zmierzyć poziom dźwięku na wszystkich częstotliwościach",
    ru: "Измерить уровень звука на всех частотах",
    hy: "Չափել ձայնի մակարդակը բոլոր հաճախականություններում",
    fi: "Mittaa äänitaso kaikilla taajuuksilla",
    ka: "გაზომეთ ხმის დონე ყველა სიხშირეზე",
    he: "למדוד רמת צליל בכל התדרים",
    ar: "قياس مستوى الصوت في جميع الترددات",
    ur: "تمام تعدد پر صوتی سطح کی پیمائش کریں",
    hi: "सभी आवृत्तियों पर ध्वनि स्तर को मापें",
    ta: "அனைத்து அதிர்வெண்களிலும் ஒலி அளவை அளவிடவும்",
    ml: "എല്ലാ ആവൃത്തികളിലും ശബ്ദ നില അളക്കുക",
    te: "అన్ని పౌన .పున్యాల వద్ద ధ్వని స్థాయిని కొలవండి",
    kn: "ಎಲ್ಲಾ ಆವರ್ತನಗಳಲ್ಲಿ ಧ್ವನಿ ಮಟ್ಟವನ್ನು ಅಳೆಯಿರಿ",
    bn: "সমস্ত ফ্রিকোয়েন্সিগুলিতে শব্দ স্তর পরিমাপ করুন",
    id: "Ukur level suara di semua frekuensi",
    "zh-CN": "在所有频率上测量声音水平",
    "zh-HK": "在所有頻率上測量聲音水平",
    ja: "すべての周波数でサウンドレベルを測定します",
    ko: "모든 주파수에서 사운드 레벨을 측정하십시오",
  },
  RC_soundCalibrationYes: {
    "en-US": "Yes, continue.",
    de: "Ja, weiter.",
    fr: "Oui, continuez.",
    es: "Sí, continúa.",
    pt: "Sim, continue.",
    it: "Sì, continua.",
    ro: "Da, continuă.",
    pl: "Tak, kontynuuj.",
    ru: "Да, продолжай.",
    hy: "Այո, շարունակեք:",
    fi: "Kyllä, jatka.",
    ka: "დიახ, გააგრძელეთ.",
    he: "כן, המשך.",
    ar: "نعم ، تابع.",
    ur: "ہاں ، جاری رکھیں۔",
    hi: "हाँ, जारी रखें।",
    ta: "ஆம், தொடரவும்.",
    ml: "അതെ, തുടരുക.",
    te: "అవును, కొనసాగించండి.",
    kn: "ಹೌದು, ಮುಂದುವರಿಸಿ.",
    bn: "হ্যাঁ, চালিয়ে যান।",
    id: "Ya, lanjutkan.",
    "zh-CN": "是的，继续。",
    "zh-HK": "是的，繼續。",
    ja: "はい、続行します。",
    ko: "예, 계속하십시오.",
  },
  RC_starting: {
    "en-US": "Starting ...",
    de: "Beginnend ...",
    fr: "Commencer ...",
    es: "A partir de ...",
    pt: "Iniciando ...",
    it: "A partire da ...",
    ro: "Începând ...",
    pl: "Rozpoczęcie ...",
    ru: "Начало ...",
    hy: "Սկսելով ...",
    fi: "Aloita ...",
    ka: "დაწყებული ...",
    he: "מתחיל ...",
    ar: "ابتداء ...",
    ur: "شروع کرنا ...",
    hi: "शुरुआत ...",
    ta: "தொடங்குகிறது ...",
    ml: "തുടങ്ങുന്ന ...",
    te: "ప్రారంభం ...",
    kn: "ಆರಂಭಿಕ ...",
    bn: "শুরু ...",
    id: "Mulai ...",
    "zh-CN": "开始...",
    "zh-HK": "開始...",
    ja: "開始...",
    ko: "시작 ...",
  },
  RC_viewingBlindSpotCredit: {
    "en-US":
      "We enhanced the blindspot mapping of Li et al. (2020) by adding flicker and manual control.",
    de: "Wir haben die Blindspot -Zuordnung von Li et al. (2020) Durch Hinzufügen von Flackern und manueller Steuerung.",
    fr: "Nous avons amélioré la cartographie de Blindspot de Li et al. (2020) en ajoutant du scintillement et du contrôle manuel.",
    es: "Mejoramos el mapeo de punto ciego de Li et al. (2020) agregando parpadeo y control manual.",
    pt: "Aprimoramos o mapeamento de Li et al. (2020) Adicionando controle de glandela e manual.",
    it: "Abbiamo migliorato la mappatura cieca di Li et al. (2020) aggiungendo sfarfallio e controllo manuale.",
    ro: "Am îmbunătățit cartografierea Blindspot a Li și colab. (2020) prin adăugarea de pâlpâie și control manual.",
    pl: "Zwiększyliśmy mapowanie Blindspot Li i in. (2020), dodając migotanie i kontrolę ręczną.",
    ru: "Мы усилили картирование слепых пятен Li et al. (2020) путем добавления мерцания и ручного контроля.",
    hy: "Մենք բարձրացրեցինք Li et al- ի կույր կետը: (2020) ավելացնելով թարթիչ եւ ձեռքով հսկողություն:",
    fi: "Parannasimme Li et ai. (2020) lisäämällä välkkymistä ja manuaalista ohjausta.",
    ka: "ჩვენ გავაუმჯობესეთ Li et al. (2020) ციმციმისა და სახელმძღვანელო კონტროლის დამატებით.",
    he: "שיפרנו את מיפוי העיוורים של Li et al. (2020) על ידי הוספת הבהוב ושליטה ידנית.",
    ar: "قمنا بتحسين رسم الخرائط العمياء لـ Li et al. (2020) عن طريق إضافة وميض والسيطرة اليدوية.",
    ur: "ہم نے لی ایٹ ال کی بلائنڈ اسپاٹ میپنگ میں اضافہ کیا۔ (2020) فلکر اور دستی کنٹرول شامل کرکے۔",
    hi: "हमने ली एट अल के ब्लाइंडस्पॉट मैपिंग को बढ़ाया। (2020) फ़्लिकर और मैनुअल नियंत्रण जोड़कर।",
    ta: "லி மற்றும் பலரின் பிளைண்ட்ஸ்பாட் மேப்பிங்கை நாங்கள் மேம்படுத்தினோம். (2020) ஃப்ளிக்கர் மற்றும் கையேடு கட்டுப்பாட்டைச் சேர்ப்பதன் மூலம்.",
    ml: "ഞങ്ങൾ li et al al toon the the than thangined വർദ്ധിപ്പിച്ചു. (2020) സ്ലിക്കറും മാനുവൽ നിയന്ത്രണവും ചേർത്തുകൊണ്ട്.",
    te: "మేము లి మరియు ఇతరుల బ్లైండ్ స్పాట్ మ్యాపింగ్‌ను మెరుగుపరిచాము. (2020) ఫ్లికర్ మరియు మాన్యువల్ నియంత్రణను జోడించడం ద్వారా.",
    kn: "ನಾವು ಲಿ ಮತ್ತು ಇತರರ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್ ಮ್ಯಾಪಿಂಗ್ ಅನ್ನು ಹೆಚ್ಚಿಸಿದ್ದೇವೆ. (2020) ಫ್ಲಿಕರ್ ಮತ್ತು ಹಸ್ತಚಾಲಿತ ನಿಯಂತ್ರಣವನ್ನು ಸೇರಿಸುವ ಮೂಲಕ.",
    bn: "আমরা লি এট আল এর ব্লাইন্ডস্পট ম্যাপিং বাড়িয়েছি। (2020) ফ্লিকার এবং ম্যানুয়াল নিয়ন্ত্রণ যুক্ত করে।",
    id: "Kami meningkatkan pemetaan blindspot Li et al. (2020) dengan menambahkan flicker dan kontrol manual.",
    "zh-CN": "我们增强了Li等人的盲点映射。 （2020）通过添加闪烁和手动控制。",
    "zh-HK": "我們增強了Li等人的盲點映射。 （2020）通過添加閃爍和手動控制。",
    ja: "Li et alのブラインドスポットマッピングを強化しました。 （2020）フリッカーと手動制御を追加することにより。",
    ko: "우리는 Li et al. (2020) 깜박임 및 수동 제어를 추가함으로써.",
  },
  RC_viewingBlindSpotRejected: {
    "en-US":
      "Sorry, the most recent left and right measurements were inconsistent. Can we try again? Keep one eye closed. Focus on the cross hairs and monitor the flashing dot out of the corner of your eye. Slide the dot left and right until it disappears into your blindspot, and then hit RETURN. Maintain your viewing distance as you test both eyes.",
    de: "Entschuldigung, die jüngsten linken und rechten Messungen waren inkonsistent. Können wir es erneut versuchen? Halten Sie ein Auge geschlossen. Konzentrieren Sie sich auf die Kreuzhaare und überwachen Sie den blinkenden Punkt aus dem Augenwinkel. Schieben Sie den Punkt nach links und rechts, bis er in Ihren Blindspot verschwindet, und drücken Sie dann die Rückkehr. Behalten Sie Ihren Betrachtungsabstand bei, während Sie beide Augen testen.",
    fr: "Désolé, les mesures les plus récentes de gauche et de droite étaient incohérentes. Pouvons-nous réessayer? Gardez un œil fermé. Concentrez-vous sur les poils et surveillez le point clignotant du coin de votre œil. Faites glisser le point à gauche et à droite jusqu'à ce qu'il disparaisse dans votre point mort, puis appuyez sur Return. Maintenez votre distance de visualisation lorsque vous testez les deux yeux.",
    es: "Lo sentimos, las mediciones más recientes de izquierda y derecha fueron inconsistentes. ¿Podemos intentarlo de nuevo? Mantén un ojo cerrado. Concéntrese en la cruz y monitoree el punto intermitente en la esquina del ojo. Deslice el punto a la izquierda y a la derecha hasta que desaparezca en su punto ciego, y luego presione el retorno. Mantenga su distancia de visión mientras prueba ambos ojos.",
    pt: "Desculpe, as medições mais recentes de esquerda e direita foram inconsistentes. Podemos tentar de novo? Mantenha um olho fechado. Concentre -se nos cabelos cruzados e monitore o ponto piscando pelo canto do olho. Deslize o ponto para a esquerda e para a direita até que ele desapareça no seu ponto cego e, em seguida, pressione o retorno. Mantenha sua distância de visualização ao testar os dois olhos.",
    it: "Mi dispiace, le misurazioni più recenti di sinistra e destra erano incoerenti. Possiamo riprovare? Tieni un occhio chiuso. Concentrati sui peli incrociati e monitora il punto lampeggiante dall'angolo dell'occhio. Fai scorrere il punto a sinistra e a destra finché non scompare nel tuo cieco, quindi premi ritorno. Mantieni la distanza di visione mentre si verificano entrambi gli occhi.",
    ro: "Ne pare rău, cele mai recente măsurători la stânga și la dreapta au fost inconsistente. Putem încerca din nou? Țineți un ochi închis. Concentrați -vă pe părul încrucișat și monitorizați punctul intermitent din colțul ochiului. Glisați punctul la stânga și la dreapta până când dispare în Blindspot, apoi atingeți întoarcerea. Mențineți -vă distanța de vizionare în timp ce testați ambii ochi.",
    pl: "Przepraszam, najnowsze pomiary lewych i prawych były niespójne. Czy możemy spróbować ponownie? Miej jedno oko zamknięte. Skoncentruj się na włosach krzyżowych i monitoruj migającą kropkę z rogu oka. Wsuń kropkę w lewo i w prawo, aż zniknie w twojej ślepej, a następnie naciśnij powrót. Utrzymuj odległość oglądania podczas testowania obu oczu.",
    ru: "Извините, самые последние левые и правые измерения были непоследовательными. Мы можем попробовать еще раз? Держите один глаз закрытым. Сосредоточьтесь на кросс -волосках и следите за мигающей точкой из -за угла глаза. Сдвиньте точку влево и вправо, пока она не исчезнет в вашей слепой точке, а затем нажмите на возврат. Поддерживайте свое просмотр, когда вы тестируете оба глаза.",
    hy: "Ներեցեք, ամենավերջին ձախ եւ աջ չափումները անհամատեղելի էին: Կարող ենք կրկին փորձել: Մի աչքի փակ պահեք: Կենտրոնացեք խաչի մազերի վրա եւ վերահսկեք ձեր աչքի անկյունից ing րամեկուսացման կետը: Սահեցրեք կետը ձախ եւ աջ, մինչեւ այն անհետանա ձեր կույր կետի մեջ, ապա հարվածեք վերադարձին: Պահպանեք ձեր դիտման հեռավորությունը, երբ փորձարկում եք երկու աչքերը:",
    fi: "Valitettavasti viimeisimmät vasen ja oikea mittaukset olivat epäjohdonmukaisia. Voimmeko yrittää uudelleen? Pidä yksi silmä kiinni. Keskity ristikarvoihin ja tarkkaile vilkkuvaa pistettä silmän nurkasta. Liu'uta piste vasemmalle ja oikealle, kunnes se katoaa blindspotisi ja osu sitten paluun. Pidä katseluetäisyys testattaessasi molemmat silmät.",
    ka: "უკაცრავად, მარცხენა და მარჯვენა უახლესი გაზომვები არათანმიმდევრული იყო. შეგვიძლია ისევ ვცადოთ? ერთი თვალი დახუჭე. ფოკუსირება მოახდინეთ ჯვრის თმებზე და აკონტროლეთ მოციმციმე წერტილი თქვენი თვალის კუთხიდან. გადაიტანეთ წერტილი მარცხნივ და მარჯვნივ, სანამ ის არ ქრება თქვენს ჟალუზაში, შემდეგ კი მოხვდით. შეინარჩუნეთ სანახავი მანძილი, როდესაც ორივე თვალს შეამოწმებთ.",
    he: "סליחה, המדידות השמאליות והימניות האחרונות לא היו עקביות. האם נוכל לנסות שוב? שמור עין אחת עצומה. התמקדו בשערות הצלב ועקבו אחר הנקודה המהבהבת מפינת העין. החלק את הנקודה שמאלה וימינה עד שהוא נעלם אל כתב העיוור שלך ואז פגע בחזרה. שמור על מרחק הצפייה שלך כשאתה בודק את שתי העיניים.",
    ar: "آسف ، كانت القياسات اليمنى واليسرى الأحدث غير متسقة. أيمكننا المحاولة مرة أخرى؟ الحفاظ على عين واحدة مغلقة. ركز على الشعر المتقاطع ومراقبة النقطة الوامضة من زاوية عينيك. حرك النقطة اليسرى واليمين حتى تختفي في النطاق العمياء الخاص بك ، ثم ضرب العودة. حافظ على مسافة المشاهدة الخاصة بك وأنت تختبر كلتا العينين.",
    ur: "معذرت ، حالیہ بائیں اور دائیں پیمائش متضاد تھیں۔ کیا ہم دوبارہ کوشش کر سکتے ہیں؟ ایک آنکھ بند رکھیں۔ کراس بالوں پر توجہ دیں اور اپنی آنکھ کے کونے سے باہر چمکتے ڈاٹ کی نگرانی کریں۔ ڈاٹ کو بائیں اور دائیں سلائڈ کریں جب تک کہ یہ آپ کے اندھے مقام میں غائب نہ ہوجائے ، اور پھر واپسی کو ماریں۔ جب آپ دونوں آنکھوں کی جانچ کرتے ہو تو اپنے دیکھنے کا فاصلہ برقرار رکھیں۔",
    hi: "क्षमा करें, सबसे हालिया बाएं और दाएं माप असंगत थे। क्या हम फिर से कोशिश कर सकते हैं? एक आंख बंद रखें। क्रॉस हेयर पर ध्यान दें और अपनी आंख के कोने से चमकती डॉट की निगरानी करें। बाईं और दाईं ओर स्लाइड करें जब तक कि यह आपके ब्लाइंडस्पॉट में गायब न हो जाए, और फिर रिटर्न हिट करें। जैसे -जैसे आप दोनों आंखों का परीक्षण करते हैं, अपनी देखने की दूरी बनाए रखें।",
    ta: "மன்னிக்கவும், மிக சமீபத்திய இடது மற்றும் வலது அளவீடுகள் சீரற்றவை. நாம் மீண்டும் முயற்சிக்கலாமா? ஒரு கண்ணை மூடிக்கொள்ளவும். குறுக்கு முடிகளில் கவனம் செலுத்துங்கள் மற்றும் உங்கள் கண்ணின் மூலையில் இருந்து ஒளிரும் புள்ளியைக் கண்காணிக்கவும். உங்கள் பிளைண்ட்ஸ்பாட்டில் மறைந்துவிடும் வரை இடது மற்றும் வலதுபுறத்தை சறுக்கி, பின்னர் திரும்பவும். நீங்கள் இரு கண்களையும் சோதிக்கும்போது உங்கள் பார்வை தூரத்தை பராமரிக்கவும்.",
    ml: "ക്ഷമിക്കണം, ഏറ്റവും പുതിയ ഇടത്, വലത് അളവുകൾ പൊരുത്തപ്പെടുന്നില്ല. നമുക്ക് വീണ്ടും ശ്രമിക്കാമോ? ഒരു കണ്ണ് അടച്ച് സൂക്ഷിക്കുക. ക്രോസ് ഹെറിൽസിൽ ശ്രദ്ധ കേന്ദ്രീകരിക്കുക, നിങ്ങളുടെ കണ്ണിന്റെ മൂലയിൽ നിന്ന് മിന്നുന്ന ഡോട്ട് നിരീക്ഷിക്കുക. നിങ്ങളുടെ ബ്ലൈൻഡ്സ്പോട്ടിലേക്ക് അപ്രത്യക്ഷമാകുന്നതുവരെ ഡോട്ട് ഇടത്തോട്ടും വലത്തോട്ടും സ്ലൈഡുചെയ്യുക, തുടർന്ന് മടങ്ങുക. നിങ്ങൾ രണ്ട് കണ്ണുകളും പരീക്ഷിക്കുമ്പോൾ നിങ്ങളുടെ കാണൽ ദൂരം നിലനിർത്തുക.",
    te: "క్షమించండి, ఇటీవలి ఎడమ మరియు కుడి కొలతలు అస్థిరంగా ఉన్నాయి. మేము మళ్ళీ ప్రయత్నించగలమా? ఒక కన్ను మూసుకుని ఉంచండి. క్రాస్ హెయిర్స్ పై దృష్టి పెట్టండి మరియు మీ కంటి మూలలో నుండి మెరుస్తున్న చుక్కను పర్యవేక్షించండి. మీ బ్లైండ్‌స్పాట్‌లో అదృశ్యమయ్యే వరకు చుక్కను ఎడమ మరియు కుడి వైపుకు జారండి, ఆపై రిటర్న్ నొక్కండి. మీరు రెండు కళ్ళను పరీక్షిస్తున్నప్పుడు మీ వీక్షణ దూరాన్ని కొనసాగించండి.",
    kn: "ಕ್ಷಮಿಸಿ, ಇತ್ತೀಚಿನ ಎಡ ಮತ್ತು ಬಲ ಅಳತೆಗಳು ಅಸಮಂಜಸವಾಗಿವೆ. ನಾವು ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಬಹುದೇ? ಒಂದು ಕಣ್ಣು ಮುಚ್ಚಿಡಿ. ಅಡ್ಡ ಕೂದಲಿನ ಮೇಲೆ ಕೇಂದ್ರೀಕರಿಸಿ ಮತ್ತು ನಿಮ್ಮ ಕಣ್ಣಿನ ಮೂಲೆಯಿಂದ ಮಿನುಗುವ ಚುಕ್ಕೆ ಮೇಲ್ವಿಚಾರಣೆ ಮಾಡಿ. ನಿಮ್ಮ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್‌ಗೆ ಕಣ್ಮರೆಯಾಗುವವರೆಗೆ ಡಾಟ್ ಅನ್ನು ಎಡ ಮತ್ತು ಬಲಕ್ಕೆ ಸ್ಲೈಡ್ ಮಾಡಿ, ತದನಂತರ ರಿಟರ್ನ್ ಒತ್ತಿರಿ. ನೀವು ಎರಡೂ ಕಣ್ಣುಗಳನ್ನು ಪರೀಕ್ಷಿಸುವಾಗ ನಿಮ್ಮ ವೀಕ್ಷಣೆಯ ದೂರವನ್ನು ಕಾಪಾಡಿಕೊಳ್ಳಿ.",
    bn: "দুঃখিত, সর্বাধিক সাম্প্রতিক বাম এবং ডান পরিমাপগুলি বেমানান ছিল। আমরা কি আবার চেষ্টা করতে পারি? এক চোখ বন্ধ রাখুন। ক্রস কেশগুলিতে ফোকাস করুন এবং আপনার চোখের কোণ থেকে ফ্ল্যাশিং ডটটি পর্যবেক্ষণ করুন। এটি আপনার ব্লাইন্ডস্পটে অদৃশ্য না হওয়া পর্যন্ত বাম এবং ডানদিকে ডটটি স্লাইড করুন এবং তারপরে ফিরে আঘাত করুন। আপনি উভয় চোখ পরীক্ষা করার সাথে সাথে আপনার দেখার দূরত্ব বজায় রাখুন।",
    id: "Maaf, pengukuran kiri dan kanan terbaru tidak konsisten. Bisakah kita mencoba lagi? Tutup satu mata. Fokus pada rambut silang dan pantau titik berkedip dari sudut mata Anda. Geser titik ke kiri dan ke kanan sampai menghilang ke dalam blindspot Anda, dan kemudian tekan kembali. Pertahankan jarak menonton Anda saat Anda menguji kedua mata.",
    "zh-CN":
      "抱歉，最近的左右测量结果不一致。我们可以再试一次吗？闭上眼睛。专注于十字头发，并从眼角监视闪烁的点。向左和向右滑动点，直到它消失在盲点中​​，然后击中返回。在测试双眼时，保持观看距离。",
    "zh-HK":
      "抱歉，最近的左右測量結果不一致。我們可以再試一次嗎？閉上眼睛。專注於十字頭髮，並從眼角監視閃爍的點。向左和向右滑動點，直到它消失在盲點中，然後擊中返回。在測試雙眼時，保持觀看距離。",
    ja: "申し訳ありませんが、最新の左右の測定値は一貫していませんでした。もう一度やり直してもいいですか？片目を閉じてください。交差毛に焦点を合わせ、目の隅から点滅するドットを監視します。ブラインドスポットに消えてしまうまで、ドットを左右にスライドさせ、リターンを押します。両目をテストするときに、視聴距離を維持します。",
    ko: "죄송합니다. 가장 최근의 왼쪽 및 오른쪽 측정은 일관성이 없었습니다. 다시 시도해도 될까요? 한쪽 눈을 닫으십시오. 십자 머리카락에 초점을 맞추고 눈 모서리에서 번쩍이는 점을 모니터링하십시오. 점을 왼쪽과 오른쪽으로 맹목 스팟으로 사라질 때까지 왼쪽과 오른쪽으로 밀어 넣은 다음 리턴을 누르십시오. 양쪽 눈을 테스트 할 때 시청 거리를 유지하십시오.",
  },
  RC_viewingDistance: {
    "en-US": "Viewing Distance",
    de: "Betrachtungsabstand",
    fr: "Distance de visualisation",
    es: "Distancia de observación",
    pt: "Distância de visualização",
    it: "Distanza di visualizzazione",
    ro: "Distanța de vizualizare",
    pl: "Odległość oglądania",
    ru: "Просмотр расстояния",
    hy: "Դիտողության հեռավորությունը",
    fi: "Katseluetäisyys",
    ka: "ხედვის მანძილი",
    he: "מרחק צפייה",
    ar: "عرض المسافة",
    ur: "دیکھنے کا فاصلہ",
    hi: "देखने की दूरी",
    ta: "பார்க்கும் தூரம்",
    ml: "ദൂരം കാണുന്നു",
    te: "దూరం చూస్తున్నారు",
    kn: "ದೂರ ವೀಕ್ಷಿಸುವುದು",
    bn: "দূরত্ব দেখার",
    id: "Jarak melihat",
    "zh-CN": "观看距离",
    "zh-HK": "觀看距離",
    ja: "距離の表示",
    ko: "보기 거리",
  },
  RC_viewingDistanceIntro: {
    "en-US":
      "EXPLANATION. For this step, come closer, if necessary, so you're no farther than twice the screen width. Every eye has a blindspot. We'll find yours to discover how far your eye is from the screen. You'll be asked to close or cover one eye to isolate the other. You'll use your keyboard's arrow keys ← → to slide the flickering dot left and right until it disappears into your blindspot. You'll do this with each eye, alternating, until the last two measurements are consistent. Please maintain your viewing distance as you test both eyes. NOTE. If the dot never disappears, it means that your eye is following the dot, instead of staying on the cross hairs. This test requires that you keep your eye on the cross hairs while monitoring the flickering dot in the corner of your eye. That's hard at first, but becomes easier with practice.",
    de: "ERLÄUTERUNG. Kommen Sie für diesen Schritt bei Bedarf näher, damit Sie nicht weiter als doppelt so hoch wie die Bildschirmbreite sind. Jedes Auge hat einen Blindspot. Wir werden Ihre finden, um herauszufinden, wie weit Ihr Auge vom Bildschirm vom Bildschirm entfernt ist. Sie werden gebeten, ein Auge zu schließen oder abzudecken, um das andere zu isolieren. Sie verwenden die Pfeiltasten Ihrer Tastatur ← → um den flackernden Punkt nach links und rechts zu schieben, bis er in Ihren Blindspot verschwindet. Sie werden dies mit jedem Auge wechseln, bis die letzten beiden Messungen konsistent sind. Bitte behalten Sie Ihren Betrachtungsabstand bei, während Sie beide Augen testen. NOTIZ. Wenn der Punkt nie verschwindet, bedeutet dies, dass Ihr Auge dem Punkt folgt, anstatt auf den Kreuzhaaren zu bleiben. Dieser Test erfordert, dass Sie die Kreuzhaare im Auge behalten, während Sie den flackernden Punkt in Ihrem Augenwinkel überwachen. Das ist zunächst schwierig, wird aber beim Üben einfacher.",
    fr: "EXPLICATION. Pour cette étape, approchez-vous, si nécessaire, vous n'êtes donc pas plus loin que deux fois la largeur de l'écran. Chaque œil a un point mort. Nous trouverons le vôtre pour découvrir à quel point votre œil est de l'écran. On vous demandera de fermer ou de couvrir un œil pour isoler l'autre. Vous utiliserez les touches fléchées de votre clavier ← → Pour glisser le point scintillant à gauche et à droite jusqu'à ce qu'il disparaisse dans votre point mort. Vous le ferez avec chaque œil, en alternant, jusqu'à ce que les deux dernières mesures soient cohérentes. Veuillez maintenir votre distance de visualisation lorsque vous testez les deux yeux. NOTE. Si le point ne disparaît jamais, cela signifie que votre œil suit le point, au lieu de rester sur les poils. Ce test nécessite de garder un œil sur les poils tout en surveillant le point vacillant dans le coin de votre œil. C'est difficile au début, mais devient plus facile avec la pratique.",
    es: "EXPLICACIÓN. Para este paso, acércate, si es necesario, por lo que no estás más lejos del doble del ancho de la pantalla. Cada ojo tiene un punto ciego. Encontraremos el suyo para descubrir qué tan lejos está su ojo desde la pantalla. Se le pedirá que cierre o cubra un ojo para aislar al otro. Usará las teclas de flecha de su teclado ← → para deslizar el punto parpadeante a la izquierda y a la derecha hasta que desaparezca en su punto ciego. Hará esto con cada ojo, alternando, hasta que las dos últimas medidas sean consistentes. Mantenga su distancia de visión mientras prueba ambos ojos. NOTA. Si el punto nunca desaparece, significa que su ojo está siguiendo el punto, en lugar de permanecer en la cruz. Esta prueba requiere que mantenga su ojo en la cruz mientras monitorea el punto parpadeante en la esquina de su ojo. Eso es difícil al principio, pero se vuelve más fácil con la práctica.",
    pt: "EXPLICAÇÃO. Para esta etapa, chegue mais perto, se necessário, para que você não esteja mais longe do que o dobro da largura da tela. Todo olho tem um ponto cego. Vamos encontrar o seu para descobrir até que ponto seus olhos estão da tela. Você será solicitado a fechar ou cobrir um olho para isolar o outro. Você usará as teclas de seta do teclado ← → para deslizar o ponto de tremeling para a esquerda e para a direita até que ele desapareça no seu ponto cego. Você fará isso a cada olho, alternando, até que as duas últimas medições sejam consistentes. Por favor, mantenha sua distância de visualização enquanto você testa os dois olhos. OBSERVAÇÃO. Se o ponto nunca desaparece, significa que seu olho está seguindo o ponto, em vez de ficar nos cabelos cruzados. Este teste requer que você fique de olho nos cabelos cruzados enquanto monitora o ponto tremeluzente no canto do olho. Isso é difícil no começo, mas se torna mais fácil com a prática.",
    it: "SPIEGAZIONE. Per questo passaggio, avvicinati, se necessario, quindi non sei più lontano del doppio della larghezza dello schermo. Ogni occhio ha un cieco. Troveremo il tuo per scoprire quanto è lontano il tuo occhio dallo schermo. Ti verrà chiesto di chiudere o coprire un occhio per isolare l'altro. Utilizzerai i tasti freccia della tastiera ← → Per far scorrere il punto tremolante a sinistra e a destra fino a quando non scompare nel tuo cieco. Lo farai con ogni occhio, alternando, fino a quando le ultime due misurazioni sono coerenti. Si prega di mantenere la distanza di visualizzazione mentre si verificano entrambi gli occhi. NOTA. Se il punto non scompare mai, significa che il tuo occhio sta seguendo il punto, invece di rimanere sui peli incrociati. Questo test richiede di tenere d'occhio i peli incrociati mentre monitora il punto tremolo nell'angolo dell'occhio. All'inizio è difficile, ma diventa più facile con la pratica.",
    ro: "EXPLICAŢIE. Pentru acest pas, apropiați -vă, dacă este necesar, deci nu sunteți mai departe de două ori mai mult decât lățimea ecranului. Fiecare ochi are un orb. Vom găsi al tău pentru a descoperi cât de departe este ochiul tău de ecran. Vi se va cere să închideți sau să acoperiți un ochi pentru a -l izola pe celălalt. Veți utiliza tastele de săgeată ale tastaturii dvs. ← → Pentru a glisa punctul pâlpâit la stânga și la dreapta până când va dispărea în Blindspot. Veți face acest lucru cu fiecare ochi, alternând, până când ultimele două măsurători sunt consecvente. Vă rugăm să mențineți distanța de vizionare în timp ce testați ambii ochi. NOTĂ. Dacă punctul nu dispare niciodată, înseamnă că ochiul tău urmărește punctul, în loc să rămâi pe părul încrucișat. Acest test necesită să vă țineți cu ochii pe părul încrucișat în timp ce monitorizați punctul pâlpâitor din colțul ochiului. La început este greu, dar devine mai ușor cu practica.",
    pl: "WYJAŚNIENIE. W tym kroku w razie potrzeby zbliżaj się bliżej, więc nie jesteś dalej niż dwa razy większa niż szerokość ekranu. Każde oko ma ślepy. Znajdziemy twoje, aby dowiedzieć się, jak daleko jest twoje oko od ekranu. Zostaniesz poproszony o zamknięcie lub zakrycie jednego oka, aby odizolować drugie. Użyjesz klawiszy strzałek klawiatury ← →, aby przesunąć migotającą kropkę w lewo i w prawo, aż zniknie w twojej ślepej. Zrobisz to z każdym okiem, na przemian, aż dwa ostatnie pomiary będą spójne. Proszę zachować odległość oglądania podczas testowania obu oczu. NOTATKA. Jeśli kropka nigdy się nie znika, oznacza to, że twoje oko podąża za kropką, zamiast pozostać na krzyżowych włosach. Ten test wymaga, aby mieć oko na włosy krzyżowe podczas monitorowania migoczącej kropki w rogu oka. Na początku jest to trudne, ale staje się łatwiejsze w praktyce.",
    ru: "ОБЪЯСНЕНИЕ. Для этого шага при необходимости приходите ближе, так что вы не дальше, чем вдвое больше ширины экрана. У каждого глаза есть слепой точка зрения. Мы найдем ваш, чтобы узнать, как далеко ваш взгляд от экрана. Вас попросят закрыть или прикрыть один глаз, чтобы изолировать другого. Вы используете клавиши со стрелками клавиатуры ← →, чтобы сдвинуть мерцающую точку влево и вправо, пока она не исчезнет в вашей слепой точке. Вы делаете это с каждым глазом, чередующиеся, пока последние два измерения не станут последовательными. Пожалуйста, сохраните свое просмотр, когда вы тестируете оба глаза. ПРИМЕЧАНИЕ. Если точка никогда не исчезает, это означает, что ваш глаз следит за точкой, вместо того, чтобы оставаться на кросс -волосках. Этот тест требует, чтобы вы следили за кросс -волосами, следив за мерцающей точкой в ​​углу глаза. Сначала это сложно, но с практикой становится легче.",
    hy: "Բացատրություն: Այս քայլի համար ավելի մոտենալ, անհրաժեշտության դեպքում, այնպես որ էկրանի երկու անգամ ավելի հեռու եք: Յուրաքանչյուր աչք ունի կույր: Մենք կգտնենք ձերը `հայտնաբերելու համար, թե որքանով է ձեր աչքը էկրանից: Ձեզանից կառաջարկվի փակել կամ ծածկել մեկ աչքը `մյուսին մեկուսացնելու համար: Դուք կօգտագործեք ձեր ստեղնաշարի սլաքի ստեղները ← → դեպի սահելու համար ձախ եւ աջ, մինչեւ այն անհետանա ձեր կույր: Դուք դա կանեք յուրաքանչյուր աչքով, փոխարինելով, մինչեւ վերջին երկու չափումները հետեւողական լինեն: Խնդրում ենք պահպանել ձեր դիտման հեռավորությունը, երբ փորձարկում եք երկու աչքերը: ՆՇՈՒՄ. Եթե ​​կետը երբեք չի վերանում, նշանակում է, որ ձեր աչքը հետեւում է կետին, փոխարենը մնալով խաչմերուկի վրա: Այս թեստը պահանջում է, որ ձեր աչքը պահեք խաչմերուկի վրա, մինչդեռ ձեր աչքի անկյունում դիտորդական կետը դիտարկելը: Սկզբում դժվար է, բայց դյուրին է դառնում պրակտիկայով:",
    fi: "SELITYS. Tätä vaihetta varten tule tarvittaessa lähemmäksi, joten et ole kauempana kuin kaksinkertainen näytön leveys. Jokaisessa silmässä on blindspot. Löydämme sinun selvittää, kuinka kaukana silmäsi on näytöltä. Sinua pyydetään sulkemaan tai peittämään yksi silmä eristääksesi toisen. Käytä näppäimistön nuolinäppäimiä ← → liu'uttaa vilkkuvan pisteen vasemmalle ja oikealle, kunnes se katoaa blindspotisi. Teet tämän jokaisella silmällä vuorotellen, kunnes kaksi viimeistä mittausta ovat yhdenmukaisia. Pidä katseluetäisyys testattaessa molemmat silmät. HUOMAUTUS. Jos piste ei koskaan katoa, se tarkoittaa, että silmäsi seuraa pistettä sen sijaan, että pysyisivät ristikästen päällä. Tämä testi edellyttää, että pidät silmäsi ristiinkarvoissa tarkkailemalla silmän nurkassa olevaa vilkkuvaa pistettä. Se on aluksi vaikeaa, mutta siitä tulee helpompaa harjoittelussa.",
    ka: "ახსნა. ამ ნაბიჯისთვის, მიუახლოვდით, საჭიროების შემთხვევაში, ასე რომ, თქვენ არ ხართ უფრო შორს, ვიდრე ორჯერ ეკრანის სიგანე. ყველა თვალს აქვს ჟალუზი. ჩვენ ვიპოვით თქვენს, რომ გაირკვეს, თუ რამდენად შორს არის თქვენი თვალი ეკრანიდან. თქვენ მოგეთხოვებათ ერთი თვალის დახუჭვა ან დაფარვა, რომ მეორეს იზოლირება მოახდინოთ. თქვენ გამოიყენებთ თქვენი კლავიშის ისრის კლავიშებს ← →, რომ მბზინავი წერტილი მარცხნივ და მარჯვნივ გადაიტანოთ, სანამ ის არ გაქრება თქვენს ჟალუზაში. ამას გააკეთებთ თითოეული თვალით, მონაცვლეობით, სანამ ბოლო ორი გაზომვა არ შეესაბამება. გთხოვთ, შეინარჩუნოთ თქვენი სანახავი მანძილი, როდესაც ორივე თვალს შეამოწმებთ. ᲨᲔᲜᲘᲨᲕᲜᲐ. თუ წერტილი არასდროს ქრება, ეს ნიშნავს, რომ თქვენი თვალი მიჰყვება წერტილს, ნაცვლად იმისა, რომ ჯვრის თმებზე დარჩით. ეს ტესტი მოითხოვს, რომ თვალის დახუჭვა თვალის კუთხეში მბზინავი წერტილის მონიტორინგის დროს. ეს თავიდან რთულია, მაგრამ პრაქტიკაში უფრო ადვილი ხდება.",
    he: "הֶסבֵּר. לשלב זה, התקרב, במידת הצורך, כך שאתה לא רחוק יותר מרוחב המסך. לכל עין יש כתב עיוור. נמצא את שלך כדי לגלות כמה רחוק העין שלך מהמסך. תתבקש לסגור או לכסות עין אחת כדי לבודד את השני. תשתמש במקשי החצים של המקלדת שלך ← → כדי להחליק את הנקודה המהבהבת שמאלה וימינה עד שהוא ייעלם לתוך העיוור שלך. תעשה זאת עם כל עין, לסירוגין, עד ששתי המדידות האחרונות יהיו עקביות. אנא שמור על מרחק הצפייה שלך כשאתה בודק את שתי העיניים. הערה. אם הנקודה לעולם לא נעלמת, פירוש הדבר שהעין שלך עוקבת אחר הנקודה, במקום להישאר על שערות הצלב. מבחן זה מחייב שתפקח עין על שערות הצלב תוך כדי פיקוח על הנקודה המהבהבת בפינת העין שלך. זה קשה בהתחלה, אבל הופך להיות קל יותר עם תרגול.",
    ar: "توضيح. لهذه الخطوة ، اقترب أكثر ، إذا لزم الأمر ، لذلك أنت لست أكثر من ضعف عرض الشاشة. كل عين لديها مجموعة أعمى. سنجد لك لاكتشاف مدى انتباهك عن الشاشة. يُطلب منك إغلاق أو تغطية عين واحدة لعزل الآخر. ستستخدم مفاتيح سهم لوحة المفاتيح ← → لنقل النقطة الخفية إلى اليسار واليمين حتى تختفي في لعبة BlindSpot. ستقوم بذلك مع كل عين ، بالتناوب ، حتى يتسق القياسين الأخيرين. يرجى الحفاظ على مسافة المشاهدة الخاصة بك أثناء اختبار كلتا العينين. ملحوظة. إذا لم تختفي النقطة أبدًا ، فهذا يعني أن عينيك تتبع النقطة ، بدلاً من البقاء على الشعر المتقاطع. يتطلب هذا الاختبار أن تراقب عن الشعر المتقاطع أثناء مراقبة النقطة الوهمية في زاوية عينيك. هذا صعب في البداية ، لكنه يصبح أسهل مع الممارسة.",
    ur: "وضاحت اس مرحلے کے ل ، ، اگر ضروری ہو تو قریب آجائیں ، لہذا آپ اسکرین کی چوڑائی سے دوگنا نہیں ہیں۔ ہر آنکھ میں ایک بلائنڈ اسپاٹ ہوتا ہے۔ ہم آپ کو یہ معلوم کرنے کے ل find آپ کو تلاش کریں گے کہ آپ کی آنکھ اسکرین سے کتنی دور ہے۔ دوسرے کو الگ تھلگ کرنے کے لئے آپ سے ایک آنکھ بند کرنے یا ڈھانپنے کے لئے کہا جائے گا۔ جب تک یہ آپ کے بلائنڈ اسپاٹ میں غائب نہ ہوجائے تب تک آپ اپنے کی بورڈ کی تیر والے چابیاں ← tl ٹمٹماہٹ ڈاٹ بائیں اور دائیں سلائیڈ کرنے کے لئے استعمال کریں گے۔ جب تک آخری دو پیمائش مستقل نہ ہوں تب تک آپ ہر آنکھ کے ساتھ یہ کام کریں گے۔ جب آپ دونوں آنکھوں کی جانچ کرتے ہیں تو براہ کرم اپنے دیکھنے کا فاصلہ برقرار رکھیں۔ نوٹ. اگر ڈاٹ کبھی غائب نہیں ہوتا ہے تو ، اس کا مطلب یہ ہے کہ آپ کی آنکھ کراس بالوں پر رہنے کی بجائے ڈاٹ کی پیروی کررہی ہے۔ اس امتحان کا تقاضا ہے کہ آپ اپنی آنکھ کے کونے میں ٹمٹماہٹ ڈاٹ کی نگرانی کرتے ہوئے کراس بالوں پر نگاہ رکھیں۔ یہ سب سے پہلے مشکل ہے ، لیکن پریکٹس کے ساتھ آسان ہوجاتا ہے۔",
    hi: "व्याख्या। इस कदम के लिए, यदि आवश्यक हो, तो करीब आएं, इसलिए आप स्क्रीन की चौड़ाई से दोगुने से दूर नहीं हैं। हर आंख में एक अंधा होता है। हम आपको यह पता लगाने के लिए पाएंगे कि आपकी आंख स्क्रीन से कितनी दूर है। आपको दूसरे को अलग करने के लिए एक आंख को बंद करने या कवर करने के लिए कहा जाएगा। आप अपने कीबोर्ड की तीर कुंजियों का उपयोग करेंगे → → जब तक कि यह आपके ब्लाइंडस्पॉट में गायब न हो जाए, तब तक टिमटिमाती डॉट को स्लाइड करने के लिए। आप प्रत्येक आंख के साथ ऐसा करेंगे, बारी -बारी से, जब तक कि अंतिम दो माप सुसंगत न हों। कृपया अपनी देखने की दूरी बनाए रखें क्योंकि आप दोनों आंखों का परीक्षण करते हैं। टिप्पणी। यदि डॉट कभी गायब नहीं होता है, तो इसका मतलब है कि आपकी आंख क्रॉस हेयर पर रहने के बजाय डॉट का अनुसरण कर रही है। इस परीक्षण के लिए आवश्यक है कि आप अपनी आंख के कोने में टिमटिमाती डॉट की निगरानी करते हुए क्रॉस हेयर पर अपनी नज़र रखें। यह पहली बार में कठिन है, लेकिन अभ्यास के साथ आसान हो जाता है।",
    ta: "விளக்கம். இந்த படிக்கு, தேவைப்பட்டால், நெருக்கமாக வாருங்கள், எனவே நீங்கள் திரை அகலத்தை விட இரண்டு மடங்கு அதிகமாக இல்லை. ஒவ்வொரு கண்ணுக்கும் ஒரு குருட்டு இடம் உள்ளது. திரையில் இருந்து உங்கள் கண் எவ்வளவு தூரம் இருக்கிறது என்பதைக் கண்டறிய உங்களுடையதைக் கண்டுபிடிப்போம். மற்றொன்றை தனிமைப்படுத்த ஒரு கண்ணை மூட அல்லது மறைக்கும்படி கேட்கப்படுவீர்கள். உங்கள் விசைப்பலகையின் அம்பு விசைகளைப் பயன்படுத்துவீர்கள் ← the ஒளிபரப்பும் புள்ளியை இடது மற்றும் வலதுபுறமாக உங்கள் பிளைண்ட்ஸ்பாட்டில் மறைந்துவிடும் வரை சரிய பயன்படுத்துவீர்கள். கடைசி இரண்டு அளவீடுகள் சீரானதாக இருக்கும் வரை, ஒவ்வொரு கண்ணிலும் இதைச் செய்வீர்கள். நீங்கள் இரு கண்களையும் சோதிக்கும்போது உங்கள் பார்வை தூரத்தை பராமரிக்கவும். குறிப்பு. புள்ளி ஒருபோதும் மறைந்துவிடாவிட்டால், குறுக்கு முடிகளில் தங்குவதற்கு பதிலாக, உங்கள் கண் புள்ளியைப் பின்பற்றுகிறது என்று அர்த்தம். இந்த சோதனைக்கு உங்கள் கண்ணின் மூலையில் உள்ள ஒளிரும் புள்ளியைக் கண்காணிக்கும் போது குறுக்கு முடிகள் மீது உங்கள் கண் வைத்திருக்க வேண்டும். இது முதலில் கடினம், ஆனால் நடைமுறையில் எளிதாகிறது.",
    ml: "വിശദീകരണം. ഈ ഘട്ടത്തിനായി, ആവശ്യമെങ്കിൽ, നിങ്ങൾ രണ്ടുതവണ സ്ക്രീൻ വീതിയേക്കാൾ കൂടുതൽ ദൂരെയാണ്. ഓരോ കണ്ണിനും ഒരു കണ്ണടപ്പോണ്ട് ഉണ്ട്. സ്ക്രീനിൽ നിന്ന് നിങ്ങളുടെ കണ്ണ് എത്ര ദൂരെയാണെന്ന് കണ്ടെത്താൻ ഞങ്ങൾ നിങ്ങളുടേത് കണ്ടെത്തും. മറ്റൊന്ന് ഒറ്റപ്പെടുത്താൻ ഒരു കണ്ണ് അടയ്ക്കാനോ മൂടാനോ നിങ്ങളോട് ആവശ്യപ്പെടും. നിങ്ങളുടെ കീബോർഡിന്റെ അമ്പടയാള കീകൾ ഉപയോഗിക്കും ← ← → നിങ്ങളുടെ ബ്ലൈൻഡ്സ്പോട്ടിലേക്ക് അപ്രത്യക്ഷമാകുന്നതുവരെ ഇടത്, വലത് വലിക്കുക. ഓരോ കണ്ണിലും നിങ്ങൾ ഇത് ചെയ്യുന്നത്, മാറിമാറി, അവസാന രണ്ട് അളവുകൾ സ്ഥിരീകരിക്കുന്നതുവരെ. നിങ്ങൾ രണ്ട് കണ്ണുകളും പരീക്ഷിക്കുമ്പോൾ നിങ്ങളുടെ കാണൽ ദൂരം നിലനിർത്തുക. കുറിപ്പ്. ഡോട്ട് ഒരിക്കലും അപ്രത്യക്ഷമാകില്ലെങ്കിൽ, ക്രോസ് ഹെയർസിൽ താമസിക്കുന്നതിനുപകരം നിങ്ങളുടെ കണ്ണ് ഡോട്ടിനെ പിന്തുടരുകയാണെന്ന് ഇതിനർത്ഥം. നിങ്ങളുടെ കണ്ണിന്റെ മൂലയിൽ മിന്നുന്ന ഡോട്ട് നിരീക്ഷിക്കുമ്പോൾ ക്രോസ് ഹെറികളിൽ നിങ്ങളുടെ കണ്ണ് സൂക്ഷിക്കുക എന്നതിന് ഈ പരിശോധന ആവശ്യമാണ്. അത് ആദ്യം ബുദ്ധിമുട്ടാണ്, പക്ഷേ പരിശീലനത്തിലൂടെ എളുപ്പമാകും.",
    te: "వివరణ. ఈ దశ కోసం, దగ్గరకు రండి, అవసరమైతే, మీరు స్క్రీన్ వెడల్పు కంటే రెండు రెట్లు ఎక్కువ కాదు. ప్రతి కంటికి బ్లైండ్ స్పాట్ ఉంటుంది. స్క్రీన్ నుండి మీ కన్ను ఎంత దూరంలో ఉందో తెలుసుకోవడానికి మేము మీదిని కనుగొంటాము. మరొకటి వేరుచేయడానికి ఒక కన్ను మూసివేయడానికి లేదా కవర్ చేయమని మిమ్మల్ని అడుగుతారు. మీరు మీ కీబోర్డ్ బాణం కీలను ఉపయోగిస్తారు ← Your మీ బ్లైండ్‌స్పాట్‌లో అదృశ్యమయ్యే వరకు మినుకుమినుకుమనే చుక్కను ఎడమ మరియు కుడి వైపుకు స్లైడ్ చేయడానికి. చివరి రెండు కొలతలు స్థిరంగా ఉండే వరకు మీరు ప్రతి కన్నుతో దీన్ని చేస్తారు. మీరు రెండు కళ్ళను పరీక్షిస్తున్నప్పుడు దయచేసి మీ వీక్షణ దూరాన్ని కొనసాగించండి. గమనిక. చుక్క ఎప్పుడూ కనిపించకపోతే, మీ కన్ను క్రాస్ హెయిర్స్ మీద ఉండటానికి బదులుగా చుక్కను అనుసరిస్తుందని అర్థం. ఈ పరీక్షలో మీరు మీ కంటి మూలలో ఉన్న మినుకుమినుకుమనే చుక్కను పర్యవేక్షించేటప్పుడు క్రాస్ హెయిర్‌లపై మీ కన్ను వేసి ఉంచాలి. ఇది మొదట కష్టం, కానీ ప్రాక్టీస్‌తో సులభం అవుతుంది.",
    kn: "ವಿವರಣೆ. ಈ ಹಂತಕ್ಕಾಗಿ, ಅಗತ್ಯವಿದ್ದರೆ ಹತ್ತಿರ ಬನ್ನಿ, ಆದ್ದರಿಂದ ನೀವು ಪರದೆಯ ಅಗಲಕ್ಕಿಂತ ಎರಡು ಪಟ್ಟು ಹೆಚ್ಚು ದೂರವಿರುವುದಿಲ್ಲ. ಪ್ರತಿ ಕಣ್ಣಿಗೆ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್ ಇದೆ. ನಿಮ್ಮ ಕಣ್ಣು ಪರದೆಯಿಂದ ಎಷ್ಟು ದೂರದಲ್ಲಿದೆ ಎಂಬುದನ್ನು ಕಂಡುಹಿಡಿಯಲು ನಾವು ನಿಮ್ಮದನ್ನು ಕಂಡುಕೊಳ್ಳುತ್ತೇವೆ. ಇನ್ನೊಂದನ್ನು ಪ್ರತ್ಯೇಕಿಸಲು ಒಂದು ಕಣ್ಣನ್ನು ಮುಚ್ಚಲು ಅಥವಾ ಮುಚ್ಚಲು ನಿಮ್ಮನ್ನು ಕೇಳಲಾಗುತ್ತದೆ. ನಿಮ್ಮ ಕೀಬೋರ್ಡ್‌ನ ಬಾಣದ ಕೀಲಿಗಳನ್ನು ನಿಮ್ಮ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್‌ಗೆ ಕಣ್ಮರೆಯಾಗುವವರೆಗೆ ಎಡ ಮತ್ತು ಬಲಕ್ಕೆ ಮಿನುಗುವ ಚುಕ್ಕೆ ಸ್ಲೈಡ್ ಮಾಡಲು ನೀವು ಬಳಸುತ್ತೀರಿ. ಕೊನೆಯ ಎರಡು ಅಳತೆಗಳು ಸ್ಥಿರವಾಗುವವರೆಗೆ ನೀವು ಇದನ್ನು ಪ್ರತಿ ಕಣ್ಣಿನಿಂದ ಮಾಡುತ್ತೀರಿ. ನೀವು ಎರಡೂ ಕಣ್ಣುಗಳನ್ನು ಪರೀಕ್ಷಿಸುವಾಗ ದಯವಿಟ್ಟು ನಿಮ್ಮ ವೀಕ್ಷಣೆಯ ದೂರವನ್ನು ಕಾಪಾಡಿಕೊಳ್ಳಿ. ಸೂಚನೆ. ಡಾಟ್ ಎಂದಿಗೂ ಕಣ್ಮರೆಯಾಗದಿದ್ದರೆ, ಅಡ್ಡ ಕೂದಲಿನ ಮೇಲೆ ಉಳಿಯುವ ಬದಲು ನಿಮ್ಮ ಕಣ್ಣು ಚುಕ್ಕೆ ಅನುಸರಿಸುತ್ತಿದೆ ಎಂದರ್ಥ. ಈ ಪರೀಕ್ಷೆಯು ನಿಮ್ಮ ಕಣ್ಣಿನ ಮೂಲೆಯಲ್ಲಿ ಮಿನುಗುವ ಚುಕ್ಕೆಯನ್ನು ಮೇಲ್ವಿಚಾರಣೆ ಮಾಡುವಾಗ ಅಡ್ಡ ಕೂದಲಿನ ಮೇಲೆ ಕಣ್ಣಿಡಬೇಕು. ಅದು ಮೊದಲಿಗೆ ಕಷ್ಟ, ಆದರೆ ಅಭ್ಯಾಸದೊಂದಿಗೆ ಸುಲಭವಾಗುತ್ತದೆ.",
    bn: "ব্যাখ্যা। এই পদক্ষেপের জন্য, প্রয়োজনে আরও কাছাকাছি আসুন, সুতরাং আপনি স্ক্রিনের প্রস্থের দ্বিগুণের চেয়ে বেশি কিছু নয়। প্রতিটি চোখের একটি অন্ধস্পট আছে। স্ক্রিন থেকে আপনার চোখ কত দূরে তা আবিষ্কার করতে আমরা আপনার সন্ধান করব। অন্যটিকে বিচ্ছিন্ন করতে আপনাকে এক চোখ বন্ধ করতে বা cover াকতে বলা হবে। আপনি আপনার ব্লাইন্ডস্পটে অদৃশ্য না হওয়া পর্যন্ত বাম এবং ডানদিকে ঝাঁকুনির বিন্দুটি স্লাইড করতে আপনার কীবোর্ডের তীর কীগুলি ← use ব্যবহার করবেন। শেষ দুটি পরিমাপ সামঞ্জস্যপূর্ণ না হওয়া পর্যন্ত আপনি প্রতিটি চোখ দিয়ে এটি করবেন। আপনি উভয় চোখ পরীক্ষা করার সাথে সাথে আপনার দেখার দূরত্ব বজায় রাখুন। বিঃদ্রঃ. যদি বিন্দুটি কখনই অদৃশ্য না হয় তবে এর অর্থ হ'ল আপনার চোখ ক্রস চুলগুলিতে থাকার পরিবর্তে বিন্দু অনুসরণ করছে। এই পরীক্ষার জন্য আপনার চোখের কোণে ঝলকানি বিন্দু পর্যবেক্ষণ করার সময় আপনি ক্রস কেশগুলিতে নজর রাখেন। এটি প্রথমে শক্ত, তবে অনুশীলনের সাথে আরও সহজ হয়ে যায়।",
    id: "PENJELASAN. Untuk langkah ini, datang lebih dekat, jika perlu, jadi Anda tidak lebih dari dua kali lebar layar. Setiap mata memiliki buta. Kami akan menemukan milik Anda untuk menemukan seberapa jauh mata Anda dari layar. Anda akan diminta untuk menutup atau menutupi satu mata untuk mengisolasi yang lain. Anda akan menggunakan tombol panah keyboard Anda ← → → Untuk menggeser titik berkedip ke kiri dan kanan sampai menghilang ke dalam blindspot Anda. Anda akan melakukan ini dengan masing -masing mata, bergantian, sampai dua pengukuran terakhir konsisten. Harap pertahankan jarak menonton Anda saat Anda menguji kedua mata. CATATAN. Jika titik tidak pernah menghilang, itu berarti mata Anda mengikuti titik, alih -alih tetap di rambut salib. Tes ini mengharuskan Anda mengawasi rambut silang sambil memantau titik -titik yang berkedip di sudut mata Anda. Itu sulit pada awalnya, tetapi menjadi lebih mudah dengan latihan.",
    "zh-CN":
      "解释。对于此步骤，如有必要，靠近，因此您的屏幕宽度不超过两倍。每只眼睛都有一个盲点。我们会发现您的眼睛离屏幕有多远。您会被要求闭上或遮住一只眼睛以隔离另一只眼睛。您将使用键盘的箭头键←→向左和向右滑动闪烁的点，直到它消失在盲点中​​。您将用每只眼睛交替进行此操作，直到最后两个测量一致。测试双眼时，请保持观看距离。笔记。如果点永远不会消失，则意味着您的眼睛正在跟随点，而不是留在十字头发上。该测试要求您在监视眼角的闪烁点时密切注意十字头发。一开始很难，但是练习变得更加容易。",
    "zh-HK":
      "解釋。對於此步驟，如有必要，靠近，因此您的屏幕寬度不超過兩倍。每隻眼睛都有一個盲點。我們會發現您的眼睛離屏幕有多遠。您會被要求閉上或遮住一隻眼睛以隔離另一隻眼睛。您將使用鍵盤的箭頭鍵←→向左和向右滑動閃爍的點，直到它消失在盲點中。您將用每隻眼睛交替進行此操作，直到最後兩個測量一致。測試雙眼時，請保持觀看距離。筆記。如果點永遠不會消失，則意味著您的眼睛正在跟隨點，而不是留在十字頭髮上。該測試要求您在監視眼角的閃爍點時密切注意十字頭髮。一開始很難，但是練習變得更加容易。",
    ja: "説明。このステップでは、必要に応じて近づくと、画面の幅の2倍よりも遠くはありません。すべての目には盲点があります。画面からあなたの目がどれだけ離れているかを発見するためにあなたのものを見つけます。片方の目を閉じたり覆ったりするように求められます。キーボードの矢印キー←→→左右に左右にスライドさせると、盲点が消えるまで左右にスライドします。最後の2つの測定値が一貫しているまで、各目でこれを交互に行います。両目をテストするときに、視聴距離を維持してください。ノート。ドットが消えない場合、それはあなたの目が十字毛にとどまるのではなく、ドットに続いていることを意味します。このテストでは、目の隅にあるちらつきのドットを監視しながら、十字毛に目を向ける必要があります。最初は難しいですが、練習すると簡単になります。",
    ko: "설명. 이 단계에서는 필요한 경우 더 가까워 지므로 화면 너비의 두 배 이상 더 멀지 않습니다. 모든 눈에는 사각 지대가 있습니다. 우리는 당신의 눈이 화면에서 얼마나 멀리 떨어져 있는지 알게 될 것입니다. 다른 눈을 닫거나 덮으려면 다른 눈을 분리해야합니다. 키보드의 화살표 키를 사용하여 → 깜박 거리는 점을 사각 지대로 사라질 때까지 깜박 거리는 점을 왼쪽과 오른쪽으로 밀어 넣습니다. 마지막 두 측정이 일관 될 때까지 번갈아 가며 각 눈으로이를 수행합니다. 두 눈을 테스트 할 때 시청 거리를 유지하십시오. 메모. 점이 사라지지 않으면, 그것은 당신의 눈이 십자형 머리카락에 머무르는 대신 점을 따르고 있음을 의미합니다. 이 테스트를 위해서는 눈 구석에 깜박 거리는 점을 모니터링하면서 십자 머리카락을 주시해야합니다. 처음에는 어렵지만 연습으로 더 쉬워집니다.",
  },
  RC_viewingDistanceIntroTitle: {
    "en-US": "Explanation",
    de: "Erläuterung",
    fr: "Explication",
    es: "Explicación",
    pt: "Explicação",
    it: "Spiegazione",
    ro: "Explicaţie",
    pl: "Wyjaśnienie",
    ru: "Объяснение",
    hy: "Բացատրություն",
    fi: "Selitys",
    ka: "ახსნა",
    he: "הֶסבֵּר",
    ar: "توضيح",
    ur: "وضاحت",
    hi: "व्याख्या",
    ta: "விளக்கம்",
    ml: "വിശദീകരണം",
    te: "వివరణ",
    kn: "ವಿವರಣೆ",
    bn: "ব্যাখ্যা",
    id: "Penjelasan",
    "zh-CN": "解释",
    "zh-HK": "解釋",
    ja: "説明",
    ko: "설명",
  },
  RC_viewingDistanceIntroLiMethod: {
    "en-US":
      "EXPLANATION. For this step, come closer, if necessary, so you're no farther than twice the screen width. Every eye has a blindspot. We'll find yours to discover how far your eye is from the screen. You'll be asked to close or cover one eye to isolate the other. While you focus on the cross hairs, watch the flickering dot out of the corner of your eye, and hit RETURN when it disappears into your blindspot. Depending on viewing distance and screen size, the flickering dot typically will reach your blindspot before it gets to the middle of the screen. You'll do this with each eye, alternating, until the last two measurements are consistent. Please maintain your viewing distance as you test both eyes. NOTE. If the dot never disappears, it means that your eye is following the dot, instead of staying on the cross hairs. This test requires that you keep your eye on the cross hairs while monitoring the flickering dot in the corner of your eye. That's hard at first, but becomes easier with practice.",
    de: "ERLÄUTERUNG. Kommen Sie für diesen Schritt bei Bedarf näher, damit Sie nicht weiter als doppelt so hoch wie die Bildschirmbreite sind. Jedes Auge hat einen Blindspot. Wir werden Ihre finden, um herauszufinden, wie weit Ihr Auge vom Bildschirm vom Bildschirm entfernt ist. Sie werden gebeten, ein Auge zu schließen oder abzudecken, um das andere zu isolieren. Während Sie sich auf die Kreuzhaare konzentrieren, beobachten Sie den flackernden Punkt aus dem Augenwinkel und treffen Sie die Rückkehr, wenn er in Ihrem Blindspot verschwindet. Abhängig von der Betrachtung der Entfernung und der Bildschirmgröße erreicht der flackernde Punkt normalerweise Ihren Blindspot, bevor er in die Mitte des Bildschirms gelangt. Sie werden dies mit jedem Auge wechseln, bis die letzten beiden Messungen konsistent sind. Bitte behalten Sie Ihren Betrachtungsabstand bei, während Sie beide Augen testen. NOTIZ. Wenn der Punkt nie verschwindet, bedeutet dies, dass Ihr Auge dem Punkt folgt, anstatt auf den Kreuzhaaren zu bleiben. Dieser Test erfordert, dass Sie die Kreuzhaare im Auge behalten, während Sie den flackernden Punkt in Ihrem Augenwinkel überwachen. Das ist zunächst schwierig, wird aber beim Üben einfacher.",
    fr: "EXPLICATION. Pour cette étape, approchez-vous, si nécessaire, vous n'êtes donc pas plus loin que deux fois la largeur de l'écran. Chaque œil a un point mort. Nous trouverons le vôtre pour découvrir à quel point votre œil est de l'écran. On vous demandera de fermer ou de couvrir un œil pour isoler l'autre. Pendant que vous vous concentrez sur les cheveux, regardez le point scintillant du coin de votre œil et appuyez sur votre retour lorsqu'il disparaît dans votre point mort. Selon la distance de visualisation et la taille de l'écran, le point scintillant atteindra généralement votre point mort avant qu'il n'atteigne au milieu de l'écran. Vous le ferez avec chaque œil, en alternant, jusqu'à ce que les deux dernières mesures soient cohérentes. Veuillez maintenir votre distance de visualisation lorsque vous testez les deux yeux. NOTE. Si le point ne disparaît jamais, cela signifie que votre œil suit le point, au lieu de rester sur les poils. Ce test nécessite de garder un œil sur les poils tout en surveillant le point vacillant dans le coin de votre œil. C'est difficile au début, mais devient plus facile avec la pratique.",
    es: "EXPLICACIÓN. Para este paso, acércate, si es necesario, por lo que no estás más lejos del doble del ancho de la pantalla. Cada ojo tiene un punto ciego. Encontraremos el suyo para descubrir qué tan lejos está su ojo desde la pantalla. Se le pedirá que cierre o cubra un ojo para aislar al otro. Mientras te enfocas en la cruz, mira el punto parpadeante en la esquina del ojo y presiona retorno cuando desaparezca en tu punto ciego. Dependiendo de la distancia de vista y el tamaño de la pantalla, el punto parpadeante generalmente llegará a su punto ciego antes de que llegue al centro de la pantalla. Hará esto con cada ojo, alternando, hasta que las dos últimas medidas sean consistentes. Mantenga su distancia de visión mientras prueba ambos ojos. NOTA. Si el punto nunca desaparece, significa que su ojo está siguiendo el punto, en lugar de permanecer en la cruz. Esta prueba requiere que mantenga su ojo en la cruz mientras monitorea el punto parpadeante en la esquina de su ojo. Eso es difícil al principio, pero se vuelve más fácil con la práctica.",
    pt: "EXPLICAÇÃO. Para esta etapa, chegue mais perto, se necessário, para que você não esteja mais longe do que o dobro da largura da tela. Todo olho tem um ponto cego. Vamos encontrar o seu para descobrir até que ponto seus olhos estão da tela. Você será solicitado a fechar ou cobrir um olho para isolar o outro. Enquanto você se concentra nos cabelos cruzados, observe o ponto trêmulo pelo canto do olho e acerte o retorno quando ele desaparecer no seu ponto cego. Dependendo da distância de visualização e do tamanho da tela, o ponto de trementação normalmente atinge seu ponto cego antes de chegar ao meio da tela. Você fará isso a cada olho, alternando, até que as duas últimas medições sejam consistentes. Por favor, mantenha sua distância de visualização enquanto você testa os dois olhos. OBSERVAÇÃO. Se o ponto nunca desaparece, significa que seu olho está seguindo o ponto, em vez de ficar nos cabelos cruzados. Este teste requer que você fique de olho nos cabelos cruzados enquanto monitora o ponto tremeluzente no canto do olho. Isso é difícil no começo, mas se torna mais fácil com a prática.",
    it: "SPIEGAZIONE. Per questo passaggio, avvicinati, se necessario, quindi non sei più lontano del doppio della larghezza dello schermo. Ogni occhio ha un cieco. Troveremo il tuo per scoprire quanto è lontano il tuo occhio dallo schermo. Ti verrà chiesto di chiudere o coprire un occhio per isolare l'altro. Mentre ti concentri sui capelli incrociati, guarda il punto tremolante dall'angolo dell'occhio e colpisci ritorna quando scompare nel tuo cieco. A seconda della distanza di visualizzazione e delle dimensioni dello schermo, il punto tremolante in genere raggiungerà il tuo blindspot prima che arrivi al centro dello schermo. Lo farai con ogni occhio, alternando, fino a quando le ultime due misurazioni sono coerenti. Si prega di mantenere la distanza di visualizzazione mentre si verificano entrambi gli occhi. NOTA. Se il punto non scompare mai, significa che il tuo occhio sta seguendo il punto, invece di rimanere sui peli incrociati. Questo test richiede di tenere d'occhio i peli incrociati mentre monitora il punto tremolo nell'angolo dell'occhio. All'inizio è difficile, ma diventa più facile con la pratica.",
    ro: "EXPLICAŢIE. Pentru acest pas, apropiați -vă, dacă este necesar, deci nu sunteți mai departe de două ori mai mult decât lățimea ecranului. Fiecare ochi are un orb. Vom găsi al tău pentru a descoperi cât de departe este ochiul tău de ecran. Vi se va cere să închideți sau să acoperiți un ochi pentru a -l izola pe celălalt. În timp ce vă concentrați pe părul încrucișat, urmăriți punctul pâlpâit din colțul ochiului și lovit -o pe întoarcerea când dispare în Blindspot. În funcție de vizualizarea distanței și a dimensiunii ecranului, punctul pâlpâitor va ajunge de obicei la Blindspot înainte de a ajunge la mijlocul ecranului. Veți face acest lucru cu fiecare ochi, alternând, până când ultimele două măsurători sunt consecvente. Vă rugăm să mențineți distanța de vizionare în timp ce testați ambii ochi. NOTĂ. Dacă punctul nu dispare niciodată, înseamnă că ochiul tău urmărește punctul, în loc să rămâi pe părul încrucișat. Acest test necesită să vă țineți cu ochii pe părul încrucișat în timp ce monitorizați punctul pâlpâitor din colțul ochiului. La început este greu, dar devine mai ușor cu practica.",
    pl: "WYJAŚNIENIE. W tym kroku w razie potrzeby zbliżaj się bliżej, więc nie jesteś dalej niż dwa razy większa niż szerokość ekranu. Każde oko ma ślepy. Znajdziemy twoje, aby dowiedzieć się, jak daleko jest twoje oko od ekranu. Zostaniesz poproszony o zamknięcie lub zakrycie jednego oka, aby odizolować drugie. Podczas gdy skupiasz się na włosach krzyżowych, obserwuj migającą kropkę z rogu oka i uderzaj, gdy zniknie w twojej ślepej. W zależności od odległości wyświetlania i rozmiaru ekranu migocząca kropka zazwyczaj dotrze do Blindspot, zanim dojdzie do środka ekranu. Zrobisz to z każdym okiem, na przemian, aż dwa ostatnie pomiary będą spójne. Proszę zachować odległość oglądania podczas testowania obu oczu. NOTATKA. Jeśli kropka nigdy się nie znika, oznacza to, że twoje oko podąża za kropką, zamiast pozostać na krzyżowych włosach. Ten test wymaga, aby mieć oko na włosy krzyżowe podczas monitorowania migoczącej kropki w rogu oka. Na początku jest to trudne, ale staje się łatwiejsze w praktyce.",
    ru: "ОБЪЯСНЕНИЕ. Для этого шага при необходимости приходите ближе, так что вы не дальше, чем вдвое больше ширины экрана. У каждого глаза есть слепой точка зрения. Мы найдем ваш, чтобы узнать, как далеко ваш взгляд от экрана. Вас попросят закрыть или прикрыть один глаз, чтобы изолировать другого. Пока вы сосредотачиваетесь на кросс -волосах, следите за мерцающей точкой из -за угла глаза и ударяйте возвращение, когда исчезает в вашей слепой точке. В зависимости от расстояния просмотра и размера экрана, мерцающая точка, как правило, достигает вашей слепой точки, прежде чем добраться до середины экрана. Вы делаете это с каждым глазом, чередующиеся, пока последние два измерения не станут последовательными. Пожалуйста, сохраните свое просмотр, когда вы тестируете оба глаза. ПРИМЕЧАНИЕ. Если точка никогда не исчезает, это означает, что ваш глаз следит за точкой, вместо того, чтобы оставаться на кросс -волосках. Этот тест требует, чтобы вы следили за кросс -волосами, следив за мерцающей точкой в ​​углу глаза. Сначала это сложно, но с практикой становится легче.",
    hy: "Բացատրություն: Այս քայլի համար ավելի մոտենալ, անհրաժեշտության դեպքում, այնպես որ էկրանի երկու անգամ ավելի հեռու եք: Յուրաքանչյուր աչք ունի կույր: Մենք կգտնենք ձերը `հայտնաբերելու համար, թե որքանով է ձեր աչքը էկրանից: Ձեզանից կառաջարկվի փակել կամ ծածկել մեկ աչքը `մյուսին մեկուսացնելու համար: Մինչ դուք կենտրոնանում եք խաչի մազերի վրա, դիտեք ձեր աչքի անկյունից դուրս գալու կետը եւ հարվածել վերադառնալիս, երբ այն վերանում է ձեր կույր: Կախված հեռավորության եւ էկրանի չափի դիտումից, շողոքորթ կետը, որպես կանոն, կհասնվի ձեր կույրին, նախքան էկրանի կեսին հասնելը: Դուք դա կանեք յուրաքանչյուր աչքով, փոխարինելով, մինչեւ վերջին երկու չափումները հետեւողական լինեն: Խնդրում ենք պահպանել ձեր դիտման հեռավորությունը, երբ փորձարկում եք երկու աչքերը: ՆՇՈՒՄ. Եթե ​​կետը երբեք չի վերանում, նշանակում է, որ ձեր աչքը հետեւում է կետին, փոխարենը մնալով խաչմերուկի վրա: Այս թեստը պահանջում է, որ ձեր աչքը պահեք խաչմերուկի վրա, մինչդեռ ձեր աչքի անկյունում դիտորդական կետը դիտարկելը: Սկզբում դժվար է, բայց դյուրին է դառնում պրակտիկայով:",
    fi: "SELITYS. Tätä vaihetta varten tule tarvittaessa lähemmäksi, joten et ole kauempana kuin kaksinkertainen näytön leveys. Jokaisessa silmässä on blindspot. Löydämme sinun selvittää, kuinka kaukana silmäsi on näytöltä. Sinua pyydetään sulkemaan tai peittämään yksi silmä eristääksesi toisen. Kun keskityt ristiinkarvoihin, tarkkaile vilkkuvaa pistettä silmän nurkasta ja lyö paluuta, kun se katoaa blindspotisi. Katseluetäisyydestä ja näytön koosta riippuen vilkkuva piste saavuttaa tyypillisesti blindspotin ennen kuin se pääsee näytön keskelle. Teet tämän jokaisella silmällä vuorotellen, kunnes kaksi viimeistä mittausta ovat yhdenmukaisia. Pidä katseluetäisyys testattaessa molemmat silmät. HUOMAUTUS. Jos piste ei koskaan katoa, se tarkoittaa, että silmäsi seuraa pistettä sen sijaan, että pysyisivät ristikästen päällä. Tämä testi edellyttää, että pidät silmäsi ristiinkarvoissa tarkkailemalla silmän nurkassa olevaa vilkkuvaa pistettä. Se on aluksi vaikeaa, mutta siitä tulee helpompaa harjoittelussa.",
    ka: "ახსნა. ამ ნაბიჯისთვის, მიუახლოვდით, საჭიროების შემთხვევაში, ასე რომ, თქვენ არ ხართ უფრო შორს, ვიდრე ორჯერ ეკრანის სიგანე. ყველა თვალს აქვს ჟალუზი. ჩვენ ვიპოვით თქვენს, რომ გაირკვეს, თუ რამდენად შორს არის თქვენი თვალი ეკრანიდან. თქვენ მოგეთხოვებათ ერთი თვალის დახუჭვა ან დაფარვა, რომ მეორეს იზოლირება მოახდინოთ. მიუხედავად იმისა, რომ თქვენ ყურადღებას ამახვილებთ ჯვრის თმებზე, უყურეთ მბზინავი წერტილის თვალის კუთხიდან და მოხვდით, როდესაც ის თქვენს ჟალუზაში ქრება. დამოკიდებულია მანძილზე და ეკრანის ზომიდან, მბზინავი წერტილი, როგორც წესი, მიაღწევს თქვენს ჟალუზას, სანამ ის ეკრანზე შუა პერიოდში მოხვდება. ამას გააკეთებთ თითოეული თვალით, მონაცვლეობით, სანამ ბოლო ორი გაზომვა არ შეესაბამება. გთხოვთ, შეინარჩუნოთ თქვენი სანახავი მანძილი, როდესაც ორივე თვალს შეამოწმებთ. ᲨᲔᲜᲘᲨᲕᲜᲐ. თუ წერტილი არასდროს ქრება, ეს ნიშნავს, რომ თქვენი თვალი მიჰყვება წერტილს, ნაცვლად იმისა, რომ ჯვრის თმებზე დარჩით. ეს ტესტი მოითხოვს, რომ თვალის დახუჭვა თვალის კუთხეში მბზინავი წერტილის მონიტორინგის დროს. ეს თავიდან რთულია, მაგრამ პრაქტიკაში უფრო ადვილი ხდება.",
    he: "הֶסבֵּר. לשלב זה, התקרב, במידת הצורך, כך שאתה לא רחוק יותר מרוחב המסך. לכל עין יש כתב עיוור. נמצא את שלך כדי לגלות כמה רחוק העין שלך מהמסך. תתבקש לסגור או לכסות עין אחת כדי לבודד את השני. בזמן שאתה מתמקד בשערות הצלב, צפה בנקודה המהבהבת מזווית העין שלך, ופגע בחזרה כשהוא נעלם לתוך העיוור שלך. תלוי במרחק הצפייה ובגודל המסך, הנקודה המהבהבת בדרך כלל תגיע לספוט העיוור שלך לפני שהוא יגיע לאמצע המסך. תעשה זאת עם כל עין, לסירוגין, עד ששתי המדידות האחרונות יהיו עקביות. אנא שמור על מרחק הצפייה שלך כשאתה בודק את שתי העיניים. הערה. אם הנקודה לעולם לא נעלמת, פירוש הדבר שהעין שלך עוקבת אחר הנקודה, במקום להישאר על שערות הצלב. מבחן זה מחייב שתפקח עין על שערות הצלב תוך כדי פיקוח על הנקודה המהבהבת בפינת העין שלך. זה קשה בהתחלה, אבל הופך להיות קל יותר עם תרגול.",
    ar: "توضيح. لهذه الخطوة ، اقترب أكثر ، إذا لزم الأمر ، لذلك أنت لست أكثر من ضعف عرض الشاشة. كل عين لديها مجموعة أعمى. سنجد لك لاكتشاف مدى انتباهك عن الشاشة. يُطلب منك إغلاق أو تغطية عين واحدة لعزل الآخر. أثناء التركيز على الشعر المتقاطع ، شاهد النقطة الخفقان من زاوية عينيك ، واضغط على العودة عندما تختفي في لعبة BlindSpot. اعتمادًا على عرض المسافة وحجم الشاشة ، ستصل النقطة الوهمية عادةً إلى النقطة العمياء قبل وصولها إلى منتصف الشاشة. ستقوم بذلك مع كل عين ، بالتناوب ، حتى يتسق القياسين الأخيرين. يرجى الحفاظ على مسافة المشاهدة الخاصة بك أثناء اختبار كلتا العينين. ملحوظة. إذا لم تختفي النقطة أبدًا ، فهذا يعني أن عينيك تتبع النقطة ، بدلاً من البقاء على الشعر المتقاطع. يتطلب هذا الاختبار أن تراقب عن الشعر المتقاطع أثناء مراقبة النقطة الوهمية في زاوية عينيك. هذا صعب في البداية ، لكنه يصبح أسهل مع الممارسة.",
    ur: "وضاحت اس مرحلے کے ل ، ، اگر ضروری ہو تو قریب آجائیں ، لہذا آپ اسکرین کی چوڑائی سے دوگنا نہیں ہیں۔ ہر آنکھ میں ایک بلائنڈ اسپاٹ ہوتا ہے۔ ہم آپ کو یہ معلوم کرنے کے ل find آپ کو تلاش کریں گے کہ آپ کی آنکھ اسکرین سے کتنی دور ہے۔ دوسرے کو الگ تھلگ کرنے کے لئے آپ سے ایک آنکھ بند کرنے یا ڈھانپنے کے لئے کہا جائے گا۔ جب آپ کراس بالوں پر توجہ مرکوز کرتے ہیں تو ، اپنی آنکھ کے کونے سے ٹمٹماہٹ ڈاٹ دیکھیں ، اور جب آپ کے اندھے مقام میں غائب ہوجاتے ہیں تو واپسی کو ماریں۔ فاصلے اور اسکرین کے سائز کو دیکھنے پر منحصر ہے ، ٹمٹماہٹ ڈاٹ عام طور پر اسکرین کے وسط تک پہنچنے سے پہلے آپ کے اندھے مقام پر پہنچ جاتا ہے۔ جب تک آخری دو پیمائش مستقل نہ ہوں تب تک آپ ہر آنکھ کے ساتھ یہ کام کریں گے۔ جب آپ دونوں آنکھوں کی جانچ کرتے ہیں تو براہ کرم اپنے دیکھنے کا فاصلہ برقرار رکھیں۔ نوٹ. اگر ڈاٹ کبھی غائب نہیں ہوتا ہے تو ، اس کا مطلب یہ ہے کہ آپ کی آنکھ کراس بالوں پر رہنے کی بجائے ڈاٹ کی پیروی کررہی ہے۔ اس امتحان کا تقاضا ہے کہ آپ اپنی آنکھ کے کونے میں ٹمٹماہٹ ڈاٹ کی نگرانی کرتے ہوئے کراس بالوں پر نگاہ رکھیں۔ یہ سب سے پہلے مشکل ہے ، لیکن پریکٹس کے ساتھ آسان ہوجاتا ہے۔",
    hi: "व्याख्या। इस कदम के लिए, यदि आवश्यक हो, तो करीब आएं, इसलिए आप स्क्रीन की चौड़ाई से दोगुने से दूर नहीं हैं। हर आंख में एक अंधा होता है। हम आपको यह पता लगाने के लिए पाएंगे कि आपकी आंख स्क्रीन से कितनी दूर है। आपको दूसरे को अलग करने के लिए एक आंख को बंद करने या कवर करने के लिए कहा जाएगा। जब आप क्रॉस हेयर पर ध्यान केंद्रित करते हैं, तो अपनी आंख के कोने से टिमटिमाते हुए डॉट को देखें, और जब यह आपके ब्लाइंडस्पॉट में गायब हो जाता है तो रिटर्न हिट करें। दूरी और स्क्रीन के आकार को देखने के आधार पर, फ़्लिकरिंग डॉट आमतौर पर स्क्रीन के बीच में आने से पहले आपके ब्लाइंडस्पॉट तक पहुंच जाएगा। आप प्रत्येक आंख के साथ ऐसा करेंगे, बारी -बारी से, जब तक कि अंतिम दो माप सुसंगत न हों। कृपया अपनी देखने की दूरी बनाए रखें क्योंकि आप दोनों आंखों का परीक्षण करते हैं। टिप्पणी। यदि डॉट कभी गायब नहीं होता है, तो इसका मतलब है कि आपकी आंख क्रॉस हेयर पर रहने के बजाय डॉट का अनुसरण कर रही है। इस परीक्षण के लिए आवश्यक है कि आप अपनी आंख के कोने में टिमटिमाती डॉट की निगरानी करते हुए क्रॉस हेयर पर अपनी नज़र रखें। यह पहली बार में कठिन है, लेकिन अभ्यास के साथ आसान हो जाता है।",
    ta: "விளக்கம். இந்த படிக்கு, தேவைப்பட்டால், நெருக்கமாக வாருங்கள், எனவே நீங்கள் திரை அகலத்தை விட இரண்டு மடங்கு அதிகமாக இல்லை. ஒவ்வொரு கண்ணுக்கும் ஒரு குருட்டு இடம் உள்ளது. திரையில் இருந்து உங்கள் கண் எவ்வளவு தூரம் இருக்கிறது என்பதைக் கண்டறிய உங்களுடையதைக் கண்டுபிடிப்போம். மற்றொன்றை தனிமைப்படுத்த ஒரு கண்ணை மூட அல்லது மறைக்கும்படி கேட்கப்படுவீர்கள். நீங்கள் குறுக்கு முடிகளில் கவனம் செலுத்தும்போது, ​​உங்கள் கண்ணின் மூலையில் இருந்து ஒளிரும் புள்ளியைப் பாருங்கள், மேலும் அது உங்கள் குருட்டு இடத்திற்குள் மறைந்தவுடன் திரும்பவும். பார்க்கும் தூரம் மற்றும் திரை அளவைப் பொறுத்து, ஒளிரும் புள்ளி பொதுவாக திரையின் நடுவில் வருவதற்கு முன்பு உங்கள் பிளைண்ட்ஸ்பாட்டை அடையும். கடைசி இரண்டு அளவீடுகள் சீரானதாக இருக்கும் வரை, ஒவ்வொரு கண்ணிலும் இதைச் செய்வீர்கள். நீங்கள் இரு கண்களையும் சோதிக்கும்போது உங்கள் பார்வை தூரத்தை பராமரிக்கவும். குறிப்பு. புள்ளி ஒருபோதும் மறைந்துவிடாவிட்டால், குறுக்கு முடிகளில் தங்குவதற்கு பதிலாக, உங்கள் கண் புள்ளியைப் பின்பற்றுகிறது என்று அர்த்தம். இந்த சோதனைக்கு உங்கள் கண்ணின் மூலையில் உள்ள ஒளிரும் புள்ளியைக் கண்காணிக்கும் போது குறுக்கு முடிகள் மீது உங்கள் கண் வைத்திருக்க வேண்டும். இது முதலில் கடினம், ஆனால் நடைமுறையில் எளிதாகிறது.",
    ml: "വിശദീകരണം. ഈ ഘട്ടത്തിനായി, ആവശ്യമെങ്കിൽ, നിങ്ങൾ രണ്ടുതവണ സ്ക്രീൻ വീതിയേക്കാൾ കൂടുതൽ ദൂരെയാണ്. ഓരോ കണ്ണിനും ഒരു കണ്ണടപ്പോണ്ട് ഉണ്ട്. സ്ക്രീനിൽ നിന്ന് നിങ്ങളുടെ കണ്ണ് എത്ര ദൂരെയാണെന്ന് കണ്ടെത്താൻ ഞങ്ങൾ നിങ്ങളുടേത് കണ്ടെത്തും. മറ്റൊന്ന് ഒറ്റപ്പെടുത്താൻ ഒരു കണ്ണ് അടയ്ക്കാനോ മൂടാനോ നിങ്ങളോട് ആവശ്യപ്പെടും. നിങ്ങൾ ക്രോസ് ഹെയറുകളിൽ ശ്രദ്ധ കേന്ദ്രീകരിക്കുമ്പോൾ, നിങ്ങളുടെ കണ്ണിന്റെ മൂലയിൽ നിന്ന് മിന്നുന്ന ഡോട്ട് കാണുക, അത് നിങ്ങളുടെ ബ്ലൈൻഡ്സ്പോട്ടിലേക്ക് അപ്രത്യക്ഷമാകുമ്പോൾ മടങ്ങുക. ദൂരത്തെയും സ്ക്രീൻ വലുപ്പത്തെയും ആശ്രയിച്ച്, ഫ്ലിക്കറിംഗ് ഡോട്ട് സ്ക്രീനിന്റെ മധ്യത്തിൽ ലഭിക്കുന്നതിന് മുമ്പ് സാധാരണയായി നിങ്ങളുടെ അന്ധതകളിൽ എത്തും. ഓരോ കണ്ണിലും നിങ്ങൾ ഇത് ചെയ്യുന്നത്, മാറിമാറി, അവസാന രണ്ട് അളവുകൾ സ്ഥിരീകരിക്കുന്നതുവരെ. നിങ്ങൾ രണ്ട് കണ്ണുകളും പരീക്ഷിക്കുമ്പോൾ നിങ്ങളുടെ കാണൽ ദൂരം നിലനിർത്തുക. കുറിപ്പ്. ഡോട്ട് ഒരിക്കലും അപ്രത്യക്ഷമാകില്ലെങ്കിൽ, ക്രോസ് ഹെയർസിൽ താമസിക്കുന്നതിനുപകരം നിങ്ങളുടെ കണ്ണ് ഡോട്ടിനെ പിന്തുടരുകയാണെന്ന് ഇതിനർത്ഥം. നിങ്ങളുടെ കണ്ണിന്റെ മൂലയിൽ മിന്നുന്ന ഡോട്ട് നിരീക്ഷിക്കുമ്പോൾ ക്രോസ് ഹെറികളിൽ നിങ്ങളുടെ കണ്ണ് സൂക്ഷിക്കുക എന്നതിന് ഈ പരിശോധന ആവശ്യമാണ്. അത് ആദ്യം ബുദ്ധിമുട്ടാണ്, പക്ഷേ പരിശീലനത്തിലൂടെ എളുപ്പമാകും.",
    te: "వివరణ. ఈ దశ కోసం, దగ్గరకు రండి, అవసరమైతే, మీరు స్క్రీన్ వెడల్పు కంటే రెండు రెట్లు ఎక్కువ కాదు. ప్రతి కంటికి బ్లైండ్ స్పాట్ ఉంటుంది. స్క్రీన్ నుండి మీ కన్ను ఎంత దూరంలో ఉందో తెలుసుకోవడానికి మేము మీదిని కనుగొంటాము. మరొకటి వేరుచేయడానికి ఒక కన్ను మూసివేయడానికి లేదా కవర్ చేయమని మిమ్మల్ని అడుగుతారు. మీరు క్రాస్ హెయిర్స్ పై దృష్టి పెడుతున్నప్పుడు, మీ కంటి మూలలో నుండి మినుకుమినుకుమనే చుక్కను చూడండి మరియు మీ బ్లైండ్ స్పాట్ లోకి అదృశ్యమైనప్పుడు రిటర్న్ కొట్టండి. దూరం మరియు స్క్రీన్ పరిమాణాన్ని చూడటం మీద ఆధారపడి, మినుకుమినుకుమనే చుక్క సాధారణంగా మీ బ్లైండ్‌స్పాట్‌ను స్క్రీన్ మధ్యలో చేరుకోవడానికి ముందు చేరుకుంటుంది. చివరి రెండు కొలతలు స్థిరంగా ఉండే వరకు మీరు ప్రతి కన్నుతో దీన్ని చేస్తారు. మీరు రెండు కళ్ళను పరీక్షిస్తున్నప్పుడు దయచేసి మీ వీక్షణ దూరాన్ని కొనసాగించండి. గమనిక. చుక్క ఎప్పుడూ కనిపించకపోతే, మీ కన్ను క్రాస్ హెయిర్స్ మీద ఉండటానికి బదులుగా చుక్కను అనుసరిస్తుందని అర్థం. ఈ పరీక్షలో మీరు మీ కంటి మూలలో ఉన్న మినుకుమినుకుమనే చుక్కను పర్యవేక్షించేటప్పుడు క్రాస్ హెయిర్‌లపై మీ కన్ను వేసి ఉంచాలి. ఇది మొదట కష్టం, కానీ ప్రాక్టీస్‌తో సులభం అవుతుంది.",
    kn: "ವಿವರಣೆ. ಈ ಹಂತಕ್ಕಾಗಿ, ಅಗತ್ಯವಿದ್ದರೆ ಹತ್ತಿರ ಬನ್ನಿ, ಆದ್ದರಿಂದ ನೀವು ಪರದೆಯ ಅಗಲಕ್ಕಿಂತ ಎರಡು ಪಟ್ಟು ಹೆಚ್ಚು ದೂರವಿರುವುದಿಲ್ಲ. ಪ್ರತಿ ಕಣ್ಣಿಗೆ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್ ಇದೆ. ನಿಮ್ಮ ಕಣ್ಣು ಪರದೆಯಿಂದ ಎಷ್ಟು ದೂರದಲ್ಲಿದೆ ಎಂಬುದನ್ನು ಕಂಡುಹಿಡಿಯಲು ನಾವು ನಿಮ್ಮದನ್ನು ಕಂಡುಕೊಳ್ಳುತ್ತೇವೆ. ಇನ್ನೊಂದನ್ನು ಪ್ರತ್ಯೇಕಿಸಲು ಒಂದು ಕಣ್ಣನ್ನು ಮುಚ್ಚಲು ಅಥವಾ ಮುಚ್ಚಲು ನಿಮ್ಮನ್ನು ಕೇಳಲಾಗುತ್ತದೆ. ನೀವು ಅಡ್ಡ ಕೂದಲಿನ ಮೇಲೆ ಕೇಂದ್ರೀಕರಿಸುವಾಗ, ನಿಮ್ಮ ಕಣ್ಣಿನ ಮೂಲೆಯಿಂದ ಮಿನುಗುವ ಚುಕ್ಕೆ ನೋಡಿ, ಮತ್ತು ಅದು ನಿಮ್ಮ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್‌ಗೆ ಕಣ್ಮರೆಯಾದಾಗ ರಿಟರ್ನ್ ಒತ್ತಿರಿ. ದೂರ ಮತ್ತು ಪರದೆಯ ಗಾತ್ರವನ್ನು ನೋಡುವುದರ ಆಧಾರದ ಮೇಲೆ, ಮಿನುಗುವ ಚುಕ್ಕೆ ಸಾಮಾನ್ಯವಾಗಿ ಪರದೆಯ ಮಧ್ಯಕ್ಕೆ ಬರುವ ಮೊದಲು ನಿಮ್ಮ ಬ್ಲೈಂಡ್‌ಸ್ಪಾಟ್ ಅನ್ನು ತಲುಪುತ್ತದೆ. ಕೊನೆಯ ಎರಡು ಅಳತೆಗಳು ಸ್ಥಿರವಾಗುವವರೆಗೆ ನೀವು ಇದನ್ನು ಪ್ರತಿ ಕಣ್ಣಿನಿಂದ ಮಾಡುತ್ತೀರಿ. ನೀವು ಎರಡೂ ಕಣ್ಣುಗಳನ್ನು ಪರೀಕ್ಷಿಸುವಾಗ ದಯವಿಟ್ಟು ನಿಮ್ಮ ವೀಕ್ಷಣೆಯ ದೂರವನ್ನು ಕಾಪಾಡಿಕೊಳ್ಳಿ. ಸೂಚನೆ. ಡಾಟ್ ಎಂದಿಗೂ ಕಣ್ಮರೆಯಾಗದಿದ್ದರೆ, ಅಡ್ಡ ಕೂದಲಿನ ಮೇಲೆ ಉಳಿಯುವ ಬದಲು ನಿಮ್ಮ ಕಣ್ಣು ಚುಕ್ಕೆ ಅನುಸರಿಸುತ್ತಿದೆ ಎಂದರ್ಥ. ಈ ಪರೀಕ್ಷೆಯು ನಿಮ್ಮ ಕಣ್ಣಿನ ಮೂಲೆಯಲ್ಲಿ ಮಿನುಗುವ ಚುಕ್ಕೆಯನ್ನು ಮೇಲ್ವಿಚಾರಣೆ ಮಾಡುವಾಗ ಅಡ್ಡ ಕೂದಲಿನ ಮೇಲೆ ಕಣ್ಣಿಡಬೇಕು. ಅದು ಮೊದಲಿಗೆ ಕಷ್ಟ, ಆದರೆ ಅಭ್ಯಾಸದೊಂದಿಗೆ ಸುಲಭವಾಗುತ್ತದೆ.",
    bn: "ব্যাখ্যা। এই পদক্ষেপের জন্য, প্রয়োজনে আরও কাছাকাছি আসুন, সুতরাং আপনি স্ক্রিনের প্রস্থের দ্বিগুণের চেয়ে বেশি কিছু নয়। প্রতিটি চোখের একটি অন্ধস্পট আছে। স্ক্রিন থেকে আপনার চোখ কত দূরে তা আবিষ্কার করতে আমরা আপনার সন্ধান করব। অন্যটিকে বিচ্ছিন্ন করতে আপনাকে এক চোখ বন্ধ করতে বা cover াকতে বলা হবে। আপনি ক্রস কেশগুলিতে ফোকাস করার সময়, আপনার চোখের কোণ থেকে ঝলকানি বিন্দুটি দেখুন এবং এটি আপনার ব্লাইন্ডস্পটে অদৃশ্য হয়ে গেলে ফিরে আঘাত করুন। দূরত্ব এবং পর্দার আকার দেখার উপর নির্ভর করে, ঝাঁকুনির বিন্দু সাধারণত স্ক্রিনের মাঝখানে পৌঁছানোর আগে আপনার ব্লাইন্ডস্পটে পৌঁছে যায়। শেষ দুটি পরিমাপ সামঞ্জস্যপূর্ণ না হওয়া পর্যন্ত আপনি প্রতিটি চোখ দিয়ে এটি করবেন। আপনি উভয় চোখ পরীক্ষা করার সাথে সাথে আপনার দেখার দূরত্ব বজায় রাখুন। বিঃদ্রঃ. যদি বিন্দুটি কখনই অদৃশ্য না হয় তবে এর অর্থ হ'ল আপনার চোখ ক্রস চুলগুলিতে থাকার পরিবর্তে বিন্দু অনুসরণ করছে। এই পরীক্ষার জন্য আপনার চোখের কোণে ঝলকানি বিন্দু পর্যবেক্ষণ করার সময় আপনি ক্রস কেশগুলিতে নজর রাখেন। এটি প্রথমে শক্ত, তবে অনুশীলনের সাথে আরও সহজ হয়ে যায়।",
    id: "PENJELASAN. Untuk langkah ini, datang lebih dekat, jika perlu, jadi Anda tidak lebih dari dua kali lebar layar. Setiap mata memiliki buta. Kami akan menemukan milik Anda untuk menemukan seberapa jauh mata Anda dari layar. Anda akan diminta untuk menutup atau menutupi satu mata untuk mengisolasi yang lain. Saat Anda fokus pada rambut salib, saksikan titik -titik yang berkedip -kedip dari sudut mata Anda, dan tekan kembali saat menghilang ke dalam blindspot Anda. Bergantung pada jarak menonton dan ukuran layar, titik -titik yang berkedip biasanya akan mencapai titik blindspot Anda sebelum sampai ke tengah layar. Anda akan melakukan ini dengan masing -masing mata, bergantian, sampai dua pengukuran terakhir konsisten. Harap pertahankan jarak menonton Anda saat Anda menguji kedua mata. CATATAN. Jika titik tidak pernah menghilang, itu berarti mata Anda mengikuti titik, alih -alih tetap di rambut salib. Tes ini mengharuskan Anda mengawasi rambut silang sambil memantau titik -titik yang berkedip di sudut mata Anda. Itu sulit pada awalnya, tetapi menjadi lebih mudah dengan latihan.",
    "zh-CN":
      "解释。对于此步骤，如有必要，靠近，因此您的屏幕宽度不超过两倍。每只眼睛都有一个盲点。我们会发现您的眼睛离屏幕有多远。您会被要求闭上或遮住一只眼睛以隔离另一只眼睛。当您专注于十字头发时，请观察眼睛角落的闪烁点，并在盲点消失时击中回报。根据观看距离和屏幕尺寸，闪烁的点通常会在盲点到达屏幕中间之前到达盲点。您将用每只眼睛交替进行此操作，直到最后两个测量一致。测试双眼时，请保持观看距离。笔记。如果点永远不会消失，则意味着您的眼睛正在跟随点，而不是留在十字头发上。该测试要求您在监视眼角的闪烁点时密切注意十字头发。一开始很难，但是练习变得更加容易。",
    "zh-HK":
      "解釋。對於此步驟，如有必要，靠近，因此您的屏幕寬度不超過兩倍。每隻眼睛都有一個盲點。我們會發現您的眼睛離屏幕有多遠。您會被要求閉上或遮住一隻眼睛以隔離另一隻眼睛。當您專注於十字頭髮時，請觀察眼睛角落的閃爍點，並在盲點消失時擊中回報。根據觀看距離和屏幕尺寸，閃爍的點通常會在盲點到達屏幕中間之前到達盲點。您將用每隻眼睛交替進行此操作，直到最後兩個測量一致。測試雙眼時，請保持觀看距離。筆記。如果點永遠不會消失，則意味著您的眼睛正在跟隨點，而不是留在十字頭髮上。該測試要求您在監視眼角的閃爍點時密切注意十字頭髮。一開始很難，但是練習變得更加容易。",
    ja: "説明。このステップでは、必要に応じて近づくと、画面の幅の2倍よりも遠くはありません。すべての目には盲点があります。画面からあなたの目がどれだけ離れているかを発見するためにあなたのものを見つけます。片方の目を閉じたり覆ったりするように求められます。交差毛に焦点を合わせている間、目の隅からちらつきのドットを見て、それがあなたの盲点に消えたときに戻ってきてください。距離と画面のサイズの表示に応じて、通常、画面の中央に到達する前に、ちらつきのドットが盲点に到達します。最後の2つの測定値が一貫しているまで、各目でこれを交互に行います。両目をテストするときに、視聴距離を維持してください。ノート。ドットが消えない場合、それはあなたの目が十字毛にとどまるのではなく、ドットに続いていることを意味します。このテストでは、目の隅にあるちらつきのドットを監視しながら、十字毛に目を向ける必要があります。最初は難しいですが、練習すると簡単になります。",
    ko: "설명. 이 단계에서는 필요한 경우 더 가까워 지므로 화면 너비의 두 배 이상 더 멀지 않습니다. 모든 눈에는 사각 지대가 있습니다. 우리는 당신의 눈이 화면에서 얼마나 멀리 떨어져 있는지 알게 될 것입니다. 다른 눈을 닫거나 덮으려면 다른 눈을 분리해야합니다. 십자 머리카락에 집중하는 동안 눈 모퉁이에서 깜박 거리는 점을보고, 사각 지대로 사라질 때 귀환을 누르십시오. 볼거리와 화면 크기를 볼 때, 깜박 거리는 점은 일반적으로 화면 중앙에 도달하기 전에 사각 지대에 도달합니다. 마지막 두 측정이 일관 될 때까지 번갈아 가며 각 눈으로이를 수행합니다. 두 눈을 테스트 할 때 시청 거리를 유지하십시오. 메모. 점이 사라지지 않으면, 그것은 당신의 눈이 십자형 머리카락에 머무르는 대신 점을 따르고 있음을 의미합니다. 이 테스트를 위해서는 눈 구석에 깜박 거리는 점을 모니터링하면서 십자 머리카락을 주시해야합니다. 처음에는 어렵지만 연습으로 더 쉬워집니다.",
  },
  RC_viewingDistanceRedo: {
    "en-US": "Redo last response",
    de: "Letzte Antwort wiederholen",
    fr: "Refaire la dernière réponse",
    es: "Rehacer la última respuesta",
    pt: "Refazer a última resposta",
    it: "Rifiuta l'ultima risposta",
    ro: "Redo ultimul răspuns",
    pl: "Rówić ostatnią odpowiedź",
    ru: "Повторить последний ответ",
    hy: "Redo Վերջին պատասխանը",
    fi: "Suunnittele viimeinen vastaus",
    ka: "გადაკეთება ბოლო პასუხი",
    he: "לבצע מחדש את התגובה האחרונה",
    ar: "إعادة الاستجابة الأخيرة",
    ur: "آخری جواب دوبارہ کریں",
    hi: "पुनर्वितरण अंतिम प्रतिक्रिया",
    ta: "கடைசி பதிலை மீண்டும் செய்யுங்கள்",
    ml: "അവസാന പ്രതികരണം വീണ്ടും ചെയ്യുക",
    te: "చివరి ప్రతిస్పందనను పునరావృతం చేయండి",
    kn: "ಕೊನೆಯ ಪ್ರತಿಕ್ರಿಯೆಯನ್ನು ಪುನಃ ಮಾಡಿ",
    bn: "শেষ প্রতিক্রিয়া আবার",
    id: "Mengulang respons terakhir",
    "zh-CN": "重做上一个",
    "zh-HK": "重做上一個",
    ja: "最後の回答をやり直す",
    ko: "마지막 응답을 다시하십시오",
  },
  RC_viewingDistanceTitle: {
    "en-US": "Measure Viewing Distance",
    de: "Messen Sie den Betrachtungsabstand",
    fr: "Mesurer la distance d'observation",
    es: "Medir la distancia de visualización",
    pt: "Medir a distância de visualização",
    it: "Misurare la distanza di visualizzazione",
    ro: "Măsurați distanța de vizualizare",
    pl: "Zmierz odległość oglądania",
    ru: "Измерение расстояния просмотра",
    hy: "Չափել դիտման հեռավորությունը",
    fi: "Mittaa katseluetäisyys",
    ka: "გაზომეთ სანახავი მანძილი",
    he: "למדוד את מרחק הצפייה",
    ar: "قياس مسافة المشاهدة",
    ur: "دیکھنے کے فاصلے کی پیمائش کریں",
    hi: "देखने की दूरी को मापें",
    ta: "பார்க்கும் தூரத்தை அளவிடவும்",
    ml: "കാണുന്ന ദൂരം അളക്കുക",
    te: "వీక్షణ దూరాన్ని కొలవండి",
    kn: "ವೀಕ್ಷಣೆ ದೂರವನ್ನು ಅಳೆಯಿರಿ",
    bn: "দেখার দূরত্ব পরিমাপ করুন",
    id: "Ukur jarak tampilan",
    "zh-CN": "测量观看距离",
    "zh-HK": "測量觀看距離",
    ja: "表示距離を測定します",
    ko: "시청 거리를 측정하십시오",
  },
  T_beep: {
    "en-US": "Beep",
    de: "Piepton",
    fr: "Bip",
    es: "Bip",
    pt: "Bip",
    it: "Segnalino",
    ro: "Bip",
    pl: "Sygnal dzwiękowy",
    ru: "гудок",
    hy: "Hooouck",
    fi: "Piippaus",
    ka: "BOEP",
    he: "ביפ",
    ar: "Beep",
    ur: "بیپ",
    hi: "बीप",
    ta: "பீப்",
    ml: "ബീപ്പ്",
    te: "బీప్",
    kn: "ಬೀಪ್",
    bn: "বীপ",
    id: "Berbunyi",
    "zh-CN": "哔",
    "zh-HK": "嗶",
    ja: "ピー",
    ko: "삑 하는 소리",
  },
  T_cancel: {
    "en-US": "Cancel",
    de: "Stornieren",
    fr: "Annuler",
    es: "Cancelar",
    pt: "Cancelar",
    it: "Annulla",
    ro: "Anulare",
    pl: "Anuluj",
    ru: "Отмена",
    hy: "Չեղյալ հայտարարում",
    fi: "Peruuttaa",
    ka: "გაუქმება",
    he: "לְבַטֵל",
    ar: "الغاء",
    ur: "بند کریں",
    hi: "बंद करे",
    ta: "ரத்துசெய்",
    ml: "റദ്ദാക്കുക",
    te: "రద్దు చేయండి",
    kn: "ರದ್ದುಮಾಡು",
    bn: "বাতিল",
    id: "Membatalkan",
    "zh-CN": "取消",
    "zh-HK": "取消",
    ja: "キャンセル",
    ko: "취소",
  },
  T_clickingLetter: {
    "en-US": "by clicking it in the displayed list of letters.",
    de: "indem Sie es in der angezeigten Buchstabenliste anklicken.",
    fr: "En cliquant dessus dans la liste des lettres affichées.",
    es: "haciendo clic en ella en la lista de letras que se muestra.",
    pt: "Clicando -o na lista exibida de letras.",
    it: "facendo clic su di essa nell'elenco di lettere visualizzate.",
    ro: "Făcând clic pe lista de litere afișate.",
    pl: "klikając prawidłową literę na liście liter za pomocą myszki",
    ru: "щелкнув по нему в отображаемом списке букв.",
    hy: "Կտտացնելով այն տառերի ցուցադրված ցուցակում:",
    fi: "napsauttamalla sitä näytetyssä kirjainluettelossa.",
    ka: "დააჭირეთ მას ასოების ნაჩვენები სიაში.",
    he: "על ידי לחיצה עליו ברשימת האותיות המוצגת.",
    ar: "بالضغط عليه في قائمة الحروف المعروضة.",
    ur: "خطوط کی ڈسپلے لسٹ پر اس پر کلک کرکے۔",
    hi: "अक्षरों की प्रदर्शित सूची पर इसे क्लिक करके।",
    ta: "கடிதங்களின் காட்டப்பட்ட பட்டியலில் அதைக் கிளிக் செய்வதன் மூலம்.",
    ml: "പ്രദർശിപ്പിച്ച അക്ഷരങ്ങളുടെ പട്ടികയിൽ ക്ലിക്കുചെയ്ത്.",
    te: "ప్రదర్శించిన అక్షరాల జాబితాలో క్లిక్ చేయడం ద్వారా.",
    kn: "ಅದನ್ನು ಪ್ರದರ್ಶಿಸಿದ ಅಕ್ಷರಗಳ ಪಟ್ಟಿಯಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡುವ ಮೂಲಕ.",
    bn: "অক্ষরের প্রদর্শিত তালিকায় এটি ক্লিক করে।",
    id: "dengan mengkliknya dalam daftar huruf yang ditampilkan.",
    "zh-CN": "通过点击给出文字中的相应字母。",
    "zh-HK": "通過點擊給出文字中的相應字母。",
    ja: "表示されている文字のリストをクリックすることで",
    ko: "표시된 문자 목록에서 클릭하여.",
  },
  T_clickingLetterRepeatedLetters: {
    "en-US": "by clicking them in the list of letters.",
    de: "durch Klicken auf sie in der Liste der Buchstaben.",
    fr: "en les cliquant dans la liste des lettres.",
    es: "Al hacer clic en la lista de letras.",
    pt: "clicando neles na lista de letras.",
    it: "Facendo clic su di loro nell'elenco delle lettere.",
    ro: "făcând clic pe ele în lista de litere.",
    pl: "Klikając je na liście liter.",
    ru: "Нажав их в списке букв.",
    hy: "կտտացնելով դրանք տառերի ցանկում:",
    fi: "napsauttamalla niitä kirjainluettelossa.",
    ka: "მათ ასოების სიაში დაჭერით.",
    he: "על ידי לחיצה עליהם ברשימת המכתבים.",
    ar: "من خلال النقر عليها في قائمة الرسائل.",
    ur: "خطوط کی فہرست میں ان پر کلک کرکے۔",
    hi: "उन्हें पत्रों की सूची में क्लिक करके।",
    ta: "கடிதங்களின் பட்டியலில் அவற்றைக் கிளிக் செய்வதன் மூலம்.",
    ml: "അക്ഷരങ്ങളുടെ പട്ടികയിൽ അവ ക്ലിക്കുചെയ്യുന്നതിലൂടെ.",
    te: "అక్షరాల జాబితాలో వాటిని క్లిక్ చేయడం ద్వారా.",
    kn: "ಅವುಗಳನ್ನು ಅಕ್ಷರಗಳ ಪಟ್ಟಿಯಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡುವ ಮೂಲಕ.",
    bn: "চিঠির তালিকায় তাদের ক্লিক করে।",
    id: "dengan mengkliknya dalam daftar huruf.",
    "zh-CN": "通过在字母列表中单击它们。",
    "zh-HK": "通過在字母列表中單擊它們。",
    ja: "文字のリストでそれらをクリックします。",
    ko: "문자 목록에서 클릭하여.",
  },
  T_clickingWordRsvpReading: {
    "en-US": "by clicking them in a list of words.",
    de: "durch Klicken auf sie in einer Liste von Wörtern.",
    fr: "en les cliquant dans une liste de mots.",
    es: "haciendo clic en ellos en una lista de palabras.",
    pt: "clicando neles em uma lista de palavras.",
    it: "Facendo clic su di loro in un elenco di parole.",
    ro: "făcând clic pe ele într -o listă de cuvinte.",
    pl: "Klikając je na liście słów.",
    ru: "Нажав их в списке слов.",
    hy: "Կտտացնելով դրանք բառերի ցանկում:",
    fi: "napsauttamalla niitä sanaluettelossa.",
    ka: "მათი სიტყვების ჩამონათვალში დაჭერით.",
    he: "על ידי לחיצה עליהם ברשימת מילים.",
    ar: "من خلال النقر عليها في قائمة الكلمات.",
    ur: "الفاظ کی فہرست میں ان پر کلک کرکے۔",
    hi: "उन्हें शब्दों की सूची में क्लिक करके।",
    ta: "சொற்களின் பட்டியலில் அவற்றைக் கிளிக் செய்வதன் மூலம்.",
    ml: "അവ വാക്കുകളുടെ പട്ടികയിൽ ക്ലിക്കുചെയ്യുന്നതിലൂടെ.",
    te: "వాటిని పదాల జాబితాలో క్లిక్ చేయడం ద్వారా.",
    kn: "ಅವುಗಳನ್ನು ಪದಗಳ ಪಟ್ಟಿಯಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡುವ ಮೂಲಕ.",
    bn: "শব্দের তালিকায় তাদের ক্লিক করে।",
    id: "dengan mengkliknya dalam daftar kata.",
    "zh-CN": "通过在单词列表中单击它们。",
    "zh-HK": "通過在單詞列表中單擊它們。",
    ja: "単語のリストでそれらをクリックします。",
    ko: "단어 목록에서 클릭하여.",
  },
  T_continueClickProceed: {
    "en-US": "To continue, click PROCEED.",
    de: "Klicken Sie, um fortzufahren.",
    fr: "Pour continuer, cliquez sur continuer.",
    es: "Para continuar, haga clic en continuar.",
    pt: "Para continuar, clique em prosseguir.",
    it: "Per continuare, fare clic su Procedi.",
    ro: "Pentru a continua, faceți clic pe continuați.",
    pl: "Aby kontynuować, kliknij, kontynuuj.",
    ru: "Чтобы продолжить, нажмите «Продолжить».",
    hy: "Շարունակելու համար կտտացրեք Շարունակել:",
    fi: "Jatka napsauttamalla jatka.",
    ka: "გასაგრძელებლად, დააჭირეთ გაგრძელება.",
    he: "להמשך, לחץ על המשך.",
    ar: "للمتابعة ، انقر فوق متابعة.",
    ur: "جاری رکھنے کے لئے ، آگے بڑھیں پر کلک کریں۔",
    hi: "जारी रखने के लिए, आगे बढ़ने पर क्लिक करें।",
    ta: "தொடர, தொடரவும் என்பதைக் கிளிக் செய்க.",
    ml: "തുടരാൻ, തുടരുക ക്ലിക്കുചെയ്യുക.",
    te: "కొనసాగించడానికి, కొనసాగండి క్లిక్ చేయండి.",
    kn: "ಮುಂದುವರಿಸಲು, ಮುಂದುವರಿಯಿರಿ ಕ್ಲಿಕ್ ಮಾಡಿ.",
    bn: "চালিয়ে যেতে, এগিয়ে যেতে ক্লিক করুন।",
    id: "Untuk melanjutkan, klik Lanjutkan.",
    "zh-CN": "点击PROCEED以继续。",
    "zh-HK": "點擊PROCEED以繼續。",
    ja: "続行するには、[続行]をクリックします。",
    ko: "계속하려면 진행을 클릭하십시오.",
  },
  T_continueHitReturn: {
    "en-US": "To continue, hit RETURN.",
    de: "Um fortzufahren, drücken Sie bitte RETURN.",
    fr: "Pour continuer, appuyez sur Retour.",
    es: "Para continuar, presione el regreso.",
    pt: "Para continuar, pressione o retorno.",
    it: "Per continuare, premi INVIO.",
    ro: "Pentru a continua, apăsați returul.",
    pl: "Aby kontynuować, naciśnij ENTER.",
    ru: "Чтобы продолжить, нажмите ВОЗВРАТ.",
    hy: "Շարունակելու համար սեղմեք վերադարձը:",
    fi: "Jatkaaksesi paluu.",
    ka: "გასაგრძელებლად, მოხვდით.",
    he: "כדי להמשיך, לחץ על RETURN.",
    ar: "للمتابعة ، يرجى الضغط على زر الرجوع RETURN.",
    ur: "جاری رکھنے کے لئے ، براہ کرم واپسی دبائیں۔",
    hi: "जारी रखने के लिए, कृपया रिटर्न दबाएं।",
    ta: "தொடர, திரும்பவும்.",
    ml: "തുടരുന്നതിന്, തിരിച്ചുവരവ്.",
    te: "కొనసాగించడానికి, రాబడిని కొట్టండి.",
    kn: "ಮುಂದುವರಿಸಲು, ರಿಟರ್ನ್ ಒತ್ತಿರಿ.",
    bn: "চালিয়ে যেতে, হিট রিটার্ন।",
    id: "Untuk melanjutkan, tekan kembali.",
    "zh-CN": "按回车键以继续。",
    "zh-HK": "按回車鍵以繼續。",
    ja: "続行するには、Returnキーを押してください。",
    ko: "계속하려면 반환을 누르십시오.",
  },
  T_continueHitReturnOrClickProceed: {
    "en-US": "To continue, hit RETURN or click PROCEED.",
    de: "Um fortzufahren, klicken Sie auf Rückgabe oder klicken Sie auf fortfahren.",
    fr: "Pour continuer, appuyez sur Retour ou cliquez sur Procéder.",
    es: "Para continuar, presione la devolución o haga clic en continuar.",
    pt: "Para continuar, pressione o retorno ou clique em prosseguir.",
    it: "Per continuare, premi reso o fai clic su Procedi.",
    ro: "Pentru a continua, apăsați Return sau faceți clic pe Continuați.",
    pl: "Aby kontynuować, naciśnij zwrot lub kliknij, kontynuuj.",
    ru: "Чтобы продолжить, нажмите возврат или нажмите, продолжайте.",
    hy: "Շարունակել, հարվածել վերադարձին կամ կտտացնել:",
    fi: "Jatka jatka ja napsauta jatka.",
    ka: "გასაგრძელებლად, დააჭირეთ დაბრუნებას ან დააჭირეთ გაგრძელება.",
    he: "להמשך, לחץ על החזרה או לחץ על המשך.",
    ar: "للمتابعة ، اضغط على العودة أو انقر فوق متابعة.",
    ur: "جاری رکھنے کے لئے ، واپسی کو ہٹائیں یا آگے بڑھیں پر کلک کریں۔",
    hi: "जारी रखने के लिए, रिटर्न हिट करें या आगे बढ़ें पर क्लिक करें।",
    ta: "தொடர, திரும்பவும் அல்லது தொடரவும் என்பதைக் கிளிக் செய்க.",
    ml: "തുടരുന്നതിന്, മടങ്ങിവരവ് അമർത്തുക അല്ലെങ്കിൽ തുടരുക ക്ലിക്കുചെയ്യുക.",
    te: "కొనసాగించడానికి, రిటర్న్ నొక్కండి లేదా కొనసాగండి క్లిక్ చేయండి.",
    kn: "ಮುಂದುವರಿಸಲು, ರಿಟರ್ನ್ ಒತ್ತಿರಿ ಅಥವಾ ಮುಂದುವರಿಯಿರಿ ಕ್ಲಿಕ್ ಮಾಡಿ.",
    bn: "চালিয়ে যেতে, হিট রিটার্ন বা এগিয়ে ক্লিক করুন।",
    id: "Untuk melanjutkan, tekan kembali atau klik lanjutkan.",
    "zh-CN": "按回车键或者点击PROCEED以继续。",
    "zh-HK": "按回車鍵或者點擊PROCEED以繼續。",
    ja: "続行するには、Returnを押すか、[続行]をクリックします。",
    ko: "계속하려면 반품을 누르거나 진행을 클릭하십시오.",
  },
  T_continueNeitherClickNorType: {
    "en-US":
      "Cannot proceed because responseTypedBool and responseClickedBool are both FALSE.",
    de: "Kann nicht fortfahren, da ResponTypedBool und Responsedoklydbool beide falsch sind.",
    fr: "Ne peut pas continuer parce que ResponseTypedBool et ResponseClickedBool sont tous deux faux.",
    es: "No puede proceder porque ResponseTetypedBool y ResponseClickedBool son falsos.",
    pt: "Não pode prosseguir porque o responseTypedBool e o responseClickedBool são falsos.",
    it: "Non può procedere perché ResponseTypedBool e RomesClickedBool sono entrambi falsi.",
    ro: "Nu se poate continua, deoarece RespotyeTypedBool și ResponseClickedBool sunt ambele false.",
    pl: "Nie można kontynuować, ponieważ zarówno odpowiedzialne, jak i responClickedbool są fałszywe.",
    ru: "Не могу продолжить, потому что ответ Speatpedbool и ApproneClickedbool оба ложно.",
    hy: "Հնարավոր չէ շարունակել, քանի որ պատասխանատուբուլը եւ պատասխանողներն են, երկուսն էլ կեղծ են:",
    fi: "Ei voi edetä, koska ResponseTypeDBool ja ResponseClickedBool ovat molemmat vääriä.",
    ka: "არ შეიძლება გაგრძელდეს, რადგან პასუხებიტიპედბოლი და რეაგირებაClickedBool ორივე ყალბია.",
    he: "לא ניתן להמשיך מכיוון ש- RepressEtypedBool ו- ResponseClickedBool שניהם שקריים.",
    ar: "لا يمكن المضي قدمًا لأن ResponseTypedBool و ResponseClickedBool كلاهما خطأ.",
    ur: "آگے نہیں بڑھ سکتے کیونکہ جوابی ٹائپڈ بول اور جوابی کلیکڈ بول دونوں غلط ہیں۔",
    hi: "आगे नहीं बढ़ सकते क्योंकि respentEtypedBool और responslickedBool दोनों झूठे हैं।",
    ta: "தொடர முடியாது, ஏனெனில் ரெஸ்பான்டிபெட்பூல் மற்றும் ரெஸ்பான்சிலிக்இட்பூல் இரண்டும் பொய்யானவை.",
    ml: "പോകാനുള്ള ഉത്തരവാദിത്തങ്ങൾ, റെസ്പോസ്ലിക്ലിക്ഡ്ബൂൾ എന്നിവയും തുടരാൻ കഴിയില്ല.",
    te: "కొనసాగలేరు ఎందుకంటే రెస్పాన్స్‌టైప్డ్ బూల్ మరియు రెస్పాన్స్‌క్లిక్‌బూల్ రెండూ అబద్ధం.",
    kn: "ಮುಂದುವರಿಯಲು ಸಾಧ್ಯವಿಲ್ಲ ಏಕೆಂದರೆ ರೆಸ್ಪಾನ್ಸಿಟೈಪ್ಡ್ಬೂಲ್ ಮತ್ತು ರಿಸ್ಕ್ಲಿಕ್ಡ್ಬೂಲ್ ಎರಡೂ ಸುಳ್ಳು.",
    bn: "অগ্রসর হতে পারে না কারণ প্রতিক্রিয়াশীল বুল এবং প্রতিক্রিয়া কিক্লিকডবুল উভয়ই মিথ্যা।",
    id: "Tidak dapat dilanjutkan karena responsetypedBool dan responseClickedBool keduanya salah.",
    "zh-CN": "无法进行，因为ResponseTypedBool和ResponseClickedBool都是错误的。",
    "zh-HK": "無法進行，因為ResponseTypedBool和ResponseClickedBool都是錯誤的。",
    ja: "ResponseTypedBoolとResponseClickedBoolがどちらも間違っているため、続行できません。",
    ko: "ResponseTypedBool과 ResponseClickedBool은 모두 거짓이므로 진행할 수 없습니다.",
  },
  T_counterBlock: {
    "en-US": "Block xx1 of xx2.",
    de: "Block xx1 von xx2.",
    fr: "Bloc xx1 de xx2.",
    es: "Bloque xx1 de xx2.",
    pt: "Bloco xx1 de xx2.",
    it: "BLOCCO xx1 di xx2.",
    ro: "Blocul xx1 din xx2.",
    pl: "Blok xx1 xx2.",
    ru: "Блок xx1 xx2.",
    hy: "Արգելափակել xx2- ի xx1- ը:",
    fi: "xx2: n lohko xx1.",
    ka: "xx2- ის ბლოკი xx1.",
    he: "חסום xx1 של xx2.",
    ar: "كتلة xx1 من xx2.",
    ur: "xx2 کے xx1 کو مسدود کریں۔",
    hi: "xx2 के xx1 को ब्लॉक करें।",
    ta: "xx2 இன் xx1 தொகுதி.",
    ml: "xx2 ന്റെ xx1 തടയുക.",
    te: "xx2 యొక్క xx1 ని బ్లాక్ చేయండి.",
    kn: "xx2 ನ xx1 ಅನ್ನು ನಿರ್ಬಂಧಿಸಿ.",
    bn: "Xx2 এর xx1 ব্লক করুন।",
    id: "Blokir xx1 dari xx2.",
    "zh-CN": "xx2的xx1块。",
    "zh-HK": "xx2的xx1塊。",
    ja: "xx2のブロックxx1。",
    ko: "xx2의 블록 xx1.",
  },
  T_counterCm: {
    "en-US": " At xxx cm.",
    de: "Bei xxx cm.",
    fr: "Bei xxx cm.",
    es: "A xxx cm.",
    pt: " Em xxx cm.",
    it: " A xxx cm.",
    ro: " La xxx cm.",
    pl: " W xxx cm.",
    ru: " На ххх см.",
    hy: " xxx սմ-ով:",
    fi: " Xxx cm.",
    ka: " xxx სმ -ზე.",
    he: ' בגודל xxx ס"מ.',
    ar: " في xxx س.م.",
    ur: " xxx سینٹی میٹر لیکن",
    hi: " xxx सेमी. पर",
    ta: " xxx CM இல்.",
    ml: " xxx സെന്റിമീറ്ററിൽ.",
    te: " Xxx cm వద్ద.",
    kn: " Xxx cm ನಲ್ಲಿ.",
    bn: " Xxx সেমি এ।",
    id: " Di xxx cm.",
    "zh-CN": " xxx 厘米远。",
    "zh-HK": " xxx 厘米遠。",
    ja: "xxx cmの距離で。",
    ko: " xxx cm에서.",
  },
  T_counterReadingPageBlock: {
    "en-US": "Page xx1 of xx2. Block xx3 of xx4.",
    de: "Seite xx1 von xx2. Block xx3 von xx4.",
    fr: "Page xx1 de xx2. Bloc xx3 de xx4.",
    es: "Pagina xx1 de xx2. Bloque xx3 de xx4.",
    pt: "Página xx1 de xx2. Bloco xx3 de xx4.",
    it: "Pagina xx1 di xx2. BLOCCO xx3 di xx4.",
    ro: "Pagina xx1 din xx2. Blocul xx3 din xx4.",
    pl: "Strona xx1 z xx2. Blok xx3 z xx4.",
    ru: "Страница xx1 xx2. Блок xx3 xx4.",
    hy: "Xx2 էջ xx1: Արգելափակել xx4- ի xx3- ը:",
    fi: "xx2: n sivu xx1. xx4: n lohko xx3.",
    ka: "xx2 გვერდი xx1. xx4- ის ბლოკი xx3.",
    he: "עמוד xx1 של xx2. חסום xx3 של xx4.",
    ar: "الصفحة xx1 من xx2. كتلة xx3 من xx4.",
    ur: "صفحہ xx1 xx2۔ xx4 کے xx3 کو بلاک کریں۔",
    hi: "xx2 का पृष्ठ xx1। xx4 के xx3 को ब्लॉक करें।",
    ta: "xx2 இன் பக்கம் xx1. xx4 இன் xx3 தொகுதி.",
    ml: "xx2 ന്റെ xx1. xx4- ന്റെ xx3 തടയുക.",
    te: "xx2 యొక్క పేజీ xx1. xx4 యొక్క xx3 ని బ్లాక్ చేయండి.",
    kn: "xx2 ನ ಪುಟ xx1. xx4 ನ xx3 ಅನ್ನು ನಿರ್ಬಂಧಿಸಿ.",
    bn: "Xx2 এর xx1 পৃষ্ঠা। Xx4 এর xx3 ব্লক করুন।",
    id: "Halaman xx1 dari xx2. Blokir xx3 dari xx4.",
    "zh-CN": "xx2的第xx1页。 xx4的xx3块。",
    "zh-HK": "xx2的第xx1頁。 xx4的xx3塊。",
    ja: "xx2のページxx1。 xx4のブロックxx3。",
    ko: "xx2의 xx1 페이지. xx4의 블록 xx3.",
  },
  T_counterTrialBlock: {
    "en-US": "Trial xx1 of xx2. Block xx3 of xx4.",
    de: "Testversion xx1 von xx2. Block xx3 von xx4.",
    fr: "Essai xx1 de xx2. Bloc xx3 de xx4.",
    es: "Ensayo xx1 de xx2. Bloque xx3 de xx4.",
    pt: "xx1 Ensaio de xx2. Bloco xx3 de xx4.",
    it: "Prova xx1 di xx2. Blocco xx3 di xx4.",
    ro: "xx1 ESSAY OF xx2. xx3 bloc xx4.",
    pl: "Próba xx1 z xx2. Blok xx3 z xx4. ",
    ru: "Испытание xx1 из xx2. Блок xx3 из xx4.",
    hy: "Test xx1 xx2- ից: xx4- ից արգելափակեք xx3:",
    fi: "xx2: n kokeilu xx1. xx4: n lohko xx3.",
    ka: "xx2- ის საცდელი xx1. xx4- ის ბლოკი xx3.",
    he: "ניסיון xx1 של xx2. בלוק xx3 של xx4.",
    ar: "تجربة xx1 من xx2. مجموعة xx3 من xx4.",
    ur: "xx1 ٹرائل آف ایکس ایکس 2۔ xx4 کا xx3 بلاک۔",
    hi: "xx2 का xx1 ट्राइअल। xx4 का xx3 ब्लॉक।",
    ta: "xx2 இன் சோதனை xx1. xx4 இன் xx3 தொகுதி.",
    ml: "xx2 ലെ ട്രയൽ xx1. xx4- ന്റെ xx3 തടയുക.",
    te: "Xx2 యొక్క ట్రయల్ xx1. xx4 యొక్క xx3 ని బ్లాక్ చేయండి.",
    kn: "xx2 ನ ಪ್ರಯೋಗ xx1. xx4 ನ xx3 ಅನ್ನು ನಿರ್ಬಂಧಿಸಿ.",
    bn: "xx2 এর xx1 ট্রায়াল। Xx4 এর xx3 ব্লক করুন।",
    id: "Trial xx1 dari xx2. Blokir xx3 dari xx4.",
    "zh-CN": "试验xx1，共xx2次试验。 区组xx3，共xx4个区组。",
    "zh-HK": "試驗xx1，共xx2次試驗。區組xx3，共xx4個區組。",
    ja: "xx2のテストxx1。 xx4のブロックxx3。",
    ko: "xx2의 시험 xx1. xx4의 블록 xx3.",
  },
  T_escapeToQuit: {
    "en-US": "(Quit anytime by pressing ESCAPE.)",
    de: "(Beenden Sie jederzeit mit ESCAPE.)",
    fr: "(Arrêtez à tout moment en appuyant sur Escape.)",
    es: "(Deja en cualquier momento presionando Escape).",
    pt: "(Saia a qualquer momento pressionando Escape.)",
    it: "(Smettila in qualsiasi momento premendo la fuga.)",
    ro: "(Renunță oricând apăsând Escape.)",
    pl: "(Zachowaj w dowolnym momencie, naciskając ucieczkę.)",
    ru: "(Выйдите в любое время, нажимая на побег.)",
    hy: "(Հանգստացեք ցանկացած պահի `սեղմելով փախուստը):",
    fi: "(Lopeta milloin tahansa painamalla paeta.)",
    ka: "(დატოვეთ ნებისმიერ დროს გაქცევის დაჭერით.)",
    he: "(צא מכל זמן על ידי לחיצה על בריחה.)",
    ar: "(استقال في أي وقت عن طريق الضغط على الهروب.)",
    ur: "(فرار کو دبانے سے کسی بھی وقت چھوڑ دیں۔)",
    hi: "(पलायन दबाकर कभी भी छोड़ दें।)",
    ta: "(தப்பிப்பதை அழுத்துவதன் மூலம் எப்போது வேண்டுமானாலும் வெளியேறவும்.)",
    ml: "(എസ്കേപ്പ് അമർത്തിക്കൊണ്ട് എപ്പോൾ വേണമെങ്കിലും ഉപേക്ഷിക്കുക.)",
    te: "(ఎస్కేప్ నొక్కడం ద్వారా ఎప్పుడైనా నిష్క్రమించండి.)",
    kn: "(ತಪ್ಪಿಸಿಕೊಳ್ಳುವ ಮೂಲಕ ಯಾವುದೇ ಸಮಯದಲ್ಲಿ ತ್ಯಜಿಸಿ.)",
    bn: "(পালাতে চাপ দিয়ে যে কোনও সময় প্রস্থান করুন))",
    id: "(Berhenti kapan saja dengan menekan Escape.)",
    "zh-CN": "（在任何时候按Esc键以退出。）",
    "zh-HK": "（在任何時候按Esc鍵以退出。）",
    ja: "（エスケープを押していつでも辞めます。）",
    ko: "(탈출을 눌러 언제든지 종료하십시오.)",
  },
  T_generatingMovie: {
    "en-US": "Making movie.",
    de: "Film machen.",
    fr: "Faire du film.",
    es: "Haciendo una película.",
    pt: "Fazendo filme.",
    it: "Fare film.",
    ro: "Realizarea filmului.",
    pl: "Tworzenie filmu.",
    ru: "Сделать фильм.",
    hy: "Ֆիլմ պատրաստելը:",
    fi: "Elokuvan tekeminen.",
    ka: "ფილმის გადაღება.",
    he: "עושה סרט.",
    ar: "صنع الفيلم.",
    ur: "مووی بنانا۔",
    hi: "फिल्म बनाना।",
    ta: "திரைப்படம் தயாரித்தல்.",
    ml: "സിനിമ നിർമ്മിക്കുന്നു.",
    te: "సినిమా తయారు చేయడం.",
    kn: "ಚಲನಚಿತ್ರ ಮಾಡುವುದು.",
    bn: "সিনেমা করা।",
    id: "Membuat film.",
    "zh-CN": "制作电影。",
    "zh-HK": "製作電影。",
    ja: "映画を作る。",
    ko: "영화 제작.",
  },
  T_guessingGame: {
    "en-US":
      "The letter size and spacing vary from big and easy down to tiny and hard or impossible. You can't get much more than two-thirds right, so relax. Think of it as a guessing game, and just get as many as you can. You'll hear a beep whenever you're right.",
    de: "Die Buchstabengröße und der Buchstaben variieren von groß und einfach bis zu winzig und hart oder unmöglich. Sie können nicht viel mehr als zwei Drittel richtig bekommen, also entspannen Sie sich. Betrachten Sie es als ein Ratenspiel und holen Sie sich so viele wie möglich. Sie werden einen Piepton hören, wenn Sie Recht haben.",
    fr: "La taille de la lettre et l'espacement varient de grand et facile à minuscule et difficile ou impossible. Vous ne pouvez pas obtenir beaucoup plus des deux tiers à droite, alors détendez-vous. Considérez-le comme un jeu de supposition et obtenez-en autant que vous le pouvez. Vous entendrez un bip lorsque vous avez raison.",
    es: "El tamaño y el espacio de la letra varían de grande y fácil hasta pequeño y duro o imposible. No puedes tener mucho más de dos tercios, así que relájate. Piense en ello como un juego de adivinanzas, y simplemente obtenga tantas como pueda. Escucharás un pitido siempre que tengas razón.",
    pt: "O tamanho da letra e o espaçamento variam de grande e fácil para baixo a minúsculo e duro ou impossível. Você não pode acertar muito mais do que dois terços, então relaxe. Pense nisso como um jogo de adivinhação e obtenha o máximo possível. Você ouvirá um sinal sonoro sempre que estiver certo.",
    it: "Le dimensioni e lo spazio della lettera variano grandi e facili fino a quando non sono piccoli e difficili o impossibili. Non puoi avere molto più di due terzi, quindi rilassati. Pensaci come un gioco di enigma e ottieni semplicemente il maggior numero possibile. Sentirai un segnale acustico ogni volta che hai ragione.",
    ro: "Mărimea și spațiul literei variază mari și ușoare până la mici și dure sau imposibile. Nu poți avea mult mai mult de două treimi, așa că relaxează -te. Gândiți -vă la asta ca la un joc de ghicitoare și pur și simplu obțineți cât pot. Vei auzi un bip ori de câte ori ai dreptate.",
    pl: "Czasami litera będzie łatwa do zidentyfikowania. Czasami będzie to prawie niemożliwe. Nie odpowiesz poprawnie na więcej niż połowę prób, więc zrelaksuj się. Pomyśl o tym jak o grze do zgadywania i po prostu odpowiedz poprawnie na tyle ile możesz. (Przerwij w dowolnym momencie, naciskając Escape.)\n ",
    ru: "Размер букв и расстояние между буквами изменяется между большими и легкими до крошечного и тяжелого или невозможного. Вы не можете получить гораздо более двух третей, так что расслабьтесь. Подумайте об этом как об угадывании и просто получите столько, сколько сможете. Вы услышите звуковой сигнал, когда вы правы.",
    hy: "Նամակների չափը եւ տառերի միջեւ եղած հեռավորությունը տատանվում է մեծ եւ թոքերի միջեւ դեպի մանր եւ ծանր կամ անհնար: Դուք չեք կարող ստանալ շատ ավելի քան երկու երրորդը, այնպես որ հանգստացեք: Մտածեք դրա մասին, որպես կռահում եւ պարզապես հնարավորինս ստացեք: Դուք կլսեք ձայնային ազդանշան, երբ ճիշտ եք:",
    fi: "Kirjeen koko ja etäisyys vaihtelevat isoista ja helpoista pieneen ja kovaan tai mahdottomaksi. Et voi saada paljon enemmän kuin kaksi kolmasosaa oikein, joten rentoudu. Ajattele sitä arvauspelinä ja hanki vain niin monta kuin pystyt. Kuulet piippauksen aina kun olet oikeassa.",
    ka: "ასოების ზომა და ინტერვალი განსხვავდება დიდი და მარტივი, პატარა და მძიმე და შეუძლებელი. თქვენ ვერ მიიღებთ ორ მესამედზე მეტს, ასე რომ დაისვენოთ. იფიქრეთ მასზე, როგორც გამოცნობის თამაში და უბრალოდ მიიღეთ რაც შეიძლება მეტი. თქვენ მოისმენთ beep, როდესაც მართალი ხართ.",
    he: "גודל המכתב והמרווח משתנים מגדול וקל למטה לקטנטן וקשה או בלתי אפשרי. אתה לא יכול להשיג הרבה יותר משני שלישים נכון, אז תירגע. חשבו על זה כמשחק ניחוש, ופשוט קבלו כמה שיותר. תשמע צפצוף בכל פעם שאתה צודק.",
    ar: "يختلف حجم الحرف وتباعده من كبيرة وسهلة إلى صغيرة وصعبة أو مستحيلة. لا يمكنك الحصول على أكثر من الثلثين بشكل صحيح ، لذا استرخ. فكر في الأمر على أنه لعبة تخمين ، واحصل على أكبر عدد ممكن. ستسمع صوتًا كلما كنت على حق.",
    ur: "خط کا سائز اور خالی جگہ بڑے اور آسان سے چھوٹے اور سخت یا ناممکن سے مختلف ہے۔ آپ کو دو تہائی سے زیادہ نہیں مل سکتے ہیں ، لہذا آرام کریں۔ اس کو تخمینہ کھیل کے طور پر سوچیں ، اور زیادہ سے زیادہ حاصل کریں۔ جب بھی آپ ٹھیک کہتے ہیں ، آپ کو بیپ سنی جائے گی۔",
    hi: "पत्र का आकार और रिक्ति बड़े और आसान से छोटे और कठोर या असंभव से भिन्न होती है। आप दो-तिहाई से अधिक सही नहीं पा सकते हैं, इसलिए आराम करें। इसे एक अनुमान लगाने के खेल के रूप में सोचें, और बस जितने भी हो सके उतने प्राप्त करें। जब भी आप सही हों तो आप एक बीप सुनेंगे।",
    ta: "கடித அளவு மற்றும் இடைவெளி பெரிய மற்றும் எளிதான முதல் சிறிய மற்றும் கடினமான அல்லது சாத்தியமற்றது வரை மாறுபடும். மூன்றில் இரண்டு பங்கிற்கு மேல் நீங்கள் பெற முடியாது, எனவே ஓய்வெடுங்கள். யூகிக்கும் விளையாட்டாக இதை நினைத்துப் பாருங்கள், உங்களால் முடிந்தவரை பலவற்றைப் பெறுங்கள். நீங்கள் சரியாக இருக்கும் போதெல்லாம் ஒரு பீப்பைக் கேட்பீர்கள்.",
    ml: "കത്ത് വലുപ്പവും അകലവും വലുതും എളുപ്പത്തിൽ ചെറിയതും കഠിനവുമായ അല്ലെങ്കിൽ അസാധ്യമാണ്. നിങ്ങൾക്ക് മൂന്നിൽ രണ്ട് വലതുവശത്ത് കൂടുതൽ നേടാൻ കഴിയില്ല, അതിനാൽ വിശ്രമിക്കുക. ഇത് ഒരു ess ഹിക്കുന്ന ഗെയിമായി കരുതുക, നിങ്ങൾക്ക് കഴിയുന്നത്രയും നേടുക. നിങ്ങൾ ശരിയാകുമ്പോഴെല്ലാം നിങ്ങൾ ഒരു ബീപ്പ് കേൾക്കും.",
    te: "అక్షరాల పరిమాణం మరియు అంతరం పెద్ద మరియు తేలికైన నుండి చిన్న మరియు కఠినమైన లేదా అసాధ్యం వరకు మారుతూ ఉంటాయి. మీరు మూడింట రెండు వంతుల కంటే ఎక్కువ పొందలేరు, కాబట్టి విశ్రాంతి తీసుకోండి. దీన్ని game హించే ఆటగా భావించండి మరియు మీకు వీలైనన్ని ఎక్కువ పొందండి. మీరు సరైనప్పుడల్లా బీప్ వింటారు.",
    kn: "ಅಕ್ಷರ ಗಾತ್ರ ಮತ್ತು ಅಂತರವು ದೊಡ್ಡ ಮತ್ತು ಸುಲಭದಿಂದ ಸಣ್ಣ ಮತ್ತು ಕಠಿಣ ಅಥವಾ ಅಸಾಧ್ಯಕ್ಕೆ ಬದಲಾಗುತ್ತದೆ. ನೀವು ಮೂರನೇ ಎರಡರಷ್ಟು ಭಾಗವನ್ನು ಪಡೆಯಲು ಸಾಧ್ಯವಿಲ್ಲ, ಆದ್ದರಿಂದ ವಿಶ್ರಾಂತಿ ಪಡೆಯಿರಿ. ಇದನ್ನು ess ಹಿಸುವ ಆಟವೆಂದು ಯೋಚಿಸಿ, ಮತ್ತು ನಿಮಗೆ ಸಾಧ್ಯವಾದಷ್ಟು ಪಡೆಯಿರಿ. ನೀವು ಸರಿಯಾಗಿದ್ದಾಗಲೆಲ್ಲಾ ನೀವು ಬೀಪ್ ಕೇಳುತ್ತೀರಿ.",
    bn: "অক্ষরের আকার এবং ব্যবধানটি বড় এবং সহজ থেকে ছোট এবং শক্ত বা অসম্ভব থেকে পৃথক। আপনি দুই-তৃতীয়াংশের চেয়ে বেশি কিছু পেতে পারেন না, তাই শিথিল করুন। এটিকে একটি অনুমানের খেলা হিসাবে ভাবেন এবং যতটা পারেন আপনি পান। আপনি যখনই ঠিক আছেন তখন আপনি একটি বীপ শুনতে পাবেন।",
    id: "Ukuran dan jarak huruf bervariasi dari besar dan mudah hingga kecil dan keras atau tidak mungkin. Anda tidak bisa mendapatkan lebih dari dua pertiga, jadi santai. Anggap saja sebagai permainan menebak, dan dapatkan sebanyak yang Anda bisa. Anda akan mendengar bunyi bip kapan pun Anda benar.",
    "zh-CN":
      "字母的大小和间隔是多种多样的，从大而简单到小而困难或不可能。您的正确率不会比三分之二高太多，所以请不要担心。请把这个实验看作一个猜谜游戏，尽您所能即可。当您提供了正确答案时，您将听到“哔”的提示音。",
    "zh-HK":
      "字母的大小和間隔是多種多樣的，從大而簡單到小而困難或不可能。您的正確率不會比三分之二高太多，所以請不要擔心。請把這個實驗看作一個猜謎遊戲，盡您所能即可。當您提供了正確答案時，您將聽到“嗶”的提示音。",
    ja: "文字の識別が容易な時も、不可能な時もあります。半分以上も識別することはできないでしょう。ですから、リラックスしてください。推測ゲームのような感覚で、できる限り多く正解してください。（ESCAPEキーでいつでも中止できます。）",
    ko: "문자 크기와 간격은 크고 쉽게 다르고 작고 단단하거나 불가능합니다. 당신은 3 분의 2 이상을 얻을 수 없으므로 긴장을 풀십시오. 그것을 추측 게임으로 생각하고 가능한 한 많은 것을 얻으십시오. 당신이 옳을 때마다 경고음이 들립니다.",
  },
  T_identifyClickIt: {
    "en-US": "Please identify the middle letter, by clicking it below.",
    de: "Bitte identifizieren Sie den mittleren Buchstaben, indem Sie unten darauf klicken.",
    fr: "Veuillez identifier la lettre du milieu, en cliquant dessus ci-dessous.",
    es: "Identifica la letra del medio haciendo clic en ella abajo.",
    pt: "Identifique a carta do meio, clicando abaixo.",
    it: "Si prega di identificare la lettera centrale, facendo clic sull'alternativa in basso.",
    ro: "Identificați scrisoarea de la mijloc făcând clic pe ea mai jos.",
    pl: "Zidentyfikuj środkową literę, klikając ją na liście poniżej. ",
    ru: "Укажите среднюю букву, щелкнув по ней ниже.",
    hy: "Նշեք միջին նամակը `կտտացնելով ներքեւում:",
    fi: "Tunnista keskimmäinen kirjain napsauttamalla sitä alla.",
    ka: "გთხოვთ, დაადგინოთ შუა წერილი, დააჭირეთ მას ქვემოთ.",
    he: "אנא זיהוי האות האמצעית על ידי לחיצה עליה למטה.",
    ar: "رجاءً حدّد الحرف الأوسط بالضغط عليه أدناه.",
    ur: "براہ کرم نیچے کلک کرکے درمیانی خط کی شناخت کریں۔",
    hi: "कृपया मध्य अक्षर को नीचे क्लिक करके पहचानें।",
    ta: "நடுத்தர கடிதத்தை கீழே கிளிக் செய்வதன் மூலம் அடையாளம் காணவும்.",
    ml: "ചുവടെ ക്ലിക്കുചെയ്ത് മധ്യ അക്ഷരം തിരിച്ചറിയുക.",
    te: "దయచేసి మధ్య లేఖను క్రింద క్లిక్ చేయడం ద్వారా గుర్తించండి.",
    kn: "ದಯವಿಟ್ಟು ಮಧ್ಯದ ಅಕ್ಷರವನ್ನು ಕೆಳಗೆ ಕ್ಲಿಕ್ ಮಾಡುವ ಮೂಲಕ ಗುರುತಿಸಿ.",
    bn: "নীচে ক্লিক করে মাঝারি অক্ষরটি সনাক্ত করুন।",
    id: "Harap identifikasi surat tengah, dengan mengkliknya di bawah ini.",
    "zh-CN": "请点击下方的字母来报告您认出的中间字母。",
    "zh-HK": "請點擊下方的字母來報告您認出的中間字母。",
    ja: "下をクリックして、認識した真ん中の文字を報告してください。",
    ko: "아래를 클릭하여 중간 편지를 식별하십시오.",
  },
  T_identifyClickItRepeatedLetters: {
    "en-US": "Please identify the two middle letters by clicking them below.",
    de: "Bitte identifizieren Sie die beiden mittleren Buchstaben, indem Sie unten auf sie klicken.",
    fr: "Veuillez identifier les deux lettres du milieu en les cliquant ci-dessous.",
    es: "Identifique las dos letras intermedias haciendo clic a continuación.",
    pt: "Identifique as duas letras do meio clicando nelas abaixo.",
    it: "Si prega di identificare le due lettere centrali facendo clic di seguito.",
    ro: "Vă rugăm să identificați cele două litere de mijloc făcând clic mai jos.",
    pl: "Zidentyfikuj dwie środkowe litery, klikając je poniżej.",
    ru: "Пожалуйста, определите две средние буквы, нажав на них ниже.",
    hy: "Խնդրում ենք նույնականացնել երկու միջին տառերը `կտտացնելով դրանք ստորեւ:",
    fi: "",
    ka: "",
    he: "",
    ar: "يرجى تحديد الحرفين الأوسط من خلال النقر عليها أدناه.",
    ur: "براہ کرم ذیل میں دو درمیانی خطوط پر کلک کرکے ان کی شناخت کریں۔",
    hi: "कृपया उन्हें नीचे क्लिक करके दो मध्य अक्षरों को पहचानें।",
    ta: "இரண்டு நடுத்தர எழுத்துக்களையும் கீழே கிளிக் செய்வதன் மூலம் அடையாளம் காணவும்.",
    ml: "ചുവടെ ക്ലിക്കുചെയ്ത് രണ്ട് മധ്യ അക്ഷരങ്ങൾ തിരിച്ചറിയുക.",
    te: "దయచేసి రెండు మధ్య అక్షరాలను క్రింద క్లిక్ చేయడం ద్వారా గుర్తించండి.",
    kn: "ದಯವಿಟ್ಟು ಎರಡು ಮಧ್ಯದ ಅಕ್ಷರಗಳನ್ನು ಕೆಳಗೆ ಕ್ಲಿಕ್ ಮಾಡುವುದರ ಮೂಲಕ ಗುರುತಿಸಿ.",
    bn: "দুটি মিডল অক্ষর নীচে ক্লিক করে দয়া করে সনাক্ত করুন।",
    id: "Harap identifikasi dua huruf tengah dengan mengkliknya di bawah ini.",
    "zh-CN": "请通过在下面单击它们来确定两个中间字母。",
    "zh-HK": "請通過在下面單擊它們來確定兩個中間字母。",
    ja: "以下をクリックして、2つの中央の文字を特定してください。",
    ko: "아래를 클릭하여 두 개의 중간 문자를 식별하십시오.",
  },
  T_identifyClickItSentence: {
    "en-US":
      "All the possible sentences are displayed. Click the one you heard or your best guess.",
    de: "Alle möglichen Sätze werden angezeigt. Klicken Sie auf den, den Sie gehört haben, oder Ihre beste Vermutung.",
    fr: "Toutes les phrases possibles sont affichées. Cliquez sur celui que vous avez entendu ou votre meilleure supposition.",
    es: "Se muestran todas las oraciones posibles. Haga clic en el que escuchó o su mejor suposición.",
    pt: "Todas as frases possíveis são exibidas. Clique no que você ouviu ou seu melhor palpite.",
    it: "",
    ro: "",
    pl: "",
    ru: "",
    hy: "",
    fi: "",
    ka: "",
    he: "",
    ar: "يتم عرض جميع الجمل الممكنة. انقر فوق الشخص الذي سمعته أو أفضل تخمين لك.",
    ur: "تمام ممکنہ جملے آویزاں ہیں۔ آپ نے سنا ہے یا آپ کا بہترین اندازہ لگائیں۔",
    hi: "सभी संभावित वाक्य प्रदर्शित किए जाते हैं। जो आपने सुना है या अपना सबसे अच्छा अनुमान लगाया है।",
    ta: "சாத்தியமான அனைத்து வாக்கியங்களும் காட்டப்படும். நீங்கள் கேட்ட ஒன்றையோ அல்லது உங்கள் சிறந்த யூகத்தையோ கிளிக் செய்க.",
    ml: "സാധ്യമായ എല്ലാ വാക്യങ്ങളും പ്രദർശിപ്പിക്കും. നിങ്ങൾ കേട്ട അല്ലെങ്കിൽ നിങ്ങളുടെ മികച്ച ess ഹം ക്ലിക്കുചെയ്യുക.",
    te: "సాధ్యమయ్యే అన్ని వాక్యాలు ప్రదర్శించబడతాయి. మీరు విన్నదాన్ని లేదా మీ ఉత్తమ అంచనాను క్లిక్ చేయండి.",
    kn: "ಸಾಧ್ಯವಿರುವ ಎಲ್ಲಾ ವಾಕ್ಯಗಳನ್ನು ಪ್ರದರ್ಶಿಸಲಾಗುತ್ತದೆ. ನೀವು ಕೇಳಿದ ಒಂದು ಅಥವಾ ನಿಮ್ಮ ಉತ್ತಮ .ಹೆಯನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ.",
    bn: "সমস্ত সম্ভাব্য বাক্য প্রদর্শিত হয়। আপনি শুনেছেন বা আপনার সেরা অনুমানটি ক্লিক করুন।",
    id: "Semua kalimat yang mungkin ditampilkan. Klik yang Anda dengar atau tebakan terbaik Anda.",
    "zh-CN": "显示所有可能的句子。单击您听到的或最好的猜测。",
    "zh-HK": "顯示所有可能的句子。單擊您聽到的或最好的猜測。",
    ja: "考えられるすべての文が表示されます。聞いたものまたは最善の推測をクリックしてください。",
    ko: "가능한 모든 문장이 표시됩니다. 듣거나 가장 좋은 추측을 클릭하십시오.",
  },
  T_identifyClickItRsvpReading: {
    "en-US":
      "Please identify the words that were shown by clicking one word from each column below.",
    de: "Bitte identifizieren Sie die Wörter, die angezeigt wurden, indem Sie auf ein Wort aus jeder Spalte unten klicken.",
    fr: "Veuillez identifier les mots affichés en cliquant sur un mot de chaque colonne ci-dessous.",
    es: "Identifique las palabras que se muestran haciendo clic en una palabra de cada columna a continuación.",
    pt: "Identifique as palavras que foram mostradas clicando em uma palavra de cada coluna abaixo.",
    it: "",
    ro: "",
    pl: "",
    ru: "",
    hy: "",
    fi: "",
    ka: "",
    he: "",
    ar: "يرجى تحديد الكلمات التي تم عرضها بالنقر فوق كلمة واحدة من كل عمود أدناه.",
    ur: "براہ کرم ان الفاظ کی نشاندہی کریں جو ذیل میں ہر کالم سے ایک لفظ پر کلک کرکے دکھائے گئے تھے۔",
    hi: "कृपया उन शब्दों को पहचानें जो नीचे प्रत्येक कॉलम से एक शब्द पर क्लिक करके दिखाए गए थे।",
    ta: "கீழே உள்ள ஒவ்வொரு நெடுவரிசையிலிருந்தும் ஒரு வார்த்தையை கிளிக் செய்வதன் மூலம் காட்டப்பட்ட சொற்களை அடையாளம் காணவும்.",
    ml: "ചുവടെയുള്ള ഓരോ നിരയിൽ നിന്നും ഒരു വാക്ക് ക്ലിക്കുചെയ്ത് കാണിച്ച വാക്കുകൾ ദയവായി തിരിച്ചറിയുക.",
    te: "దయచేసి దిగువ ప్రతి కాలమ్ నుండి ఒక పదాన్ని క్లిక్ చేయడం ద్వారా చూపిన పదాలను గుర్తించండి.",
    kn: "ಕೆಳಗಿನ ಪ್ರತಿಯೊಂದು ಕಾಲಮ್‌ನಿಂದ ಒಂದು ಪದವನ್ನು ಕ್ಲಿಕ್ ಮಾಡುವುದರ ಮೂಲಕ ತೋರಿಸಿರುವ ಪದಗಳನ್ನು ದಯವಿಟ್ಟು ಗುರುತಿಸಿ.",
    bn: "দয়া করে নীচের প্রতিটি কলাম থেকে একটি শব্দ ক্লিক করে প্রদর্শিত শব্দগুলি সনাক্ত করুন।",
    id: "Harap identifikasi kata -kata yang ditunjukkan dengan mengklik satu kata dari setiap kolom di bawah ini.",
    "zh-CN": "请通过单击下面的每一列单击一个单词来识别显示的单词。",
    "zh-HK": "請通過單擊下面的每一列單擊一個單詞來識別顯示的單詞。",
    ja: "以下の各列から1つの単語をクリックして表示された単語を特定してください。",
    ko: "아래 각 열에서 한 단어를 클릭하여 표시된 단어를 식별하십시오.",
  },
  T_identifyPressIt: {
    "en-US": "Please identify the middle letter by pressing its keyboard key.",
    de: "Bitte identifizieren Sie den mittleren Buchstaben, indem Sie ihn in der Tastatur drücken.",
    fr: "Veuillez identifier la lettre du milieu en appuyant sur sa touche de clavier.",
    es: "Identifica la letra del medio presionándola en el teclado.",
    pt: "Identifique a carta do meio pressionando sua tecla do teclado.",
    it: "Si prega di identificare la lettera centrale premendo il tasto  corrispondente sulla tastiera.",
    ro: "Identificați litera mediului apăsând -o pe tastatură.",
    pl: "Zidentyfikuj środkową literę, naciskając ją na klawiaturze. ",
    ru: "Определите среднюю букву, нажав ее на клавиатуре.",
    hy: "Որոշեք միջին նամակը `սեղմելով այն ստեղնաշարի վրա:",
    fi: "Tunnista keskimmäinen kirjain painamalla sen näppäimistönäppäintä.",
    ka: "გთხოვთ, დაადგინოთ შუა ასო მისი კლავიშის ღილაკის დაჭერით.",
    he: "אנא זיהוי האות האמצעית על ידי לחיצה עליה במקלדת.",
    ar: "رجاءً حدّد الحرف الأوسط بالضغط عليه في لوحة المفاتيح.",
    ur: "براہ کرم درمیانی خط کو کی بورڈ میں دبانے سے شناخت کریں۔",
    hi: "कृपया मध्य अक्षर को कीबोर्ड में दबाकर पहचानें।",
    ta: "நடுத்தர கடிதத்தை அதன் விசைப்பலகை விசையை அழுத்துவதன் மூலம் அடையாளம் காணவும்.",
    ml: "അതിന്റെ കീബോർഡ് കീ അമർത്തിക്കൊണ്ട് മധ്യ കത്ത് തിരിച്ചറിയുക.",
    te: "దయచేసి దాని కీబోర్డ్ కీని నొక్కడం ద్వారా మధ్య లేఖను గుర్తించండి.",
    kn: "ಕೀಬೋರ್ಡ್ ಕೀಲಿಯನ್ನು ಒತ್ತುವ ಮೂಲಕ ದಯವಿಟ್ಟು ಮಧ್ಯದ ಅಕ್ಷರವನ್ನು ಗುರುತಿಸಿ.",
    bn: "মাঝারি অক্ষরটি কীবোর্ড কী টিপে দয়া করে সনাক্ত করুন।",
    id: "Harap identifikasi huruf tengah dengan menekan tombol keyboard -nya.",
    "zh-CN": "请按下键盘上的对应按键来报告您认出的中间字母。",
    "zh-HK": "請按下鍵盤上的對應按鍵來報告您認出的中間字母。",
    ja: "キーボードで対応するキーを押して、認識した真ん中の文字を報告してください。",
    ko: "키보드 키를 눌러 중간 편지를 식별하십시오.",
  },
  T_identifyPressItRsvpReading: {
    "en-US":
      "Please identify the words that were shown by speaking them aloud.",
    de: "Bitte identifizieren Sie die Wörter, die gezeigt wurden, indem sie laut sprechen.",
    fr: "Veuillez identifier les mots qui ont été montrés en les parlant à haute voix.",
    es: "Identifique las palabras que se mostraron al hablarlas en voz alta.",
    pt: "Identifique as palavras que foram mostradas ao falar em voz alta.",
    it: "Identifica le parole che sono state mostrate quando le parlano ad alta voce.",
    ro: "Identificați cuvintele care au fost arătate atunci când le vorbiți cu voce tare.",
    pl: "Zidentyfikuj słowa, które zostały pokazane podczas mówienia na głos.",
    ru: "Определите слова, которые были показаны, когда говорили о них вслух.",
    hy: "Որոշեք այն բառերը, որոնք ցույց են տրվել, երբ բարձրաձայն խոսում են:",
    fi: "Tunnista sanat, jotka näytettiin puhuessaan ne ääneen.",
    ka: "დაადგინეთ სიტყვები, რომლებიც ნაჩვენებია ხმამაღლა საუბრისას.",
    he: "זהה את המילים שהוצגו כשמדברים אותן בקול רם.",
    ar: "حدد الكلمات التي تم عرضها عند التحدث بها بصوت عالٍ.",
    ur: "ان الفاظ کی نشاندہی کریں جو ان کو اونچی آواز میں بولتے وقت دکھائے گئے تھے۔",
    hi: "उन शब्दों को पहचानें जो उन्हें जोर से बोलते समय दिखाए गए थे।",
    ta: "காட்டப்பட்ட சொற்களை சத்தமாக பேசும்போது அடையாளம் காணவும்.",
    ml: "ഉച്ചത്തിൽ സംസാരിക്കുമ്പോൾ കാണിച്ചിരിക്കുന്ന വാക്കുകൾ തിരിച്ചറിയുക.",
    te: "వాటిని బిగ్గరగా మాట్లాడేటప్పుడు చూపిన పదాలను గుర్తించండి.",
    kn: "ಅವುಗಳನ್ನು ಜೋರಾಗಿ ಮಾತನಾಡುವಾಗ ತೋರಿಸಿದ ಪದಗಳನ್ನು ಗುರುತಿಸಿ.",
    bn: "উচ্চস্বরে কথা বলার সময় প্রদর্শিত শব্দগুলি সনাক্ত করুন।",
    id: "Identifikasi kata -kata yang ditunjukkan saat berbicara dengan keras.",
    "zh-CN": "确定大声说出来时显示的单词。",
    "zh-HK": "確定大聲說出來時顯示的單詞。",
    ja: "大声で話すときに示された単語を特定します。",
    ko: "큰 소리로 말할 때 표시된 단어를 식별하십시오.",
  },
  T_identifyPressItOrClickIt: {
    "en-US":
      "Please identify the middle letter by clicking it below or pressing it in the keyboard.",
    de: "Bitte identifizieren Sie den mittleren Buchstaben, indem Sie ihn in der Tastatur drücken oder unten darauf klicken.",
    fr: "Veuillez identifier la lettre du milieu en cliquant dessus ci-dessous ou en appuyant sur le clavier.",
    es: "Identifica la letra del medio presionándola en el teclado o haciendo clic en ella abajo.",
    pt: "Identifique a carta do meio clicando nela abaixo ou pressionando -a no teclado.",
    it: "Si prega di identificare la lettera centrale, premendola sulla tastiera o facendo clic su di essa in basso.",
    ro: "Identificați litera mediului apăsând -o pe tastatură sau făcând clic pe ea.",
    pl: "Zidentyfikuj środkową literę, naciskając ją na klawiaturze lub klikając ją na liście poniżej. ",
    ru: "Определите среднюю букву, нажав ее на клавиатуре или щелкнув по ней ниже.",
    hy: "Որոշեք միջին նամակը `սեղմելով այն ստեղնաշարի վրա կամ կտտացնելով այն ստորեւ:",
    fi: "Tunnista keskimmäinen kirjain napsauttamalla sitä alla tai painamalla sitä näppäimistössä.",
    ka: "გთხოვთ, დაადგინოთ შუა ასო, დააჭირეთ მას ქვემოთ ან დააჭირეთ მას კლავიატურაზე.",
    he: "אנא זיהוי האות האמצעית, על ידי לחיצה עליה במקלדת או לחיצה עליה למטה.",
    ar: "رجاءً حدّد الحرف الأوسط بالضغط عليه في لوحة المفاتيح أو الضغط عليه أدناه.",
    ur: "براہ کرم درمیانی خط کو کی بورڈ میں دبانے یا نیچے کلک کرکے شناخت کریں۔",
    hi: "कृपया मध्य अक्षर को कीबोर्ड में दबाकर या नीचे क्लिक करके पहचानें।",
    ta: "நடுத்தர கடிதத்தை கீழே கிளிக் செய்வதன் மூலம் அல்லது விசைப்பலகையில் அழுத்துவதன் மூலம் அடையாளம் காணவும்.",
    ml: "ചുവടെ ക്ലിക്കുചെയ്ത് അല്ലെങ്കിൽ കീബോർഡിൽ അമർത്തിക്കൊണ്ട് മധ്യ അക്ഷരം തിരിച്ചറിയുക.",
    te: "దయచేసి మధ్య లేఖను క్రింద క్లిక్ చేయడం ద్వారా లేదా కీబోర్డ్‌లో నొక్కడం ద్వారా గుర్తించండి.",
    kn: "ದಯವಿಟ್ಟು ಮಧ್ಯದ ಅಕ್ಷರವನ್ನು ಕೆಳಗೆ ಕ್ಲಿಕ್ ಮಾಡುವ ಮೂಲಕ ಅಥವಾ ಕೀಬೋರ್ಡ್‌ನಲ್ಲಿ ಒತ್ತುವ ಮೂಲಕ ಗುರುತಿಸಿ.",
    bn: "মাঝারি অক্ষরটি নীচে ক্লিক করে বা কীবোর্ডে টিপে দয়া করে সনাক্ত করুন।",
    id: "Harap identifikasi huruf tengah dengan mengkliknya di bawah ini atau menekannya di keyboard.",
    "zh-CN": "请按下键盘上的对应按键或者点击下方的字母来报告您认出的中间字母。",
    "zh-HK": "請按下鍵盤上的對應按鍵或者點擊下方的字母來報告您認出的中間字母。",
    ja: "キーボードで対応するキーを押すか、下をクリックして、認識した真ん中の文字を報告してください。",
    ko: "아래를 클릭하거나 키보드에 누르면 중간 문자를 식별하십시오.",
  },
  T_identifyPressItOrClickItRepeatedLetters: {
    "en-US":
      "Please identify the two middle letters by clicking them below or pressing them on the keyboard.",
    de: "Bitte identifizieren Sie die beiden mittleren Buchstaben, indem Sie sie unten klicken oder sie auf der Tastatur drücken.",
    fr: "Veuillez identifier les deux lettres du milieu en les cliquant ci-dessous ou en les appuyant sur le clavier.",
    es: "Identifique las dos letras intermedias haciendo clic a continuación o presionándolas en el teclado.",
    pt: "Identifique as duas letras intermediárias clicando ou pressionando -as no teclado.",
    it: "Identifica le due lettere intermedie facendo clic o premendole sulla tastiera.",
    ro: "Identificați cele două litere intermediare făcând clic sau apăsându -le pe tastatură.",
    pl: "Zidentyfikuj dwie pośrednie litery, klikając lub naciskając je na klawiaturze.",
    ru: "Определите две промежуточные буквы, нажав или нажав их на клавиатуре.",
    hy: "Նշեք երկու միջանկյալ տառերը `կտտացնելով կամ սեղմելով դրանք ստեղնաշարի վրա:",
    fi: "Tunnista kaksi välikirjainta napsauttamalla tai painamalla niitä näppäimistöllä.",
    ka: "დაასახელეთ ორი შუალედური ასო კლავიატურაზე დაჭერით ან დაჭერით.",
    he: "זהה את שתי האותיות הביניים על ידי לחיצה או על ידי לחיצה על המקלדת.",
    ar: "حدد الحرفين الوسيطين من خلال النقر أو الضغط عليها على لوحة المفاتيح.",
    ur: "دو انٹرمیڈیٹ خطوط پر کلک کرکے یا کی بورڈ پر دبانے سے ان کی شناخت کریں۔",
    hi: "पर क्लिक करके या उन्हें कीबोर्ड पर दबाकर दो मध्यवर्ती अक्षरों को पहचानें।",
    ta: "இரண்டு இடைநிலை கடிதங்களை கிளிக் செய்வதன் மூலம் அல்லது விசைப்பலகையில் அழுத்துவதன் மூலம் அடையாளம் காணவும்.",
    ml: "ക്ലിക്കുചെയ്ത് അല്ലെങ്കിൽ കീബോർഡിൽ അമർത്തിക്കൊണ്ട് രണ്ട് ഇന്റർമീഡിയറ്റ് അക്ഷരങ്ങൾ തിരിച്ചറിയുക.",
    te: "రెండు ఇంటర్మీడియట్ అక్షరాలను క్లిక్ చేయడం ద్వారా లేదా కీబోర్డ్‌లో నొక్కడం ద్వారా గుర్తించండి.",
    kn: "ಎರಡು ಮಧ್ಯಂತರ ಅಕ್ಷರಗಳನ್ನು ಕ್ಲಿಕ್ ಮಾಡುವುದರ ಮೂಲಕ ಅಥವಾ ಕೀಬೋರ್ಡ್‌ನಲ್ಲಿ ಒತ್ತುವ ಮೂಲಕ ಗುರುತಿಸಿ.",
    bn: "দুটি মধ্যবর্তী অক্ষর ক্লিক করে বা কীবোর্ডে চাপ দিয়ে সনাক্ত করুন।",
    id: "Identifikasi dua huruf menengah dengan mengklik atau dengan menekannya di keyboard.",
    "zh-CN": "通过单击或在键盘上按两个中间字母来识别两个中间字母。",
    "zh-HK": "通過單擊或在鍵盤上按兩個中間字母來識別兩個中間字母。",
    ja: "キーボードでクリックするか、キーボードを押して2つの中間文字を識別します。",
    ko: "키보드를 클릭하거나 눌러 두 개의 중간 문자를 식별하십시오.",
  },
  T_identifyPressItRepeatedLetters: {
    "en-US":
      "Please identify the two middle letters, by pressing them on the keyboard.",
    de: "Bitte identifizieren Sie die beiden mittleren Buchstaben, indem Sie sie auf die Tastatur drücken.",
    fr: "Veuillez identifier les deux lettres du milieu, en les appuyant sur le clavier.",
    es: "Identifique las dos letras intermedias, presionándolas en el teclado.",
    pt: "Identifique as duas letras intermediárias, pressionando -as no teclado.",
    it: "Identifica le due lettere intermedie, premendole sulla tastiera.",
    ro: "Identificați cele două litere intermediare, apăsându -le pe tastatură.",
    pl: "Zidentyfikuj dwie pośrednie litery, naciskając je na klawiaturze.",
    ru: "Определите две промежуточные буквы, нажимая их на клавиатуре.",
    hy: "Նշեք երկու միջանկյալ տառերը, սեղմելով դրանք ստեղնաշարի վրա:",
    fi: "Tunnista kaksi välikirjainta painamalla niitä näppäimistöllä.",
    ka: "დაადგინეთ ორი შუალედური ასო, დააჭირეთ მათ კლავიატურაზე.",
    he: "זהה את שתי האותיות הביניים, ולחץ עליהן על המקלדת.",
    ar: "حدد الحرفين المتوسطين ، والضغط عليهما على لوحة المفاتيح.",
    ur: "دو انٹرمیڈیٹ خطوط کی شناخت کریں ، انہیں کی بورڈ پر دبائیں۔",
    hi: "दो मध्यवर्ती अक्षरों को पहचानें, उन्हें कीबोर्ड पर दबाएं।",
    ta: "இரண்டு இடைநிலை கடிதங்களை அடையாளம் கண்டு, அவற்றை விசைப்பலகையில் அழுத்தவும்.",
    ml: "രണ്ട് ഇന്റർമീഡിയറ്റ് അക്ഷരങ്ങൾ തിരിച്ചറിയുക, അവ കീബോർഡിൽ അമർത്തുന്നു.",
    te: "రెండు ఇంటర్మీడియట్ అక్షరాలను గుర్తించండి, వాటిని కీబోర్డ్‌లో నొక్కండి.",
    kn: "ಎರಡು ಮಧ್ಯಂತರ ಅಕ್ಷರಗಳನ್ನು ಗುರುತಿಸಿ, ಅವುಗಳನ್ನು ಕೀಬೋರ್ಡ್‌ನಲ್ಲಿ ಒತ್ತಿ.",
    bn: "দুটি মধ্যবর্তী অক্ষর সনাক্ত করুন, কীবোর্ডে টিপুন।",
    id: "Identifikasi dua huruf perantara, tekan di keyboard.",
    "zh-CN": "标识两个中间字母，将它们按在键盘上。",
    "zh-HK": "標識兩個中間字母，將它們按在鍵盤上。",
    ja: "2つの中間文字を識別し、キーボードに押し付けます。",
    ko: "두 개의 중간 문자를 식별하여 키보드에 누릅니다.",
  },
  T_letterPopularDemandFeatures: {
    "en-US": "  ",
    de: "  ",
    fr: "  ",
    es: "  ",
    pt: "  ",
    it: "  ",
    ro: "  ",
    pl: "  ",
    ru: "  ",
    hy: "  ",
    fi: "  ",
    ka: "  ",
    he: "  ",
    ar: "  ",
    ur: "  ",
    hi: "  ",
    ta: "  ",
    ml: "  ",
    te: "  ",
    kn: "  ",
    bn: "  ",
    id: "  ",
    "zh-CN": "  ",
    "zh-HK": "  ",
    ja: "  ",
    ko: "  ",
  },
  T_loadingSounds: {
    "en-US": "Loading sounds ...",
    de: "Ladegeräusche ...",
    fr: "Chargement des sons ...",
    es: "Cargando sonidos ...",
    pt: "Sons de carregamento ...",
    it: "Caricamento dei suoni ...",
    ro: "Încărcarea sunetelor ...",
    pl: "Ładowanie dźwięków ...",
    ru: "Звуки загрузки ...",
    hy: "Ձայնի բեռնումը ...",
    fi: "Äänien lataaminen ...",
    ka: "იტვირთება ხმები ...",
    he: "טוען צלילי ...",
    ar: "تحميل الأصوات ...",
    ur: "یہ لوڈ ہو رہا ہے ...",
    hi: "लोड हो रहा है ...",
    ta: "ஒலிகளை ஏற்றுகிறது ...",
    ml: "ലോഡുചെയ്യുന്നു ...",
    te: "శబ్దాలు లోడ్ అవుతున్నాయి ...",
    kn: "ಶಬ್ದಗಳನ್ನು ಲೋಡ್ ಮಾಡಲಾಗುತ್ತಿದೆ ...",
    bn: "লোডিং শব্দ ...",
    id: "Memuat suara ...",
    "zh-CN":
      "我们会定期暂停让您休息，以防您过于疲劳。休息时间会出现在区组之间和区组之中。",
    "zh-HK":
      "我們會定期暫停讓您休息，以防您過於疲勞。休息時間會出現在區組之間和區組之中。",
    ja: "こんにちは。ピーボタンをクリックしてこのコンピュータの音が機能していることを確認してください。ちなみに、テストでは正解する度にこの音が鳴ります。",
    ko: "소리로드 ...",
  },
  T_middleLetterBrief: {
    "en-US":
      "It’s an R. This display is static, so it would be easy to move your eyes to the letter and identify it. But, in a real trial, the letters are FLASHED, so there's no time to move your eyes. It's important to look at the cross hairs as you click it. The letters will flash immediately. It doesn't matter where your eyes go after the flash. The first block of testing is just a few trials of practice, before we start in earnest. ",
    de: "Es ist ein R. Diese Anzeige ist statisch, daher wäre es einfach, Ihre Augen in den Buchstaben zu bewegen und zu identifizieren. Aber in einem echten Versuch werden die Buchstaben geblitzt, so dass es keine Zeit gibt, Ihre Augen zu bewegen. Es ist wichtig, die Kreuzhaare beim Klicken zu betrachten. Die Buchstaben blinken sofort. Es spielt keine Rolle, wohin Ihre Augen nach dem Blitz gehen. Der erste Testblock ist nur ein paar Versuche der Praxis, bevor wir ernsthaft anfangen.",
    fr: "C'est un R. Cet affichage est statique, il serait donc facile de déplacer vos yeux vers la lettre et de l'identifier. Mais, dans un vrai procès, les lettres sont flashé, il n'y a donc pas de temps pour bouger les yeux. Il est important de regarder les poils à cuire pendant que vous cliquez dessus. Les lettres clignoteront immédiatement. Peu importe où vos yeux vont après le flash. Le premier bloc de tests n'est que quelques essais de pratique, avant de commencer sérieusement.",
    es: "Es un R. Esta pantalla es estática, por lo que sería fácil mover los ojos a la letra e identificarla. Pero, en una prueba real, las letras se muestran, por lo que no hay tiempo para mover los ojos. Es importante mirar los pelos de la cruz mientras hace clic. Las letras parpadearán de inmediato. No importa a dónde van tus ojos después del flash. El primer bloque de pruebas es solo unas pocas pruebas de práctica, antes de comenzar en serio.",
    pt: "É uma R. Essa tela é estática, por isso seria fácil mover os olhos para a letra e identificá -la. Mas, em um teste real, as letras são mostradas, portanto não há tempo para mover os olhos. É importante olhar para os cabelos Cruz enquanto clica. As letras piscarão imediatamente. Não importa onde seus olhos vão depois do flash. O primeiro bloco de teste são apenas alguns testes de prática, antes de começar seriamente.",
    it: "È un R. Questo display è statico, quindi sarebbe facile spostare gli occhi sulla lettera e identificarla. Ma, in una vera prova, le lettere vengono lampeggiate, quindi non c'è tempo per muovere gli occhi. È importante guardare i capelli incrociati mentre fai clic. Le lettere lamperanno immediatamente. Non importa dove vanno gli occhi dopo il flash. Il primo blocco di test è solo alcune prove di pratica, prima di iniziare sul serio.",
    ro: "Este un R. Acest ecran este static, așa că ar fi ușor să vă mutați ochii spre scrisoare și să îl identificați. Dar, într -un test real, literele sunt arătate, așa că nu există timp să vă mișcați ochii. Este important să ne uităm la părul Cruz în timp ce faceți clic. Literele vor clipi imediat. Nu contează unde îți dau ochii după flash. Primul bloc de testare este doar câteva teste de practică, înainte de a începe serios.",
    pl: "Prawidłowa odpowiedz to R. Ta próba jest duga, więc łatwo byłoby przenieść twoje oko do liter i zidentyfikować środkową. W prawdziwym teście litery będą prezentowanie przez krótki okres czasu, więc nie będziesz miał czasu, aby przenieść wzrok.\n \n\n",
    ru: "Это R. Этот дисплей статичен, поэтому было бы легко переместить ваши глаза на букву и идентифицировать ее. Но, в реальном исследовании, буквы вспыхнут, поэтому не будет времени перемещать глаза. Важно посмотреть на прицел, когда вы нажимаете на нее. Буквы будут мигать немедленно. Неважно, где ваши глаза идут после вспышки. Первый блок тестирования - это всего лишь несколько тренировочных испытаний, прежде чем начать всерьез.",
    hy: "Այս R. Այս ցուցադրումը վիճակագրական է, ուստի հեշտ կլինի ձեր աչքերը տառին տեղափոխել եւ նույնացնել: Բայց, իրական ուսումնասիրության մեջ, տառերը բռնկվում են, ուստի ժամանակ չի լինի աչքերը տեղափոխելու համար: Կարեւոր է նայել այն տեսողությանը, երբ սեղմում եք այն: Նամակները անմիջապես կթողնեն: Անկախ նրանից, թե որտեղ են ձեր աչքերը անցնում Flash- ից հետո: Առաջին փորձարկման միավորը ընդամենը մի քանի ուսումնական թեստ է, նախքան լրջորեն սկսելը:",
    fi: "Se on R. Tämä näyttö on staattinen, joten olisi helppo siirtää silmäsi kirjaimeen ja tunnistaa se. Mutta todellisessa oikeudenkäynnissä kirjeet vilkkuvat, joten silmiäsi ei ole aikaa liikuttaa. On tärkeää tarkastella ristikarvoja napsauttaessasi sitä. Kirjeet vilkkuvat heti. Ei ole väliä missä silmäsi menevät salaman jälkeen. Ensimmäinen testauslohko on vain muutama harjoittelukoe, ennen kuin aloitamme tosissaan.",
    ka: "ეს არის R. ეს ჩვენება სტატიკურია, ასე რომ ადვილი იქნება თქვენი თვალების ასოზე გადატანა და მისი იდენტიფიცირება. მაგრამ, რეალურ განსაცდელში, ასოები ააფეთქეს, ასე რომ, თვალების გადაადგილების დრო არ არის. მნიშვნელოვანია, რომ გადახედოთ ჯვრის თმას, როდესაც დააჭირეთ მას. ასოები დაუყოვნებლივ გაისვრის. არ აქვს მნიშვნელობა სად მიდის თქვენი თვალები flash- ის შემდეგ. ტესტირების პირველი ბლოკი მხოლოდ რამდენიმე პრაქტიკის გამოცდაა, სანამ სერიოზულად დავიწყებთ.",
    he: "זה R. תצוגה זו סטטית, כך שיהיה קל להעביר את העיניים למכתב ולזהות אותו. אבל, במשפט אמיתי, המכתבים מבהיקים, כך שאין זמן להזיז את העיניים. חשוב להסתכל על שערות הצלב בזמן שאתה לוחץ עליו. האותיות יבהבו מייד. לא משנה לאן העיניים שלך הולכות אחרי ההבזק. גוש הבדיקה הראשון הוא רק כמה ניסויים של תרגול, לפני שנתחיל ברצינות.",
    ar: "إنه R. هذا العرض ثابت ، لذلك سيكون من السهل نقل عينيك إلى الرسالة وتحديدها. ولكن ، في تجربة حقيقية ، تومض الحروف ، لذلك لا يوجد وقت لتحريك عينيك. من المهم أن ننظر إلى الشعر المتقاطع أثناء النقر عليه. سوف تومض الحروف على الفور. لا يهم أين تذهب عينيك بعد الفلاش. الكتلة الأولى للاختبار هي مجرد تجارب قليلة من الممارسة ، قبل أن نبدأ بجدية.",
    ur: "یہ ایک آر ہے۔ یہ کارکردگی مستحکم ہے ، لہذا آپ کی آنکھوں کو خط میں منتقل کرنا اور اس کی شناخت کرنا آسان ہوگا۔ لیکن ، ایک حقیقی امتحان میں ، خطوط چمکتے ہیں ، لہذا آپ کی آنکھیں منتقل کرنے کا وقت نہیں ہے۔ جیسا کہ آپ اس پر کلک کرتے ہیں ، کراس بال دیکھنا ضروری ہے۔ خطوط فوری طور پر فلیش ہوجائیں گے۔ اس سے کوئی فرق نہیں پڑتا ہے کہ آپ کی آنکھیں فلیش کے بعد کہاں جاتی ہیں۔ بیانا میں شروع ہونے سے پہلے ، ٹیسٹ کے پہلے بلاک میں مشق کے کچھ ٹیسٹوں کے کچھ ٹیسٹ ہوتے ہیں۔",
    hi: "यह एक आर है। यह प्रदर्शन स्थिर है, इसलिए आपकी आंखों को पत्र में ले जाना और इसे पहचानना आसान होगा। लेकिन, एक वास्तविक परीक्षण में, पत्र चमकते हैं, इसलिए आपकी आँखों को स्थानांतरित करने का समय नहीं है। जैसा कि आप इसे क्लिक करते हैं, क्रॉस हेयर को देखना महत्वपूर्ण है। पत्र तुरंत फ्लैश हो जाएंगे। इससे कोई फर्क नहीं पड़ता कि आपकी आँखें फ्लैश के बाद कहां जाती हैं। परीक्षण का पहला ब्लॉक अभ्यास के कुछ परीक्षणों के कुछ परीक्षण हैं, इससे पहले कि हम बयाना में शुरू करें।",
    ta: "இது ஒரு ஆர். இந்த காட்சி நிலையானது, எனவே உங்கள் கண்களை கடிதத்திற்கு நகர்த்தி அதை அடையாளம் காண்பது எளிது. ஆனால், ஒரு உண்மையான சோதனையில், கடிதங்கள் பறக்கப்படுகின்றன, எனவே உங்கள் கண்களை நகர்த்த நேரமில்லை. குறுக்கு முடிகளை நீங்கள் கிளிக் செய்யும்போது அதைப் பார்ப்பது முக்கியம். கடிதங்கள் உடனடியாக ஒளிரும். ஃபிளாஷ் பின் உங்கள் கண்கள் எங்கு செல்கின்றன என்பது முக்கியமல்ல. சோதனையின் முதல் தொகுதி, நாம் ஆர்வத்துடன் தொடங்குவதற்கு முன்பு, நடைமுறையின் சில சோதனைகள் மட்டுமே.",
    ml: "ഇത് ഒരു r. ഈ ഡിസ്പ്ലേ സ്റ്റാറ്റിക് ആണ്, അതിനാൽ നിങ്ങളുടെ കണ്ണുകൾ കത്തിച്ച് തിരിച്ചറിയുകയും അത് തിരിച്ചറിയുകയും ചെയ്യും. പക്ഷേ, ഒരു യഥാർത്ഥ ട്രയലിൽ, അക്ഷരങ്ങൾ മിന്നുന്നു, അതിനാൽ നിങ്ങളുടെ കണ്ണുകൾ നീക്കാൻ സമയമില്ല. നിങ്ങൾ അതിൽ ക്ലിക്കുചെയ്യുമ്പോൾ ക്രോസ് രോമങ്ങൾ നോക്കേണ്ടത് പ്രധാനമാണ്. അക്ഷരങ്ങൾ ഉടനടി ഫ്ലാഷ് ചെയ്യും. നിങ്ങളുടെ കണ്ണുകൾ ഫ്ലാഷിനുശേഷം പോകുന്നത് പ്രശ്നമല്ല. പരിശോധനയുടെ ആദ്യ ബ്ലോക്ക് പരിശീലനത്തിന്റെ ഏതാനും പരീക്ഷണങ്ങൾ മാത്രമാണ്, ഞങ്ങൾ ആത്മാർത്ഥതയോടെ ആരംഭിക്കുന്നതിന് മുമ്പ്.",
    te: "ఇది R. ఈ ప్రదర్శన స్థిరంగా ఉంది, కాబట్టి మీ కళ్ళను లేఖకు తరలించి గుర్తించడం సులభం. కానీ, నిజమైన విచారణలో, అక్షరాలు వెలిగిపోతాయి, కాబట్టి మీ కళ్ళను కదిలించడానికి సమయం లేదు. మీరు క్లిక్ చేస్తున్నప్పుడు క్రాస్ వెంట్రుకలను చూడటం చాలా ముఖ్యం. అక్షరాలు వెంటనే ఫ్లాష్ అవుతాయి. ఫ్లాష్ తర్వాత మీ కళ్ళు ఎక్కడికి వెళ్తాడనేది పట్టింపు లేదు. పరీక్ష యొక్క మొదటి బ్లాక్ మేము ఆసక్తిగా ప్రారంభించడానికి ముందు, ప్రాక్టీస్ యొక్క కొన్ని ప్రయత్నాలు.",
    kn: "ಇದು ಆರ್. ಈ ಪ್ರದರ್ಶನವು ಸ್ಥಿರವಾಗಿದೆ, ಆದ್ದರಿಂದ ನಿಮ್ಮ ಕಣ್ಣುಗಳನ್ನು ಪತ್ರಕ್ಕೆ ಸರಿಸಿ ಅದನ್ನು ಗುರುತಿಸುವುದು ಸುಲಭ. ಆದರೆ, ನಿಜವಾದ ಪ್ರಯೋಗದಲ್ಲಿ, ಅಕ್ಷರಗಳು ಹರಿಯುತ್ತವೆ, ಆದ್ದರಿಂದ ನಿಮ್ಮ ಕಣ್ಣುಗಳನ್ನು ಸರಿಸಲು ಸಮಯವಿಲ್ಲ. ನೀವು ಕ್ಲಿಕ್ ಮಾಡುವಾಗ ಅಡ್ಡ ಕೂದಲನ್ನು ನೋಡುವುದು ಮುಖ್ಯ. ಅಕ್ಷರಗಳು ತಕ್ಷಣ ಮಿಂಚುತ್ತವೆ. ಫ್ಲ್ಯಾಷ್ ನಂತರ ನಿಮ್ಮ ಕಣ್ಣುಗಳು ಎಲ್ಲಿಗೆ ಹೋಗುತ್ತವೆ ಎಂಬುದು ಮುಖ್ಯವಲ್ಲ. ನಾವು ಶ್ರದ್ಧೆಯಿಂದ ಪ್ರಾರಂಭಿಸುವ ಮೊದಲು ಪರೀಕ್ಷೆಯ ಮೊದಲ ಬ್ಲಾಕ್ ಅಭ್ಯಾಸದ ಕೆಲವು ಪ್ರಯೋಗಗಳು.",
    bn: "এটি একটি আর। এই প্রদর্শনটি স্থির, সুতরাং আপনার চোখটি চিঠির দিকে নিয়ে যাওয়া এবং এটি সনাক্ত করা সহজ হবে। তবে, একটি বাস্তব পরীক্ষায়, চিঠিগুলি জ্বলজ্বল করা হয়, তাই আপনার চোখ সরানোর কোনও সময় নেই। আপনি এটি ক্লিক করার সাথে সাথে ক্রস চুলগুলি দেখতে গুরুত্বপূর্ণ। চিঠিগুলি তাত্ক্ষণিকভাবে ফ্ল্যাশ হবে। ফ্ল্যাশের পরে আপনার চোখ কোথায় যায় তা বিবেচ্য নয়। পরীক্ষার প্রথম ব্লকটি আমরা আন্তরিকভাবে শুরু করার আগে অনুশীলনের কয়েকটি পরীক্ষা।",
    id: "Ini adalah R. Layar ini statis, jadi akan mudah untuk memindahkan mata Anda ke surat dan mengidentifikasinya. Tapi, dalam uji coba nyata, surat -surat itu muncul, jadi tidak ada waktu untuk menggerakkan mata Anda. Penting untuk melihat rambut silang saat Anda mengkliknya. Huruf akan segera berkedip. Tidak masalah ke mana mata Anda mengejar kilat. Blok pengujian pertama hanyalah beberapa percobaan latihan, sebelum kita mulai dengan sungguh -sungguh.",
    "zh-CN":
      "这是一个 R。因为字母一直显示在屏幕上，所以您只需移动一下视线就能轻松认出它。在接下来的试验中，字母都将一闪而过，所以您不会有时间来调整您的视线。",
    "zh-HK":
      "這是一個 R。因為字母一直顯示在屏幕上，所以您只需移動一下視線就能輕鬆認出它。在接下來的試驗中，字母都將一閃而過，所以您不會有時間來調整您的視線。",
    ja: "これはRです。この画面は動いていないため、目を文字の位置に動かして確認するのは容易でしょう。本番のテストでは、文字は一瞬しか映されないので、目を動かす時間はないでしょう。",
    ko: "이 디스플레이는 정적이므로 눈을 편지로 움직이고 식별하기가 쉽습니다. 그러나 실제 시험에서는 편지가 번쩍이므로 눈을 움직일 시간이 없습니다. 클릭 할 때 십자 머리카락을 보는 것이 중요합니다. 편지는 즉시 플래시됩니다. 플래시 후 눈이 어디로 가는지는 중요하지 않습니다. 테스트의 첫 번째 블록은 우리가 본격적으로 시작하기 전에 몇 가지 실습 시험입니다.",
  },
  T_middleLetterDemo: {
    "en-US":
      "On each trial, while you look at the cross hairs, a triplet of three random letters will FLASH in the corner of your eye. Just a flash. To learn the task, let’s try it first with a STATIC display. While you look at the cross hairs, below, without moving your eyes, try to identify the middle letter. ",
    de: "Während Sie bei jedem Versuch die Kreuzhaare betrachten, blitzt in Ihrem Augenwinkel ein Drunken mit drei zufälligen Buchstaben. Nur ein Blitz. Um die Aufgabe zu lernen, versuchen wir es zuerst mit einer statischen Anzeige. Versuchen Sie, den mittleren Buchstaben zu identifizieren, während Sie sich die Kreuzhaare unten ansehen, ohne Ihre Augen zu bewegen.",
    fr: "À chaque essai, pendant que vous regardez les poils, un triplet de trois lettres aléatoires clignotera dans le coin de votre œil. Juste un flash. Pour apprendre la tâche, essayons-le d'abord avec un écran statique. Pendant que vous regardez les poils, ci-dessous, sans bouger les yeux, essayez d'identifier la lettre du milieu.",
    es: "En cada prueba, mientras mira la cruz, un triplete de tres letras aleatorias parpadeará en la esquina de su ojo. Solo un destello. Para aprender la tarea, intentémosla primero con una pantalla estática. Mientras mira los pelos de la cruz, a continuación, sin mover los ojos, intente identificar la letra central.",
    pt: "Em cada teste, enquanto olhava para a cruz, três letras que rolam três letras piscam no canto do olho. Apenas um instante. Para aprender a tarefa, vamos tentar primeiro com uma tela estática. Enquanto olha para o cabelo da cruz, sem mover os olhos, tente identificar a letra central.",
    it: "Ad ogni prova, mentre guardi i capelli incrociati, una tripletta di tre lettere casuali lampeggerà nell'angolo dell'occhio. Solo un lampo. Per imparare l'attività, proviamo prima con un display statico. Mentre guardi i peli incrociati, sotto, senza muovere gli occhi, prova a identificare la lettera di mezzo.",
    ro: "În fiecare test, în timp ce privești crucea, un trei -ștergători care rulează trei -ștergători vor clipi la colțul ochiului. Doar un bliț. Pentru a învăța sarcina, să încercăm mai întâi cu un ecran static. În timp ce privești părul crucii, apoi fără să -ți mișci ochii, încearcă să identifici litera centrală.",
    pl: "Podczas każdej proby gdy patrzysz na krzyżyk pojawią się trzy losowe litery w kącie pola widzenia. Podczas gdy patrzysz na krzyżyk, bez przenoszenia wzroku na inne części ekranu, spróbuj zidentyfikować środkową literę.\n \n\n",
    ru: "На каждом испытании, пока вы смотрите на прицел, триплет из трех случайных букв будет мигать в углу глаза. Просто вспышка. Чтобы узнать задачу, давайте сначала попробуем ее со статическим дисплеем. Пока вы смотрите на прицел, ниже, не двигая глазами, попробуйте определить среднюю букву.",
    hy: "Յուրաքանչյուր թեստում, մինչ դուք նայում եք հայացքից, երեք պատահական տառերի եռյակը կթողնի աչքի անկյունում: Պարզապես ֆլեշ: Առաջադրանքը պարզելու համար եկեք առաջին հերթին փորձենք ստատիկ էկրանով: Մինչ դուք նայում եք տեսողությանը, ներքեւում, առանց ձեր աչքերը տեղափոխելու, փորձեք որոշել միջին նամակը:",
    fi: "Jokaisessa kokeessa, kun tarkastellaan ristikarvoja, kolmen satunnaisen kirjaimen kolmoinen vilkkuu silmäsi nurkassa. Vain salama. Kokeilemme sitä ensin staattisella näytöllä. Kun tarkastellaan ristikkäitä hiuksia alla, liikuttamatta silmiäsi, yritä tunnistaa keskimmäinen kirjain.",
    ka: "თითოეულ საცდელზე, სანამ ჯვრის თმას უყურებთ, სამი შემთხვევითი ასოების ტრიპლეტი თვალის კუთხეში აჩერებს. უბრალოდ ფლეშ. დავალების შესასწავლად, მოდით, პირველ რიგში ვცადოთ სტატიკური დისპლეით. სანამ ჯვრის თმას უყურებთ, ქვემოთ, თვალების გადაადგილების გარეშე, შეეცადეთ დაადგინოთ შუა ასო.",
    he: "בכל ניסיון, בזמן שאתה מסתכל על שערות הצלב, שלישייה של שלוש אותיות אקראיות תבהב בפינת העין שלך. רק פלאש. כדי ללמוד את המשימה, בואו ננסה אותה תחילה עם תצוגה סטטית. בזמן שאתה מסתכל על שערות הצלב, למטה, מבלי להזיז את העיניים, נסה לזהות את האות האמצעית.",
    ar: "في كل تجربة ، بينما تنظر إلى الشعر المتقاطع ، ستوضيب ثلاثة أحرف عشوائية في زاوية عينيك. مجرد ومضة. لمعرفة المهمة ، دعنا نجربها أولاً بعرض ثابت. بينما تنظر إلى الشعر المتقاطع ، أدناه ، دون تحريك عينيك ، حاول تحديد الحرف الأوسط.",
    ur: "ہر ٹیسٹ پر ، جب آپ کراس کے بالوں کو دیکھیں تو ، آپ کی آنکھ کے کونے میں تین بے ترتیب حرفوں کا ایک ٹرپل چمک اٹھے گا۔ صرف ایک جھلک کام کو جاننے کے ل first ، پہلے اسے مستحکم کارکردگی کے ساتھ آزمائیں۔ جب آپ نیچے کراس بال کو دیکھیں تو ، آنکھیں ہلائے بغیر ، درمیانی خط کی شناخت کرنے کی کوشش کریں۔",
    hi: "प्रत्येक परीक्षण पर, जब आप क्रॉस हेयर को देखते हैं, तो तीन यादृच्छिक अक्षरों का एक ट्रिपल आपकी आंख के कोने में चमक जाएगा। बस एक झलक। कार्य को जानने के लिए, इसे पहले एक स्थिर प्रदर्शन के साथ आज़माएं। जब आप क्रॉस हेयर को देखते हैं, तो नीचे, अपनी आँखें हिलाने के बिना, मध्य पत्र की पहचान करने का प्रयास करें।",
    ta: "ஒவ்வொரு சோதனையிலும், நீங்கள் குறுக்கு முடிகளைப் பார்க்கும்போது, ​​மூன்று சீரற்ற எழுத்துக்களின் மும்மடங்கு உங்கள் கண்ணின் மூலையில் ஒளிரும். ஒரு ஃபிளாஷ். பணியைக் கற்றுக்கொள்ள, நிலையான காட்சியுடன் முதலில் முயற்சிப்போம். குறுக்கு முடிகளைப் பார்க்கும்போது, ​​கீழே, கண்களை நகர்த்தாமல், நடுத்தர கடிதத்தை அடையாளம் காண முயற்சிக்கவும்.",
    ml: "ഓരോ വിചാരണയിലും, നിങ്ങൾ ക്രോസ് ഹെയർ നോക്കുമ്പോൾ, ക്രമരഹിതമായ മൂന്ന് അക്ഷരങ്ങളുടെ ഒരു മൂന്നിരട്ടി നിങ്ങളുടെ കണ്ണിന്റെ മൂലയിൽ മിന്നുമാകും. ഒരു ഫ്ലാഷ് മാത്രം. ടാസ്ക് പഠിക്കാൻ, ഒരു സ്റ്റാറ്റിക് ഡിസ്പ്ലേ ഉപയോഗിച്ച് ആദ്യം ഇത് പരീക്ഷിക്കാം. നിങ്ങളുടെ കണ്ണുകൾ നീക്കാതെ, ചുവടെയുള്ള ക്രോസ് ഹെയർ നോക്കുമ്പോൾ, മധ്യ കത്ത് തിരിച്ചറിയാൻ ശ്രമിക്കുക.",
    te: "ప్రతి ట్రయల్‌లో, మీరు క్రాస్ హెయిర్స్ వైపు చూస్తున్నప్పుడు, మూడు యాదృచ్ఛిక అక్షరాల యొక్క మూడు రెట్లు మీ కంటి మూలలో మెరుస్తుంది. కేవలం ఒక ఫ్లాష్. పనిని తెలుసుకోవడానికి, మొదట స్టాటిక్ డిస్ప్లేతో దీన్ని ప్రయత్నిద్దాం. మీరు కళ్ళు కదిలించకుండా, క్రింద, క్రింద ఉన్న క్రాస్ హెయిర్స్ వైపు చూస్తున్నప్పుడు, మధ్య అక్షరాన్ని గుర్తించడానికి ప్రయత్నించండి.",
    kn: "ಪ್ರತಿ ಪ್ರಯೋಗದಲ್ಲೂ, ನೀವು ಅಡ್ಡ ಕೂದಲನ್ನು ನೋಡುವಾಗ, ನಿಮ್ಮ ಕಣ್ಣಿನ ಮೂಲೆಯಲ್ಲಿ ಮೂರು ಯಾದೃಚ್ leters ಿಕ ಅಕ್ಷರಗಳ ತ್ರಿವಳಿಗಳು ಮಿಂಚುತ್ತವೆ. ಕೇವಲ ಒಂದು ಫ್ಲ್ಯಾಷ್. ಕಾರ್ಯವನ್ನು ಕಲಿಯಲು, ಸ್ಥಿರ ಪ್ರದರ್ಶನದೊಂದಿಗೆ ಮೊದಲು ಇದನ್ನು ಪ್ರಯತ್ನಿಸೋಣ. ನೀವು ಅಡ್ಡ ಕೂದಲನ್ನು ನೋಡುವಾಗ, ಕೆಳಗೆ, ನಿಮ್ಮ ಕಣ್ಣುಗಳನ್ನು ಚಲಿಸದೆ, ಮಧ್ಯದ ಅಕ್ಷರವನ್ನು ಗುರುತಿಸಲು ಪ್ರಯತ್ನಿಸಿ.",
    bn: "প্রতিটি পরীক্ষায়, আপনি ক্রস চুলের দিকে তাকানোর সময়, তিনটি এলোমেলো অক্ষরের একটি ট্রিপলেট আপনার চোখের কোণে ফ্ল্যাশ হয়ে যাবে। শুধু একটা ঝলকানি. টাস্কটি শিখতে, আসুন প্রথমে এটি একটি স্থির প্রদর্শন দিয়ে চেষ্টা করি। আপনি যখন আপনার চোখ না সরিয়ে নীচে ক্রস চুলের দিকে তাকান, মাঝের অক্ষরটি সনাক্ত করার চেষ্টা করুন।",
    id: "Pada setiap percobaan, saat Anda melihat rambut silang, kembar tiga dari tiga huruf acak akan berkedip di sudut mata Anda. Hanya sekejap. Untuk mempelajari tugas ini, mari kita coba terlebih dahulu dengan tampilan statis. Saat Anda melihat rambut salib, di bawah, tanpa menggerakkan mata, cobalah untuk mengidentifikasi huruf tengah.",
    "zh-CN":
      "每次试验中，在您注视十字准线的某个时刻，三个随机字母将短暂地出现在您的视野边缘。让我们来试一试。请注视十字准线，保持视线不动并试着认出中间的字母。",
    "zh-HK":
      "每次試驗中，在您注視十字準線的某個時刻，三個隨機字母將短暫地出現在您的視野邊緣。讓我們來試一試。請注視十字準線，保持視線不動並試著認出中間的字母。",
    ja: "各テストでは、あなたが十字線を見ている間に3つのランダムな文字があなたの目の端に短く表示されます。この場で試してみましょう。\n目を動かさずに十字線を見なが、真ん中の文字を識別してみてください。",
    ko: "각 시험에서 십자형 머리카락을 보면서 세 개의 임의의 문자가 눈의 모퉁이에 번쩍입니다. 단지 플래시. 작업을 배우려면 먼저 정적 디스플레이를 사용하여 시도해 봅시다. 눈을 움직이지 않고 아래의 십자 머리카락을 보는 동안 중간 문자를 식별하십시오.",
  },
  T_ok: {
    "en-US": "OK",
    de: "OK",
    fr: "OK",
    es: "DE ACUERDO",
    pt: "DE ACORDO",
    it: "OK",
    ro: "Bine",
    pl: "Ok",
    ru: "ХОРОШО",
    hy: "Տուգանք",
    fi: "Hyvä",
    ka: "კარგი",
    he: "בסדר",
    ar: "نعم",
    ur: "درست",
    hi: "ठीक",
    ta: "சரி",
    ml: "ശരി",
    te: "అలాగే",
    kn: "ಸರಿ",
    bn: "ঠিক আছে",
    id: "OKE",
    "zh-CN": "确认",
    "zh-HK": "確認",
    ja: "確認",
    ko: "좋아요",
  },
  T_participant: {
    "en-US": "Participant",
    de: "Teilnehmer",
    fr: "Participant",
    es: "Partícipante",
    pt: "Concorrente",
    it: "Partecipante",
    ro: "Concurent",
    pl: "Uczestnik",
    ru: "Участник",
    hy: "Մասնակից",
    fi: "Osallistuja",
    ka: "Მონაწილე",
    he: "מִשׁתַתֵף",
    ar: "مشارك",
    ur: "شریک",
    hi: "प्रतिभागी",
    ta: "பங்கேற்பாளராக",
    ml: "പങ്കാളി",
    te: "పాల్గొనేవారు",
    kn: "ಭಾಗವಹಿಸುವವನು",
    bn: "অংশগ্রহণকারী",
    id: "Peserta",
    "zh-CN": "被试",
    "zh-HK": "被試",
    ja: "参加者",
    ko: "참가자",
  },
  T_pressingKey: {
    "en-US": "by pressing it in the keyboard.",
    de: "indem Sie es in der Tastatur drücken.",
    fr: "Appuyez sur celui-ci sur le clavier.",
    es: "presionándola en el teclado.",
    pt: "pressionando -o no teclado.",
    it: "premendolo nella tastiera.",
    ro: "apăsând -o pe tastatură.",
    pl: "naciskając literę na klawiaturze. ",
    ru: "нажав его на клавиатуре.",
    hy: "Սեղմելով այն ստեղնաշարի վրա:",
    fi: "painamalla sitä näppäimistöön.",
    ka: "კლავიატურაზე დაჭერით.",
    he: "על ידי לחיצה עליו במקלדת.",
    ar: "بالضغط عليه في لوحة المفاتيح.",
    ur: "اسے کی بورڈ پر دبانے سے۔",
    hi: "इसे कीबोर्ड पर दबाकर।",
    ta: "விசைப்பலகையில் அதை அழுத்துவதன் மூலம்.",
    ml: "കീബോർഡിൽ അമർത്തിക്കൊണ്ട്.",
    te: "కీబోర్డ్‌లో నొక్కడం ద్వారా.",
    kn: "ಕೀಬೋರ್ಡ್‌ನಲ್ಲಿ ಒತ್ತುವ ಮೂಲಕ.",
    bn: "এটি কীবোর্ডে টিপে।",
    id: "dengan menekannya di keyboard.",
    "zh-CN": "通过按下键盘上的对应按键",
    "zh-HK": "通過按下鍵盤上的對應按鍵",
    ja: "キーボードで対応するキーを押すことで",
    ko: "키보드에 눌러.",
  },
  T_pressingKeyOrClickingLetter: {
    "en-US":
      "by clicking it in the displayed list of letters or pressing it in the keyboard.",
    de: "indem Sie es in der Tastatur drücken oder in der angezeigten Buchstabenliste anklicken.",
    fr: "Appuyez sur celui-ci sur le clavier ou en cliquant sur la liste des lettres affichées.",
    es: "presionándola en el teclado o haciendo clic en ella en la lista de letras que se muestra.",
    pt: "pressionando -o no teclado ou clicando na lista de letras mostradas.",
    it: "premendolo sulla tastiera o facendo clic su di esse nell'elenco di lettere visualizzate.",
    ro: "apăsând -o pe tastatură sau făcând clic pe lista de litere afișate.",
    pl: "Naciśnij na klawiaturze lub kliknij na liście liter. ",
    ru: "нажав его на клавиатуре или щелкнув в отображаемом списке букв.",
    hy: "Սեղմելով այն ստեղնաշարի վրա կամ կտտացնելով տառերի ցուցադրված ցուցակում:",
    fi: "Napsauttamalla sitä näytetyssä kirjainluettelossa tai painamalla sitä näppäimistössä.",
    ka: "დააჭირეთ მას ასოების ნაჩვენები სიაში ან დააჭირეთ მას კლავიატურაზე.",
    he: "על ידי לחיצה עליו במקלדת או לחיצה עליו ברשימת האותיות המוצגת.",
    ar: "بالضغط عليه في لوحة المفاتيح أو الضغط عليه في قائمة الأحرف المعروضة على الشاشة.",
    ur: "اسے کی بورڈ پر دبانے یا خطوط کی ڈسپلے لسٹ پر کلک کرکے۔",
    hi: "इसे कीबोर्ड पर दबाकर या अक्षरों की प्रदर्शित सूची पर इसे क्लिक करके।",
    ta: "காண்பிக்கப்படும் எழுத்துக்களின் பட்டியலில் அதைக் கிளிக் செய்வதன் மூலம் அல்லது விசைப்பலகையில் அழுத்துவதன் மூலம்.",
    ml: "പ്രദർശിപ്പിച്ച അക്ഷരങ്ങളുടെ പട്ടികയിൽ ക്ലിക്കുചെയ്യുന്നതിലൂടെ അല്ലെങ്കിൽ കീബോർഡിൽ അമർത്തുക.",
    te: "ప్రదర్శించిన అక్షరాల జాబితాలో క్లిక్ చేయడం ద్వారా లేదా కీబోర్డ్‌లో నొక్కడం ద్వారా.",
    kn: "ಅದನ್ನು ಪ್ರದರ್ಶಿಸಿದ ಅಕ್ಷರಗಳ ಪಟ್ಟಿಯಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡುವ ಮೂಲಕ ಅಥವಾ ಕೀಬೋರ್ಡ್‌ನಲ್ಲಿ ಒತ್ತುವ ಮೂಲಕ.",
    bn: "অক্ষরগুলির প্রদর্শিত তালিকায় এটি ক্লিক করে বা কীবোর্ডে টিপুন।",
    id: "Dengan mengkliknya dalam daftar huruf yang ditampilkan atau menekannya di keyboard.",
    "zh-CN": "通过按下键盘上的对应按键或者点击给出文字中的相应字母。",
    "zh-HK": "通過按下鍵盤上的對應按鍵或者點擊給出文字中的相應字母。",
    ja: "キーボードで対応するキーを押すか、表示されている文字のリストをクリックすることで",
    ko: "표시된 문자 목록에서 클릭하거나 키보드에 누르면됩니다.",
  },
  T_pressingKeyOrClickingLetterRepeatedLetters: {
    "en-US":
      "by clicking them on the displayed list of letters, or pressing them on the keyboard.",
    de: "Durch Klicken auf die angezeigte Liste der Buchstaben oder das Drücken auf der Tastatur.",
    fr: "En les cliquant sur la liste affichée des lettres ou en les appuyant sur le clavier.",
    es: "Al hacer clic en la lista mostrada de letras, o presionándolas en el teclado.",
    pt: "Ao clicar neles na lista exibida de letras ou pressioná -las no teclado.",
    it: "Facendo clic su di loro nell'elenco visualizzato delle lettere o premendole sulla tastiera.",
    ro: "Făcând clic pe ele pe lista afișată de litere sau apăsându -le pe tastatură.",
    pl: "Klikając je na wyświetlonej listy liter lub naciskając je na klawiaturze.",
    ru: "Нажав их в отображаемом списке букв или нажав их на клавиатуре.",
    hy: "Կտտացնելով դրանք տառերի ցուցադրված ցուցակում կամ սեղմելով դրանք ստեղնաշարի վրա:",
    fi: "Napsauttamalla niitä näytetyssä kirjainluettelossa tai painamalla niitä näppäimistöllä.",
    ka: "დაჭერით მათ ასოების ნაჩვენები სიაში, ან კლავიატურაზე დაჭერით.",
    he: "על ידי לחיצה עליהם ברשימת האותיות המוצגת, או לחיצה על המקלדת.",
    ar: "من خلال النقر عليها على قائمة الرسائل المعروضة ، أو الضغط عليها على لوحة المفاتيح.",
    ur: "خطوط کی ظاہر کردہ فہرست پر ان پر کلک کرکے ، یا کی بورڈ پر دبانے سے۔",
    hi: "पत्रों की प्रदर्शित सूची में उन्हें क्लिक करके, या उन्हें कीबोर्ड पर दबाकर।",
    ta: "காட்டப்பட்ட கடிதங்களின் பட்டியலில் அவற்றைக் கிளிக் செய்வதன் மூலம் அல்லது விசைப்பலகையில் அவற்றை அழுத்துவதன் மூலம்.",
    ml: "പ്രദർശിപ്പിച്ച അക്ഷരങ്ങളുടെ പട്ടികയിൽ അവ ക്ലിക്കുചെയ്യുന്നതിലൂടെ, അല്ലെങ്കിൽ കീബോർഡിൽ അമർത്തുക.",
    te: "ప్రదర్శించిన అక్షరాల జాబితాలో వాటిని క్లిక్ చేయడం ద్వారా లేదా వాటిని కీబోర్డ్‌లో నొక్కడం ద్వారా.",
    kn: "ಅವುಗಳನ್ನು ಪ್ರದರ್ಶಿಸಿದ ಅಕ್ಷರಗಳ ಪಟ್ಟಿಯಲ್ಲಿ ಕ್ಲಿಕ್ ಮಾಡುವ ಮೂಲಕ ಅಥವಾ ಕೀಬೋರ್ಡ್‌ನಲ್ಲಿ ಒತ್ತುವ ಮೂಲಕ.",
    bn: "চিঠির প্রদর্শিত তালিকায় এগুলি ক্লিক করে বা কীবোর্ডে টিপুন।",
    id: "Dengan mengkliknya pada daftar huruf yang ditampilkan, atau menekannya pada keyboard.",
    "zh-CN": "通过在显示的字母列表中单击它们，或在键盘上按下它们。",
    "zh-HK": "通過在顯示的字母列表中單擊它們，或在鍵盤上按下它們。",
    ja: "表示された文字のリストでそれらをクリックするか、キーボードで押します。",
    ko: "표시된 문자 목록에서 클릭하거나 키보드에 누르면됩니다.",
  },
  T_pressingKeyRepeatedLetters: {
    "en-US": "by pressing them on the keyboard.",
    de: "durch Drücken Sie sie auf die Tastatur.",
    fr: "en les appuyant sur le clavier.",
    es: "presionándolos en el teclado.",
    pt: "pressionando -os no teclado.",
    it: "premendoli sulla tastiera.",
    ro: "apăsându -le pe tastatură.",
    pl: "naciskając je na klawiaturze.",
    ru: "Нажав их на клавиатуре.",
    hy: "սեղմելով դրանք ստեղնաշարի վրա:",
    fi: "painamalla niitä näppäimistölle.",
    ka: "კლავიატურაზე დაჭერით.",
    he: "על ידי לחיצה עליהם על המקלדת.",
    ar: "عن طريق الضغط عليها على لوحة المفاتيح.",
    ur: "انہیں کی بورڈ پر دبانے سے۔",
    hi: "उन्हें कीबोर्ड पर दबाकर।",
    ta: "விசைப்பலகையில் அவற்றை அழுத்துவதன் மூலம்.",
    ml: "കീബോർഡിൽ അമർത്തിക്കൊണ്ട്.",
    te: "కీబోర్డ్‌లో వాటిని నొక్కడం ద్వారా.",
    kn: "ಕೀಬೋರ್ಡ್‌ನಲ್ಲಿ ಅವುಗಳನ್ನು ಒತ್ತುವ ಮೂಲಕ.",
    bn: "তাদের কীবোর্ডে টিপে।",
    id: "Dengan menekannya di keyboard.",
    "zh-CN": "通过在键盘上按下它们。",
    "zh-HK": "通過在鍵盤上按下它們。",
    ja: "キーボードでそれらを押すことにより。",
    ko: "키보드에 눌러서.",
  },
  T_pressingKeyRsvpReading: {
    "en-US": "by saying the words aloud.",
    de: "indem Sie die Worte laut sagen.",
    fr: "en disant les mots à haute voix.",
    es: "diciendo las palabras en voz alta.",
    pt: "dizendo as palavras em voz alta.",
    it: "dicendo le parole ad alta voce.",
    ro: "spunând cuvintele cu voce tare.",
    pl: "Mówiąc na głos.",
    ru: "Сказав слова вслух.",
    hy: "Բառերը բարձրաձայն ասելով:",
    fi: "Sanomalla sanat ääneen.",
    ka: "სიტყვების ხმამაღლა თქმით.",
    he: "באמירת המילים בקול רם.",
    ar: "بقول الكلمات بصوت عال.",
    ur: "الفاظ بلند آواز سے کہہ کر۔",
    hi: "शब्दों को जोर से कहकर।",
    ta: "வார்த்தைகளை சத்தமாகச் சொல்வதன் மூலம்.",
    ml: "വാക്കുകൾ ഉറക്കെ പറഞ്ഞുകൊണ്ട്.",
    te: "మాటలు బిగ్గరగా చెప్పడం ద్వారా.",
    kn: "ಪದಗಳನ್ನು ಗಟ್ಟಿಯಾಗಿ ಹೇಳುವ ಮೂಲಕ.",
    bn: "উচ্চস্বরে কথা বলে।",
    id: "dengan mengucapkan kata -kata itu dengan keras.",
    "zh-CN": "大声说出这些字。",
    "zh-HK": "大聲說出這些字。",
    ja: "言葉を声に出して言うことによって。",
    ko: "말을 큰 소리로 말함으로써.",
  },
  T_PROCEED: {
    "en-US": "PROCEED",
    de: "FORTFAHREN",
    fr: "PROCÉDER",
    es: "PROCEDER",
    pt: "CONTINUAR",
    it: "PROCEDERE",
    ro: "CONTINUA",
    pl: "PRZYSTĘPOWAĆ",
    ru: "ПРОДОЛЖИТЬ",
    hy: "ՎԱՐԵԼ",
    fi: "EDETÄ",
    ka: "ᲒᲐᲜᲒᲠᲫᲝᲑᲐ",
    he: "להמשיך",
    ar: "يتابع",
    ur: "آگے بڑھو",
    hi: "आगे बढ़ना",
    ta: "தொடரவும்",
    ml: "മുന്നോട്ടുനീങ്ങുക",
    te: "కొనసాగండి",
    kn: "ಮುಂದುವರೆಯಲು",
    bn: "অগ্রসর",
    id: "MELANJUTKAN",
    "zh-CN": "继续",
    "zh-HK": "繼續",
    ja: "続行",
    ko: "진행하다",
  },
  T_proceed: {
    "en-US": "Proceed",
    de: "Fortfahren",
    fr: "Procéder",
    es: "Proceder",
    pt: "Continuar",
    it: "Procedere",
    ro: "Continua",
    pl: "Przystępować",
    ru: "Продолжить",
    hy: "Վարել",
    fi: "Edetä",
    ka: "განგრძობა",
    he: "להמשיך",
    ar: "يتابع",
    ur: "آگے بڑھو",
    hi: "आगे बढ़ना",
    ta: "தொடரவும்",
    ml: "മുന്നോട്ടുനീങ്ങുക",
    te: "కొనసాగండి",
    kn: "ಮುಂದುವರೆಯಲು",
    bn: "অগ্রসর",
    id: "Melanjutkan",
    "zh-CN": "继续",
    "zh-HK": "繼續",
    ja: "続行",
    ko: "진행하다",
  },
  T_proportionCorrectPopup: {
    "en-US": "Nice work! You got xxx% right.",
    de: "Gute Arbeit! Sie haben xxx% richtig.",
    fr: "Bon travail! Vous avez bien xxx%.",
    es: "¡Buen trabajo! Tienes xxx% correcto.",
    pt: "Bom trabalho! Você tem xxx% correto.",
    it: "Bel lavoro! Hai il xxx% giusto.",
    ro: "Buna treaba! Ai xxx% corect.",
    pl: "Dobra robota! Masz xxx%.",
    ru: "Хорошая работа! Вы получили xxx% верно.",
    hy: "Լավ աշխատանք! Դուք ստացել եք xxx% True:",
    fi: "Hyvää työtä! Sinulla on xxx% oikeassa.",
    ka: "Კარგი მუშაობა! თქვენ მიიღეთ xxx% მართალი.",
    he: "עבודה נחמדה! יש לך xxx% נכון.",
    ar: "عمل جيد! لقد حصلت على xxx ٪ بشكل صحيح.",
    ur: "اچھا کام! آپ کو xxx ٪ درست ملا۔",
    hi: "अच्छा काम! आपको xxx% सही मिला।",
    ta: "நன்றாக செய்தாய்! உங்களுக்கு xxx% சரியானது.",
    ml: "നല്ല ജോലി! നിങ്ങൾക്ക് xxx% ശരിയാണ്.",
    te: "మంచి పని! మీకు xxx% సరైనది.",
    kn: "ಉತ್ತಮವಾದ ಕೆಲಸ! ನಿಮಗೆ xxx% ಸರಿ ಸಿಕ್ಕಿದೆ.",
    bn: "চমৎকার কাজ! আপনি xxx% ঠিক পেয়েছেন।",
    id: "Kerja bagus! Anda mendapat xxx% benar.",
    "zh-CN": "做得好！您的xxx％正确。",
    "zh-HK": "做得好！您的xxx％正確。",
    ja: "よくやった！ xxx％が正しくなりました。",
    ko: "잘 하셨어요! 당신은 xxx%가 맞습니다.",
  },
  T_readingNextPage: {
    "en-US": "Press SPACE for next page.",
    de: "Drücken Sie Platz für die nächste Seite.",
    fr: "Appuyez sur l'espace pour la page suivante.",
    es: "Presione el espacio para la página siguiente.",
    pt: "Pressione espaço para a próxima página.",
    it: "Premere Space per la pagina successiva.",
    ro: "Apăsați spațiul pentru pagina următoare.",
    pl: "Naciśnij miejsce na następną stronę.",
    ru: "Нажмите пространство для следующей страницы.",
    hy: "Մամուլի տարածություն հաջորդ էջի համար:",
    fi: "Paina seuraava sivua.",
    ka: "დააჭირეთ ადგილს შემდეგი გვერდისთვის.",
    he: "לחץ על שטח לדף הבא.",
    ar: "اضغط على مساحة للصفحة التالية.",
    ur: "اگلے صفحے کے لئے جگہ دبائیں۔",
    hi: "अगले पृष्ठ के लिए स्थान दबाएं।",
    ta: "அடுத்த பக்கத்திற்கான இடத்தை அழுத்தவும்.",
    ml: "അടുത്ത പേജിനായി സ്പേസ് അമർത്തുക.",
    te: "తదుపరి పేజీ కోసం స్థలాన్ని నొక్కండి.",
    kn: "ಮುಂದಿನ ಪುಟಕ್ಕಾಗಿ ಸ್ಥಳವನ್ನು ಒತ್ತಿರಿ.",
    bn: "পরবর্তী পৃষ্ঠার জন্য স্পেস টিপুন।",
    id: "Tekan ruang untuk halaman berikutnya.",
    "zh-CN": "按下下一页的空间。",
    "zh-HK": "按下下一頁的空間。",
    ja: "次のページのスペースを押します。",
    ko: "다음 페이지를 위해 공간을 누릅니다.",
  },
  T_rsvpReadingTask: {
    "en-US":
      "You are about to start a block of xxx trials. You'll be readings a story, a few words at a time. Each trial will quickly show you about a sentence-worth of words, one after the other, and then ask you to remember what words you read.",
    de: "Sie sind dabei, einen Block von xxx -Versuchen zu starten. Sie werden eine Geschichte lesen, ein paar Worte gleichzeitig. Jeder Versuch zeigt Ihnen schnell über ein Satz von Wörtern nacheinander und bittet Sie dann, sich daran zu erinnern, welche Wörter Sie lesen.",
    fr: "Vous êtes sur le point de démarrer un bloc d'essais xxx. Vous aurez des lectures une histoire, quelques mots à la fois. Chaque essai vous montrera rapidement une phrase de mots, l'une après l'autre, puis vous demandera de vous rappeler quels mots vous lisez.",
    es: "Estás a punto de comenzar un bloque de pruebas xxx. Serás lecturas de una historia, algunas palabras a la vez. Cada juicio le mostrará rápidamente sobre un valor de las palabras, una tras otra, y luego le pedirá que recuerde qué palabras lees.",
    pt: "Você está prestes a iniciar um bloco de ensaios xxx. Você será uma história de leitura, algumas palavras de cada vez. Cada tentativa mostrará rapidamente sobre uma frase de palavras, uma após a outra, e depois pedirá que você se lembre de quais palavras você lê.",
    it: "Stai per avviare un blocco di prove xxx. Letturete una storia, alcune parole alla volta. Ogni prova ti mostrerà rapidamente una pena di parole, una dopo l'altra, e poi ti chiederà di ricordare quali parole leggi.",
    ro: "Sunteți pe cale să începeți un bloc de încercări xxx. Vei citi o poveste, câteva cuvinte la un moment dat. Fiecare proces vă va arăta rapid despre o propoziție de cuvinte, una după alta, apoi vă va cere să vă amintiți ce cuvinte citiți.",
    pl: "Niedługo rozpoczniesz blok prób xxx. Będziesz czytał historię, kilka słów na raz. Każda próba szybko pokaże ci o wartości zdania, jeden po drugim, a następnie poprosi o pamiętanie, jakie słowa przeczytasz.",
    ru: "Вы собираетесь запустить блок xxx испытаний. Вы станете чтением историей, по нескольким словам за раз. Каждое испытание быстро покажет вам о поклонении слов, один за другим, а затем попросит вас вспомнить, какие слова вы читаете.",
    hy: "Դուք պատրաստվում եք սկսել xxx փորձարկումների բլոկ: Դուք ընթերցում եք մի պատմություն, միանգամից մի քանի բառ: Յուրաքանչյուր փորձություն արագորեն ցույց կտա ձեզ մի նախադասության մասին բառերի մասին, մեկը մյուսի հետեւից, եւ հետո խնդրեք հիշել, թե ինչ բառեր եք կարդում:",
    fi: "Olet aloittamassa xxx -kokeiden lohkon. Luket tarinaa, muutama sana kerrallaan. Jokainen oikeudenkäynti näyttää sinulle nopeasti lauseen arvosta sanoja peräkkäin ja pyydä sitten muistamaan, mitä sanoja olet lukenut.",
    ka: "თქვენ აპირებთ xxx ტესტების ბლოკის დაწყებას. თქვენ კითხულობთ მოთხრობას, ერთდროულად რამდენიმე სიტყვას. თითოეული სასამართლო პროცესი სწრაფად გაჩვენებთ სიტყვების წინადადებას, ერთი მეორის მიყოლებით, შემდეგ კი მოგთხოვთ გახსოვდეთ რა სიტყვები წაიკითხეთ.",
    he: "אתה עומד להתחיל בלוק של ניסויים ב- xxx. אתה תהיה קריאה סיפור, כמה מילים בכל פעם. כל משפט יראה לך במהירות על שווי משפט של מילים, בזה אחר זה, ואז יבקש ממך לזכור אילו מילים קראת.",
    ar: "أنت على وشك بدء كتلة من تجارب xxx. سوف تكون قراءات قصة ، بضع كلمات في وقت واحد. ستظهر لك كل تجربة بسرعة حول قيمة الجملة ، واحدة تلو الأخرى ، ثم تطلب منك أن تتذكر الكلمات التي تقرأها.",
    ur: "آپ xxx ٹرائلز کا ایک بلاک شروع کرنے والے ہیں۔ آپ ایک وقت میں ایک کہانی ، کچھ الفاظ پڑھیں گے۔ ہر مقدمے کی سماعت آپ کو ایک جملے کے قابل الفاظ کے بارے میں جلدی سے دکھائے گی ، ایک کے بعد ایک ، اور پھر آپ سے یہ یاد کرنے کے لئے کہے کہ آپ کیا الفاظ پڑھتے ہیں۔",
    hi: "आप xxx परीक्षणों का एक ब्लॉक शुरू करने वाले हैं। आप एक कहानी, एक समय में कुछ शब्द पढ़ेंगे। प्रत्येक परीक्षण आपको जल्दी से शब्दों के वाक्य-मूल्य के बारे में दिखाएगा, एक के बाद एक, और फिर आपको याद करने के लिए कहेगा कि आप कौन से शब्द पढ़ते हैं।",
    ta: "நீங்கள் xxx சோதனைகளின் ஒரு தொகுதியைத் தொடங்க உள்ளீர்கள். நீங்கள் ஒரு கதையை வாசிப்பீர்கள், ஒரு நேரத்தில் சில சொற்கள். ஒவ்வொரு சோதனையும் ஒரு வாக்கியம் மதிப்புள்ள சொற்களைப் பற்றி விரைவாக உங்களுக்குக் காண்பிக்கும், ஒன்றன் பின் ஒன்றாக, பின்னர் நீங்கள் என்ன வார்த்தைகளைப் படிக்கிறீர்கள் என்பதை நினைவில் கொள்ளும்படி கேட்கவும்.",
    ml: "നിങ്ങൾ xxx ട്രയലുകളുടെ ഒരു ബ്ലോക്ക് ആരംഭിക്കാൻ പോകുന്നു. നിങ്ങൾ ഒരു കഥ വായിക്കുന്നതായിരിക്കും, ഒരു സമയം കുറച്ച് വാക്കുകൾ. ഓരോ ട്രയലും ഒരു വാക്യ വാക്കുകളെക്കുറിച്ച് വേഗത്തിൽ കാണിക്കും, തുടർന്ന് നിങ്ങൾ വായിക്കുന്ന വാക്കുകൾ ഓർമ്മിക്കാൻ നിങ്ങളോട് ആവശ്യപ്പെടുക.",
    te: "మీరు xxx ట్రయల్స్ యొక్క బ్లాక్‌ను ప్రారంభించబోతున్నారు. మీరు ఒక కథను, ఒకేసారి కొన్ని పదాలు రీడింగులు చేస్తారు. ప్రతి ట్రయల్ పదాల వాక్య-విలువ గురించి, ఒకదాని తరువాత ఒకటి తర్వాత మీకు త్వరగా చూపుతుంది, ఆపై మీరు ఏ పదాలు చదివారో గుర్తుంచుకోవాలని మిమ్మల్ని అడగండి.",
    kn: "ನೀವು xxx ಪ್ರಯೋಗಗಳ ಬ್ಲಾಕ್ ಅನ್ನು ಪ್ರಾರಂಭಿಸಲಿದ್ದೀರಿ. ನೀವು ಒಂದು ಕಥೆಯನ್ನು, ಒಂದು ಸಮಯದಲ್ಲಿ ಕೆಲವು ಪದಗಳನ್ನು ವಾಚ ಹೊಂದುತ್ತೀರಿ. ಪ್ರತಿ ಪ್ರಯೋಗವು ಒಂದರ ನಂತರ ಒಂದರಂತೆ ಒಂದು ವಾಕ್ಯ-ಮೌಲ್ಯದ ಪದಗಳ ಬಗ್ಗೆ ತ್ವರಿತವಾಗಿ ನಿಮಗೆ ತೋರಿಸುತ್ತದೆ, ತದನಂತರ ನೀವು ಯಾವ ಪದಗಳನ್ನು ಓದಿದ್ದೀರಿ ಎಂಬುದನ್ನು ನೆನಪಿಟ್ಟುಕೊಳ್ಳಲು ಕೇಳುತ್ತೀರಿ.",
    bn: "আপনি xxx ট্রায়ালগুলির একটি ব্লক শুরু করতে চলেছেন। আপনি একটি গল্প পড়বেন, একবারে কয়েকটি শব্দ। প্রতিটি পরীক্ষা আপনাকে একের পর এক একের পর এক বাক্য-মূল্যবান শব্দ সম্পর্কে দ্রুত প্রদর্শন করবে এবং তারপরে আপনি কোন শব্দগুলি পড়েছেন তা মনে রাখতে আপনাকে জিজ্ঞাসা করুন।",
    id: "Anda akan memulai blok uji coba xxx. Anda akan membaca sebuah cerita, beberapa kata sekaligus. Setiap uji coba akan dengan cepat menunjukkan kepada Anda tentang kata-kata yang bernilai kalimat, satu demi satu, dan kemudian meminta Anda untuk mengingat kata-kata apa yang Anda baca.",
    "zh-CN":
      "您即将开始xxx试验。您会读一个故事，一次读几句话。每个审判都会迅速向您展示一个值得一句话的句子，然后要求您记住您阅读的单词。",
    "zh-HK":
      "您即將開始xxx試驗。您會讀一個故事，一次讀幾句話。每個審判都會迅速向您展示一個值得一句話的句子，然後要求您記住您閱讀的單詞。",
    ja: "xxxトライアルのブロックを開始しようとしています。あなたは一度にいくつかの言葉を読むでしょう。各トライアルは、次々に一言で言えば、文の文章についてすぐに表示され、読んだ言葉を覚えておくように頼みます。",
    ko: "xxx 시험 블록을 시작하려고합니다. 한 번에 몇 마디 씩 이야기를 읽을 수 있습니다. 각 시험은 다른 단어의 문장 가치에 대해 신속하게 보여준 다음, 당신이 읽은 단어를 기억하도록 요청합니다.",
  },
  T_readingTask: {
    "en-US":
      "Story time! You'll read 111 pages from a book. Once you start, please silently read all the pages as fast as you can, while maintaining full comprehension, until you reach the end. Then we'll ask you a few questions about the story. On each page, press the SPACE bar for the next page. When you're ready, press SPACE.",
    de: "Märchenstunde! Sie werden 111 Seiten aus einem Buch lesen. Sobald Sie angefangen haben, lesen Sie bitte alle Seiten schweigend so schnell wie möglich, während Sie das volle Verständnis beibehalten, bis Sie das Ende erreichen. Dann stellen wir Ihnen ein paar Fragen zur Geschichte. Drücken Sie auf jeder Seite die Space -Leiste für die nächste Seite. Wenn Sie bereit sind, drücken Sie Platz.",
    fr: "L'heure du conte! Vous lirez 111 pages d'un livre. Onzes vous commencez, veuillez lire silencieusement toutes les pages aussi vite que possible, tout en maintenant la pleine compréhension, jusqu'à ce que vous atteigniez la fin. Nous vous poserons quelques questions sur l'histoire. Sur chaque page, appuyez sur la barre d'espace pour la page suivante. Lorsque vous êtes prêt, appuyez sur l'espace.",
    es: "¡Tiempo de cuentos! Leerás 111 páginas de un libro. Una vez que comience, lea en silencio todas las páginas lo más rápido que pueda, mientras mantiene la comprensión completa, hasta llegar al final. Luego le haremos algunas preguntas sobre la historia. En cada página, presione la barra espacial para la página siguiente. Cuando esté listo, presione el espacio.",
    pt: "Hora da história! Você lerá 111 páginas de um livro. Depois de começar, leia silenciosamente todas as páginas o mais rápido possível, mantendo a compreensão total, até chegar ao fim. Em seguida, vamos fazer algumas perguntas sobre a história. Em cada página, pressione a barra de espaço para a próxima página. Quando estiver pronto, pressione o espaço.",
    it: "Ora della favola! Leggerai 111 pagine da un libro. Una volta che inizi, leggi silenziosamente tutte le pagine il più velocemente possibile, mantenendo la piena comprensione, fino a raggiungere la fine. Quindi ti faremo alcune domande sulla storia. Su ogni pagina, premere la barra dello spazio per la pagina successiva. Quando sei pronto, premere lo spazio.",
    ro: "Timpul pentru o poveste! Veți citi 111 pagini dintr -o carte. După ce începeți, vă rugăm să citiți în tăcere toate paginile cât de repede puteți, păstrând în același timp o înțelegere completă, până când ajungeți la sfârșit. Apoi vă vom pune câteva întrebări despre poveste. Pe fiecare pagină, apăsați bara de spațiu pentru pagina următoare. Când sunteți gata, apăsați spațiu.",
    pl: "Czas opowiadania! Przeczytasz 111 stron z książki. Kiedy zaczniesz, po cichu przeczytaj wszystkie strony tak szybko, jak to możliwe, przy jednoczesnym zachowaniu pełnego zrozumienia, dopóki nie osiągniesz końca. Następnie zadamy Ci kilka pytań na temat historii. Na każdej stronie naciśnij pasek przestrzeni dla następnej strony. Kiedy będziesz gotowy, naciśnij przestrzeń.",
    ru: "Время историй! Вы прочитаете 111 страницы из книги. После того, как вы начнете, пожалуйста, молча прочитайте все страницы как можно быстрее, сохраняя при этом полное понимание, пока не достигнете конца. Тогда мы зададим вам несколько вопросов об истории. На каждой странице нажмите пробел, для следующей страницы. Когда вы будете готовы, нажмите пространство.",
    hy: "Պատմության ժամանակը Դուք կարդում եք 111 էջ գրքից: Սկսելուց հետո լուռ կարդացեք բոլոր էջերը հնարավորինս արագ, միաժամանակ պահպանելով ամբողջական հասկացողություն, մինչեւ հասնեք ավարտին: Այնուհետեւ մենք ձեզ մի քանի հարց կտանք պատմության վերաբերյալ: Յուրաքանչյուր էջում սեղմեք տիեզերական սանդղակը հաջորդ էջի համար: Երբ պատրաստ եք, սեղմեք տարածքը:",
    fi: "Satuhetki! Luet 111 sivua kirjasta. Kun olet aloittanut, lue kaikki sivut hiljaa niin nopeasti kuin pystyt pitäen samalla ymmärrystä, kunnes saavut loppuun. Sitten kysymme sinulle muutamia kysymyksiä tarinasta. Paina jokaisella sivulla seuraavan sivun välilyöntiä. Kun olet valmis, paina tilaa.",
    ka: "Მოთხრობის დროა! თქვენ წაიკითხავთ 111 გვერდს წიგნიდან. დაწყებისთანავე, გთხოვთ, ჩუმად წაიკითხოთ ყველა გვერდი რაც შეიძლება სწრაფად, სრულად გააზრების შენარჩუნებისას, სანამ არ მიაღწევთ ბოლომდე. შემდეგ მოგვმართავთ რამდენიმე კითხვას ამბის შესახებ. თითოეულ გვერდზე, დააჭირეთ Space Bar– ს შემდეგი გვერდისთვის. როდესაც მზად ხართ, დააჭირეთ ადგილს.",
    he: "זמן סיפור! תקרא 111 עמודים מתוך ספר. לאחר שתתחיל, אנא קרא בשקט את כל הדפים הכי מהר שאתה יכול, תוך שמירה על הבנה מלאה, עד שתגיע לסוף. ואז נשאל אותך כמה שאלות על הסיפור. בכל עמוד, לחץ על סרגל החלל לדף הבא. כשאתה מוכן, לחץ על שטח.",
    ar: "وقت القصة! ستقرأ 111 صفحة من كتاب. بمجرد أن تبدأ ، يرجى قراءة جميع الصفحات بصمت بأسرع ما يمكن ، مع الحفاظ على الفهم الكامل ، حتى تصل إلى النهاية. ثم سنطرح عليك بعض الأسئلة حول القصة. في كل صفحة ، اضغط على شريط الفضاء للصفحة التالية. عندما تكون جاهزًا ، اضغط على الفضاء.",
    ur: "کہانی کا وقت! آپ کسی کتاب سے 111 صفحات پڑھیں گے۔ ایک بار جب آپ شروع کریں تو ، براہ کرم خاموشی سے تمام صفحات کو جتنی جلدی ہو سکے پڑھیں ، جبکہ مکمل فہم کو برقرار رکھتے ہوئے ، جب تک کہ آپ اختتام تک نہ پہنچیں۔ تب ہم آپ سے کہانی کے بارے میں کچھ سوالات پوچھیں گے۔ ہر صفحے پر ، اگلے صفحے کے لئے اسپیس بار دبائیں۔ جب آپ تیار ہوں تو ، جگہ دبائیں۔",
    hi: "कहानी की समय! आप एक पुस्तक से 111 पृष्ठ पढ़ेंगे। एक बार जब आप शुरू कर देते हैं, तो कृपया चुपचाप सभी पृष्ठों को जितनी तेजी से पढ़ सकें, तब तक पूरी समझ बनाए रखते हुए, जब तक आप अंत तक नहीं पहुंच जाते। फिर हम आपसे कहानी के बारे में कुछ सवाल पूछेंगे। प्रत्येक पृष्ठ पर, अगले पृष्ठ के लिए स्पेस बार दबाएं। जब आप तैयार हों, तो स्थान दबाएं।",
    ta: "கதை நேரம்! ஒரு புத்தகத்திலிருந்து 111 பக்கங்களைப் படிப்பீர்கள். நீங்கள் தொடங்கியதும், தயவுசெய்து எல்லா பக்கங்களையும் உங்களால் முடிந்தவரை விரைவாகப் படியுங்கள், முழு புரிதலையும் பராமரிக்கும் போது, ​​நீங்கள் முடிவை அடையும் வரை. கதையைப் பற்றி சில கேள்விகளை நாங்கள் உங்களிடம் கேட்போம். ஒவ்வொரு பக்கத்திலும், அடுத்த பக்கத்திற்கான விண்வெளி பட்டியை அழுத்தவும். நீங்கள் தயாராக இருக்கும்போது, ​​இடத்தை அழுத்தவும்.",
    ml: "സ്റ്റോറി സമയം! ഒരു പുസ്തകത്തിൽ നിന്ന് നിങ്ങൾ 111 പേജുകൾ വായിക്കും. നിങ്ങൾ ആരംഭിച്ചുകഴിഞ്ഞാൽ, ദയവായി എല്ലാ പേജുകളും നിങ്ങൾക്ക് കഴിയുന്നത്ര വേഗത്തിൽ വായിക്കുക, നിങ്ങൾ അവസാനം എത്തുന്നതുവരെ പൂർണ്ണസംഖ്യ നിലനിർത്തുമ്പോൾ. കഥയെക്കുറിച്ച് ഞങ്ങൾ നിങ്ങളോട് കുറച്ച് ചോദ്യങ്ങൾ ചോദിക്കും. ഓരോ പേജിലും, അടുത്ത പേജിനായി സ്പേസ് ബാർ അമർത്തുക. നിങ്ങൾ തയ്യാറാകുമ്പോൾ, സ്പേസ് അമർത്തുക.",
    te: "కథ సమయం! మీరు పుస్తకం నుండి 111 పేజీలు చదువుతారు. మీరు ప్రారంభించిన తర్వాత, దయచేసి మీరు చివరికి చేరే వరకు పూర్తి గ్రహణశక్తిని కొనసాగిస్తూ, అన్ని పేజీలను నిశ్శబ్దంగా మీకు వీలైనంత వేగంగా చదవండి. అప్పుడు మేము కథ గురించి కొన్ని ప్రశ్నలు అడుగుతాము. ప్రతి పేజీలో, తదుపరి పేజీ కోసం స్పేస్ బార్‌ను నొక్కండి. మీరు సిద్ధంగా ఉన్నప్పుడు, స్థలాన్ని నొక్కండి.",
    kn: "ಕಥೆಯ ಸಮಯ! ನೀವು ಪುಸ್ತಕದಿಂದ 111 ಪುಟಗಳನ್ನು ಓದುತ್ತೀರಿ. ನೀವು ಪ್ರಾರಂಭಿಸಿದ ನಂತರ, ದಯವಿಟ್ಟು ಎಲ್ಲಾ ಪುಟಗಳನ್ನು ನಿಮಗೆ ಸಾಧ್ಯವಾದಷ್ಟು ವೇಗವಾಗಿ ಓದಿ, ಪೂರ್ಣ ಗ್ರಹಿಕೆಯನ್ನು ಕಾಪಾಡಿಕೊಳ್ಳುವಾಗ, ನೀವು ಅಂತ್ಯವನ್ನು ತಲುಪುವವರೆಗೆ. ನಂತರ ನಾವು ಕಥೆಯ ಬಗ್ಗೆ ಕೆಲವು ಪ್ರಶ್ನೆಗಳನ್ನು ಕೇಳುತ್ತೇವೆ. ಪ್ರತಿ ಪುಟದಲ್ಲಿ, ಮುಂದಿನ ಪುಟಕ್ಕಾಗಿ ಸ್ಪೇಸ್ ಬಾರ್ ಒತ್ತಿರಿ. ನೀವು ಸಿದ್ಧರಾದಾಗ, ಸ್ಥಳವನ್ನು ಒತ್ತಿರಿ.",
    bn: "গল্পের সময়! আপনি একটি বই থেকে 111 পৃষ্ঠাগুলি পড়বেন। একবার আপনি শুরু করার পরে, আপনি শেষ না হওয়া পর্যন্ত সম্পূর্ণ বোধগম্যতা বজায় রেখে নিঃশব্দে সমস্ত পৃষ্ঠাগুলি যত তাড়াতাড়ি সম্ভব পড়ুন। তারপরে আমরা আপনাকে গল্পটি সম্পর্কে কয়েকটি প্রশ্ন জিজ্ঞাসা করব। প্রতিটি পৃষ্ঠায়, পরবর্তী পৃষ্ঠার জন্য স্পেস বার টিপুন। আপনি যখন প্রস্তুত হন, জায়গা টিপুন।",
    id: "Waktu cerita! Anda akan membaca 111 halaman dari sebuah buku. Setelah Anda mulai, silahkan membaca semua halaman secepat mungkin, sambil mempertahankan pemahaman penuh, sampai Anda mencapai akhir. Lalu kami akan menanyakan beberapa pertanyaan tentang cerita tersebut. Pada setiap halaman, tekan bilah ruang untuk halaman berikutnya. Saat Anda siap, tekan ruang.",
    "zh-CN":
      "讲故事的时间！您将从书中阅读111页。一旦开始，请尽可能快地静静地阅读所有页面，同时保持充分的理解，直到到达末尾。然后，我们会问您一些有关这个故事的问题。在每个页面上，按下下一页的空格栏。准备好后，请按空格。",
    "zh-HK":
      "講故事的時間！您將從書中閱讀111頁。一旦開始，請盡可能快地靜靜地閱讀所有頁面，同時保持充分的理解，直到到達末尾。然後，我們會問您一些有關這個故事的問題。在每個頁面上，按下下一頁的空格欄。準備好後，請按空格。",
    ja: "ストーリータイム！本から111ページを読みます。開始したら、最後に到達するまで、完全な理解を維持しながら、できるだけ早くすべてのページを静かに読んでください。次に、ストーリーについていくつか質問します。各ページで、次のページのスペースバーを押します。準備ができたら、スペースを押します。",
    ko: "이야기 시간! 책에서 111 페이지를 읽을 것입니다. 시작하면 끝까지 도달 할 때까지 모든 이해를 유지하면서 최대한 빨리 모든 페이지를 조용히 읽으십시오. 그런 다음 이야기에 대해 몇 가지 질문을합니다. 각 페이지에서 다음 페이지의 공간 표시 줄을 누릅니다. 준비가되면 공간을 누릅니다.",
  },
  T_readingTaskQuestionPrompt: {
    "en-US":
      "Which of the following words appeared in the passage that you just read?",
    de: "Welche der folgenden Wörter erschienen in der Passage, die Sie gerade gelesen haben?",
    fr: "Lequel des mots suivants est apparu dans le passage que vous venez de lire?",
    es: "¿Cuál de las siguientes palabras apareció en el pasaje que acabas de leer?",
    pt: "Qual das seguintes palavras apareceu na passagem que você acabou de ler?",
    it: "Quale delle seguenti parole è apparsa nel passaggio che hai appena letto?",
    ro: "Care dintre următoarele cuvinte au apărut în pasajul pe care tocmai l -ați citit?",
    pl: "Które z poniższych słów pojawiły się w fragmencie, który właśnie przeczytałeś?",
    ru: "Какое из следующих слов появилось в отрывке, который вы только что прочитали?",
    hy: "Հետեւյալ բառերից որ մեկն է հայտնվում այն ​​հատվածում, որը դուք պարզապես կարդացել եք:",
    fi: "Mikä seuraavista sanoista ilmestyi juuri lukemasi kohtaan?",
    ka: "ქვემოთ ჩამოთვლილი სიტყვებიდან რომელი გამოჩნდა იმ პასაჟში, რომელიც ახლახან წაიკითხეთ?",
    he: "אילו מהמילים הבאות הופיעו בקטע שרק קראת?",
    ar: "أي من الكلمات التالية ظهرت في المقطع الذي قرأته للتو؟",
    ur: "مندرجہ ذیل میں سے کون سا لفظ جس گزرنے میں آپ نے ابھی پڑھا ہے؟",
    hi: "निम्नलिखित में से कौन सा शब्द उस मार्ग में दिखाई दिया जिसे आप अभी पढ़ते हैं?",
    ta: "நீங்கள் இப்போது படித்த பத்தியில் பின்வரும் வார்த்தைகளில் எது தோன்றியது?",
    ml: "നിങ്ങൾ ഇപ്പോൾ വായിച്ച ഭാഗത്തിൽ ഏത് വാക്കുകളിൽ ഏതാണ്?",
    te: "మీరు ఇప్పుడే చదివిన ప్రకరణంలో ఈ క్రింది పదాలలో ఏది కనిపించింది?",
    kn: "ನೀವು ಈಗ ಓದಿದ ಅಂಗೀಕಾರದಲ್ಲಿ ಈ ಕೆಳಗಿನ ಯಾವ ಪದಗಳು ಕಾಣಿಸಿಕೊಂಡಿವೆ?",
    bn: "আপনি সবেমাত্র পড়েছেন এমন প্যাসেজে নিম্নলিখিত শব্দগুলির মধ্যে কোনটি উপস্থিত হয়েছিল?",
    id: "Manakah dari kata -kata berikut yang muncul di bagian yang baru saja Anda baca?",
    "zh-CN": "您刚刚阅读的段落中出现了以下哪个单词？",
    "zh-HK": "您剛剛閱讀的段落中出現了以下哪個單詞？",
    ja: "次の言葉のうち、あなたがちょうど読んだパッセージに登場したのはどれですか？",
    ko: "방금 읽은 구절에 다음 중 어떤 단어가 나타 났습니까?",
  },
  T_readingTheEnd: {
    "en-US": "THE END",
    de: "DAS ENDE",
    fr: "LA FIN",
    es: "EL FIN",
    pt: "O FIM",
    it: "FINE",
    ro: "SFÂRȘITUL",
    pl: "KONIEC",
    ru: "КОНЕЦ",
    hy: "ՎԵՐՋ",
    fi: "LOPPU",
    ka: "ᲓᲐᲡᲐᲡᲠᲣᲚᲘ",
    he: "הסוף",
    ar: "النهاية",
    ur: "ختم شد",
    hi: "समाप्त",
    ta: "முற்றும்",
    ml: "അവസാനം",
    te: "ముగింపు",
    kn: "ಅಂತ್ಯ",
    bn: "শেষ",
    id: "TAMAT",
    "zh-CN": "结束",
    "zh-HK": "結束",
    ja: "終わり",
    ko: "끝",
  },
  T_readyClickCrosshair: {
    "en-US": "Ready to identify a letter? Click the center of the cross hairs.",
    de: "Bereit? Während Sie direkt auf das Fadenkreuz schauen, klicken Sie bitte auf das Fadenkreuz.",
    fr: "Prêt? Cliquez sur le centre de la croix.",
    es: "¿Listo? Haz clic en el centro de la cruz.",
    pt: "Preparar? Clique no centro da cruz.",
    it: "Pronto? Guardando direttamente il mirino, fare clic sul mirino.",
    ro: "Gata? Faceți clic pe centrul crucii.",
    pl: "Gotowy? Patrząc bezpośrednio na krzyżyk, kliknij na krzyżyk. \n\n",
    ru: "Готовый? Глядя прямо на перекрестие, нажмите на него.",
    hy: "Պատրաստ? Նայելով ճիշտ խաչմերուկին, կտտացրեք այն:",
    fi: "Oletko valmis tunnistamaan kirjeen? Napsauta ristikästen keskustaa.",
    ka: "მზად ხართ წერილის იდენტიფიცირებისთვის? დააჭირეთ ჯვრის თმას ცენტრს.",
    he: "מוּכָן? תוך התבוננות ישירה על השיער, אנא לחץ על הכוונת.",
    ar: "مستعد؟ أثناء النظر مباشرةً إلى علامة التقاطع ، يرجى الضغط على علامة التقاطع.",
    ur: "تیار؟ براہ راست کراس ہیر کو دیکھ کر ، براہ کرم کراس ہیر پر کلک کریں۔",
    hi: "तैयार? सीधे क्रॉसहेयर को देखते हुए, कृपया क्रॉसहेयर पर क्लिक करें।",
    ta: "ஒரு கடிதத்தை அடையாளம் காண தயாரா? குறுக்கு முடிகளின் மையத்தைக் கிளிக் செய்க.",
    ml: "ഒരു കത്ത് തിരിച്ചറിയാൻ തയ്യാറാണോ? ക്രോസ് ഹെയർ സെന്ററിൽ ക്ലിക്കുചെയ്യുക.",
    te: "ఒక లేఖను గుర్తించడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో క్లిక్ చేయండి.",
    kn: "ಪತ್ರವನ್ನು ಗುರುತಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ.",
    bn: "একটি চিঠি সনাক্ত করতে প্রস্তুত? ক্রস চুলের কেন্দ্রে ক্লিক করুন।",
    id: "Siap mengidentifikasi surat? Klik tengah rambut silang.",
    "zh-CN": "准备好了吗？保持注视十字准线，并点击十字准线。",
    "zh-HK": "準備好了嗎？保持注視十字準線，並點擊十字準線。",
    ja: "準備はできましたか？十字線を直視しながら、十字線をクリックしてください。",
    ko: "편지를 식별 할 준비가 되셨습니까? 십자 머리카락의 중앙을 클릭하십시오.",
  },
  T_readyClickCrosshairRepeatedLetters: {
    "en-US":
      "Ready to identify the letters? Click the center of the cross hairs.",
    de: "Bereit, die Buchstaben zu identifizieren? Klicken Sie auf die Mitte der Kreuzhaare.",
    fr: "Prêt à identifier les lettres? Cliquez sur le centre des poils.",
    es: "Listo para identificar las letras? Haga clic en el centro de la cruz.",
    pt: "Pronto para identificar as letras? Clique no centro dos cabelos cruzados.",
    it: "Pronto a identificare le lettere? Fai clic sul centro dei capelli incrociati.",
    ro: "Sunteți gata să identificați scrisorile? Faceți clic pe centrul părului încrucișat.",
    pl: "Gotowy do zidentyfikowania liter? Kliknij środek włosów krzyżowych.",
    ru: "Готовы идентифицировать буквы? Нажмите на центр кросс -волосков.",
    hy: "Պատրաստ եք նույնականացնել տառերը: Կտտացրեք խաչի մազերի կենտրոնը:",
    fi: "Oletko valmis tunnistamaan kirjaimet? Napsauta ristikästen keskustaa.",
    ka: "მზად ხართ ასოების იდენტიფიცირებისთვის? დააჭირეთ ჯვრის თმას ცენტრს.",
    he: "מוכנים לזהות את האותיות? לחץ על מרכז שערות הצלב.",
    ar: "هل أنت مستعد لتحديد الحروف؟ انقر فوق وسط الشعر عبر.",
    ur: "خطوط کی شناخت کے لئے تیار ہیں؟ کراس بالوں کے مرکز پر کلک کریں۔",
    hi: "पत्रों की पहचान करने के लिए तैयार हैं? क्रॉस हेयर के केंद्र पर क्लिक करें।",
    ta: "கடிதங்களை அடையாளம் காண தயாரா? குறுக்கு முடிகளின் மையத்தைக் கிளிக் செய்க.",
    ml: "അക്ഷരങ്ങൾ തിരിച്ചറിയാൻ തയ്യാറാണോ? ക്രോസ് ഹെയർ സെന്ററിൽ ക്ലിക്കുചെയ്യുക.",
    te: "అక్షరాలను గుర్తించడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో క్లిక్ చేయండి.",
    kn: "ಅಕ್ಷರಗಳನ್ನು ಗುರುತಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ.",
    bn: "চিঠিগুলি সনাক্ত করতে প্রস্তুত? ক্রস চুলের কেন্দ্রে ক্লিক করুন।",
    id: "Siap mengidentifikasi surat -surat itu? Klik tengah rambut silang.",
    "zh-CN": "准备识别字母了吗？单击十字头发的中心。",
    "zh-HK": "準備識別字母了嗎？單擊十字頭髮的中心。",
    ja: "文字を識別する準備はできましたか？十字毛の中心をクリックします。",
    ko: "글자를 식별 할 준비가 되셨습니까? 십자 머리카락의 중앙을 클릭하십시오.",
  },
  T_readyClickCrosshairRsvpReading: {
    "en-US": "Ready to read some words? Click the center of the cross hairs.",
    de: "Bereit, ein paar Wörter zu lesen? Klicken Sie auf die Mitte der Kreuzhaare.",
    fr: "Prêt à lire quelques mots? Cliquez sur le centre des poils.",
    es: "¿Listo para leer algunas palabras? Haga clic en el centro de la cruz.",
    pt: "Pronto para ler algumas palavras? Clique no centro dos cabelos cruzados.",
    it: "Pronto a leggere alcune parole? Fai clic sul centro dei capelli incrociati.",
    ro: "Sunteți gata să citiți câteva cuvinte? Faceți clic pe centrul părului încrucișat.",
    pl: "Gotowy do przeczytania niektórych słów? Kliknij środek włosów krzyżowych.",
    ru: "Готовы прочитать несколько слов? Нажмите на центр кросс -волосков.",
    hy: "Պատրաստ եք կարդալ որոշ բառեր: Կտտացրեք խաչի մազերի կենտրոնը:",
    fi: "Oletko valmis lukemaan joitain sanoja? Napsauta ristikästen keskustaa.",
    ka: "მზად ხართ რამდენიმე სიტყვის წასაკითხად? დააჭირეთ ჯვრის თმას ცენტრს.",
    he: "מוכנים לקרוא כמה מילים? לחץ על מרכז שערות הצלב.",
    ar: "هل أنت مستعد لقراءة بعض الكلمات؟ انقر فوق وسط الشعر عبر.",
    ur: "کچھ الفاظ پڑھنے کے لئے تیار ہیں؟ کراس بالوں کے مرکز پر کلک کریں۔",
    hi: "कुछ शब्द पढ़ने के लिए तैयार हैं? क्रॉस हेयर के केंद्र पर क्लिक करें।",
    ta: "சில வார்த்தைகளைப் படிக்க தயாரா? குறுக்கு முடிகளின் மையத்தைக் கிளிக் செய்க.",
    ml: "ചില വാക്കുകൾ വായിക്കാൻ തയ്യാറാണോ? ക്രോസ് ഹെയർ സെന്ററിൽ ക്ലിക്കുചെയ്യുക.",
    te: "కొన్ని పదాలు చదవడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో క్లిక్ చేయండి.",
    kn: "ಕೆಲವು ಪದಗಳನ್ನು ಓದಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ.",
    bn: "কিছু শব্দ পড়তে প্রস্তুত? ক্রস চুলের কেন্দ্রে ক্লিক করুন।",
    id: "Siap membaca beberapa kata? Klik tengah rambut silang.",
    "zh-CN": "准备阅读一些单词了吗？单击十字头发的中心。",
    "zh-HK": "準備閱讀一些單詞了嗎？單擊十字頭髮的中心。",
    ja: "いくつかの言葉を読む準備はできましたか？十字毛の中心をクリックします。",
    ko: "단어를 읽을 준비가 되셨습니까? 십자 머리카락의 중앙을 클릭하십시오.",
  },
  T_readyPressSpace: {
    "en-US":
      "Ready to identify a letter? While looking directly at the center of the cross hairs, please press the SPACE bar.",
    de: "Bereit? Während Sie direkt auf das Fadenkreuz schauen, drücken Sie bitte die Leertaste.",
    fr: "Prêt? Pendant que vous regardez directement au centre de la croix, appuyez sur l'espace.",
    es: "¿Listo? Mientras miras directamente al centro de la cruz, presiona ESPACIO.",
    pt: "Preparar? Enquanto você olha diretamente para o centro da cruz, pressione o espaço.",
    it: "Pronto? Mentre guardi direttamente il mirino, premi la barra spaziatrice.",
    ro: "Gata? În timp ce te uiți direct la centrul crucii, apăsați spațiul.",
    pl: "Gotowy? Patrząc bezpośrednio na krzyżyk, naciśnij spację. ",
    ru: "Готовый? Глядя прямо на перекрестие, нажмите пробел.",
    hy: "Պատրաստ? Նայելով ճիշտ խաչմերուկին, կտտացրեք բացը:",
    fi: "Oletko valmis tunnistamaan kirjeen? Kun katsot suoraan ristikäsien keskellä, paina avaruuspalkki.",
    ka: "მზად ხართ წერილის იდენტიფიცირებისთვის? ჯვრის თმების ცენტრში პირდაპირ დათვალიერებისას, გთხოვთ, დააჭირეთ კოსმოსურ ზოლს.",
    he: "מוּכָן? תוך התבוננות ישירה על השיער, לחץ על מקש הרווח.",
    ar: "مستعد؟ أثناء النظر مباشرة إلى علامة التقاطع ، يرجى الضغط على زر المسافة.",
    ur: "تیار؟ براہ راست کراس ہیر کو دیکھ کر ، براہ کرم اسپیس بار دبائیں۔",
    hi: "तैयार? सीधे क्रॉसहेयर को देखते हुए, कृपया SPACE बार दबाएं।",
    ta: "ஒரு கடிதத்தை அடையாளம் காண தயாரா? குறுக்கு முடிகளின் மையத்தை நேரடியாகப் பார்க்கும்போது, ​​தயவுசெய்து விண்வெளி பட்டியை அழுத்தவும்.",
    ml: "ഒരു കത്ത് തിരിച്ചറിയാൻ തയ്യാറാണോ? ക്രോസ് ഹെയർ മധ്യഭാഗത്ത് നേരിട്ട് നോക്കുമ്പോൾ, ദയവായി സ്പേസ് ബാർ അമർത്തുക.",
    te: "ఒక లేఖను గుర్తించడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో నేరుగా చూస్తున్నప్పుడు, దయచేసి స్పేస్ బార్‌ను నొక్కండి.",
    kn: "ಪತ್ರವನ್ನು ಗುರುತಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ನೇರವಾಗಿ ನೋಡುವಾಗ, ದಯವಿಟ್ಟು ಸ್ಪೇಸ್ ಬಾರ್ ಒತ್ತಿರಿ.",
    bn: "একটি চিঠি সনাক্ত করতে প্রস্তুত? ক্রস চুলের কেন্দ্রে সরাসরি দেখার সময়, দয়া করে স্পেস বারটি টিপুন।",
    id: "Siap mengidentifikasi surat? Sambil melihat langsung di tengah rambut silang, silakan tekan bilah ruang angkasa.",
    "zh-CN": "准备好了吗？保持注视十字准线，并按下空格键。",
    "zh-HK": "準備好了嗎？保持注視十字準線，並按下空格鍵。",
    ja: "準備はできましたか？十字線を直視しながら、スペースキーを押してください。",
    ko: "편지를 식별 할 준비가 되셨습니까? 크로스 털의 중앙을 직접 보면서 우주 막대를 누릅니다.",
  },
  T_readyPressSpaceRepeatedLetters: {
    "en-US":
      "Ready to identify the letters? While looking directly at the center of the cross hairs, please press the SPACE bar.",
    de: "Bereit, die Buchstaben zu identifizieren? Während Sie direkt in die Mitte der Cross -Haare schauen, drücken Sie bitte die Raumleiste.",
    fr: "Prêt à identifier les lettres? Tout en regardant directement le centre des poils, veuillez appuyer sur la barre d'espace.",
    es: "Listo para identificar las letras? Mientras mira directamente el centro de la cruz, presione la barra espacial.",
    pt: "Pronto para identificar as letras? Enquanto olha diretamente para o centro dos cabelos cruzados, pressione a barra de espaço.",
    it: "Pronto a identificare le lettere? Mentre guardi direttamente al centro dei capelli incrociati, premi la barra spaziale.",
    ro: "Sunteți gata să identificați scrisorile? În timp ce privești direct în centrul firelor de păr, vă rugăm să apăsați bara spațială.",
    pl: "Gotowy do zidentyfikowania liter? Patrząc bezpośrednio na środek włosów krzyżowych, naciśnij pasek kosmiczny.",
    ru: "Готовы идентифицировать буквы? Глядя прямо на центр кросс -волос, пожалуйста, нажмите на космическую полосу.",
    hy: "Պատրաստ եք նույնականացնել տառերը: Խաչի մազերի կենտրոնում ուղղակիորեն նայելիս խնդրում ենք սեղմել տիեզերական սանդղակը:",
    fi: "Oletko valmis tunnistamaan kirjaimet? Kun katsot suoraan ristikäsien keskellä, paina avaruuspalkki.",
    ka: "მზად ხართ ასოების იდენტიფიცირებისთვის? ჯვრის თმების ცენტრში პირდაპირ დათვალიერებისას, გთხოვთ, დააჭირეთ კოსმოსურ ზოლს.",
    he: "מוכנים לזהות את האותיות? תוך כדי התבוננות ישירות במרכז שערות הצלב, אנא לחץ על מוט החלל.",
    ar: "هل أنت مستعد لتحديد الحروف؟ أثناء النظر مباشرة إلى وسط الشعر المتقاطع ، يرجى الضغط على شريط الفضاء.",
    ur: "خطوط کی شناخت کے لئے تیار ہیں؟ کراس بالوں کے مرکز کو براہ راست دیکھتے ہوئے ، براہ کرم اسپیس بار دبائیں۔",
    hi: "पत्रों की पहचान करने के लिए तैयार हैं? क्रॉस हेयर के केंद्र में सीधे देखते हुए, कृपया स्पेस बार दबाएं।",
    ta: "கடிதங்களை அடையாளம் காண தயாரா? குறுக்கு முடிகளின் மையத்தை நேரடியாகப் பார்க்கும்போது, ​​தயவுசெய்து விண்வெளி பட்டியை அழுத்தவும்.",
    ml: "അക്ഷരങ്ങൾ തിരിച്ചറിയാൻ തയ്യാറാണോ? ക്രോസ് ഹെയർ മധ്യഭാഗത്ത് നേരിട്ട് നോക്കുമ്പോൾ, ദയവായി സ്പേസ് ബാർ അമർത്തുക.",
    te: "అక్షరాలను గుర్తించడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో నేరుగా చూస్తున్నప్పుడు, దయచేసి స్పేస్ బార్‌ను నొక్కండి.",
    kn: "ಅಕ್ಷರಗಳನ್ನು ಗುರುತಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ನೇರವಾಗಿ ನೋಡುವಾಗ, ದಯವಿಟ್ಟು ಸ್ಪೇಸ್ ಬಾರ್ ಒತ್ತಿರಿ.",
    bn: "চিঠিগুলি সনাক্ত করতে প্রস্তুত? ক্রস চুলের কেন্দ্রে সরাসরি দেখার সময়, দয়া করে স্পেস বারটি টিপুন।",
    id: "Siap mengidentifikasi surat -surat itu? Sambil melihat langsung di tengah rambut silang, silakan tekan bilah ruang angkasa.",
    "zh-CN": "准备识别字母了吗？直接看着十字头发的中心时，请按太空杆。",
    "zh-HK": "準備識別字母了嗎？直接看著十字頭髮的中心時，請按太空桿。",
    ja: "文字を識別する準備はできましたか？交差毛の中心を直接見ながら、スペースバーを押してください。",
    ko: "글자를 식별 할 준비가 되셨습니까? 크로스 털의 중앙을 직접 보면서 우주 막대를 누릅니다.",
  },
  T_readyPressSpaceRsvpReading: {
    "en-US":
      "Ready to read some words? While looking directly at the center of the cross hairs, please press the SPACE bar.",
    de: "Bereit, ein paar Wörter zu lesen? Während Sie direkt in die Mitte der Cross -Haare schauen, drücken Sie bitte die Raumleiste.",
    fr: "Prêt à lire quelques mots? Tout en regardant directement le centre des poils, veuillez appuyer sur la barre d'espace.",
    es: "¿Listo para leer algunas palabras? Mientras mira directamente el centro de la cruz, presione la barra espacial.",
    pt: "Pronto para ler algumas palavras? Enquanto olha diretamente para o centro dos cabelos cruzados, pressione a barra de espaço.",
    it: "Pronto a leggere alcune parole? Mentre guardi direttamente al centro dei capelli incrociati, premi la barra spaziale.",
    ro: "Sunteți gata să citiți câteva cuvinte? În timp ce privești direct în centrul firelor de păr, vă rugăm să apăsați bara spațială.",
    pl: "Gotowy do przeczytania niektórych słów? Patrząc bezpośrednio na środek włosów krzyżowych, naciśnij pasek kosmiczny.",
    ru: "Готовы прочитать несколько слов? Глядя прямо на центр кросс -волос, пожалуйста, нажмите на космическую полосу.",
    hy: "Պատրաստ եք կարդալ որոշ բառեր: Խաչի մազերի կենտրոնում ուղղակիորեն նայելիս խնդրում ենք սեղմել տիեզերական սանդղակը:",
    fi: "Oletko valmis lukemaan joitain sanoja? Kun katsot suoraan ristikäsien keskellä, paina avaruuspalkki.",
    ka: "მზად ხართ რამდენიმე სიტყვის წასაკითხად? ჯვრის თმების ცენტრში პირდაპირ დათვალიერებისას, გთხოვთ, დააჭირეთ კოსმოსურ ზოლს.",
    he: "מוכנים לקרוא כמה מילים? תוך כדי התבוננות ישירות במרכז שערות הצלב, אנא לחץ על מוט החלל.",
    ar: "هل أنت مستعد لقراءة بعض الكلمات؟ أثناء النظر مباشرة إلى وسط الشعر المتقاطع ، يرجى الضغط على شريط الفضاء.",
    ur: "کچھ الفاظ پڑھنے کے لئے تیار ہیں؟ کراس بالوں کے مرکز کو براہ راست دیکھتے ہوئے ، براہ کرم اسپیس بار دبائیں۔",
    hi: "कुछ शब्द पढ़ने के लिए तैयार हैं? क्रॉस हेयर के केंद्र में सीधे देखते हुए, कृपया स्पेस बार दबाएं।",
    ta: "சில வார்த்தைகளைப் படிக்க தயாரா? குறுக்கு முடிகளின் மையத்தை நேரடியாகப் பார்க்கும்போது, ​​தயவுசெய்து விண்வெளி பட்டியை அழுத்தவும்.",
    ml: "ചില വാക്കുകൾ വായിക്കാൻ തയ്യാറാണോ? ക്രോസ് ഹെയർ മധ്യഭാഗത്ത് നേരിട്ട് നോക്കുമ്പോൾ, ദയവായി സ്പേസ് ബാർ അമർത്തുക.",
    te: "కొన్ని పదాలు చదవడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో నేరుగా చూస్తున్నప్పుడు, దయచేసి స్పేస్ బార్‌ను నొక్కండి.",
    kn: "ಕೆಲವು ಪದಗಳನ್ನು ಓದಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ನೇರವಾಗಿ ನೋಡುವಾಗ, ದಯವಿಟ್ಟು ಸ್ಪೇಸ್ ಬಾರ್ ಒತ್ತಿರಿ.",
    bn: "কিছু শব্দ পড়তে প্রস্তুত? ক্রস চুলের কেন্দ্রে সরাসরি দেখার সময়, দয়া করে স্পেস বারটি টিপুন।",
    id: "Siap membaca beberapa kata? Sambil melihat langsung di tengah rambut silang, silakan tekan bilah ruang angkasa.",
    "zh-CN": "准备阅读一些单词了吗？直接看着十字头发的中心时，请按太空杆。",
    "zh-HK": "準備閱讀一些單詞了嗎？直接看著十字頭髮的中心時，請按太空桿。",
    ja: "いくつかの言葉を読む準備はできましたか？交差毛の中心を直接見ながら、スペースバーを押してください。",
    ko: "단어를 읽을 준비가 되셨습니까? 크로스 털의 중앙을 직접 보면서 우주 막대를 누릅니다.",
  },
  T_readyPressSpaceOrClickCrosshair: {
    "en-US":
      "Ready to identify a letter? While looking directly at the center of the cross hairs, please click it, or press the SPACE bar.",
    de: "Bereit? Während Sie direkt auf das Fadenkreuz schauen, drücken Sie bitte die Leertaste oder klicken Sie auf das Fadenkreuz.",
    fr: "Prêt? Pendant que vous regardez directement le centre de la croix, cliquez sur le centre ou appuyez sur l'espace.",
    es: "¿Listo? Mientras miras directamente al centro de la cruz, haz clic en el centro o presiona ESPACIO.",
    pt: "Preparar? Enquanto você olha diretamente para o centro da cruz, clique no centro ou pressione espaço.",
    it: "Pronto? Guardando direttamente il mirino, fare clic sul mirino.",
    ro: "Gata? În timp ce te uiți direct la centrul crucii, faceți clic pe centru sau apăsați spațiul.",
    pl: "Gotowy? Patrząc bezpośrednio na krzyżyk, naciśnij spację albo kliknij na krzyżyk",
    ru: "Готовый? Глядя прямо на перекрестие, нажмите пробел или щелкните на перекрестии.",
    hy: "Պատրաստ? Նայելով ճիշտ խաչմերուկին, կտտացրեք բացը կամ կտտացրեք խաչմերուկին:",
    fi: "Oletko valmis tunnistamaan kirjeen? Kun tarkastellaan suoraan ristikäsien keskustaa, napsauta sitä tai paina välilyöntiä.",
    ka: "მზად ხართ წერილის იდენტიფიცირებისთვის? ჯვრის თმას ცენტრის პირდაპირ დათვალიერებისას, გთხოვთ, დააჭირეთ მას, ან დააჭირეთ Space Bar- ს.",
    he: "מוּכָן? תוך התבוננות ישירה על הכוונת, לחץ על מקש הרווח או לחץ על הכוונת.",
    ar: "مستعد؟ أثناء النظر مباشرة إلى علامة التقاطع ، يرجى الضغط على زر المسافة أو الضغط على علامة التقاطع.",
    ur: "تیار؟ براہ راست کراس ہیر کو دیکھ کر ، براہ کرم اسپیس بار دبائیں یا کراس ہیر پر کلک کریں۔",
    hi: "तैयार? सीधे क्रॉसहेयर को देखते हुए, कृपया SPACE बार दबाएं या क्रॉसहेयर पर क्लिक करें।",
    ta: "ஒரு கடிதத்தை அடையாளம் காண தயாரா? குறுக்கு முடிகளின் மையத்தை நேரடியாகப் பார்க்கும்போது, ​​தயவுசெய்து அதைக் கிளிக் செய்க அல்லது ஸ்பேஸ் பட்டியை அழுத்தவும்.",
    ml: "ഒരു കത്ത് തിരിച്ചറിയാൻ തയ്യാറാണോ? ക്രോസ് ഹെയറുകളുടെ മധ്യഭാഗത്ത് നേരിട്ട് നോക്കുമ്പോൾ, ദയവായി അതിൽ ക്ലിക്കുചെയ്യുക, അല്ലെങ്കിൽ സ്പേസ് ബാർ അമർത്തുക.",
    te: "ఒక లేఖను గుర్తించడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో నేరుగా చూస్తున్నప్పుడు, దయచేసి దాన్ని క్లిక్ చేయండి లేదా స్పేస్ బార్‌ను నొక్కండి.",
    kn: "ಪತ್ರವನ್ನು ಗುರುತಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ನೇರವಾಗಿ ನೋಡುವಾಗ, ದಯವಿಟ್ಟು ಅದನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ, ಅಥವಾ ಸ್ಪೇಸ್ ಬಾರ್ ಒತ್ತಿರಿ.",
    bn: "একটি চিঠি সনাক্ত করতে প্রস্তুত? ক্রস চুলের কেন্দ্রে সরাসরি দেখার সময়, দয়া করে এটি ক্লিক করুন, বা স্পেস বার টিপুন।",
    id: "Siap mengidentifikasi surat? Sambil melihat langsung di tengah rambut silang, silakan klik, atau tekan bilah ruang angkasa.",
    "zh-CN": "准备好了吗？保持注视十字准线，并按下空格键或者点击十字准线。",
    "zh-HK": "準備好了嗎？保持注視十字準線，並按下空格鍵或者點擊十字準線。",
    ja: "準備はできましたか？十字線を直視しながら、スペースキーを押すか、十字線をクリックしてください。",
    ko: "편지를 식별 할 준비가 되셨습니까? 십자 머리카락의 중앙을 직접 보면서 클릭하거나 우주 막대를 누릅니다.",
  },
  T_readyPressSpaceOrClickCrosshairRepeatedLetters: {
    "en-US":
      "Ready to identify the letters? While looking directly at the center of the cross hairs, please click it or press the SPACE bar.",
    de: "Bereit, die Buchstaben zu identifizieren? Wenn Sie direkt in die Mitte der Kreuzhaare schauen, klicken Sie bitte darauf oder drücken Sie die Space -Leiste.",
    fr: "Prêt à identifier les lettres? Tout en regardant directement le centre des poils, veuillez cliquer dessus ou appuyer sur la barre d'espace.",
    es: "Listo para identificar las letras? Mientras mira directamente el centro de la cruz, haga clic en él o presione la barra espacial.",
    pt: "Pronto para identificar as letras? Enquanto olha diretamente para o centro dos cabelos cruzados, clique nele ou pressione a barra de espaço.",
    it: "Pronto a identificare le lettere? Mentre guardi direttamente al centro dei peli incrociati, fai clic o premi la barra spaziale.",
    ro: "Sunteți gata să identificați scrisorile? În timp ce priviți direct în centrul firelor de păr încrucișat, vă rugăm să faceți clic pe acesta sau să apăsați bara spațială.",
    pl: "Gotowy do zidentyfikowania liter? Patrząc bezpośrednio na środek włosów krzyżowych, kliknij je lub naciśnij pasek przestrzeni.",
    ru: "Готовы идентифицировать буквы? Посмотрев прямо на центр кросс -волос, нажмите на него или нажмите на пробел.",
    hy: "Պատրաստ եք նույնականացնել տառերը: Խաչի մազերի կենտրոնում ուղղակիորեն նայելիս կտտացրեք այն կամ սեղմեք տիեզերական սանդղակը:",
    fi: "Oletko valmis tunnistamaan kirjaimet? Kun tarkastellaan suoraan ristikäsien keskustaa, napsauta sitä tai paina välilyöntiä.",
    ka: "მზად ხართ ასოების იდენტიფიცირებისთვის? ჯვრის თმების ცენტრში პირდაპირ დათვალიერებისას, დააჭირეთ მას ან დააჭირეთ კოსმოსურ ზოლს.",
    he: "מוכנים לזהות את האותיות? תוך כדי התבוננות ישירות במרכז שערות הצלב, אנא לחץ עליו או לחץ על סרגל החלל.",
    ar: "هل أنت مستعد لتحديد الحروف؟ أثناء النظر مباشرة إلى مركز الشعر المتقاطع ، يرجى النقر فوقه أو الضغط على شريط الفضاء.",
    ur: "خطوط کی شناخت کے لئے تیار ہیں؟ کراس بالوں کے مرکز کو براہ راست دیکھتے ہوئے ، براہ کرم اس پر کلک کریں یا اسپیس بار کو دبائیں۔",
    hi: "पत्रों की पहचान करने के लिए तैयार हैं? क्रॉस हेयर के केंद्र में सीधे देखते हुए, कृपया इसे क्लिक करें या स्पेस बार दबाएं।",
    ta: "கடிதங்களை அடையாளம் காண தயாரா? குறுக்கு முடிகளின் மையத்தை நேரடியாகப் பார்க்கும்போது, ​​தயவுசெய்து அதைக் கிளிக் செய்க அல்லது ஸ்பேஸ் பட்டியை அழுத்தவும்.",
    ml: "അക്ഷരങ്ങൾ തിരിച്ചറിയാൻ തയ്യാറാണോ? ക്രോസ് ഹെയർ മധ്യഭാഗത്ത് നേരിട്ട് നോക്കുമ്പോൾ, ദയവായി അതിൽ ക്ലിക്കുചെയ്യുക അല്ലെങ്കിൽ സ്പേസ് ബാർ അമർത്തുക.",
    te: "అక్షరాలను గుర్తించడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో నేరుగా చూస్తున్నప్పుడు, దయచేసి దాన్ని క్లిక్ చేయండి లేదా స్పేస్ బార్‌ను నొక్కండి.",
    kn: "ಅಕ್ಷರಗಳನ್ನು ಗುರುತಿಸಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ನೇರವಾಗಿ ನೋಡುವಾಗ, ದಯವಿಟ್ಟು ಅದನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ ಅಥವಾ ಸ್ಪೇಸ್ ಬಾರ್ ಒತ್ತಿರಿ.",
    bn: "চিঠিগুলি সনাক্ত করতে প্রস্তুত? ক্রস চুলের কেন্দ্রে সরাসরি দেখার সময়, দয়া করে এটি ক্লিক করুন বা স্পেস বার টিপুন।",
    id: "Siap mengidentifikasi surat -surat itu? Sambil melihat langsung di tengah rambut silang, silakan klik atau tekan bilah ruang angkasa.",
    "zh-CN": "准备识别字母了吗？直接看着十字头发的中心时，请单击它或按太空栏。",
    "zh-HK": "準備識別字母了嗎？直接看著十字頭髮的中心時，請單擊它或按太空欄。",
    ja: "文字を識別する準備はできましたか？交差毛の中心を直接見ながら、クリックするか、スペースバーを押してください。",
    ko: "글자를 식별 할 준비가 되셨습니까? 십자 머리카락의 중앙을 직접 보면서 클릭하거나 우주 막대를 누릅니다.",
  },
  T_readyPressSpaceOrClickCrosshairRsvpReading: {
    "en-US":
      "Ready to read some words? While looking directly at the center of the cross hairs, please press the SPACE bar.",
    de: "Bereit, ein paar Wörter zu lesen? Während Sie direkt in die Mitte der Cross -Haare schauen, drücken Sie bitte die Raumleiste.",
    fr: "Prêt à lire quelques mots? Tout en regardant directement le centre des poils, veuillez appuyer sur la barre d'espace.",
    es: "¿Listo para leer algunas palabras? Mientras mira directamente el centro de la cruz, presione la barra espacial.",
    pt: "Pronto para ler algumas palavras? Enquanto olha diretamente para o centro dos cabelos cruzados, pressione a barra de espaço.",
    it: "Pronto a leggere alcune parole? Mentre guardi direttamente al centro dei capelli incrociati, premi la barra spaziale.",
    ro: "Sunteți gata să citiți câteva cuvinte? În timp ce privești direct în centrul firelor de păr, vă rugăm să apăsați bara spațială.",
    pl: "Gotowy do przeczytania niektórych słów? Patrząc bezpośrednio na środek włosów krzyżowych, naciśnij pasek kosmiczny.",
    ru: "Готовы прочитать несколько слов? Глядя прямо на центр кросс -волос, пожалуйста, нажмите на космическую полосу.",
    hy: "Պատրաստ եք կարդալ որոշ բառեր: Խաչի մազերի կենտրոնում ուղղակիորեն նայելիս խնդրում ենք սեղմել տիեզերական սանդղակը:",
    fi: "Oletko valmis lukemaan joitain sanoja? Kun katsot suoraan ristikäsien keskellä, paina avaruuspalkki.",
    ka: "მზად ხართ რამდენიმე სიტყვის წასაკითხად? ჯვრის თმების ცენტრში პირდაპირ დათვალიერებისას, გთხოვთ, დააჭირეთ კოსმოსურ ზოლს.",
    he: "מוכנים לקרוא כמה מילים? תוך כדי התבוננות ישירות במרכז שערות הצלב, אנא לחץ על מוט החלל.",
    ar: "هل أنت مستعد لقراءة بعض الكلمات؟ أثناء النظر مباشرة إلى وسط الشعر المتقاطع ، يرجى الضغط على شريط الفضاء.",
    ur: "کچھ الفاظ پڑھنے کے لئے تیار ہیں؟ کراس بالوں کے مرکز کو براہ راست دیکھتے ہوئے ، براہ کرم اسپیس بار دبائیں۔",
    hi: "कुछ शब्द पढ़ने के लिए तैयार हैं? क्रॉस हेयर के केंद्र में सीधे देखते हुए, कृपया स्पेस बार दबाएं।",
    ta: "சில வார்த்தைகளைப் படிக்க தயாரா? குறுக்கு முடிகளின் மையத்தை நேரடியாகப் பார்க்கும்போது, ​​தயவுசெய்து விண்வெளி பட்டியை அழுத்தவும்.",
    ml: "ചില വാക്കുകൾ വായിക്കാൻ തയ്യാറാണോ? ക്രോസ് ഹെയർ മധ്യഭാഗത്ത് നേരിട്ട് നോക്കുമ്പോൾ, ദയവായി സ്പേസ് ബാർ അമർത്തുക.",
    te: "కొన్ని పదాలు చదవడానికి సిద్ధంగా ఉన్నారా? క్రాస్ హెయిర్స్ మధ్యలో నేరుగా చూస్తున్నప్పుడు, దయచేసి స్పేస్ బార్‌ను నొక్కండి.",
    kn: "ಕೆಲವು ಪದಗಳನ್ನು ಓದಲು ಸಿದ್ಧರಿದ್ದೀರಾ? ಅಡ್ಡ ಕೂದಲಿನ ಮಧ್ಯಭಾಗವನ್ನು ನೇರವಾಗಿ ನೋಡುವಾಗ, ದಯವಿಟ್ಟು ಸ್ಪೇಸ್ ಬಾರ್ ಒತ್ತಿರಿ.",
    bn: "কিছু শব্দ পড়তে প্রস্তুত? ক্রস চুলের কেন্দ্রে সরাসরি দেখার সময়, দয়া করে স্পেস বারটি টিপুন।",
    id: "Siap membaca beberapa kata? Sambil melihat langsung di tengah rambut silang, silakan tekan bilah ruang angkasa.",
    "zh-CN": "准备阅读一些单词了吗？直接看着十字头发的中心时，请按太空杆。",
    "zh-HK": "準備閱讀一些單詞了嗎？直接看著十字頭髮的中心時，請按太空桿。",
    ja: "いくつかの言葉を読む準備はできましたか？交差毛の中心を直接見ながら、スペースバーを押してください。",
    ko: "단어를 읽을 준비가 되셨습니까? 크로스 털의 중앙을 직접 보면서 우주 막대를 누릅니다.",
  },
  T_sentenceGuessingGame: {
    "en-US":
      "The intelligibility of the sentence will vary from easy down to hard or impossible. You can't get much more than two-thirds right, so relax. Think of it as a guessing game, and just get as many as you can. You'll hear a beep whenever you're right.",
    de: "Die Verständlichkeit des Satzes variiert von einfach bis schwer oder unmöglich. Sie können nicht viel mehr als zwei Drittel richtig bekommen, also entspannen Sie sich. Betrachten Sie es als ein Ratenspiel und holen Sie sich so viele wie möglich. Sie werden einen Piepton hören, wenn Sie Recht haben.",
    fr: "L'intelligibilité de la phrase variera de la dure à la dure ou impossible. Vous ne pouvez pas obtenir beaucoup plus des deux tiers à droite, alors détendez-vous. Considérez-le comme un jeu de supposition et obtenez-en autant que vous le pouvez. Vous entendrez un bip lorsque vous avez raison.",
    es: "La inteligibilidad de la oración variará de fácil hasta difícil o imposible. No puedes tener mucho más de dos tercios, así que relájate. Piense en ello como un juego de adivinanzas, y simplemente obtenga tantas como pueda. Escucharás un pitido siempre que tengas razón.",
    pt: "A inteligibilidade da frase variará de fácil para difícil ou impossível. Você não pode acertar muito mais do que dois terços, então relaxe. Pense nisso como um jogo de adivinhação e obtenha o máximo possível. Você ouvirá um sinal sonoro sempre que estiver certo.",
    it: "L'intelligibilità della frase varierà da facile fino a difficile o impossibile. Non puoi ottenere molto più di due terzi giusti, quindi rilassati. Pensalo come un gioco di ipotesi e ottieni il maggior numero possibile. Sentirai un segnale acustico ogni volta che hai ragione.",
    ro: "Intelibilitatea propoziției va varia de la ușor până la greu sau imposibil. Nu puteți obține mult mai mult de două treimi corect, așa că relaxați-vă. Gândiți -vă la el ca la un joc de ghicire și obțineți doar cât puteți. Veți auzi un bip ori de câte ori ai dreptate.",
    pl: "Zrozumiałość zdania będzie się różnić od łatwego do trudnego lub niemożliwego. Nie możesz uzyskać znacznie więcej niż dwie trzecie, więc zrelaksuj się. Pomyśl o tym jak o zgadywaniu i po prostu zdobądź tyle, ile możesz. Usłyszysz sygnał dźwiękowy, gdy masz rację.",
    ru: "Интеллектуальность предложения будет варьироваться от легкого до жесткого или невозможного. Вы не можете получить гораздо более двух третей правильно, так что расслабьтесь. Думайте об этом как о игре догадков, и просто получите как можно больше. Вы услышите звуковой сигнал, когда вы правы.",
    hy: "Պատժի խելամտությունը կտարբերվի հեշտից մինչեւ ծանր կամ անհնար: Դուք չեք կարող ստանալ շատ ավելին, քան երկու երրորդը ճիշտ, այնպես որ հանգստացեք: Մտածեք դրա մասին որպես գուշակության խաղ եւ պարզապես հնարավորինս շատ ստացեք: Դուք կօգտագործեք փղուկը, երբ ճիշտ եք:",
    fi: "Lauseen ymmärrettävyys vaihtelee helposti kovaa tai mahdotonta. Et voi saada paljon enemmän kuin kaksi kolmasosaa oikein, joten rentoudu. Ajattele sitä arvauspelinä ja hanki vain niin monta kuin pystyt. Kuulet piippauksen aina kun olet oikeassa.",
    ka: "სასჯელის გააზრება განსხვავდება მარტივად ქვემოთ ან შეუძლებლად. თქვენ ვერ მიიღებთ ორ მესამედზე მეტს, ასე რომ დაისვენოთ. იფიქრეთ მასზე, როგორც გამოცნობის თამაში და უბრალოდ მიიღეთ რაც შეიძლება მეტი. თქვენ მოისმენთ beep, როდესაც მართალი ხართ.",
    he: "הבנת המשפט תשתנה מקלות למטה לקשה או בלתי אפשרית. אתה לא יכול להשיג הרבה יותר משני שלישים נכון, אז תירגע. חשבו על זה כמשחק ניחוש, ופשוט קבלו כמה שיותר. תשמע צפצוף בכל פעם שאתה צודק.",
    ar: "سوف تختلف وضوح الجملة من السهل إلى الصلب أو المستحيل. لا يمكنك الحصول على أكثر من الثلثين بشكل صحيح ، لذا استرخ. فكر في الأمر على أنه لعبة تخمين ، واحصل على أكبر عدد ممكن. ستسمع صوتًا كلما كنت على حق.",
    ur: "جملے کی سمجھداری آسانی سے مشکل یا ناممکن تک مختلف ہوگی۔ آپ کو دو تہائی سے زیادہ ٹھیک نہیں مل سکتا ، لہذا آرام کریں۔ اس کو ایک اندازہ لگانے والا کھیل سمجھیں ، اور جتنا ہو سکے حاصل کریں۔ جب بھی آپ ٹھیک ہوں گے آپ کو بیپ سنیں گے۔",
    hi: "वाक्य की समझदारी आसान से कठिन या असंभव से भिन्न होगी। आप दो-तिहाई से अधिक सही नहीं पा सकते हैं, इसलिए आराम करें। इसे एक अनुमान लगाने के खेल के रूप में सोचें, और बस जितने भी हो सके उतने प्राप्त करें। जब भी आप सही हों तो आप एक बीप सुनेंगे।",
    ta: "வாக்கியத்தின் புத்திசாலித்தனம் எளிதானது முதல் கடினமான அல்லது சாத்தியமற்றது வரை மாறுபடும். மூன்றில் இரண்டு பங்கிற்கு மேல் நீங்கள் பெற முடியாது, எனவே ஓய்வெடுங்கள். யூகிக்கும் விளையாட்டாக இதை நினைத்துப் பாருங்கள், உங்களால் முடிந்தவரை பலவற்றைப் பெறுங்கள். நீங்கள் சரியாக இருக்கும் போதெல்லாம் ஒரு பீப்பைக் கேட்பீர்கள்.",
    ml: "വാക്യത്തിന്റെ ബുദ്ധിശക്തി എളുപ്പത്തിൽ കഠിനമോ അസാധ്യമോ ആയി മാറും. നിങ്ങൾക്ക് മൂന്നിൽ രണ്ട് വലതുവശത്ത് കൂടുതൽ നേടാൻ കഴിയില്ല, അതിനാൽ വിശ്രമിക്കുക. ഇത് ഒരു ess ഹിക്കുന്ന ഗെയിമായി കരുതുക, നിങ്ങൾക്ക് കഴിയുന്നത്രയും നേടുക. നിങ്ങൾ ശരിയാകുമ്പോഴെല്ലാം നിങ്ങൾ ഒരു ബീപ്പ് കേൾക്കും.",
    te: "వాక్యం యొక్క తెలివితేటలు సులభంగా నుండి కఠినమైన లేదా అసాధ్యం వరకు మారుతూ ఉంటాయి. మీరు మూడింట రెండు వంతుల కంటే ఎక్కువ పొందలేరు, కాబట్టి విశ్రాంతి తీసుకోండి. దీన్ని game హించే ఆటగా భావించండి మరియు మీకు వీలైనన్ని ఎక్కువ పొందండి. మీరు సరైనప్పుడల్లా బీప్ వింటారు.",
    kn: "ವಾಕ್ಯದ ಬುದ್ಧಿವಂತಿಕೆಯು ಸುಲಭದಿಂದ ಕಠಿಣ ಅಥವಾ ಅಸಾಧ್ಯಕ್ಕೆ ಬದಲಾಗುತ್ತದೆ. ನೀವು ಮೂರನೇ ಎರಡರಷ್ಟು ಭಾಗವನ್ನು ಪಡೆಯಲು ಸಾಧ್ಯವಿಲ್ಲ, ಆದ್ದರಿಂದ ವಿಶ್ರಾಂತಿ ಪಡೆಯಿರಿ. ಇದನ್ನು ess ಹಿಸುವ ಆಟವೆಂದು ಯೋಚಿಸಿ, ಮತ್ತು ನಿಮಗೆ ಸಾಧ್ಯವಾದಷ್ಟು ಪಡೆಯಿರಿ. ನೀವು ಸರಿಯಾಗಿದ್ದಾಗಲೆಲ್ಲಾ ನೀವು ಬೀಪ್ ಕೇಳುತ್ತೀರಿ.",
    bn: "বাক্যটির বোধগম্যতা সহজ থেকে শক্ত বা অসম্ভব হয়ে যায়। আপনি দুই-তৃতীয়াংশের চেয়ে বেশি কিছু পেতে পারেন না, তাই শিথিল করুন। এটিকে একটি অনুমানের খেলা হিসাবে ভাবেন এবং যতটা পারেন আপনি পান। আপনি যখনই ঠিক আছেন তখন আপনি একটি বীপ শুনতে পাবেন।",
    id: "Kejadian kalimat akan bervariasi dari mudah hingga keras atau tidak mungkin. Anda tidak bisa mendapatkan lebih dari dua pertiga, jadi santai. Anggap saja sebagai permainan menebak, dan dapatkan sebanyak yang Anda bisa. Anda akan mendengar bunyi bip kapan pun Anda benar.",
    "zh-CN":
      "该句子的可理解性将从轻松到难以置信或不可能。您的收入不能超过三分之二，所以放松。将其视为一个猜测游戏，只要获得尽可能多的游戏。每当您正确时，您都会听到哔哔声。",
    "zh-HK":
      "該句子的可理解性將從輕鬆到難以置信或不可能。您的收入不能超過三分之二，所以放鬆。將其視為一個猜測遊戲，只要獲得盡可能多的遊戲。每當您正確時，您都會聽到嗶嗶聲。",
    ja: "文の明瞭度は、簡単から難しいか不可能までさまざまです。 3分の2を超えることはできないので、リラックスしてください。それを推測ゲームと考えて、できるだけ多くのことを手に入れてください。あなたが正しいときはいつでもビープ音が聞こえます。",
    ko: "문장의 명료성은 쉬운 곳에서 단단하거나 불가능한 곳에 달라집니다. 당신은 3 분의 2 이상을 얻을 수 없으므로 긴장을 풀십시오. 그것을 추측 게임으로 생각하고 가능한 한 많은 것을 얻으십시오. 당신이 옳을 때마다 경고음이 들립니다.",
  },
  T_sentenceIdentifyClick: {
    "en-US":
      "All possible sentences are shown. Please click the one you heard, or your best guess.",
    de: "Alle möglichen Sätze werden angezeigt. Bitte klicken Sie auf den, den Sie gehört haben, oder Ihre beste Vermutung.",
    fr: "Toutes les phrases possibles sont affichées. Veuillez cliquer sur celui que vous avez entendu ou votre meilleure supposition.",
    es: "Se muestran todas las oraciones posibles. Haga clic en el que escuchó o su mejor suposición.",
    pt: "Todas as frases possíveis são mostradas. Clique no que você ouviu ou seu melhor palpite.",
    it: "Vengono mostrate tutte le frasi possibili. Fai clic su quello che hai sentito o la tua ipotesi migliore.",
    ro: "Toate propozițiile posibile sunt afișate. Vă rugăm să faceți clic pe cel pe care l -ați auzit sau pe cea mai bună presupunere a dvs.",
    pl: "Wszystkie możliwe zdania są pokazane. Kliknij ten, który usłyszałeś, lub najlepsze przypuszczenie.",
    ru: "Все возможные предложения показаны. Пожалуйста, нажмите на тот, который вы слышали, или ваше лучшее предположение.",
    hy: "Shown ուցադրվում են բոլոր հնարավոր նախադասությունները: Խնդրում ենք կտտացնել ձեր լսածին կամ ձեր լավագույն գուշակությունը:",
    fi: "Kaikki mahdolliset lauseet on esitetty. Napsauta kuulemasi tai paras arvaus.",
    ka: "ნაჩვენებია ყველა შესაძლო წინადადება. გთხოვთ, დააჭირეთ მას, რაც გსმენიათ, ან თქვენი საუკეთესო გამოცნობა.",
    he: "כל המשפטים האפשריים מוצגים. אנא לחץ על זה ששמעת, או על הניחוש הטוב ביותר שלך.",
    ar: "يتم عرض جميع الجمل الممكنة. يرجى النقر فوق الشخص الذي سمعته ، أو أفضل تخمين لك.",
    ur: "تمام ممکنہ جملے دکھائے گئے ہیں۔ براہ کرم آپ نے سنا ہوا ایک پر کلک کریں ، یا آپ کا بہترین اندازہ لگائیں۔",
    hi: "सभी संभावित वाक्य दिखाए गए हैं। कृपया जो आपने सुना, या अपना सबसे अच्छा अनुमान लगाएं।",
    ta: "சாத்தியமான அனைத்து வாக்கியங்களும் காட்டப்பட்டுள்ளன. தயவுசெய்து நீங்கள் கேட்ட ஒன்றைக் கிளிக் செய்க அல்லது உங்கள் சிறந்த யூகம்.",
    ml: "സാധ്യമായ എല്ലാ വാക്യങ്ങളും കാണിച്ചിരിക്കുന്നു. നിങ്ങൾ കേട്ട ഒന്ന് ക്ലിക്കുചെയ്യുക, അല്ലെങ്കിൽ നിങ്ങളുടെ മികച്ച ess ഹം ക്ലിക്കുചെയ്യുക.",
    te: "సాధ్యమయ్యే అన్ని వాక్యాలు చూపించబడ్డాయి. దయచేసి మీరు విన్నదాన్ని లేదా మీ ఉత్తమ అంచనాను క్లిక్ చేయండి.",
    kn: "ಸಾಧ್ಯವಿರುವ ಎಲ್ಲಾ ವಾಕ್ಯಗಳನ್ನು ತೋರಿಸಲಾಗಿದೆ. ದಯವಿಟ್ಟು ನೀವು ಕೇಳಿದ ಒಂದು ಅಥವಾ ನಿಮ್ಮ ಉತ್ತಮ .ಹೆಯನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ.",
    bn: "সমস্ত সম্ভাব্য বাক্য প্রদর্শিত হয়। আপনি যে শুনেছেন তা দয়া করে ক্লিক করুন বা আপনার সেরা অনুমান করুন।",
    id: "Semua kalimat yang mungkin ditampilkan. Silakan klik yang Anda dengar, atau tebakan terbaik Anda.",
    "zh-CN": "显示所有可能的句子。请点击您听到的那个或最好的猜测。",
    "zh-HK": "顯示所有可能的句子。請點擊您聽到的那個或最好的猜測。",
    ja: "考えられるすべての文が示されています。聞いたものをクリックしてください。",
    ko: "가능한 모든 문장이 표시됩니다. 들었던 것을 클릭하거나 최선의 추측을 클릭하십시오.",
  },
  T_sentenceProcedure: {
    "en-US":
      "On each trial, you'll hear a sentence, usually in noise. Then you'll see a list of all the possible sentences, and you'll click the one you heard, or your best guess. ",
    de: "Bei jedem Versuch hören Sie einen Satz, normalerweise in Lärm. Dann sehen Sie eine Liste aller möglichen Sätze und klicken auf den, den Sie gehört haben, oder Ihre beste Vermutung.",
    fr: "À chaque essai, vous entendrez une phrase, généralement dans le bruit. Ensuite, vous verrez une liste de toutes les phrases possibles, et vous cliquez sur celui que vous avez entendu, ou votre meilleure supposition.",
    es: "En cada prueba, escuchará una oración, generalmente en ruido. Luego verá una lista de todas las oraciones posibles, y hará clic en el que escuchó o su mejor suposición.",
    pt: "Em cada teste, você ouvirá uma frase, geralmente em ruído. Em seguida, você verá uma lista de todas as frases possíveis e clicará na que ouviu ou seu melhor palpite.",
    it: "Ad ogni prova, sentirai una frase, di solito nel rumore. Quindi vedrai un elenco di tutte le frasi possibili e fai clic su quello che hai sentito o la tua ipotesi migliore.",
    ro: "La fiecare proces, veți auzi o propoziție, de obicei în zgomot. Apoi veți vedea o listă cu toate propozițiile posibile și veți face clic pe cea pe care ați auzit -o sau cea mai bună presupunere a dvs.",
    pl: "Podczas każdej próby usłyszysz zdanie, zwykle w hałasie. Następnie zobaczysz listę wszystkich możliwych zdań i klikniesz tę, którą usłyszałeś lub swoje najlepsze przypuszczenie.",
    ru: "На каждом испытании вы услышите предложение, обычно в шуме. Затем вы увидите список всех возможных предложений, и вы нажмете на то, что вы слышали, или ваше лучшее предположение.",
    hy: "Յուրաքանչյուր փորձության ընթացքում դուք կօգտագործեք նախադասություն, սովորաբար աղմուկով: Այնուհետեւ կտեսնեք բոլոր հնարավոր նախադասությունների ցանկը, եւ կտտացրեք ձեր լսածին կամ ձեր լավագույն գուշակությունը:",
    fi: "Jokaisessa kokeessa kuulet lauseen, yleensä melusta. Sitten näet luettelon kaikista mahdollisista lauseista, ja napsautat kuulemasi tai parhaan arvauksenne.",
    ka: "თითოეულ სასამართლო პროცესზე, თქვენ მოისმენთ წინადადებას, ჩვეულებრივ, ხმაურით. შემდეგ ნახავთ ყველა შესაძლო წინადადების ჩამონათვალს და დააჭირეთ ღილაკს, რომელიც მოისმინეთ, ან თქვენი საუკეთესო გამოცნობა.",
    he: "בכל משפט תשמעו משפט, בדרך כלל ברעש. אז תראה רשימה של כל המשפטים האפשריים, ותלחץ על זה ששמעת, או על הניחוש הטוב ביותר שלך.",
    ar: "في كل تجربة ، ستسمع جملة ، عادة في ضوضاء. بعد ذلك ، سترى قائمة بجميع الجمل الممكنة ، وسوف تنقر على الجمل التي سمعتها ، أو أفضل تخمين لك.",
    ur: "ہر آزمائش پر ، آپ کو عام طور پر شور میں ، ایک جملہ سنیں گے۔ تب آپ کو تمام ممکنہ جملوں کی ایک فہرست نظر آئے گی ، اور آپ جو کچھ سنا ہے ، یا آپ کے بہترین اندازہ پر کلک کریں گے۔",
    hi: "प्रत्येक परीक्षण पर, आप एक वाक्य सुनेंगे, आमतौर पर शोर में। फिर आपको सभी संभावित वाक्यों की एक सूची दिखाई देगी, और आप जो सुना, या अपना सबसे अच्छा अनुमान लगाएंगे।",
    ta: "ஒவ்வொரு சோதனையிலும், வழக்கமாக சத்தத்தில் ஒரு வாக்கியத்தைக் கேட்பீர்கள். சாத்தியமான அனைத்து வாக்கியங்களின் பட்டியலையும் நீங்கள் காண்பீர்கள், மேலும் நீங்கள் கேட்டதை அல்லது உங்கள் சிறந்த யூகத்தை கிளிக் செய்வீர்கள்.",
    ml: "ഓരോ വിചാരണയിലും, സാധാരണയായി ശബ്ദത്തിൽ നിങ്ങൾ ഒരു വാചകം കേൾക്കും. അപ്പോൾ നിങ്ങൾ സാധ്യമായ എല്ലാ വാക്യങ്ങളുടെയും ഒരു ലിസ്റ്റ് കാണും, നിങ്ങൾ കേട്ടതോ അല്ലെങ്കിൽ നിങ്ങളുടെ ഏറ്റവും മികച്ച ess ഹക്കമോ നിങ്ങൾ ക്ലിക്കുചെയ്യും.",
    te: "ప్రతి విచారణలో, మీరు సాధారణంగా శబ్దంలో ఒక వాక్యాన్ని వింటారు. అప్పుడు మీరు సాధ్యమయ్యే అన్ని వాక్యాల జాబితాను చూస్తారు మరియు మీరు విన్నదాన్ని లేదా మీ ఉత్తమ అంచనాను క్లిక్ చేస్తారు.",
    kn: "ಪ್ರತಿ ಪ್ರಯೋಗದಲ್ಲೂ, ನೀವು ಸಾಮಾನ್ಯವಾಗಿ ಶಬ್ದದಲ್ಲಿ ಒಂದು ವಾಕ್ಯವನ್ನು ಕೇಳುತ್ತೀರಿ. ನಂತರ ನೀವು ಸಂಭವನೀಯ ಎಲ್ಲಾ ವಾಕ್ಯಗಳ ಪಟ್ಟಿಯನ್ನು ನೋಡುತ್ತೀರಿ, ಮತ್ತು ನೀವು ಕೇಳಿದ ಒಂದು ಅಥವಾ ನಿಮ್ಮ ಉತ್ತಮ .ಹೆಯನ್ನು ನೀವು ಕ್ಲಿಕ್ ಮಾಡುತ್ತೀರಿ.",
    bn: "প্রতিটি পরীক্ষায়, আপনি একটি বাক্য শুনতে পাবেন, সাধারণত শব্দে। তারপরে আপনি সমস্ত সম্ভাব্য বাক্যগুলির একটি তালিকা দেখতে পাবেন এবং আপনি যা শুনেছেন তা বা আপনার সেরা অনুমানটি ক্লিক করবেন।",
    id: "Pada setiap percobaan, Anda akan mendengar kalimat, biasanya dalam kebisingan. Kemudian Anda akan melihat daftar semua kalimat yang mungkin, dan Anda akan mengklik yang Anda dengar, atau tebakan terbaik Anda.",
    "zh-CN":
      "在每个审判中，通常会听到一句话，通常会在噪音中听到。然后，您将看到所有可能的句子的列表，然后单击所听到的句子或最佳猜测。",
    "zh-HK":
      "在每個審判中，通常會聽到一句話，通常會在噪音中聽到。然後，您將看到所有可能的句子的列表，然後單擊所聽到的句子或最佳猜測。",
    ja: "各トライアルでは、通常はノイズで文が聞こえます。次に、考えられるすべての文のリストが表示され、聞いた文をクリックします。",
    ko: "각 시험에서 일반적으로 소음으로 문장이 들립니다. 그런 다음 가능한 모든 문장 목록이 표시되며, 듣는 문장을 클릭하거나 최선의 추측을 클릭합니다.",
  },
  T_session: {
    "en-US": "",
    de: "",
    fr: "",
    es: "",
    pt: "",
    it: "",
    ro: "",
    pl: "",
    ru: "",
    hy: "",
    fi: "",
    ka: "",
    he: "",
    ar: "",
    ur: "",
    hi: "",
    ta: "",
    ml: "",
    te: "",
    kn: "",
    bn: "",
    id: "",
    "zh-CN": "",
    "zh-HK": "",
    ja: "",
    ko: "",
  },
  T_soundPhraseBlock: {
    "en-US":
      "On each trial you'll hear a sentence, and then you'll be asked to remember two of the words. The sound is distorted and may be hard to recognize, so you can't get them all right. Don't worry. Just treat it as a game and do your best. Good luck. Hit RETURN or click PROCEED to continue.\n\nThe PROCEED button will disappear immediately, and the experiment will begin once the sounds are loaded, which takes 40 seconds or longer, depending on the speed of the internet connection. We are working to shorten this delay in a future release.\n\nLoading sounds ...",
    de: "Bei jedem Versuch hören Sie einen Satz und dann werden Sie gebeten, sich an zwei der Wörter zu erinnern. Der Klang ist verzerrt und kann schwer zu erkennen sein, sodass Sie sie nicht in Ordnung bringen können. Mach dir keine Sorge. Behandle es einfach als Spiel und gib dein Bestes. Viel Glück. Klicken Sie auf Rückgabe oder klicken Sie auf fort, um fortzufahren.\n\nDie Taste zur Ausführung wird sofort verschwindet, und das Experiment beginnt, sobald die Geräusche geladen sind, was je nach Geschwindigkeit der Internetverbindung 40 Sekunden oder länger dauert. Wir arbeiten daran, diese Verzögerung in einer zukünftigen Veröffentlichung zu verkürzen.\n\nLadegeräusche ...",
    fr: "À chaque procès, vous entendrez une phrase, puis vous vous demanderez de vous souvenir de deux des mots. Le son est déformé et peut être difficile à reconnaître, vous ne pouvez donc pas les bien faire. Ne t'inquiète pas. Traitez-le simplement comme un jeu et faites de votre mieux. Bonne chance. Appuyez sur Retour ou cliquez sur Continuer pour continuer.\n\nLe bouton Procéder disparaîtra immédiatement et l'expérience commencera une fois les sons chargés, ce qui prend 40 secondes ou plus, selon la vitesse de la connexion Internet. Nous travaillons à raccourcir ce retard dans une version future.\n\nChargement des sons ...",
    es: "En cada prueba, escuchará una oración, y luego se le pedirá que recuerde dos de las palabras. El sonido está distorsionado y puede ser difícil de reconocer, por lo que no puede hacerlos todo bien. No te preocupes. Solo trátelo como un juego y haz tu mejor esfuerzo. Buena suerte. Presione la devolución o haga clic en continuar para continuar.\n\nEl botón proceder desaparecerá inmediatamente, y el experimento comenzará una vez que se carguen los sonidos, que tarda 40 segundos o más, dependiendo de la velocidad de la conexión a Internet. Estamos trabajando para acortar este retraso en un lanzamiento futuro.\n\nCargando sonidos ...",
    pt: "Em cada tentativa, você ouvirá uma frase e, em seguida, será solicitado a se lembrar de duas das palavras. O som é distorcido e pode ser difícil de reconhecer, para que você não possa acertá -los. Não se preocupe. Apenas trate -o como um jogo e faça o seu melhor. Boa sorte. Pressione o retorno ou clique em prosseguir para continuar.\n\nO botão Procena desaparecerá imediatamente e o experimento começará assim que os sons forem carregados, o que leva 40 segundos ou mais, dependendo da velocidade da conexão com a Internet. Estamos trabalhando para reduzir esse atraso em um lançamento futuro.\n\nSons de carregamento ...",
    it: "Ad ogni prova sentirai una frase e poi ti verrà chiesto di ricordare due delle parole. Il suono è distorto e potrebbe essere difficile da riconoscere, quindi non puoi farlo bene. Non preoccuparti. Trattalo come un gioco e fai del tuo meglio. Buona fortuna. Premi reso o fai clic su procedi per continuare.\n\nIl pulsante di procedimento scomparirà immediatamente e l'esperimento inizierà una volta caricati i suoni, il che richiede 40 secondi o più, a seconda della velocità della connessione Internet. Stiamo lavorando per abbreviare questo ritardo in una versione futura.\n\nCaricamento dei suoni ...",
    ro: "La fiecare proces veți auzi o propoziție și apoi vi se va cere să vă amintiți două dintre cuvinte. Sunetul este distorsionat și poate fi greu de recunoscut, astfel încât nu le puteți obține bine. Nu vă faceți griji. Tratează -l doar ca un joc și fă tot posibilul. Noroc. Apăsați Return sau faceți clic pe Continuați pentru a continua.\n\nButonul continuu va dispărea imediat, iar experimentul va începe odată ce sunetele vor fi încărcate, ceea ce durează 40 de secunde sau mai mult, în funcție de viteza conexiunii la internet. Lucrăm pentru a scurta această întârziere într -o versiune viitoare.\n\nÎncărcarea sunetelor ...",
    pl: "Podczas każdej próby usłyszysz zdanie, a następnie zostaniesz poproszony o zapamiętanie dwóch słów. Dźwięk jest zniekształcony i może być trudny do rozpoznania, więc nie możesz ich poprawić. Nie martw się. Po prostu traktuj to jako grę i daj z siebie wszystko. Powodzenia. Naciśnij zwrot lub kliknij, kontynuuj.\n\nPrzycisk Przejdź zniknie natychmiast, a eksperyment rozpocznie się po załadowaniu dźwięków, co zajmuje 40 sekund lub dłużej, w zależności od prędkości połączenia internetowego. Pracujemy nad skróceniem tego opóźnienia w przyszłym wydaniu.\n\nŁadowanie dźwięków ...",
    ru: "На каждом испытании вы услышите предложение, а затем вас попросят вспомнить два слова. Звук искажен и может быть трудно узнать, поэтому вы не можете получить их в порядке. Не волнуйся. Просто относитесь к этому как к игре и сделайте все возможное. Удачи. Нажмите возврат или нажмите, продолжайте, чтобы продолжить.\n\nКнопка продолжения сразу исчезнет, ​​и эксперимент начнется после загрузки звуков, что займет 40 секунд или дольше, в зависимости от скорости подключения к Интернету. Мы работаем, чтобы сократить эту задержку в будущем выпуске.\n\nЗвуки загрузки ...",
    hy: "Յուրաքանչյուր դատաքննության ընթացքում դուք կիմանաք մի նախադասություն, եւ այդ ժամանակ ձեզանից կառաջարկվի հիշել բառերից երկուսը: Ձայնը աղավաղվում է եւ կարող է դժվար լինել ճանաչել, այնպես որ դուք չեք կարող դրանք ճիշտ ստանալ: Մի անհանգստացեք: Պարզապես վերաբերվեք դրան որպես խաղ եւ արեք հնարավորը: Հաջողություն. Հիթ վերադարձը կամ կտտացրեք Շարունակել շարունակելու համար:\n\nՇարունակվող կոճակը անմիջապես կվերանա, եւ փորձը կսկսվի այն ժամանակ, երբ հնչյունները բեռնվեն, ինչը տեւում է 40 վայրկյան կամ ավելի երկար, կախված ինտերնետային կապի արագությունից: Մենք աշխատում ենք կարճացնել այս ձգձգումը հետագա թողարկումում:\n\nՁայնի բեռնումը ...",
    fi: "Jokaisessa oikeudenkäynnissä kuulet lauseen, ja sitten sinua pyydetään muistamaan kaksi sanaa. Ääni on vääristynyt ja se voi olla vaikea tunnistaa, joten et voi saada niitä kaikki hyvin. Älä huoli. Käsittele sitä vain pelinä ja tee parhaasi. Onnea. Paina palauta tai napsauta jatkaaksesi.\n\nJatka -painike katoaa heti, ja koe alkaa, kun äänet on ladattu, mikä vie 40 sekuntia tai pidempään Internet -yhteyden nopeudesta riippuen. Pyrimme lyhentämään tätä viivettä tulevassa julkaisussa.\n\nÄänien lataaminen ...",
    ka: "თითოეულ სასამართლო პროცესზე მოისმენთ წინადადებას, შემდეგ კი მოგეთხოვებათ ორი სიტყვის დამახსოვრება. ხმა დამახინჯებულია და შეიძლება ძნელად აღიაროთ, ასე რომ თქვენ ვერ მიიღებთ მათ. არ ინერვიულო. უბრალოდ მოექეცი მას როგორც თამაშს და გააკეთე ყველაფერი. Წარმატებები. დააჭირეთ დაბრუნებას ან დააჭირეთ გააგრძელეთ გასაგრძელებლად.\n\nმიმდინარე ღილაკი დაუყოვნებლივ გაქრება, ხოლო ექსპერიმენტი დაიწყება ბგერების დატვირთვის შემდეგ, რაც 40 წამს ან მეტხანს სჭირდება, რაც დამოკიდებულია ინტერნეტ კავშირის სიჩქარეზე. ჩვენ ვმუშაობთ ამ შეფერხების შემცირებისთვის მომავალში.\n\nიტვირთება ხმები ...",
    he: "בכל משפט תשמע משפט ואז תתבקש לזכור שתיים מהמילים. הצליל מעוות ועשוי להיות קשה לזהות, כך שלא תוכלו להשיג את זה בסדר. אל תדאג. פשוט התייחסו אליו כאל משחק ועשו כמיטב יכולתכם. בהצלחה. לחץ על החזרה או לחץ על המשך כדי להמשיך.\n\nכפתור ההמשך ייעלם מייד והניסוי יתחיל ברגע שהצלילים יועברו, שנמשך 40 שניות ומעלה, תלוי במהירות החיבור לאינטרנט. אנו פועלים לקצר את העיכוב הזה במהדורה עתידית.\n\nטוען צלילי ...",
    ar: "في كل تجربة ، ستسمع جملة ، وبعد ذلك يُطلب منك تذكر اثنين من الكلمتين. الصوت مشوه وقد يكون من الصعب التعرف عليه ، لذلك لا يمكنك الحصول عليها على ما يرام. لا تقلق. فقط تعامل معها على أنها لعبة وأبذل قصارى جهدك. حظ سعيد. اضغط على العودة أو انقر فوق متابعة للمتابعة.\n\nسيختفي زر المتابعة على الفور ، وستبدأ التجربة بمجرد تحميل الأصوات ، والتي تستغرق 40 ثانية أو أكثر ، اعتمادًا على سرعة اتصال الإنترنت. نحن نعمل على تقصير هذا التأخير في إصدار مستقبلي.\n\nتحميل الأصوات ...",
    ur: "ہر آزمائش پر آپ کو ایک جملہ سنیں گے ، اور پھر آپ سے دو الفاظ یاد رکھنے کو کہا جائے گا۔ آواز کو مسخ کردیا گیا ہے اور اسے پہچاننا مشکل ہوسکتا ہے ، لہذا آپ ان سب کو ٹھیک نہیں کرسکتے ہیں۔ فکر نہ کرو۔ بس اسے ایک کھیل کی طرح سلوک کریں اور اپنی پوری کوشش کریں۔ اچھی قسمت. ریٹرن کو دبائیں یا جاری رکھنے کے لئے آگے بڑھنے پر کلک کریں۔\n\nایڈوانس بٹن فوری طور پر ختم ہوجائے گا ، اور انٹرنیٹ کنکشن کی رفتار پر منحصر ہے کہ آواز بھری ہوئی ایک بار جب 40 سیکنڈ یا اس سے زیادہ وقت لگتا ہے تو تجربہ شروع ہوجائے گا۔ ہم آئندہ کی رہائی میں اس تاخیر کو مختصر کرنے کے لئے کام کر رہے ہیں۔\n\nلوڈنگ آوازیں ...",
    hi: "प्रत्येक परीक्षण पर आप एक वाक्य सुनेंगे, और फिर आपको दो शब्दों को याद रखने के लिए कहा जाएगा। ध्वनि विकृत है और पहचानना मुश्किल हो सकता है, इसलिए आप उन्हें ठीक नहीं कर सकते। चिंता मत करो। बस इसे एक खेल के रूप में मानें और अपना सर्वश्रेष्ठ करें। आपको कामयाबी मिले। रिटर्न हिट करें या जारी रखने के लिए आगे बढ़ें पर क्लिक करें।\n\nआगे बटन तुरंत गायब हो जाएगा, और ध्वनि लोड होने के बाद प्रयोग शुरू हो जाएगा, जो इंटरनेट कनेक्शन की गति के आधार पर 40 सेकंड या उससे अधिक समय लेता है। हम भविष्य की रिलीज में इस देरी को कम करने के लिए काम कर रहे हैं।\n\nलोड हो रहा है ...",
    ta: "ஒவ்வொரு சோதனையிலும் நீங்கள் ஒரு வாக்கியத்தைக் கேட்பீர்கள், பின்னர் இரண்டு சொற்களை நினைவில் கொள்ளும்படி கேட்கப்படுவீர்கள். ஒலி சிதைந்துவிட்டது மற்றும் அடையாளம் காண கடினமாக இருக்கலாம், எனவே அவற்றை நீங்கள் சரியாகப் பெற முடியாது. கவலைப்பட வேண்டாம். அதை ஒரு விளையாட்டாகக் கருதி, உங்கள் சிறந்ததைச் செய்யுங்கள். நல்ல அதிர்ஷ்டம். திரும்பவும் அல்லது தொடர தொடரவும் என்பதைக் கிளிக் செய்க.\n\nதொடர்ச்சியான பொத்தான் உடனடியாக மறைந்துவிடும், மேலும் ஒலிகள் ஏற்றப்பட்டவுடன் சோதனை தொடங்கும், இது இணைய இணைப்பின் வேகத்தைப் பொறுத்து 40 வினாடிகள் அல்லது அதற்கு மேற்பட்ட நேரம் ஆகும். எதிர்கால வெளியீட்டில் இந்த தாமதத்தை குறைக்க நாங்கள் பணியாற்றி வருகிறோம்.\n\nஒலிகளை ஏற்றுகிறது ...",
    ml: "ഓരോ ട്രിപ്പിലും നിങ്ങൾ ഒരു വാചകം കേൾക്കും, എന്നിട്ട് നിങ്ങൾ രണ്ടു വാക്കുകൾ ഓർക്കാൻ നിങ്ങളോട് ആവശ്യപ്പെടും. ശബ്ദം വികലമാണ്, തിരിച്ചറിയാൻ പ്രയാസമാണ്, അതിനാൽ നിങ്ങൾക്ക് അവ ശരിയാക്കാൻ കഴിയില്ല. വിഷമിക്കേണ്ട. ഇത് ഒരു ഗെയിമായി പരിഗണിച്ച് നിങ്ങളുടെ പരമാവധി ചെയ്യുക. നല്ലതുവരട്ടെ. മടങ്ങിവരവ് അമർത്തുക അല്ലെങ്കിൽ തുടരാൻ തുടരുക ക്ലിക്കുചെയ്യുക.\n\nആംവർ ബട്ടൺ ഉടനടി അപ്രത്യക്ഷമാകും, മാത്രമല്ല ഇത് ഇന്റർനെറ്റ് കണക്ഷന്റെ വേഗതയെ ആശ്രയിച്ച് 40 സെക്കൻഡോ അതിൽ കൂടുതലോ എടുക്കും. ഭാവിയിലെ ഒരു റിലീസിൽ ഈ കാലതാമസം ചുരുക്കുന്നതിന് ഞങ്ങൾ പ്രവർത്തിക്കുന്നു.\n\nലോഡുചെയ്യുന്നു ...",
    te: "ప్రతి ట్రయల్‌లో మీరు ఒక వాక్యాన్ని వింటారు, ఆపై మీరు రెండు పదాలను గుర్తుంచుకోవాలని అడుగుతారు. ధ్వని వక్రీకరించబడింది మరియు గుర్తించడం కష్టం, కాబట్టి మీరు అవన్నీ సరిగ్గా పొందలేరు. చింతించకండి. దీన్ని ఆటగా పరిగణించండి మరియు మీ వంతు కృషి చేయండి. అదృష్టం. రిటర్న్ నొక్కండి లేదా కొనసాగించడానికి కొనసాగండి క్లిక్ చేయండి.\n\nఆదాయ బటన్ వెంటనే అదృశ్యమవుతుంది మరియు శబ్దాలు లోడ్ అయిన తర్వాత ప్రయోగం ప్రారంభమవుతుంది, ఇది ఇంటర్నెట్ కనెక్షన్ వేగాన్ని బట్టి 40 సెకన్లు లేదా అంతకంటే ఎక్కువ సమయం పడుతుంది. భవిష్యత్ విడుదలలో ఈ ఆలస్యాన్ని తగ్గించడానికి మేము కృషి చేస్తున్నాము.\n\nశబ్దాలు లోడ్ అవుతున్నాయి ...",
    kn: "ಪ್ರತಿ ಪ್ರಯೋಗದಲ್ಲೂ ನೀವು ಒಂದು ವಾಕ್ಯವನ್ನು ಕೇಳುತ್ತೀರಿ, ಮತ್ತು ನಂತರ ಎರಡು ಪದಗಳನ್ನು ನೆನಪಿಟ್ಟುಕೊಳ್ಳಲು ನಿಮ್ಮನ್ನು ಕೇಳಲಾಗುತ್ತದೆ. ಶಬ್ದವು ವಿರೂಪಗೊಂಡಿದೆ ಮತ್ತು ಗುರುತಿಸಲು ಕಷ್ಟವಾಗಬಹುದು, ಆದ್ದರಿಂದ ನೀವು ಎಲ್ಲವನ್ನೂ ಸರಿಯಾಗಿ ಪಡೆಯಲು ಸಾಧ್ಯವಿಲ್ಲ. ಚಿಂತಿಸಬೇಡಿ. ಅದನ್ನು ಆಟವೆಂದು ಪರಿಗಣಿಸಿ ಮತ್ತು ನಿಮ್ಮ ಕೈಲಾದಷ್ಟು ಮಾಡಿ. ಒಳ್ಳೆಯದಾಗಲಿ. ರಿಟರ್ನ್ ಒತ್ತಿರಿ ಅಥವಾ ಮುಂದುವರಿಯಲು ಮುಂದುವರಿಯಿರಿ ಕ್ಲಿಕ್ ಮಾಡಿ.\n\nಮುಂದುವರಿಯುವ ಬಟನ್ ತಕ್ಷಣವೇ ಕಣ್ಮರೆಯಾಗುತ್ತದೆ, ಮತ್ತು ಶಬ್ದಗಳನ್ನು ಲೋಡ್ ಮಾಡಿದ ನಂತರ ಪ್ರಯೋಗವು ಪ್ರಾರಂಭವಾಗುತ್ತದೆ, ಇದು ಇಂಟರ್ನೆಟ್ ಸಂಪರ್ಕದ ವೇಗವನ್ನು ಅವಲಂಬಿಸಿ 40 ಸೆಕೆಂಡುಗಳು ಅಥವಾ ಅದಕ್ಕಿಂತ ಹೆಚ್ಚು ಸಮಯ ತೆಗೆದುಕೊಳ್ಳುತ್ತದೆ. ಭವಿಷ್ಯದ ಬಿಡುಗಡೆಯಲ್ಲಿ ಈ ವಿಳಂಬವನ್ನು ಕಡಿಮೆ ಮಾಡಲು ನಾವು ಕೆಲಸ ಮಾಡುತ್ತಿದ್ದೇವೆ.\n\nಶಬ್ದಗಳನ್ನು ಲೋಡ್ ಮಾಡಲಾಗುತ್ತಿದೆ ...",
    bn: "প্রতিটি পরীক্ষায় আপনি একটি বাক্য শুনতে পাবেন এবং তারপরে আপনাকে দুটি শব্দ মনে রাখতে বলা হবে। শব্দটি বিকৃত এবং এটি সনাক্ত করা শক্ত হতে পারে, তাই আপনি সেগুলি ঠিকঠাক করতে পারবেন না। চিন্তা করবেন না। এটি কেবল একটি গেম হিসাবে বিবেচনা করুন এবং আপনার সেরাটি করুন। শুভকামনা। হিট রিটার্ন বা চালিয়ে যেতে ক্লিক করুন।\n\nপ্রসেস বোতামটি অবিলম্বে অদৃশ্য হয়ে যাবে এবং শব্দগুলি লোড হয়ে গেলে পরীক্ষাটি শুরু হবে, যা ইন্টারনেট সংযোগের গতির উপর নির্ভর করে 40 সেকেন্ড বা তার বেশি সময় নেয়। আমরা ভবিষ্যতের প্রকাশে এই বিলম্বটি সংক্ষিপ্ত করার জন্য কাজ করছি।\n\nলোডিং শব্দ ...",
    id: "Pada setiap percobaan Anda akan mendengar kalimat, dan kemudian Anda akan diminta untuk mengingat dua kata. Suara terdistorsi dan mungkin sulit dikenali, jadi Anda tidak bisa mendapatkannya dengan benar. Jangan khawatir. Perlakukan saja sebagai permainan dan lakukan yang terbaik. Semoga beruntung. Tekan Pengembalian atau Klik Lanjutkan untuk melanjutkan.\n\nTombol Prosed akan segera hilang, dan percobaan akan dimulai setelah suara dimuat, yang membutuhkan waktu 40 detik atau lebih, tergantung pada kecepatan koneksi internet. Kami berupaya mempersingkat keterlambatan ini dalam rilis di masa mendatang.\n\nMemuat suara ...",
    "zh-CN":
      "在每个审判中，您都会听到一句话，然后您会被要求记住两个单词。声音被扭曲了，可能很难识别，因此您无法使它们正确。不用担心。只需将其视为游戏，并尽力而为。祝你好运。点击返回或单击继续继续进行。\n\n继续按钮将立即消失，一旦加载声音，实验将开始，这取决于Internet连接的速度，需要40秒或更长时间。我们正在努力在以后的版本中缩短这一延迟。\n\n加载声音...",
    "zh-HK":
      "在每個審判中，您都會聽到一句話，然後您會被要求記住兩個單詞。聲音被扭曲了，可能很難識別，因此您無法使它們正確。不用擔心。只需將其視為遊戲，並儘力而為。祝你好運。點擊返回或單擊繼續繼續進行。\n\n繼續按鈕將立即消失，一旦加載聲音，實驗將開始，這取決於Internet連接的速度，需要40秒或更長時間。我們正在努力在以後的版本中縮短這一延遲。\n\n加載聲音...",
    ja: "各トライアルで、文が聞こえると、2つの単語を覚えておくように求められます。音は歪んでおり、認識が難しいかもしれないので、それらをすべて正しくすることはできません。心配しないで。それをゲームとして扱い、最善を尽くしてください。幸運を。戻るかクリックして続行して続行します。\n\n進行ボタンはすぐに消え、インターネット接続の速度に応じて、音が読み込まれると実験が開始され、40秒以上かかります。将来のリリースでこの遅延を短縮するために取り組んでいます。\n\n音の読み込み...",
    ko: "각 시험에서 문장을 듣고 두 단어를 기억하도록 요청받을 것입니다. 소리가 왜곡되어 인식하기 어려울 수 있으므로 올바르게 얻을 수는 없습니다. 괜찮아요. 게임으로 취급하고 최선을 다하십시오. 행운을 빌어요. 반품을 누르거나 계속 진행하려면 진행하십시오.\n\n진행 버튼이 즉시 사라지고 사운드가로드되면 실험이 시작되며 인터넷 연결 속도에 따라 40 초 이상이 걸립니다. 우리는 향후 릴리스 에서이 지연을 단축하기 위해 노력하고 있습니다.\n\n소리로드 ...",
  },
  T_soundPhraseResponse: {
    "en-US":
      "You just heard this sentence. Please select words from the two menus to fill in the missing words.",
    de: "Sie haben gerade diesen Satz gehört. Bitte wählen Sie Wörter aus den beiden Menüs aus, um die fehlenden Wörter auszufüllen.",
    fr: "Vous venez d'entendre cette phrase. Veuillez sélectionner des mots dans les deux menus pour remplir les mots manquants.",
    es: "Acabas de escuchar esta oración. Seleccione palabras de los dos menús para completar las palabras faltantes.",
    pt: "Você acabou de ouvir essa frase. Selecione as palavras dos dois menus para preencher as palavras que faltam.",
    it: "Hai appena sentito questa frase. Seleziona le parole dai due menu per riempire le parole mancanti.",
    ro: "Tocmai ai auzit această propoziție. Vă rugăm să selectați cuvinte din cele două meniuri pentru a completa cuvintele care lipsesc.",
    pl: "Właśnie usłyszałeś to zdanie. Wybierz słowa z dwóch menu, aby wypełnić brakujące słowa.",
    ru: "Вы только что услышали это предложение. Пожалуйста, выберите слова из двух меню, чтобы заполнить пропущенные слова.",
    hy: "Դուք պարզապես լսել եք այս նախադասությունը: Խնդրում ենք երկու ընտրացանկից բառեր ընտրել բացակայող բառերը լրացնելու համար:",
    fi: "Kuulit juuri tämän lauseen. Valitse kahdesta valikosta sanat täyttämään puuttuvat sanat.",
    ka: "თქვენ უბრალოდ გსმენიათ ეს წინადადება. გთხოვთ, შეარჩიოთ სიტყვები ორი მენიუდან, რომ შეავსოთ დაკარგული სიტყვები.",
    he: "פשוט שמעת את המשפט הזה. אנא בחר מילים משני התפריטים כדי למלא את המילים החסרות.",
    ar: "لقد سمعت للتو هذه الجملة. يرجى تحديد كلمات من القوائم لملء الكلمات المفقودة.",
    ur: "آپ نے ابھی یہ جملہ سنا ہے۔ براہ کرم گمشدہ الفاظ کو پُر کرنے کے لئے دو مینو کے الفاظ منتخب کریں۔",
    hi: "आपने अभी यह वाक्य सुना है। कृपया लापता शब्दों को भरने के लिए दो मेनू से शब्दों का चयन करें।",
    ta: "இந்த வாக்கியத்தை நீங்கள் கேட்டீர்கள். காணாமல் போன சொற்களை நிரப்ப இரண்டு மெனுக்களிலிருந்து சொற்களைத் தேர்ந்தெடுக்கவும்.",
    ml: "നിങ്ങൾ ഈ ശിക്ഷ കേട്ടു. കാണാതായ വാക്കുകൾ പൂരിപ്പിക്കുന്നതിന് രണ്ട് മെനുകൾ മുതൽ ദയവായി വാക്കുകൾ തിരഞ്ഞെടുക്കുക.",
    te: "మీరు ఈ వాక్యాన్ని విన్నారు. తప్పిపోయిన పదాలను పూరించడానికి దయచేసి రెండు మెనూల నుండి పదాలను ఎంచుకోండి.",
    kn: "ನೀವು ಈ ವಾಕ್ಯವನ್ನು ಕೇಳಿದ್ದೀರಿ. ಕಾಣೆಯಾದ ಪದಗಳನ್ನು ತುಂಬಲು ದಯವಿಟ್ಟು ಎರಡು ಮೆನುಗಳ ಪದಗಳನ್ನು ಆಯ್ಕೆಮಾಡಿ.",
    bn: "আপনি এই বাক্যটি শুনেছেন। অনুপস্থিত শব্দগুলি পূরণ করতে দয়া করে দুটি মেনু থেকে শব্দ নির্বাচন করুন।",
    id: "Anda baru saja mendengar kalimat ini. Pilih kata -kata dari dua menu untuk mengisi kata -kata yang hilang.",
    "zh-CN": "你只是听到这句话。请从两个菜单中选择单词以填写缺失的单词。",
    "zh-HK": "你只是聽到這句話。請從兩個菜單中選擇單詞以填寫缺失的單詞。",
    ja: "あなたはちょうどこの文を聞いた。 2つのメニューから単語を選択して、不足している単語を記入してください。",
    ko: "방금이 문장을 들었습니다. 누락 된 단어를 채우려면 두 메뉴에서 단어를 선택하십시오.",
  },
  T_soundPhraseTrial: {
    "en-US": "Hit the SPACE bar to hear the next sentence",
    de: "Schlagen Sie die Space Bar, um den nächsten Satz zu hören",
    fr: "Frappez la barre spatiale pour entendre la phrase suivante",
    es: "Golpea la barra espacial para escuchar la próxima oración",
    pt: "Bata na barra de espaço para ouvir a próxima frase",
    it: "Colpisci la barra spaziale per ascoltare la frase successiva",
    ro: "Loveste bara spațială pentru a auzi următoarea propoziție",
    pl: "Uderz w pasek kosmiczny, aby usłyszeć następne zdanie",
    ru: "Ударьте космический планку, чтобы услышать следующее предложение",
    hy: "Հարվածեք տիեզերական սանդղակը `հաջորդ նախադասությունը լսելու համար",
    fi: "Osu avaruuspalkki kuullaksesi seuraavan lauseen",
    ka: "მოხვდით კოსმოსურ ზოლს შემდეგი წინადადების მოსასმენად",
    he: "פגע בסרגל החלל כדי לשמוע את המשפט הבא",
    ar: "اضغط على شريط الفضاء لسماع الجملة التالية",
    ur: "اگلے جملے کو سننے کے لئے خلائی بار کو مارو",
    hi: "अगला वाक्य सुनने के लिए स्पेस बार को मारो",
    ta: "அடுத்த வாக்கியத்தைக் கேட்க விண்வெளி பட்டியைத் தாக்கவும்",
    ml: "അടുത്ത വാചകം കേൾക്കാൻ ബഹിരാകാശ ബാർ അമർത്തുക",
    te: "తదుపరి వాక్యాన్ని వినడానికి స్పేస్ బార్‌ను నొక్కండి",
    kn: "ಮುಂದಿನ ವಾಕ್ಯವನ್ನು ಕೇಳಲು ಬಾಹ್ಯಾಕಾಶ ಪಟ್ಟಿಯನ್ನು ಹೊಡೆಯಿರಿ",
    bn: "পরবর্তী বাক্যটি শুনতে স্পেস বারে আঘাত করুন",
    id: "Tekan Space Bar untuk mendengar kalimat berikutnya",
    "zh-CN": "登上太空吧听下一句话",
    "zh-HK": "登上太空吧聽下一句話",
    ja: "次の文を聞くためにスペースバーを押してください",
    ko: "다음 문장을 들으려면 우주 막대를 치십시오",
  },
  T_takeABreakPopup: {
    "en-US": "Good work! Please take a moment to relax and blink.",
    de: "Gute Arbeit! Bitte nehmen Sie sich einen Moment Zeit, um sich zu entspannen und zu blinken.",
    fr: "Bon travail! Veuillez prendre un moment pour vous détendre et cligner des yeux.",
    es: "¡Buen trabajo! Tómese un momento para relajarse y parpadear.",
    pt: "Bom trabalho! Por favor, reserve um momento para relaxar e piscar.",
    it: "Buon lavoro! Per favore, prenditi un momento per rilassarti e sbattere le palpebre.",
    ro: "Buna treaba! Vă rugăm să luați un moment pentru a vă relaxa și a clipi.",
    pl: "Dobra robota! Poświęć chwilę na relaks i mrugnięcie.",
    ru: "Хорошая работа! Пожалуйста, найдите время, чтобы расслабиться и моргнуть.",
    hy: "Լավ գործ է! Խնդրում ենք մի պահ հանգստանալ եւ թարթել:",
    fi: "Hyvää työtä! Ota hetki rentoutua ja vilkkua.",
    ka: "Კარგი ნამუშევარია! გთხოვთ, ერთი წუთით დაისვენოთ და მოციმციმე.",
    he: "עבודה טובה! אנא קח רגע להירגע ולהבהב.",
    ar: "عمل جيد! من فضلك خذ لحظة للاسترخاء والغميض.",
    ur: "اچھا کام! براہ کرم آرام اور پلک جھپکنے میں ایک لمحہ لگائیں۔",
    hi: "अच्छा काम! कृपया आराम करने और पलक झपकने के लिए एक पल लें।",
    ta: "நல்ல வேலை! தயவுசெய்து ஓய்வெடுக்க சிறிது நேரம் ஒதுக்குங்கள்.",
    ml: "നല്ല ജോലി! വിശ്രമിക്കാനും മിന്നിത്തിളങ്ങാനും ദയവായി ഒരു നിമിഷം എടുക്കുക.",
    te: "మంచి పని! దయచేసి విశ్రాంతి తీసుకోవడానికి మరియు రెప్పపాటు చేయడానికి కొంత సమయం కేటాయించండి.",
    kn: "ಒಳ್ಳೆಯ ಕೆಲಸ! ದಯವಿಟ್ಟು ವಿಶ್ರಾಂತಿ ಮತ್ತು ಮಿಟುಕಿಸಲು ಸ್ವಲ್ಪ ಸಮಯ ತೆಗೆದುಕೊಳ್ಳಿ.",
    bn: "ভাল কাজ! শিথিল এবং ঝলকানোর জন্য দয়া করে কিছুক্ষণ সময় নিন।",
    id: "Kerja bagus! Mohon luangkan waktu sejenak untuk bersantai dan berkedip.",
    "zh-CN": "干得好！请短暂的休息一下，眨眨眼放轻松。",
    "zh-HK": "幹得好！請短暫的休息一下，眨眨眼放輕鬆。",
    ja: "よくできました！少し時間をかけてリラックスして点滅してください。",
    ko: "잘 했어! 잠시 휴식을 취하고 깜박이십시오.",
  },
  T_thresholdBeginBlock: {
    "en-US":
      "You are about to start a block of xxx trials. A triplet of three letters will appear in each trial, for example: Z R N. Please report only the middle letter,",
    de: "Sie sind dabei, einen Block von xxx-Testversionen zu beginnen. Jeder Versuch enthält drei Buchstaben, wie z. B.: Z R N. Bitte melden Sie nur den mittleren Buchstaben,",
    fr: "Vous êtes sur le point de commencer un bloc d'essais xxx. Chaque essai présentera trois lettres, comme ceci : Z R N. Veuillez ne signaler que la lettre du milieu,",
    es: "Estas a punto de comenzar un bloque de xxx ensayos. En cada ensayo, presentaremos tres letras, asi: Z R N. Por favor, reporta solo la letra del medio,",
    pt: "Você está prestes a iniciar um bloco de ensaios xxx. Em cada julgamento, apresentaremos três letras também: Z R N. Por favor, relate apenas a letra do meio,",
    it: "Stai per iniziare un blocco di xxx prove. Ogni prova presenterà tre lettere, in questo modo: Z R N. Devi riportare solo la lettera centrale,",
    ro: "Sunteți pe cale să începeți un bloc de încercări xxx. În fiecare proces, vom prezenta și trei scrisori: Z R N. Vă rugăm să raportați doar scrisoarea mediului,",
    pl: "Zamierzasz rozpocząć blok testów xxx. Każda próba zawiera trzy litery, takie jak: Z R N. Proszę podać tylko środkową literę",
    ru: "Вы собираетесь начать серию из ххх испытаний. В каждом испытании будут представлены три буквы, например: Z R N. Пожалуйста, укажите только среднюю букву,",
    hy: "Դուք պատրաստվում եք սկսել xxx թեստերի շարք: Յուրաքանչյուր թեստում կներկայացվեն երեք նամակ, օրինակ `z r n. Խնդրում ենք նշել միայն միջին տառը,",
    fi: "Olet aloittamassa xxx -kokeiden lohkon. Kolmen kirjaimen kolminkertainen näkyy jokaisessa kokeessa, esimerkiksi Z R N. Ilmoita vain keskikirjeestä,",
    ka: "თქვენ აპირებთ xxx ტესტების ბლოკის დაწყებას. სამი ასოების ტრიპლეტი გამოჩნდება თითოეულ საცდელში, მაგალითად: Z R N. გთხოვთ შეატყობინოთ მხოლოდ შუა წერილს,",
    he: "אתה עומד להתחיל בלוק של ניסויי xxx. כל משפט יציג שלוש אותיות, כך: Z R N. אנא דווחו רק על האות האמצעית,",
    ar: "أنت على وشك بدء مجموعة من تجارب xxx. ستقدم كل تجربة ثلاثة أحرف ، على النحو التالي: Z R N. يرجى الإبلاغ عن الحرف الأوسط فقط ،",
    ur: "آپ xxx ٹرائلز کا ایک بلاک شروع کرنے والے ہیں۔ ہر آزمائشی تین خطوط پیش کرے گا ، جیسے: Z R N. براہ کرم صرف درمیانی خط کی اطلاع دیں ،",
    hi: "आप xxx ट्राइअलों का एक ब्लॉक शुरू करने वाले हैं। प्रत्येक ट्राइअल तीन अक्षर प्रस्तुत करेगा, जैसे: Z R N. कृपया केवल मध्य अक्षर को रिपोर्ट करें,",
    ta: "நீங்கள் xxx சோதனைகளின் ஒரு தொகுதியைத் தொடங்க உள்ளீர்கள். ஒவ்வொரு சோதனையிலும் மூன்று எழுத்துக்களின் மும்மடங்கு தோன்றும், எடுத்துக்காட்டாக: Z R N. தயவுசெய்து நடுத்தர கடிதத்தை மட்டுமே புகாரளிக்கவும்,",
    ml: "നിങ്ങൾ xxx ട്രയലുകളുടെ ഒരു ബ്ലോക്ക് ആരംഭിക്കാൻ പോകുന്നു. ഓരോ ട്രിപ്പിലും മൂന്ന് അക്ഷരങ്ങളുടെ ഒരു മൂന്നിരട്ടി ദൃശ്യമാകും, ഉദാഹരണത്തിന്: z r N. ദയവായി മധ്യ കത്ത് മാത്രം റിപ്പോർട്ടുചെയ്യുക,",
    te: "మీరు xxx ట్రయల్స్ యొక్క బ్లాక్‌ను ప్రారంభించబోతున్నారు. ప్రతి ట్రయల్‌లో మూడు అక్షరాల త్రిపాది కనిపిస్తుంది, ఉదాహరణకు: Z R N. దయచేసి మధ్య లేఖను మాత్రమే నివేదించండి,",
    kn: "ನೀವು xxx ಪ್ರಯೋಗಗಳ ಬ್ಲಾಕ್ ಅನ್ನು ಪ್ರಾರಂಭಿಸಲಿದ್ದೀರಿ. ಪ್ರತಿ ಪ್ರಯೋಗದಲ್ಲಿ ಮೂರು ಅಕ್ಷರಗಳ ತ್ರಿವಳಿಗಳು ಕಾಣಿಸಿಕೊಳ್ಳುತ್ತವೆ, ಉದಾಹರಣೆಗೆ: Z R N. ದಯವಿಟ್ಟು ಮಧ್ಯದ ಅಕ್ಷರವನ್ನು ಮಾತ್ರ ವರದಿ ಮಾಡಿ,",
    bn: "আপনি xxx ট্রায়ালগুলির একটি ব্লক শুরু করতে চলেছেন। প্রতিটি পরীক্ষায় তিনটি চিঠির একটি ট্রিপলেট উপস্থিত হবে, উদাহরণস্বরূপ: জেড আর এন। দয়া করে কেবল মাঝের চিঠিটি রিপোর্ট করুন,",
    id: "Anda akan memulai blok uji coba xxx. Kembar tiga dari tiga huruf akan muncul di setiap percobaan, misalnya: Z R N. Harap hanya melaporkan huruf tengahnya,",
    "zh-CN":
      "您即将开始进行一个含有xxx次试验的区组。每次试验将会有三个字母出现，比如：Z R N。请只报告中间的字母。",
    "zh-HK":
      "您即將開始進行一個含有xxx次試驗的區組。每次試驗將會有三個字母出現，比如：Z R N。請只報告中間的字母。",
    ja: "あなたはxxxテストのブロックを始めようとしています。各テストではこのように3文字が表示されます：Z R N。真ん中の文字だけを報告してください。",
    ko: "xxx 시험 블록을 시작하려고합니다. 예를 들어 Z R N. 중간 편지 만보 고하십시오.",
  },
  T_thresholdRsvpReadingBeginBlock: {
    "en-US":
      "You are about to start a block of xxx trials. Each trial will quickly show you words, one after the other, and then ask you to report what you read",
    de: "Sie sind dabei, einen Block von xxx -Versuchen zu starten. Jeder Versuch zeigt Ihnen schnell Wörter, eine nacheinander, und die Bitten Sie, zu melden, was Sie lesen",
    fr: "Vous êtes sur le point de démarrer un bloc d'essais xxx. Chaque essai vous montrera rapidement des mots, l'un après l'autre, et vous demandez de signaler ce que vous lisez",
    es: "You are about to start a block of xxx trials. Each trial will quickly show you words, one after the other, and then ask you to report what you read",
    pt: "Você está prestes a iniciar um bloco de ensaios xxx. Cada julgamento mostrará rapidamente palavras, uma após a outra, e pede que você relate o que lê",
    it: "Stai per avviare un blocco di prove xxx. Ogni prova ti mostrerà rapidamente le parole, una dopo l'altra, e ti chiederà di segnalare ciò che leggi",
    ro: "Sunteți pe cale să începeți un bloc de încercări xxx. Fiecare proces vă va arăta rapid cuvinte, unul după altul și vă va cere să raportați ce ați citit",
    pl: "Niedługo rozpoczniesz blok prób xxx. Każda próba szybko pokaże słowa jeden po drugiej, a poproś o zgłoszenie, co przeczytałeś",
    ru: "Вы собираетесь запустить блок xxx испытаний. Каждое испытание быстро покажет вам слова, один за другим, и попросить вас сообщить, что вы читаете",
    hy: "Դուք պատրաստվում եք սկսել xxx փորձարկումների բլոկ: Յուրաքանչյուր փորձություն արագորեն ձեզ կներկայացնի բառեր, մեկը մյուսի հետեւից, եւ խնդրեք ձեզ զեկուցել ձեր կարդացածը",
    fi: "Olet aloittamassa xxx -kokeiden lohkon. Jokainen kokeilu näyttää nopeasti sanat, peräkkäin, ja pyydä sinua ilmoittamaan lukemasi",
    ka: "თქვენ აპირებთ xxx ტესტების ბლოკის დაწყებას. თითოეული სასამართლო პროცესი სწრაფად გაჩვენებთ სიტყვებს, ერთმანეთის მიყოლებით, და მოგთხოვთ შეატყობინოთ რა წაიკითხეთ",
    he: "אתה עומד להתחיל בלוק של ניסויים ב- xxx. כל משפט יראה לך במהירות מילים, בזה אחר זה, ובקש אותך לדווח על מה שאתה קורא",
    ar: "أنت على وشك بدء كتلة من تجارب xxx. ستظهر لك كل تجربة بسرعة كلمات ، واحدة تلو الأخرى ، ويطلب منك الإبلاغ عن ما تقرأه",
    ur: "آپ xxx ٹرائلز کا ایک بلاک شروع کرنے والے ہیں۔ ہر مقدمے کی سماعت جلدی سے آپ کو الفاظ دکھائے گی ، ایک کے بعد ایک ، اور آپ سے جو کچھ پڑھتا ہے اس کی اطلاع دینے کے لئے کہے",
    hi: "आप xxx परीक्षणों का एक ब्लॉक शुरू करने वाले हैं। प्रत्येक परीक्षण आपको जल्दी से शब्द दिखाएगा, एक के बाद एक, और आप जो पढ़ते हैं उसे रिपोर्ट करने के लिए कहेंगे",
    ta: "நீங்கள் xxx சோதனைகளின் ஒரு தொகுதியைத் தொடங்க உள்ளீர்கள். ஒவ்வொரு சோதனையும் விரைவாக உங்களுக்கு வார்த்தைகளைக் காண்பிக்கும், ஒன்றன் பின் ஒன்றாக, நீங்கள் படித்ததைப் புகாரளிக்கும்படி கேட்கவும்",
    ml: "നിങ്ങൾ xxx ട്രയലുകളുടെ ഒരു ബ്ലോക്ക് ആരംഭിക്കാൻ പോകുന്നു. ഓരോ ട്രയലും നിങ്ങൾക്ക് വാക്കുകൾ വേഗത്തിൽ കാണിക്കും, മറ്റൊന്ന്, നിങ്ങൾ വായിച്ച കാര്യങ്ങൾ റിപ്പോർട്ടുചെയ്യാൻ നിങ്ങളോട് ആവശ്യപ്പെടും",
    te: "మీరు xxx ట్రయల్స్ యొక్క బ్లాక్‌ను ప్రారంభించబోతున్నారు. ప్రతి ట్రయల్ మీకు త్వరగా పదాలను చూపిస్తుంది, ఒకదాని తరువాత ఒకటి, మరియు మీరు చదివిన వాటిని నివేదించమని అడుగుతుంది",
    kn: "ನೀವು xxx ಪ್ರಯೋಗಗಳ ಬ್ಲಾಕ್ ಅನ್ನು ಪ್ರಾರಂಭಿಸಲಿದ್ದೀರಿ. ಪ್ರತಿ ಪ್ರಯೋಗವು ಒಂದರ ನಂತರ ಒಂದರಂತೆ ಪದಗಳನ್ನು ತೋರಿಸುತ್ತದೆ, ಮತ್ತು ನೀವು ಓದಿದ್ದನ್ನು ವರದಿ ಮಾಡಲು ಕೇಳಿಕೊಳ್ಳಿ",
    bn: "আপনি xxx ট্রায়ালগুলির একটি ব্লক শুরু করতে চলেছেন। প্রতিটি পরীক্ষায় দ্রুত আপনাকে শব্দগুলি প্রদর্শন করা হবে, একের পর এক এবং আপনি কী পড়েন তা আপনাকে জানাতে জিজ্ঞাসা করুন",
    id: "Anda akan memulai blok uji coba xxx. Setiap percobaan akan dengan cepat menunjukkan kepada Anda kata -kata, satu demi satu, dan meminta Anda untuk melaporkan apa yang Anda baca",
    "zh-CN":
      "您即将开始xxx试验。每个审判都会迅速向您展示一句话，然后要求您报告您阅读的内容",
    "zh-HK":
      "您即將開始xxx試驗。每個審判都會迅速向您展示一句話，然後要求您報告您閱讀的內容",
    ja: "xxxトライアルのブロックを開始しようとしています。各トライアルは次々に単語をすぐに表示し、あなたが読んだものを報告するように頼みます",
    ko: "xxx 시험 블록을 시작하려고합니다. 각 시험은 당신에게 다른 말을하면 신속하게 말하고, 당신이 읽은 내용을보고하도록 요청합니다.",
  },
  T_thresholdRepeatedLettersBeginBlock: {
    "en-US":
      "You are about to start a block of xxx trials. A field of letters will appear in each trial. Report the two middle characters that you see",
    de: "Sie sind dabei, einen Block von xxx -Versuchen zu starten. In jedem Versuch erscheint ein Buchstabengebiet. Melden Sie die beiden mittleren Charaktere, die Sie sehen",
    fr: "Vous êtes sur le point de démarrer un bloc d'essais xxx. Un champ de lettres apparaîtra dans chaque essai. Signaler les deux personnages du milieu que vous voyez",
    es: "You are about to start a block of xxx trials. A field of letters will appear in each trial. Report the two middle characters that you see",
    pt: "Você está prestes a iniciar um bloco de ensaios xxx. Um campo de letras aparecerá em cada tentativa. Relate os dois personagens do meio que você vê",
    it: "Stai per avviare un blocco di prove xxx. Un campo di lettere apparirà in ogni prova. Segnala i due personaggi medi che vedi",
    ro: "Sunteți pe cale să începeți un bloc de încercări xxx. În fiecare proces va apărea un câmp de scrisori. Raportați cele două personaje de mijloc pe care le vedeți",
    pl: "Niedługo rozpoczniesz blok prób xxx. W każdej próbie pojawi się pole liter. Zgłoś dwie środkowe postacie, które widzisz",
    ru: "Вы собираетесь запустить блок xxx испытаний. Поле писем появится в каждом испытании. Сообщите о двух средних персонажах, которые вы видите",
    hy: "Դուք պատրաստվում եք սկսել xxx փորձարկումների բլոկ: Յուրաքանչյուր փորձության մեջ կհայտնվի տառերի մի դաշտ: Հաղորդեք ձեր տեսած երկու միջին նիշերի մասին",
    fi: "Olet aloittamassa xxx -kokeiden lohkon. Jokaisessa kokeessa ilmestyy kirjainkenttä. Ilmoita kaksi keskimmäistä merkkiä, jotka näet",
    ka: "თქვენ აპირებთ xxx ტესტების ბლოკის დაწყებას. წერილების ველი გამოჩნდება თითოეულ სასამართლო პროცესზე. შეატყობინეთ ორი საშუალო პერსონაჟის, რომელსაც ხედავთ",
    he: "אתה עומד להתחיל בלוק של ניסויים ב- xxx. שדה מכתבים יופיע בכל משפט. דווח על שתי הדמויות הבינוניות שאתה רואה",
    ar: "أنت على وشك بدء كتلة من تجارب xxx. سيظهر مجال الرسائل في كل تجربة. أبلغ عن الشخصين الأوسطين اللذين تراهما",
    ur: "آپ xxx ٹرائلز کا ایک بلاک شروع کرنے والے ہیں۔ ہر آزمائش میں خطوط کا ایک فیلڈ ظاہر ہوگا۔ آپ دیکھتے ہیں کہ دو درمیانی کرداروں کی اطلاع دیں",
    hi: "आप xxx परीक्षणों का एक ब्लॉक शुरू करने वाले हैं। प्रत्येक परीक्षण में पत्रों का एक क्षेत्र दिखाई देगा। उन दो मध्य वर्णों की रिपोर्ट करें जिन्हें आप देखते हैं",
    ta: "நீங்கள் xxx சோதனைகளின் ஒரு தொகுதியைத் தொடங்க உள்ளீர்கள். ஒவ்வொரு சோதனையிலும் கடிதங்களின் புலம் தோன்றும். நீங்கள் பார்க்கும் இரண்டு நடுத்தர எழுத்துக்களைப் புகாரளிக்கவும்",
    ml: "നിങ്ങൾ xxx ട്രയലുകളുടെ ഒരു ബ്ലോക്ക് ആരംഭിക്കാൻ പോകുന്നു. ഓരോ വിചാരണയിലും അക്ഷരങ്ങളുടെ ഒരു മേഖല ദൃശ്യമാകും. നിങ്ങൾ കാണുന്ന രണ്ട് മധ്യ പ്രതീകങ്ങൾ റിപ്പോർട്ടുചെയ്യുക",
    te: "మీరు xxx ట్రయల్స్ యొక్క బ్లాక్‌ను ప్రారంభించబోతున్నారు. ప్రతి ట్రయల్‌లో అక్షరాల క్షేత్రం కనిపిస్తుంది. మీరు చూసే రెండు మధ్య అక్షరాలను నివేదించండి",
    kn: "ನೀವು xxx ಪ್ರಯೋಗಗಳ ಬ್ಲಾಕ್ ಅನ್ನು ಪ್ರಾರಂಭಿಸಲಿದ್ದೀರಿ. ಪ್ರತಿ ಪ್ರಯೋಗದಲ್ಲೂ ಅಕ್ಷರಗಳ ಕ್ಷೇತ್ರ ಕಾಣಿಸುತ್ತದೆ. ನೀವು ನೋಡುವ ಎರಡು ಮಧ್ಯದ ಅಕ್ಷರಗಳನ್ನು ವರದಿ ಮಾಡಿ",
    bn: "আপনি xxx ট্রায়ালগুলির একটি ব্লক শুরু করতে চলেছেন। প্রতিটি পরীক্ষায় চিঠির একটি ক্ষেত্র উপস্থিত হবে। আপনি যে দুটি মাঝারি চরিত্র দেখেন তা রিপোর্ট করুন",
    id: "Anda akan memulai blok uji coba xxx. Bidang huruf akan muncul di setiap percobaan. Laporkan dua karakter tengah yang Anda lihat",
    "zh-CN":
      "您即将开始xxx试验。每个试验中将出现一个字母字段。报告您看到的两个中间字符",
    "zh-HK":
      "您即將開始xxx試驗。每個試驗中將出現一個字母字段。報告您看到的兩個中間字符",
    ja: "xxxトライアルのブロックを開始しようとしています。各トライアルに文字のフィールドが表示されます。あなたが見る2つの中間文字を報告してください",
    ko: "xxx 시험 블록을 시작하려고합니다. 각 시험에서 편지 분야가 나타납니다. 당신이 보는 두 개의 중간 문자를보고하십시오",
  },
  T_thresholdSoundBeginBlock: {
    "en-US":
      "SOUND DETECTION. You're about to start a block of many trials. You're trying to detect a particular target sound: one note, repeated 8 times, always the same note. That target is only present on half the trials, randomly. On each trial, your job is to listen and decide whether it was present, yes or no. As you progress from trial to trial, based on your responses, the target sound volume will adapt so that it's barely detectable. In the initial practice block, the target will be alone. In later blocks, the target will be harder to detect because of other sounds. Always ignore the other sounds, and try to report, yes or no, whether the target sound was present. Use the \"y\" key for yes and the \"n\" key for no. \n\nHit RETURN or click PROCEED as soon as you're ready. The PROCEED button will disappear immediately, and the experiment will begin once the sounds are loaded, which takes 40 seconds or longer, depending on the speed of the internet connection. We are working to shorten this delay in a future release.\n\nLoading sounds ...",
    de: 'Schallerkennung. Sie beginnen dabei, einen Block vieler Versuche zu beginnen. Sie versuchen, einen bestimmten Zielgeräusch zu erkennen: eine Note, die 8 Mal wiederholt wurde, immer dieselbe Note. Dieses Ziel ist nur zufällig in der Hälfte der Studien vorhanden. Bei jeder Testversion ist es Ihre Aufgabe, zuzuhören und zu entscheiden, ob es vorhanden war, ja oder nein. Während Sie von der Versuch zu Versuch, basierend auf Ihren Antworten, wird sich das Zielschallvolumen so anpassen, dass es kaum erkennbar ist. Im anfänglichen Übungsblock wird das Ziel allein sein. In späteren Blöcken wird das Ziel aufgrund anderer Geräusche schwerer zu erkennen sein. Ignorieren Sie immer die anderen Geräusche und versuchen Sie, Ja oder Nein zu berichten, ob der Zielgeräusch vorhanden war. Verwenden Sie den Schlüssel "Y" für Yes und den "n" -Staste für Nein.\n\nKlicken Sie auf Rückgabe oder klicken Sie auf Fahren, sobald Sie fertig sind. Die Taste zur Ausführung wird sofort verschwindet, und das Experiment beginnt, sobald die Geräusche geladen sind, was je nach Geschwindigkeit der Internetverbindung 40 Sekunden oder länger dauert. Wir arbeiten daran, diese Verzögerung in einer zukünftigen Veröffentlichung zu verkürzen.\n\nLadegeräusche ...',
    fr: "Détection sonore. Vous êtes sur le point de commencer un bloc de nombreux essais. Vous essayez de détecter un son cible particulier: une note, répétée 8 fois, toujours la même note. Cette cible n'est présente que sur la moitié des essais, au hasard. À chaque essai, votre travail consiste à écouter et à décider s'il était présent, oui ou non. Au fur et à mesure que vous progressez d'un essai à l'autre, en fonction de vos réponses, le volume sonore cible s'adaptera afin qu'il soit à peine détectable. Dans le bloc d'entraînement initial, la cible sera seule. Dans les blocs ultérieurs, la cible sera plus difficile à détecter en raison d'autres sons. Ignorez toujours les autres sons et essayez de signaler, oui ou non, que le son cible était présent. Utilisez la clé \"Y\" pour oui et la clé \"n\" pour non.\n\nAppuyez sur Retour ou cliquez sur Procéder dès que vous êtes prêt. Le bouton Procéder disparaîtra immédiatement et l'expérience commencera une fois les sons chargés, ce qui prend 40 secondes ou plus, selon la vitesse de la connexion Internet. Nous travaillons à raccourcir ce retard dans une version future.\n\nChargement des sons ...",
    es: 'Detección de sonido. Estás a punto de comenzar un bloque de muchas pruebas. Estás tratando de detectar un sonido objetivo en particular: una nota, repetida 8 veces, siempre la misma nota. Ese objetivo solo está presente en la mitad de los ensayos, al azar. En cada prueba, su trabajo es escuchar y decidir si estaba presente, sí o no. A medida que avanza de juicio a prueba, según sus respuestas, el volumen de sonido objetivo se adaptará para que apenas sea detectable. En el bloque de práctica inicial, el objetivo estará solo. En bloques posteriores, el objetivo será más difícil de detectar debido a otros sonidos. Siempre ignore los otros sonidos e intente informar, sí o no, si el sonido objetivo estaba presente. Use la tecla "Y" para sí y la tecla "n" para no.\n\nPresione la devolución o haga clic en continuar tan pronto como esté listo. El botón proceder desaparecerá inmediatamente, y el experimento comenzará una vez que se carguen los sonidos, que tarda 40 segundos o más, dependiendo de la velocidad de la conexión a Internet. Estamos trabajando para acortar este retraso en un lanzamiento futuro.\n\nCargando sonidos ...',
    pt: 'Detecção de som. Você está prestes a iniciar um bloco de muitas tentativas. Você está tentando detectar um som de destino específico: uma nota, repetida 8 vezes, sempre a mesma nota. Esse alvo está presente apenas em metade dos ensaios, aleatoriamente. Em cada julgamento, seu trabalho é ouvir e decidir se estava presente, sim ou não. À medida que você avança de tentativa para tentativa, com base em suas respostas, o volume de som de destino se adaptará para que quase não seja detectável. No bloco de prática inicial, o alvo estará sozinho. Nos blocos posteriores, o alvo será mais difícil de detectar por causa de outros sons. Sempre ignore os outros sons e tente relatar, sim ou não, se o som alvo estava presente. Use a tecla "y" para sim e a tecla "n" para não.\n\nPressione o retorno ou clique em prosseguir assim que estiver pronto. O botão Procena desaparecerá imediatamente e o experimento começará assim que os sons forem carregados, o que leva 40 segundos ou mais, dependendo da velocidade da conexão com a Internet. Estamos trabalhando para reduzir esse atraso em um lançamento futuro.\n\nSons de carregamento ...',
    it: 'Rilevamento del suono. Stai per iniziare un blocco di molte prove. Stai cercando di rilevare un particolare suono target: una nota, ripetuta 8 volte, sempre la stessa nota. Quel bersaglio è presente solo a metà degli studi, in modo casuale. Ad ogni prova, il tuo compito è ascoltare e decidere se era presente, sì o no. Man mano che progredisci da una prova a prova, in base alle tue risposte, il volume del suono target si adatterà in modo che sia a malapena rilevabile. Nel blocco di pratica iniziale, l\'obiettivo sarà solo. Nei blocchi successivi, l\'obiettivo sarà più difficile da rilevare a causa di altri suoni. Ignora sempre gli altri suoni e prova a segnalare, sì o no, se il suono target era presente. Usa il tasto "Y" per sì e il tasto "N" per il no.\n\nPremi reso o fai clic su Procedi non appena sei pronto. Il pulsante di procedimento scomparirà immediatamente e l\'esperimento inizierà una volta caricati i suoni, il che richiede 40 secondi o più, a seconda della velocità della connessione Internet. Stiamo lavorando per abbreviare questo ritardo in una versione futura.\n\nCaricamento dei suoni ...',
    ro: "Detectarea sunetului. Ești pe cale să începi un bloc de multe încercări. Încercați să detectați un anumit sunet țintă: o notă, repetată de 8 ori, întotdeauna aceeași notă. Această țintă este prezentă doar pe jumătate din încercări, la întâmplare. La fiecare proces, meseria dvs. este să ascultați și să decideți dacă a fost prezent, da sau nu. Pe măsură ce progresați de la încercare la încercare, pe baza răspunsurilor dvs., volumul sunetului țintă se va adapta astfel încât să fie abia detectabil. În blocul inițial de practică, ținta va fi singură. În blocurile ulterioare, ținta va fi mai greu de detectat din cauza altor sunete. Ignorați întotdeauna celelalte sunete și încercați să raportați, da sau nu, dacă sunetul țintă a fost prezent. Utilizați cheia „Y” pentru da și cheia „n” pentru nr.\n\nApăsați Return sau faceți clic pe Continuați imediat ce sunteți gata. Butonul continuu va dispărea imediat, iar experimentul va începe odată ce sunetele vor fi încărcate, ceea ce durează 40 de secunde sau mai mult, în funcție de viteza conexiunii la internet. Lucrăm pentru a scurta această întârziere într -o versiune viitoare.\n\nÎncărcarea sunetelor ...",
    pl: "Wykrywanie dźwięku. Zaraz zaczniesz blok wielu prób. Próbujesz wykryć określony dźwięk docelowy: jedna nuta, powtarzana 8 razy, zawsze ta sama nuta. Ten cel jest obecny tylko w połowie prób, losowo. Podczas każdej próby Twoim zadaniem jest wysłuchanie i decydowanie, czy było to obecne, tak czy nie. W miarę postępów z procesu do próby, w oparciu o twoje odpowiedzi, docelowa głośność dźwięku dostosuje się, aby była ledwo wykrywalna. W początkowym bloku ćwiczeń cel będzie sam. W późniejszych blokach cel będzie trudniejszy do wykrycia z powodu innych dźwięków. Zawsze ignoruj ​​inne dźwięki i staraj się zgłosić, tak czy nie, czy docelowy dźwięk był obecny. Użyj klucza „Y” dla tak i klucza „N” dla nie.\n\nNaciśnij zwrot lub kliknij, kontynuuj, gdy tylko będziesz gotowy. Przycisk Przejdź zniknie natychmiast, a eksperyment rozpocznie się po załadowaniu dźwięków, co zajmuje 40 sekund lub dłużej, w zależności od prędkości połączenia internetowego. Pracujemy nad skróceniem tego opóźnienia w przyszłym wydaniu.\n\nŁadowanie dźwięków ...",
    ru: 'Обнаружение звука. Вы собираетесь начать блок многих испытаний. Вы пытаетесь обнаружить конкретный целевой звук: одна нота, повторяющаяся 8 раз, всегда одна и та же нота. Эта цель присутствует только на половине испытаний случайным образом. На каждом испытании ваша работа состоит в том, чтобы слушать и решить, присутствовал ли она, да или нет. По мере продвижения от испытания к испытанию, основываясь на ваших ответах, целевой объем звука будет адаптироваться так, чтобы он едва обнаруживался. В первоначальном практическом блоке цель будет одна. В более поздних блоках цель будет сложнее обнаружить из -за других звуков. Всегда игнорируйте другие звуки и старайтесь сообщить, да или нет, независимо от того, присутствовал ли целевой звук. Используйте ключ «Y» для да и клавишу "N" для NO.\n\nНажмите возврат или нажмите, продолжайте, как только вы будете готовы. Кнопка продолжения сразу исчезнет, ​​и эксперимент начнется после загрузки звуков, что займет 40 секунд или дольше, в зависимости от скорости подключения к Интернету. Мы работаем, чтобы сократить эту задержку в будущем выпуске.\n\nЗвуки загрузки ...',
    hy: "Ձայնի հայտնաբերում: Դուք պատրաստվում եք սկսել բազմաթիվ փորձությունների բլոկ: Դուք փորձում եք հայտնաբերել որոշակի նպատակային ձայն. Մեկ նոտա, կրկնեց 8 անգամ, միշտ նույն նոտան: Այդ թիրախը ներկայումս առկա է միայն դատավարությունների կեսին: Յուրաքանչյուր փորձության վրա ձեր գործն է լսել եւ որոշել, ներկա է, այո, թե ոչ: Երբ դատավարությունից առաջընթաց եք ապրում, ձեր պատասխաններից ելնելով, թիրախային ձայնի ծավալը հարմարվելու է այնպես, որ այն հազիվ թե հայտնաբերելի լինի: Նախնական պրակտիկայում բլոկում թիրախը մենակ կլինի: Հետագա բլոկներում թիրախը դժվար կլինի հայտնաբերել այլ հնչյունների պատճառով: Միշտ անտեսեք մյուս հնչյունները եւ փորձեք զեկուցել, այո կամ ոչ, լինի թիրախ ձայնը: Օգտագործեք «Y» ստեղնը `այո եւ« N »ստեղնը` ոչ:\n\nՀիթ վերադարձեք կամ կտտացրեք Շարունը, հենց որ պատրաստ եք: Շարունակվող կոճակը անմիջապես կվերանա, եւ փորձը կսկսվի այն ժամանակ, երբ հնչյունները բեռնվեն, ինչը տեւում է 40 վայրկյան կամ ավելի երկար, կախված ինտերնետային կապի արագությունից: Մենք աշխատում ենք կարճացնել այս ձգձգումը հետագա թողարկումում:\n\nՁայնի բեռնումը ...",
    fi: 'Äänen havaitseminen. Olet aloittamassa monien kokeiden lohkon. Yrität havaita tietyn kohdeäänen: yksi huomautus, toistetaan 8 kertaa, aina sama huomautus. Tämä tavoite on läsnä vain puolet tutkimuksista, satunnaisesti. Jokaisessa oikeudenkäynnissä sinun tehtäväsi on kuunnella ja päättää, oliko se läsnä, kyllä ​​vai ei. Kun etenet kokeilusta oikeudenkäyntiin vastausten perusteella, kohdeäänen äänenvoimakkuus mukautuu siten, että se on tuskin havaittavissa. Alkuperäisessä harjoittelulohkossa tavoite on yksin. Myöhemmissä lohkoissa kohde on vaikeampi havaita muiden äänien takia. Ohita aina muut äänet ja yritä ilmoittaa, kyllä ​​vai ei, onko kohdeääni läsnä. Käytä "Y" -näppäintä kyllä ​​ja "N" -näppäimelle ei.\n\nPaina palauta tai napsauta Jatka heti kun olet valmis. Jatka -painike katoaa heti, ja koe alkaa, kun äänet on ladattu, mikä vie 40 sekuntia tai pidempään Internet -yhteyden nopeudesta riippuen. Pyrimme lyhentämään tätä viivettä tulevassa julkaisussa.\n\nÄänien lataaminen ...',
    ka: 'ხმის გამოვლენა. თქვენ აპირებთ მრავალი განსაცდელის ბლოკის დაწყებას. თქვენ ცდილობთ გამოავლინოთ კონკრეტული სამიზნე ხმა: ერთი შენიშვნა, განმეორებით 8 ჯერ, ყოველთვის იგივე შენიშვნა. ეს სამიზნე მხოლოდ ტესტების ნახევარზეა წარმოდგენილი, შემთხვევით. თითოეულ სასამართლო პროცესზე, თქვენი საქმეა მოუსმინოთ და გადაწყვიტეთ, იყო თუ არა ეს, დიახ თუ არა. როდესაც თქვენ პროგრესირებთ სასამართლო პროცესზე, თქვენი პასუხების საფუძველზე, სამიზნე ხმის მოცულობა ადაპტირდება ისე, რომ იგი ძლივს გამოვლინდეს. საწყისი პრაქტიკის ბლოკში, სამიზნე მარტო იქნება. მოგვიანებით ბლოკებში, სამიზნე უფრო რთული იქნება სხვა ბგერების გამო. ყოველთვის უგულებელყოფა სხვა ბგერებს და შეეცადეთ შეატყობინოთ, დიახ თუ არა, იყო თუ არა სამიზნე ხმა. გამოიყენეთ "y" გასაღები დიახ და "n" გასაღები არა.\n\nდააჭირეთ დაბრუნებას ან დააჭირეთ გააგრძელეთ, როგორც კი მზად იქნებით. მიმდინარე ღილაკი დაუყოვნებლივ გაქრება, ხოლო ექსპერიმენტი დაიწყება ბგერების დატვირთვის შემდეგ, რაც 40 წამს ან მეტხანს სჭირდება, რაც დამოკიდებულია ინტერნეტ კავშირის სიჩქარეზე. ჩვენ ვმუშაობთ ამ შეფერხების შემცირებისთვის მომავალში.\n\nიტვირთება ხმები ...',
    he: 'איתור קול. אתה עומד להתחיל גוש של ניסויים רבים. אתה מנסה לאתר צליל יעד מסוים: הערה אחת, חוזרת על עצמה 8 פעמים, תמיד אותה תו. יעד זה קיים רק במחצית הניסויים, באופן אקראי. בכל משפט, התפקיד שלך הוא להקשיב ולהחליט אם הוא נוכח, כן או לא. ככל שתתקדם ממשפט למשפט, בהתבסס על התגובות שלך, עוצמת הקול של היעד תסתגל כך שהוא בקושי ניתן לזהות. בבלוק התרגול הראשוני היעד יהיה לבד. בבלוקים מאוחרים יותר, יהיה קשה יותר לזהות את היעד בגלל צלילים אחרים. תמיד התעלם מהצלילים האחרים, ונסה לדווח, כן או לא, האם צליל היעד היה קיים. השתמש במפתח "Y" עבור YES ובמפתח "N" עבור לא.\n\nלחץ על החזרה או לחץ על המשך ברגע שאתה מוכן. כפתור ההמשך ייעלם מייד והניסוי יתחיל ברגע שהצלילים יועברו, שנמשך 40 שניות ומעלה, תלוי במהירות החיבור לאינטרנט. אנו פועלים לקצר את העיכוב הזה במהדורה עתידית.\n\nטוען צלילי ...',
    ar: 'اكتشاف الصوت. أنت على وشك بدء كتلة من العديد من التجارب. أنت تحاول اكتشاف صوت مستهدف معين: ملاحظة واحدة ، تتكرر 8 مرات ، ودائما نفس الملاحظة. هذا الهدف موجود فقط في نصف التجارب ، بشكل عشوائي. في كل تجربة ، تتمثل عملك في الاستماع وتحديد ما إذا كان موجودًا ، نعم أم لا. مع تقدمك من التجربة إلى المحاكمة ، بناءً على ردودك ، سيتكيف حجم الصوت المستهدف بحيث يكون بالكاد يمكن اكتشافه. في كتلة الممارسة الأولية ، سيكون الهدف وحده. في الكتل اللاحقة ، سيكون الهدف أكثر صعوبة بسبب الأصوات الأخرى. تجاهل دائمًا الأصوات الأخرى ، وحاول الإبلاغ ، نعم أو لا ، سواء كان الصوت المستهدف موجودًا. استخدم مفتاح "Y" لنعم ومفتاح "N" لـ NO.\n\nاضغط على العودة أو انقر فوق متابعة بمجرد استعدادك. سيختفي زر المتابعة على الفور ، وستبدأ التجربة بمجرد تحميل الأصوات ، والتي تستغرق 40 ثانية أو أكثر ، اعتمادًا على سرعة اتصال الإنترنت. نحن نعمل على تقصير هذا التأخير في إصدار مستقبلي.\n\nتحميل الأصوات ...',
    ur: 'آواز کا پتہ لگانا۔ آپ بہت ساری آزمائشوں کا ایک بلاک شروع کرنے والے ہیں۔ آپ کسی خاص ہدف کی آواز کا پتہ لگانے کی کوشش کر رہے ہیں: ایک نوٹ ، 8 بار دہرایا جاتا ہے ، ہمیشہ ایک ہی نوٹ۔ یہ ہدف صرف آدھے آزمائشوں پر ہی موجود ہے ، تصادفی طور پر۔ ہر آزمائش پر ، آپ کا کام سننا اور فیصلہ کرنا ہے کہ آیا یہ موجود تھا ، ہاں یا نہیں۔ جب آپ اپنے جوابات کی بنیاد پر مقدمے کی سماعت سے لے کر آزمائش تک ترقی کرتے ہیں تو ، ہدف کی آواز کا حجم موافقت پذیر ہوگا تاکہ یہ بمشکل پتہ لگانے والا ہو۔ ابتدائی پریکٹس بلاک میں ، ہدف تنہا ہوگا۔ بعد کے بلاکس میں ، دیگر آوازوں کی وجہ سے ہدف کا پتہ لگانا مشکل ہوگا۔ ہمیشہ دوسری آوازوں کو نظرانداز کریں ، اور ہاں یا نہیں ، اطلاع دینے کی کوشش کریں کہ آیا ہدف کی آواز موجود تھی یا نہیں۔ ہاں کے لئے "Y" کی کلید اور نہیں کے لئے "N" کلید استعمال کریں۔\n\nواپس ہٹائیں یا تیار ہوتے ہی آگے بڑھنے پر کلک کریں۔ ایڈوانس بٹن فوری طور پر ختم ہوجائے گا ، اور انٹرنیٹ کنکشن کی رفتار پر منحصر ہے کہ آواز بھری ہوئی ایک بار جب 40 سیکنڈ یا اس سے زیادہ وقت لگتا ہے تو تجربہ شروع ہوجائے گا۔ ہم آئندہ کی رہائی میں اس تاخیر کو مختصر کرنے کے لئے کام کر رہے ہیں۔\n\nلوڈنگ آوازیں ...',
    hi: 'ध्वनि का पता लगाना। आप कई परीक्षणों का एक ब्लॉक शुरू करने वाले हैं। आप एक विशेष लक्ष्य ध्वनि का पता लगाने की कोशिश कर रहे हैं: एक नोट, 8 बार दोहराया, हमेशा एक ही नोट। यह लक्ष्य केवल आधे परीक्षणों पर मौजूद है, बेतरतीब ढंग से। प्रत्येक परीक्षण पर, आपका काम यह सुनना और तय करना है कि यह मौजूद था, हाँ या नहीं। जैसा कि आप परीक्षण से परीक्षण तक आगे बढ़ते हैं, आपकी प्रतिक्रियाओं के आधार पर, लक्ष्य ध्वनि की मात्रा अनुकूल होगी ताकि यह मुश्किल से पता लगाने योग्य हो। प्रारंभिक अभ्यास ब्लॉक में, लक्ष्य अकेले होगा। बाद के ब्लॉकों में, अन्य ध्वनियों के कारण लक्ष्य का पता लगाना कठिन होगा। हमेशा अन्य ध्वनियों को अनदेखा करें, और रिपोर्ट करने की कोशिश करें, हाँ या नहीं, क्या लक्ष्य ध्वनि मौजूद थी। हां के लिए "Y" कुंजी और "N" कुंजी के लिए उपयोग करें।\n\nरिटर्न हिट करें या तैयार होते ही आगे बढ़ें। आगे बटन तुरंत गायब हो जाएगा, और ध्वनि लोड होने के बाद प्रयोग शुरू हो जाएगा, जो इंटरनेट कनेक्शन की गति के आधार पर 40 सेकंड या उससे अधिक समय लेता है। हम भविष्य की रिलीज में इस देरी को कम करने के लिए काम कर रहे हैं।\n\nलोड हो रहा है ...',
    ta: 'ஒலி கண்டறிதல். நீங்கள் பல சோதனைகளின் ஒரு தொகுதியைத் தொடங்க உள்ளீர்கள். நீங்கள் ஒரு குறிப்பிட்ட இலக்கு ஒலியைக் கண்டறிய முயற்சிக்கிறீர்கள்: ஒரு குறிப்பு, 8 முறை மீண்டும் மீண்டும், எப்போதும் ஒரே குறிப்பு. அந்த இலக்கு பாதி சோதனைகளில் மட்டுமே உள்ளது, தோராயமாக. ஒவ்வொரு சோதனையிலும், உங்கள் வேலை அது இருக்கிறதா என்று கேட்பது மற்றும் தீர்மானிக்க வேண்டும், ஆம் அல்லது இல்லை. உங்கள் பதில்களின் அடிப்படையில், சோதனையிலிருந்து சோதனைக்கு நீங்கள் முன்னேறும்போது, ​​இலக்கு ஒலி அளவு மாற்றியமைக்கும், இதனால் அது கண்டறிய முடியாதது. ஆரம்ப பயிற்சி தொகுதியில், இலக்கு தனியாக இருக்கும். பிற்கால தொகுதிகளில், மற்ற ஒலிகளின் காரணமாக இலக்கு கண்டறிவது கடினமாக இருக்கும். மற்ற ஒலிகளை எப்போதும் புறக்கணித்து, இலக்கு ஒலி இருக்கிறதா என்று ஆம் அல்லது இல்லை என்று புகாரளிக்க முயற்சி செய்யுங்கள். ஆம் என்பதற்கான "y" விசையையும், இல்லை என்பதற்கு "N" விசையையும் பயன்படுத்தவும்.\n\nதிரும்பவும் அல்லது நீங்கள் தயாரானவுடன் தொடரவும் என்பதைக் கிளிக் செய்யவும். தொடர்ச்சியான பொத்தான் உடனடியாக மறைந்துவிடும், மேலும் ஒலிகள் ஏற்றப்பட்டவுடன் சோதனை தொடங்கும், இது இணைய இணைப்பின் வேகத்தைப் பொறுத்து 40 வினாடிகள் அல்லது அதற்கு மேற்பட்ட நேரம் ஆகும். எதிர்கால வெளியீட்டில் இந்த தாமதத்தை குறைக்க நாங்கள் பணியாற்றி வருகிறோம்.\n\nஒலிகளை ஏற்றுகிறது ...',
    ml: 'ശബ്ദ കണ്ടെത്തൽ. നിങ്ങൾ നിരവധി പരീക്ഷണങ്ങളുടെ ഒരു ബ്ലോക്ക് ആരംഭിക്കാൻ പോകുന്നു. നിങ്ങൾ ഒരു പ്രത്യേക ടാർഗെറ്റ് ശബ്ദം കണ്ടെത്തുന്നതിന് ശ്രമിക്കുക: ഒരു കുറിപ്പ്, 8 തവണ ആവർത്തിച്ചു, എല്ലായ്പ്പോഴും സമാന കുറിപ്പ്. ആ ലക്ഷ്യം പകുതി പരീക്ഷണങ്ങളിൽ മാത്രമേ ക്രമരഹിതമായിട്ടുള്ളൂ. ഓരോ വിചാരണയിലും, അത് നിലവിലുണ്ടോ, അതെ അല്ലെങ്കിൽ ഇല്ല എന്ന് ശ്രദ്ധിക്കുക എന്നതാണ് നിങ്ങളുടെ ജോലി. നിങ്ങളുടെ പ്രതികരണങ്ങളെ അടിസ്ഥാനമാക്കി, ട്രയലിലേക്ക് നിങ്ങൾ ട്രയലിലേക്ക് പുരോഗമിക്കുമ്പോൾ, ടാർഗെറ്റ് ശബ്ദ വോളിയം പൊരുത്തപ്പെടും, അതിനാൽ ഇത് കഷ്ടിച്ച് കണ്ടെത്താനാകും. പ്രാരംഭ പരിശീലനത്തിൽ, ലക്ഷ്യം ഒറ്റയ്ക്ക് ആയിരിക്കും. പിന്നീടുള്ള ബ്ലോക്കുകളിൽ, മറ്റ് ശബ്ദങ്ങൾ കാരണം ടാർഗെറ്റ് കണ്ടെത്തുന്നതിന് ബുദ്ധിമുട്ടായിരിക്കും. എപ്പോഴും മറ്റ് ശബ്ദങ്ങൾ അവഗണിക്കുക, ടാർഗെറ്റ് ശബ്ദം നിലവിലുണ്ടോ എന്ന് റിപ്പോർട്ടുചെയ്യാൻ ശ്രമിക്കുക. അതെ, ഇല്ല എന്നതിനായുള്ള "y" കീ എന്നിവയും ഇല്ല.\n\nമടങ്ങിവരവ് അമർത്തുക അല്ലെങ്കിൽ നിങ്ങൾ തയ്യാറായ ഉടൻ തുടരുക ക്ലിക്കുചെയ്യുക. ആംവർ ബട്ടൺ ഉടനടി അപ്രത്യക്ഷമാകും, മാത്രമല്ല ഇത് ഇന്റർനെറ്റ് കണക്ഷന്റെ വേഗതയെ ആശ്രയിച്ച് 40 സെക്കൻഡോ അതിൽ കൂടുതലോ എടുക്കും. ഭാവിയിലെ ഒരു റിലീസിൽ ഈ കാലതാമസം ചുരുക്കുന്നതിന് ഞങ്ങൾ പ്രവർത്തിക്കുന്നു.\n\nലോഡുചെയ്യുന്നു ...',
    te: 'సౌండ్ డిటెక్షన్. మీరు చాలా ప్రయత్నాల బ్లాక్‌ను ప్రారంభించబోతున్నారు. మీరు ఒక నిర్దిష్ట లక్ష్య ధ్వనిని గుర్తించడానికి ప్రయత్నిస్తున్నారు: ఒక గమనిక, 8 సార్లు పునరావృతం, ఎల్లప్పుడూ ఒకే గమనిక. ఆ లక్ష్యం యాదృచ్ఛికంగా సగం ట్రయల్స్‌లో మాత్రమే ఉంటుంది. ప్రతి ట్రయల్‌లో, మీ పని వినడం మరియు అది హాజరైనదా అని నిర్ణయించుకోవడం, అవును లేదా కాదు. మీరు ట్రయల్ నుండి ట్రయల్ వరకు అభివృద్ధి చెందుతున్నప్పుడు, మీ ప్రతిస్పందనల ఆధారంగా, టార్గెట్ సౌండ్ వాల్యూమ్ అనుగుణంగా ఉంటుంది, తద్వారా ఇది గుర్తించబడదు. ప్రారంభ ప్రాక్టీస్ బ్లాక్‌లో, లక్ష్యం ఒంటరిగా ఉంటుంది. తరువాతి బ్లాకులలో, ఇతర శబ్దాల కారణంగా లక్ష్యం గుర్తించడం కష్టం. ఇతర శబ్దాలను ఎల్లప్పుడూ విస్మరించండి మరియు లక్ష్య శబ్దం ఉందా అని నివేదించడానికి ప్రయత్నించండి. అవును కోసం "Y" కీని మరియు NO కోసం "N" కీని ఉపయోగించండి.\n\nరిటర్న్ నొక్కండి లేదా మీరు సిద్ధంగా ఉన్న వెంటనే కొనసాగండి క్లిక్ చేయండి. ఆదాయ బటన్ వెంటనే అదృశ్యమవుతుంది మరియు శబ్దాలు లోడ్ అయిన తర్వాత ప్రయోగం ప్రారంభమవుతుంది, ఇది ఇంటర్నెట్ కనెక్షన్ వేగాన్ని బట్టి 40 సెకన్లు లేదా అంతకంటే ఎక్కువ సమయం పడుతుంది. భవిష్యత్ విడుదలలో ఈ ఆలస్యాన్ని తగ్గించడానికి మేము కృషి చేస్తున్నాము.\n\nశబ్దాలు లోడ్ అవుతున్నాయి ...',
    kn: 'ಧ್ವನಿ ಪತ್ತೆ. ನೀವು ಅನೇಕ ಪ್ರಯೋಗಗಳ ಬ್ಲಾಕ್ ಅನ್ನು ಪ್ರಾರಂಭಿಸಲಿದ್ದೀರಿ. ನೀವು ನಿರ್ದಿಷ್ಟ ಗುರಿ ಧ್ವನಿಯನ್ನು ಕಂಡುಹಿಡಿಯಲು ಪ್ರಯತ್ನಿಸುತ್ತಿದ್ದೀರಿ: ಒಂದು ಟಿಪ್ಪಣಿ, 8 ಬಾರಿ ಪುನರಾವರ್ತಿಸಲಾಗಿದೆ, ಯಾವಾಗಲೂ ಒಂದೇ ಟಿಪ್ಪಣಿ. ಆ ಗುರಿ ಯಾದೃಚ್ ly ಿಕವಾಗಿ ಅರ್ಧ ಪ್ರಯೋಗಗಳಲ್ಲಿ ಮಾತ್ರ ಇರುತ್ತದೆ. ಪ್ರತಿ ಪ್ರಯೋಗದಲ್ಲೂ, ನಿಮ್ಮ ಕೆಲಸವೆಂದರೆ ಅದು ಇದೆಯೇ ಎಂದು ಆಲಿಸಿ ಮತ್ತು ನಿರ್ಧರಿಸುವುದು, ಹೌದು ಅಥವಾ ಇಲ್ಲ. ನಿಮ್ಮ ಪ್ರತಿಕ್ರಿಯೆಗಳ ಆಧಾರದ ಮೇಲೆ ನೀವು ವಿಚಾರಣೆಯಿಂದ ಪ್ರಯೋಗಕ್ಕೆ ಪ್ರಗತಿಯಲ್ಲಿರುವಾಗ, ಗುರಿ ಧ್ವನಿ ಪರಿಮಾಣವು ಹೊಂದಿಕೊಳ್ಳುತ್ತದೆ ಇದರಿಂದ ಅದು ಪತ್ತೆಹಚ್ಚಲಾಗುವುದಿಲ್ಲ. ಆರಂಭಿಕ ಅಭ್ಯಾಸ ಬ್ಲಾಕ್‌ನಲ್ಲಿ, ಗುರಿ ಏಕಾಂಗಿಯಾಗಿರುತ್ತದೆ. ನಂತರದ ಬ್ಲಾಕ್‌ಗಳಲ್ಲಿ, ಇತರ ಶಬ್ದಗಳ ಕಾರಣದಿಂದಾಗಿ ಗುರಿಯನ್ನು ಕಂಡುಹಿಡಿಯುವುದು ಕಷ್ಟವಾಗುತ್ತದೆ. ಇತರ ಶಬ್ದಗಳನ್ನು ಯಾವಾಗಲೂ ನಿರ್ಲಕ್ಷಿಸಿ, ಮತ್ತು ವರದಿ ಮಾಡಲು ಪ್ರಯತ್ನಿಸಿ, ಹೌದು ಅಥವಾ ಇಲ್ಲ, ಗುರಿ ಧ್ವನಿ ಇದೆಯೇ ಎಂದು. ಹೌದು ಗಾಗಿ "Y" ಕೀಲಿಯನ್ನು ಮತ್ತು "N" ಕೀಲಿಯನ್ನು ಇಲ್ಲ.\n\nರಿಟರ್ನ್ ಒತ್ತಿರಿ ಅಥವಾ ನೀವು ಸಿದ್ಧವಾದ ತಕ್ಷಣ ಮುಂದುವರಿಯಿರಿ ಕ್ಲಿಕ್ ಮಾಡಿ. ಮುಂದುವರಿಯುವ ಬಟನ್ ತಕ್ಷಣವೇ ಕಣ್ಮರೆಯಾಗುತ್ತದೆ, ಮತ್ತು ಶಬ್ದಗಳನ್ನು ಲೋಡ್ ಮಾಡಿದ ನಂತರ ಪ್ರಯೋಗವು ಪ್ರಾರಂಭವಾಗುತ್ತದೆ, ಇದು ಇಂಟರ್ನೆಟ್ ಸಂಪರ್ಕದ ವೇಗವನ್ನು ಅವಲಂಬಿಸಿ 40 ಸೆಕೆಂಡುಗಳು ಅಥವಾ ಅದಕ್ಕಿಂತ ಹೆಚ್ಚು ಸಮಯ ತೆಗೆದುಕೊಳ್ಳುತ್ತದೆ. ಭವಿಷ್ಯದ ಬಿಡುಗಡೆಯಲ್ಲಿ ಈ ವಿಳಂಬವನ್ನು ಕಡಿಮೆ ಮಾಡಲು ನಾವು ಕೆಲಸ ಮಾಡುತ್ತಿದ್ದೇವೆ.\n\nಶಬ್ದಗಳನ್ನು ಲೋಡ್ ಮಾಡಲಾಗುತ್ತಿದೆ ...',
    bn: 'শব্দ সনাক্তকরণ। আপনি অনেক পরীক্ষার একটি ব্লক শুরু করতে চলেছেন। আপনি একটি নির্দিষ্ট টার্গেট সাউন্ড সনাক্ত করার চেষ্টা করছেন: একটি নোট, 8 বার পুনরাবৃত্তি, সর্বদা একই নোট। সেই লক্ষ্যটি কেবল এলোমেলোভাবে অর্ধেক ট্রায়ালগুলিতে উপস্থিত রয়েছে। প্রতিটি পরীক্ষায়, আপনার কাজটি হ\'ল এটি উপস্থিত ছিল কিনা তা শুনতে এবং সিদ্ধান্ত নেওয়া, হ্যাঁ বা না। আপনার প্রতিক্রিয়াগুলির উপর ভিত্তি করে আপনি যখন ট্রায়াল থেকে ট্রায়ালে অগ্রগতি করেন, লক্ষ্য সাউন্ড ভলিউমটি এমনভাবে মানিয়ে নেবে যাতে এটি সবে সনাক্তযোগ্য। প্রাথমিক অনুশীলন ব্লকে, লক্ষ্যটি একা থাকবে। পরবর্তী ব্লকগুলিতে, অন্যান্য শব্দগুলির কারণে লক্ষ্যটি সনাক্ত করা আরও শক্ত হবে। সর্বদা অন্যান্য শব্দগুলি উপেক্ষা করুন এবং হ্যাঁ বা না, লক্ষ্য শব্দটি উপস্থিত ছিল কিনা তা রিপোর্ট করার চেষ্টা করুন। হ্যাঁ এর জন্য "y" কী এবং নং এর জন্য "এন" কী ব্যবহার করুন।\n\nহিট রিটার্ন বা আপনি প্রস্তুত হওয়ার সাথে সাথে এগিয়ে যান ক্লিক করুন। প্রসেস বোতামটি অবিলম্বে অদৃশ্য হয়ে যাবে এবং শব্দগুলি লোড হয়ে গেলে পরীক্ষাটি শুরু হবে, যা ইন্টারনেট সংযোগের গতির উপর নির্ভর করে 40 সেকেন্ড বা তার বেশি সময় নেয়। আমরা ভবিষ্যতের প্রকাশে এই বিলম্বটি সংক্ষিপ্ত করার জন্য কাজ করছি।\n\nলোডিং শব্দ ...',
    id: 'Deteksi suara. Anda akan memulai satu blok dari banyak uji coba. Anda mencoba mendeteksi suara target tertentu: satu catatan, diulangi 8 kali, selalu nada yang sama. Target itu hanya ada pada setengah uji coba, secara acak. Pada setiap percobaan, pekerjaan Anda adalah mendengarkan dan memutuskan apakah itu ada, ya atau tidak. Saat Anda maju dari percobaan ke percobaan, berdasarkan tanggapan Anda, volume suara target akan beradaptasi sehingga hampir tidak terdeteksi. Di blok latihan awal, target akan sendirian. Di blok selanjutnya, target akan lebih sulit dideteksi karena suara lain. Selalu abaikan suara lainnya, dan coba laporkan, ya atau tidak, apakah suara target ada. Gunakan tombol "y" untuk Ya dan tombol "n" untuk tidak.\n\nTekan kembali atau klik lanjutkan segera setelah Anda siap. Tombol Prosed akan segera hilang, dan percobaan akan dimulai setelah suara dimuat, yang membutuhkan waktu 40 detik atau lebih, tergantung pada kecepatan koneksi internet. Kami berupaya mempersingkat keterlambatan ini dalam rilis di masa mendatang.\n\nMemuat suara ...',
    "zh-CN":
      "声音检测。您将开始进行许多试验。您正在尝试检测特定的目标声音：一个音符，重复8次，总是相同的音符。该目标仅在一半的试验中存在。在每个试验中，您的工作都是倾听并决定是否在场，是还是否。随着您从试验的进行，根据您的响应，目标声音量将适应几乎无法检测到。在最初的练习块中，目标将是一个人。在以后的块中，由于其他声音，目标将很难检测到。始终忽略其他声音，并尝试报告，是或否，是否存在目标声音。使用“是”的“ y”键，而“ n”键表示否。\n\n点击返回或单击，请立即开始。继续按钮将立即消失，一旦加载声音，实验将开始，这取决于Internet连接的速度，需要40秒或更长时间。我们正在努力在以后的版本中缩短这一延迟。\n\n加载声音...",
    "zh-HK":
      "聲音檢測。您將開始進行許多試驗。您正在嘗試檢測特定的目標聲音：一個音符，重複8次，總是相同的音符。該目標僅在一半的試驗中存在。在每個試驗中，您的工作都是傾聽並決定是否在場，是還是否。隨著您從試驗的進行，根據您的響應，目標聲音量將適應幾乎無法檢測到。在最初的練習塊中，目標將是一個人。在以後的塊中，由於其他聲音，目標將很難檢測到。始終忽略其他聲音，並嘗試報告，是或否，是否存在目標聲音。使用“是”的“ y”鍵，而“ n”鍵表示否。\n\n點擊返回或單擊，請立即開始。繼續按鈕將立即消失，一旦加載聲音，實驗將開始，這取決於Internet連接的速度，需要40秒或更長時間。我們正在努力在以後的版本中縮短這一延遲。\n\n加載聲音...",
    ja: "音の検出。多くの試験のブロックを開始しようとしています。特定のターゲットサウンドを検出しようとしています。1つの音が8回繰り返され、常に同じメモです。そのターゲットは、ランダムに試行の半分にのみ存在します。各トライアルでは、あなたの仕事は、それが存在するかどうかを聞いて決定することです。応答に基づいて、試用から試用へと進むにつれて、ターゲットの音量が適応して、ほとんど検出できないようにします。最初の練習ブロックでは、ターゲットは単独で行われます。後のブロックでは、他の音のためにターゲットを検出するのが難しくなります。常に他の音を無視し、ターゲットサウンドが存在するかどうかを報告してみてください。はいのために「Y」キーを使用し、「n」キーを使用します。\n\n返品を押すか、準備ができたらすぐにクリックしてください。進行ボタンはすぐに消え、インターネット接続の速度に応じて、音が読み込まれると実験が開始され、40秒以上かかります。将来のリリースでこの遅延を短縮するために取り組んでいます。\n\n音の読み込み...",
    ko: '소리 감지. 많은 시련의 블록을 시작하려고합니다. 특정 대상 사운드를 감지하려고합니다. 하나의 노트, 8 번 반복, 항상 같은 음표입니다. 그 목표는 시험의 절반에만 존재합니다. 각 재판에서 귀하의 임무는 그것이 존재하는지 여부를 듣고 결정하는 것입니다. 응답에 따라 시험에서 시험으로 진행하면서 대상 사운드 볼륨이 거의 감지 할 수 있도록 적응합니다. 초기 연습 블록에서는 대상이 혼자가됩니다. 이후 블록에서는 다른 소리 때문에 대상이 감지하기가 더 어려워집니다. 항상 다른 소리를 무시하고 대상 사운드가 있는지 여부를보고하십시오. 예에 "y"키를 사용하고 "n"키를 사용하십시오.\n\n준비 되 자마자 반품을 누르거나 클릭하십시오. 진행 버튼이 즉시 사라지고 사운드가로드되면 실험이 시작되며 인터넷 연결 속도에 따라 40 초 이상이 걸립니다. 우리는 향후 릴리스 에서이 지연을 단축하기 위해 노력하고 있습니다.\n\n소리로드 ...',
  },
  T_thresholdSoundCheck: {
    "en-US":
      "Hello. Please make sure this computer's sound is working by clicking the Beep button. You'll hear a beep like that every time your answer is right. ",
    de: "Hallo. Bitte vergewissern Sie sich, dass der Sound dieses Computers funktioniert, indem Sie auf die Schaltfläche Piepton klicken. Übrigens, Sie werden jedes Mal einen solchen Piepton hören, wenn Sie eine Testversionen richtig machen.",
    fr: "Bonjour. Veuillez vous assurer que le son de cet ordinateur est activé en cliquant sur le bouton Bip. Au fait, vous entendrez un bip comme celui-ci à chaque fois que vous réussirez un essai.",
    es: "Hola. Asegúrate de que el sonido de esta computadora funcione haciendo clic en el botón Bip. Después, oirás un sonido como ese cada vez que realices un ensayo correctamente.",
    pt: "Olá. Verifique se o som deste computador funciona clicando no botão BIP. Então, você ouvirá um som assim toda vez que executa um ensaio corretamente.",
    it: "Ciao. Assicurati che il suono di questo computer funzioni facendo clic sul pulsante segnale acustico. Sentirai un segnale acustico così ogni volta che la tua risposta è giusta.",
    ro: "Buna ziua. Asigurați -vă că sunetul acestui computer funcționează făcând clic pe butonul BIP. Apoi, veți auzi un sunet de genul acesta de fiecare dată când efectuați corect un eseu.",
    pl: "Cześć. Upewnij się, że dźwięk tego komputera działa, klikając przycisk Sygnał dzwiękowy. Usłyszysz taki sygnał dźwiękowy za każdym razem, gdy poprawnie odpowiesz.\n \n\n",
    ru: "Привет. Убедитесь, что на этом компьютере включен звук, нажав кнопку «Звуковой сигнал». Между прочим, вы будете слышать такой звуковой сигнал каждый раз, когда пройдете пробную версию.",
    hy: "Բարեւ Ձեզ. Համոզվեք, որ ձայնը միացված է այս համակարգչում `կտտացնելով« Ձայն ազդանշանը »կոճակը: Ի դեպ, դուք կտեսնեք այդպիսի ձայնային ազդանշան ամեն անգամ, երբ փորձարկման տարբերակը անցնեք:",
    fi: "Hei. Varmista, että tämän tietokoneen ääni toimii napsauttamalla piippauspainiketta. Kuulet sellaisen piippauksen joka kerta, kun vastauksesi on oikea.",
    ka: "გამარჯობა. გთხოვთ, დარწმუნდეთ, რომ ამ კომპიუტერის ხმა მუშაობს BEEP ღილაკის დაჭერით. თქვენ მოისმენთ beep- ს, როგორც ყოველთვის, როდესაც თქვენი პასუხი სწორია.",
    he: "שלום. אנא ודא כי צליל המחשב פועל על ידי לחיצה על כפתור ביפ. אגב, אתה תשמע צפצוף כזה בכל פעם שתקבל ניסיון נכון.",
    ar: "مرحبا. يرجى التأكد من تمكين صوت هذا الكمبيوتر بالضغط على زر التزمير (Beep). بالمناسبة ، ستسمع صوتًا كهذا في كل مرة تحصل فيها على تجربة صحيحة.",
    ur: "سلام براہ کرم بیپ بٹن پر کلک کریں ، براہ کرم یقینی بنائیں کہ اس کمپیوٹر کی آواز کام کر رہی ہے۔ ویسے ، جب بھی آپ کی ٹری درست تھی ، آپ کو ایسی قسم کی بیپ سنی جائے گی۔",
    hi: "नमस्ते। बीप बटन पर क्लिक करके कृपया सुनिश्चित करें कि इस कंप्यूटर की ध्वनि काम कर रही है। वैसे, जब भी आपका ट्राइअल सही होगा, आपको ईस तरह की एक बीप सुनाई देगी।",
    ta: "வணக்கம். பீப் பொத்தானைக் கிளிக் செய்வதன் மூலம் இந்த கணினியின் ஒலி செயல்படுகிறது என்பதை உறுதிப்படுத்தவும். ஒவ்வொரு முறையும் உங்கள் பதில் சரியாக இருக்கும் ஒரு பீப்பைக் கேட்பீர்கள்.",
    ml: "ഹലോ. ബീപ്പ് ബട്ടൺ ക്ലിക്കുചെയ്തുകൊണ്ട് ഈ കമ്പ്യൂട്ടറിന്റെ ശബ്ദം പ്രവർത്തിക്കുന്നുവെന്ന് ദയവായി ഉറപ്പാക്കുക. നിങ്ങളുടെ ഉത്തരം ശരിയാണെല്ലാം നിങ്ങൾ ഒരു ബീപ്പ് കേൾക്കും.",
    te: "హలో. దయచేసి బీప్ బటన్‌ను క్లిక్ చేయడం ద్వారా ఈ కంప్యూటర్ శబ్దం పనిచేస్తుందని నిర్ధారించుకోండి. మీ సమాధానం సరైన ప్రతిసారీ మీరు అలాంటి బీప్ వింటారు.",
    kn: "ಹಲೋ. ಬೀಪ್ ಬಟನ್ ಕ್ಲಿಕ್ ಮಾಡುವ ಮೂಲಕ ಈ ಕಂಪ್ಯೂಟರ್‌ನ ಧ್ವನಿ ಕಾರ್ಯನಿರ್ವಹಿಸುತ್ತಿದೆ ಎಂದು ದಯವಿಟ್ಟು ಖಚಿತಪಡಿಸಿಕೊಳ್ಳಿ. ನಿಮ್ಮ ಉತ್ತರವು ಸರಿಯಾಗಿರುವಾಗಲೆಲ್ಲಾ ನೀವು ಅಂತಹ ಬೀಪ್ ಅನ್ನು ಕೇಳುತ್ತೀರಿ.",
    bn: "হ্যালো. দয়া করে নিশ্চিত করুন যে এই কম্পিউটারের শব্দটি বীপ বোতামটি ক্লিক করে কাজ করছে। আপনি প্রতিবার আপনার উত্তরটি ঠিক থাকাকালীন এমন একটি বীপ শুনতে পাবেন।",
    id: "Halo. Pastikan suara komputer ini berfungsi dengan mengklik tombol bip. Anda akan mendengar bunyi bip seperti itu setiap kali jawaban Anda benar.",
    "zh-CN":
      "您好。请点击“哔”以确认此电脑的声音播放功能没有问题。顺带一提，当您每正确完成一次试验时，您将听到相同的“哔”声。",
    "zh-HK":
      "您好。請點擊“嗶”以確認此電腦的聲音播放功能沒有問題。順帶一提，當您每正確完成一次試驗時，您將聽到相同的“嗶”聲。",
    ja: "こんにちは。ピーボタンをクリックしてこのコンピュータの音が機能していることを確認してください。ちなみに、テストでは正解する度にこの音が鳴ります。",
    ko: "안녕하세요. 경고음 버튼을 클릭 하여이 컴퓨터의 사운드가 작동하는지 확인하십시오. 당신은 당신의 대답이 옳을 때마다 그런 경고음을들을 것입니다.",
  },
  T_thresholdSoundNewTrial: {
    "en-US": "Hit the SPACE bar to hear the next trial.",
    de: "Schlagen Sie die Space Bar, um den nächsten Versuch zu hören.",
    fr: "Frappez la barre spatiale pour entendre le prochain essai.",
    es: "Golpea la barra espacial para escuchar la próxima prueba.",
    pt: "Bata na barra de espaço para ouvir o próximo julgamento.",
    it: "Colpisci la barra spaziale per ascoltare la prova successiva.",
    ro: "Apăsați bara spațială pentru a auzi următorul proces.",
    pl: "Uderz w bar kosmiczny, aby usłyszeć następny proces.",
    ru: "Ударьте космическую планку, чтобы услышать следующее испытание.",
    hy: "Հարվածեք տիեզերական սանդղակը `հաջորդ դատական ​​նիստը լսելու համար:",
    fi: "Osuta avaruuspalkki kuullaksesi seuraavan oikeudenkäynnin.",
    ka: "მოხვდით კოსმოსურ ზოლში, რომ მოისმინოთ შემდეგი სასამართლო პროცესი.",
    he: "פגע בסרגל החלל כדי לשמוע את המשפט הבא.",
    ar: "اضغط على شريط الفضاء لسماع المحاكمة التالية.",
    ur: "اگلی آزمائش سننے کے لئے خلائی بار کو مارو۔",
    hi: "अगला परीक्षण सुनने के लिए स्पेस बार को मारो।",
    ta: "அடுத்த சோதனையைக் கேட்க விண்வெளி பட்டியில் அடியுங்கள்.",
    ml: "അടുത്ത ട്രയൽ കേൾക്കാൻ ബഹിരാകാശ ബാർ അമർത്തുക.",
    te: "తదుపరి ట్రయల్ వినడానికి స్పేస్ బార్‌ను నొక్కండి.",
    kn: "ಮುಂದಿನ ಪ್ರಯೋಗವನ್ನು ಕೇಳಲು ಬಾಹ್ಯಾಕಾಶ ಪಟ್ಟಿಯನ್ನು ಹೊಡೆಯಿರಿ.",
    bn: "পরবর্তী ট্রায়াল শুনতে স্পেস বারে আঘাত করুন।",
    id: "Tekan Space Bar untuk mendengar persidangan berikutnya.",
    "zh-CN": "登上太空酒吧，听到下一个审判。",
    "zh-HK": "登上太空酒吧，聽到下一個審判。",
    ja: "次のトライアルを聞くためにスペースバーを押してください。",
    ko: "다음 시험을 들으려면 우주 막대를 치십시오.",
  },
  T_thresholdSoundResponse: {
    "en-US": "Was the target sound present? Press y or n.",
    de: "War der Zielklang vorhanden? Drücken Sie y oder n.",
    fr: "Le son cible était-il présent? Appuyez sur y ou n.",
    es: "¿Estaba presente el sonido objetivo? Presione y o n.",
    pt: "O som alvo estava presente? Pressione y ou n.",
    it: "Il suono target era presente? Premere y o n.",
    ro: "Sunetul țintă a fost prezent? Apăsați y sau n.",
    pl: "Czy docelowy dźwięk był obecny? Naciśnij y lub n.",
    ru: "Был ли представлен целевой звук? Нажмите Y или N.",
    hy: "Նպատակը ներկա էր: Սեղմեք Y կամ N.",
    fi: "Oliko kohdeääni läsnä? Paina y tai n.",
    ka: "იყო სამიზნე ხმა? დააჭირეთ y ან n.",
    he: "האם צליל היעד היה קיים? לחץ על y או n.",
    ar: "هل كان الصوت المستهدف موجودًا؟ اضغط y أو n.",
    ur: "کیا ہدف کی آواز موجود تھی؟ Y یا N دبائیں۔",
    hi: "क्या लक्ष्य ध्वनि मौजूद थी? Y या n दबाएं।",
    ta: "இலக்கு ஒலி இருந்ததா? Y அல்லது N ஐ அழுத்தவும்.",
    ml: "ടാർഗെറ്റ് സൗണ്ട് നിലവിലുണ്ടോ? Y അല്ലെങ്കിൽ n അമർത്തുക.",
    te: "లక్ష్య ధ్వని ఉందా? Y లేదా n నొక్కండి.",
    kn: "ಗುರಿ ಧ್ವನಿ ಪ್ರಸ್ತುತವಾಗಿದೆಯೇ? Y ಅಥವಾ n ಒತ್ತಿರಿ.",
    bn: "লক্ষ্য শব্দ উপস্থিত ছিল? Y বা n টিপুন।",
    id: "Apakah suara target hadir? Tekan y atau n.",
    "zh-CN": "目标声音存在吗？按y或n。",
    "zh-HK": "目標聲音存在嗎？按y或n。",
    ja: "ターゲットサウンドは存在していましたか？ yまたはnを押します。",
    ko: "대상 사운드가 존재 했습니까? y 또는 n을 누릅니다.",
  },
  T_thresholdTitle: {
    "en-US": "Welcome to EasyEyes Threshold.",
    de: "Willkommen bei Easyeyes Threshold.",
    fr: "Bienvenue dans le seuil EasyEyes.",
    es: "Bienvenido a EasyEyes Threshold.",
    pt: "Bem -vindo ao limite easyeyes.",
    it: "Benvenuto in EasyEyes Threshold.",
    ro: "Bine ați venit la Pragul EasyEyes.",
    pl: "Witamy w Easyeyes treshold. ",
    ru: "Добро пожаловать в порог Easyeyes.",
    hy: "Բարի գալուստ EasyEyes- ի շեմը:",
    fi: "Tervetuloa Easyeyes -kynnysarvoon.",
    ka: "კეთილი იყოს თქვენი მობრძანება Easyeyes- ის ზღურბლზე.",
    he: "ברוך הבא לסף EasyEyes.",
    ar: "مرحبا بك في EasyEyes Threshold.",
    ur: "ایزیئس دہلیز میں خوش آمدید۔\n",
    hi: "EasyEyes थ्रेसहोल्ड में आपका स्वागत है।\n",
    ta: "எளிதான வாசலுக்கு வருக.",
    ml: "എളുപ്പമുള്ള പരിധിയിലേക്ക് സ്വാഗതം.",
    te: "ఈజీయెస్ థ్రెషోల్డ్‌కు స్వాగతం.",
    kn: "ಈಸಿಐಇಗಳ ಮಿತಿಗೆ ಸುಸ್ವಾಗತ.",
    bn: "ইজিয়েস থ্রেশহোল্ডে আপনাকে স্বাগতম।",
    id: "Selamat datang di Ambang Eyeseyes.",
    "zh-CN": "欢迎使用 EasyEyes Threshold。",
    "zh-HK": "歡迎使用 EasyEyes Threshold。",
    ja: "EasyEyes Thresholdへようこそ。",
    ko: "Easyeyes 임계 값에 오신 것을 환영합니다.",
  },
  T_whyClick: {
    "en-US":
      "In this block we'll use clicks, instead of the keyboard, because clicking gets your eye to the right spot at the right time.",
    de: "In diesem Block verwenden wir Klicks anstelle der Tastatur, da das Klicken zum richtigen Zeitpunkt zum richtigen Ort zum richtigen Ort ist.",
    fr: "Dans ce bloc, nous utiliserons des clics, au lieu du clavier, car en cliquant sur votre œil à l'endroit juste au bon moment.",
    es: "En este bloque, usaremos clics, en lugar del teclado, porque hacer clic pone tu ojo en el lugar justo en el momento justo.",
    pt: "Neste bloco, usaremos cliques, em vez do teclado, porque clicando em seu olho no local na hora certa.",
    it: "In questo blocco utilizzeremo i clic, anziché la tastiera, perché i clic portano gli occhi al punto giusto al momento giusto.",
    ro: "În acest bloc, vom folosi clicuri, în loc de tastatură, deoarece faceți clic pe ochiul dvs. în loc chiar la momentul potrivit.",
    pl: "W tym bloku użyjemy kliknięć, zamiast klawiatury, ponieważ kliknięcie przychodzi do właściwego miejsca we właściwym czasie.",
    ru: "В этом блоке мы будем использовать нажатие мыши, а не клавиатуру, потому что нажатие мыши направляет ваш взгляд на правильное место в правильное время.",
    hy: "Այս բլոկում մենք կօգտագործենք կտտոցներ, ստեղնաշարի փոխարեն, քանի որ կտտացնելով ճիշտ ժամանակին ձեր աչքը ճիշտ տեղում է ստանում:",
    fi: "Tässä lohkossa käytämme napsautuksia näppäimistön sijasta, koska napsauttaminen saa silmäsi oikeaan kohtaan oikeaan aikaan.",
    ka: "ამ ბლოკში ჩვენ გამოვიყენებთ დაწკაპუნებებს, კლავიშის ნაცვლად, რადგან დაჭერით თქვენს თვალს სწორ დროს მიჰყვება.",
    he: "בחסימה זו נשתמש בקליקים, במקום במקלדת, מכיוון שלחיצה מקבלת את העין שלך למקום הנכון בזמן הנכון.",
    ar: "في هذه الكتلة ، سنستخدم النقرات ، بدلاً من لوحة المفاتيح ، لأن النقر فوق عينيك إلى المكان المناسب في الوقت المناسب.",
    ur: "اس بلاک میں ہم کی بورڈ کے بجائے کلکس کا استعمال کریں گے ، کیونکہ کلک کرکے ، آپ کی آنکھیں صحیح وقت پر صحیح جگہ پر پہنچ جاتی ہیں۔",
    hi: "इस ब्लॉक में हम कीबोर्ड के बजाय क्लिकों का उपयोग करेंगे, क्योंकि क्लिक करने से आपकी आंखें सही समय पर सही स्थान पर पहुंच जाती हैं।",
    ta: "இந்த தொகுதியில் விசைப்பலகைக்கு பதிலாக கிளிக்குகளைப் பயன்படுத்துவோம், ஏனென்றால் கிளிக் செய்வதைக் கிளிக் செய்வது சரியான நேரத்தில் சரியான இடத்திற்கு உங்கள் கண்ணைப் பெறுகிறது.",
    ml: "ഈ ബ്ലോക്കിൽ ഞങ്ങൾ കീബോർഡിന് പകരം ക്ലിക്കുകൾ ഉപയോഗിക്കും, കാരണം ക്ലിക്കുചെയ്യുന്നത് ശരിയായ സമയത്ത് ശരിയായ സ്ഥലത്ത് തന്നെ ലഭിക്കും.",
    te: "ఈ బ్లాక్‌లో మేము కీబోర్డ్‌కు బదులుగా క్లిక్‌లను ఉపయోగిస్తాము, ఎందుకంటే క్లిక్ చేయడం సరైన సమయంలో సరైన ప్రదేశానికి మీ కన్ను పొందుతుంది.",
    kn: "ಈ ಬ್ಲಾಕ್‌ನಲ್ಲಿ ನಾವು ಕೀಬೋರ್ಡ್ ಬದಲಿಗೆ ಕ್ಲಿಕ್‌ಗಳನ್ನು ಬಳಸುತ್ತೇವೆ, ಏಕೆಂದರೆ ಕ್ಲಿಕ್ ಮಾಡುವುದರಿಂದ ಸರಿಯಾದ ಸಮಯದಲ್ಲಿ ನಿಮ್ಮ ಕಣ್ಣನ್ನು ಸರಿಯಾದ ಸ್ಥಳಕ್ಕೆ ಬರುತ್ತದೆ.",
    bn: "এই ব্লকে আমরা কীবোর্ডের পরিবর্তে ক্লিকগুলি ব্যবহার করব, কারণ ক্লিক করা আপনার চোখটি সঠিক সময়ে সঠিক জায়গায় পেয়েছে।",
    id: "Di blok ini kami akan menggunakan klik, alih -alih keyboard, karena mengklik mengarahkan Anda ke tempat yang tepat pada waktu yang tepat.",
    "zh-CN":
      "在此区组中请使用鼠标而非键盘，因为点击鼠标可以让您在合适的时机看合适的位置。",
    "zh-HK":
      "在此區組中請使用鼠標而非鍵盤，因為點擊鼠標可以讓您在合適的時機看合適的位置。",
    ja: "このブロックでは、キーボードの代わりにクリックを使用します。クリックが適切なタイミングで適切な場所に目を向けるためです。",
    ko: "이 블록에서는 키보드 대신 클릭을 사용합니다.",
  },
};
