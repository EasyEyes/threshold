...
export const phrases = {
  ...
  RC_gazeTrackingIntro: {
    ...
    ja: "「Catch the Cross！」を再生しますこれで、目の位置を監視するために、視線追跡を設定します。ビデオフィードに顔を入れてください（以下を参照）。どこにいても、交差毛の中心をクリックします。クリックしたときに目がそれにあることを確認してください。クリックすると、毛が新しい場所に移動します。キャリブレーションが完了するまで、新しい場所ごとにクリックします。それはゲームです。十字架をキャッチ！",
  ...
  RC_gazeTrackingNudge: {
    ...
    "zh-HK":
      "抓住十字架！我們需要你的幫助。眼神跟踪器需要更多的培訓。如箭頭所示，當您單擊時，跟踪器認為您的目光遠非叉毛。請點擊每個新位置的雜毛。遊戲而已。抓住十字架！",
  ...
  RC_nearPointIntro: {
    ...
    "zh-HK":
      "我們將測量您的眼睛之間的距離。使用視頻圖像觀看您的眼睛。在眼前，水平握住筆在您的臉上。將您的左右拇指指甲與左右學生的中心對齊。然後將筆帶到屏幕上的統治者，以測量拇指之間的分離。用零左右拇指，並註意右拇指在哪裡。放下筆，然後單擊標尺。最後，單擊確定繼續。",
  ...
  T_sentenceGuessingGame: {
    ...
    "zh-HK":
      "該句子的可理解性將從輕鬆到硬或不可能不等。您不能獲得超過三分之二的正確，所以放鬆。將其視為一個猜測遊戲，只需獲得盡可能多的遊戲。每當您正確時，您都會聽到嗶嗶聲。",
  ...
  T_sentenceProcedure: {
    ...
    it: "Ad ogni prova, sentirai una frase, di solito nel rumore. Quindi vedrai un elenco di tutte le possibilità per ogni parola nella frase. Segnala la frase o la tua ipotesi migliore, facendo clic sulle parole che hai sentito.",
