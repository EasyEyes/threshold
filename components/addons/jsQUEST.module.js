...
function interp1(xs, vs, xqs, method) {
  ...
  var zipped = xs.map(function (x, index) {
    return [x, vs[index]];
  });
  ...
  var yqs = xqs.map(function (xq) {
    ...
    if (index === -1) {
      throw new Error(
        "Query value " +
          xq +
          " lies outside of range. Extrapolation is not\n        supported."
      );
    }
    /* Interpolate value. */
...
function QuestRecompute(q, plotIt, chart_width, chart_height) {
  ...
  if (min_of_array(q.p2) > q.pThreshold || max_of_array(q.p2) < q.pThreshold) {
    alert(
      `psychometric function range [${min_of_array(q.p2)} ${max_of_array(
        q.p2
      )}] omits ${q.pThreshold} threshold`
    );
  }
  ...
  if (index.length < 2) {
    alert(
      `psychometric function has only ${index.length} strictly monotonic point(s)`
    );
  }
  ...
  for (let k = 0; k < q.trialCount; k++) {
    ...
    q.pdf = numeric.mul(
      q.pdf,
      get_array_using_index(q.s2[q.response[k]], ii_start_at_zero)
    );
...
function QuestQuantile(q, quantileOrder) {
  ...
  if (typeof q === "undefined")
    alert("Usage: intensity=QuestQuantile(q,[quantileOrder])");
  ...
  // end
  if (Array.isArray(q) && q.length > 1) {
    if (typeof quantileOrder !== "undefined")
      alert(
        "Cannot accept quantileOrder for q vector. Set each q.quantileOrder instead."
      );
...
function QuestSimulate(q, tTest, tActual, plotIt, chart_width, chart_height) {
  ...
  if (
    typeof q === "undefined" ||
    typeof tTest === "undefined" ||
    typeof tActual === "undefined"
  ) {
  ...
  // end
  if (Array.isArray(q) && q.length > 1)
    alert("can not deal with q being a vector");
  ...
  if (typeof plotIt !== "undefined" && plotIt > 0) {
    ...
    if (plotIt === 2) {
      ...
      negative = find_less_than_or_equal_to_zero_index(
        q.response.slice(0, q.trialCount)
      );
      pcol = "RGBA(0, 128, 0, 1)";
    } else {
  ...
  if (q.updatePdf) {
    ...
    if (ii[0] < 1 || ii[ii.length - 1] > numeric.dim(q.s2)[1]) {
      if (q.warnPdf) {
        ...
        const high =
          (numeric.dim(q.s2)[1] - q.pdf.length - q.i[q.i.length - 1]) *
            q.grain +
          q.tGuess;

        alert(
          `QuestUpdate: intensity ${intensity} out of range ${low} to ${high}. Pdf will be inexact. Suggest that you increase "range" in call to QuestCreate.`
        );
        // pending
        // oldWarning=warning;
    ...
    q.pdf = numeric.mul(
      q.pdf,
      get_array_using_index(q.s2[response], ii_start_at_zero)
    );
    // if q.normalizePdf
    // 	q.pdf=q.pdf/sum(q.pdf);		% keep the pdf normalized	% 3 ms
    // end
...
function QuestBetaAnalysis1(q) {
  ...
  console.log(
    `Beta mode ${q3[maximum.index].beta} mean ${round2(
      betaMean,
      2
    )} +- ${round2(betaSd, 2)}`
  );
