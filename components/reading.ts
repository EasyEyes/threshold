...
import {
  ...
  preprocessRawCorpus,
} from "./readingTokenizer";
// Re-export for existing consumers (eg imageAndVideoGeneration.js)
export { canonical, preprocessCorpusToWordList, preprocessRawCorpus };
...
export const prepareReadingQuestions = (
  ...
  // Get displayed words
  const displayedCanonicalWords = new Set<string>();
  for (const page of textPages) {
    const pageList = preprocessCorpusToWordList(preprocessRawCorpus(page));
    for (const word of pageList.filter((s) => s.length)) {
      displayedCanonicalWords.add(canonical(word, "displayedCanonicalWords"));
    }
  }
