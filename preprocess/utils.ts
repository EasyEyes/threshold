...
export const getFontNameListBySource = (
  ...
  let defaultFont = window.GLOSSARY["font"].default as string;
  let defaultFontSource = window.GLOSSARY["fontSource"].default as string;
  ...
  if (!foundInstructionFontSourceRow) {
    let defaultValue = window.GLOSSARY["instructionFontSource"].default;
...
export const getColumnValuesOrDefaults = (
  ...
  const rows = df.dim()[0];
  const defaultValue = window.GLOSSARY[columnName].default as unknown as string;
...
export const getTargetSoundListList = (
  ...
  for (let j = 1; j < maxLength; j++) {
    ...
    const conditionTrialsValueDefault = window.GLOSSARY["conditionTrials"]
      .default as string;
